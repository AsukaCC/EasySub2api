<template>
  <span
    class="features-channel-monitor-v2-monitor-rank-badge__text"
    :class="rankClass"
    :title="titleText"
  >
    <svg
      v-if="showTrophy && palette"
      class="features-channel-monitor-v2-monitor-rank-badge__icon rank-trophy"
    ...
    <span v-if="showTrophy" class="features-channel-monitor-v2-monitor-rank-badge__text-2">{{ ariaLabel }}</span>
    <span
      class="features-channel-monitor-v2-monitor-rank-badge__text-3"
      :class="showTrophy ? rankClass : 'features-channel-monitor-v2-monitor-rank-badge__text-4'"
    >
      {{ label }}
    </span>
  </span>
...
const rankClass = computed(() => {
  if (rankNum.value === 1) return 'features-channel-monitor-v2-monitor-rank-badge__state'
  if (rankNum.value === 2) return 'features-channel-monitor-v2-monitor-rank-badge__state-2'
  if (rankNum.value === 3) return 'features-channel-monitor-v2-monitor-rank-badge__state-3'
  return ''
})
</script>
