/**
 * Core Type Definitions for EasySub2api Frontend
 */
...
export type UserAuthProvider = 'email' | 'oidc' | 'wechat' | 'github' | 'google' | 'dingtalk'
...
export interface User {
  ...
  role: 'admin' | 'user' // User role for authorization
	...
	next_bonus_expires_at?: string | null
	next_expiring_bonus_amount?: number
  concurrency: number // Allowed concurrent requests
  rpm_limit?: number // User-level RPM cap (0 = unlimited); effective as fallback when group has no rpm_limit
  status: 'active' | 'disabled' // Account status
  allowed_groups: string[] | null // Allowed group IDs (null = all non-exclusive groups)
...
export interface PublicSettings {
  ...
  /** Legacy compatibility field; passive aggregation is always selected. */
...
export type AnnouncementConditionType = 'subscription' | 'balance' | 'user' | 'level'
...
export interface Group {
  ...
  fallback_group_id_on_invalid_request: string | null
...
export interface ApiKey {
  ...
  quota: number // Platform-point quota (0 = unlimited)
  quota_used: number // Used platform points
...
export interface UpdateApiKeyRequest {
  ...
  ip_whitelist?: string[]
  ip_blacklist?: string[]
  quota?: number // Platform-point quota (null = no change, 0 = unlimited)
...
export interface CreateGroupRequest {
  ...
  weekly_limit_points?: number | null
  monthly_limit_points?: number | null
  /** @deprecated Compatibility aliases; these values are platform points. */
  daily_limit_usd?: number | null
  /** @deprecated Compatibility aliases; these values are platform points. */
  weekly_limit_usd?: number | null
  /** @deprecated Compatibility aliases; these values are platform points. */
...
export interface UpstreamBillingData {
  object: 'easysub2api.key_billing' | 'sub2api.key_billing'
  schema_version: 1
  billing_scope: 'token'
...
export interface Account {
  ...
  proxy_fallback_origin_id?: string | null
  ...
  updated_at: string
  proxy?: Proxy
  group_ids?: string[] // Groups this account belongs to
  ...
  tls_fingerprint_profile_id?: string | null
...
export interface WindowStats {
  ...
  user_cost?: number // User/API key billed platform points
}
...
export type UsageRequestType = 'unknown' | 'sync' | 'stream' | 'ws_v2' | 'cyber' | 'live' | 'test'
...
export interface EndpointStat {
  ...
  requests: number
  total_tokens: number
  cost: number // Standard cost in USD
  actual_cost: number // User/API key billed platform points
  account_cost?: number // Account cost in USD (admin responses)
}
...
export interface AccountUsageHistory {
  ...
  tokens: number
  cost: number // Standard cost in USD
  account_cost: number // Account cost in USD (account multiplier)
  actual_cost: number // User/API key billed platform points (group multiplier)
  user_cost: number // Compatibility alias for actual_cost
}
...
export interface AccountUsageSummary {
  ...
  total_user_cost: number // Platform points billed to users/API keys
  total_standard_cost: number
  total_requests: number
  total_tokens: number
  avg_daily_cost: number // Account cost
  avg_daily_user_cost: number // Average platform points billed per day
