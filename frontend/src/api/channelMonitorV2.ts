...
export type MonitorRange = '90m' | '24h' | '7d' | '14d' | '30d'
...
/** First-upgrade historical fill for 90m/24h/7d/14d/30d; omitted when complete. */
...
export interface MonitorDimensions {
  ...
  groups: Array<{ id: string; name: string; platform?: string; request_count: number }>
...
/** Public compact monitor view. It exposes only the redacted matrix, without requiring JWT. */
export async function getPublicMatrix(filter: MonitorFilter, groupBy: MonitorMatrixGroupBy, signal?: AbortSignal) {
  const { data } = await apiClient.get<MonitorMatrixResponse>('/channel-monitor-v2/public/matrix', requestConfig(filter, signal, { group_by: groupBy }))
  return data
}
