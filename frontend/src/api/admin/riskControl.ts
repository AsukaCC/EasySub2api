...
export interface TestContentModerationAPIKeysPayload {
  ...
  model?: string
  timeout_ms?: number
  // undefined 沿用已保存配置；null 强制直连；UUID 指定代理
  proxy_id?: string | null
  prompt?: string
  images?: string[]
}
...
export interface UpdateContentModerationConfig {
  ...
  base_url?: string
  model?: string
  // undefined 不修改；null 清除（直连）；UUID 指定代理
  proxy_id?: string | null
  api_key?: string
  api_keys?: string[]
...
export async function unbanUser(userID: string): Promise<ContentModerationUnbanUserResponse> {
