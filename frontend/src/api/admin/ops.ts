...
export async function getConcurrencyStats(platform?: string, groupId?: string | null): Promise<OpsConcurrencyStatsResponse> {
...
export async function getAccountAvailabilityStats(platform?: string, groupId?: string | null): Promise<OpsAccountAvailabilityStatsResponse> {
  const params: Record<string, any> = {}
  if (platform) {
    params.platform = platform
  }
...
const OPS_WS_BASE_PROTOCOL = 'easysub2api-admin'
...
export function subscribeQPS(onMessage: (data: any) => void, options: SubscribeQPSOptions = {}): () => void {
  ...
  const connect = () => {
    ...
    // Sec-WebSocket-Protocol (subprotocol list): ["easysub2api-admin", "jwt.<token>"].
...
export async function getErrorLogDetail(id: string): Promise<OpsErrorDetail> {
  const { data } = await apiClient.get<OpsErrorDetail>(`/admin/ops/errors/${id}`)
  return data
}

export async function updateErrorResolved(errorId: string, resolved: boolean): Promise<void> {
...
export async function getRequestErrorDetail(id: string): Promise<OpsErrorDetail> {
  const { data } = await apiClient.get<OpsErrorDetail>(`/admin/ops/request-errors/${id}`)
  return data
}

export async function getUpstreamErrorDetail(id: string): Promise<OpsErrorDetail> {
  const { data } = await apiClient.get<OpsErrorDetail>(`/admin/ops/upstream-errors/${id}`)
  return data
}

export async function updateRequestErrorResolved(errorId: string, resolved: boolean): Promise<void> {
  await apiClient.put(`/admin/ops/request-errors/${errorId}/resolve`, { resolved })
}

export async function updateUpstreamErrorResolved(errorId: string, resolved: boolean): Promise<void> {
