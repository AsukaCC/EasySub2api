...
export interface MonitorQuotaTier {
  ...
  window: string
  /** 同窗口多档时的机器标识（例如 grok requests/tokens） */
  label?: string
  used_percent: number
  used?: number
  limit?: number
 ...
 */
export async function get(id: string): Promise<ChannelMonitor> {
...
function duplicateOperationScope(id: string): DuplicateOperationScope | null {
  ...
  return {
    adminID,
    key: `easysub2api:admin:channel-monitor-duplicate:${adminID}:${id}`,
  }
}
...
export async function duplicate(id: string): Promise<ChannelMonitor> {
 ...
 */
export async function update(id: string, params: UpdateParams): Promise<ChannelMonitor> {
 ...
 */
export async function runNow(id: string): Promise<RunNowResponse> {
