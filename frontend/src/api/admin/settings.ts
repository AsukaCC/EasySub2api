...
export function normalizeDefaultSubscriptionSettings(
  ...
  return subscriptions
    .filter((item) => Boolean(item.group_id) && item.validity_days > 0)
    .map((item) => ({
...
export function buildAuthSourceDefaultsState(
  ...
  return AUTH_SOURCE_TYPES.reduce((acc, source) => {
    ...
    acc[source] = {
	  ...
	  bonus_validity_days: Math.max(0, Number(raw[`auth_source_default_${source}_bonus_validity_days`] ?? 0)),
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
	...
	target[`auth_source_default_${source}_bonus_validity_days`] = Math.max(0, Math.floor(Number(current.bonus_validity_days) || 0));
...
export interface SystemSettings {
  ...
  affiliate_transfer_validity_days: number;
  ...
  auth_source_default_oidc_balance?: number;
	auth_source_default_oidc_bonus_validity_days?: number;
  ...
  auth_source_default_wechat_balance?: number;
	auth_source_default_wechat_bonus_validity_days?: number;
  ...
  auth_source_default_dingtalk_balance?: number;
	auth_source_default_dingtalk_bonus_validity_days?: number;
  ...
  auth_source_default_github_balance?: number;
	auth_source_default_github_bonus_validity_days?: number;
  ...
  auth_source_default_google_balance?: number;
	auth_source_default_google_bonus_validity_days?: number;
  ...
  // ── 平台限额（嵌套 JSON，系统层 + auth-source 层）────────────────────────────────
  ...
  available_channels_user_visible: boolean;
