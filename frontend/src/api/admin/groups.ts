...
/** 获取当前 EasySub2api 服务端的 Live 运行环境能力。 */
 ...
 * Creation uses the collection route; editing uses the entity UUID route.
 */
export async function getModelsListCandidates(
  ...
  const { data } = await apiClient.get<{ models: string[] }>(
    id
      ...
      : '/admin/groups/models-list-candidates',
    {
      params: platform ? { platform } : undefined
    }
  )
...
function duplicateOperationScope(id: string): DuplicateOperationScope | null {
  const adminID = getCurrentAdminID()
  if (!adminID) return null

  return {
    adminID,
    key: `easysub2api:admin:group-duplicate:${adminID}:${id}`
  }
}
...
export async function duplicate(id: string): Promise<AdminGroup> {
 ...
 * @returns Updated group
 */
export async function toggleStatus(id: string, status: 'active' | 'inactive'): Promise<AdminGroup> {
...
export async function listCompositeRoutes(id: string): Promise<CompositeModelRoute[]> {
 ...
 */
export async function getGroupRateMultipliers(id: string): Promise<GroupRateMultiplierEntry[]> {
 ...
 * @returns Success confirmation
 */
export async function clearGroupRateMultipliers(id: string): Promise<{ message: string }> {
...
export async function batchSetGroupRateMultipliers(
  id: string,
  entries: Array<{ user_id: string; rate_multiplier: number }>
): Promise<{ message: string }> {
 ...
 */
export async function getGroupRPMOverrides(id: string): Promise<GroupRPMOverrideEntry[]> {
  const { data } = await apiClient.get<GroupRateMultiplierEntry[]>(
    `/admin/groups/${id}/rate-multipliers`
  )
  return data
...
export async function batchSetGroupRPMOverrides(
  id: string,
  entries: Array<{ user_id: string; rpm_override: number }>
): Promise<{ message: string }> {
  const { data } = await apiClient.put<{ message: string }>(
 ...
 */
export async function clearGroupRPMOverrides(id: string): Promise<{ message: string }> {
...
export async function getUsageSummary(): Promise<
  { group_id: string; today_cost: number; yesterday_cost: number; total_cost: number }[]
> {
  const { data } = await apiClient.get<
    { group_id: string; today_cost: number; yesterday_cost: number; total_cost: number }[]
...
export async function getCapacitySummary(): Promise<
  { group_id: string; concurrency_used: number; concurrency_max: number; sessions_used: number; sessions_max: number; rpm_used: number; rpm_max: number }[]
> {
  const { data } = await apiClient.get<
    { group_id: string; concurrency_used: number; concurrency_max: number; sessions_used: number; sessions_max: number; rpm_used: number; rpm_max: number }[]
