...
interface VertexLocationOption extends Record<string, unknown> {
  value: string
  label: string
}

interface VertexLocationGroup {
  label: string
  options: VertexLocationOption[]
}

export const VERTEX_LOCATION_OPTIONS: VertexLocationGroup[] = [
  {
    label: 'Common',
    options: [
...
export const VERTEX_LOCATION_SELECT_OPTIONS: VertexLocationOption[] =
  VERTEX_LOCATION_OPTIONS.flatMap((group) => group.options)
