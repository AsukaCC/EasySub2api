...
function numberFormatLocale(locale?: string): string {
  const normalized = (locale || getLocale()).toLowerCase()
...
/** Format a platform-point amount with a localized unit label. */
export function formatPoints(amount: number | null | undefined, locale?: string): string {
...
/** Format a base-currency payment amount in Chinese yuan. */
export function formatCNY(amount: number | null | undefined, locale?: string): string {
  return formatKnownCurrency(amount, 'CNY', locale)
}

/** Format an internal/model-pricing amount in US dollars. */
export function formatUSD(amount: number | null | undefined, locale?: string): string {
  return formatKnownCurrency(amount, 'USD', locale)
}
...
export function parseDateTimeLocalInput(value: string): number | null {
  ...
  // datetime-local has no timezone. Parse components explicitly so timezone-bearing
  // values are rejected and Date does not silently normalize invalid calendar dates.
...
export function reasoningEffortValuesEqual(
  ...
  const normalizedLeft = normalizeReasoningEffortKey(left)
  const normalizedRight = normalizeReasoningEffortKey(right)
...
export function formatReasoningEffortMapping(
  ...
  const actualLabel = formatReasoningEffort(actual)
  if (requestedLabel === '-' && actualLabel === '-') return '-'
  if (requestedLabel === '-' || reasoningEffortValuesEqual(requested, actual)) {
    return actualLabel === '-' ? requestedLabel : actualLabel
  }
  if (actualLabel === '-') return requestedLabel
