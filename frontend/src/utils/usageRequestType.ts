...
const VALID_REQUEST_TYPES = new Set<UsageRequestType>(['unknown', 'sync', 'stream', 'ws_v2', 'cyber', 'live', 'test'])
...
export const requestTypeToLegacyStream = (requestType?: UsageRequestType | null): boolean | null | undefined => {
  ...
  if (!requestType || requestType === 'unknown' || requestType === 'cyber' || requestType === 'live' || requestType === 'test') {
    return null
  }
  if (requestType === 'sync') {
    return false
