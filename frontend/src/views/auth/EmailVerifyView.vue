<template>
  <AuthLayout>
    <div class="views-auth-email-verify-view__panel">
      <!-- Title -->
      <div class="views-auth-email-verify-view__panel-2">
        <h2 class="views-auth-email-verify-view__heading">
          {{ t('auth.verifyYourEmail') }}
        </h2>
        <p class="views-auth-email-verify-view__description">
          {{ t('auth.sendCodeDesc') }}
          <span class="views-auth-email-verify-view__text">{{ email }}</span>
        </p>
      </div>
      ...
      <div
        ...
        class="views-auth-email-verify-view__panel-3"
      >
        <div class="views-auth-email-verify-view__panel-4">
          <div class="views-auth-email-verify-view__panel-5">
            <Icon name="exclamationCircle" size="md" class="views-auth-email-verify-view__icon" />
          </div>
          <div class="views-auth-email-verify-view__panel-6">
            <p class="views-auth-email-verify-view__description-2">{{ t('auth.sessionExpired') }}</p>
            <p class="views-auth-email-verify-view__description-3">{{ t('auth.sessionExpiredDesc') }}</p>
          </div>
        </div>
      </div>

      <!-- Verification Form -->
      <form v-else @submit.prevent="handleVerify" class="views-auth-email-verify-view__form">
        <!-- Verification Code Input -->
        <div>
          <label for="code" class="views-auth-email-verify-view__panel-2 input-label">
          ...
          <input
            ...
            maxlength="6"
            :disabled="isLoading"
            class="views-auth-email-verify-view__field input"
            :class="{ 'input-error': errors.code }"
            placeholder="000000"
          />
          <p class="views-auth-email-verify-view__panel-2 input-hint">{{ t('auth.verificationCodeHint') }}</p>
        </div>

        <!-- Code Status -->
        <div
          v-if="codeSent"
          class="views-auth-email-verify-view__panel-7"
        >
          <div class="views-auth-email-verify-view__panel-4">
            <div class="views-auth-email-verify-view__panel-5">
              <Icon name="checkCircle" size="md" class="views-auth-email-verify-view__icon-2" />
            </div>
            <p class="views-auth-email-verify-view__description-4">
        ...
        <div v-if="pendingOAuthCreateCaptchaEnabled" class="views-auth-email-verify-view__panel-8">
          <TurnstileWidget
        ...
        <button
          ...
          class="views-auth-email-verify-view__action btn btn-primary"
        >
          <svg
            v-if="isLoading"
            class="views-auth-email-verify-view__icon-3"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
              class="views-auth-email-verify-view__circle"
              cx="12"
              cy="12"
              r="10"
              stroke="currentColor"
            ...
            ></circle>
            <path
              class="views-auth-email-verify-view__path"
              fill="currentColor"
              d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
            ></path>
          </svg>
          <Icon v-else name="checkCircle" size="md" class="views-auth-email-verify-view__icon-4" />
        ...
        <div class="views-auth-email-verify-view__panel-2">
          <button
            ...
            type="button"
            disabled
            class="views-auth-email-verify-view__action-2"
          >
          ...
          <button
            ...
            "
            class="views-auth-email-verify-view__action-3"
          >
    ...
    <template #footer>
      <button
        @click="handleBack"
        class="views-auth-email-verify-view__action-4"
      >
...
const siteName = ref<string>('EasySub2api')
...
onMounted(async () => {
  ...
  try {
    ...
    siteName.value = settings.site_name || 'EasySub2api'
