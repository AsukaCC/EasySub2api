  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'views-user-stripe-payment-view__component' : ''">
    <div class="views-user-stripe-payment-view__panel" :class="isPopup ? 'views-user-stripe-payment-view__panel-21' : ''">
      ...
      <div v-else-if="initError" class="views-user-stripe-payment-view__panel-4 card">
        ...
        <h3 class="views-user-stripe-payment-view__heading">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="views-user-stripe-payment-view__description">{{ initError }}</p>
        <button class="views-user-stripe-payment-view__action btn btn-primary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="views-user-stripe-payment-view__panel-6 card">
          <div class="views-user-stripe-payment-view__panel-7">
            <p class="views-user-stripe-payment-view__description-2">{{ t('payment.actualPay') }}</p>
            <p class="views-user-stripe-payment-view__description-3">{{ formatGatewayAmount(order.pay_amount) }}</p>
          </div>
        </div>
        ...
        <template v-else-if="stripeSuccess">
          <div class="views-user-stripe-payment-view__panel-15 card">
            <div class="views-user-stripe-payment-view__panel-16">
              ...
              <p class="views-user-stripe-payment-view__description-6">{{ t('payment.result.success') }}</p>
              <p class="views-user-stripe-payment-view__description-5">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="views-user-stripe-payment-view__panel-8 card">
            ...
            <button class="views-user-stripe-payment-view__action-2 btn btn-stripe" :disabled="stripeSubmitting || !stripeReady" @click="handleGenericPay">
              <span v-if="stripeSubmitting" class="views-user-stripe-payment-view__text-2">
                <span class="views-user-stripe-payment-view__text-3"></span>
                {{ t('common.processing') }}
              </span>
              <span v-else>{{ t('payment.stripePay') }}</span>
            </button>
          </div>
          <div class="views-user-stripe-payment-view__panel-19">
            <button class="btn btn-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
          </div>
        </template>

        <!-- 错误状态 -->
        <div v-if="stripeError && !showPaymentElement" class="views-user-stripe-payment-view__panel-20 card">
          <p class="views-user-stripe-payment-view__description-8">{{ stripeError }}</p>
          <button class="views-user-stripe-payment-view__action-3 btn btn-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
        </div>
      </template>
    </div>
  </component>
...
async function confirmAlipay(stripe: Stripe, clientSecret: string, orderId: string) {
