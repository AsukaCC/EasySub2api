  ...
  <div class="views-user-stripe-popup-view__panel">
    ...
    >
      ...
      <div v-if="amount" class="views-user-stripe-popup-view__panel-3">
        <p class="views-user-stripe-popup-view__description" :style="{ color: methodColor }">¥{{ amount }}</p>
        <p v-if="orderId" class="views-user-stripe-popup-view__description-2">
          {{ t('payment.orders.orderId') }}: {{ orderId }}
        </p>
      </div>

      <!-- Error -->
      <div v-if="error" class="views-user-stripe-popup-view__panel-4">
        <div
          class="views-user-stripe-popup-view__panel-5"
        >
          {{ error }}
        </div>
        <button
          class="views-user-stripe-popup-view__action"
      ...
      <!-- Success -->
      <div v-else-if="success" class="views-user-stripe-popup-view__panel-6">
        <div class="views-user-stripe-popup-view__panel-7">✓</div>
        <p class="views-user-stripe-popup-view__description-3">{{ t('payment.result.success') }}</p>
        <button
          class="views-user-stripe-popup-view__action-2"
          :style="{ color: methodColor }"
          @click="closeWindow"
        >
          {{ t('common.close') }}
      ...
      <!-- Loading / Redirecting -->
      <div v-else class="views-user-stripe-popup-view__panel-8">
        <div
          class="views-user-stripe-popup-view__panel-9"
          :style="{ borderColor: methodColor, borderTopColor: 'transparent' }"
        />
        <span class="views-user-stripe-popup-view__text">{{ hint }}</span>
      </div>
    </div>
  </div>
</template>
