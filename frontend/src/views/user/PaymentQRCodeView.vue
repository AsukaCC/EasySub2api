<template>
  <AppLayout>
    <div class="views-user-payment-qrcode-view__panel">
      ...
      <p v-if="qrUrl && !expired && scanHint" class="views-user-payment-qrcode-view__description">
        {{ scanHint }}
      </p>
      <div v-if="expired" class="views-user-payment-qrcode-view__panel-3">
        <p class="views-user-payment-qrcode-view__description-2">{{ t('payment.qr.expired') }}</p>
        <button class="views-user-payment-qrcode-view__action btn btn-primary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <div v-else class="views-user-payment-qrcode-view__panel-3">
        <p class="views-user-payment-qrcode-view__description-3">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="views-user-payment-qrcode-view__description-4">{{ countdownDisplay }}</p>
        <p class="views-user-payment-qrcode-view__description-5">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
      <a v-if="payUrl && !qrUrl && !expired" :href="payUrl" target="_blank" rel="noopener noreferrer"
        class="views-user-payment-qrcode-view__link btn btn-primary">
        {{ t('payment.qr.openPayWindow') }}
      </a>
      <!-- Cancel button -->
      <button v-if="!expired && orderId" class="views-user-payment-qrcode-view__action-2 btn btn-secondary" :disabled="cancelling" @click="handleCancel">
...
onMounted(() => {
  orderId.value = typeof route.query.order_id === 'string' ? route.query.order_id : ''
