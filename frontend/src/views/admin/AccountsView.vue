<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-accounts-view__panel">
          <AccountTableFilters
            ...
            <template #after>
              ...
              <div class="views-admin-accounts-view__panel-2" ref="autoRefreshDropdownRef">
                <button
                  ...
                  "
                  class="views-admin-accounts-view__action btn btn-secondary"
                  :title="t('admin.accounts.autoRefresh')"
                >
                  <Icon name="refresh" size="sm" :class="[autoRefreshEnabled ? 'views-admin-accounts-view__icon-6' : '']" />
                  <span class="views-admin-accounts-view__text">
                    {{
                      autoRefreshEnabled
                ...
                >
                  <div class="views-admin-accounts-view__panel-4">
                    ...
                    <div class="views-admin-accounts-view__panel-5"></div>
                    <button
              ...
              <!-- More Tools Dropdown -->
              <div class="views-admin-accounts-view__panel-2" ref="accountToolsDropdownRef">
                <button
                ...
                >
                  <Icon name="more" size="sm" class="views-admin-accounts-view__icon-2" />
                  <span class="views-admin-accounts-view__text">{{ t('admin.accounts.moreActions') }}</span>
                  <Icon name="chevronDown" size="xs" class="views-admin-accounts-view__icon-3" />
                </button>
                <Teleport to="body">
                  <div
                    ...
                    @click.stop
                  >
                    <div class="views-admin-accounts-view__panel-7" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="views-admin-accounts-view__panel-8">
                        <div class="views-admin-accounts-view__panel-9">
                      ...
                      <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                        ...
                        <span class="views-admin-accounts-view__text-3">
                          {{ selIds.length ? t('admin.accounts.dataExportSelected') : t('admin.accounts.dataExport') }}
                        </span>
                        <span
                          v-if="selIds.length"
                          class="views-admin-accounts-view__text-6"
                        >
                          {{ t('admin.accounts.selectedCount', { count: selIds.length }) }}
                        </span>
                      </button>

                      <div class="views-admin-accounts-view__panel-10"></div>
                      <div class="views-admin-accounts-view__panel-8">
                        <div class="views-admin-accounts-view__panel-9">
                      ...
                      <button class="account-tools-menu-item" @click="openTLSFingerprintProfiles">
                        ...
                        <span class="views-admin-accounts-view__text-3">{{ t('admin.tlsFingerprintProfiles.title') }}</span>
                      </button>
                      <div class="views-admin-accounts-view__panel-10"></div>
                      <div class="views-admin-accounts-view__panel-8">
                        <div class="views-admin-accounts-view__panel-11">
                          <span class="views-admin-accounts-view__panel-9">
                            {{ t('admin.accounts.viewColumns') }}
                          </span>
                          <Icon name="grid" size="sm" class="views-admin-accounts-view__icon-4" />
                        </div>
                      </div>
                      <div class="views-admin-accounts-view__panel-12">
                        <button
        ...
        <div
          ...
          class="views-admin-accounts-view__panel-13"
        >
          <span>{{ t('admin.accounts.listPendingSyncHint') }}</span>
          <button
            class="views-admin-accounts-view__action-4 btn btn-secondary"
      ...
      <template #table>
        ...
        />
        <div ref="accountTableRef" class="views-admin-accounts-view__panel-14">
        <DataTable
          ref="dataTableRef"
          :columns="cols"
          :data="accounts"
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="views-admin-accounts-view__field"
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="views-admin-accounts-view__field-2" />
          </template>
          <template #cell-id="{ value }">
            <span class="views-admin-accounts-view__text-10">#{{ value }}</span>
          </template>
          <template #cell-name="{ row, value }">
            <div class="views-admin-accounts-view__panel-15">
              <HelpTooltip
                v-if="accountHomepageUrl(row)"
                :content="accountHomepageUrl(row)"
                width-class="views-admin-accounts-view__state"
                class="views-admin-accounts-view__help-tooltip"
              >
                <template #trigger>
                  <a
                    ...
                    class="views-admin-accounts-view__link"
                  >
                    {{ value }}
                  </a>
                </template>
              </HelpTooltip>
              <span v-else class="views-admin-accounts-view__text-11">{{ value }}</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="views-admin-accounts-view__text-12"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="views-admin-accounts-view__text-13">{{ value }}</span>
            <span v-else class="views-admin-accounts-view__text-14">-</span>
          </template>
          <template #cell-platform="{ row }">
            <div class="account-platform-cell">
              <PlatformTypeBadge
                :platform="row.platform"
                :type="row.type"
                :show-type="false"
              />
              <span :class="['account-expiry', `account-expiry--${getAccountExpiryState(getAccountDisplayExpiry(row))}`]">
                {{ formatAccountExpiry(getAccountDisplayExpiry(row)) }}
              </span>
              <span v-if="row.auto_pause_on_expired && row.expires_at" class="account-expiry-auto-pause">
                {{ t('admin.accounts.autoPauseOnExpired') }}
              </span>
            </div>
          </template>
          <template #cell-type="{ row }">
            <div class="views-admin-accounts-view__panel-16">
              <div class="views-admin-accounts-view__panel-17">
                <PlatformTypeBadge :platform="row.platform" :type="row.type" :show-platform="false"
              ...
              <div
                ...
                :class="[
                  'views-admin-accounts-view__panel-27',
                  getOpenAICompactMeta(row)?.className
                ]"
                :title="getOpenAICompactTitle(row)"
              >
                <span :class="['views-admin-accounts-view__text-30', getOpenAICompactMeta(row)?.dotClass]" />
          ...
          <template #cell-model_fingerprint="{ row }">
            <ModelFingerprintCell :account="row" @open="handleFingerprint" @update="updateFingerprint" />
          </template>
          ...
          <template #cell-proxy="{ row }">
            <div class="views-admin-accounts-view__panel-20">
              <div v-if="row.proxy" class="views-admin-accounts-view__panel-21">
                <span class="views-admin-accounts-view__text-16">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="views-admin-accounts-view__text-17">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="views-admin-accounts-view__text-14">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="views-admin-accounts-view__panel-22">
                <span class="views-admin-accounts-view__text-18">{{ formatDateTime(row.proxy.expires_at) }}</span>
                <span :class="proxyExpiryBadge(row.proxy)">{{ proxyExpiryText(row.proxy) }}</span>
              </div>
              <div v-if="row.proxy_fallback_origin_id" class="views-admin-accounts-view__panel-23">
                <span class="views-admin-accounts-view__text-19" :title="t('admin.accounts.fallbackActiveTip', { origin: row.proxy_fallback_origin_name })">
                  {{ t('admin.accounts.fallbackActive') }}
                </span>
                <button class="views-admin-accounts-view__action-6" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row }">
            <span class="views-admin-accounts-view__text-20">
              <span>{{ formatMultiplier(row.rate_multiplier ?? 1) }}x</span>
              <span
                v-if="row.extra?.upstream_billing_rate_sync_enabled === true"
                class="views-admin-accounts-view__text-21"
          ...
          <template #header-upstream_billing_rate="{ column }">
            <div class="views-admin-accounts-view__panel-23">
              <span>{{ column.label }}</span>
              <span @click.stop>
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="views-admin-accounts-view__panel-24">
              <div
                v-for="score in getSchedulerScoreRows(row)"
                :key="String(score.group_id)"
                class="views-admin-accounts-view__panel-25"
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="views-admin-accounts-view__text-22">{{ formatSchedulerScoreGroup(score) }}</span>
                <span class="views-admin-accounts-view__text-23">/</span>
                <span>{{ formatSchedulerScore(score.base_score) }}</span>
                <span class="views-admin-accounts-view__text-23">/</span>
                <span class="views-admin-accounts-view__text-24">{{ formatStickySchedulerScore(score) }}</span>
              </div>
            </div>
            <span v-else class="views-admin-accounts-view__text-14">-</span>
          </template>
          <template #cell-last_used_at="{ value }">
            <span class="views-admin-accounts-view__text-25">{{ formatRelativeTime(value) }}</span>
          </template>
          <template #cell-created_at="{ value }">
            <span class="views-admin-accounts-view__text-25">{{ formatDateTime(value) }}</span>
          </template>
          <template #cell-actions="{ row }">
            <div class="views-admin-accounts-view__panel-23">
              <button @click="handleEdit(row)" class="views-admin-accounts-view__action-7">
                <svg class="views-admin-accounts-view__icon-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="views-admin-accounts-view__text-28">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="views-admin-accounts-view__action-8">
                <svg class="views-admin-accounts-view__icon-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="views-admin-accounts-view__text-28">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="views-admin-accounts-view__action-9">
                <svg class="views-admin-accounts-view__icon-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM12.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM18.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" /></svg>
                <span class="views-admin-accounts-view__text-28">{{ t('common.more') }}</span>
              </button>
            </div>
          </template>
        </DataTable>
    ...
    <ModelFingerprintModal :show="!!fingerprintAcc" :account="fingerprintAcc" @close="fingerprintAcc = null" @update="updateFingerprint" />
    <AccountStatsModal :show="showStats" :account="statsAcc" @close="closeStatsModal" />
    <ScheduledTestsPanel :show="showSchedulePanel" :account-id="scheduleAcc?.id ?? null" :model-options="scheduleModelOptions" @close="closeSchedulePanel" />
    <AccountActionMenu :show="menu.show" :account="menu.acc" :anchor-rect="menu.anchorRect" @close="menu.show = false" @test="handleTest" @fingerprint="handleFingerprint" @stats="handleViewStats" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="views-admin-accounts-view__label">
        <input type="checkbox" class="views-admin-accounts-view__field-3" v-model="includeProxyOnExport" />
...
import ModelFingerprintModal from '@/components/admin/account/ModelFingerprintModal.vue'
import ModelFingerprintCell from '@/components/admin/account/ModelFingerprintCell.vue'
...
const fingerprintAcc = ref<Account | null>(null)
const handleFingerprint = (account: Account) => { fingerprintAcc.value = account }
const updateFingerprint = (id: string, snapshot: ModelFingerprintSnapshot | null) => {
  const account = accounts.value.find(item => item.id === id)
  if (account) account.extra = { ...account.extra, model_fingerprint: snapshot }
}
...
const HIDDEN_COLUMNS_CURRENT_VERSION = 'platform-type-split-v1'
...
const usageBatchCache = new Map<string, { data: AccountUsageInfo; ts: number }>()
const USAGE_BATCH_CACHE_TTL = 5 * 60 * 1000
const pendingUsageBatchIds = new Set<string>()
...
const setUsageBatchLoading = (accountID: string, loadingState: boolean) => {
...
const setUsageBatchState = (accountID: string, usage: AccountUsageInfo | null, error: string | null) => {
...
const handleAccountUsageLoaded = (accountID: string, usage: AccountUsageInfo) => {
...
const selectedAllResultIDs = ref<Set<string> | null>(null)
  ...
  switch (state) {
    case 'active':
      return {
        ...
        className: 'views-admin-accounts-view__state-2',
        dotClass: 'views-admin-accounts-view__state-3'
      }
    case 'blocked':
      return {
        label: t('admin.accounts.openai.compactUnsupported'),
        className: 'views-admin-accounts-view__state-4',
        dotClass: 'views-admin-accounts-view__state-5'
      }
    case 'auto':
      return {
        label: t('admin.accounts.openai.compactAuto'),
        className: 'views-admin-accounts-view__state-6',
        dotClass: 'views-admin-accounts-view__state-7'
      }
  }
}
...
const normalizeBulkSchedulableResult = (
  result: {
    ...
    success_ids?: string[]
    failed_ids?: string[]
    results?: Array<{ account_id: string; success: boolean }>
  },
  accountIds: string[]
) => {
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (filters.group) {
    ...
    } else if (!groupIds.includes(filters.group)) {
      return false
    }
  }
...
const patchUpstreamBillingSnapshot = (accountID: string, snapshot: UpstreamBillingProbeSnapshot) => {
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `easysub2api-account-${timestamp}.json`
...
const duplicatingAccountIDs = new Set<string>()
...
type AccountDisplayExpiry = number | string | null | undefined
const getAccountDisplayExpiry = (account: Account): AccountDisplayExpiry => {
  const subscriptionExpiresAt = account.credentials?.subscription_expires_at
    || account.parent_subscription_expires_at
  return typeof subscriptionExpiresAt === 'string' && subscriptionExpiresAt.trim()
    ? subscriptionExpiresAt
    : account.expires_at
}
const parseAccountExpiry = (value: AccountDisplayExpiry) => {
  if (!value) return null
  const parsed = typeof value === 'number' ? dayjs.unix(value) : dayjs(value)
  return parsed.isValid() ? parsed : null
}
const formatExpiresAt = (value: AccountDisplayExpiry) => {
  const parsed = parseAccountExpiry(value)
  if (!parsed) return '-'
  return parsed.format(typeof value === 'string' ? 'YYYY-MM-DD' : 'YYYY-MM-DD HH:mm')
}
type AccountExpiryState = 'none' | 'valid' | 'expiring' | 'expired'
const getAccountExpiryState = (value: AccountDisplayExpiry): AccountExpiryState => {
  if (!value) return 'none'
  const expiresAt = parseAccountExpiry(value)
  if (!expiresAt) return 'none'
  const now = dayjs()
  if (!expiresAt.isAfter(now)) return 'expired'
  if (expiresAt.diff(now, 'day', true) <= 7) return 'expiring'
  return 'valid'
}
const formatAccountExpiry = (value: AccountDisplayExpiry) => {
  return t('admin.accounts.accountExpiresAt', { time: formatExpiresAt(value) })
}
...
.account-expiry,
.account-expiry-auto-pause {
  font-size: var(--font-size-xs);
  line-height: 1.15rem;
}

.account-expiry--none,
.account-expiry--valid,
.account-expiry-auto-pause {
  color: var(--color-text-tertiary);
}

.account-expiry--expiring {
  color: #ca8a04;
  font-weight: 600;
}
...
:global(.dark) .account-expiry--expiring {
  color: #facc15;
}

:global(.dark) .account-expiry--expired {
  color: #f87171;
}
</style>
