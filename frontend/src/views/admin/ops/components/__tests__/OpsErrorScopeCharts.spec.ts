...
vi.mock('@/components/charts/d3/D3DonutChart.vue', async () => {
...
vi.mock('@/components/charts/d3/D3LineChart.vue', async () => {
  const { defineComponent } = await import('vue')
...
describe('Ops SLA-scoped error charts', () => {
  it('错误分布图按 SLA 错误数统计，不把业务限制错误算进请求错误分布', () => {
    ...
    const doughnut = wrapper.findComponent({ name: 'D3DonutChart' })
  ...
  it('错误分布图在只有业务限制错误时显示为空态', () => {
    ...
    expect(wrapper.findComponent({ name: 'D3DonutChart' }).exists()).toBe(false)
