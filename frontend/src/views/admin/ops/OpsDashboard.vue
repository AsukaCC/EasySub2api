<template>
  <component :is="isFullscreen ? 'div' : AppLayout" :class="isFullscreen ? 'views-admin-ops-ops-dashboard__component' : ''">
    <div :class="[isFullscreen ? 'views-admin-ops-ops-dashboard__panel-9' : '', 'space-y-6 pb-12']">
      <div
        v-if="errorMessage"
        class="views-admin-ops-ops-dashboard__panel"
      >
        {{ errorMessage }}
      </div>
      ...
      <nav
        v-if="!isFullscreen"
        class="views-admin-ops-ops-dashboard__navigation"
        role="tablist"
        :aria-label="t('admin.ops.moduleTabs.ariaLabel')"
      >
        <div class="views-admin-ops-ops-dashboard__panel-2">
          <button
            ...
            :class="[
              'views-admin-ops-ops-dashboard__action',
              activeTab === tab.value
                ? 'views-admin-ops-ops-dashboard__action-2'
                : 'views-admin-ops-ops-dashboard__action-3'
            ]"
      ...
      <OpsDashboardHeader
        ...
        :compact="!isFullscreen && activeTab !== 'overview'"
        ...
        @open-alert-rules="selectTab('alerts')"
        @enter-fullscreen="enterFullscreen"
        @exit-fullscreen="exitFullscreen"
      />

      <template v-if="opsEnabled && !(loading && !hasLoadedOnce)">
        <!-- Main monitoring overview -->
        <div v-if="activeTab === 'overview' || isFullscreen" class="views-admin-ops-ops-dashboard__panel-3">
          <div class="views-admin-ops-ops-dashboard__panel-4">
            <OpsConcurrencyCard :platform-filter="platform" :group-id-filter="groupId" :refresh-token="dashboardRefreshToken" />
          </div>
          <div class="views-admin-ops-ops-dashboard__panel-5">
          ...
          <div class="views-admin-ops-ops-dashboard__panel-6">
        ...
        <!-- Request analysis -->
        <div v-if="activeTab === 'analysis' && !isFullscreen" class="views-admin-ops-ops-dashboard__panel-7">
          <div class="views-admin-ops-ops-dashboard__panel-8">
        ...
        <!-- Alert center -->
        <div v-if="activeTab === 'alerts' && !isFullscreen" class="views-admin-ops-ops-dashboard__panel-7">
        ...
        <!-- System logs -->
        <OpsSystemLogTable
          v-if="activeTab === 'logs' && !isFullscreen"
          :platform-filter="platform"
          :refresh-token="dashboardRefreshToken"
        />
      </template>
...
type OpsModuleTab = 'overview' | 'analysis' | 'alerts' | 'logs'
const allowedModuleTabs = new Set<OpsModuleTab>(['overview', 'analysis', 'alerts', 'logs'])
const activeTab = ref<OpsModuleTab>('overview')
const moduleTabs = computed(() => [
  { value: 'overview' as const, label: t('admin.ops.moduleTabs.overview'), icon: 'grid' as const },
  { value: 'analysis' as const, label: t('admin.ops.moduleTabs.analysis'), icon: 'chart' as const },
  { value: 'alerts' as const, label: t('admin.ops.moduleTabs.alerts'), icon: 'bell' as const },
  { value: 'logs' as const, label: t('admin.ops.moduleTabs.logs'), icon: 'terminal' as const }
])

function selectTab(tab: OpsModuleTab) {
...
const applyRouteQueryToState = () => {
  const nextTab = readQueryString(QUERY_KEYS.tab)
  activeTab.value = allowedModuleTabs.has(nextTab as OpsModuleTab) ? (nextTab as OpsModuleTab) : 'overview'
  ...
  const ruleID = readQueryString(QUERY_KEYS.alertRuleId)
  if (ruleID) {
    activeTab.value = 'alerts'
  }
...
const buildQueryFromState = () => {
  ...
  if (groupId.value) next[QUERY_KEYS.groupId] = groupId.value
  if (queryMode.value !== 'auto') next[QUERY_KEYS.queryMode] = queryMode.value
  if (activeTab.value !== 'overview') next[QUERY_KEYS.tab] = activeTab.value
...
function handleThroughputSelectGroup(nextGroupId: string) {
  groupId.value = nextGroupId || null
}
...
function onGroupChange(v: string | number | boolean | null) {
  groupId.value = typeof v === 'string' && v ? v : null
}
