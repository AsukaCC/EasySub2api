<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-channels-view__panel">
          <!-- Left: Search + Filters -->
          <div class="views-admin-channels-view__panel-2">
            <div class="views-admin-channels-view__panel-3">
              <Icon
                name="search"
                size="md"
                class="views-admin-channels-view__icon"
              />
              <input
                v-model="searchQuery"
                type="text"
                :placeholder="t('admin.channels.searchChannels', 'Search channels...')"
                class="views-admin-channels-view__field input"
                @input="handleSearch"
              />
            </div>
            ...
            <Select
              ...
              class="views-admin-channels-view__field-2"
              @change="loadChannels"
            />
          </div>

          <!-- Right: Actions -->
          <div class="views-admin-channels-view__panel-4">
            <button
              @click="loadChannels"
              :disabled="loading"
            ...
            >
              <Icon name="refresh" size="md" :class="loading ? 'views-admin-channels-view__icon-3' : ''" />
            </button>
            <button @click="openCreateDialog" class="btn btn-primary">
              <Icon name="plus" size="md" class="views-admin-channels-view__icon-2" />
      ...
      <template #table>
        ...
        >
          <template #cell-name="{ value }">
            <span class="views-admin-channels-view__text">{{ value }}</span>
          </template>

          <template #cell-description="{ value }">
            <span class="views-admin-channels-view__text-2">{{ value || '-' }}</span>
          </template>
          ...
          <template #cell-group_count="{ row }">
            <span
              class="views-admin-channels-view__text-3"
            >
          ...
          <template #cell-created_at="{ value }">
            <span class="views-admin-channels-view__text-2">
              {{ formatDate(value) }}
            </span>
          </template>

          <template #cell-actions="{ row }">
            <div class="views-admin-channels-view__panel-5">
              <button
                @click="openEditDialog(row)"
                class="views-admin-channels-view__action"
              >
                <Icon name="edit" size="sm" />
                <span class="views-admin-channels-view__text-4">{{ t('common.edit', 'Edit') }}</span>
              </button>
              <button
                @click="handleDelete(row)"
                class="views-admin-channels-view__action-2"
              >
                <Icon name="trash" size="sm" />
                <span class="views-admin-channels-view__text-4">{{ t('common.delete', 'Delete') }}</span>
              </button>
            </div>
          </template>
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div class="views-admin-channels-view__panel-6">
          <!-- Basic Settings Tab -->
          <button
            type="button"
          ...
          <button
            ...
            class="views-admin-channels-view__action-3 channel-tab"
        ...
        <!-- Tab Content -->
        <form id="channel-form" @submit.prevent="handleSubmit" class="views-admin-channels-view__form">
          <!-- Basic Settings Tab -->
          <div v-show="activeTab === 'basic'" class="views-admin-channels-view__panel-7">
            <!-- Name -->
            <div>
              <label class="input-label">{{ t('admin.channels.form.name', 'Name') }} <span class="views-admin-channels-view__text-5">*</span></label>
              <input
                v-model="form.name"
                type="text"
                required
            ...
            <!-- Model Restriction -->
            <div>
              <label class="views-admin-channels-view__label">
                <input
                  type="checkbox"
                  v-model="form.restrict_models"
                  class="views-admin-channels-view__field-3"
                />
                <span class="views-admin-channels-view__text-6 input-label">{{ t('admin.channels.form.restrictModels', 'Restrict Models') }}</span>
              </label>
              <p class="views-admin-channels-view__description">
            ...
            <div>
              ...
              <p class="views-admin-channels-view__description-2">
                {{ t('admin.channels.form.billingModelSourceHint', 'Controls which model name is used for pricing lookup') }}
              </p>
            </div>

            <!-- Platform Management -->
            <div class="views-admin-channels-view__panel-8">
              <label class="views-admin-channels-view__text-6 input-label">{{ t('admin.channels.form.platformConfig') }}</label>
              <div class="views-admin-channels-view__panel-9">
                <label
                  v-for="p in platformOrder"
                  :key="p"
                  class="views-admin-channels-view__label-2"
                  :class="activePlatforms.includes(p)
                    ? 'views-admin-channels-view__label-9'
                    : 'views-admin-channels-view__label-10'"
                >
                  <input
                    type="checkbox"
                    :checked="activePlatforms.includes(p)"
                    class="views-admin-channels-view__field-4"
            ...
            <div class="views-admin-channels-view__panel-10">
              <div class="views-admin-channels-view__panel-11">
                <div>
                  <label class="views-admin-channels-view__label-3">
                    {{ t('admin.channels.form.applyPricingToAccountStats') }}
                  </label>
                  <p class="views-admin-channels-view__description-3">
          ...
          <div
            ...
            class="views-admin-channels-view__panel-12"
          >
            <!-- Groups -->
            <div>
              <label class="views-admin-channels-view__text-4 input-label">
                {{ t('admin.channels.form.groups', 'Associated Groups') }} <span class="views-admin-channels-view__text-5">*</span>
                <span v-if="section.group_ids.length > 0" class="views-admin-channels-view__text-7">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }) }})
                </span>
              </label>
              <div class="views-admin-channels-view__panel-13">
                <div v-if="groupsLoading" class="views-admin-channels-view__panel-14">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="views-admin-channels-view__panel-14">
                  {{ t('admin.channels.form.noGroupsAvailable', 'No groups available') }}
                </div>
                <div v-else class="views-admin-channels-view__panel-15">
                  <label
                    v-for="group in getGroupsForPlatform(section.platform)"
                    :key="group.id"
                    class="views-admin-channels-view__label-4"
                    :class="[
                      section.group_ids.includes(group.id) ? 'views-admin-channels-view__label-9' : '',
                      isGroupInOtherChannel(group.id, section.platform) ? 'views-admin-channels-view__label-11' : ''
                    ]"
                  >
                    <input
                      type="checkbox"
                      ...
                      class="views-admin-channels-view__field-5"
                      @change="toggleGroupInSection(sIdx, group.id)"
                    />
                    <span :class="['views-admin-channels-view__text-12', platformTextClass(group.platform)]">{{ group.name }}</span>
                    <span
                      :class="['views-admin-channels-view__text-13', platformBadgeLightClass(group.platform)]"
                    >{{ group.rate_multiplier }}x</span>
                    <span class="views-admin-channels-view__text-8">{{ group.account_count || 0 }}</span>
                    <span
                      v-if="isGroupInOtherChannel(group.id, section.platform)"
                      class="views-admin-channels-view__text-8"
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="views-admin-channels-view__panel-16">
              <div class="views-admin-channels-view__panel-11">
                <div>
                  <label class="views-admin-channels-view__label-5">
                    {{ t('admin.channels.form.webSearchEmulation') }}
                  </label>
                  <p class="views-admin-channels-view__description-4">
            ...
            <div v-if="section.platform === 'openai'" class="views-admin-channels-view__panel-16">
              <div class="views-admin-channels-view__panel-17">
                <div>
                  <label class="views-admin-channels-view__label-5">
                    {{ t('admin.channels.form.codexImageGenerationBridge') }}
                  </label>
                  <p class="views-admin-channels-view__description-5">
            ...
            <div v-if="section.platform === 'anthropic'" class="views-admin-channels-view__panel-16">
              <div class="views-admin-channels-view__panel-17">
                <div>
                  <label class="views-admin-channels-view__label-5">
            ...
            <!-- Model Mapping -->
            <div>
              <div class="views-admin-channels-view__panel-18">
                <label class="views-admin-channels-view__label-6 input-label">{{ t('admin.channels.form.modelMapping', 'Model Mapping') }}</label>
                <button type="button" @click="addMappingEntry(sIdx)" class="views-admin-channels-view__action-4">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="views-admin-channels-view__panel-19"
              >
                {{ t('admin.channels.form.noMappingRules', 'No mapping rules. Click "Add" to create one.') }}
              </div>
              <div v-else class="views-admin-channels-view__panel-20">
                <div
                  v-for="(_, srcModel) in section.model_mapping"
                  :key="srcModel"
                  class="views-admin-channels-view__panel-21"
                >
                  <input
                    :value="srcModel"
                    type="text"
                    class="views-admin-channels-view__field-6 input"
                  ...
                  />
                  <span class="views-admin-channels-view__text-9">→</span>
                  <input
                  ...
                  <button
                    ...
                    class="views-admin-channels-view__action-5"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
            ...
            <div>
              <div class="views-admin-channels-view__panel-18">
                <label class="views-admin-channels-view__label-6 input-label">{{ t('admin.channels.form.modelPricing', 'Model Pricing') }}</label>
                <div class="views-admin-channels-view__panel-21">
                  <button
                    type="button"
                    @click="syncLatestModels(sIdx)"
                    :disabled="syncingPlatform === section.platform"
                    class="views-admin-channels-view__action-6"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="views-admin-channels-view__action-4">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
              ...
              <div v-else class="views-admin-channels-view__panel-22">
                <PricingEntryCard
            ...
            <div class="views-admin-channels-view__panel-23">
              <div class="views-admin-channels-view__panel-11">
                <h4 class="views-admin-channels-view__label-3">
                  {{ t('admin.channels.form.accountStatsPricingRules') }}
                </h4>
                <button
                  type="button"
                  @click="addAccountStatsRule(sIdx)"
                  class="views-admin-channels-view__action-7"
                >
              ...
              <p
                ...
                class="views-admin-channels-view__description-6"
              >
              ...
              <div
                ...
                :key="ruleIndex"
                class="views-admin-channels-view__panel-24"
              >
                <div class="views-admin-channels-view__panel-11">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="views-admin-channels-view__field-7"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="views-admin-channels-view__action-8">
                    {{ t('common.delete') }}
                  </button>
                </div>

                <div>
                  <label class="views-admin-channels-view__label-7">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="views-admin-channels-view__panel-25">
                    <label
                      v-for="gid in section.group_ids"
                      :key="gid"
                      class="views-admin-channels-view__label-8"
                      :class="rule.group_ids.includes(gid)
                        ? 'views-admin-channels-view__label-12'
                        : 'views-admin-channels-view__label-10'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="views-admin-channels-view__field-5" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['views-admin-channels-view__text-12', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="views-admin-channels-view__description-2">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="views-admin-channels-view__label-7">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  <!-- Selected account chips -->
                  <div class="views-admin-channels-view__panel-25">
                    <span
                      v-for="accountId in rule.account_ids"
                      :key="accountId"
                      class="views-admin-channels-view__text-10"
                    >
                      <span :class="['views-admin-channels-view__text-12', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) }}</span>
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="views-admin-channels-view__action-9">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  <!-- Account search input -->
                  <div class="views-admin-channels-view__panel-26 rule-account-search-container">
                    <input
                      v-model="ruleAccountSearchKeyword[`${section.platform}-${ruleIndex}`]"
                      type="text"
                      class="views-admin-channels-view__field-8 input"
                    ...
                    >
                      ...
                      >
                        ...
                        <span class="views-admin-channels-view__text-11">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="views-admin-channels-view__description-2">
                ...
                <div>
                  <div class="views-admin-channels-view__panel-18">
                    <label class="views-admin-channels-view__label-7">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="views-admin-channels-view__action-4">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="views-admin-channels-view__panel-19">
      ...
      <template #footer>
        <div class="views-admin-channels-view__panel-28">
...
function isGroupInOtherChannel(groupId: string, _platform: string): boolean {
...
function getGroupInOtherChannelLabel(groupId: string): string {
...
function toggleGroupInSection(sectionIdx: number, groupId: string) {
...
// ── Form ↔ API conversion ──
function formToAPI(): { group_ids: string[], model_pricing: ChannelModelPricing[], model_mapping: Record<string, Record<string, string>>, features_config: Record<string, unknown> } {
...
function apiToForm(channel: Channel): PlatformSection[] {
  ...
  const groupPlatformMap = new Map<string, GroupPlatform>()
...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of [...section.model_pricing, ...section.account_stats_pricing_rules.flatMap(rule => rule.pricing)]) {
      if (validateReasoningEffortMultipliers(entry.reasoning_effort_multipliers, t)) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        const modelLabel = entry.models.join(', ') || t('admin.channels.form.unnamed')
