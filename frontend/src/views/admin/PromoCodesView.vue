<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-promo-codes-view__panel">
          <!-- Left: Search + Filters -->
          <div class="views-admin-promo-codes-view__panel-2">
            <input
              v-model="searchQuery"
              type="text"
          ...
          <Select
            ...
            class="views-admin-promo-codes-view__field"
            @change="loadCodes"
          />

          <!-- Right: Action buttons -->
          <div class="views-admin-promo-codes-view__panel-3">
            <button
              @click="loadCodes"
              :disabled="loading"
            ...
            >
              <Icon name="refresh" size="md" :class="loading ? 'views-admin-promo-codes-view__icon-5' : ''" />
            </button>
            <button @click="showCreateDialog = true" class="btn btn-primary">
              <Icon name="plus" size="md" class="views-admin-promo-codes-view__icon" />
      ...
      <template #table>
        ...
        >
          <template #cell-code="{ value }">
            <div class="views-admin-promo-codes-view__panel-4">
              <code class="views-admin-promo-codes-view__code">{{ value }}</code>
              <button
                @click="copyToClipboard(value)"
                :class="[
                  'views-admin-promo-codes-view__action-5',
                  copiedCode === value
                    ? 'views-admin-promo-codes-view__action-6'
                    : 'views-admin-promo-codes-view__action-7'
                ]"
                :title="copiedCode === value ? t('admin.promo.copied') : t('keys.copyToClipboard')"
              >
                <Icon v-if="copiedCode !== value" name="copy" size="sm" :stroke-width="2" />
                <svg v-else class="views-admin-promo-codes-view__icon-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
          ...
          <template #cell-bonus_amount="{ value }">
            <span class="views-admin-promo-codes-view__text">
              {{ formatPoints(value) }}
            </span>
          </template>

          <template #cell-usage="{ row }">
            <span class="views-admin-promo-codes-view__text-2">
          ...
          <template #cell-expires_at="{ value }">
            <span class="views-admin-promo-codes-view__text-3">
          ...
          <template #cell-actions="{ row }">
            <div class="views-admin-promo-codes-view__panel-5">
              <button
                @click="copyRegisterLink(row)"
                class="views-admin-promo-codes-view__action"
              ...
              <button
                ...
                class="views-admin-promo-codes-view__action-2"
              ...
              <button
                ...
                class="views-admin-promo-codes-view__action-3"
                :title="t('common.edit')"
              >
              ...
              <button
                ...
                class="views-admin-promo-codes-view__action-4"
    ...
    >
      <form id="create-promo-form" @submit.prevent="handleCreate" class="views-admin-promo-codes-view__form">
        <div>
          <label class="input-label">
            {{ t('admin.promo.code') }}
            <span class="views-admin-promo-codes-view__text-4">({{ t('admin.promo.autoGenerate') }})</span>
          </label>
          <input
            v-model="createForm.code"
            type="text"
            class="views-admin-promo-codes-view__field-2 input"
        ...
        <div>
          <label class="input-label">
            ...
            <span class="views-admin-promo-codes-view__text-4">({{ t('admin.promo.zeroUnlimited') }})</span>
          </label>
          <input
      ...
      <template #footer>
        <div class="views-admin-promo-codes-view__panel-6">
    ...
    >
      <form id="edit-promo-form" @submit.prevent="handleUpdate" class="views-admin-promo-codes-view__form">
        <div>
    ...
    >
      <div v-if="usagesLoading" class="views-admin-promo-codes-view__panel-7">
        <Icon name="refresh" size="lg" class="views-admin-promo-codes-view__icon-3" />
      </div>
      <div v-else-if="usages.length === 0" class="views-admin-promo-codes-view__panel-8">
        {{ t('admin.promo.noUsages') }}
      </div>
      <div v-else class="views-admin-promo-codes-view__panel-9">
        <div
          v-for="usage in usages"
          :key="usage.id"
          class="views-admin-promo-codes-view__panel-10"
        >
          <div class="views-admin-promo-codes-view__panel-11">
            <div class="views-admin-promo-codes-view__panel-12">
              <Icon name="user" size="sm" class="views-admin-promo-codes-view__icon-4" />
            </div>
            <div>
              <p class="views-admin-promo-codes-view__text">
                {{ usage.user?.email || t('admin.promo.userPrefix', { id: usage.user_id }) }}
              </p>
              <p class="views-admin-promo-codes-view__description">
                {{ formatDateTime(usage.used_at) }}
              </p>
            </div>
          </div>
          <div class="views-admin-promo-codes-view__panel-13">
            <span class="views-admin-promo-codes-view__text-5">
              +{{ formatPoints(usage.bonus_amount) }}
            </span>
          </div>
        </div>
        <!-- Usages Pagination -->
        <div v-if="usagesTotal > usagesPageSize" class="views-admin-promo-codes-view__panel-14">
          <Pagination
            :page="usagesPage"
            :total="usagesTotal"
      ...
      <template #footer>
        <div class="views-admin-promo-codes-view__panel-15">
...
import { formatDateTime, formatDateTimeLocalInput, formatPoints } from '@/utils/format'
...
const handleCreate = async () => {
  ...
  try {
    await adminAPI.promo.create({
	  ...
	  bonus_validity_days: createForm.bonus_validity_days,
...
const resetCreateForm = () => {
  ...
  createForm.bonus_amount = 1
	createForm.bonus_validity_days = 90
...
const handleEdit = (code: PromoCode) => {
  ...
  editForm.bonus_amount = code.bonus_amount
	editForm.bonus_validity_days = code.bonus_validity_days || 90
...
const handleUpdate = async () => {
  ...
  try {
    await adminAPI.promo.update(editingCode.value.id, {
	  ...
	  bonus_validity_days: editForm.bonus_validity_days,
