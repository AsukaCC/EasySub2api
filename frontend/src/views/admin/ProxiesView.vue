<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-proxies-view__panel">
          <!-- Left: Search + Filters -->
          <div class="views-admin-proxies-view__panel-2">
            <Icon
              name="search"
              size="md"
              class="views-admin-proxies-view__icon"
            />
            <input
              v-model="searchQuery"
              type="text"
              :placeholder="t('admin.proxies.searchProxies')"
              class="views-admin-proxies-view__field input"
              @input="handleSearch"
            />
          </div>

          <div class="views-admin-proxies-view__panel-3">
            <Select
          ...
          <div class="views-admin-proxies-view__panel-4">
            <Select
          ...
          <div class="views-admin-proxies-view__panel-5">
            <button
              @click="loadProxies"
              :disabled="loading"
            ...
            >
              <Icon name="refresh" size="md" :class="loading ? 'views-admin-proxies-view__icon-11' : ''" />
            </button>
            <button
            ...
            >
              <Icon name="play" size="md" class="views-admin-proxies-view__icon-2" />
            ...
            >
              <Icon name="shield" size="md" class="views-admin-proxies-view__icon-2" :class="batchQualityChecking ? 'views-admin-proxies-view__icon-12' : ''" />
            ...
            >
              <Icon name="trash" size="md" class="views-admin-proxies-view__icon-2" />
            ...
            <button @click="showCreateModal = true" class="btn btn-primary">
              <Icon name="plus" size="md" class="views-admin-proxies-view__icon-2" />
      ...
      <template #table>
        <div ref="proxyTableRef" class="views-admin-proxies-view__panel-6">
        <DataTable
          :columns="columns"
          :data="proxies"
          :loading="loading"
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="views-admin-proxies-view__field-2"
          ...
          <template #cell-name="{ value }">
            <span class="views-admin-proxies-view__text">{{ value }}</span>
          </template>
          ...
          <template #cell-protocol="{ value }">
            ...
            <span v-else class="views-admin-proxies-view__text-2">-</span>
          </template>

          <template #cell-address="{ row }">
            <div class="views-admin-proxies-view__panel-7">
              <code class="views-admin-proxies-view__code code">{{ row.host }}:{{ row.port }}</code>
              <div class="views-admin-proxies-view__panel-8">
                <button
                  type="button"
                  class="views-admin-proxies-view__action"
                ...
                <div
                  ...
                  class="views-admin-proxies-view__panel-9"
                >
                  <button
                    v-for="fmt in getCopyFormats(row)"
                    :key="fmt.label"
                    class="views-admin-proxies-view__action-2"
                    @click.stop="copyFormat(fmt.value)"
                  >
                    <span class="views-admin-proxies-view__text-3">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="views-admin-proxies-view__panel-7">
              <div class="views-admin-proxies-view__panel-10">
                <span v-if="row.username" class="views-admin-proxies-view__text-4">{{ row.username }}</span>
                <span v-if="row.password" class="views-admin-proxies-view__text-5">
              ...
              <button
                v-if="row.password"
                type="button"
                class="views-admin-proxies-view__action-3"
          ...
          <template #cell-location="{ row }">
            <div class="views-admin-proxies-view__panel-11">
              <img
                v-if="row.country_code"
                :src="flagUrl(row.country_code)"
                :alt="row.country || row.country_code"
                class="views-admin-proxies-view__image"
              />
              <span v-if="formatLocation(row)" class="views-admin-proxies-view__text-6">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="views-admin-proxies-view__text-2">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="views-admin-proxies-view__action-4"
            ...
            <span
              v-else
              class="views-admin-proxies-view__text-7"
            >
          ...
          <template #cell-latency="{ row }">
            <div class="views-admin-proxies-view__panel-12">
              <span
              ...
              <div
                ...
                class="views-admin-proxies-view__panel-13"
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="views-admin-proxies-view__text-2">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="views-admin-proxies-view__panel-10">
              <span class="views-admin-proxies-view__text-4">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="views-admin-proxies-view__text-8">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="views-admin-proxies-view__panel-14">
              ...
              <button
                ...
                class="views-admin-proxies-view__action-7"
              >
                <Icon name="edit" size="sm" />
                <span class="views-admin-proxies-view__code">{{ t('common.edit') }}</span>
              </button>
              <button
                @click="handleDelete(row)"
                class="views-admin-proxies-view__action-8"
              >
                <Icon name="trash" size="sm" />
                <span class="views-admin-proxies-view__code">{{ t('common.delete') }}</span>
              </button>
            </div>
          </template>
    ...
    >
      <!-- Tab Switch -->
      <div
        class="views-admin-proxies-view__panel-15"
      >
        <div class="views-admin-proxies-view__panel-16">
          <button
            type="button"
            @click="createMode = 'standard'"
            :class="[
              'views-admin-proxies-view__action-10',
              createMode === 'standard'
                ? 'views-admin-proxies-view__action-11'
                : 'views-admin-proxies-view__action-12'
            ]"
          >
            <Icon name="plus" size="sm" class="views-admin-proxies-view__icon-4" />
          ...
          >
            <svg
              class="views-admin-proxies-view__icon-5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="1.5"
      ...
      <form
        ...
        class="views-admin-proxies-view__form"
      >
        <div>
        ...
        <div class="views-admin-proxies-view__panel-17">
          <div>
        ...
        <div>
          ...
          <div class="views-admin-proxies-view__panel-8">
            <input
              ...
              class="views-admin-proxies-view__field-3 input"
              :placeholder="t('admin.proxies.optionalAuth')"
            />
            <button
              type="button"
              class="views-admin-proxies-view__action-9"
        ...
        <div>
          ...
          <div class="views-admin-proxies-view__panel-18">
            <button
      ...
      <!-- Batch Add Form -->
      <div v-else class="views-admin-proxies-view__form">
        <div>
          <label class="input-label">{{ t('admin.proxies.batchInput') }}</label>
          <textarea
            v-model="batchInput"
            rows="10"
            class="views-admin-proxies-view__field-5 input"
            :placeholder="t('admin.proxies.batchInputPlaceholder')"
            @input="parseBatchInput"
          ></textarea>
          <p class="views-admin-proxies-view__description input-hint">
            {{ t('admin.proxies.batchInputHint') }}
          </p>
        </div>

        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="views-admin-proxies-view__panel-19">
            <div class="views-admin-proxies-view__panel-20">
              <div class="views-admin-proxies-view__panel-7">
              <Icon name="checkCircle" size="sm" :stroke-width="2" class="views-admin-proxies-view__icon-6" />
              <span class="views-admin-proxies-view__text-9">
                {{ t('admin.proxies.parsedCount', { count: batchParseResult.valid }) }}
              </span>
            </div>
            <div v-if="batchParseResult.invalid > 0" class="views-admin-proxies-view__panel-7">
              <Icon
                name="exclamationCircle"
                size="sm"
                :stroke-width="2"
                class="views-admin-proxies-view__icon-7"
              />
              <span class="views-admin-proxies-view__text-10">
                {{ t('admin.proxies.invalidCount', { count: batchParseResult.invalid }) }}
              </span>
            </div>
            <div v-if="batchParseResult.duplicate > 0" class="views-admin-proxies-view__panel-7">
              <svg
                class="views-admin-proxies-view__icon-8"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
              ...
              <span class="views-admin-proxies-view__text-11">
      ...
      <template #footer>
        <div class="views-admin-proxies-view__panel-21">
    ...
    >
      <div v-if="qualityReport" class="views-admin-proxies-view__panel-22">
        <div class="views-admin-proxies-view__panel-23">
          <div class="views-admin-proxies-view__panel-24">
            <div>
              <div class="views-admin-proxies-view__panel-25">
                {{ qualityReportProxy?.name || '-' }}
              </div>
              <div class="views-admin-proxies-view__panel-26">
                {{ qualityReport.summary }}
              </div>
            </div>
            <div class="views-admin-proxies-view__panel-27">
              <div class="views-admin-proxies-view__panel-28">
                {{ qualityReport.score }}
              </div>
              <div class="views-admin-proxies-view__panel-29">
                {{ t('admin.proxies.qualityGrade', { grade: qualityReport.grade }) }}
              </div>
            </div>
          </div>
          <div class="views-admin-proxies-view__panel-30">
        ...
        <div class="views-admin-proxies-view__panel-31">
          <table class="views-admin-proxies-view__table">
            <thead class="views-admin-proxies-view__header">
              <tr>
                <th class="views-admin-proxies-view__heading">{{ t('admin.proxies.qualityTableTarget') }}</th>
                <th class="views-admin-proxies-view__heading">{{ t('admin.proxies.qualityTableStatus') }}</th>
                <th class="views-admin-proxies-view__heading">HTTP</th>
                <th class="views-admin-proxies-view__heading">{{ t('admin.proxies.qualityTableLatency') }}</th>
                <th class="views-admin-proxies-view__heading-2">{{ t('admin.proxies.qualityTableMessage') }}</th>
              </tr>
            </thead>
            <tbody class="views-admin-proxies-view__body">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="views-admin-proxies-view__cell">{{ qualityTargetLabel(item.target) }}</td>
                <td class="views-admin-proxies-view__cell-2">
                  <span class="views-admin-proxies-view__text-12 badge" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) }}</span>
                </td>
                <td class="views-admin-proxies-view__cell-3">{{ item.http_status ?? '-' }}</td>
                <td class="views-admin-proxies-view__cell-3">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="views-admin-proxies-view__cell-4">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="views-admin-proxies-view__text-13">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <template #footer>
        <div class="views-admin-proxies-view__panel-32">
    ...
    >
      <div v-if="accountsLoading" class="views-admin-proxies-view__panel-33">
        <Icon name="refresh" size="md" class="views-admin-proxies-view__icon-10" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="views-admin-proxies-view__panel-34">
        {{ t('admin.proxies.accountsEmpty') }}
      </div>
      <div v-else class="views-admin-proxies-view__panel-35">
        <table class="views-admin-proxies-view__table">
          <thead class="views-admin-proxies-view__header">
            <tr>
              <th class="views-admin-proxies-view__heading-3">{{ t('admin.proxies.accountName') }}</th>
              <th class="views-admin-proxies-view__heading-3">{{ t('admin.accounts.columns.platformType') }}</th>
              <th class="views-admin-proxies-view__heading-3">{{ t('admin.proxies.accountNotes') }}</th>
            </tr>
          </thead>
          <tbody class="views-admin-proxies-view__body">
            <tr v-for="account in proxyAccounts" :key="account.id">
              <td class="views-admin-proxies-view__cell-5">{{ account.name }}</td>
              <td class="views-admin-proxies-view__cell-6">
                <PlatformTypeBadge :platform="account.platform" :type="account.type" />
              </td>
              <td class="views-admin-proxies-view__cell-7">
                {{ account.notes || '-' }}
              </td>
            </tr>
          </tbody>
...
const visiblePasswordIds = reactive(new Set<string>())
const copyMenuProxyId = ref<string | null>(null)
...
const testingProxyIds = ref<Set<string>>(new Set())
const qualityCheckingProxyIds = ref<Set<string>>(new Set())
...
const createForm = reactive({
  ...
  backup_proxy_id: null as string | null,
...
const backupProxyOptions = (excludeId?: string) =>
...
const toggleSelectRow = (id: string, event: Event) => {
...
const applyQualityResult = (proxyId: string, result: ProxyQualityCheckResult) => {
  const target = proxies.value.find((proxy) => proxy.id === proxyId)
  if (!target) return
...
const runBatchProxyQualityChecks = async (ids: string[]) => {
...
const runBatchProxyTests = async (ids: string[]) => {
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `easysub2api-proxy-${timestamp}.json`
