<template>
  <div class="card">
    <div
      class="views-admin-settings-email-template-editor__panel"
    >
      <div>
        <h2 class="views-admin-settings-email-template-editor__heading">
          {{ t("admin.settings.emailTemplates.title") }}
        </h2>
        <p class="views-admin-settings-email-template-editor__description">
          {{ t("admin.settings.emailTemplates.description") }}
        </p>
      </div>
      <div class="views-admin-settings-email-template-editor__panel-2">
        <button
          type="button"
    ...
    <div class="views-admin-settings-email-template-editor__panel-3">
      ...
      >
        <span
          class="views-admin-settings-email-template-editor__text"
        ></span>
        {{ t("common.loading") }}
      </div>

      <template v-else>
        <div class="views-admin-settings-email-template-editor__panel-5">
          <div>
            ...
            <Select
              ...
              :options="eventOptions.map(option => ({ value: option.value, label: formatEventOptionLabel(option) }))"
            />
          </div>
          <div>
            ...
            <Select
              ...
              :options="localeOptions.map(localeOption => ({ value: localeOption, label: formatLocale(localeOption) }))"
            />
          </div>
        </div>
        ...
        <div
          ...
          class="views-admin-settings-email-template-editor__panel-6"
        >
          <div class="views-admin-settings-email-template-editor__panel-7">
            <div class="views-admin-settings-email-template-editor__panel-8">
              {{ selectedEventMeta.label }}
            </div>
            <span
              class="views-admin-settings-email-template-editor__text-2"
            >
              {{ selectedEventMeta.categoryLabel }}
            </span>
            <span
              class="views-admin-settings-email-template-editor__text-3"
              :class="
                selectedEventMeta.optional
                  ? 'views-admin-settings-email-template-editor__text-5'
                  : 'views-admin-settings-email-template-editor__text-6'
              "
            >
              {{ selectedEventMeta.optional ? localText("可退订通知", "Optional") : localText("事务邮件", "Transactional") }}
            </span>
          </div>
          <p class="views-admin-settings-email-template-editor__description-2">
            {{ selectedEventMeta.timing }}
          </p>
          <p
            v-if="selectedEventDescription"
            class="views-admin-settings-email-template-editor__description-3"
          >
        ...
        <div
          ...
          class="views-admin-settings-email-template-editor__panel-9"
        >
          {{ t("admin.settings.emailTemplates.empty") }}
        </div>

        <div v-else class="views-admin-settings-email-template-editor__panel-10">
          <div class="views-admin-settings-email-template-editor__panel-11">
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                v-model="html"
                rows="18"
                class="views-admin-settings-email-template-editor__field input"
                :disabled="loadingTemplate"
            ...
            <div
              class="views-admin-settings-email-template-editor__panel-12"
            >
              <div class="views-admin-settings-email-template-editor__panel-13">
                {{ t("admin.settings.emailTemplates.placeholders") }}
              </div>
              <p class="views-admin-settings-email-template-editor__description-3">
                {{ t("admin.settings.emailTemplates.placeholdersHelp") }}
              </p>
              <div class="views-admin-settings-email-template-editor__panel-14">
                <button
                  v-for="placeholder in placeholderList"
                  :key="placeholder"
                  type="button"
                  class="views-admin-settings-email-template-editor__action"
          ...
          <div class="views-admin-settings-email-template-editor__panel-11">
            <div
              class="views-admin-settings-email-template-editor__panel-15"
            >
              <div
                class="views-admin-settings-email-template-editor__panel-16"
              >
                <div>
                  <div class="views-admin-settings-email-template-editor__panel-13">
                    {{ t("admin.settings.emailTemplates.livePreview") }}
                  </div>
                  <div class="views-admin-settings-email-template-editor__panel-17">
                    {{ previewSubject || t("admin.settings.emailTemplates.noPreview") }}
                  </div>
                </div>
                <span
                  v-if="isCustomTemplate"
                  class="views-admin-settings-email-template-editor__text-4"
                >
                  {{ t("admin.settings.emailTemplates.customized") }}
                </span>
              </div>
              <div class="views-admin-settings-email-template-editor__panel-18">
                <iframe
                  class="views-admin-settings-email-template-editor__iframe"
                  sandbox=""
                  :srcdoc="previewHtml"
            ...
            <p class="views-admin-settings-email-template-editor__description-4">
...
const eventDisplayMeta: Record<string, EventDisplayMeta> = {
  ...
  "balance.low": {
    label: "积分不足提醒",
    timing: "用户平台积分低于全局或个人配置的提醒阈值时发送。",
    categoryLabel: "计费",
  },
  "balance.recharge_success": {
    label: "积分充值成功",
    timing: "积分充值订单支付完成并到账后发送。",
    categoryLabel: "计费",
  },
  "account.quota_alert": {
    label: "账号限额告警",
...
const eventDisplayMetaEn: Record<string, EventDisplayMeta> = {
  ...
  "subscription.purchase_success": {
    ...
    timing: "Sent after platform points are deducted and the subscription is activated or extended.",
  ...
  "balance.low": {
    ...
    timing: "Sent when a user's platform points drop below the global or personal reminder threshold.",
    categoryLabel: "Billing",
  },
  "balance.recharge_success": {
    label: "Points Top-Up Success",
    timing: "Sent after a point top-up order is paid and the points are credited.",
    categoryLabel: "Billing",
  },
  "account.quota_alert": {
