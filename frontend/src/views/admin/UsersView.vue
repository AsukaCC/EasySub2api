<template>
  <AppLayout>
    <TablePageLayout>
      ...
      <template #filters>
        <div class="views-admin-users-view__panel">
          <!-- Left: Search + Active Filters -->
          <div class="views-admin-users-view__panel-2">
            <!-- Search Box -->
            <div class="views-admin-users-view__panel-3">
              <Icon
                name="search"
                size="md"
                class="views-admin-users-view__icon"
              />
              <input
                v-model="searchQuery"
                type="text"
                :placeholder="t('admin.users.searchUsers')"
                class="views-admin-users-view__field input"
                @input="handleSearch"
              />
            </div>

            <!-- Role Filter (visible when enabled) -->
            <div v-if="visibleFilters.has('role')" class="views-admin-users-view__panel-4">
              <Select
            ...
            <div v-if="visibleFilters.has('status')" class="views-admin-users-view__panel-4">
              <Select
            ...
            <div v-if="visibleFilters.has('group')" class="views-admin-users-view__panel-5">
              <Select
            ...
            <div v-if="visibleFilters.has('apiKeyGroup')" class="views-admin-users-view__panel-5">
              <Select
            ...
            <template v-for="(value, attrId) in activeAttributeFilters" :key="attrId">
              <div
                ...
                class="views-admin-users-view__panel-6"
              >
                <!-- Text/Email/URL/Textarea/Date type: styled input -->
                <input
                  v-if="['text', 'textarea', 'email', 'url', 'date'].includes(getAttributeDefinition(attrId)?.type || 'text')"
                  :value="value"
                  @input="(e) => updateAttributeFilter(attrId, (e.target as HTMLInputElement).value)"
                  @keyup.enter="applyFilter"
                  :placeholder="getAttributeDefinitionName(attrId)"
                  class="views-admin-users-view__field-2 input"
                />
                <!-- Number type: number input -->
                <input
                  v-else-if="getAttributeDefinition(attrId)?.type === 'number'"
                  :value="value"
                  type="number"
                  @input="(e) => updateAttributeFilter(attrId, (e.target as HTMLInputElement).value)"
                  @keyup.enter="applyFilter"
                ...
                <template v-else-if="['select', 'multi_select'].includes(getAttributeDefinition(attrId)?.type || '')">
                  <div class="views-admin-users-view__field-2">
                    <Select
                      :model-value="value"
                      :options="[
                        { value: '', label: getAttributeDefinitionName(attrId) },
                        ...(getAttributeDefinition(attrId)?.options || [])
                      ]"
                      @update:model-value="(val) => { updateAttributeFilter(attrId, String(val ?? '')); applyFilter() }"
                    />
                  </div>
                </template>
                <!-- Fallback -->
          ...
          <div class="views-admin-users-view__panel-7">
            <!-- Mobile: Secondary buttons (icon only) -->
            <div class="views-admin-users-view__panel-8">
              <!-- Refresh Button -->
              <button
                @click="loadUsers"
                :disabled="loading"
              ...
              <div class="views-admin-users-view__panel-9 filter-toolbar" ref="filterDropdownRef">
                ...
                >
                  <Icon name="filter" size="sm" class="views-admin-users-view__icon-2" />
                  <span class="views-admin-users-view__text">{{ t('admin.users.filterSettings') }}</span>
                </button>
                <!-- Dropdown menu -->
                <div
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      class="views-admin-users-view__icon-3"
                      :stroke-width="2"
                    />
                  </button>
                  ...
                  <div
                    ...
                    class="views-admin-users-view__panel-11"
                  ></div>
              ...
              <div class="views-admin-users-view__panel-9 filter-toolbar" ref="columnDropdownRef">
                ...
                >
                  <svg class="views-admin-users-view__icon-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 4.5v15m6-15v15m-10.875 0h15.75c.621 0 1.125-.504 1.125-1.125V5.625c0-.621-.504-1.125-1.125-1.125H4.125C3.504 4.5 3 5.004 3 5.625v12.75c0 .621.504 1.125 1.125 1.125z" />
                  </svg>
                  <span class="views-admin-users-view__text">{{ t('admin.users.columnSettings') }}</span>
                </button>
                <!-- Dropdown menu -->
                <div
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'views-admin-users-view__icon-15' : 'views-admin-users-view__icon-3'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
              ...
              >
                <Icon name="cog" size="sm" class="views-admin-users-view__icon-2" />
                <span class="views-admin-users-view__text">{{ t('admin.users.attributes.configButton') }}</span>
              </button>
            </div>
            ...
            >
              <Icon name="users" size="md" class="views-admin-users-view__icon-5" />
        ...
        >
          <template #cell-email="{ value }">
            <div class="views-admin-users-view__panel-13">
              <div
                class="views-admin-users-view__panel-14"
              >
                <span class="views-admin-users-view__text-2">
                  {{ value.charAt(0).toUpperCase() }}
                </span>
              </div>
              <span class="views-admin-users-view__text-3">{{ value }}</span>
            </div>
          </template>

          <template #cell-username="{ value }">
            <span class="views-admin-users-view__text-4">{{ value || '-' }}</span>
          </template>

          <template #cell-notes="{ value }">
            <div class="views-admin-users-view__panel-15">
              <span
                v-if="value"
                :title="value.length > 30 ? value : undefined"
                class="views-admin-users-view__text-5"
              >
                {{ value.length > 30 ? value.substring(0, 25) + '...' : value }}
              </span>
              <span v-else class="views-admin-users-view__text-6">-</span>
            </div>
          </template>
          ...
          >
            <div class="views-admin-users-view__panel-15">
              <span
                class="views-admin-users-view__text-7"
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="views-admin-users-view__panel-16">
              ...
              <span
                ...
                class="views-admin-users-view__text-8"
                @click.stop="toggleExpandedGroup(row.id)"
              >
                <Icon name="shield" size="xs" class="views-admin-users-view__icon-6" />
                <span class="views-admin-users-view__text-9">{{ getUserGroups(row).exclusive.length }}</span>
                <span class="views-admin-users-view__text-10">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="views-admin-users-view__panel-17"
                >
                  <div class="views-admin-users-view__panel-18"></div>
                  <div class="views-admin-users-view__panel-19">
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="views-admin-users-view__panel-20"
                >
                  <div class="views-admin-users-view__panel-21">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="views-admin-users-view__panel-22"
                    @click.stop="openGroupReplace(row, g)"
                  >
                    <Icon name="swap" size="xs" class="views-admin-users-view__icon-7" />
                    <span class="views-admin-users-view__text-11">{{ g.name }}</span>
                  </div>
                </div>
              </span>
              <!-- 公开分组行 -->
              <span
                ...
                class="views-admin-users-view__text-12"
              >
                <Icon name="globe" size="xs" class="views-admin-users-view__icon-8" />
                <span class="views-admin-users-view__text-13">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="views-admin-users-view__text-14">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="views-admin-users-view__panel-23">
                  <div class="views-admin-users-view__panel-18"></div>
                  <div class="views-admin-users-view__panel-19">
              ...
              <span
                ...
                class="views-admin-users-view__text-15"
              >-</span>
            </div>
            <span v-else class="views-admin-users-view__text-15">-</span>
          </template>

          <template #cell-subscriptions="{ row }">
            <div
              v-if="row.subscriptions && row.subscriptions.length > 0"
              class="views-admin-users-view__panel-24"
            >
              <GroupBadge
            ...
            <span
              v-else
              class="views-admin-users-view__text-16"
            >
              <Icon name="ban" size="xs" class="views-admin-users-view__icon-9" />
          ...
          <template #cell-balance="{ value, row }">
            <div class="views-admin-users-view__panel-13">
              ...
              <button
                ...
                class="views-admin-users-view__action-5"
          ...
          <template #cell-balance_platform_quota="{ row }">
            <button
              type="button"
              class="views-admin-users-view__action-6"
          ...
          >
            <div class="views-admin-users-view__panel-28">
              <span>{{ column.label }}</span>
              <div class="views-admin-users-view__panel-9 usage-sort-trigger">
                <button
                  type="button"
                  class="views-admin-users-view__action-7"
                  :class="usageSort && usageSort.key === usageKey
                    ? 'views-admin-users-view__action-16'
                    : 'views-admin-users-view__text-14'"
                ...
                >
                  <span
                    ...
                    class="views-admin-users-view__text-17"
                  >{{ usageSort.metric === 'today' ? t('admin.users.today') : t('admin.users.total') }}</span>
                  <svg
                    v-if="usageSort && usageSort.key === usageKey"
                    class="views-admin-users-view__icon-9"
                    :class="{ 'views-admin-users-view__icon-16': usageSort.order === 'desc' }"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                  ...
                  <svg v-else class="views-admin-users-view__icon-9" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div
                  ...
                  class="views-admin-users-view__panel-29"
                >
                  <button
                    v-for="metric in (['today', 'total'] as const)"
                    :key="metric"
                    type="button"
                    class="views-admin-users-view__action-8"
                    :class="isUsageSortActive(usageKey, metric)
                      ? 'views-admin-users-view__action-17'
                      : 'views-admin-users-view__action-18'"
                  ...
                  >
                    ...
                    <svg
                      ...
                      class="views-admin-users-view__icon-10"
                      :class="{ 'views-admin-users-view__icon-16': getUsageSortOrder(usageKey, metric) === 'desc' }"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
          ...
          <template #cell-status="{ value }">
            <div class="views-admin-users-view__panel-28">
              <span
                :class="[
                  'views-admin-users-view__text-20',
                  value === 'active' ? 'views-admin-users-view__text-22' : 'views-admin-users-view__text-23'
                ]"
              ></span>
              <span class="views-admin-users-view__text-4">
          ...
          <template #cell-created_at="{ value }">
            <span class="views-admin-users-view__text-18">{{ formatDateTime(value) }}</span>
          </template>

          <template #cell-last_used_at="{ value }">
            <span class="views-admin-users-view__text-18">
          ...
          <template #cell-actions="{ row }">
            <div class="views-admin-users-view__panel-31">
              <!-- Edit Button -->
              <button
                @click="handleEdit(row)"
                class="views-admin-users-view__action-9"
              >
                <Icon name="edit" size="sm" />
                <span class="views-admin-users-view__text-19">{{ t('common.edit') }}</span>
              </button>
              ...
              <button
                ...
                :class="[
                  'views-admin-users-view__action-19',
                  row.status === 'active'
                    ? 'views-admin-users-view__action-20'
                    : 'views-admin-users-view__action-21'
                ]"
              >
                <Icon v-if="row.status === 'active'" name="ban" size="sm" />
                <Icon v-else name="checkCircle" size="sm" />
                <span class="views-admin-users-view__text-19">{{ row.status === 'active' ? t('admin.users.disable') : t('admin.users.enable') }}</span>
              </button>

              <!-- More Actions Menu Trigger -->
              <button
                @click="openActionMenu(row, $event)"
                class="views-admin-users-view__action-10 action-menu-trigger"
                :class="{ 'views-admin-users-view__action-22': activeMenuId === row.id }"
              >
                <Icon name="more" size="sm" />
                <span class="views-admin-users-view__text-19">{{ t('common.more') }}</span>
              </button>
            </div>
          </template>
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <Icon name="key" size="sm" class="views-admin-users-view__icon-11" :stroke-width="2" />
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <Icon name="users" size="sm" class="views-admin-users-view__icon-11" :stroke-width="2" />
                {{ t('admin.users.groups') }}
              </button>

              <div class="views-admin-users-view__panel-11"></div>
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <Icon name="plus" size="sm" class="views-admin-users-view__icon-12" :stroke-width="2" />
                {{ t('admin.users.deposit') }}
              </button>
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <svg class="views-admin-users-view__icon-13" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <Icon name="chartBar" size="sm" class="views-admin-users-view__icon-11" :stroke-width="2" />
              ...
              <button
                ...
                class="views-admin-users-view__action-11"
              >
                <Icon name="sparkles" size="sm" class="views-admin-users-view__icon-11" :stroke-width="2" />
              ...
              <button
                ...
                class="views-admin-users-view__action-12"
              >
...
const getAttributeValue = (userId: string, attrId: string): string => {
...
const filters = reactive({
  ...
  apiKeyGroup: null as string | null  // group id bound to the user's API keys, null = all
})
...
const loadSavedFilters = () => {
  try {
    ...
    if (savedValues) {
      ...
      if (typeof parsed.apiKeyGroup === 'string') filters.apiKeyGroup = parsed.apiKeyGroup
...
const getAttributeDefinition = (attrId: string): UserAttributeDefinition | undefined => {
...
const expandedGroupUserId = ref<string | null>(null)
const toggleExpandedGroup = (userId: string) => {
...
const groupReplaceOldGroup = ref<{ id: string; name: string } | null>(null)
...
// Filter helpers
const getAttributeDefinitionName = (attrId: string): string => {
  const def = attributeDefinitions.value.find(d => d.id === attrId)
...
const updateAttributeFilter = (attrId: string, value: string) => {
...
const openGroupReplace = (user: AdminUser, group: { id: string; name: string }) => {
  expandedGroupUserId.value = null
