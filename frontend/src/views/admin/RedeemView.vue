<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-redeem-view__panel">
          <!-- Left: Search + Filters -->
          <div class="views-admin-redeem-view__panel-2">
            <input
              v-model="searchQuery"
              type="text"
          ...
          <Select
            ...
            class="views-admin-redeem-view__field"
            @change="loadCodes"
          />
          <Select
          ...
          <div class="views-admin-redeem-view__panel-3">
            <button
              @click="loadCodes"
              :disabled="loading"
            ...
            >
              <Icon name="refresh" size="md" :class="loading ? 'views-admin-redeem-view__icon-4' : ''" />
            </button>
            ...
            <button
              ...
              class="btn btn-secondary"
            >
              <Icon name="edit" size="md" class="views-admin-redeem-view__icon" />
        ...
        >
          <template #header-select>
            <input
              ...
              type="checkbox"
              class="views-admin-redeem-view__field-2"
          ...
          <template #cell-code="{ value }">
            <div class="views-admin-redeem-view__panel-4">
              <code class="views-admin-redeem-view__code">{{ value }}</code>
              <button
                @click="copyToClipboard(value)"
                :class="[
                  'views-admin-redeem-view__action-5',
                  copiedCode === value
                    ? 'views-admin-redeem-view__action-6'
                    : 'views-admin-redeem-view__action-7'
                ]"
                :title="copiedCode === value ? t('admin.redeem.copied') : t('keys.copyToClipboard')"
              >
                <Icon v-if="copiedCode !== value" name="copy" size="sm" :stroke-width="2" />
                <svg v-else class="views-admin-redeem-view__icon-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
          ...
          <template #cell-value="{ value, row }">
            <span class="views-admin-redeem-view__text">
              <template v-if="row.type === 'balance'">{{ formatPoints(value) }}</template>
              <template v-else-if="row.type === 'subscription'">
                {{ row.validity_days || 30 }} {{ t('admin.redeem.days') }}
                <span v-if="row.group" class="views-admin-redeem-view__text-2"
                  >({{ row.group.name }})</span
                >
              </template>
          ...
          <template #cell-used_by="{ value, row }">
            <span class="views-admin-redeem-view__text-3">
          ...
          <template #cell-expires_at="{ value, row }">
            <span
              :class="[
                'views-admin-redeem-view__text-8',
                row.status === 'expired'
                  ? 'views-admin-redeem-view__text-9'
                  : 'views-admin-redeem-view__text-10'
              ]"
            >
          ...
          <template #cell-actions="{ row }">
            <div class="views-admin-redeem-view__panel-4">
              <button
                ...
                class="views-admin-redeem-view__action"
              >
                <svg class="views-admin-redeem-view__icon-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                ...
                <span class="views-admin-redeem-view__text-4">{{ t('common.delete') }}</span>
              </button>
              <span v-else class="views-admin-redeem-view__text-5">-</span>
            </div>
          </template>
        </DataTable>
      </template>
      ...
      <template #pagination>
        <div
          ...
          class="views-admin-redeem-view__panel-5"
        >
          <span class="views-admin-redeem-view__text-6">
            {{ t('admin.redeem.selectedCount', { count: selectedCount }) }}
          </span>
          <div class="views-admin-redeem-view__panel-6">
            <button
              type="button"
              class="views-admin-redeem-view__action-2"
        ...
        <!-- Batch Actions -->
        <div v-if="filters.status === 'unused'" class="views-admin-redeem-view__panel-7">
    ...
    <Teleport to="body">
      <div v-if="showGenerateDialog" class="views-admin-redeem-view__panel-8">
        ...
        >
          ...
          <form @submit.prevent="handleGenerateCodes" class="views-admin-redeem-view__form">
            <div>
              ...
              <div>
                ...
                >
                  <template #selected="{ option }">
                    ...
                    />
                    <span v-else class="views-admin-redeem-view__text-7">{{
            ...
            <div>
              ...
              <div class="views-admin-redeem-view__panel-12">
                <button
                  ...
                  :class="[
                    'views-admin-redeem-view__action-8',
                    generateForm.expiry_option === option.value
                      ? 'views-admin-redeem-view__action-9'
                      : 'views-admin-redeem-view__action-10'
                  ]"
                >
                  {{ option.label }}
                </button>
              ...
              <input
                ...
                max="3650"
                required
                class="views-admin-redeem-view__field-3 input"
            ...
            <div class="views-admin-redeem-view__panel-13">
    ...
    <Teleport to="body">
      <div
        ...
        class="views-admin-redeem-view__panel-14"
      >
        ...
        >
          ...
          <p class="views-admin-redeem-view__description-2">
            {{ t('admin.redeem.selectedCount', { count: selectedCount }) }}
          </p>

          <form data-test="batch-update-form" class="views-admin-redeem-view__form" @submit.prevent="handleBatchUpdate">
            <div class="views-admin-redeem-view__panel-16">
              <label class="views-admin-redeem-view__label">
                <input
                  data-test="batch-field-status"
                  v-model="batchUpdateForm.update_status"
                  type="checkbox"
                  class="views-admin-redeem-view__field-4"
                />
    ...
    <Teleport to="body">
      <div v-if="showResultDialog" class="views-admin-redeem-view__panel-14">
        <div class="views-admin-redeem-view__panel-9" @click="closeResultDialog"></div>
        <div class="views-admin-redeem-view__panel-17">
          <!-- Header -->
          <div
            class="views-admin-redeem-view__panel-18"
          >
            <div class="views-admin-redeem-view__panel-19">
              <div
                class="views-admin-redeem-view__panel-20"
              >
                <svg
                  class="views-admin-redeem-view__icon-3"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div>
                <h2 class="views-admin-redeem-view__heading-3">
                  {{ t('admin.redeem.generatedSuccessfully') }}
                </h2>
                <p class="views-admin-redeem-view__description-3">
            ...
            <button
              ...
              class="views-admin-redeem-view__action-3"
            >
              <Icon name="x" size="md" :stroke-width="2" />
            </button>
          </div>
          <!-- Content -->
          <div class="views-admin-redeem-view__panel-21">
            <div class="views-admin-redeem-view__panel-22">
              <textarea
                readonly
                :value="generatedCodesText"
                :style="{ height: textareaHeight }"
                class="views-admin-redeem-view__field-5"
              ></textarea>
            </div>
          </div>
          <!-- Footer -->
          <div
            class="views-admin-redeem-view__panel-23"
          >
            <button
              @click="copyGeneratedCodes"
              :class="[
                'views-admin-redeem-view__action-11 btn',
            ...
            <button @click="downloadGeneratedCodes" class="views-admin-redeem-view__action-4 btn btn-primary">
...
const handleGenerateCodes = async () => {
  ...
  try {
    const result = await adminAPI.redeem.generate(
	  ...
	  expiresInDays,
	  generateForm.type === 'balance' ? generateForm.bonus_validity_days : undefined,
    )
	...
	generateForm.bonus_validity_days = 90
