...
const routeHarness = vi.hoisted(() => ({
  current: null as null | { meta: { settingsSection: string } },
}));

vi.mock("vue-router", async () => {
  const { reactive } = await import("vue");
  routeHarness.current = reactive({ meta: { settingsSection: "platform" } });
...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.authSourceDefaults.requireEmailHint": "启用后，OIDC、微信注册缺少邮箱时必须先补充邮箱地址。",
    ...
    "admin.settings.upstreamBillingProbe.description": "定期获取 OpenAI API Key 所连接上游 EasySub2api 站点声明的计费倍率。",
...
function mountView(section = "platform") {
  if (routeHarness.current) routeHarness.current.meta.settingsSection = section;
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("renders panel rate limit card and saves settings", async () => {
    ...
    const wrapper = mountView("access");
    await flushPromises();
  ...
  it("shows the fixed point contract and saves sorted decimal-safe recharge bonus tiers", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
    ...
    expect(wrapper.get('[data-testid="payment-points-conversion"]').text()).toContain("1 RMB");
    expect(wrapper.get('[data-testid="payment-points-conversion"]').text()).toContain(
      "admin.settings.payment.onePlatformPoint",
    );
    expect((wrapper.get('[data-testid="recharge-bonus-threshold-0"]').element as HTMLInputElement).value).toBe("100");
    ...
    expect(payload).not.toHaveProperty("payment_subscription_usd_to_cny_rate");
  });

  it("does not hide payment controls inside a native template element", async () => {
    const wrapper = mountView("payment");
    await flushPromises();

    const bonusTiers = wrapper.get('[data-testid="payment-recharge-bonus-tiers"]');
    expect(bonusTiers.element.closest("template")).toBeNull();
    expect(wrapper.find(".views-admin-settings-view__panel-5 > template").exists()).toBe(false);
  });
  ...
  it("rejects recharge bonus precision instead of rounding the input", async () => {
    ...
    await wrapper.get('[data-testid="recharge-bonus-threshold-0"]').setValue("100.001");
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();

    expect(updateSettings).not.toHaveBeenCalled();
    expect(showError).toHaveBeenCalledWith("admin.settings.payment.invalidBonusTier");
  });

  it("keeps the affiliate freeze window fixed at 168 hours", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
    ...
    expect(wrapper.get('[data-testid="affiliate-freeze-hours-fixed"]').text()).toContain("168");
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("保存时 updateSettings payload 应包含所有平台的嵌套限额对象", async () => {
    const wrapper = mountView();
    await flushPromises();
    await openUsersTab(wrapper);
