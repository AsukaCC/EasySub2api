    ...
    <div class="views-admin-usage-view__panel">
      ...
      <div class="views-admin-usage-view__panel-2">
        <div class="views-admin-usage-view__panel-3 card">
          <div class="views-admin-usage-view__panel-4">
            <div class="views-admin-usage-view__panel-5">
              <span class="views-admin-usage-view__text">{{ t('admin.dashboard.timeRange') }}:</span>
              <DateRangePicker
            ...
            <div class="views-admin-usage-view__panel-6">
              <span class="views-admin-usage-view__text">{{ t('admin.dashboard.granularity') }}:</span>
              <div class="views-admin-usage-view__panel-7">
                <Select v-model="granularity" :options="granularityOptions" @change="loadChartData" />
              </div>
            </div>
          </div>
        </div>
        <div class="views-admin-usage-view__panel-8">
      ...
      <div class="card">
        <div class="views-admin-usage-view__panel-9">
          <button
            v-for="tab in detailTabs"
            :key="tab.key"
            type="button"
            data-testid="usage-detail-tab"
            class="views-admin-usage-view__action"
            :class="activeTab === tab.key
              ? 'views-admin-usage-view__action-4'
              : 'views-admin-usage-view__action-5'"
        ...
        <UsageFilters v-model="filters" ref="usageFiltersRef" flat :mode="activeTab" class="views-admin-usage-view__usage-filters" :start-date="startDate" :end-date="endDate" :exporting="exporting" :model-options="modelNameOptions" :api-base-path="usageApiBasePath" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
          <template #after-reset>
            <div v-if="activeTab !== 'ranking'" class="views-admin-usage-view__panel-10 filter-toolbar" ref="columnDropdownRef">
              ...
              >
                <svg class="views-admin-usage-view__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 4.5v15m6-15v15m-10.875 0h15.75c.621 0 1.125-.504 1.125-1.125V5.625c0-.621-.504-1.125-1.125-1.125H4.125C3.504 4.5 3 5.004 3 5.625v12.75c0 .621.504 1.125 1.125 1.125z" />
                </svg>
                <span class="views-admin-usage-view__text-2">{{ t('admin.users.columnSettings') }}</span>
              </button>
              <div
              ...
              >
                ...
                >
                  ...
                  <Icon
                    ...
                    name="check"
                    size="sm"
                    class="views-admin-usage-view__icon-2"
                    :stroke-width="2"
                  />
                </button>
              </div>
        ...
        <div v-show="activeTab === 'usage'" class="views-admin-usage-view__panel-12">
          <UsageTable
            flat
            :data="usageLogs"
            :loading="loading"
        ...
        <div v-show="activeTab === 'errors'" class="views-admin-usage-view__panel-12">
          <OpsErrorLogTable
            flat
        ...
        <div v-if="rankingMounted" v-show="activeTab === 'ranking'" class="views-admin-usage-view__panel-12">
          <UserTokenRanking
            ref="rankingRef"
            :start-date="startDate"
            :end-date="endDate"
...
const usageStats = ref<AdminUsageStatsResponse | null>(null); const adminUsageStats = ref<AdminUsageStatsResponse | null>(null); const usageLogs = ref<AdminUsageLog[]>([]); const loading = ref(true); const exporting = ref(false)
...
const handleRankingSelectUser = (userId: string, email: string) => {
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.rate'), t('usage.accountMultiplier'), t('usage.standardCost'), t('usage.billedPoints'), t('usage.accountBilled'),
...
const openError = (id: string) => { selectedErrorId.value = id; showErrorModal.value = true }
