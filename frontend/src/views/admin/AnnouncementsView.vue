<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="views-admin-announcements-view__panel">
          <!-- Left: Search + Filters -->
          <div class="views-admin-announcements-view__panel-2">
            <input
              v-model="searchQuery"
              type="text"
          ...
          <Select
            ...
            class="views-admin-announcements-view__field"
            @change="handleStatusChange"
          />

          <!-- Right: Action buttons -->
          <div class="views-admin-announcements-view__panel-3">
            <button
            ...
            >
              <Icon name="refresh" size="md" :class="loading ? 'views-admin-announcements-view__icon-2' : ''" />
            </button>
            <button @click="openCreateDialog" class="btn btn-primary">
              <Icon name="plus" size="md" class="views-admin-announcements-view__icon" />
      ...
      <template #table>
        ...
        >
          <template #cell-title="{ value, row }">
            <div class="views-admin-announcements-view__panel-4">
              <div class="views-admin-announcements-view__panel-5">
                <span class="views-admin-announcements-view__text">{{ value }}</span>
              </div>
              <div class="views-admin-announcements-view__panel-6">
                <span>#{{ row.id }}</span>
                <span class="views-admin-announcements-view__text-2">·</span>
          ...
          <template #cell-targeting="{ row }">
            <span class="views-admin-announcements-view__text-3">
              {{ targetingSummary(row.notify_mode === 'silent' ? { any_of: [] } : row.targeting) }}
            </span>
          </template>

          <template #cell-timeRange="{ row }">
            <div v-if="row.notify_mode === 'silent'" class="views-admin-announcements-view__text-3">
              {{ t('admin.announcements.timeNotApplicable') }}
            </div>
            <div v-else class="views-admin-announcements-view__text-3">
              <div>
                <span class="views-admin-announcements-view__text-4">{{ t('admin.announcements.form.startsAt') }}:</span>
                <span class="views-admin-announcements-view__text-5">{{ row.starts_at ? formatDateTime(row.starts_at) : t('admin.announcements.timeImmediate') }}</span>
              </div>
              <div class="views-admin-announcements-view__panel-7">
                <span class="views-admin-announcements-view__text-4">{{ t('admin.announcements.form.endsAt') }}:</span>
                <span class="views-admin-announcements-view__text-5">{{ row.ends_at ? formatDateTime(row.ends_at) : t('admin.announcements.timeNever') }}</span>
              </div>
            </div>
          </template>

          <template #cell-created_at="{ value }">
            <span class="views-admin-announcements-view__text-6">{{ formatDateTime(value) }}</span>
          </template>

          <template #cell-actions="{ row }">
            <div class="views-admin-announcements-view__panel-8">
              <button
                @click="openPreview(row)"
                class="views-admin-announcements-view__action"
              ...
              <button
                ...
                class="views-admin-announcements-view__action-2"
                :title="t('common.edit')"
              >
              ...
              <button
                ...
                class="views-admin-announcements-view__action-3"
    ...
    <BaseDialog
      ...
      @close="closeEdit"
    >
      <form id="announcement-form" @submit.prevent="handleSave" class="views-admin-announcements-view__form">
        <div>
        ...
        <div class="views-admin-announcements-view__panel-9">
          <div>
          ...
          <div>
            ...
            <Select v-model="form.notify_mode" :options="notifyModeOptions" @change="handleNotifyModeChange" />
      ...
      <template #footer>
        <div class="views-admin-announcements-view__panel-10">
...
function handleNotifyModeChange(value: string | number | boolean | null) {
...
function fillFormFromAnnouncement(a: Announcement) {
  ...
  form.starts_at_str = form.notify_mode === 'silent' || !a.starts_at
    ? ''
    : formatDateTimeLocalInput(Math.floor(new Date(a.starts_at).getTime() / 1000))
  form.ends_at_str = form.notify_mode === 'silent' || !a.ends_at
    ? ''
    : formatDateTimeLocalInput(Math.floor(new Date(a.ends_at).getTime() / 1000))

  form.targeting = form.notify_mode === 'silent' ? { any_of: [] } : (a.targeting ?? { any_of: [] })
}
