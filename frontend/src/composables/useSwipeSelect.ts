...
export interface SwipeSelectAdapter {
  ...
  batchUpdate?: (updater: (draft: Set<string>) => void) => void
}
...
export function useSwipeSelect(
  ...
  let lastMouseY = 0
  let pendingStartY = 0
  let initialSelectedSnapshot = new Map<string, boolean>()
  ...
  function getRowId(el: HTMLElement): string | null {
  ...
  function createMarquee() {
    ...
    Object.assign(marqueeEl.style, {
      ...
      border: isDark
        ? '1.5px solid color-mix(in srgb, var(--theme-accent) 50%, transparent)'
        : '1.5px solid color-mix(in srgb, var(--theme-accent) 40%, transparent)',
      borderRadius: '4px',
      pointerEvents: 'none',
      zIndex: '9999',
      transition: 'none',
