...
describe('useNavigationLoading', () => {
  ...
  it('keeps loading until navigation and all page regions finish', () => {
    const state = useNavigationLoading()
    state.startNavigation()
    const finishFirst = state.beginPageLoad()
    const finishSecond = state.beginPageLoad()
    vi.advanceTimersByTime(100)
    state.endNavigation()
    finishFirst()
    finishFirst()
  ...
  it('does not let a cancelled old navigation finish a newer one', () => {
    const state = useNavigationLoading()
    const previous = state.startNavigation()
    const current = state.startNavigation()
    vi.advanceTimersByTime(100)
    state.endNavigation(previous)
    expect(state.isLoading.value).toBe(true)
    state.endNavigation(current)
    expect(state.isLoading.value).toBe(false)
  })

  it('does not flash for a fast page load or let an old completion affect a reset', () => {
    const state = useNavigationLoading()
    const finishOld = state.beginPageLoad()
    vi.advanceTimersByTime(50)
    state.resetState()
    const finishNew = state.beginPageLoad()
    finishOld()
    vi.advanceTimersByTime(100)
    expect(state.isLoading.value).toBe(true)
    finishNew()
    const finishFast = state.beginPageLoad()
    finishFast()
    vi.advanceTimersByTime(100)
    expect(state.isLoading.value).toBe(false)
  })
