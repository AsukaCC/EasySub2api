<template>
  <div v-if="eligible" class="components-account-upstream-billing-rate-cell__panel">
    <HelpTooltip class="components-account-upstream-billing-rate-cell__help-tooltip" width-class="components-account-upstream-billing-rate-cell__state" data-testid="upstream-billing-details">
      <template #trigger>
        <span
          class="components-account-upstream-billing-rate-cell__text"
          :class="hasEffectiveRate ? 'components-account-upstream-billing-rate-cell__text-5' : statusClass || 'components-account-upstream-billing-rate-cell__text-6'"
          data-testid="upstream-billing-rate"
        >
          {{ primaryValue }}
        </span>
      </template>
      <div class="components-account-upstream-billing-rate-cell__panel-2">
        ...
        <p class="components-account-upstream-billing-rate-cell__description" data-testid="upstream-billing-probe-state">
          {{ t('admin.accounts.upstreamBilling.accountProbeState') }}
          <span :class="probeEnabled ? 'status-text--success' : 'components-account-upstream-billing-rate-cell__text-2'">
        ...
        <p
          ...
          class="components-account-upstream-billing-rate-cell__description-2"
          data-testid="upstream-billing-global-probe-state"
        >
          {{ t('admin.accounts.upstreamBilling.globalProbeState') }}
          <span class="components-account-upstream-billing-rate-cell__text-2">{{ t('admin.accounts.upstreamBilling.disabled') }}</span>
        </p>
      </div>
    </HelpTooltip>
    <span v-if="hasEffectiveRate && statusLabel" :class="statusClass" class="components-account-upstream-billing-rate-cell__text-3">
      {{ statusLabel }}
    </span>
    <button
      type="button"
      class="components-account-upstream-billing-rate-cell__action"
      :disabled="probing"
      ...
      @click="$emit('probe')"
    >
      <Icon name="refresh" size="xs" :class="{ 'components-account-upstream-billing-rate-cell__icon': probing }" />
    </button>
  </div>
  <span v-else class="components-account-upstream-billing-rate-cell__text-4">-</span>
</template>
...
// 探测资格已放宽到全部 API-key 平台（上游是 EasySub2api 即可应答）。
...
const currentEffectiveRate = computed(() => {
  ...
  // Peak and legacy independent media multipliers are configuration-only.
  // User billing is group x user, with any time-window discount applied by
  // the transactional billing snapshot rather than this display helper.
  return base
})
...
const statusClass = computed(() => {
  if (!snapshot.value) return 'components-account-upstream-billing-rate-cell__text-6'
  if (snapshot.value.status === 'unsupported') return 'components-account-upstream-billing-rate-cell__state-2'
  if (stale.value) return 'components-account-upstream-billing-rate-cell__state-3'
  if (snapshot.value.status === 'failed') return 'components-account-upstream-billing-rate-cell__state-4'
  return ''
})
