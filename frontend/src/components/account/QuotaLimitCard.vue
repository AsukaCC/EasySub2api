...
<template>
  <div class="components-account-quota-limit-card__panel">
      <!-- Header: toggle + collapse -->
      <div
        class="components-account-quota-limit-card__panel-2"
        :class="{ 'quota-limit-card__body--expanded': localEnabled && !collapsed }"
      >
        <div class="components-account-quota-limit-card__panel-3" @click="localEnabled && (collapsed = !collapsed)">
          <svg v-if="localEnabled" class="components-account-quota-limit-card__icon" :class="{ 'components-account-quota-limit-card__icon-2': collapsed }" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
          </svg>
          <div>
            <label class="components-account-quota-limit-card__label input-label">{{ t('admin.accounts.quotaLimitToggle') }}</label>
            <p class="components-account-quota-limit-card__description">
        ...
        <button
          ...
          :class="[
            'components-account-quota-limit-card__action',
            localEnabled ? 'components-account-quota-limit-card__action-2' : 'components-account-quota-limit-card__action-3'
          ]"
        >
          <span
            :class="[
              'components-account-quota-limit-card__text',
              localEnabled ? 'toggle-thumb--on' : 'components-account-quota-limit-card__text-2'
            ]"
          />
        </button>
      </div>
      ...
      <!-- Collapsible content -->
      <div v-if="localEnabled && !collapsed" class="components-account-quota-limit-card__panel-4">
        <!-- Daily quota -->
        <QuotaDimensionRow
          dim="daily"
