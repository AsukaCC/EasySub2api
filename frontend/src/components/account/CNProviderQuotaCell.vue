<template>
  <div v-if="visible" class="components-account-cnprovider-quota-cell__panel">
    <div class="components-account-cnprovider-quota-cell__panel-2">
      <button
        type="button"
        :class="[
          'components-account-cnprovider-quota-cell__action',
      ...
      >
        <svg
          class="components-account-cnprovider-quota-cell__icon"
          :class="{ 'components-account-cnprovider-quota-cell__icon-2': loading }"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    <div v-if="data?.success && data.tiers?.length" class="components-account-cnprovider-quota-cell__panel">
      <div v-for="tier in data.tiers" :key="tier.window" class="components-account-cnprovider-quota-cell__panel-3">
        <span class="components-account-cnprovider-quota-cell__text">{{ windowLabel(tier.window) }}</span>
        <div class="components-account-cnprovider-quota-cell__panel-4">
          <div
            class="components-account-cnprovider-quota-cell__panel-5"
            :class="utilizationColor(tier.used_percent)"
            :style="{ width: `${Math.min(100, Math.max(0, tier.used_percent))}%` }"
          />
        </div>
        <span :class="['components-account-cnprovider-quota-cell__text-3', utilizationTextColor(tier.used_percent)]">
          {{ Math.round(tier.used_percent) }}%
        </span>
        <span v-if="tier.reset_at" class="components-account-cnprovider-quota-cell__text-2" :title="tier.reset_at">
          · {{ formatReset(tier.reset_at) }}
        </span>
      </div>
    </div>

    <div v-if="error" class="components-account-cnprovider-quota-cell__panel-6" :title="error">
      {{ truncatedError }}
    </div>
  </div>
</template>
...
const utilizationTextColor = (pct: number) => {
  if (pct >= 90) return 'components-account-cnprovider-quota-cell__state'
  if (pct >= 75) return 'components-account-cnprovider-quota-cell__state-2'
  return 'components-account-cnprovider-quota-cell__state-3'
}
