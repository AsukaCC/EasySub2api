...
const ACCOUNT_ID = '018f4a2e-9c6b-7a5d-8e3f-1b2c3d4e5f60'
...
const account = (platform = 'grok', type = 'oauth', extra: Record<string, unknown> = {}) => ({
  id: ACCOUNT_ID, name: 'Grok', notes: '', platform, type,
...
describe('EditAccountModal Grok media eligibility', () => {
  beforeEach(() => {
    ...
    getEligibilityMock.mockResolvedValue({ account_id: ACCOUNT_ID, mode: 'auto', eligible: false, reason: 'billing_inconclusive' })
    updateEligibilityMock.mockImplementation(async (_id: string, mode: string) => ({
      account_id: ACCOUNT_ID, mode, eligible: mode === 'enabled', reason: mode === 'enabled' ? 'override_enabled' : 'override_disabled'
    }))
  })

  it('shows only for Grok OAuth and fills the current decision', async () => {
    const wrapper = mountModal()
    await vi.waitFor(() => expect(getEligibilityMock).toHaveBeenCalledWith(ACCOUNT_ID))
    expect(wrapper.find('[data-testid="grok-media-eligibility-card"]').exists()).toBe(true)
    expect(wrapper.findComponent('[data-testid="grok-media-eligibility-mode"]').props('modelValue')).toBe('auto')
  ...
  it('updates the dedicated endpoint only when the mode changes', async () => {
    ...
    await wrapper.findComponent('[data-testid="grok-media-eligibility-mode"]').setValue('enabled')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
    await vi.waitFor(() => expect(updateEligibilityMock).toHaveBeenCalledWith(ACCOUNT_ID, 'enabled'))
  })
  ...
  it('reports partial-save errors when the dedicated endpoint fails', async () => {
    ...
    const wrapper = mountModal()
    await vi.waitFor(() => expect(getEligibilityMock).toHaveBeenCalled())
    await wrapper.findComponent('[data-testid="grok-media-eligibility-mode"]').setValue('disabled')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
