<template>
  <div v-if="visible" class="components-account-grok-quota-probe-cell__panel">
    <div class="components-account-grok-quota-probe-cell__panel-2">
      <button
        type="button"
        class="components-account-grok-quota-probe-cell__action"
        :disabled="loading"
        :title="t('admin.accounts.usageWindow.grokProbeTooltip')"
        @click="handleProbe"
      >
        <svg
          class="components-account-grok-quota-probe-cell__icon"
          :class="{ 'components-account-grok-quota-probe-cell__icon-2': loading }"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    <div
      ...
      class="components-account-grok-quota-probe-cell__panel-3"
    >
      {{ summary }}
    </div>
    <div v-if="error" class="components-account-grok-quota-probe-cell__panel-4" :title="error">
      {{ truncatedError }}
    </div>
  </div>
</template>
