<template>
  <div class="components-account-account-capacity-cell__panel">
    <!-- 并发槽位 -->
    <CapacityBadge :color-class="concurrencyClass" :current="currentConcurrency" :max="account.concurrency">
      <svg class="components-account-account-capacity-cell__icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <CapacityBadge v-if="showRpmLimit" :color-class="rpmClass" :tooltip="rpmTooltip" :current="currentRPM" :max="account.base_rpm!" :suffix="rpmStrategyTag">
      <svg class="components-account-account-capacity-cell__icon" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
const concurrencyClass = computed(() => {
  ...
  if (current >= max) return 'components-account-account-capacity-cell__state'
  if (current > 0) return 'components-account-account-capacity-cell__state-2'
  return 'components-account-account-capacity-cell__state-3'
})
...
const windowCostClass = computed(() => {
  ...
  if (current >= limit + reserve) return 'components-account-account-capacity-cell__state'
  if (current >= limit) return 'components-account-account-capacity-cell__state-4'
  if (current >= limit * 0.8) return 'components-account-account-capacity-cell__state-2'
  return 'components-account-account-capacity-cell__state-5'
})
...
const sessionLimitClass = computed(() => {
  ...
  if (current >= max) return 'components-account-account-capacity-cell__state'
  if (current >= max * 0.8) return 'components-account-account-capacity-cell__state-2'
  return 'components-account-account-capacity-cell__state-5'
})
...
const rpmClass = computed(() => {
  ...
  if (rpmStrategy.value === 'tiered') {
    if (current >= base + buffer) return 'components-account-account-capacity-cell__state'
    if (current >= base) return 'components-account-account-capacity-cell__state-4'
  } else {
    if (current >= base) return 'components-account-account-capacity-cell__state-4'
  }
  if (current >= base * 0.8) return 'components-account-account-capacity-cell__state-2'
  return 'components-account-account-capacity-cell__state-5'
})
