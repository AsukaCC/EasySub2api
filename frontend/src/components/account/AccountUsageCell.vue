<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <!-- Usage data -->
      <div v-else-if="usageInfo" class="components-account-account-usage-cell__panel-6">
        <!-- API error (degraded response) -->
        <div v-if="usageInfo.error" class="components-account-account-usage-cell__panel-7" :title="usageInfo.error">
          {{ usageInfo.error }}
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
        ...
        <div class="components-account-account-usage-cell__panel-8">
          <span
            v-if="usageInfo.source === 'passive'"
            class="components-account-account-usage-cell__text"
          >
            {{ t('admin.accounts.usageWindow.passiveSampled') }}
          </span>
          <button
            type="button"
            class="components-account-account-usage-cell__action"
            :disabled="activeQueryLoading"
            @click="loadActiveUsage"
          >
            <svg
              class="components-account-account-usage-cell__icon"
              :class="{ 'components-account-account-usage-cell__icon-4': activeQueryLoading }"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
    ...
    <template v-else-if="account.platform === 'openai' && account.type === 'oauth'">
      <div v-if="hasOpenAIUsageFallback" class="components-account-account-usage-cell__panel-6">
        <UsageProgressBar
          v-if="usageInfo?.five_hour"
          label="5h"
          :utilization="usageInfo.five_hour.utilization"
        ...
        <OpenAIQuotaResetCell :account="account" @account-updated="handleQuotaResetAccountUpdated">
          <template #pre-actions>
            <button
              type="button"
              class="components-account-account-usage-cell__action-2"
              :disabled="activeQueryLoading"
              @click="loadActiveUsage"
            >
              <svg
      ...
      <div v-else>
        ...
        <OpenAIQuotaResetCell
          :account="account"
          class="components-account-account-usage-cell__open-aiquota-reset-cell"
          @account-updated="handleQuotaResetAccountUpdated"
        />
      </div>
    </template>

    <!-- Gemini OAuth / service-account quota windows -->
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-else-if="usageInfo" class="components-account-account-usage-cell__panel-6">
        <UsageProgressBar v-if="usageInfo.gemini_shared_daily" label="1d" :utilization="usageInfo.gemini_shared_daily.utilization" :resets-at="usageInfo.gemini_shared_daily.resets_at" color="indigo" />
        <UsageProgressBar v-if="usageInfo.gemini_pro_daily" label="1d Pro" :utilization="usageInfo.gemini_pro_daily.utilization" :resets-at="usageInfo.gemini_pro_daily.resets_at" color="purple" />
        <UsageProgressBar v-if="usageInfo.gemini_flash_daily" label="1d Flash" :utilization="usageInfo.gemini_flash_daily.utilization" :resets-at="usageInfo.gemini_flash_daily.resets_at" color="emerald" />
        <UsageProgressBar v-if="usageInfo.gemini_shared_minute" label="1m" :utilization="usageInfo.gemini_shared_minute.utilization" :resets-at="usageInfo.gemini_shared_minute.resets_at" color="amber" />
        <div v-if="!hasGeminiUsage" class="components-account-account-usage-cell__panel-9">-</div>
      </div>
      <div v-else class="components-account-account-usage-cell__panel-9">-</div>
    </template>

    <!-- Antigravity reports quota per upstream model. -->
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-else-if="error" class="components-account-account-usage-cell__panel-5">{{ error }}</div>
      <div v-else-if="antigravityQuotaBars.length > 0" class="components-account-account-usage-cell__panel-6">
        <UsageProgressBar
          v-for="bar in antigravityQuotaBars"
          :key="bar.model"
          :label="bar.label"
          :title="bar.model"
    ...
    <!-- Grok OAuth accounts: passive xAI quota headers + local EasySub2api usage -->
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="needsReauth" class="components-account-account-usage-cell__panel-6">
        <span class="components-account-account-usage-cell__text-4">
          {{ t('admin.accounts.needsReauth') }}
        </span>
      </div>
      <div v-else-if="isForbidden" class="components-account-account-usage-cell__panel-6">
        <span class="components-account-account-usage-cell__text-6">
      ...
      <div v-else-if="usageInfo" class="components-account-account-usage-cell__panel-6">
        ...
        <template v-if="grokIsFree">
          <UsageProgressBar
            ...
            color="emerald"
          />
          <div v-else-if="grokQuotaUnknown" class="components-account-account-usage-cell__panel-12">
        ...
        <template v-else>
          ...
          <div
            ...
            class="components-account-account-usage-cell__panel-13"
          >
            <span
              v-if="grokPrepaidMoneyLine.showPrepaid"
              class="components-account-account-usage-cell__text-7"
          ...
          <div v-if="grokQuotaUnknown" class="components-account-account-usage-cell__panel-12">
            {{ grokQuotaUnknownLabel }}
          </div>
        </template>
        <div v-if="usageInfo.error" class="components-account-account-usage-cell__panel-14" :title="usageInfo.error">
          {{ usageErrorLabel }}
        </div>
        <div v-if="grokRetryAfterLabel" class="components-account-account-usage-cell__panel-15">
  ...
  <div ref="rootRef" v-else>
    ...
    <div class="components-account-account-usage-cell__panel-6">
      ...
      <div
        v-if="todayStats"
        class="components-account-account-usage-cell__panel-20"
      >
        <div class="components-account-account-usage-cell__panel-21">
          <span class="components-account-account-usage-cell__text-8">
            {{ formatKeyRequests }} req
          </span>
          <span class="components-account-account-usage-cell__text-8">
            {{ formatKeyTokens }}
          </span>
          <span class="components-account-account-usage-cell__text-8" :title="t('usage.accountBilled')">
            A ${{ formatKeyCost }}
          </span>
          <span
            v-if="todayStats.user_cost != null"
            class="components-account-account-usage-cell__text-8"
...
const hasGeminiUsage = computed(() => Boolean(
...
const antigravityQuotaBars = computed(() => {
  const quota = usageInfo.value?.antigravity_quota || {}
  return Object.entries(quota)
    .map(([model, value]) => ({
      model,
      label: model.replace(/^gemini-/, 'G ').replace(/^claude-/, 'C ').slice(0, 18),
...
const formatKeyUserCost = computed(() => {
  return formatPoints(props.todayStats?.user_cost)
})
