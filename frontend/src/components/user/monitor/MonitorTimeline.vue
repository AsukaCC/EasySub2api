<template>
  <div class="components-user-monitor-monitor-timeline__panel">
    <div
      class="components-user-monitor-monitor-timeline__panel-2"
    >
      <span>{{ t('monitorCommon.history60pts', { n: length }) }}</span>
      <span class="components-user-monitor-monitor-timeline__text">{{ t('monitorCommon.nextUpdateIn', { n: countdownSeconds }) }}</span>
    </div>

    <div
      v-if="maintenance"
      class="components-user-monitor-monitor-timeline__panel-3"
    >
      {{ t('monitorCommon.maintenancePaused') }}
    </div>
    <div v-else class="components-user-monitor-monitor-timeline__panel-4">
      <div
        v-for="(bar, idx) in displayBars"
        :key="idx"
        class="components-user-monitor-monitor-timeline__panel-5"
    ...
    <div
      class="components-user-monitor-monitor-timeline__panel-6"
    >
...
const STATUS_COLOR: Record<string, string> = {
  operational: 'status-fill--success',
  degraded: 'status-fill--warning',
  failed: 'status-fill--danger',
  error: 'status-fill--danger',
  empty: 'components-user-monitor-monitor-timeline__state',
}
