<template>
  <div v-if="attributes.length > 0" class="components-user-user-attribute-form__panel">
    <div v-for="attr in attributes" :key="attr.id">
      <label class="input-label">
        {{ attr.name }}
        <span v-if="attr.required" class="components-user-user-attribute-form__text">*</span>
      </label>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="components-user-user-attribute-form__panel-2">
        <label
          v-for="opt in attr.options"
          :key="opt.value"
          class="components-user-user-attribute-form__label"
        >
          <input
            type="checkbox"
            :value="opt.value"
            ...
            class="components-user-user-attribute-form__field"
          />
          <span class="components-user-user-attribute-form__text-2">{{ opt.label }}</span>
        </label>
      </div>
  ...
  <!-- Loading State -->
  <div v-else-if="loading" class="components-user-user-attribute-form__panel-3">
    <svg class="components-user-user-attribute-form__icon" fill="none" viewBox="0 0 24 24">
      <circle class="components-user-user-attribute-form__circle" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
      <path class="components-user-user-attribute-form__path" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
    </svg>
  </div>
</template>
...
const isOptionSelected = (attrId: string, optionValue: string): boolean => {
...
const toggleMultiSelectOption = (attrId: string, optionValue: string) => {
  let arr: string[] = []
  const value = localValues.value[attrId]
  if (value) {
    try {
