<template>
  <div v-if="hasProviders" class="components-auth-email-oauth-buttons__panel">
    <div v-if="showDivider" class="components-auth-email-oauth-buttons__panel-2">
      <div class="components-auth-email-oauth-buttons__panel-3"></div>
      <span class="components-auth-email-oauth-buttons__text">
    ...
    <div :class="providerGridClass">
      <button
        ...
        type="button"
        :disabled="disabled"
        class="components-auth-email-oauth-buttons__action btn btn-secondary"
        @click="startLogin(provider)"
      >
        <GitHubMark v-if="provider === 'github'" class="components-auth-email-oauth-buttons__git-hub-mark" />
        <GoogleMark v-else class="components-auth-email-oauth-buttons__google-mark" />
        <span class="components-auth-email-oauth-buttons__text-2">{{ providerLabel(provider) }}</span>
      </button>
    </div>
  </div>
</template>
...
const providerGridClass = computed(() => [
  'email-oauth-buttons__grid',
  { 'email-oauth-buttons__grid--multiple': hasMultipleProviders.value }
])
