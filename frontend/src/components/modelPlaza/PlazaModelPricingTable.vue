<template>
  <div class="components-model-plaza-plaza-model-pricing-table__panel plaza-pricing-table" :style="accentStyle">
    <table class="components-model-plaza-plaza-model-pricing-table__table">
      <colgroup>
        <col class="components-model-plaza-plaza-model-pricing-table__col" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-2" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-2" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-3" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-2" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-2" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-3" />
        <col class="components-model-plaza-plaza-model-pricing-table__col-2" />
      ...
      <thead>
        <tr
          class="components-model-plaza-plaza-model-pricing-table__row"
        >
          <th
            rowspan="2"
            class="components-model-plaza-plaza-model-pricing-table__heading"
          >
            {{ t('modelPlaza.table.model') }}
          </th>
          <th colspan="3" class="components-model-plaza-plaza-model-pricing-table__heading-2 pz-bg">
            <div class="components-model-plaza-plaza-model-pricing-table__panel-2 pz-title">
              {{ t('modelPlaza.table.paidPrice') }}
              <span class="components-model-plaza-plaza-model-pricing-table__text pz-unit">{{ t('modelPlaza.table.unitPerMillion') }}</span>
            </div>
          </th>
          <th
            colspan="3"
            class="components-model-plaza-plaza-model-pricing-table__heading-3"
          >
            <div class="components-model-plaza-plaza-model-pricing-table__panel-3">
              {{ t('modelPlaza.table.officialPrice') }}
              <span class="components-model-plaza-plaza-model-pricing-table__text-2">{{ t('modelPlaza.table.unitPerMillion') }}</span>
            </div>
          </th>
          <th
            rowspan="2"
            class="components-model-plaza-plaza-model-pricing-table__heading-4"
          >
            {{ t('modelPlaza.table.rate') }}
          </th>
        </tr>
        <tr
          class="components-model-plaza-plaza-model-pricing-table__row-2"
        >
          <th class="components-model-plaza-plaza-model-pricing-table__heading-5 pz-bg">{{ t('modelPlaza.table.input') }}</th>
          <th class="components-model-plaza-plaza-model-pricing-table__heading-5 pz-bg">{{ t('modelPlaza.table.output') }}</th>
          <th class="components-model-plaza-plaza-model-pricing-table__heading-5 pz-bg">{{ t('modelPlaza.table.cache') }}</th>
          <th class="components-model-plaza-plaza-model-pricing-table__heading-6">
            {{ t('modelPlaza.table.input') }}
          </th>
          <th class="components-model-plaza-plaza-model-pricing-table__heading-5">{{ t('modelPlaza.table.output') }}</th>
          <th class="components-model-plaza-plaza-model-pricing-table__heading-5">{{ t('modelPlaza.table.cache') }}</th>
        </tr>
      </thead>
      <tbody>
        <tr
          ...
          class="components-model-plaza-plaza-model-pricing-table__row-3"
        >
          <!-- 模型名 + 非 token 计费模式徽章 -->
          <td class="components-model-plaza-plaza-model-pricing-table__cell">
            <div class="components-model-plaza-plaza-model-pricing-table__panel-4">
              <span class="components-model-plaza-plaza-model-pricing-table__text-3">{{ m.name }}</span>
              <span
                v-if="platform && m.platform !== platform"
                :class="[
                  'components-model-plaza-plaza-model-pricing-table__text-14',
              ...
              <span
                ...
                class="components-model-plaza-plaza-model-pricing-table__text-4"
              >
                {{ billingModeLabel(m) }}
              </span>
            </div>
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="components-model-plaza-plaza-model-pricing-table__cell-2 pz-cell">
          ...
          <template v-else>
            <td colspan="3" class="components-model-plaza-plaza-model-pricing-table__cell-3 pz-cell">
              <div
                v-if="requestIntervals(m).length"
                class="components-model-plaza-plaza-model-pricing-table__panel-4"
              >
                <span
                  v-for="(iv, idx) in requestIntervals(m)"
                  :key="idx"
                  class="components-model-plaza-plaza-model-pricing-table__text-7"
                >
                  <span class="components-model-plaza-plaza-model-pricing-table__text-8">{{ tierLabel(iv) }}</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  }}<span class="components-model-plaza-plaza-model-pricing-table__text-8">{{ perUnitSuffix(m) }}</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                <span class="components-model-plaza-plaza-model-pricing-table__text-9">
                  {{ paidRequestPrice(m, m.pricing.per_request_price) }}
                </span>
                <span class="components-model-plaza-plaza-model-pricing-table__text-10">{{ perUnitSuffix(m) }}</span>
              </template>
              <span v-else class="components-model-plaza-plaza-model-pricing-table__text-6">-</span>
            </td>
          </template>
          ...
          <td
            class="components-model-plaza-plaza-model-pricing-table__cell-4"
          >
            {{ official(m.official_pricing?.input_price) }}
          </td>
          <td class="components-model-plaza-plaza-model-pricing-table__cell-5">
            {{ official(m.official_pricing?.output_price) }}
          </td>
          <td class="components-model-plaza-plaza-model-pricing-table__cell-3">
            <div
              v-if="m.official_pricing && hasOfficialCache(m.official_pricing)"
              class="components-model-plaza-plaza-model-pricing-table__panel-7"
            >
              <div>
                <span class="components-model-plaza-plaza-model-pricing-table__text-5">{{ t('modelPlaza.table.cacheWrite') }}</span>
                {{ official(m.official_pricing.cache_write_price)
                }}<template v-if="m.official_pricing.cache_write_1h_price != null"
                  ><span class="components-model-plaza-plaza-model-pricing-table__text-8"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
