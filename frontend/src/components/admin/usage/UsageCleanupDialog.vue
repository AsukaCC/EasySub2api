<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="components-admin-usage-usage-cleanup-dialog__panel">
      <UsageFilters
      ...
      <div class="components-admin-usage-usage-cleanup-dialog__panel-2">
        {{ t('admin.usage.cleanup.warning') }}
      </div>

      <div class="components-admin-usage-usage-cleanup-dialog__panel-3">
        <div class="components-admin-usage-usage-cleanup-dialog__panel-4">
          <h4 class="components-admin-usage-usage-cleanup-dialog__heading">
        ...
        <div class="components-admin-usage-usage-cleanup-dialog__panel-5">
          <div v-if="tasksLoading" class="components-admin-usage-usage-cleanup-dialog__panel-6">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="components-admin-usage-usage-cleanup-dialog__panel-6">
            {{ t('admin.usage.cleanup.noTasks') }}
          </div>
          <div v-else class="components-admin-usage-usage-cleanup-dialog__panel-7">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="components-admin-usage-usage-cleanup-dialog__panel-8"
            >
              <div class="components-admin-usage-usage-cleanup-dialog__panel-9">
                <div class="components-admin-usage-usage-cleanup-dialog__panel-10">
                  <span :class="statusClass(task.status)" class="components-admin-usage-usage-cleanup-dialog__text">
                    {{ statusLabel(task.status) }}
                  </span>
                  <span class="components-admin-usage-usage-cleanup-dialog__text-2">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
                    class="components-admin-usage-usage-cleanup-dialog__action btn btn-ghost btn-xs"
                    @click="openCancelConfirm(task)"
                  >
                    {{ t('admin.usage.cleanup.cancel') }}
                  </button>
                </div>
                <div class="components-admin-usage-usage-cleanup-dialog__text-2">
                  {{ formatDateTime(task.created_at) }}
                </div>
              </div>
              <div class="components-admin-usage-usage-cleanup-dialog__panel-11">
              ...
              <div v-if="task.error_message" class="components-admin-usage-usage-cleanup-dialog__panel-12">
                {{ task.error_message }}
              </div>
            </div>
          </div>
        ...
        <Pagination
          ...
          class="components-admin-usage-usage-cleanup-dialog__pagination"
          :total="tasksTotal"
          :page="tasksPage"
    ...
    <template #footer>
      <div class="components-admin-usage-usage-cleanup-dialog__panel-13">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    pending: 'components-admin-usage-usage-cleanup-dialog__state',
    running: 'components-admin-usage-usage-cleanup-dialog__state-2',
    succeeded: 'components-admin-usage-usage-cleanup-dialog__state-3',
    failed: 'components-admin-usage-usage-cleanup-dialog__state-4',
    canceled: 'components-admin-usage-usage-cleanup-dialog__state-5'
  }
  return map[status] || 'components-admin-usage-usage-cleanup-dialog__state-6'
}
