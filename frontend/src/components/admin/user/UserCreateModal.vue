<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <form id="create-user-form" @submit.prevent="submit" class="components-admin-user-user-create-modal__form">
      <div>
      ...
      <div>
        ...
        <div class="components-admin-user-user-create-modal__panel">
          <div class="components-admin-user-user-create-modal__panel-2">
            <input v-model="form.password" type="text" required class="components-admin-user-user-create-modal__field input" :placeholder="t('admin.users.enterPassword')" />
          </div>
          <button type="button" @click="generateRandomPassword" class="components-admin-user-user-create-modal__action btn btn-secondary">
      ...
      <div class="components-admin-user-user-create-modal__panel-3">
        <div>
	  ...
	  <div v-if="form.balance !== ''">
		<label class="input-label">{{ t('admin.users.balanceType') }}</label>
		<Select v-model="form.balance_type" :options="[
...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user' as 'user' | 'admin', balance: '', balance_type: 'bonus' as 'recharge' | 'bonus', bonus_validity_days: 90, concurrency: 1, rpm_limit: 0 })
...
watch(() => props.show, (v) => { if(v) Object.assign(form, { email: '', password: '', username: '', notes: '', role: 'user', balance: '', balance_type: 'bonus', bonus_validity_days: 90, concurrency: 1, rpm_limit: 0 }) })
