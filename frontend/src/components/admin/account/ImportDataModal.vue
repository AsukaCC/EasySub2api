<template>
  <BaseDialog
    ...
    @close="handleClose"
  >
    <form id="import-data-form" class="components-admin-account-import-data-modal__form" @submit.prevent="handleImport">
      <div class="components-admin-account-import-data-modal__panel">
        {{ t('admin.accounts.dataImportHint') }}
      </div>
      <div
        class="components-admin-account-import-data-modal__panel-2"
      >
      ...
      <div>
        ...
        <div
          class="components-admin-account-import-data-modal__panel-3"
          :class="dragActive
            ? 'components-admin-account-import-data-modal__panel-15'
            : 'components-admin-account-import-data-modal__panel-16'"
          @dragenter.prevent="handleDragEnter"
          @dragover.prevent
          @dragleave.prevent="handleDragLeave"
          @drop.prevent="handleDrop"
        >
          <div class="components-admin-account-import-data-modal__panel-4">
            <div class="components-admin-account-import-data-modal__panel-5" :title="fileListTitle">
              {{ selectedFilesLabel || t('admin.accounts.dataImportSelectFile') }}
            </div>
            <div class="components-admin-account-import-data-modal__panel-6">
              JSON (.json)
              <span v-if="files.length > 1"> · {{ fileListTitle }}</span>
            </div>
          </div>
          <button type="button" class="components-admin-account-import-data-modal__action btn btn-secondary" @click="openFilePicker">
            {{ t('common.chooseFile') }}
          </button>
        </div>
        <input
          ref="fileInput"
          type="file"
          class="components-admin-account-import-data-modal__field"
      ...
      <div
        v-if="result"
        class="components-admin-account-import-data-modal__panel-7"
      >
        <div class="components-admin-account-import-data-modal__panel-8">
          {{ t('admin.accounts.dataImportResult') }}
        </div>
        <div class="components-admin-account-import-data-modal__panel-9">
          {{ t('admin.accounts.dataImportResultSummary', result) }}
        </div>

        <div v-if="errorItems.length" class="components-admin-account-import-data-modal__panel-10">
          <div class="components-admin-account-import-data-modal__panel-11">
            {{ t('admin.accounts.dataImportErrors') }}
          </div>
          <div
            class="components-admin-account-import-data-modal__panel-12"
          >
            <div v-for="(item, idx) in errorItems" :key="idx" class="components-admin-account-import-data-modal__panel-13">
    ...
    <template #footer>
      <div class="components-admin-account-import-data-modal__panel-14">
...
const SUPPORTED_DATA_TYPES = ['easysub2api-data', 'sub2api-data', 'sub2api-bundle']
