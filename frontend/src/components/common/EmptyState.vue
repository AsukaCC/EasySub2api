<template>
  <div class="empty-state">
    <!-- Icon -->
    <div class="empty-state__icon-surface">
      <slot name="icon">
        <component v-if="icon" :is="icon" class="empty-state__icon empty-state-icon" aria-hidden="true" />
        <svg
          v-else
          class="empty-state__icon empty-state-icon"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          stroke-width="1.5"
    ...
    <div v-if="actionText || $slots.action" class="empty-state__actions">
      <slot name="action">
        ...
        >
          <Icon v-if="actionIcon" name="plus" size="md" class="empty-state__action-icon" />
          {{ actionText }}
        </component>
      </slot>
    </div>
