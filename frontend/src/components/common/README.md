...
This directory contains reusable Vue 3 components built with Composition API, TypeScript, and semantic SCSS.

## Components

### DataTable.vue

A generic data table component with sorting, resizable columns, loading states, and custom cell rendering.
...
- `resizableColumns?: boolean` - Enable desktop column resizing (default: `true`)
- `columnWidthStorageKey?: string` - Persist custom widths; falls back to a key derived from `sortStorageKey`
- `minColumnWidth?: number` - Minimum user-selected width in pixels (default: `72`)
...
- **Semantic SCSS styling** for consistent design
