<template>
  <BaseDialog
    ...
    @close="emit('close')"
  >
    <div class="components-keys-use-key-modal__panel">
      <!-- No Group Assigned Warning -->
      <div v-if="!platform" class="components-keys-use-key-modal__panel-2">
        <svg class="components-keys-use-key-modal__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
        </svg>
        <div>
          <p class="components-keys-use-key-modal__description">
            {{ t('keys.useKeyModal.noGroupTitle') }}
          </p>
          <p class="components-keys-use-key-modal__description-2">
      ...
      <template v-else>
        <!-- Description -->
        <p class="components-keys-use-key-modal__description-3">
          {{ platformDescription }}
        </p>

        <!-- Client Tabs -->
        <div v-if="clientTabs.length" class="components-keys-use-key-modal__panel-3">
          <nav class="components-keys-use-key-modal__navigation" aria-label="Client">
            <button
              ...
              :class="[
                'components-keys-use-key-modal__action-2',
                activeClientTab === tab.id
                  ? 'components-keys-use-key-modal__action-3'
                  : 'components-keys-use-key-modal__action-4'
              ]"
            >
              <span class="components-keys-use-key-modal__text">
                <component :is="tab.icon" class="components-keys-use-key-modal__component" />
                {{ tab.label }}
              </span>
            </button>
          </nav>
        ...
        <div
          ...
          class="components-keys-use-key-modal__panel-4"
        >
          <div class="components-keys-use-key-modal__panel-5">
            <p class="components-keys-use-key-modal__description-4">
              {{ t('keys.useKeyModal.openai.authModeTitle') }}
            </p>
            <p class="components-keys-use-key-modal__description-5">
              {{ t('keys.useKeyModal.openai.authModeDescription') }}
            </p>
          </div>
          <div
            class="components-keys-use-key-modal__panel-6"
            role="radiogroup"
          ...
          >
            <button
              ...
              :class="[
                'components-keys-use-key-modal__action-5',
                codexAuthMode === 'legacy'
                  ? 'components-keys-use-key-modal__action-6'
                  : 'components-keys-use-key-modal__action-7'
              ]"
          ...
          <div
            ...
            class="components-keys-use-key-modal__panel-7"
          >
            <Icon name="exclamationCircle" size="sm" class="components-keys-use-key-modal__icon-2" />
        ...
        <div class="components-keys-use-key-modal__panel">
          <div
            ...
            :key="index"
            class="components-keys-use-key-modal__panel-8"
          >
            <!-- File Hint (if exists) -->
            <p v-if="file.hint" class="components-keys-use-key-modal__description-6">
              <Icon name="exclamationCircle" size="sm" class="components-keys-use-key-modal__icon-3" />
              {{ file.hint }}
            </p>
            <div class="components-keys-use-key-modal__panel-9">
              <!-- Code Header -->
              <div class="components-keys-use-key-modal__panel-10">
                <span class="components-keys-use-key-modal__text-2">{{ file.path }}</span>
                <button
                  type="button"
                  @click="copyContent(file.content, index)"
                  class="components-keys-use-key-modal__action"
                  :class="copiedIndex === index
                    ? 'components-keys-use-key-modal__action-8'
                    : 'components-keys-use-key-modal__action-9'"
                >
                  <svg v-if="copiedIndex === index" class="components-keys-use-key-modal__icon-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <svg v-else class="components-keys-use-key-modal__icon-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              ...
              <!-- Code Content -->
              <pre class="components-keys-use-key-modal__pre"><code v-if="file.highlighted" v-html="file.highlighted"></code><code v-else v-text="file.content"></code></pre>
            </div>
          </div>
        </div>

        <!-- Usage Note -->
        <div v-if="showPlatformNote" class="components-keys-use-key-modal__panel-11">
          <Icon name="infoCircle" size="md" class="components-keys-use-key-modal__icon-5" />
          <p class="components-keys-use-key-modal__description-7">
            {{ platformNote }}
          </p>
        </div>
      </template>
    ...
    <template #footer>
      <div class="components-keys-use-key-modal__panel-12">
        <button
          @click="emit('close')"
...
const AppleIcon = {
  render() {
    return h('svg', {
      fill: 'currentColor',
      viewBox: '0 0 24 24',
      class: 'components-keys-use-key-modal__component'
    }, [
...
const currentFiles = computed((): FileConfig[] => {
  ...
  if (activeClientTab.value === 'opencode') {
    switch (props.platform) {
      ...
      case 'antigravity':
        return [generateOpenCodeConfig('anthropic', apiBase, apiKey, undefined, true)]
      case 'openai':
...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready EasySub2api setups.
  ...
  const configContent = `# Grok Build CLI → EasySub2api Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for EasySub2api Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches EasySub2api).
...
description = "Grok 4.5 via EasySub2api (Responses)"
...
# Imagine tools: model IDs go to EasySub2api media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set EASYSUB2API_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:EASYSUB2API_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export EASYSUB2API_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → EasySub2api Grok group
...
[model_providers.easysub2api]
name = "EasySub2api Grok"
base_url = "${baseUrl}"
# Prefer env_key (variable NAME). Do not combine with experimental_bearer_token.
env_key = "EASYSUB2API_API_KEY"
...
# Grok/EasySub2api path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(
  ...
  apiKey: string,
  pathLabel?: string,
  includeAntigravityClaudeModels = false
): FileConfig {
  ...
  if (platform === 'anthropic') {
    ...
    if (includeAntigravityClaudeModels) {
      provider[platform].models = claudeModels
    }
  } else if (platform === 'openai') {
    provider[platform].models = openaiModels
  } else if (platform === 'grok') {
    // Custom provider pointing at EasySub2api OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via EasySub2api'
