...
const failures = ref<Array<{ id: string; name: string; message: string }>>([])
...
const errorMessage = (error: unknown): string => {
  ...
  const detail = candidate?.response?.data?.detail ?? candidate?.response?.data?.message ?? candidate?.message
  return typeof detail === 'string' && detail ? detail : t('keys.failedToSave')
}
