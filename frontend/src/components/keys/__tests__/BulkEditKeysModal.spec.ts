...
const mountModal = () => mount(BulkEditKeysModal, {
  props: {
    show: true,
    selectedKeys: [
      { id: 'key-first', name: 'First' },
      { id: 'key-second', name: 'Second' }
    ],
    groups: [{ id: 'group-available', name: 'Available group' }] as Group[]
  },
  global: {
    stubs: {
      BaseDialog: {
...
describe('BulkEditKeysModal', () => {
  beforeEach(() => {
    ...
    bulkUpdate.mockResolvedValue({
      succeededIds: ['key-first', 'key-second'],
      failures: []
    })
  })
  ...
  it('requires an explicit field choice and sends only that field', async () => {
    ...
    expect(wrapper.emitted('updated')).toEqual([[['key-first', 'key-second']]])
  ...
  it('requires an available string group ID when changing group', async () => {
    const wrapper = mountModal()
    await wrapper.get('[data-test="enable-group"]').setValue(true)
    await wrapper.get('form').trigger('submit')
    expect(bulkUpdate).not.toHaveBeenCalled()

    await wrapper.get('[data-test="group-input"]').setValue('group-available')
    await wrapper.get('form').trigger('submit')

    expect(bulkUpdate).toHaveBeenCalledWith(
      ['key-first', 'key-second'],
      { group_id: 'group-available' }
    )
  })
  ...
  it('normalizes IP lines and converts the chosen local date to ISO', async () => {
    ...
    expect(bulkUpdate).toHaveBeenCalledWith(['key-first', 'key-second'], {
      ip_blacklist: ['192.0.2.1', '198.51.100.0/24'],
      expires_at: new Date('2030-01-02T03:04').toISOString()
    })
  })

  it('reports individual failures and retries only failed string IDs', async () => {
    bulkUpdate.mockResolvedValueOnce({
      succeededIds: ['key-first'],
      failures: [{
        id: 'key-second',
        error: { response: { data: { detail: 'Group access denied' } } }
      }]
    }).mockResolvedValueOnce({ succeededIds: ['key-second'], failures: [] })
    const wrapper = mountModal()
    await wrapper.get('[data-test="enable-quota"]').setValue(true)
    ...
    expect(wrapper.text()).toContain('#key-second Second: Group access denied')
    expect(wrapper.emitted('updated')).toEqual([[['key-first']]])
    ...
    await wrapper.setProps({
      selectedKeys: [
        { id: 'key-second', name: 'Second' },
        { id: 'key-new', name: 'New selection' }
      ]
    })
    await wrapper.get('form').trigger('submit')
    await flushPromises()

    expect(bulkUpdate).toHaveBeenLastCalledWith(['key-second'], { quota: 25.5 })
    expect(wrapper.emitted('updated')).toEqual([[['key-first']], [['key-second']]])
    expect(wrapper.emitted('close')).toHaveLength(1)
  })

  it('prevents duplicate submissions and closing during an update', async () => {
    let finish!: (result: { succeededIds: string[]; failures: [] }) => void
    ...
    finish({ succeededIds: ['key-first', 'key-second'], failures: [] })
    await flushPromises()
  })
  ...
  it('resets all field choices when reopened for a new selection', async () => {
    ...
    await wrapper.setProps({
      show: true,
      selectedKeys: [{ id: 'key-third', name: 'Third' }]
    })
    ...
    expect(bulkUpdate).toHaveBeenCalledWith(['key-third'], { status: 'active' })
  })
})
