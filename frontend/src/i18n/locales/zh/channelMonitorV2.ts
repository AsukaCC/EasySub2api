...
export default {
  channelMonitorV2: {
    ...
    bootstrap: {
      title: '正在补齐历史监控数据',
      description:
        '首次启用被动监控时，系统会在后台静默聚合 90 分钟、24 小时、7 天、14 天与 30 天窗口；完成后可切换全部时间范围。',
      progress: '进度 {percent}%',
      working: '后台聚合中…',
    },
    timeRange: '时间范围',
    ...
    currentUser: '当前用户',
    compact: {
      availability: '{value}% 可用率',
      errorRate: '错误率 {value}%',
      noData: '暂无数据',
      unnamedGroup: '未命名分组',
      timelineAria: '{group} 最近 14 天错误率，每格 2 小时',
      noTrafficAt: '{time} 暂无请求',
      noTraffic: '暂无请求',
      bucketAt: '{time} 错误率 {value}%',
      tooltipErrorRate: '错误率 {value}%',
      requestCount: '请求数 {value}',
      errorRequests: '错误请求 {value}',
      emptyTitle: '暂无监控分组',
      emptyDescription: '请管理员在渠道监控配置中选择需要展示的分组。',
      loadFailed: '渠道状态加载失败',
      pageTitle: '渠道状态',
      pageDescription: '最近 14 天各渠道分组的错误率，每格 2 小时',
      autoRefresh: '每分钟自动刷新',
      timeAxisNow: '现在',
      legendAria: '错误率图例',
      legendLowError: '低',
    ...
    matrix: {
      title: '可用性趋势', description: '每行是一种渠道组合，每个色块代表一个统计区间；悬停查看明细', wheelZoom: '在色块上滚轮放大（区间变窄、色块变宽）', wheelZoomX: '在色块上滚轮放大（区间变窄、色块变宽）', dimension: '渠道维度', emptyTitle: '当前筛选窗口没有矩阵数据', legendAria: '错误率图例', bad: '高错误率', good: '低错误率', healthyLegend: '低错误率', warningLegend: '中等错误率', criticalLegend: '高错误率', unknownLegend: '无流量 / 样本不足', noTraffic: '该区间无流量', noTrafficAt: '{time} · 无流量', scoreLine: '健康分 {score}', resetZoom: '重置缩放'
    },
    chart: {
    ...
    settings: {
      title: '被动监控配置',
      description:
        '配置基于真实网关流量的汇总维度、刷新频率和健康阈值。',
      save: '保存',
      loading: '加载中...',
      loadFailed: '被动监控配置加载失败',
      saveSuccess: '被动监控配置已保存',
      saveFailed: '被动监控配置保存失败',
      modeBanner:
      ...
      healthTitle: '健康阈值',
      healthHint: '控制汇总指标与明细状态。默认阈值较宽松，避免少量错误或低缓存率立即显示异常。',
      fields: {
        minimumSample: '最小样本数',
        warningError: '错误率关注 %',
        criticalError: '错误率异常 %',
