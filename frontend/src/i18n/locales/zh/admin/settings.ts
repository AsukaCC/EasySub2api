export default {
    settings: {
      ...
      systemUpdates: {
        title: '系统更新',
        description: '检查新版本、查看更新日志，并管理发布版本的更新、重启和回退。',
        check: '检查更新',
        checking: '检查中...',
        checkFailed: '检查更新失败',
        releaseBuild: '发布构建',
        sourceBuild: '源码构建',
        loadRollback: '加载可回退版本',
        rollbackPrompt: '点击“加载可回退版本”查看最近的发布版本。',
        rollbackDescription: '仅发布构建支持在线回退；源码构建请使用 git 管理版本。',
      },
      featureManagement: {
        title: '功能管理',
        description: '集中控制功能运行和用户发布状态。先启用并完成配置，再向用户开放。',
        running: '运行中',
        stopped: '已停用',
        published: '已发布',
        hidden: '未发布',
        ...
        moduleDescriptions: {
          ...
          supportTickets: '统一处理账号问题和退款申请；开关仅限制新建，不影响已有工单。',
        },
        opsSettings: {
          title: '运维监控设置',
          description: '配置监控数据采集方式与现有运维运行参数。功能总开关请在功能管理页操作。',
          openDashboard: '打开监控面板',
          realtime: '实时监控',
          realtimeHint: '启用实时并发和流量信息采集。',
          queryMode: '默认查询模式',
          metricsInterval: '指标采集间隔（秒）',
          advanced: '高级运维设置',
          intervalError: '指标采集间隔必须为 1 到 3600 秒的整数',
        },
        riskSettings: {
          ttlError: '屏蔽时长必须为 1 到 604800 秒的整数',
        },
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: '基于真实网关流量被动汇总错误率、延迟、缓存率和吞吐趋势，不会向上游发送探活请求。',
          configureLink: '打开被动监控配置',
          openDashboard: '查看监控看板',
          enabled: '启用渠道监控',
          enabledHint: '关闭后停止被动数据聚合并隐藏用户入口；已有配置与历史保留。',
        ...
        affiliate: {
          ...
          recipientHint: '决定新产生的返利积分进入邀请方还是受邀请方账户；历史返利仍按产生时的接收方退款和结算。',
          freezeHours: '返利固定冻结期',
          hoursUnit: '小时（7 天）',
      ...
      apiKeyAcl: {
        ...
        trustForwardedIpHint:
          '默认关闭并严格使用 server.trusted_proxies 配置的 Gin 可信代理链。开启后 CF-Connecting-IP、X-Real-IP 或 X-Forwarded-For 会直接接管客户端 IP 解析并覆盖 trusted_proxies；仅在源站无法被直接访问且代理会覆盖这些请求头时显式开启。切换会改变现有会话的 IP 指纹。',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 EasySub2api 用户登录',
      ...
      defaults: {
        title: '用户默认设置',
        description: '新用户的默认值',
        defaultBalance: '默认平台积分',
        defaultBalanceHint: '新用户的初始平台积分',
		bonusValidityDays: '赠送积分默认有效期（天）',
		bonusValidityDaysHint: '注册、认证来源、优惠码、兑换码和返利未单独配置时使用此有效期',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，EasySub2api 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游 EasySub2api 站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      site: {
        title: '站点设置',
        description: '按业务模块配置站点运行、品牌、链接和展示内容。',
        sections: {
          runtime: {
            title: '运行模式',
            description: '控制平台是否进入仅管理员可用的后台模式。',
          },
          branding: {
            title: '站点品牌',
            description: '配置站点名称、标识和用户联系信息。',
          },
          links: {
            title: '接口与外部链接',
            description: '设置 API 地址和文档入口，便于用户复制与访问。',
          },
          home: {
            title: '首页展示',
            description: '配置首页内容和用户侧的展示偏好。',
          },
          tables: {
            title: '表格与自定义端点',
            description: '统一设置分页行为，并维护用户可复制的 API 端点。',
          },
          menu: {
            title: '自定义菜单',
            description: '管理侧边栏中的自定义页面及其可见角色。',
          },
        },
        ...
        themeAccent: '主题强调色',
        themeAccentHint: '用于主按钮、链接和焦点环。不改变亮暗模式或玻璃底色。',
        themeAccentCustom: '自定义',
        siteSubtitle: '站点副标题',
      ...
      payment: {
        ...
        maxAmount: '最高金额',
        dailyLimit: '每日限额',
        pointsConversion: '充值积分兑换比例',
        onePlatformPoint: '1 平台积分',
        pointsConversionHint: '兑换比例固定为 1:1，不参与消费倍率计算；支付手续费单独计入充值订单。',
        bonusTiers: '充值奖励档位',
        bonusTiersHint: '按人民币充值本金匹配，只发放达到条件的最高一档赠送积分，不叠加。',
        addBonusTier: '添加档位',
        noBonusTiers: '未配置充值奖励',
        bonusThreshold: '充值门槛（RMB）',
        bonusPoints: '赠送积分',
        invalidBonusTier: '第 {row} 档格式无效：充值门槛最多 2 位小数，赠送积分最多 8 位小数，且均不能为负数。',
        duplicateBonusThreshold: '充值门槛 {amount} RMB 重复，请调整后再保存。',
        ...
        field_paymentCurrencyHint: '仅 CNY 渠道可用于平台积分充值；其他币种配置仅为兼容保留，不参与充值流程。',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 EasySub2api 订单中，上游 type 会作为易支付 type 参数提交。',
      ...
      balanceNotify: {
        title: '积分不足提醒',
        description: '当用户平台积分低于阈值时发送邮件提醒',
        enabled: '启用积分不足提醒',
        threshold: '默认提醒阈值',
      ...
      authSourceDefaults: {
        title: '认证来源默认值',
        description: '按注册来源配置新用户默认平台积分、并发、订阅与授权策略。',
        requireEmailLabel: '第三方注册强制补充邮箱',
        requireEmailHint: '启用后，OIDC、微信注册缺少邮箱时必须先补充邮箱地址。',
        enabledHint: '以下默认值会在该来源注册新用户时发放；首次绑定时授权仅作用于已有账号绑定该来源。',
		bonusValidityOverrideHint: '填写 0 表示沿用全局赠送有效期。',
        sources: {
          email: {
            title: '邮箱注册',
