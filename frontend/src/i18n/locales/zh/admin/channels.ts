export default {
    ...
    channels: {
      ...
      form: {
        ...
        reasoningEffortMultipliers: '推理力度倍率',
        inheritReasoningEffortMultipliers: '继承模型默认倍率',
        reasoningEffortMultiplierLabel: '{effort} 倍率',
        reasoningEffortMultiplierDefault: '1',
        clearReasoningEffortMultipliers: '清空倍率',
        reasoningEffortMultiplierPositive: '{effort} 倍率必须为大于零的有限数字',
        reasoningEffortLevelInvalid: '不支持的推理力度：{effort}',
        multiplierPlaceholder: '可选，填写正数倍率',
        fable51DefaultMaxReasoningMultiplier: 'Claude Fable 5.1 未配置时默认使用 3.0',
        multiplierPositive: '推理强度倍率必须为正数',
        platformConfig: '平台配置',
    ...
    subscriptions: {
      ...
      bulk: {
        ...
        restore: '批量恢复',
        clearSelection: '清除选择',
        confirmTargets: '将对 {count} 个订阅执行此操作',
        groupFallback: '分组 #{id}',
        extendHint: '正数延长，负数缩短；调整后订阅必须仍未过期。',
        resetWindows: '选择要重置的额度窗口',
        resetHint: '所选窗口的已用额度将清零，并从当前规则对应的时间重新计算。',
        revokeHint: '所选订阅将立即停止生效，之后仍可从已撤销列表恢复。',
        restoreHint: '仅恢复不存在同平台冲突或待生效权益的已撤销订阅。',
        selectionRequired: '请至少选择一个可操作的订阅',
        selectionLimit: '每次最多处理 100 个订阅',
        invalidDays: '调整天数必须是 -36500 到 36500 之间的非零整数',
        selectWindow: '请至少选择一个额度窗口',
        retryHint: '结果可能已部分写入，请使用同一操作重试以获取确定结果。',
        result: '成功 {success} 个，失败 {failed} 个',
        itemFailed: '操作失败',
        retry: '使用同一请求重试',
        confirm: '确认执行',
        requestFailed: '批量订阅操作失败'
      },
      batchAssign: {
        enable: '一次分配给多个用户',
        hint: '每次最多选择 100 个用户；每个用户会独立返回成功、排队或失败结果。',
        selected: '已选择 {count} 个用户',
        removeUser: '移除 {email}',
        result: '分配成功或排队 {success} 个，失败 {failed} 个',
        retryHint: '失败用户会保留在列表中，可修正后再次提交。'
      },
      issueResetCards: '发放重置卡',
      issueResetCardsSelected: '为已选择的 {count} 个订阅发放重置卡',
      resetCardQuantity: '发放数量（每个订阅）',
      resetCardValidityDays: '有效期（天）',
      issueResetCardsTotal: '预计发放总数：{count}',
      resetCardQuantityInvalid: '发放数量必须是 1 到 1000 的整数',
      resetCardValidityInvalid: '有效期必须是 1 到 3650 天',
      issueResetCardsPartial: '已为 {success} 个订阅发卡，{failed} 个失败',
      issueResetCardsSuccess: '已发放 {count} 张重置卡',
      issueResetCardsFailed: '发放重置卡失败',
      resetCardsAvailable: '可用 {count} 张',
      resetCardsExpired: '已过期 {count} 张',
      resetCardsNextExpiry: '最近到期：{time}',
      resetWeekly: '直接重置周限额',
      resetWeeklyConfirm: '确定要重置选中的 {count} 个订阅的周限额吗？周用量会清零，新的 7 天周期从现在开始；不会消耗重置卡，也不会改变日/月用量和订阅到期时间。没有周限额的订阅会失败。',
      resetWeeklyConfirmSingle: "确定要重置 '{user}' 的周限额吗？新的 7 天周期从现在开始，不会消耗重置卡。",
      resetWeeklyPartial: '已重置 {success} 个订阅，{failed} 个失败',
      resetWeeklySuccess: '已重置 {count} 个订阅的周限额',
      operationResultTitle: '批量操作结果',
      operationResultIssueTitle: '发放重置卡结果',
      operationResultWeeklyTitle: '直接重置周限额结果',
      operationResultSummary: '成功 {success} 个，失败 {failed} 个',
      operationResultTotalIssued: '共发放 {count} 张',
      operationResultSuccess: '成功',
      operationResultFailed: '失败',
      operationResultIssuedDetail: '发放 {count} 张重置卡',
      operationResultResetDetail: '重置时间：{resetAt}，新周期结束：{endAt}',
      operationResultError: '原因：{reason}',
      quotaResetSuccess: '配额重置成功',
      failedToResetQuota: '重置配额失败',
      failedToResetWeekly: '直接重置周限额失败',
      ...
      guide: {
        ...
        actions: {
          ...
          resetWeekly: '直接重置周限额',
          resetWeeklyDesc: '仅将周用量归零，并从当前时间重新开始 7 天周期；不消耗重置卡',
          resetQuota: '重置配额',
