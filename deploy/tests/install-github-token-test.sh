...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        "$ROOT_DIR/deploy/legacy/install.sh" "https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected() {
    ...
    if CURL_ARGS_LOG="$TEMP_DIR/$name" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="update-secret" \
        ...
        "$ROOT_DIR/deploy/legacy/install.sh" "$@" 2>/dev/null; then
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/AsukaCC/EasySub2api/releases/download/v1/asset"
assert_unsafe_invocation_rejected mixed-host -s \
    "https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest" \
    "https://example.com/collect"
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest" \
    "https://api.github.com/repos/AsukaCC/EasySub2api/releases"
assert_unsafe_invocation_rejected url-option -s --url \
    "https://example.com/collect" \
    "https://api.github.com/repos/AsukaCC/EasySub2api/releases/latest"

# Every installer release API request must use the scoped helper.
test "$(grep -c 'github_api_curl .*https://api.github.com/' "$ROOT_DIR/deploy/legacy/install.sh")" -eq 3

# Asset and checksum downloads must continue to call curl directly.
grep -Fq 'curl -sL "$download_url"' "$ROOT_DIR/deploy/legacy/install.sh"
grep -Fq 'curl -sL "$checksum_url"' "$ROOT_DIR/deploy/legacy/install.sh"
