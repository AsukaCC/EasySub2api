...
ENV_FILE="${EASYSUB2API_ENV_FILE:-${SUB2API_ENV_FILE:-${SCRIPT_DIR}/.env}}"

STACK_LABEL_KEY="org.easysub2api.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="easysub2api-apple"
APP_CONTAINER="easysub2api-apple"
POSTGRES_CONTAINER="easysub2api-apple-postgres"
REDIS_CONTAINER="easysub2api-apple-redis"
APP_VOLUME="easysub2api-apple-data"
POSTGRES_VOLUME="easysub2api-apple-postgres-data"
REDIS_VOLUME="easysub2api-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/easysub2api-apple-container.lock"
LOCK_ACQUIRED=false
...
Commands:
  ...
  up [--recreate]       Create and start the complete EasySub2api stack
...
Environment:
  EASYSUB2API_ENV_FILE  Path to the deployment env file (default: deploy/.env)
  SUB2API_ENV_FILE      Deprecated compatibility alias for one release
EOF
}
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another EasySub2api Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
cmd_init() {
    ...
    info "Review the file, then run: EASYSUB2API_ENV_FILE='${ENV_FILE}' ${SCRIPT_DIR}/apple-container.sh up"
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_EASYSUB2API_IMAGE '')"
    if [ -z "$APP_IMAGE" ]; then
        APP_IMAGE="$(read_env_value APPLE_CONTAINER_SUB2API_IMAGE ghcr.io/asukacc/easysub2api:latest)"
    fi
    ...
    POSTGRES_USER="$(read_env_value POSTGRES_USER easysub2api)"
    POSTGRES_PASSWORD="$(read_env_value POSTGRES_PASSWORD)"
    POSTGRES_DB="$(read_env_value POSTGRES_DB easysub2api)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/easysub2api-apple.XXXXXX")"
...
create_app_container() {
    info "Creating EasySub2api container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
        ...
        --entrypoint /bin/sh \
        "${APP_IMAGE}" \
        -c 'set -e; mkdir -p "$DATA_DIR"; chown -R easysub2api:easysub2api "$DATA_DIR"; exec su-exec easysub2api /app/easysub2api' \
        >/dev/null
}
...
start_app() {
    ...
    if ! wait_for_probe "EasySub2api" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "EasySub2api did not become ready."
    fi
    if ! wait_for_probe "EasySub2api host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "EasySub2api is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "EasySub2api stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the EasySub2api stack and all persistent data? [y/N] '
    else
        printf 'Delete the EasySub2api containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "EasySub2api stack and persistent data deleted."
    else
        info "EasySub2api stack deleted; persistent volumes were preserved."
    fi
}
