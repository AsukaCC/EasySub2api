...
EasySub2api can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published EasySub2api, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts EasySub2api.
...
# Restart PostgreSQL, Redis, and EasySub2api in dependency order.
...
The script uses `deploy/.env`, the same source file used by Docker Compose. Export `EASYSUB2API_ENV_FILE` to use another file for every command in the current shell:

```bash
export EASYSUB2API_ENV_FILE=/absolute/path/to/easysub2api.env
./apple-container.sh init
./apple-container.sh up
```
...
```dotenv
APPLE_CONTAINER_EASYSUB2API_IMAGE=ghcr.io/asukacc/easysub2api:latest
...
|---|---|
| Application and gateway variables | Passed to EasySub2api from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and EasySub2api |
...
The script creates only resources carrying the `org.easysub2api.stack=apple-container` label:

| Type | Names |
|---|---|
| Containers | `easysub2api-apple`, `easysub2api-apple-postgres`, `easysub2api-apple-redis` |
| Network | `easysub2api-apple` |
| Volumes | `easysub2api-apple-data`, `easysub2api-apple-postgres-data`, `easysub2api-apple-redis-data` |

The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. EasySub2api and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts EasySub2api. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.

All three services attach only to the private `easysub2api-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `easysub2api-apple-data`.
...
container exec easysub2api-apple sh -c \
  'PGPASSWORD="$DATABASE_PASSWORD" pg_dump -h "$DATABASE_HOST" -U "$DATABASE_USER" "$DATABASE_DBNAME"' \
  > backups/easysub2api.sql

# Application configuration and local files.
container exec easysub2api-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
  > backups/easysub2api-data.tar.gz
...
container delete easysub2api-apple
EASYSUB2API_IMAGE=ghcr.io/asukacc/easysub2api:latest # Match APPLE_CONTAINER_EASYSUB2API_IMAGE in .env.
container run --rm --name easysub2api-apple-data-restore \
  --entrypoint /bin/sh \
  --volume easysub2api-apple-data:/restore \
  --volume "$PWD/backups:/backup:ro" \
  "$EASYSUB2API_IMAGE" \
  -c 'rm -rf /restore/data && mkdir -p /restore/data && tar -xzf /backup/easysub2api-data.tar.gz -C /restore/data'

# Restore the logical database while the application is absent.
container start easysub2api-apple-postgres
until container exec easysub2api-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/easysub2api.sql easysub2api-apple-postgres:/tmp/easysub2api.sql
container exec easysub2api-apple-postgres sh -c '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  dropdb -h 127.0.0.1 -U "$POSTGRES_USER" --if-exists --force "$POSTGRES_DB"
  createdb -h 127.0.0.1 -U "$POSTGRES_USER" "$POSTGRES_DB"
  psql -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /tmp/easysub2api.sql
  rm /tmp/easysub2api.sql
'
...
- The script targets native `linux/arm64` images. The normal EasySub2api release publishes an arm64 variant.
