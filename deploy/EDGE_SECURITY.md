...
EasySub2api supports long-lived SSE and WebSocket requests. Protect the request
...
- `gateway.max_body_size: 268435456` remains available to multimodal, image,
  and video endpoints.
...
explicitly enabled, raw forwarding headers take over client-IP
...
CIDR/IP addresses that connect directly to EasySub2api. An explicit empty list
trusts no forwarded client IPs.

Existing installations retain their persisted administrator choice. New
installations default to the trusted-proxy chain and require an explicit
compatibility opt-in for raw forwarding headers.
...
```nginx
limit_conn_zone $binary_remote_addr zone=easysub2api_conn:20m;
limit_req_zone  $binary_remote_addr zone=easysub2api_auth:20m rate=5r/s;
limit_req_zone  $binary_remote_addr zone=easysub2api_api:40m rate=30r/s;
...
server {
    ...
    limit_conn easysub2api_conn 40;

    location ~ ^/(auth|api/auth)/ {
        limit_req zone=easysub2api_auth burst=10 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }

    location ~ ^/(v1/)?(embeddings|alpha/search)$ {
        client_max_body_size 32m;
        limit_req zone=easysub2api_api burst=60 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }
...
`gzip_types` and do not use `gzip_types *` for EasySub2api. The
...
EasySub2api `server.trusted_proxies` with the Caddy address/private subnet so the
