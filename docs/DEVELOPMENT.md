...
| 数据库凭据 | user=`easysub2api`, dbname=`easysub2api`，密码由本地 `.env` 设置 |
...
| Workflow | 触发条件 | 检查内容 |
|----------|----------|----------|
| **backend-ci.yml** | 分支 push、pull_request | Shell 部署检查、Go build、前端 lint/typecheck、golangci-lint |
| **security-scan.yml** | 分支 push、pull_request、每周一 | govulncheck + pnpm audit |
| **release.yml** | 仅 workflow_dispatch | 由本地发布脚本触发 GHCR 镜像和 GitHub Release |

### CI 要求

- Go 版本必须与 `backend/go.mod`、CI 版本断言以及根/部署 Dockerfile 的构建镜像一致。
- 前端使用 `pnpm install --frozen-lockfile`，必须提交 `pnpm-lock.yaml`
- tag push 不会发布。维护者必须使用 [手动发布流程](RELEASE.md)。
...
- 但在**批量修改同时选中不同平台账号**时，模型白名单/映射可能被跨平台策略覆盖；
...
├── deploy/                   # Compose、systemd 与迁移工具
├── docs/                     # 开发、发布、迁移和功能文档
├── scripts/                  # 维护者本地入口（含 release.ps1）
└── .github/workflows/        # 检查与手动发布工作流
```

## 七、参考资源

- [项目仓库](https://github.com/AsukaCC/EasySub2api)
