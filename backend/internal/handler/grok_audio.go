...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	if result := grokRealtimeBillingResult(model, elapsed, audioObserved); result != nil {
		h.recordGrokVoiceUsage(c, apiKey, selection.Account, subscription, selection, "realtime", nil, result)
	}
}
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		if forwardErr == nil {
			h.recordGrokVoiceUsage(c, apiKey, account, subscription, selection, endpoint, body, result)
			return
		}
