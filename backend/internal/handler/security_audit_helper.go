...
const securityAuditCompletedContextKey = "easysub2api.security_audit.completed"
const securityAuditWSTurnContextKey = "easysub2api.security_audit.ws_turn"
const securityAuditWSDedupeContextKey = "easysub2api.security_audit.ws_dedupe"
...
func logSecurityAuditStart(reqLog *zap.Logger, request securityaudit.Request, bodyBytes int, cached bool) {
	...
	reqLog.Info("security_audit.gateway_check_start",
		zap.String("request_id", request.RequestID), zap.String("user_id", request.UserID),
		zap.String("api_key_id", request.APIKeyID), zap.Stringp("group_id", request.GroupID),
...
func cloneSecurityAuditGroupID(value *string) *string {
	if value == nil {
		return nil
	}
	cloned := *value
