...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	if rejectRetiredGatewayModel(c, requestedModel) {
		return
	}
	...
	for {
		...
		if err == nil {
			...
			if result != nil {
				h.recordAlphaSearchUsage(c, apiKey, account, subscription, selection, channelMapping, requestedModel, body, result, subject.UserID)
			}
			return
		}
		...
		if !errors.As(err, &failoverErr) {
			...
			reqLog.Warn("openai_alpha_search.forward_failed", zap.String("account_id", account.ID), zap.Error(err))
			return
		}
