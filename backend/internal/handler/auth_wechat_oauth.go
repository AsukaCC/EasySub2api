...
func exchangeWeChatOAuthCode(ctx context.Context, cfg wechatOAuthConfig, code string) (*wechatOAuthTokenResponse, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, fmt.Errorf("wechat access token status=%v", resp.StatusCode)
	}
	...
	if tokenResp.ErrCode != 0 {
		return nil, fmt.Errorf("wechat access token error=%v %s", tokenResp.ErrCode, strings.TrimSpace(tokenResp.ErrMsg))
	}
...
func fetchWeChatUserInfo(ctx context.Context, tokenResp *wechatOAuthTokenResponse) (*wechatOAuthUserInfoResponse, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, fmt.Errorf("wechat userinfo status=%v", resp.StatusCode)
	}
	...
	if userInfo.ErrCode != 0 {
		return nil, fmt.Errorf("wechat userinfo error=%v %s", userInfo.ErrCode, strings.TrimSpace(userInfo.ErrMsg))
	}
	return &userInfo, nil
}
...
func parseWeChatPaymentPlanID(raw string) string {
	id, _ := parseEntityID(strings.TrimSpace(raw))
	return id
}
