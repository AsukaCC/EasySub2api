...
// SetUserLevelDependencies attaches the personal level summary without
// changing the long-standing constructor used by focused handler tests.
func (h *UserHandler) SetUserLevelDependencies(userLevelService *service.UserLevelService, apiKeyService *service.APIKeyService) {
	h.userLevelService = userLevelService
	h.apiKeyService = apiKeyService
}

// GetLevel returns the current user's rolling-spend level and the lowest
// applicable multiplier across their available groups.
// GET /api/v1/user/level
func (h *UserHandler) GetLevel(c *gin.Context) {
	...
	level, err := h.userLevelService.ResolveDashboard(c.Request.Context(), subject.UserID, groupIDs, time.Now())
	if err != nil {
...
func (h *UserHandler) RegenerateAffiliateCode(c *gin.Context) {
	...
	result, err := h.affiliateService.RegenerateAffiliateCode(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UserHandler) buildUserProfileResponse(ctx context.Context, userID string, user *service.User) (userProfileResponse, error) {
...
func thirdPartyIdentityProviders(identities service.UserIdentitySummarySet) []service.UserIdentitySummary {
	out := make([]service.UserIdentitySummary, 0, 4)
	for _, summary := range []service.UserIdentitySummary{identities.OIDC, identities.WeChat, identities.DingTalk, identities.LinuxDo} {
		if summary.Bound {
