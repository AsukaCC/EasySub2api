...
func (h *ModelPlazaHandler) Get(c *gin.Context) {
	...
	if role != service.RoleAdmin && !h.settingService.IsModelPlazaUserAvailable(c.Request.Context()) {
		response.NotFound(c, "Model plaza is not enabled")
		return
	}
	...
	var allowedExclusive map[string]struct{}
...
// allowedExclusive == nil 表示匿名（仅非专属）；非 nil 表示登录（非专属 + 授权/有效订阅的专属）。
func filterPlazaVisibleGroups(
	groups []service.PlazaGroup,
	allowedExclusive map[string]struct{},
) []service.PlazaGroup {
...
func toModelPlazaGroupDTO(g *service.PlazaGroup, userRates map[string]float64) modelPlazaGroup {
