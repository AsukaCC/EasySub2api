...
func (e *oidcTokenExchangeError) Error() string {
	...
	parts := []string{fmt.Sprintf("token exchange status=%v", e.StatusCode)}
...
func (h *AuthHandler) OIDCOAuthCallback(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.As(err, &exchangeErr) && exchangeErr != nil {
			log.Printf(
				"[OIDC OAuth] token exchange failed: status=%v provider_error=%q provider_description=%q body=%s",
...
func oidcFetchUserInfo(
	...
	if !resp.IsSuccessState() {
		return nil, fmt.Errorf("userinfo status=%v", resp.StatusCode)
	}
...
func oidcFetchJWKSet(ctx context.Context, jwksURL string) (*oidcJWKSet, error) {
	...
	if !resp.IsSuccessState() {
		return nil, fmt.Errorf("jwks status=%v", resp.StatusCode)
	}
	set := &oidcJWKSet{}
...
func (k oidcJWK) publicKey() (any, error) {
	...
	case "EC":
		...
		x, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(k.X))
		if err != nil {
			return nil, fmt.Errorf("decode ec x: %w", err)
		}
		y, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(k.Y))
		if err != nil {
			return nil, fmt.Errorf("decode ec y: %w", err)
		}
		if len(x) == 0 || len(y) == 0 || len(x) > coordinateSize || len(y) > coordinateSize {
...
func (h *AuthHandler) tryOIDCVerifiedEmailFastPath(
	...
	fragment.Set("expires_in", fmt.Sprintf("%v", tokenPair.ExpiresIn))
