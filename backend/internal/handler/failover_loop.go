...
type TempUnscheduler interface {
	TempUnscheduleRetryableError(ctx context.Context, accountID string, failoverErr *service.UpstreamFailoverError)
}
...
type FailoverState struct {
	...
	SameAccountRetryCount map[string]int
	...
	profitVetoedAccountIDs map[string]struct{}
...
func NewFailoverState(maxSwitches int, hasBoundSession bool) *FailoverState {
	return &FailoverState{
		...
		profitVetoedAccountIDs: make(map[string]struct{}),
	}
}
...
// 不得继续 continue。
func (s *FailoverState) RecordProfitVeto(accountID string) FailoverAction {
	s.FailedAccountIDs[accountID] = struct{}{}
	if s.profitVetoedAccountIDs == nil {
		s.profitVetoedAccountIDs = make(map[string]struct{})
	}
...
func (s *FailoverState) HandleSelectionExhausted(ctx context.Context) FailoverAction {
	...
	if s.LastFailoverErr != nil &&
		...
		s.FailedAccountIDs = make(map[string]struct{})
