...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/gemini"
...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	// 分组配置了自定义模型展示列表时，/v1beta/models 与 /v1/models 一致直接返回
	// 配置列表，不再依赖上游账号目录。
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	reqLog := requestLogger(
		...
		zap.String("user_id", authSubject.UserID),
	...
	useDigestFallback := sessionBoundAccountID == ""
			...
			if geminiDigestChain != "" {
				...
				if found {
					...
					reqLog.Info("gemini.digest_fallback_matched",
						...
						zap.String("account_id", foundAccountID),
	...
	for {
		...
		if sessionBoundAccountID != "" && sessionBoundAccountID != account.ID {
			reqLog.Info("gemini.sticky_session_account_switched",
				zap.String("from_account_id", sessionBoundAccountID),
				zap.String("to_account_id", account.ID),
				zap.Bool("clean_thought_signature", true),
			)
			body = service.CleanGeminiNativeThoughtSignatures(body)
			sessionBoundAccountID = account.ID
		} else if sessionKey != "" && sessionBoundAccountID == "" && !cleanedForUnknownBinding && bytes.Contains(body, []byte(`"thoughtSignature"`)) {
		...
		if !selection.Acquired {
			...
			if err != nil {
				reqLog.Warn("gemini.account_wait_counter_increment_failed", zap.String("account_id", account.ID), zap.Error(err))
			} else if !canWait {
			...
			if err != nil {
				reqLog.Warn("gemini.account_slot_acquire_failed", zap.String("account_id", account.ID), zap.Error(err))
				googleError(c, http.StatusTooManyRequests, err.Error())
				return
			}
			if accountWaitCounted {
		...
		if vetoed {
			...
			reqLog.Debug("gemini.account_slot_profit_vetoed", zap.String("account_id", account.ID), zap.String("reason", reason))
		...
		if selection.ProfitGateActive() || !selection.Acquired {
			if err := h.gatewayService.BindStickySessionAfterProfitAdmission(admissionCtx, apiKey.GroupID, sessionKey, account.ID); err != nil {
				reqLog.Warn("gemini.bind_sticky_session_after_profit_admission_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
		...
		if err != nil {
			...
			reqLog.Error("gemini.forward_failed", zap.String("account_id", account.ID), zap.Error(err))
			return
		}
		...
		if useDigestFallback && geminiDigestChain != "" && geminiPrefixHash != "" {
			if err := h.gatewayService.SaveGeminiSession(
				...
				reqLog.Warn("gemini.digest_session_save_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
...
// derefGroupID safely dereferences a group UUID.
