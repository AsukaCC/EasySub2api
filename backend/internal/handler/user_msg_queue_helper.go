...
func (h *UserMsgQueueHelper) AcquireWithWait(
	...
	if result.Acquired {
		...
		reqLog.Debug("gateway.umq_lock_acquired", zap.String("account_id", accountID))
...
func (h *UserMsgQueueHelper) waitForLockWithPing(
	...
	for {
		select {
		case <-ctx.Done():
			return nil, fmt.Errorf("umq wait timeout for account %v", accountID)
...
func (h *UserMsgQueueHelper) makeReleaseFunc(accountID string, requestID string, reqLog *zap.Logger) func() {
	var once sync.Once
	return func() {
		once.Do(func() {
			bgCtx, bgCancel := context.WithTimeout(context.Background(), 5*time.Second)
			...
			if err := h.queueService.Release(bgCtx, accountID, requestID); err != nil {
				...
				reqLog.Debug("gateway.umq_lock_released", zap.String("account_id", accountID))
			}
		})
	}
}
