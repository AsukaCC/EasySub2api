...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/oauth"
...
func pendingOAuthCompletionCanIssueTokenPair(session *dbent.PendingAuthSession, payload map[string]any) bool {
	...
	if session.TargetUserID == nil || *session.TargetUserID == "" {
		return false
	}
...
func resolvePendingOAuthTargetUserID(ctx context.Context, client *dbent.Client, session *dbent.PendingAuthSession) (string, error) {
	if session == nil {
	...
	if err != nil {
		if errors.Is(err, service.ErrUserNotFound) {
			return "", infraerrors.InternalServer("PENDING_AUTH_TARGET_USER_NOT_FOUND", "pending auth target user was not found")
		}
		return "", err
	}
...
func ensurePendingOAuthIdentityForUser(ctx context.Context, tx *dbent.Tx, session *dbent.PendingAuthSession, userID string) (*dbent.AuthIdentity, error) {
...
func ensurePendingWeChatOAuthIdentityForUser(ctx context.Context, tx *dbent.Tx, session *dbent.PendingAuthSession, userID string) (*dbent.AuthIdentity, error) {
	client := tx.Client()
...
func chooseWeChatIdentityForUser(ctx context.Context, client *dbent.Client, records []*dbent.AuthIdentity, userID string, preferredProviderKey string) (*dbent.AuthIdentity, bool, error) {
...
func chooseWeChatChannelForUser(ctx context.Context, client *dbent.Client, records []*dbent.AuthIdentityChannel, userID string, preferredProviderKey string) (*dbent.AuthIdentityChannel, bool, error) {
...
func findActiveUserByID(ctx context.Context, client *dbent.Client, userID string) (*dbent.User, error) {
...
func applyPendingOAuthBindingTx(
	...
	targetUserID := ""
	if overrideUserID != nil && *overrideUserID != "" {
...
func pendingOAuthIdentityExistsForUser(
	...
	if client == nil || session == nil || userID == "" {
		return false, nil
	}
...
func (h *AuthHandler) bindPendingOAuthLogin(c *gin.Context, provider string) {
	...
	if session.TargetUserID != nil && *session.TargetUserID != "" && user.ID != *session.TargetUserID {
