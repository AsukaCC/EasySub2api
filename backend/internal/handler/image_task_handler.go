...
func NewAsyncImageHandlerWithQueue(tasks *service.ImageTaskService, openAI *OpenAIGatewayHandler, queue imageTaskQueue, apiKeys *service.APIKeyService, workers int, maxAttempts int, claimIdleSeconds int) *AsyncImageHandler {
	h := NewAsyncImageHandler(tasks, openAI); h.queue, h.apiKeys, h.workers, h.maxAttempts = queue, apiKeys, workers, maxAttempts
	if h.workers <= 0 { h.workers = 2 }
	if h.maxAttempts < 1 { h.maxAttempts = 3 }
	h.claimIdle = time.Duration(claimIdleSeconds) * time.Second; if h.claimIdle <= 0 { h.claimIdle = 5 * time.Minute }
	if h.queue != nil {
		if err := h.queue.Ensure(context.Background()); err != nil { logger.L().Error("image_task.queue_init_failed", zap.Error(err)) }
		go h.startWorkers()
	}
	return h
}
...
func (h *AsyncImageHandler) Submit(c *gin.Context) {
	...
	endpoint := "generations"; if strings.Contains(c.Request.URL.Path, "/edits") { endpoint = "edits" }
	payloadKey := "image-tasks/" + strings.TrimSpace(apiKey.ID) + "/" + strings.ReplaceAll(time.Now().UTC().Format("20060102T150405.000000000"), ".", "")
	if h.queue != nil {
		store := h.tasks.ArtifactStore(); if store == nil { imageTaskJSONError(c, http.StatusServiceUnavailable, "IMAGE_TASK_UNAVAILABLE", "async image object storage is unavailable"); return }
		taskID := "imgtask_" + strings.ReplaceAll(uuid.NewString(), "-", "")
		payloadKey = "image-tasks/" + taskID + "/request"
		if err := store.Put(c.Request.Context(), payloadKey, c.GetHeader("Content-Type"), body); err != nil { imageTaskJSONError(c, http.StatusServiceUnavailable, "IMAGE_TASK_UNAVAILABLE", "failed to store image task request"); return }
		...
		if err == nil { err = h.queue.Enqueue(c.Request.Context(), task.ID) }
		if err != nil { _ = store.Delete(context.Background(), payloadKey); imageTaskError(c, err); return }
		pollURL := imageTaskPollURL(c.Request.URL.Path, task.ID); c.Header("Cache-Control", "no-store"); c.Header("Location", pollURL); c.Header("Retry-After", "3")
		c.JSON(http.StatusAccepted, gin.H{"id": task.ID, "task_id": task.TaskID, "object": task.Object, "status": task.Status, "created_at": task.CreatedAt, "expires_at": task.ExpiresAt, "poll_url": pollURL}); return
	}
...
func (h *AsyncImageHandler) Cancel(c *gin.Context) {
	apiKey, ok := middleware2.GetAPIKeyFromContext(c); if !ok || apiKey == nil { imageTaskError(c, service.ErrImageTaskForbidden); return }
	task, err := h.tasks.Cancel(c.Request.Context(), service.ImageTaskOwner{UserID: apiKey.UserID, APIKeyID: apiKey.ID}, c.Param("task_id")); if err != nil { imageTaskError(c, err); return }
	h.cancelMu.Lock(); if cancel := h.cancel[task.ID]; cancel != nil { cancel() }; delete(h.cancel, task.ID); h.cancelMu.Unlock()
	c.Header("Cache-Control", "no-store"); c.JSON(http.StatusOK, task)
}

func (h *AsyncImageHandler) startWorkers() {
	if err := h.queue.Ensure(context.Background()); err != nil { logger.L().Error("image_task.queue_init_failed", zap.Error(err)); return }
	for i := 0; i < h.workers; i++ { go h.workerLoop(fmt.Sprintf("image-worker-%d", i)) }
}

func (h *AsyncImageHandler) workerLoop(consumer string) {
	for {
		messages, err := h.queue.Claim(context.Background(), consumer, h.claimIdle)
		if err == nil && len(messages) == 0 { messages, err = h.queue.Read(context.Background(), consumer, 5*time.Second) }
		if err != nil { time.Sleep(time.Second); continue }
		for _, msg := range messages { taskID, _ := msg.Values["task_id"].(string); if taskID == "" { _ = h.queue.Ack(context.Background(), msg.ID); continue }; h.processQueuedTask(taskID); _ = h.queue.Ack(context.Background(), msg.ID) }
	}
}

func (h *AsyncImageHandler) processQueuedTask(taskID string) {
	if h.apiKeys == nil { _ = h.tasks.Fail(context.Background(), taskID, http.StatusServiceUnavailable, imageTaskErrorPayload("api_error", "image task worker is unavailable")); return }
	record, err := h.tasks.Record(context.Background(), taskID); if err != nil { return }
	record.Attempt++
	if err := h.tasks.SaveRecord(context.Background(), record); err != nil { return }
	apiKey, err := h.apiKeys.GetByID(context.Background(), record.APIKeyID); if err != nil || apiKey == nil { _ = h.tasks.Fail(context.Background(), taskID, http.StatusUnauthorized, imageTaskErrorPayload("authentication_error", "API key is no longer available")); return }
	store := h.tasks.ArtifactStore(); if store == nil { _ = h.tasks.Fail(context.Background(), taskID, http.StatusServiceUnavailable, imageTaskErrorPayload("api_error", "image task object storage is unavailable")); return }
	body, contentType, err := store.Get(context.Background(), record.PayloadKey); if err != nil { _ = h.tasks.Fail(context.Background(), taskID, http.StatusBadGateway, imageTaskErrorPayload("api_error", "failed to read image task request")); return }
	ctx, cancel := context.WithTimeout(context.Background(), h.tasks.ExecutionTimeout()); h.cancelMu.Lock(); h.cancel[taskID] = cancel; h.cancelMu.Unlock(); defer func(){ cancel(); h.cancelMu.Lock(); delete(h.cancel, taskID); h.cancelMu.Unlock(); _ = store.Delete(context.Background(), record.PayloadKey) }()
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/images/"+record.Endpoint, bytes.NewReader(body)).WithContext(ctx); req.Header.Set("Content-Type", contentType)
	writer := httptest.NewRecorder(); taskCtx, _ := gin.CreateTestContext(writer); taskCtx.Request = req; taskCtx.Set(string(middleware2.ContextKeyAPIKey), apiKey); taskCtx.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: apiKey.UserID}); if apiKey.User != nil { taskCtx.Set(string(middleware2.ContextKeyUserRole), apiKey.User.Role) }
	h.execute(record.Platform, taskCtx)
	response := bytes.TrimSpace(writer.Body.Bytes()); code := writer.Code; if code == 0 { code = http.StatusOK }
	if ctx.Err() != nil && len(response) == 0 { _ = h.tasks.Fail(context.Background(), taskID, http.StatusGatewayTimeout, imageTaskErrorPayload("timeout_error", "image generation task timed out")); return }
	if code >= 200 && code < 300 && json.Valid(response) { _ = h.tasks.Complete(context.Background(), taskID, code, response); return }
	if code >= 500 && record.Attempt < h.maxAttempts {
		time.Sleep(time.Duration(record.Attempt*record.Attempt) * 5 * time.Second)
		if enqueueErr := h.queue.Enqueue(context.Background(), taskID); enqueueErr != nil { _ = h.tasks.Fail(context.Background(), taskID, code, extractImageTaskError(response)) }
		return
	}
