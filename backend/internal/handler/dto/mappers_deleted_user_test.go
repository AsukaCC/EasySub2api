...
func TestUserFromServiceShallow_MapsDeletedAt(t *testing.T) {
	...
	deleted := UserFromServiceShallow(&service.User{ID: "user-1", Email: "d@test.com", DeletedAt: &ts})
	require.NotNil(t, deleted.DeletedAt)
	require.Equal(t, ts, *deleted.DeletedAt)

	active := UserFromServiceShallow(&service.User{ID: "user-2", Email: "a@test.com"})
