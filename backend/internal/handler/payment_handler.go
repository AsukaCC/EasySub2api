...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	type planWithPlatform struct {
		...
		DailyLimitPoints      *float64 `json:"daily_limit_points"`
		WeeklyLimitUSD        *float64 `json:"weekly_limit_usd"`
		WeeklyLimitPoints     *float64 `json:"weekly_limit_points"`
		MonthlyLimitUSD       *float64 `json:"monthly_limit_usd"`
		MonthlyLimitPoints    *float64 `json:"monthly_limit_points"`
	...
	for _, p := range plans {
		...
		result = append(result, planWithPlatform{
			...
			DailyLimitUSD: gi.DailyLimitUSD, DailyLimitPoints: gi.DailyLimitUSD,
			WeeklyLimitUSD: gi.WeeklyLimitUSD, WeeklyLimitPoints: gi.WeeklyLimitUSD,
			MonthlyLimitUSD: gi.MonthlyLimitUSD, MonthlyLimitPoints: gi.MonthlyLimitUSD,
			Name: p.Name, Description: p.Description, Price: p.Price, PricePoints: p.Price,
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	planList := checkoutPlansForResponse(plans, groupInfo)
...
type checkoutInfoResponse struct {
	...
	Wallet                        service.WalletSummary           `json:"wallet"`
}
...
func (h *PaymentHandler) CreateOrder(c *gin.Context) {
	...
	if err := normalizeCreateOrderPrincipalAmount(&req); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func normalizeCreateOrderPrincipalAmount(req *CreateOrderRequest) error {
	if req == nil || req.PrincipalAmount == nil {
		return nil
	}
	if req.Amount != 0 {
		return infraerrors.BadRequest("CONFLICTING_AMOUNT_FIELDS", "amount and principal_amount cannot be provided together")
	}
	req.Amount = *req.PrincipalAmount
	return nil
}
...
// GetRefundQuote returns the maximum refundable RMB principal and the point
// recovery split for a recharge order.
...
func (h *PaymentHandler) GetRefundQuote(c *gin.Context) {
	...
	if raw := strings.TrimSpace(c.Query("principal_amount")); raw != "" {
		value, parseErr := strconv.ParseFloat(raw, 64)
		if parseErr != nil {
			response.BadRequest(c, "Invalid principal_amount")
			return
		}
		principal = &value
	}
	quote, err := h.paymentService.GetRefundQuote(c.Request.Context(), orderID, subject.UserID, principal)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// CreateRefund starts an idempotent self-service refund.
...
func (h *PaymentHandler) CreateRefund(c *gin.Context) {
	...
	var req CreateRefundBody
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	result, err := h.paymentService.CreateSelfServiceRefund(c.Request.Context(), service.CreatePaymentRefundInput{
		OrderID: orderID, UserID: subject.UserID, RequestedBy: subject.UserID,
		IdempotencyKey: c.GetHeader("Idempotency-Key"), Principal: req.PrincipalAmount, Reason: req.Reason,
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
// all refund-capable providers. The legacy provider_instance_ids field remains
// the self-service list.
func (h *PaymentHandler) GetRefundEligibleProviders(c *gin.Context) {
	userRefundIDs, refundEnabledIDs, err := h.configService.GetRefundEligibleInstanceIDs(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{
		"provider_instance_ids":                userRefundIDs,
		"refund_enabled_provider_instance_ids": refundEnabledIDs,
	})
}
...
func buildPublicOrderResult(order *dbent.PaymentOrder) PublicOrderResult {
	...
	result.SubscriptionGroupID, result.SubscriptionDays, result.SubscriptionResetCardCount, result.SubscriptionResetCardValidityDays = subscriptionSnapshotPointers(order)
	return result
}
...
func subscriptionSnapshotPointers(order *dbent.PaymentOrder) (*string, *int, *int, *int) {
	if order == nil || order.OrderType != payment.OrderTypeSubscription {
		return nil, nil, nil, nil
	}
	count := order.SubscriptionResetCardCount
	validity := order.SubscriptionResetCardValidityDays
	return order.SubscriptionGroupID, order.SubscriptionDays, &count, &validity
}
