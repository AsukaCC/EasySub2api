...
// SetConcurrencyService wires the shared Redis-backed concurrency reader into
// the user dashboard without changing the constructor used by lightweight
// handler tests and integrations.
func (h *UsageHandler) SetConcurrencyService(concurrencyService *service.ConcurrencyService) {
...
func (h *UsageHandler) GetErrorDetail(c *gin.Context) {
	...
	id, err := parseEntityID(strings.TrimSpace(c.Param("id")))
	if err != nil || id == "" {
...
func (h *UsageHandler) GetByID(c *gin.Context) {
	...
	usageID, err := parseEntityID(c.Param("id"))
	if err != nil {
...
func (h *UsageHandler) Stats(c *gin.Context) {
	...
	for i := range stats.Endpoints {
		stats.Endpoints[i].AccountCost = 0
	}
...
func (h *UsageHandler) DashboardStats(c *gin.Context) {
	...
	stats.Concurrency = int64(subject.Concurrency)
	if h.concurrencyService != nil {
		if current, concurrencyErr := h.concurrencyService.GetUserConcurrency(c.Request.Context(), subject.UserID); concurrencyErr == nil {
			stats.CurrentConcurrency = int64(current)
		}
	}
...
func (h *UsageHandler) DashboardAPIKeysUsage(c *gin.Context) {
	...
	endTime := time.Time{}
	if req.Days > 0 {
		startTime = timezone.Today().AddDate(0, 0, -(req.Days - 1))
		endTime = time.Now()
	}
...
func (h *UsageHandler) GetMyAPIKeyDailyUsage(c *gin.Context) {
	...
	apiKeyID, err := parseEntityID(c.Param("id"))
	if err != nil {
