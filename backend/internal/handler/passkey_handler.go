...
// FinishLogin validates a passkey assertion and creates a normal EasySub2api token
...
func passkeyMutationTarget(c *gin.Context) (middleware2.AuthSubject, string, bool) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return middleware2.AuthSubject{}, "", false
	}
	credentialID, err := parseEntityID(c.Param("id"))
