...
func maybeLogOpsErrorLogDrop() {
	...
	log.Printf(
		"[OpsErrorLogger] queue is full; dropping logs (queued=%v cap=%v queued_bytes=%v bytes_cap=%v enqueued_total=%v dropped_total=%v processed_total=%v sanitized_total=%v)",
		queued,
		queueCap,
		queuedBytes,
...
func setOpsSelectedAccount(c *gin.Context, accountID string, platform ...string) {
	if c == nil || accountID == "" {
		return
	}
...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if status < 400 {
			...
			if len(events) > 0 {
				if last := events[len(events)-1]; last != nil && last.AccountID != "" {
					v := last.AccountID
					accountID = &v
				}
			}
			if accountID == nil {
				if v, ok := accountIDV.(string); ok && v != "" {
					accountID = &v
				}
			}
...
func parseOpsErrorResponse(body []byte) parsedOpsError {
	...
	if errObj, ok := m["error"].(map[string]any); ok {
		...
		// Capture numeric codes as strings for business-limited mapping if needed.
		var code string
