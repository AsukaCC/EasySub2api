...
func TestRunSecurityAuditDoesNotSkipSubsequentWebSocketTurns(t *testing.T) {
	...
	subject := middleware2.AuthSubject{UserID: "user-7", Concurrency: 1}
...
func TestRunSecurityAuditDeduplicatesRepeatedPayloadWithinWebSocketTurn(t *testing.T) {
	...
	first := runSecurityAudit(c, nil, coordinator, nil, nil, middleware2.AuthSubject{UserID: "user-7"}, "openai_responses", "gpt-test", payload, "subsequent_turn")
	second := runSecurityAudit(c, nil, coordinator, nil, nil, middleware2.AuthSubject{UserID: "user-7"}, "openai_responses", "gpt-test", payload, "subsequent_turn")
	require.NotNil(t, first)
	require.NotNil(t, second)
	require.True(t, first.AllowNextStage)
	...
	runSecurityAudit(c, nil, coordinator, nil, nil, middleware2.AuthSubject{UserID: "user-7"}, "openai_responses", "gpt-test", payload, "subsequent_turn")
...
func TestRunSecurityAuditLogsWebSocketChecksAndCacheHits(t *testing.T) {
	...
	runSecurityAudit(c, reqLog, coordinator, nil, nil, middleware2.AuthSubject{UserID: "user-7"}, "openai_responses", "gpt-test", payload, "subsequent_turn")
	runSecurityAudit(c, reqLog, coordinator, nil, nil, middleware2.AuthSubject{UserID: "user-7"}, "openai_responses", "gpt-test", payload, "subsequent_turn")
