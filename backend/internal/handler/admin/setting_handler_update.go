...
type UpdateSettingsRequest struct {
	...
	AuthSourceDefaultOIDCBonusValidityDays     *int                              `json:"auth_source_default_oidc_bonus_validity_days"`
	...
	AuthSourceDefaultWeChatBonusValidityDays   *int                              `json:"auth_source_default_wechat_bonus_validity_days"`
	...
	AuthSourceDefaultGitHubBonusValidityDays   *int                              `json:"auth_source_default_github_bonus_validity_days"`
	...
	AuthSourceDefaultGoogleBonusValidityDays   *int                              `json:"auth_source_default_google_bonus_validity_days"`
	...
	AuthSourceDefaultDingTalkBonusValidityDays *int                              `json:"auth_source_default_dingtalk_bonus_validity_days"`
	...
	PaymentUserVisible               *bool                       `json:"payment_user_visible"`
	...
	AvailableChannelsUserVisible *bool `json:"available_channels_user_visible"`
...
func resolveUserFeatureVisibility(enabled bool, requested *bool, previous bool) (bool, error) {
	if !enabled {
		if requested != nil && *requested {
			return false, fmt.Errorf("user visibility cannot be enabled while the feature is disabled")
		}
		return false, nil
	}
	if requested != nil {
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.SiteBillingMode != nil {
		normalized := service.NormalizeSiteBillingMode(*req.SiteBillingMode)
		subscriptionEnabled, balanceDisabled := service.SiteBillingModeSettings(normalized)
		req.SiteBillingMode = &normalized
		req.SubscriptionEnabled = &subscriptionEnabled
		req.PaymentBalanceDisabled = &balanceDisabled
	}
	...
	previousPaymentEnabled := false
	if h.paymentConfigService != nil {
		paymentCfg, paymentErr := h.paymentConfigService.GetPaymentConfig(c.Request.Context())
		if paymentErr != nil {
			response.ErrorFrom(c, paymentErr)
			return
		}
		previousPaymentEnabled = paymentCfg != nil && paymentCfg.Enabled
	}
	...
	channelMonitorUserVisible, err := resolveUserFeatureVisibility(channelMonitorEnabled, req.ChannelMonitorUserVisible, previousSettings.ChannelMonitorUserVisible)
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	availableChannelsUserVisible, err := resolveUserFeatureVisibility(availableChannelsEnabled, req.AvailableChannelsUserVisible, previousSettings.AvailableChannelsUserVisible)
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	modelPlazaUserVisible, err := resolveUserFeatureVisibility(modelPlazaEnabled, req.ModelPlazaUserVisible, previousSettings.ModelPlazaUserVisible)
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	affiliateUserVisible, err := resolveUserFeatureVisibility(affiliateEnabled, req.AffiliateUserVisible, previousSettings.AffiliateUserVisible)
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	...
	// A runtime disable atomically unpublishes the corresponding user feature,
	// even when an older caller omitted the visibility field.
	for key, disabled := range map[string]bool{
		service.SettingKeyChannelMonitorUserVisible:    req.ChannelMonitorEnabled != nil && !channelMonitorEnabled,
		service.SettingKeyAvailableChannelsUserVisible: req.AvailableChannelsEnabled != nil && !availableChannelsEnabled,
		service.SettingKeyModelPlazaUserVisible:        req.ModelPlazaEnabled != nil && !modelPlazaEnabled,
		service.SettingKeyAffiliateUserVisible:         req.AffiliateEnabled != nil && !affiliateEnabled,
		service.SettingPaymentUserVisible:              req.PaymentEnabled != nil && !paymentEnabled,
	} {
		if disabled {
			delete(omitted, key)
		}
	...
	if req.BonusBalanceDefaultValidityDays <= 0 {
		req.BonusBalanceDefaultValidityDays = previousSettings.BonusBalanceDefaultValidityDays
		if req.BonusBalanceDefaultValidityDays <= 0 {
			req.BonusBalanceDefaultValidityDays = 90
		}
	}
	...
	if req.OpenAICodexClientVersion != nil {
		...
		if normalized != "" && service.NormalizeCodexClientVersion(normalized) == "" {
			response.Error(c, http.StatusBadRequest, "openai_codex_client_version must be empty or a valid version (e.g. 0.154.0)")
			return
		}
		// Reject malformed and below-minimum versions, including prereleases.
		if normalized != "" && service.AcceptCodexClientVersion(normalized) == "" {
			response.Error(c, http.StatusBadRequest, "openai_codex_client_version must be valid and meet the minimum supported version")
			return
		}
	...
	settings := &service.SystemSettings{
		...
		ChannelMonitorUserVisible: channelMonitorUserVisible,
		...
		AvailableChannelsUserVisible: availableChannelsUserVisible,
		ModelPlazaEnabled:            modelPlazaEnabled,
		ModelPlazaUserVisible:        modelPlazaUserVisible,
	...
	// Platform quota maps keep request-presence semantics all the way into the
	// service layer: nil skips validation and writing, while a non-nil map is a
	// deliberate full replacement. Reusing the stored map here would make an
	// unrelated partial update revalidate legacy platform keys.
	authSourceDefaults := &service.AuthSourceDefaultSettings{
		Email: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultEmailBonusValidityDays, previousAuthSourceDefaults.Email.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceEmailPlatformQuotas,
		},
		LinuxDo: func() service.ProviderDefaultGrantSettings {
			legacy := previousAuthSourceDefaults.LinuxDo
			legacy.PlatformQuotas = nil
			return legacy
		}(),
		OIDC: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultOIDCBonusValidityDays, previousAuthSourceDefaults.OIDC.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceOIDCPlatformQuotas,
		},
		WeChat: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultWeChatBonusValidityDays, previousAuthSourceDefaults.WeChat.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceWeChatPlatformQuotas,
		},
		GitHub: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultGitHubBonusValidityDays, previousAuthSourceDefaults.GitHub.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceGitHubPlatformQuotas,
		},
		Google: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultGoogleBonusValidityDays, previousAuthSourceDefaults.Google.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceGooglePlatformQuotas,
		},
		DingTalk: service.ProviderDefaultGrantSettings{
			...
			BonusValidityDays: intValueOrDefault(req.AuthSourceDefaultDingTalkBonusValidityDays, previousAuthSourceDefaults.DingTalk.BonusValidityDays),
			...
			PlatformQuotas:    req.AuthSourceDingTalkPlatformQuotas,
		},
	...
	payload := dto.SystemSettings{
		...
		SiteBillingMode:                                        updatedSettings.SiteBillingMode,
		...
		PaymentUserVisible:                                     updatedSettings.PaymentUserVisible,
		...
		AvailableChannelsUserVisible: updatedSettings.AvailableChannelsUserVisible,
