...
const testGrokMediaAccountID = "018f4a2e-9c6b-7a5d-8e3f-1b2c3d4e5f60"
...
func (s *grokMediaEligibilityAdminServiceStub) GetAccount(_ context.Context, _ string) (*service.Account, error) {
	return s.account, nil
}

func (s *grokMediaEligibilityAdminServiceStub) UpdateAccountExtra(_ context.Context, _ string, updates map[string]any) error {
...
func TestGrokMediaEligibilityHandlerGetReturnsModeAndReason(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/accounts/"+testGrokMediaAccountID+"/grok-media-eligibility", nil)
	...
	require.Equal(t, testGrokMediaAccountID, envelope.Data.AccountID)
...
func TestGrokMediaEligibilityHandlerUpdateModesPreservesExtra(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.mode, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPut, "/accounts/"+testGrokMediaAccountID+"/grok-media-eligibility", bytes.NewReader(body))
...
func TestGrokMediaEligibilityHandlerRejectsUnsupportedAndInvalidModes(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "unsupported account", account: &service.Account{ID: testGrokMediaAccountID, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth}, body: `{"mode":"enabled"}`},
		{name: "invalid mode", account: &service.Account{ID: testGrokMediaAccountID, Platform: service.PlatformGrok, Type: service.AccountTypeOAuth}, body: `{"mode":"force"}`},
	} {
		t.Run(tt.name, func(t *testing.T) {
			router := newGrokMediaEligibilityRouter(tt.account)
			req := httptest.NewRequest(http.MethodPut, "/accounts/"+testGrokMediaAccountID+"/grok-media-eligibility", bytes.NewBufferString(tt.body))
			req.Header.Set("Content-Type", "application/json")
			rec := httptest.NewRecorder()
			router.ServeHTTP(rec, req)
