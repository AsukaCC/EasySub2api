...
func enrichShadowParentInfo(items []AccountWithConcurrency, parents map[string]*service.Account) {
	for i := range items {
		a := items[i].Account
...
func (h *AccountHandler) enrichShadowParents(ctx context.Context, items []AccountWithConcurrency) {
	...
	parentIDs := make([]string, 0, len(seen))
	for pid := range seen {
	...
	pmap := make(map[string]*service.Account, len(parents))
