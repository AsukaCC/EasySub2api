...
type GenerateRedeemCodesRequest struct {
	...
	GroupID           *string    `json:"group_id"`      // 订阅类型必填
...
type CreateAndRedeemCodeRequest struct {
	...
	GroupID           *string    `json:"group_id"`      // subscription 类型必填
...
func (h *RedeemHandler) GetByID(c *gin.Context) {
	codeID, err := parseEntityID(c.Param("id"))
	if err != nil {
...
func (h *RedeemHandler) resolveCreateAndRedeemExisting(ctx context.Context, existing *service.RedeemCode, userID string) (any, error) {
	if existing == nil {
...
func redeemBatchUpdateFieldsFromDTO(in dto.BatchUpdateRedeemCodeFields) service.RedeemCodeBatchUpdateFields {
	...
	if in.GroupID.Set {
		out.GroupID = service.NullableStringUpdate{Set: true, Value: in.GroupID.Value}
	}
	return out
}
