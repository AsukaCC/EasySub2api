...
const (
	dataType             = "easysub2api-data"
	legacyDataType       = "sub2api-data"
	oldestLegacyDataType = "sub2api-bundle"
	dataVersion          = 1
	dataPageCap          = 1000
)
...
func buildProxyKey(protocol, host string, port int, username, password string) string {
	return fmt.Sprintf("%s|%s|%v|%s|%s", strings.TrimSpace(protocol), strings.TrimSpace(host), port, strings.TrimSpace(username), strings.TrimSpace(password))
}
...
func (h *AccountHandler) ExportData(c *gin.Context) {
	...
	proxyNameByID := make(map[string]string, len(proxies))
	for i := range proxies {
		proxyNameByID[proxies[i].ID] = proxies[i].Name
	}

	proxyKeyByID := make(map[string]string, len(proxies))
...
func (h *AccountHandler) importData(ctx context.Context, req DataImportRequest) (DataImportResult, error) {
	...
	proxyKeyToID := make(map[string]string, len(existingProxies))
	// proxyNameToID 用于 backup_proxy_name 反查：DB 已有 + 本批次新建均会写入
	proxyNameToID := make(map[string]string, len(existingProxies))
		...
		if existingID, ok := proxyKeyToID[key]; ok {
			...
			if normalizedStatus != "" {
				if proxy, getErr := h.adminService.GetProxy(ctx, existingID); getErr == nil && proxy != nil && proxy.Status != normalizedStatus {
					...
					var existingBackupProxyID *string
...
func (h *AccountHandler) resolveExportAccounts(ctx context.Context, ids []string, c *gin.Context) ([]service.Account, error) {
	if len(ids) > 0 {
...
func parseAccountIDs(c *gin.Context) ([]string, error) {
...
func validateDataHeader(payload DataPayload) error {
	if payload.Type != "" && payload.Type != dataType && payload.Type != legacyDataType && payload.Type != oldestLegacyDataType {
		return fmt.Errorf("unsupported data type: %s", payload.Type)
	}
	if payload.Version != 0 && payload.Version != dataVersion {
		return fmt.Errorf("unsupported data version: %v", payload.Version)
	}
