...
func (h *UsageHandler) List(c *gin.Context) {
	...
	if apiKeyIDStr := c.Query("api_key_id"); apiKeyIDStr != "" {
		id, err := parseEntityID(apiKeyIDStr)
		if err != nil {
	...
	if accountIDStr := c.Query("account_id"); accountIDStr != "" {
		id, err := parseEntityID(accountIDStr)
		if err != nil {
	...
	if groupIDStr := c.Query("group_id"); groupIDStr != "" {
		id, err := parseEntityID(groupIDStr)
		if err != nil {
...
func (h *UsageHandler) ListCleanupTasks(c *gin.Context) {
	...
	page, pageSize := response.ParsePagination(c)
	logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 请求清理任务列表: operator=%v page=%v page_size=%v", operator, page, pageSize)
	params := pagination.PaginationParams{Page: page, PageSize: pageSize}
	tasks, result, err := h.cleanupService.ListTasks(c.Request.Context(), params)
	if err != nil {
		logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 查询清理任务列表失败: operator=%v page=%v page_size=%v err=%v", operator, page, pageSize, err)
		response.ErrorFrom(c, err)
		return
	}
	...
	logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 返回清理任务列表: operator=%v total=%v items=%v page=%v page_size=%v", operator, result.Total, len(out), page, pageSize)
	response.Paginated(c, out, result.Total, page, pageSize)
}
...
func (h *UsageHandler) CreateCleanupTask(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.usage.cleanup_tasks.create", idempotencyPayload, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 请求创建清理任务: operator=%v start=%s end=%s user_id=%v api_key_id=%v account_id=%v group_id=%v model=%v request_type=%v stream=%v billing_type=%v tz=%q",
			subject.UserID,
		...
		if err != nil {
			logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 创建清理任务失败: operator=%v err=%v", subject.UserID, err)
			return nil, err
		}
		logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 清理任务已创建: task=%v operator=%v status=%s", task.ID, subject.UserID, task.Status)
...
func (h *UsageHandler) CancelCleanupTask(c *gin.Context) {
	...
	taskID, err := parseEntityID(idStr)
	if err != nil || taskID == "" {
		response.BadRequest(c, "Invalid task id")
		return
	}
	logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 请求取消清理任务: task=%v operator=%v", taskID, subject.UserID)
	if err := h.cleanupService.CancelTask(c.Request.Context(), taskID, subject.UserID); err != nil {
		logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 取消清理任务失败: task=%v operator=%v err=%v", taskID, subject.UserID, err)
		response.ErrorFrom(c, err)
		return
	}
	logger.LegacyPrintf("handler.admin.usage", "[UsageCleanup] 清理任务已取消: task=%v operator=%v", taskID, subject.UserID)
