...
func (h *OpsHandler) GetConcurrencyStats(c *gin.Context) {
	...
	if !h.opsService.IsRealtimeMonitoringEnabled(c.Request.Context()) {
		response.Success(c, gin.H{
			...
			"group":     map[string]*service.GroupConcurrencyInfo{},
			"account":   map[string]*service.AccountConcurrencyInfo{},
...
func (h *OpsHandler) GetUserConcurrencyStats(c *gin.Context) {
	...
	if !h.opsService.IsRealtimeMonitoringEnabled(c.Request.Context()) {
		response.Success(c, gin.H{
			"enabled":   false,
			"user":      map[string]*service.UserConcurrencyInfo{},
			"timestamp": time.Now().UTC(),
		})
		return
	}
...
func (h *OpsHandler) GetAccountAvailability(c *gin.Context) {
	...
	if !h.opsService.IsRealtimeMonitoringEnabled(c.Request.Context()) {
		response.Success(c, gin.H{
			...
			"group":     map[string]*service.GroupAvailability{},
			"account":   map[string]*service.AccountAvailability{},
			"timestamp": time.Now().UTC(),
		})
		return
	}
