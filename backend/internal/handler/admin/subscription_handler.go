...
type BulkAssignSubscriptionRequest struct {
	UserIDs      []string `json:"user_ids" binding:"required,min=1,max=100,dive,required"`
	GroupID      string   `json:"group_id" binding:"required"`
	ValidityDays int      `json:"validity_days" binding:"omitempty,max=36500"` // max 100 years
	Notes        string   `json:"notes"`
}
...
// BulkAction applies one operation to selected subscriptions and returns each outcome.
...
func (h *SubscriptionHandler) BulkAction(c *gin.Context) {
	...
	executeAdminIdempotentJSONWithTimeout(
		c,
