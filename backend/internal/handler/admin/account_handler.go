...
func parseAccountExpiryStatusFilter(raw string) (string, error) {
	value := strings.TrimSpace(raw)
	switch value {
	case "", service.AccountExpiryStatusExpiring, service.AccountExpiryStatusExpired:
		return value, nil
	default:
		return "", infraerrors.BadRequest("INVALID_EXPIRY_STATUS_FILTER", "invalid expiry status filter")
	}
}
...
func (h *AccountHandler) buildAccountResponseWithRuntime(ctx context.Context, account *service.Account) AccountWithConcurrency {
	...
	if h.concurrencyService != nil {
		if counts, err := h.concurrencyService.GetAccountConcurrencyBatch(ctx, []string{account.ID}); err == nil {
	...
	if account.IsAnthropicOAuthOrSetupToken() {
		...
		if h.sessionLimitCache != nil && account.GetMaxSessions() > 0 {
			...
			idleTimeouts := map[string]time.Duration{account.ID: idleTimeout}
			if sessions, err := h.sessionLimitCache.GetActiveSessionCountBatch(ctx, []string{account.ID}, idleTimeouts); err == nil {
...
func (h *AccountHandler) scoreOpenAIAccountSchedulerPool(ctx context.Context, accounts []service.Account, loadMap map[string]*service.AccountLoadInfo) map[string]AccountSchedulerScore {
	if len(accounts) == 0 {
		return nil
	}
	...
	var scores map[string]service.OpenAIAccountSchedulerScoreSnapshot
	...
	result := make(map[string]AccountSchedulerScore, len(scores))
...
func (h *AccountHandler) fetchOpenAIAccountLoadMap(ctx context.Context, openAIAccounts []*service.Account) map[string]*service.AccountLoadInfo {
	loadMap := map[string]*service.AccountLoadInfo{}
	if h.concurrencyService == nil || len(openAIAccounts) == 0 {
		return loadMap
	}
	seen := make(map[string]struct{}, len(openAIAccounts))
...
func (h *AccountHandler) buildOpenAIAccountSchedulerScores(
...
) (map[string]*AccountSchedulerScore, map[string][]AccountSchedulerGroupScore) {
	if len(accounts) == 0 {
		return nil, nil
	}
	if len(filterPool) == 0 {
	...
	pageOpenAIAccountIDs := make(map[string]struct{})
	...
	groupPools := make(map[string][]service.Account, len(groupIDList))
	...
	baseScores := make(map[string]*AccountSchedulerScore)
	for accountID, score := range h.scoreOpenAIAccountSchedulerPool(ctx, filterPool, loadMap) {
		copiedScore := score
		baseScores[accountID] = &copiedScore
	}

	groupScoresByAccount := make(map[string][]AccountSchedulerGroupScore)
	scoreGroupPool := func(groupID *string, groupNameByID map[string]string, groupPriorityByAccount map[string]int, pool []service.Account) {
		if len(pool) == 0 {
			return
		}
	...
	for _, groupID := range groupIDList {
		...
		groupPriorityByAccount := make(map[string]int)
		for i := range pool {
			account := &pool[i]
			for _, accountGroup := range account.AccountGroups {
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	expiryStatus, expiryStatusErr := parseAccountExpiryStatusFilter(c.Query("expiry_status"))
	...
	if groupIDStr := c.Query("group"); groupIDStr != "" {
		if groupIDStr == accountListGroupUngroupedQueryValue {
			...
			parsedGroupID, parseErr := parseEntityID(groupIDStr)
			if parseErr != nil {
	...
	var windowCosts map[string]float64
	var activeSessions map[string]int
	var rpmCounts map[string]int
	// 双重门控：用户要看该列，且当前页确实有 OpenAI 账号，才进入昂贵的候选池打分路径。
	var schedulerScores map[string]*AccountSchedulerScore
	var schedulerGroupScores map[string][]AccountSchedulerGroupScore
	...
	windowCostAccountIDs := make([]string, 0)
	sessionLimitAccountIDs := make([]string, 0)
	rpmAccountIDs := make([]string, 0)
	sessionIdleTimeouts := make(map[string]time.Duration) // 各账号的会话空闲超时配置
	for i := range accounts {
		acc := &accounts[i]
	...
	if len(windowCostAccountIDs) > 0 {
		windowCosts = make(map[string]float64)
		var mu sync.Mutex
...
// Returns the updated account and an optional warning.
func (h *AccountHandler) refreshSingleAccount(ctx context.Context, account *service.Account) (*service.Account, string, error) {
	...
	if account.IsOpenAI() {
		...
		if err != nil {
			return nil, "", fmt.Errorf("failed to refresh Gemini credentials: %w", err)
		}
		newCredentials = service.MergeCredentials(account.Credentials, h.geminiOAuthService.BuildAccountCredentials(tokenInfo))
		...
		newCredentials = service.MergeCredentials(account.Credentials, h.antigravityOAuthService.BuildAccountCredentials(tokenInfo))
		newProjectID, _ := newCredentials["project_id"].(string)
		if strings.TrimSpace(account.GetCredential("project_id")) != "" && strings.TrimSpace(newProjectID) == "" {
			newCredentials["project_id"] = account.GetCredential("project_id")
		}
	...
	if h.tokenCacheInvalidator != nil {
		if invalidateErr := h.tokenCacheInvalidator.InvalidateToken(ctx, updatedAccount); invalidateErr != nil {
			log.Printf("[WARN] Failed to invalidate token cache for account %v: %v", updatedAccount.ID, invalidateErr)
		}
	}

	switch updatedAccount.Platform {
...
func (h *AccountHandler) ClearError(c *gin.Context) {
	...
	if h.tokenCacheInvalidator != nil && account.IsOAuth() {
		if invalidateErr := h.tokenCacheInvalidator.InvalidateToken(c.Request.Context(), account); invalidateErr != nil {
			log.Printf("[WARN] Failed to invalidate token cache for account %v: %v", accountID, invalidateErr)
		}
	}
...
func (h *AccountHandler) BatchDelete(c *gin.Context) {
	...
	requestedIDs := make(map[string]struct{}, len(accountIDs))
	...
	rootIDs := make([]string, 0, len(accountIDs))
	dependentIDs := make(map[string][]string)
	...
	for _, accountID := range accountIDs {
		...
		rootID := accountID
		visited := map[string]struct{}{accountID: {}}
		for {
	...
	var mu sync.Mutex
	successIDs := make([]string, 0, len(accountIDs))
	...
	for _, id := range rootIDs {
		...
		g.Go(func() error {
			...
			mu.Lock()
			defer mu.Unlock()
			affectedIDs := append([]string{accountID}, dependentIDs[accountID]...)
			if err != nil {
...
func (h *AccountHandler) BatchRefresh(c *gin.Context) {
	...
	foundIDs := make(map[string]bool, len(accounts))
...
func (h *AccountHandler) BatchUpdateCredentials(c *gin.Context) {
	...
	for _, accountID := range req.AccountIDs {
		...
		if err != nil {
			response.Error(c, 404, fmt.Sprintf("Account %v not found", accountID))
			return
		}
	...
	success := 0
	failed := 0
	successIDs := make([]string, 0, len(updates))
	failedIDs := make([]string, 0, len(updates))
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsOpenAI() {
		// 优先使用账号上游的 live 模型目录；发现失败时回落到下方本地目录，
		// 保证测试弹窗始终可用。
		if h.accountTestService != nil {
...
func (h *AccountHandler) SyncUpstreamModelsPreview(c *gin.Context) {
	...
	if len(req.ModelMapping) > 0 {
		...
		for requestedModel, upstreamModel := range req.ModelMapping {
...
// SetPrivacy handles setting privacy for OpenAI and Antigravity OAuth accounts.
...
// RefreshTier refreshes Drive storage metadata and the inferred Google One tier.
func (h *AccountHandler) RefreshTier(c *gin.Context) {
	...
	if account.Platform != service.PlatformGemini || account.Type != service.AccountTypeOAuth || account.GeminiOAuthType() != "google_one" {
		response.BadRequest(c, "Only Gemini Google One OAuth accounts support tier refresh")
		return
	}
	tierID, extra, credentials, err := h.geminiOAuthService.RefreshAccountGoogleOneTier(ctx, account)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	if _, err = h.adminService.UpdateAccount(ctx, accountID, &service.UpdateAccountInput{Credentials: credentials, Extra: extra}); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{
...
// BatchRefreshTier refreshes selected Google One accounts, or all when omitted.
func (h *AccountHandler) BatchRefreshTier(c *gin.Context) {
	...
	if len(req.AccountIDs) == 0 {
		all, _, err := h.adminService.ListAccounts(ctx, 1, 10000, service.PlatformGemini, service.AccountTypeOAuth, "", "", "", "", "", "name", "asc")
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
		for index := range all {
			if all[index].GeminiOAuthType() == "google_one" {
		...
		for _, account := range fetched {
			if account != nil && account.Platform == service.PlatformGemini && account.Type == service.AccountTypeOAuth && account.GeminiOAuthType() == "google_one" {
	...
	var mu sync.Mutex
	var successCount, failedCount int
	errorsByAccount := make([]gin.H, 0)
	for _, candidate := range accounts {
		account := candidate
		group.Go(func() error {
			_, extra, credentials, err := h.geminiOAuthService.RefreshAccountGoogleOneTier(groupCtx, account)
			if err == nil {
				_, err = h.adminService.UpdateAccount(groupCtx, account.ID, &service.UpdateAccountInput{Credentials: credentials, Extra: extra})
			}
			mu.Lock()
			defer mu.Unlock()
			if err != nil {
				failedCount++
				errorsByAccount = append(errorsByAccount, gin.H{"account_id": account.ID, "error": err.Error()})
			} else {
				successCount++
			}
			return nil
	...
	response.Success(c, gin.H{"total": len(accounts), "success": successCount, "failed": failedCount, "errors": errorsByAccount})
}

// GetAntigravityDefaultModelMapping returns the built-in Antigravity aliases.
