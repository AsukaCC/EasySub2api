...
// SetUserLevelService attaches rolling-spend level management without changing
// the long-standing constructor used by focused handler tests.
func (h *UserHandler) SetUserLevelService(userLevelService *service.UserLevelService) {
	h.userLevelService = userLevelService
}

type UpdateUserLevelSettingsRequest struct {
	L2MinSpend float64 `json:"l2_min_spend" binding:"gte=0"`
	L3MinSpend float64 `json:"l3_min_spend" binding:"required,gt=0"`
}

type BatchUserLevelProfilesRequest struct {
...
func (h *UserHandler) GetLevelSettings(c *gin.Context) {
	...
	settings, err := h.userLevelService.GetSettings(c.Request.Context())
	if err != nil {
...
// UpdateLevelSettings updates L2/L3 thresholds. L1 always starts at zero.
func (h *UserHandler) UpdateLevelSettings(c *gin.Context) {
	if h.userLevelService == nil {
		response.InternalError(c, "User level service unavailable")
		return
	}
	var req UpdateUserLevelSettingsRequest
...
func (h *UserHandler) BatchLevelProfiles(c *gin.Context) {
	if h.userLevelService == nil {
		response.InternalError(c, "User level service unavailable")
		return
	}
	var req BatchUserLevelProfilesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	...
	profilesByID, err := h.userLevelService.GetProfiles(c.Request.Context(), userIDs, time.Now())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	profiles := make([]service.UserLevelProfile, 0, len(userIDs))
...
type CreateUserRequest struct {
	...
	BalanceType       string   `json:"balance_type" binding:"omitempty,oneof=recharge bonus"`
	BonusValidityDays int      `json:"bonus_validity_days" binding:"omitempty,min=1,max=3650"`
...
type UpdateUserRequest struct {
	...
	GroupRates map[string]*float64 `json:"group_rates"`
}
...
func (h *UserHandler) List(c *gin.Context) {
	...
	if raw := strings.TrimSpace(c.Query("api_key_group_id")); raw != "" {
		if id, parseErr := parseEntityID(raw); parseErr == nil && id != "" {
...
func (h *UserHandler) UpdateBalance(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.users.balance.update", idempotencyPayload, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		user, execErr := h.adminService.UpdateUserWalletBalance(ctx, userID, req.Balance, req.Operation, req.BalanceType, req.BonusValidityDays, req.Notes)
		if execErr != nil {
			return nil, execErr
		}
...
type ReplaceGroupRequest struct {
	OldGroupID string `json:"old_group_id" binding:"required,gt=0"`
	NewGroupID string `json:"new_group_id" binding:"required,gt=0"`
}
...
func (h *UserHandler) GetUserPlatformQuotas(c *gin.Context) {
	idStr := c.Param("id")
	userID, err := parseEntityID(idStr)
	if err != nil {
		response.BadRequest(c, "invalid user id")
		return
	}
...
type PlatformQuotaInput struct {
	...
	DailyLimitPoints   optionalLimitField `json:"daily_limit_points"`
	WeeklyLimitPoints  optionalLimitField `json:"weekly_limit_points"`
	MonthlyLimitPoints optionalLimitField `json:"monthly_limit_points"`
...
func (h *UserHandler) UpdateUserPlatformQuotas(c *gin.Context) {
	...
	if len(req.Quotas) > len(service.AllowedQuotaPlatforms) {
		response.BadRequest(c, fmt.Sprintf("quotas length must be <= %v", len(service.AllowedQuotaPlatforms)))
		return
	}
	...
	for _, q := range req.Quotas {
		...
		dailyLimit, resolveErr := resolveLimitFieldAlias(q.DailyLimitPoints, q.DailyLimitUSD, "daily_limit_points", "daily_limit_usd")
		if resolveErr != nil {
			response.BadRequest(c, resolveErr.Error())
			return
		}
		weeklyLimit, resolveErr := resolveLimitFieldAlias(q.WeeklyLimitPoints, q.WeeklyLimitUSD, "weekly_limit_points", "weekly_limit_usd")
		if resolveErr != nil {
			response.BadRequest(c, resolveErr.Error())
			return
		}
		monthlyLimit, resolveErr := resolveLimitFieldAlias(q.MonthlyLimitPoints, q.MonthlyLimitUSD, "monthly_limit_points", "monthly_limit_usd")
		if resolveErr != nil {
			response.BadRequest(c, resolveErr.Error())
			return
		}
		// *_limit_points 的语义（旧 *_limit_usd 字段仅作兼容别名）：
		...
		for _, f := range []struct {
			...
			{"daily_limit_points", dailyLimit.value},
			{"weekly_limit_points", weeklyLimit.value},
			{"monthly_limit_points", monthlyLimit.value},
		} {
			if f.val == nil {
				continue
			}
		...
		records = append(records, service.UserPlatformQuotaRecord{
			UserID:          userID,
			Platform:        q.Platform,
			DailyLimitUSD:   dailyLimit.value,
			WeeklyLimitUSD:  weeklyLimit.value,
			MonthlyLimitUSD: monthlyLimit.value,
		})
	}
