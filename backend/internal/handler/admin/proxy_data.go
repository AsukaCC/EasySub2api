...
func (h *ProxyHandler) ImportData(c *gin.Context) {
	...
	latencyProbeIDs := make([]string, 0, len(req.Data.Proxies))
	...
	if len(latencyProbeIDs) > 0 {
		ids := append([]string(nil), latencyProbeIDs...)
		go func() {
			for _, id := range ids {
...
func (h *ProxyHandler) getProxiesByIDs(ctx context.Context, ids []string) ([]service.Proxy, error) {
	if len(ids) == 0 {
...
func parseProxyIDs(c *gin.Context) ([]string, error) {
