...
type grokImportProber interface {
	QueryQuota(ctx context.Context, accountID string) (*service.GrokQuotaProbeResult, error)
}
...
func (s *grokImportProbeScheduler) schedule(prober grokImportProber, account *service.Account) {
	if s == nil || prober == nil || account == nil || account.ID == "" {
		return
	}
...
func (s *grokImportProbeScheduler) finish(accountID string) {
	s.mu.Lock()
	delete(s.inFlight, accountID)
	s.mu.Unlock()
}

func (s *grokImportProbeScheduler) run(prober grokImportProber, accountID string) {
	defer func() {
