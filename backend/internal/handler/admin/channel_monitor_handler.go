...
func ParseChannelMonitorID(c *gin.Context) (string, bool) {
...
func (h *ChannelMonitorHandler) batchSummaryFor(c *gin.Context, items []*service.ChannelMonitor) map[string]service.MonitorStatusSummary {
	ids := make([]string, 0, len(items))
	primaryByID := make(map[string]string, len(items))
	extrasByID := make(map[string][]string, len(items))
	for _, m := range items {
		ids = append(ids, m.ID)
