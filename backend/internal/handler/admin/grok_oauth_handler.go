...
func (h *GrokOAuthHandler) ResetQuota(c *gin.Context) {
	...
	result, err := h.quotaService.ResetQuota(c.Request.Context(), accountID) //nolint:staticcheck // The upstream quota reset endpoint is currently unsupported.
	if err != nil {                                                          //nolint:staticcheck // The upstream quota reset endpoint is currently unsupported.
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, result)
