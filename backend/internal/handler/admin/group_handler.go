...
func resolveLimitFieldAlias(points, legacy optionalLimitField, pointsName, legacyName string) (optionalLimitField, error) {
	if !points.set {
		return legacy, nil
	}
	if !legacy.set {
	...
	if (points.value == nil) != (legacy.value == nil) ||
		(points.value != nil && *points.value != *legacy.value) {
		return optionalLimitField{}, fmt.Errorf("%s conflicts with legacy field %s", pointsName, legacyName)
	}
	return points, nil
}
...
type CreateGroupRequest struct {
	...
	FallbackGroupIDOnInvalidRequest *string                        `json:"fallback_group_id_on_invalid_request"`
	...
	// 从指定分组复制账号（创建后自动绑定）
	CopyAccountsFromGroupIDs []string `json:"copy_accounts_from_group_ids"`
}
...
func parsePositiveIDParam(c *gin.Context, name string) (string, bool) {
	raw := c.Param(name)
...
func (h *GroupHandler) GetModelsListCandidates(c *gin.Context) {
	...
	h.getModelsListCandidates(c, groupID)
}

// GetModelsListCandidatesForCreate returns candidates before a group ID exists.
// GET /api/v1/admin/groups/models-list-candidates?platform=...
func (h *GroupHandler) GetModelsListCandidatesForCreate(c *gin.Context) {
	h.getModelsListCandidates(c, "")
}

func (h *GroupHandler) getModelsListCandidates(c *gin.Context, currentGroupID string) {
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	dailyLimit, err := resolveLimitFieldAlias(req.DailyLimitPoints, req.DailyLimitUSD, "daily_limit_points", "daily_limit_usd")
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	weeklyLimit, err := resolveLimitFieldAlias(req.WeeklyLimitPoints, req.WeeklyLimitUSD, "weekly_limit_points", "weekly_limit_usd")
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	monthlyLimit, err := resolveLimitFieldAlias(req.MonthlyLimitPoints, req.MonthlyLimitUSD, "monthly_limit_points", "monthly_limit_usd")
	if err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		LevelRateMultipliers:            req.LevelRateMultipliers,
		DynamicRateRules:                req.DynamicRateRules,
		IsExclusive:                     req.IsExclusive,
		SubscriptionType:                req.SubscriptionType,
		DailyLimitUSD:                   dailyLimit.ToServiceInput(),
		WeeklyLimitUSD:                  weeklyLimit.ToServiceInput(),
		MonthlyLimitUSD:                 monthlyLimit.ToServiceInput(),
...
func (h *GroupHandler) Update(c *gin.Context) {
	...
	group, err := h.adminService.UpdateGroup(c.Request.Context(), groupID, &service.UpdateGroupInput{
		...
		WeeklyLimitUSD:                  weeklyLimit.ToServiceInput(),
		MonthlyLimitUSD:                 monthlyLimit.ToServiceInput(),
		DailyLimitUSDSet:                dailyLimit.set,
		WeeklyLimitUSDSet:               weeklyLimit.set,
		MonthlyLimitUSDSet:              monthlyLimit.set,
		LongContextPricingEnabled:       req.LongContextPricingEnabled,
		ModelPricing:                    req.ModelPricing,
		AllowImageGeneration:            req.AllowImageGeneration,
		ImageRateIndependent:            req.ImageRateIndependent,
