...
type openAIQuotaService interface {
	QueryUsage(ctx context.Context, accountID string) (*service.OpenAIQuotaUsage, error)
	CacheResetCreditsSnapshot(ctx context.Context, accountID string, credits *service.OpenAIRateLimitResetCredits) error
	ResetCredit(ctx context.Context, accountID string) (*service.OpenAIQuotaResetResult, error)
}

type openAIAccountStateRecoverer interface {
	RecoverAccountState(ctx context.Context, accountID string, options service.AccountRecoveryOptions) (*service.SuccessfulTestRecoveryResult, error)
}
...
func (h *OpenAIOAuthHandler) RefreshQuota(c *gin.Context) {
	...
	if cache, ok := h.quotaService.(interface {
		CacheCreditsSnapshot(context.Context, string, *service.OpenAIQuotaUsage) error
	}); ok {
		if err := cache.CacheCreditsSnapshot(c.Request.Context(), accountID, usage); err == nil {
...
func (h *OpenAIOAuthHandler) CreateShadow(c *gin.Context) {
	parentID, err := parseEntityID(c.Param("id"))
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	}
