...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	// 获取平台：优先使用强制平台，其次使用 composite 解析出的目标平台，否则使用分组平台
	platform := ""
	...
	if sessionKey != "" {
		...
		reqLog.Info("sticky.cache_lookup",
			...
			zap.String("bound_account_id", sessionBoundAccountID),
		)
		if sessionBoundAccountID != "" {
			prefetchedGroupID := ""
	...
	for {
		...
		reqLog.Info("sticky.selecting_account",
			zap.String("session_key", sessionKey),
			zap.String("sticky_bound_account_id", sessionBoundAccountID),
		...
		if err != nil {
			if len(fs.FailedAccountIDs) == 0 {
				...
				reqLog.Warn("gateway.select_account_no_available",
					...
					zap.Stringp("group_id", currentAPIKey.GroupID),
		...
		reqLog.Info("sticky.account_selected",
			zap.String("selected_account_id", account.ID),
			zap.String("account_name", account.Name),
			zap.Bool("slot_acquired", selection.Acquired),
			zap.Bool("has_wait_plan", selection.WaitPlan != nil),
			zap.String("sticky_bound_account_id", sessionBoundAccountID),
			zap.Bool("sticky_honored", sessionBoundAccountID != "" && sessionBoundAccountID == account.ID),
		)
		...
		if !selection.Acquired {
			...
			if err != nil {
				reqLog.Warn("gateway.account_wait_counter_increment_failed", zap.String("account_id", account.ID), zap.Error(err))
			} else if !canWait {
			...
			if err != nil {
				reqLog.Warn("gateway.account_slot_acquire_failed", zap.String("account_id", account.ID), zap.Error(err))
				releaseWait()
		...
		if vetoed {
			...
			reqLog.Debug("gateway.account_slot_profit_vetoed", zap.String("account_id", account.ID), zap.String("reason", reason))
		...
		if selection.ProfitGateActive() || !selection.Acquired {
			if err := h.gatewayService.BindStickySessionAfterProfitAdmission(admissionCtx, currentAPIKey.GroupID, sessionKey, account.ID); err != nil {
				reqLog.Warn("gateway.bind_sticky_session_after_profit_admission_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
			...
			h.submitUsageRecordTask(c.Request.Context(), func(ctx context.Context) {
				if err := h.gatewayService.RecordUsage(ctx, &service.RecordUsageInput{
					...
					logger.L().With(
						...
						zap.String("user_id", subject.UserID),
						zap.String("api_key_id", currentAPIKey.ID),
		...
		if err != nil {
			...
			if account.Proxy != nil {
				...
				forwardFailedFields = append(forwardFailedFields, zap.Stringp("proxy_id", account.ProxyID))
			}
		...
		if account.IsAnthropicOAuthOrSetupToken() && account.GetBaseRPM() > 0 {
			if err := h.gatewayService.IncrementAccountRPM(c.Request.Context(), account.ID); err != nil {
				reqLog.Warn("gateway.rpm_increment_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
		...
		//   下次请求粘性账号恢复后仍可命中
		if sessionKey != "" && (sessionBoundAccountID == "" || sessionBoundAccountID == account.ID) {
			if err := h.gatewayService.BindStickySession(c.Request.Context(), currentAPIKey.GroupID, sessionKey, account.ID); err != nil {
				reqLog.Warn("gateway.bind_sticky_session_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
...
func (h *GatewayHandler) buildUsageData(ctx context.Context, apiKeyID string) gin.H {
	if h.usageService == nil {
		return nil
	}
...
func (h *GatewayHandler) buildAPIKeyDailyUsage(c *gin.Context, userID, apiKeyID string, days int) any {
	if h.usageService == nil {
		return nil
	}
	...
	if apiKey.Group != nil && apiKey.Group.IsSubscriptionType() {
		...
		if ok {
			...
			resp["subscription"] = gin.H{
				...
				"daily_usage_points":   subscription.DailyUsageUSD,
				"weekly_usage_usd":     subscription.WeeklyUsageUSD,
				"weekly_usage_points":  subscription.WeeklyUsageUSD,
				"monthly_usage_usd":    subscription.MonthlyUsageUSD,
				"monthly_usage_points": subscription.MonthlyUsageUSD,
				"daily_limit_usd":      apiKey.Group.DailyLimitUSD,
				"daily_limit_points":   apiKey.Group.DailyLimitUSD,
				"weekly_limit_usd":     apiKey.Group.WeeklyLimitUSD,
				"weekly_limit_points":  apiKey.Group.WeeklyLimitUSD,
				"monthly_limit_usd":    apiKey.Group.MonthlyLimitUSD,
				"monthly_limit_points": apiKey.Group.MonthlyLimitUSD,
...
// handleStreamingAwareErrorWithCode 与 handleStreamingAwareError 相同，但允许附带
// 稳定的 error.code（如网关准入拒绝码），在 JSON / SSE / response.failed 三种形态中透出。
...
// errorResponseWithCode 在 Claude 格式错误体中可选附带 error.code。
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	if err := h.gatewayService.ForwardCountTokens(c.Request.Context(), c, account, parsedReq); err != nil {
		reqLog.Error("gateway.count_tokens_forward_failed", zap.String("account_id", account.ID), zap.Error(err))
...
func generateRealisticMsgID() string {
	...
	if _, err := rand.Read(randomBytes); err != nil {
		return fmt.Sprintf("msg_01%v", time.Now().UnixNano())
	}
	b := make([]byte, idLen)
	for i := range b {
