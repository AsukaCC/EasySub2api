...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		if !selection.Acquired {
			...
			if err != nil {
				reqLog.Warn("gateway.responses.account_slot_acquire_failed", zap.String("account_id", account.ID), zap.Error(err))
		...
		if vetoed {
			...
			reqLog.Debug("gateway.responses.account_slot_profit_vetoed", zap.String("account_id", account.ID), zap.String("reason", reason))
		...
		if selection.ProfitGateActive() {
			if err := h.gatewayService.BindStickySessionAfterProfitAdmission(admissionCtx, apiKey.GroupID, sessionHash, account.ID); err != nil {
				reqLog.Warn("gateway.responses.bind_sticky_session_after_profit_admission_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
		...
		if prepareAntigravityCompatForward(c, account) {
			result, err = h.antigravityGatewayService.ForwardAsResponses(admissionCtx, c, account, forwardBody, parsedReq)
		} else {
			result, err = h.gatewayService.ForwardAsResponses(admissionCtx, c, account, forwardBody, parsedReq)
		}
...
func (h *GatewayHandler) handleResponsesFailoverExhausted(c *gin.Context, lastErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if streamStarted {
		return // Can't write non-capacity errors after stream started
	}
