...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	for {
		...
		if !selection.Acquired {
			...
			if err != nil {
				reqLog.Warn("gateway.cc.account_slot_acquire_failed", zap.String("account_id", account.ID), zap.Error(err))
		...
		if vetoed {
			...
			reqLog.Debug("gateway.cc.account_slot_profit_vetoed", zap.String("account_id", account.ID), zap.String("reason", reason))
		...
		if selection.ProfitGateActive() {
			if err := h.gatewayService.BindStickySessionAfterProfitAdmission(admissionCtx, apiKey.GroupID, selectionSessionHash, account.ID); err != nil {
				reqLog.Warn("gateway.cc.bind_sticky_session_after_profit_admission_failed", zap.String("account_id", account.ID), zap.Error(err))
			}
		}
