...
func (h *AnnouncementHandler) SetUserLevelService(userLevelService *service.UserLevelService) {
	if h.announcementService != nil {
		h.announcementService.SetUserLevelService(userLevelService)
	}
}
...
// ListPublicBanners handles listing banner announcements visible to anonymous
// visitors (active + banner mode + no targeting conditions).
// GET /api/v1/announcements/public
func (h *AnnouncementHandler) ListPublicBanners(c *gin.Context) {
	items, err := h.announcementService.ListPublicBanners(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	out := make([]dto.UserAnnouncement, 0, len(items))
	for i := range items {
		out = append(out, *dto.UserAnnouncementFromService(&service.UserAnnouncement{Announcement: items[i]}))
	}
	response.Success(c, out)
}
...
func (h *AnnouncementHandler) MarkRead(c *gin.Context) {
	...
	announcementID, err := parseEntityID(c.Param("id"))
	if err != nil || announcementID == "" {
