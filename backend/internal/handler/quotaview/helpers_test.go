...
func TestLazyZeroQuotaForResponse_ExposesPermanentPointsAliases(t *testing.T) {
	dailyLimit, weeklyLimit, monthlyLimit := 10.0, 20.0, 30.0
	r := service.UserPlatformQuotaRecord{
		Platform:        "openai",
		DailyUsageUSD:   1.25,
		WeeklyUsageUSD:  2.5,
	...
	for _, pair := range [][2]string{
		{"daily_usage_usd", "daily_usage_points"},
		{"weekly_usage_usd", "weekly_usage_points"},
		{"monthly_usage_usd", "monthly_usage_points"},
		{"daily_limit_usd", "daily_limit_points"},
		{"weekly_limit_usd", "weekly_limit_points"},
		{"monthly_limit_usd", "monthly_limit_points"},
	} {
		if out[pair[0]] != out[pair[1]] {
			t.Fatalf("compatibility fields %s and %s differ: %#v vs %#v", pair[0], pair[1], out[pair[0]], out[pair[1]])
		}
	}
}
