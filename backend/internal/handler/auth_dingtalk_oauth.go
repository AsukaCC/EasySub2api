...
func (h *AuthHandler) createDingTalkOAuthChoicePendingSession(
	...
	if compatEmailUser != nil && compatEmailUser.ID != "" {
...
func (h *AuthHandler) syncDingTalkIdentity(ctx context.Context, cfg config.DingTalkConnectConfig, client *DingTalkClient, userID string, staff *DingTalkStaffInfo, syncUsername bool) {
...
func (h *AuthHandler) syncDingTalkIdentityFromClaims(ctx context.Context, cfg config.DingTalkConnectConfig, client *DingTalkClient, userID string, claims map[string]any, syncUsername bool) {
...
func (h *AuthHandler) maybeSyncDingTalkAfterRegistration(ctx context.Context, session *dbent.PendingAuthSession, userID string) {
...
func (h *AuthHandler) maybeSyncDingTalkAfterLogin(ctx context.Context, session *dbent.PendingAuthSession, userID string) {
	h.dispatchDingTalkPendingSync(ctx, session, userID, false)
}

func (h *AuthHandler) dispatchDingTalkPendingSync(ctx context.Context, session *dbent.PendingAuthSession, userID string, syncUsername bool) {
...
func (h *AuthHandler) setUserAttributeByKey(ctx context.Context, userID string, key, value string) error {
...
func (h *AuthHandler) resolveDingTalkDeptPath(ctx context.Context, client *DingTalkClient, deptID int64) (string, error) {
	...
	for i := 0; i < maxDepth; i++ {
		...
		if err != nil {
			return "", fmt.Errorf("get dept info %v: %w", current, err)
		}
