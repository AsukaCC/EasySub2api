...
// to production accounts and account-scoped model rules. Composite is a group
// routing platform and is intentionally excluded.
...
func IsAccountPlatform(platform string) bool {
	for _, supported := range AccountPlatforms {
...
// DefaultAntigravityModelMapping is used when an Antigravity account does not
// provide an explicit model mapping.
