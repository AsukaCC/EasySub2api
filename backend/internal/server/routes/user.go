...
func RegisterUserRoutes(
	...
	authenticated.Use(gin.HandlerFunc(auditLog))
	{
		...
		user := authenticated.Group("/user")
		{
			...
			affiliate.POST("/bind", h.User.BindAffiliateCode)
		...
		if settingService != nil {
			channels.Use(userFeatureGate(settingService.IsAvailableChannelsUserAvailable))
		}
		{
		...
		// Passive channel monitoring views derived from real gateway traffic.
		monitorV2 := authenticated.Group("/channel-monitor-v2")
		monitorV2.Use(panelRateLimiter.Heavy())
		if settingService != nil {
			monitorV2.Use(userFeatureGate(settingService.IsChannelMonitorUserAvailable))
		}
