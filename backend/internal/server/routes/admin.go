...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	users := admin.Group("/users")
	{
		...
		users.GET("/level-rules", h.Admin.User.ListLevelRules)
		users.POST("/level-rules", h.Admin.User.CreateLevelRule)
		...
		users.DELETE("/:id", gin.HandlerFunc(stepUpAuth), h.Admin.User.Delete)
		...
		users.PUT("/:id/level-rules", h.Admin.User.ReplaceUserLevelRules)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.GET("/models-list-candidates", h.Admin.Group.GetModelsListCandidatesForCreate)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/profit", h.Admin.Account.ListProfit)
		accounts.GET("/admin-usage", h.Admin.Usage.ListAdmin)
		accounts.GET("/admin-usage/stats", h.Admin.Usage.StatsAdmin)
		accounts.GET("/admin-usage/search-users", h.Admin.Usage.SearchUsersAdmin)
		accounts.GET("/admin-usage/search-api-keys", h.Admin.Usage.SearchAPIKeysAdmin)
		accounts.GET("/admin-usage/trend", h.Admin.Dashboard.GetUsageTrendAdmin)
		accounts.GET("/admin-usage/models", h.Admin.Dashboard.GetModelStatsAdmin)
		accounts.GET("/admin-usage/groups", h.Admin.Dashboard.GetGroupStatsAdmin)
		accounts.GET("/admin-usage/snapshot", h.Admin.Dashboard.GetSnapshotV2Admin)
		...
		accounts.GET("/:id/profit", h.Admin.Account.GetProfit)
		accounts.PUT("/:id/profit-settings", h.Admin.Account.UpdateProfitSettings)
...
func registerAffiliateRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	affiliates := admin.Group("/affiliates")
	{
		...
		affiliates.GET("/reward-backfill/preview", h.Admin.Affiliate.PreviewRewardBackfill)
		affiliates.POST("/reward-backfill", h.Admin.Affiliate.StartRewardBackfill)
		affiliates.GET("/reward-backfill/:id", h.Admin.Affiliate.GetRewardBackfill)
...
func registerChannelMonitorV2Routes(admin *gin.RouterGroup, h *handler.Handlers, settingService *service.SettingService) {
	// Channel monitoring has one implementation: passive aggregation from real
	// gateway traffic. Config and read endpoints share the runtime feature gate.
	featureGuard := channelMonitorAdminFeatureGuard(settingService)
	passiveGuard := channelMonitorModeV2Guard(settingService)
...
// channelMonitorModeV2Guard is retained as a compatibility name. Passive
// monitoring is the only implementation, so it only checks the runtime switch.
