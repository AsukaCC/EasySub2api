...
func RegisterGatewayRoutes(
	...
	gateway.GET("/easysub2api/billing", h.Gateway.KeyBillingInfo)
	gateway.GET("/sub2api/billing", h.Gateway.KeyBillingInfo) // Deprecated compatibility alias.
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.DELETE("/images/tasks/:task_id", h.AsyncImage.Cancel)
		gateway.POST("/images/tasks/:task_id/retry", h.AsyncImage.Retry)
	...
	r.DELETE("/images/tasks/:task_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, h.AsyncImage.Cancel)
	r.POST("/images/tasks/:task_id/retry", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, h.AsyncImage.Retry)
	...
	// Antigravity 专用路由只调度 Antigravity 账号。
	r.GET("/antigravity/models", gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, h.Gateway.AntigravityModels)
	antigravityV1 := r.Group("/antigravity/v1")
	antigravityV1.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm)
	antigravityV1.Use(middleware.ForcePlatform(service.PlatformAntigravity))
	antigravityV1.Use(gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic)
	{
	...
	antigravityV1Beta.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm)
	antigravityV1Beta.Use(middleware.ForcePlatform(service.PlatformAntigravity))
	antigravityV1Beta.Use(middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg), requireGroupGoogle)
	{
