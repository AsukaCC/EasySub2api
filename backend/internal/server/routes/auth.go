...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/middleware"
...
func RegisterAuthRoutes(
	...
	// 公开横幅公告（无需认证）：仅返回生效中且未配置定向条件的 banner 公告，
	// 与公开设置同样按客户端 IP 兜底限流。
	publicAnnouncements := v1.Group("/announcements")
	publicAnnouncements.Use(panelRateLimiter.PublicIP())
	{
		publicAnnouncements.GET("/public", h.Announcement.ListPublicBanners)
	}
