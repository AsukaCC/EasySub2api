...
func SetAuditActor(c *gin.Context, userID string, email string) {
	if userID != "" {
...
var auditSensitiveReads = map[string]string{
	...
	"GET /api/v1/admin/affiliates/reward-backfill/preview": "admin.affiliates.reward_backfill.preview",
}
...
func NewAuditLogMiddleware(auditService *service.AuditLogService) AuditLogMiddleware {
	return AuditLogMiddleware(func(c *gin.Context) {
		...
		if subject, ok := GetAuthSubjectFromContext(c); ok && subject.UserID != "" {
			uid := subject.UserID
