...
func ProvideRouter(
	...
	settingService.SetWebSearchManagerBuilder(context.Background(), func(cfg *service.WebSearchEmulationConfig, proxyURLs map[string]string) {
...
func ProvideHTTPServer(cfg *config.Config, router *gin.Engine) *http.Server {
	...
	if globalMaxSize > 0 {
		...
		log.Printf("Global max request body size: %v bytes (%.2f MB)", globalMaxSize, float64(globalMaxSize)/(1<<20))
	}
	...
	if cfg.Server.H2C.Enabled {
		...
		if err := http2.ConfigureServer(server, &http2.Server{
			...
			log.Printf("HTTP/2 Cleartext (h2c) enabled: max_concurrent_streams=%v, idle_timeout=%vs, max_read_frame_size=%v, max_upload_buffer_per_connection=%v, max_upload_buffer_per_stream=%v",
