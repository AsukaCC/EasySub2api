...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/websearch"
...
type WebSearchProviderConfig struct {
	...
	ProxyID          *string `json:"proxy_id"`                // optional proxy association
...
func validateWebSearchConfig(cfg *WebSearchEmulationConfig) error {
	...
	if len(cfg.Providers) > maxWebSearchProviders {
		return fmt.Errorf("too many providers (max %v)", maxWebSearchProviders)
	}
	seen := make(map[string]bool, len(cfg.Providers))
	for i, p := range cfg.Providers {
		if !validProviderTypes[p.Type] {
			return fmt.Errorf("provider[%v]: invalid type %q", i, p.Type)
		}
		if p.QuotaLimit != nil && *p.QuotaLimit < 0 {
			return fmt.Errorf("provider[%v]: quota_limit must be > 0 or null", i)
		}
		if seen[p.Type] {
			return fmt.Errorf("provider[%v]: duplicate type %q", i, p.Type)
		}
		seen[p.Type] = true
	}
	return nil
...
func (s *SettingService) resolveProviderProxyURLs(ctx context.Context, cfg *WebSearchEmulationConfig) map[string]string {
	if cfg == nil || s.proxyRepo == nil {
		return nil
	}
	var ids []string
	for _, p := range cfg.Providers {
		if p.ProxyID != nil && *p.ProxyID != "" {
