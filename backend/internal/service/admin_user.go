...
func (s *adminServiceImpl) loadUserGroupRatesOneByOne(ctx context.Context, users []User) {
	...
	for i := range users {
		...
		if err != nil {
			logger.LegacyPrintf("service.admin", "failed to load user group rates: user_id=%v err=%v", users[i].ID, err)
			continue
		}
		users[i].GroupRates = rates
	}
...
func (s *adminServiceImpl) GetUser(ctx context.Context, id string) (*User, error) {
	...
	if latestErr != nil {
		logger.LegacyPrintf("service.admin", "failed to load user last_used_at: user_id=%v err=%v", id, latestErr)
	} else {
	...
	if s.userGroupRateRepo != nil {
		...
		if err != nil {
			logger.LegacyPrintf("service.admin", "failed to load user group rates: user_id=%v err=%v", id, err)
		} else {
...
func (s *adminServiceImpl) GetUserIncludeDeleted(ctx context.Context, id string) (*User, error) {
...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	if input.BonusValidityDays <= 0 && s.settingService != nil {
		input.BonusValidityDays = s.settingService.GetBonusBalanceDefaultValidityDays(ctx)
	}
	balance := 0.0
	...
	user := &User{
		...
		Role:                     role,
		Balance:                  balance,
		InitialBalanceKind:       input.BalanceType,
		InitialBonusValidityDays: input.BonusValidityDays,
	...
	if user.Role == RoleAdmin {
		logger.LegacyPrintf("service.admin", "audit: admin user created actor_admin_id=%v target_user_id=%v",
...
func (s *adminServiceImpl) assignDefaultSubscriptions(ctx context.Context, userID string) {
	if s.settingService == nil || s.defaultSubAssigner == nil || userID == "" {
		return
	}
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id string, input *UpdateUserInput) (*User, error) {
	...
	oldAllowedGroups := append([]string(nil), user.AllowedGroups...)
...
func sameInt64Set(a, b []string) bool {
	if len(a) != len(b) {
		return false
	}
	if len(a) == 0 {
...
func (s *adminServiceImpl) DeleteUser(ctx context.Context, id string) error {
	...
	deletionRepo, ok := s.userRepo.(UserDeletionRepository)
	if !ok {
		return infraerrors.InternalServer("USER_PERMANENT_DELETE_UNAVAILABLE", "permanent user deletion is unavailable")
	}
	var purgeResult *UserPurgeResult
	...
	if s.entClient != nil {
		...
		purgeResult, err = deletionRepo.PermanentlyDeleteUser(opCtx, id)
		if err != nil {
			return err
		}
		...
		purgeResult, err = deletionRepo.PermanentlyDeleteUser(ctx, id)
		if err != nil {
			return err
		}
	}
	...
	if s.authCacheInvalidator != nil {
		for _, keyValue := range purgeResult.APIKeys {
	...
	logger.LegacyPrintf("service.admin", "audit: user permanently deleted user_id=%v", id)
	return nil
}
...
func (s *adminServiceImpl) BatchUpdateConcurrency(ctx context.Context, userIDs []string, value int, mode string) (int, error) {
...
func (s *adminServiceImpl) BatchUpdateLimits(ctx context.Context, userIDs []string, concurrency, rpmLimit *int) (int, error) {
...
func (s *adminServiceImpl) UpdateUserBalance(ctx context.Context, userID string, balance float64, operation string, notes string) (*User, error) {
	return s.UpdateUserWalletBalance(ctx, userID, balance, operation, WalletKindRecharge, 0, notes)
}

func (s *adminServiceImpl) UpdateUserWalletBalance(ctx context.Context, userID string, balance float64, operation, balanceType string, bonusValidityDays int, notes string) (*User, error) {
	if balanceType == "" {
		balanceType = WalletKindRecharge
	}
	...
	case "set":
		...
		if balanceType == WalletKindBonus {
			...
			input.RechargeAmount = balance
		}
		_, err = s.userRepo.SetWalletBalance(ctx, input)
	default:
	...
	if s.billingCacheService != nil {
		go func() {
			...
			if err := s.billingCacheService.InvalidateUserBalance(cacheCtx, userID); err != nil {
				logger.LegacyPrintf("service.admin", "invalidate user balance cache failed: user_id=%v err=%v", userID, err)
			}
		}()
	}
...
func (s *adminServiceImpl) GetUserAPIKeys(ctx context.Context, userID string, page, pageSize int, sortBy, sortOrder string) ([]APIKey, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
...
func (s *adminServiceImpl) GetUserRPMStatus(ctx context.Context, userID string) (*UserRPMStatus, error) {
	...
	if err != nil {
		logger.LegacyPrintf("service.admin", "failed to get user rpm: user_id=%v err=%v", userID, err)
	}
	...
	for _, key := range keys {
		if key.GroupID != nil && *key.GroupID != "" {
	...
	for _, groupID := range groupIDs {
		...
		if getErr != nil {
			logger.LegacyPrintf("service.admin", "failed to get user group rpm: user_id=%v group_id=%v err=%v", userID, groupID, getErr)
		}
		...
		if s.groupRepo != nil {
			if group, groupErr := s.groupRepo.GetByIDLite(ctx, groupID); groupErr == nil && group != nil {
				...
				logger.LegacyPrintf("service.admin", "failed to get group rpm status metadata: group_id=%v err=%v", groupID, groupErr)
			}
		}
		...
		if s.userGroupRateRepo != nil {
			...
			if overrideErr != nil {
				logger.LegacyPrintf("service.admin", "failed to get rpm override: user_id=%v group_id=%v err=%v", userID, groupID, overrideErr)
...
func (s *adminServiceImpl) GetUserUsageStats(ctx context.Context, userID string, period string) (any, error) {
...
func (s *adminServiceImpl) GetUserBalanceHistory(ctx context.Context, userID string, page, pageSize int, codeType string) ([]RedeemCode, int64, float64, error) {
	pageResult, err := s.userRepo.ListWalletTransactions(ctx, userID, page, pageSize)
	if err != nil {
		return nil, 0, 0, err
	}
	...
	for _, item := range pageResult.Items {
		...
		codes = append(codes, RedeemCode{
			ID: item.ID, Code: code, Type: item.Action, Value: item.Amount,
			Status: StatusUsed, UsedBy: &userID, UsedAt: &createdAt, CreatedAt: createdAt,
			Notes: item.Notes,
		})
	}
	user, err := s.userRepo.GetByID(ctx, userID)
	...
	return codes, pageResult.Total, user.TotalRecharged, nil
}

func (s *adminServiceImpl) getAllUserBalanceHistory(ctx context.Context, userID string, params pagination.PaginationParams) ([]RedeemCode, int64, float64, error) {
...
func (s *adminServiceImpl) listRedeemBalanceHistoryForMerge(ctx context.Context, userID string, needed int) ([]RedeemCode, int64, error) {
	if needed <= 0 {
		return nil, 0, nil
	}
...
func (s *adminServiceImpl) listAffiliateBalanceHistoryForMerge(ctx context.Context, userID string, needed int) ([]RedeemCode, int64, error) {
	if needed <= 0 {
		return nil, 0, nil
	}
...
func (s *adminServiceImpl) listAffiliateBalanceHistory(ctx context.Context, userID string, params pagination.PaginationParams) ([]RedeemCode, int64, error) {
	if s == nil || s.entClient == nil || userID == "" {
		return nil, 0, nil
	}
...
func countAffiliateBalanceHistory(ctx context.Context, client *dbent.Client, userID string) (int64, error) {
...
func (s *adminServiceImpl) BindUserAuthIdentity(ctx context.Context, userID string, input AdminBindAuthIdentityInput) (*AdminBoundAuthIdentity, error) {
	if userID == "" {
	...
	if providerType == "" {
		return nil, infraerrors.BadRequest("INVALID_INPUT", "provider_type must be one of email, oidc, wechat, or dingtalk")
	}
...
func selectOwnedAdminAuthIdentity(records []*dbent.AuthIdentity, userID string) *dbent.AuthIdentity {
...
func hasAdminAuthIdentityOwnershipConflict(records []*dbent.AuthIdentity, userID string) bool {
	for _, record := range records {
		if record.UserID != userID {
			return true
		}
...
func selectOwnedAdminAuthIdentityChannel(records []*dbent.AuthIdentityChannel, userID string) *dbent.AuthIdentityChannel {
...
func hasAdminAuthIdentityChannelOwnershipConflict(records []*dbent.AuthIdentityChannel, userID string) bool {
	for _, record := range records {
		if record.Edges.Identity != nil && record.Edges.Identity.UserID != userID {
			return true
		}
...
func (s *adminServiceImpl) GetRedeemCode(ctx context.Context, id string) (*RedeemCode, error) {
...
func (s *adminServiceImpl) GenerateRedeemCodes(ctx context.Context, input *GenerateRedeemCodesInput) ([]RedeemCode, error) {
	...
	if input.Type == RedeemTypeBalance && input.BonusValidityDays <= 0 {
		if s.settingService != nil {
			input.BonusValidityDays = s.settingService.GetBonusBalanceDefaultValidityDays(ctx)
		} else {
			input.BonusValidityDays = defaultBonusValidityDays
		}
	}
...
func (s *adminServiceImpl) DeleteRedeemCode(ctx context.Context, id string) error {
	return s.redeemCodeRepo.Delete(ctx, id)
}

func (s *adminServiceImpl) BatchDeleteRedeemCodes(ctx context.Context, ids []string) (int64, error) {
	var deleted int64
	for _, id := range ids {
...
func (s *adminServiceImpl) ExpireRedeemCode(ctx context.Context, id string) (*RedeemCode, error) {
