...
func (s *adminServiceImpl) deleteUserWithAPIKeys(ctx context.Context, userID string, apiKeys []APIKey) error {
	if s.apiKeyRepo != nil {
		for _, key := range apiKeys {
			if key.ID == "" {
				continue
			...
			if err := s.apiKeyRepo.DeleteWithAudit(ctx, key.ID); err != nil {
				logger.LegacyPrintf("service.admin", "delete user api key failed: user_id=%v api_key_id=%v err=%v", userID, key.ID, err)
				return fmt.Errorf("delete user api key %v: %w", key.ID, err)
			}
		}
	}

	if err := s.userRepo.Delete(ctx, userID); err != nil {
		logger.LegacyPrintf("service.admin", "delete user failed: user_id=%v err=%v", userID, err)
		return err
	}
	return nil
}
