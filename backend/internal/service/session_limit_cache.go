...
type SessionLimitCache interface {
	...
	//   error: 操作错误
	RegisterSession(ctx context.Context, accountID string, sessionUUID string, maxSessions int, idleTimeout time.Duration) (allowed bool, err error)
	...
	GetActiveSessionCountBatch(ctx context.Context, accountIDs []string, idleTimeouts map[string]time.Duration) (map[string]int, error)

	// IsSessionActive 检查特定会话是否活跃（未过期）
	IsSessionActive(ctx context.Context, accountID string, sessionUUID string) (bool, error)
	...
	GetWindowCost(ctx context.Context, accountID string) (cost float64, hit bool, err error)

	// SetWindowCost 设置窗口费用缓存
	SetWindowCost(ctx context.Context, accountID string, cost float64) error

	// GetWindowCostBatch 批量获取窗口费用缓存
	// 返回 map[accountID]cost，缓存未命中的账号不在 map 中
	GetWindowCostBatch(ctx context.Context, accountIDs []string) (map[string]float64, error)
}
