...
func newGeminiSSECommentTestService(cfg config.GatewayConfig) *AntigravityGatewayService {
	return &AntigravityGatewayService{
		settingService: &SettingService{cfg: &config.Config{Gateway: cfg}},
	}
}

func newGeminiSSECommentTestContext(method, path string, body []byte) (*gin.Context, *httptest.ResponseRecorder) {
...
func TestDownstreamRejectsSSECommentsReadsBothHeaders(t *testing.T) {
	...
	c, _ := newGeminiSSECommentTestContext(http.MethodPost, "/v1beta/models/gemini-3.8-flash:streamGenerateContent", nil)
...
func runAntigravityGeminiStreamWithIdle(t *testing.T, userAgent string, idle time.Duration) string {
	t.Helper()
	gin.SetMode(gin.TestMode)
	svc := newGeminiSSECommentTestService(
		config.GatewayConfig{MaxLineSize: defaultMaxLineSize, StreamKeepaliveInterval: 1},
	)
	c, recorder := newGeminiSSECommentTestContext(http.MethodPost, "/v1beta/models/gemini-3.8-flash:streamGenerateContent", nil)
	if userAgent != "" {
