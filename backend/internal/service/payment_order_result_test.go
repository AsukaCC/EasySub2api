...
func TestCalculateCreateOrderPayAmountForSubscriptionIgnoresLegacyExchangeRate(t *testing.T) {
	t.Parallel()
	...
	if amountStr != "9.99" || amount != 9.99 {
		t.Fatalf("subscription points price = (%q, %v), want (9.99, 9.99)", amountStr, amount)
	}
}

func TestCalculateCreateOrderPayAmountForSubscriptionDoesNotConvertBeforeFee(t *testing.T) {
	t.Parallel()
	...
	if amountStr != "10.24" || amount != 10.24 {
		t.Fatalf("unconverted compatibility amount with fee = (%q, %v), want (10.24, 10.24)", amountStr, amount)
	}
}
...
// The legacy rate is ignored for every order type.
...
func TestBuildPaymentSubjectAppliesAffixToSubscriptionPlanDefaultName(t *testing.T) {
	...
	got := svc.buildPaymentSubject(plan, 0, cfg, nil)
	if got != "PRE EasySub2api Subscription Team Monthly SUF" {
		t.Fatalf("buildPaymentSubject() = %q, want %q", got, "PRE EasySub2api Subscription Team Monthly SUF")
	}
}
