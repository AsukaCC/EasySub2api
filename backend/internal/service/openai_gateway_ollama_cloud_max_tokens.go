...
// isOllamaCloudRawChatCompletionsAccount 判定 openai 平台 API Key 账号是否为
// 强制走 raw Chat Completions 的 Ollama Cloud 账号（base_url 指向 ollama.com，或
// 已挂载 Ollama Cloud 用量 extra）。
...
// 只看本次请求实际选用的 CC 上游 base_url（GetOpenAIBaseURL，与
// openAIChatCompletionsTargetURL 的取值一致）是否 ollama.com，不读 usage extra。
// DeepSeek 系模型（用模型映射后的真实出站 model，body 中 model 已改写）任意平台均
// clamp；非 DeepSeek 模型仅保留 openai 平台 Ollama 账号（force_chat_completions
// 判定）的 clamp，不扩展到其它平台。
func clampOllamaCloudUpstreamMaxTokens(account *Account, body []byte) []byte {
	if account == nil || len(body) == 0 {
		return body
	}
	if !isOllamaCloudBaseURL(normalizeOllamaCloudBaseURLForMatch(account.GetOpenAIBaseURL())) {
		return body
	}
...
// base_url，取值与 buildUpstreamRequest 一致（GetOpenAIBaseURL：凭证 base_url 或
// 平台默认）。
...
func ollamaCloudResponsesMaxOutputTokensClamp(account *Account, upstreamModel string, body []byte) (int64, bool) {
	...
	if !isOllamaCloudBaseURL(normalizeOllamaCloudBaseURLForMatch(ollamaCloudResponsesUpstreamBaseURL(account))) {
		return 0, false
	}
