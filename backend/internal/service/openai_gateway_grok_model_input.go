...
func sanitizeGrokResponsesModelInput(body []byte) ([]byte, error) {
	...
	for index, rawItem := range items {
		...
		if shouldStripGrokResponsesNonPairCallID(itemType) {
			delete(item, "call_id")
		}
...
// Only call/output pairs may carry call_id in the Responses replay schema.
// Keeping this Grok-local avoids changing the shared request sanitizer.
func shouldStripGrokResponsesNonPairCallID(itemType string) bool {
