...
func makeAccWithLoad(id string, priority int, loadRate int, lastUsed *time.Time, accType string) accountWithLoad {
...
func TestSortAccountsByPriorityAndLastUsed_ByPriority(t *testing.T) {
	...
	require.Equal(t, "2", accounts[0].ID, "优先级最低的排第一")
	require.Equal(t, "3", accounts[1].ID)
	require.Equal(t, "1", accounts[2].ID)
}
...
func TestSortAccountsByPriorityAndLastUsed_SamePriorityByLastUsed(t *testing.T) {
	...
	sortAccountsByPriorityAndLastUsed(accounts, false)
	require.Equal(t, "3", accounts[0].ID, "nil LastUsedAt 排最前")
	require.Equal(t, "2", accounts[1].ID, "更早使用的排前面")
	require.Equal(t, "1", accounts[2].ID)
}
...
func TestSortAccountsByPriorityAndLastUsed_PreferOAuth(t *testing.T) {
	...
	require.Equal(t, "2", accounts[0].ID, "preferOAuth 时 OAuth 账号排前面")
}
...
func TestSortAccountsByPriorityAndLastUsed_MixedPriorityAndTime(t *testing.T) {
	...
	sortAccountsByPriorityAndLastUsed(accounts, false)
	// 优先级1排前：nil < earlier
	require.Equal(t, "3", accounts[0].ID, "优先级1 + 更早")
	require.Equal(t, "2", accounts[1].ID, "优先级1 + 现在")
	// 优先级2排后：nil < time
	require.Equal(t, "1", accounts[2].ID, "优先级2 + nil")
	require.Equal(t, "4", accounts[3].ID, "优先级2 + 有时间")
}
...
func TestFilterByMinPriority_SelectsMinPriority(t *testing.T) {
	...
	require.Equal(t, "2", result[0].account.ID)
	require.Equal(t, "3", result[1].account.ID)
}
