...
func (s *SystemSettings) normalizeUserFeatureVisibility() {
	if !s.ChannelMonitorEnabled {
		s.ChannelMonitorUserVisible = false
	}
	if !s.AvailableChannelsEnabled {
		s.AvailableChannelsUserVisible = false
	}
	if !s.ModelPlazaEnabled {
		s.ModelPlazaUserVisible = false
	}
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	settings.normalizeUserFeatureVisibility()
	...
	if settings.BonusBalanceDefaultValidityDays <= 0 {
		settings.BonusBalanceDefaultValidityDays = defaultBonusValidityDays
	}
	...
	updates[SettingKeyAvailableChannelsUserVisible] = strconv.FormatBool(settings.AvailableChannelsUserVisible)

	// Model plaza feature switches + description
	updates[SettingKeyModelPlazaEnabled] = strconv.FormatBool(settings.ModelPlazaEnabled)
	updates[SettingKeyModelPlazaUserVisible] = strconv.FormatBool(settings.ModelPlazaUserVisible)
	...
	updates[SettingKeyAffiliateUserVisible] = strconv.FormatBool(settings.AffiliateUserVisible)
	updates[SettingPaymentUserVisible] = strconv.FormatBool(settings.PaymentUserVisible)
...
func (s *SettingService) validateDefaultSubscriptionGroups(ctx context.Context, items []DefaultSubscriptionSetting) error {
	...
	for _, item := range items {
		...
		if err != nil {
			...
			return fmt.Errorf("get default subscription group %v: %w", item.GroupID, err)
		}
