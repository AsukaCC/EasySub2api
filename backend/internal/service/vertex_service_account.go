...
func vertexServiceAccountCacheKey(account *Account, key *vertexServiceAccountKey) string {
	...
	if fingerprint == "" && account != nil {
		fingerprint = fmt.Sprintf("account:%v", account.ID)
	}
...
func getVertexServiceAccountAccessToken(ctx context.Context, cache OAuthTokenCache, account *Account) (string, error) {
...
func exchangeVertexServiceAccountToken(ctx context.Context, key *vertexServiceAccountKey, proxyURL string) (string, time.Duration, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return "", 0, fmt.Errorf("service account token request returned %v: %s", resp.StatusCode, msg)
	}
...
func buildVertexGeminiURL(projectID, location, model, action string, stream bool) (string, error) {
	...
	u := fmt.Sprintf(
		...
		host, url.PathEscape(projectID), url.PathEscape(location), url.PathEscape(model), action,
	)
	if stream {
		u += "?alt=sse"
	}
