...
//   - 海外平台（anthropic/openai/grok）→ AccountUsageService.GetUsage
...
type monitorUsageSource interface {
	GetUsage(ctx context.Context, accountID string, force ...bool) (*UsageInfo, error)
}

// monitorCNQuotaSource 国产 coding plan 滚动窗口额度探测（CNProviderQuotaService 天然满足）。
type monitorCNQuotaSource interface {
	QueryUsage(ctx context.Context, accountID string) (*CNProviderQuotaProbeResult, error)
}

// monitorCNBalanceSource 国产 payg 余额探测（CNProviderBalanceService 天然满足）。
type monitorCNBalanceSource interface {
	QueryBalance(ctx context.Context, accountID string) (*CNProviderBalanceResult, error)
}
...
type ChannelMonitorQuotaFetcher struct {
	...
	mu     sync.Mutex
	cache  map[string]monitorQuotaCacheEntry
...
func NewChannelMonitorQuotaFetcher(
	...
	f := &ChannelMonitorQuotaFetcher{cache: make(map[string]monitorQuotaCacheEntry)}
	if usage != nil {
		f.usage = usage
	}
	if cnQuota != nil {
...
func (f *ChannelMonitorQuotaFetcher) LoadAccount(ctx context.Context, id string) (*Account, error) {
...
func (f *ChannelMonitorQuotaFetcher) Fetch(ctx context.Context, accountID string) *domain.MonitorQuotaSnapshot {
	if f == nil {
...
func (f *ChannelMonitorQuotaFetcher) cachedSnapshot(accountID string, now time.Time) (*domain.MonitorQuotaSnapshot, bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *ChannelMonitorQuotaFetcher) storeSnapshot(accountID string, snapshot *domain.MonitorQuotaSnapshot, expiry time.Time) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.cache[accountID] = monitorQuotaCacheEntry{snapshot: snapshot, expiry: expiry}
}

func (f *ChannelMonitorQuotaFetcher) fetchUncached(ctx context.Context, accountID string, now time.Time) *domain.MonitorQuotaSnapshot {
	if f == nil {
...
func (f *ChannelMonitorQuotaFetcher) fetchUsage(ctx context.Context, accountID string, now time.Time) *domain.MonitorQuotaSnapshot {
	if f.usage == nil {
...
func (f *ChannelMonitorQuotaFetcher) fetchCNQuota(ctx context.Context, accountID string, now time.Time) *domain.MonitorQuotaSnapshot {
	if f.cnQuota == nil {
...
func (f *ChannelMonitorQuotaFetcher) fetchCNBalance(ctx context.Context, accountID string, now time.Time) *domain.MonitorQuotaSnapshot {
	if f.cnBalance == nil {
...
// usageFailureInfo 识别 GetUsage 经「值通道」返回的失败。
...
func usageFailureInfo(usage *UsageInfo) (failed, credentialInvalid bool, msg string) {
	...
	credentialInvalid = usage.NeedsReauth || usage.IsBanned || usage.IsForbidden ||
		usage.ErrorCode == "unauthenticated" || usage.ErrorCode == "forbidden"
