...
func pendingIdentityAdoptionLockKeys(pendingAuthSessionID string, identityID *string) []string {
	keys := []string{fmt.Sprintf("pending-auth-adoption:pending:%v", pendingAuthSessionID)}
	if identityID != nil && *identityID != "" {
		keys = append(keys, fmt.Sprintf("pending-auth-adoption:identity:%v", *identityID))
	}
	return keys
}
...
func (s *AuthPendingIdentityService) UpsertAdoptionDecision(ctx context.Context, input PendingIdentityAdoptionDecisionInput) (*dbent.IdentityAdoptionDecision, error) {
	...
	if input.IdentityID != nil && *input.IdentityID != "" {
