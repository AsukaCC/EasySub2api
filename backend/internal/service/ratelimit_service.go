...
type AccountRuntimeBlocker interface {
	...
	ClearAccountSchedulingBlock(accountID string)
}
...
func NewRateLimitService(accountRepo AccountRepository, cfg *config.Config, tempUnschedCache TempUnschedCache) *RateLimitService {
...
func (s *RateLimitService) SetGeminiQuotaDependencies(usageRepo UsageLogRepository, quotaService *GeminiQuotaService) {
	s.usageRepo = usageRepo
	s.geminiQuotaService = quotaService
}

func (s *RateLimitService) PreCheckUsage(ctx context.Context, account *Account, requestedModel string) (bool, error) {
	if account == nil || account.Platform != PlatformGemini || s.usageRepo == nil || s.geminiQuotaService == nil {
		return true, nil
	}
	...
	class := geminiModelClassFromName(requestedModel)
	dailyLimit, minuteLimit := quota.ProRPD, quota.ProRPM
	if quota.SharedRPD > 0 {
		dailyLimit = quota.SharedRPD
	} else if class == geminiModelFlash {
		dailyLimit = quota.FlashRPD
	}
	if quota.SharedRPM > 0 {
		minuteLimit = quota.SharedRPM
	} else if class == geminiModelFlash {
		minuteLimit = quota.FlashRPM
	}
	now := time.Now()
	check := func(start time.Time, limit int64, shared bool) (bool, error) {
		if limit <= 0 {
			return true, nil
		}
		stats, err := s.usageRepo.GetModelStatsWithFilters(ctx, start, now, "", "", account.ID, "", nil, nil, nil)
		if err != nil {
			return true, err
		}
	...
	if allowed, err := check(geminiDailyWindowStart(now), dailyLimit, quota.SharedRPD > 0); err != nil || !allowed {
		return allowed, err
	}
	return check(now.Truncate(time.Minute), minuteLimit, quota.SharedRPM > 0)
}

func (s *RateLimitService) PreCheckUsageBatch(ctx context.Context, accounts []*Account, requestedModel string) (map[string]bool, error) {
	result := make(map[string]bool, len(accounts))
	for _, account := range accounts {
		if account == nil {
			continue
		}
		allowed, err := s.PreCheckUsage(ctx, account, requestedModel)
		if err != nil {
			return result, err
		}
		result[account.ID] = allowed
	}
	return result, nil
}
...
func (s *RateLimitService) notifyAccountSchedulingBlockCleared(accountID string) {
	if s == nil || s.runtimeBlocker == nil || accountID == "" {
		return
	}
...
func (s *RateLimitService) ApplyAccountSchedulingThreshold(ctx context.Context, account *Account) bool {
	if s == nil || s.settingService == nil || s.accountRepo == nil || account == nil || account.ID == "" {
		return false
	}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 403:
		logger.LegacyPrintf(
			"service.ratelimit",
			"[HandleUpstreamErrorRaw] account_id=%v platform=%s type=%s status=403 request_id=%s cf_ray=%s upstream_msg=%s raw_body=%s",
			account.ID,
			account.Platform,
			account.Type,
...
// OpenAI-compatible providers use a temporary cooldown; other providers use SetError.
func (s *RateLimitService) handle403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	// Kimi reports transient per-account concurrency pressure as a 403. Keep
	// the normal failover signal, but do not feed it into the escalating 403
	// counter that can permanently mark an account as invalid.
	...
	if account.Platform == PlatformOpenAI || IsMultiProtocolAPIKeyProvider(account.Platform) {
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	...
	if count >= openAI403DisableThreshold {
		msg = fmt.Sprintf("%s | consecutive_403=%v/%v", msg, count, openAI403DisableThreshold)
		s.handleAuthError(ctx, account, msg)
		return true
	}

	until := time.Now().Add(time.Duration(openAI403CooldownMinutesDefault) * time.Minute)
	reason := fmt.Sprintf("OpenAI 403 temporary cooldown (%v/%v): %s", count, openAI403DisableThreshold, msg)
...
func persistOpenAI429PlanType(ctx context.Context, repo AccountRepository, account *Account, body []byte) {
	...
	if _, err := repo.BulkUpdate(ctx, []string{account.ID}, AccountBulkUpdate{
...
func (s *RateLimitService) ClearRateLimit(ctx context.Context, accountID string) error {
...
func (s *RateLimitService) ResetOpenAI403Counter(ctx context.Context, accountID string) {
	if s == nil || s.openAI403CounterCache == nil || accountID == "" {
		return
	}
...
func (s *RateLimitService) RecoverAccountState(ctx context.Context, accountID string, options AccountRecoveryOptions) (*SuccessfulTestRecoveryResult, error) {
...
func (s *RateLimitService) RecoverAccountAfterSuccessfulTest(ctx context.Context, accountID string) (*SuccessfulTestRecoveryResult, error) {
	return s.RecoverAccountState(ctx, accountID, AccountRecoveryOptions{})
}

func (s *RateLimitService) ClearTempUnschedulable(ctx context.Context, accountID string) error {
	if err := s.accountRepo.ClearTempUnschedulable(ctx, accountID); err != nil {
		return err
	}
	if s.tempUnschedCache != nil {
...
func (s *RateLimitService) GetTempUnschedStatus(ctx context.Context, accountID string) (*TempUnschedState, error) {
	now := time.Now().Unix()
	if s.tempUnschedCache != nil {
...
func (s *RateLimitService) HandleOpenAIImageCapabilityLoss(ctx context.Context, account *Account, statusCode int, responseBody []byte) bool {
	...
	if s.settingService != nil {
		settings, err := s.settingService.GetOpenAIImagesOAuthUnavailableCooldownSettings(ctx)
		if err != nil {
			slog.Warn("openai_image_capability_loss_cooldown_setting_failed", "account_id", account.ID, "error", err)
		} else {
			cooldownMinutes = settings.CooldownMinutes
		}
	}
	resetAt := time.Now().Add(time.Duration(cooldownMinutes) * time.Minute)
...
func (s *RateLimitService) HandleUpstreamModelNotFound(ctx context.Context, account *Account, requestedModel string, statusCode int, responseBody []byte) bool {
	...
	modelKey := modelRateLimitKeyForUpstreamModelNotFound(account, requestedModel)
	if modelKey == "" {
		return false
	}
...
func modelRateLimitKeyForUpstreamModelNotFound(account *Account, requestedModel string) string {
