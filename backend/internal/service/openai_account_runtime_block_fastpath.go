...
// classifyOpenAIOAuth429 distinguishes an exhausted OAuth quota window from a
// transient 429. Explicit 5h/7d usage headers take precedence; older response
// formats with only a reset timestamp retain the quota-reset classification.
...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	// Only gateway-built image requests can prove that tools and tool_choice
	// matched. Client passthrough requests must not be able to cool an account.
...
// ShouldRetryOpenAIOAuth429 allows transient OAuth 429s to retry the same
// account briefly while quota exhaustion is parked until its reset timestamp.
...
func (s *OpenAIGatewayService) openAIAccountRuntimeBlockLock(accountID string) *sync.Mutex {
...
func (s *OpenAIGatewayService) ClearAccountSchedulingBlock(accountID string) {
	if s == nil || accountID == "" {
		return
	}
...
func (s *OpenAIGatewayService) clearOpenAIAccountModelTransientState(accountID string, model string) {
	state := s.getOpenAIAccountModelTransientState()
	if state == nil {
		return
	}
...
func (s *OpenAIGatewayService) clearOpenAIAccountRuntimeBlockIfUnchanged(accountID string, snapshot openAIAccountRuntimeBlockSnapshot) {
	if s == nil || accountID == "" || !snapshot.blocked {
		return
	}
	mu := s.openAIAccountRuntimeBlockLock(accountID)
	mu.Lock()
