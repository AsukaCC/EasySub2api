...
func (s *PaymentService) checkCancelRateLimit(ctx context.Context, userID string, cfg *PaymentConfig) error {
	if !cfg.CancelRateLimitEnabled || cfg.CancelRateLimitMax <= 0 {
		return nil
	}
	windowStart := cancelRateLimitWindowStart(cfg)
	operator := fmt.Sprintf("user:%v", userID)
...
func (s *PaymentService) CancelOrder(ctx context.Context, orderID, userID string) (string, error) {
	...
	return s.cancelCore(ctx, o, OrderStatusCancelled, fmt.Sprintf("user:%v", userID), "user cancelled order")
}

func (s *PaymentService) AdminCancelOrder(ctx context.Context, orderID string) (string, error) {
	o, err := s.entClient.PaymentOrder.Get(ctx, orderID)
	if err != nil {
		return "", infraerrors.NotFound("NOT_FOUND", "order not found")
	}
...
func (s *PaymentService) VerifyOrderByOutTradeNo(ctx context.Context, outTradeNo string, userID string) (*dbent.PaymentOrder, error) {
...
func (s *PaymentService) getOrderProvider(ctx context.Context, o *dbent.PaymentOrder) (payment.Provider, error) {
	...
	if !paymentOrderAllowsRegistryFallback(o) {
		return nil, fmt.Errorf("order %v provider instance is unresolved", o.ID)
	}
	providerKey := paymentOrderFallbackProviderKey(s.registry, o)
	if providerKey == "" {
		return nil, fmt.Errorf("order %v provider fallback key is missing", o.ID)
	}
	if !s.webhookRegistryFallbackAllowed(ctx, providerKey) {
		return nil, fmt.Errorf("order %v provider fallback is ambiguous for %s", o.ID, providerKey)
	}
