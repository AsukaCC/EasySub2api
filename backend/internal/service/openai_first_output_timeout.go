...
func (s *openAIFirstOutputStage) prepareWrite(incoming int) error {
	...
	if int64(incoming) > s.limit-s.size {
		return fmt.Errorf("%w: buffered=%v incoming=%v limit=%v", errOpenAIFirstOutputStageLimit, s.size, incoming, s.limit)
	}
...
func (s *OpenAIGatewayService) newOpenAIFirstOutputTimeoutError(
	...
	logger.LegacyPrintf(
		...
		"OpenAI first output timeout: account=%v model=%s effort=%s phase=%s elapsed=%s limit=%s",
	...
	appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
		...
		Detail: fmt.Sprintf("phase=%s elapsed_ms=%v timeout_ms=%v", phase, elapsed.Milliseconds(), timeout.Milliseconds()),
	})
