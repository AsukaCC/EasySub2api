...
func resolveGeminiThinkingVariantForLevel(account *Account, requestedModel, preferred string) (string, bool) {
	...
	if len(mapping) == 0 && account.Platform == PlatformAntigravity {
	...
	//   2. 映射目标是自己（原样透传）：仅当这条来自
	//      credentials.model_mapping 时才尊重。无显式映射账号使用的默认目录里
	//      裸名自映射不算用户意图，正是需要改写到思考变体的场景。
	if mapped, matched := resolveRequestedModelInMapping(mapping, model); matched {
		if strings.TrimSpace(mapped) != model || accountExplicitModelMappingHasKey(account, model) {
			return "", false
		}
	}
