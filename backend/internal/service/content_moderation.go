...
type ContentModerationRepository interface {
	...
	CountFlaggedByUserSince(ctx context.Context, userID string, since time.Time, excludeCyberPolicy bool) (int, error)
	...
	UpdateLogEmailSent(ctx context.Context, id string, sent bool) error
}
...
type ContentModerationService struct {
	secrets                  contentModerationSecrets
...
func NewContentModerationService(
	...
	authCacheInvalidator APIKeyAuthCacheInvalidator,
	emailService *EmailService,
	options ...contentModerationSecrets,
...
func (s *ContentModerationService) UpdateConfig(ctx context.Context, input UpdateContentModerationConfigInput) (*ContentModerationConfigView, error) {
	...
	if input.Engine != nil {
		if !validModerationEngine(*input.Engine) {
			return nil, infraerrors.BadRequest("INVALID_CONTENT_MODERATION_ENGINE", "invalid content moderation engine")
		}
	...
	cfg.normalize()
	stored, err := s.encryptTypeSafeConfig(cfg)
	if err != nil {
		return nil, err
	}
...
func (s *ContentModerationService) UnbanUser(ctx context.Context, userID string) (*ContentModerationUnbanUserResult, error) {
...
func (s *ContentModerationService) loadConfig(ctx context.Context) (*ContentModerationConfig, error) {
	...
	return s.parseStoredModerationConfig(raw)
}
...
func (s *ContentModerationService) refreshRuntimeSnapshot(ctx context.Context) (*contentModerationRuntimeSnapshot, error) {
	...
	cfg, err := s.parseStoredModerationConfig(rawConfig)
	if err != nil {
		return nil, err
	}
	cfg = cfg.effectiveEngine(cfg.Engine)
...
func (s *ContentModerationService) validateConfig(ctx context.Context, cfg *ContentModerationConfig) error {
	...
	if cfg.ProxyID != nil && s.proxyRepo != nil {
		if _, err := s.proxyRepo.GetByID(ctx, *cfg.ProxyID); err != nil {
			return infraerrors.BadRequest("INVALID_CONTENT_MODERATION_PROXY", fmt.Sprintf("代理服务器不存在: %v", *cfg.ProxyID))
		}
	}
	...
	if !cfg.AllGroups && len(cfg.GroupIDs) > 0 && s.groupRepo != nil {
		for _, groupID := range cfg.GroupIDs {
			if _, err := s.groupRepo.GetByIDLite(ctx, groupID); err != nil {
				return infraerrors.BadRequest("INVALID_CONTENT_MODERATION_GROUP", fmt.Sprintf("审计分组不存在: %v", groupID))
			}
		}
	}
	return nil
...
func (s *ContentModerationService) callModerationOnceWithInput(ctx context.Context, cfg *ContentModerationConfig, apiKey string, input any, httpStatus *int) (*moderationAPIResult, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, fmt.Errorf("moderation api status %v: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
...
func (s *ContentModerationService) resolveModerationProxyURL(ctx context.Context, proxyID string) (string, error) {
	now := time.Now()
	...
	if err != nil {
		return "", fmt.Errorf("resolve moderation proxy %v: %w", proxyID, err)
	}
	...
	if prev == nil || prev.proxyID != proxyID || prev.url != proxyURL {
		...
		slog.Info("content_moderation.proxy_enabled",
			"proxy_id", proxyID,
			"proxy_name", px.Name,
			"proxy_addr", fmt.Sprintf("%s://%s:%v", px.Protocol, px.Host, px.Port))
	}
...
func (s *ContentModerationService) buildLog(input ContentModerationCheckInput, cfg *ContentModerationConfig, action string, flagged bool, highestCategory string, highestScore float64, scores map[string]float64, text string, latency *int, queueDelay *int, errText string) *ContentModerationLog {
	...
	return &ContentModerationLog{
		...
		GroupID:           cloneStringPtr(input.GroupID),
...
func (s *ContentModerationService) applyFlaggedAccountSideEffects(ctx context.Context, cfg *ContentModerationConfig, log *ContentModerationLog) bool {
	if s == nil || cfg == nil || log == nil || !log.Flagged || log.UserID == nil || *log.UserID == "" {
		return false
	}
	count := 1
...
func contentModerationEmailUserID(log *ContentModerationLog) string {
...
func contentModerationEmailVariables(log *ContentModerationLog, cfg *ContentModerationConfig) map[string]string {
	...
	if log != nil {
		...
		variables["violation_count"] = fmt.Sprintf("%v", log.ViolationCount)
	}
	if cfg != nil {
		variables["ban_threshold"] = fmt.Sprintf("%v", cfg.BanThreshold)
	}
	return variables
}
...
func cloneContentModerationConfig(cfg *ContentModerationConfig) *ContentModerationConfig {
	...
	clone.ProxyID = cloneStringPtr(cfg.ProxyID)
	clone.APIKeys = append([]string(nil), cfg.APIKeys...)
	clone.GroupIDs = append([]string(nil), cfg.GroupIDs...)
...
func (cfg *ContentModerationConfig) normalize() {
	...
	if cfg.ProxyID != nil && *cfg.ProxyID == "" {
		cfg.ProxyID = nil
	}
	if cfg.TimeoutMS <= 0 {
...
func (cfg *ContentModerationConfig) includesGroup(groupID *string) bool {
	if cfg.AllGroups {
		return true
	}
	if groupID == nil {
...
func contentModerationLogGroupID(groupID *string) string {
	if groupID == nil {
		return ""
	}
	return *groupID
...
func (s *ContentModerationService) configView(cfg *ContentModerationConfig) *ContentModerationConfigView {
	...
	return &ContentModerationConfigView{
		...
		BaseURL:                        cfg.BaseURL,
		Model:                          cfg.Model,
		ProxyID:                        cloneStringPtr(cfg.ProxyID),
		...
		GroupIDs:                       append([]string(nil), cfg.GroupIDs...),
...
func buildModerationTestInput(prompt string, images []string) (any, int, error) {
	...
	for _, image := range images {
		...
		if len(normalizedImages) >= maxContentModerationTestImages {
			return nil, 0, infraerrors.BadRequest("TOO_MANY_MODERATION_TEST_IMAGES", fmt.Sprintf("最多上传 %v 张测试图片", maxContentModerationTestImages))
		}
...
func normalizeInt64IDs(ids []string) []string {
	if len(ids) == 0 {
		return []string{}
	}
...
func (s *ContentModerationService) RecordCyberPolicyEvent(ctx context.Context, in CyberPolicyRecordInput) {
	...
	if in.UpstreamInTok > 0 || in.UpstreamOutTok > 0 {
		errBody = fmt.Sprintf("%s\nupstream_usage=in:%v,out:%v", errBody, in.UpstreamInTok, in.UpstreamOutTok)
	}
	log := &ContentModerationLog{
		...
		GroupID:         cloneStringPtr(in.GroupID),
