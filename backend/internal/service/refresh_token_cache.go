...
type RefreshTokenCache interface {
	...
	AddToUserTokenSet(ctx context.Context, userID string, tokenHash string, ttl time.Duration) error
	...
	// 用于批量删除用户Token
	GetUserTokenHashes(ctx context.Context, userID string) ([]string, error)
