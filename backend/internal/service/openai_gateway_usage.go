...
func (s *OpenAIGatewayService) RecordCyberPolicyUsageLog(ctx context.Context, in CyberPolicyUsageInput) {
	...
	result := &OpenAIForwardResult{
		...
		Model:                    in.Model,
		RequestedReasoningEffort: in.RequestedReasoningEffort,
...
// groupBillsOpenAIFastAtStandard changes only the customer-facing charge for
// a priority request. Upstream/account accounting continues to use the actual
// priority tier cost and service tier recorded on the usage row.
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// Free Fast affects only the amount charged to the user. Recalculate the
	// customer charge at Standard while preserving the priority TotalCost,
	// account-statistics cost, and service tier used by upstream/profit controls.
	var dynamicRateStandardCost *float64
	if groupBillsOpenAIFastAtStandard(apiKey, billingAccount, serviceTier) {
		...
		if cost != nil && standardCost != nil {
			...
			dynamicRateStandardCost = &standardBase
		}
	}
	...
	usageLog := &UsageLog{
		...
		NativeCompactionV2:       input.NativeCompactionV2 || result.NativeCompactionV2,
	}
	...
	// 设置渠道信息
	usageLog.ChannelID = optionalTrimmedStringPtr(input.ChannelID)
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		...
		logger.LegacyPrintf("service.openai_gateway", "[SIMPLE MODE] Usage recorded (not billed): user=%v, tokens=%v", usageLog.UserID, usageLog.TotalTokens())
	...
	billingErr := func() error {
		_, err := applyUsageBilling(ctx, requestID, usageLog, &postUsageBillingParams{
			...
			RatePlan:                ratePlan,
			DynamicRateStandardCost: dynamicRateStandardCost,
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if result != nil && result.WebSearchCalls > 0 {
		...
		// 使用统一的分组倍率 × 用户专属分组倍率快照，与计费链路保持一致。
...
func (s *OpenAIGatewayService) apiKeyWithFreshGroupMediaPricing(ctx context.Context, apiKey *APIKey) *APIKey {
	if apiKey == nil || apiKey.GroupID == nil || *apiKey.GroupID == "" {
		return apiKey
	}
...
// 无法在此自检影子,故守卫前置到各调用点。
func (s *OpenAIGatewayService) updateCodexUsageSnapshot(ctx context.Context, accountID string, snapshot *OpenAICodexUsageSnapshot) {
	if snapshot == nil {
		return
	}
...
func (s *OpenAIGatewayService) UpdateCodexUsageSnapshotFromHeaders(ctx context.Context, accountID string, headers http.Header) {
