...
func (s *AuthService) SendEmailIdentityBindCode(ctx context.Context, userID string, email string, locale ...string) error {
	if s == nil {
...
// ensureEmailIdentityAvailableForUser performs a fast exact and alias lookup
// before sending a code or replacing the primary email. The current user's own
// inbox identity remains allowed so an alias variant can be used during rebinding.
...
// emailIdentityAliasGuardRepository is the optional transaction capability
// implemented by the production repository. Keeping it separate from
// UserRepository preserves compatibility with narrow test and plugin stubs.
type emailIdentityAliasGuardRepository interface {
	UpdateEmailWithAliasGuard(ctx context.Context, userID string, email string, passwordHash string) error
}
...
func (s *AuthService) updateBoundEmailIdentityWithClient(
	...
	if client == nil || currentUser == nil || currentUser.ID == "" {
		return ErrServiceUnavailable
	}
...
func (s *AuthService) revokeEmailIdentitySessions(ctx context.Context, userID string) {
	if err := s.RevokeAllUserSessions(ctx, userID); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to revoke refresh sessions after email identity bind for user %v: %v", userID, err)
	}
}
