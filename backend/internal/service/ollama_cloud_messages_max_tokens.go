...
// normalizeOllamaCloudBaseURLForMatch 与真实出站组装一样先 TrimSpace + TrimRight "/"
// （urlvalidator 与 nativeAnthropicTargetURL 同款归一化），保证 clamp 判定与出站 URL
// 同源：带尾部斜杠的 https://ollama.com/ 同样命中。
func normalizeOllamaCloudBaseURLForMatch(baseURL string) string {
...
// GetAnthropicProtocolBaseURL，不用 GetOpenAIBaseURL——那是 CC/Responses 地址），
// 先做尾斜杠归一化，仅当其为 ollama.com 且 body 中映射后出站 model 为 DeepSeek 系时
// 压到 cap，否则原样返回。
func clampOllamaCloudAnthropicMessagesMaxTokens(account *Account, baseURL string, body []byte) []byte {
	if account == nil || len(body) == 0 {
		return body
	}
	if !isOllamaCloudBaseURL(normalizeOllamaCloudBaseURLForMatch(baseURL)) {
		return body
	}
