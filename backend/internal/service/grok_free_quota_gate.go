...
func filterGrokFreeQuotaAccountsCore(
	...
	missingIDs := make([]string, 0, len(accounts))
	seenMissing := make(map[string]struct{})
	for i := range accounts {
		account := &accounts[i]
		if !isExplicitGrokFreeOAuthAccount(account) || account.ID == "" {
			continue
		}
...
func scheduleGrokFreeQuotaStatsRefresh(
	...
	toFetch := make([]string, 0, len(accountIDs))
...
func queryGrokFreeQuotaWindowStats(ctx context.Context, usageLogRepo UsageLogRepository, accountIDs []string, start time.Time) (map[string]*usagestats.AccountStats, error) {
	if usageLogRepo == nil {
		return nil, nil
	}
	...
	statsByID := make(map[string]*usagestats.AccountStats, len(accountIDs))
