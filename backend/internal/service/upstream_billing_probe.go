...
func (s *SettingService) SetUpstreamBillingProbeSettings(ctx context.Context, settings *UpstreamBillingProbeSettings) error {
	...
	if settings.IntervalMinutes < upstreamBillingProbeMinIntervalMinutes || settings.IntervalMinutes > upstreamBillingProbeMaxIntervalMinutes {
		return infraerrors.BadRequest(
			...
			fmt.Sprintf("interval_minutes must be between %v and %v", upstreamBillingProbeMinIntervalMinutes, upstreamBillingProbeMaxIntervalMinutes),
		)
	}
	normalizeUpstreamBillingProbeSettings(settings)
...
// UpstreamBillingProbeService discovers a remote EasySub2api billing snapshot.
	...
	for i := range due {
		...
		group.Go(func() error {
			if _, probeErr := s.probeScheduledAccount(ctx, accountID, settings.IntervalMinutes); probeErr != nil {
				logger.LegacyPrintf("service.upstream_billing_probe", "probe_due_failed: account_id=%v err=%v", accountID, probeErr)
			}
			return nil
		})
	}
...
func (s *UpstreamBillingProbeService) ProbeAccount(ctx context.Context, accountID string) (*UpstreamBillingProbeSnapshot, error) {
	if s == nil || s.accountRepo == nil {
...
func (s *UpstreamBillingProbeService) probeAccount(ctx context.Context, accountID string, intervalMinutes int) (*UpstreamBillingProbeSnapshot, error) {
	return s.probeAccountWithMode(ctx, accountID, intervalMinutes, false)
}

func (s *UpstreamBillingProbeService) probeScheduledAccount(ctx context.Context, accountID string, intervalMinutes int) (*UpstreamBillingProbeSnapshot, error) {
	return s.probeAccountWithMode(ctx, accountID, intervalMinutes, true)
}

func (s *UpstreamBillingProbeService) probeAccountWithMode(ctx context.Context, accountID string, intervalMinutes int, requireEnabled bool) (*UpstreamBillingProbeSnapshot, error) {
	key := accountID
...
func (s *UpstreamBillingProbeService) ProbeAccounts(ctx context.Context, accountIDs []string) []UpstreamBillingProbeResult {
...
func upstreamBillingProbeLeaderLockKeyAt(now time.Time) string {
	return fmt.Sprintf("%s:%v", upstreamBillingProbeLeaderLockKey, now.Unix()/int64(upstreamBillingProbeCycleInterval/time.Second))
}
...
func (s *UpstreamBillingProbeService) SetAccountEnabled(ctx context.Context, accountID string, enabled bool) error {
	if s == nil || s.accountRepo == nil {
		return ErrUpstreamBillingProbeUnavailable
	}
...
func (s *UpstreamBillingProbeService) probeLoadedAccount(ctx context.Context, account *Account, intervalMinutes int) (*UpstreamBillingProbeSnapshot, error) {
	...
	for index, billingPath := range []string{"/v1/easysub2api/billing", "/v1/sub2api/billing"} {
		probeURL := buildOpenAIEndpointURL(normalizedBaseURL, billingPath)
		req, requestErr := http.NewRequestWithContext(probeCtx, http.MethodGet, probeURL, bytes.NewReader(nil))
		if requestErr != nil {
		...
		if index == 0 && (resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusMethodNotAllowed) {
			_ = resp.Body.Close()
			resp = nil
			continue
		}
...
func parseUpstreamBillingProbeResponse(body []byte) (map[string]any, error) {
	...
	if (response.Object != "easysub2api.key_billing" && response.Object != "sub2api.key_billing") || response.SchemaVersion != 1 || response.BillingScope != "token" {
...
// present at all; AccountTypeUpstream is deliberately left out of the current
// supported set.
