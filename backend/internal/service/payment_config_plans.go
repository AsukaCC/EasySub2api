...
func validatePlanRequired(name string, groupID string, price float64, validityDays int, validityUnit string, originalPrice *float64) error {
...
func (s *PaymentConfigService) GetGroupInfoMap(ctx context.Context, plans []*dbent.SubscriptionPlan) map[string]PlanGroupInfo {
	...
	m := make(map[string]PlanGroupInfo, len(groups))
	for _, g := range groups {
		m[g.ID] = PlanGroupInfo{
			Platform:           g.Platform,
			Name:               g.Name,
...
func (s *PaymentConfigService) UpdatePlan(ctx context.Context, id string, req UpdatePlanRequest) (*dbent.SubscriptionPlan, error) {
...
func (s *PaymentConfigService) DeletePlan(ctx context.Context, id string) error {
	...
	if count > 0 {
		return infraerrors.Conflict("PENDING_ORDERS",
			fmt.Sprintf("this plan has %v in-progress orders and cannot be deleted — wait for orders to complete first", count))
	}
	return s.entClient.SubscriptionPlan.DeleteOneID(id).Exec(ctx)
}

// GetPlan returns a subscription plan by ID.
func (s *PaymentConfigService) GetPlan(ctx context.Context, id string) (*dbent.SubscriptionPlan, error) {
