...
type ChannelRepository interface {
	...
	ExistsByNameExcluding(ctx context.Context, name string, excludeID string) (bool, error)

	// 分组关联
	GetGroupIDs(ctx context.Context, channelID string) ([]string, error)
	SetGroupIDs(ctx context.Context, channelID string, groupIDs []string) error
	GetChannelIDByGroupID(ctx context.Context, groupID string) (string, error)
	GetGroupsInOtherChannels(ctx context.Context, channelID string, groupIDs []string) ([]string, error)

	// 分组平台查询
	GetGroupPlatforms(ctx context.Context, groupIDs []string) (map[string]string, error)

	// 模型定价
	ListModelPricing(ctx context.Context, channelID string) ([]ChannelModelPricing, error)
	...
	ReplaceModelPricing(ctx context.Context, channelID string, pricingList []ChannelModelPricing) error
}
...
type channelCache struct {
	...
	channelByGroupID        map[string]*Channel                                 // groupID → 渠道
	groupPlatform           map[string]string                                   // groupID → platform
...
type ChannelMappingResult struct {
	...
	ChannelID          string // 渠道 ID（空字符串 = 无渠道关联）
	Mapped             bool   // 是否发生了映射
...
func newEmptyChannelCache() *channelCache {
	return &channelCache{
		...
		channelByGroupID:        make(map[string]*Channel),
		groupPlatform:           make(map[string]string),
...
func expandPricingToCache(cache *channelCache, ch *Channel, gid string, platform string) {
...
// 各平台的模型映射严格独立。
func expandMappingToCache(cache *channelCache, ch *Channel, gid string, platform string) {
...
func (s *ChannelService) fetchChannelData(ctx context.Context) ([]Channel, map[string]string, error) {
	...
	groupPlatforms := make(map[string]string)
...
func populateChannelCache(channels []Channel, groupPlatforms map[string]string) *channelCache {
	cache := newEmptyChannelCache()
	cache.groupPlatform = groupPlatforms
	cache.byID = make(map[string]*Channel, len(channels))
...
func (c *channelCache) matchWildcard(groupID string, platform, modelLower string) *ChannelModelPricing {
...
func (c *channelCache) matchWildcardMapping(groupID string, platform, modelLower string) string {
	gpKey := channelGroupPlatformKey{groupID: groupID, platform: platform}
...
func lookupPricingAcrossPlatforms(cache *channelCache, groupID string, groupPlatform, modelLower string) *ChannelModelPricing {
...
func lookupMappingAcrossPlatforms(cache *channelCache, groupID string, groupPlatform, modelLower string) string {
	for _, p := range matchingPlatforms(groupPlatform) {
		key := channelModelKey{groupID: groupID, platform: p, model: modelLower}
...
func (s *ChannelService) GetChannelForGroup(ctx context.Context, groupID string) (*Channel, error) {
...
func (s *ChannelService) GetGroupPlatform(ctx context.Context, groupID string) string {
	cache, err := s.loadCache(ctx)
	if err != nil {
		return ""
	}
...
func (s *ChannelService) lookupGroupChannel(ctx context.Context, groupID string) (*channelLookup, error) {
	cache, err := s.loadCache(ctx)
	if err != nil {
		return nil, err
	}
...
// 各平台严格独立，只在本平台内查找定价。
func (s *ChannelService) GetChannelModelPricing(ctx context.Context, groupID string, model string) *ChannelModelPricing {
...
func (s *ChannelService) ResolveChannelMapping(ctx context.Context, groupID string, model string) ChannelMappingResult {
	lk, err := s.lookupGroupChannel(ctx, groupID)
	if err != nil {
...
func (s *ChannelService) IsModelRestricted(ctx context.Context, groupID string, model string) bool {
	lk, err := s.lookupGroupChannel(ctx, groupID)
	if err != nil {
...
func (s *ChannelService) ResolveChannelMappingAndRestrict(ctx context.Context, groupID *string, model string) (ChannelMappingResult, bool) {
	if groupID == nil {
...
func resolveMapping(lk *channelLookup, groupID string, model string) ChannelMappingResult {
...
// 只在本平台的定价列表中查找。
func checkRestricted(lk *channelLookup, groupID string, model string) bool {
	...
	for i := range rules {
		for _, pricing := range rules[i].Pricing {
			if pricing.TimePricing != nil && len(pricing.TimePricing.Periods) > 0 {
				return fmt.Errorf("account stats pricing rule #%v: %w", i+1,
		...
		if err := validatePricingEntries(rules[i].Pricing); err != nil {
			return fmt.Errorf("account stats pricing rule #%v: %w", i+1, err)
		}
	}
	return nil
}
	...
	for _, iv := range p.Intervals {
		if iv.InputPrice == nil && iv.OutputPrice == nil &&
			...
			return infraerrors.BadRequest(
				...
				fmt.Sprintf("interval [%v, %s] has no price fields set for model %v",
...
func (s *ChannelService) Create(ctx context.Context, input *CreateChannelInput) (*Channel, error) {
	...
	if err := s.checkGroupConflicts(ctx, "", input.GroupIDs); err != nil {
		return nil, err
	}
...
func (s *ChannelService) GetByID(ctx context.Context, id string) (*Channel, error) {
...
// Update 更新渠道
func (s *ChannelService) Update(ctx context.Context, id string, input *UpdateChannelInput) (*Channel, error) {
...
func (s *ChannelService) checkGroupConflicts(ctx context.Context, channelID string, groupIDs []string) error {
	if len(groupIDs) == 0 {
		return nil
	}
...
func (s *ChannelService) getOldGroupIDs(ctx context.Context, channelID string) []string {
...
func (s *ChannelService) invalidateAuthCacheForGroups(ctx context.Context, groupIDSets ...[]string) {
	if s.authCacheInvalidator == nil {
		return
	}
