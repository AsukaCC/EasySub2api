...
func WithForwardGeminiSession(groupID string, sessionHash string) ForwardGeminiOption {
...
func (s *AntigravityGatewayService) ForwardGemini(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte, isStickySession bool, options ...ForwardGeminiOption) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest &&
			...
			logger.LegacyPrintf("service.antigravity_gateway", "Antigravity Gemini account %s: detected signature-related 400, retrying with cleaned thought signatures", account.ID)
			...
			if wrapErr == nil {
				...
				if retryErr == nil {
					...
					logger.LegacyPrintf("service.antigravity_gateway", "Antigravity Gemini account %s: signature retry request failed: %v", account.ID, retryErr)
				}
			} else {
				logger.LegacyPrintf("service.antigravity_gateway", "Antigravity Gemini account %s: signature retry wrap failed: %v", account.ID, wrapErr)
			}
		}
