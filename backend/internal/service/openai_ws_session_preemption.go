...
type OpenAIWSSessionPreemptionCache interface {
	ClaimOpenAIResponsesSessionWindow(ctx context.Context, groupID string, sessionHash string, owner []byte, ttl time.Duration) ([]byte, error)
	CompareAndRefreshOpenAIResponsesSessionWindow(ctx context.Context, groupID string, sessionHash string, expected []byte, ttl time.Duration) (bool, error)
	CompareAndDeleteOpenAIResponsesSessionWindow(ctx context.Context, groupID string, sessionHash string, expected []byte) (bool, error)
}
...
func (s *OpenAIGatewayService) BeginOpenAIWSIngressSessionPreemptionWithClient(
	...
	if preemptedPrevious {
		...
		logOpenAIWSModeInfo(
			"ingress_ws_session_preempted account_id=%s group_id=%s api_key_id=%s scope=%s thread_id=%s lane=%s",
			account.ID,
			preemptGroupID,
			preemptAPIKeyID,
...
func newOpenAIWSSessionPreemptKey(groupID, apiKeyID string, sessionHash string) (openAIWSSessionPreemptKey, bool) {
	sessionHash = strings.TrimSpace(sessionHash)
	if groupID == "" || apiKeyID == "" || sessionHash == "" {
...
func openAIWSSessionPreemptCacheHash(apiKeyID string, sessionHash string) string {
	return fmt.Sprintf("%s%s:%s", openAIWSSessionPreemptCachePrefix, apiKeyID, strings.TrimSpace(sessionHash))
}
