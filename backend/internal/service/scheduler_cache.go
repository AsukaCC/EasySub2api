...
func (l SchedulerGroupLifecycleLease) ValidFor(groupID string) bool {
	return groupID != "" && l.GroupID == groupID && l.OwnerToken != ""
}
...
func (b SchedulerBucket) String() string {
	return fmt.Sprintf("%v:%s:%s", b.GroupID, b.Platform, b.Mode)
}
...
type SchedulerCache interface {
	...
	TryAcquireGroupLifecycleLease(ctx context.Context, groupID string, ttl time.Duration) (SchedulerGroupLifecycleLease, bool, error)
	...
	UpdateLastUsed(ctx context.Context, updates map[string]time.Time) error
	...
	GetOutboxWatermark(ctx context.Context) (string, error)
	// SetOutboxWatermark 保存 outbox 水位。
	SetOutboxWatermark(ctx context.Context, id string) error
}
