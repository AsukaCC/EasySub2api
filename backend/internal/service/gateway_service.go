...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	debugGatewayBodyEnv                    = "EASYSUB2API_DEBUG_GATEWAY_BODY"
	legacyDebugGatewayBodyEnv              = "SUB2API_DEBUG_GATEWAY_BODY"
...
func buildClaudeMimicDebugLine(req *http.Request, body []byte, account *Account, tokenType string, mimicClaudeCode bool) string {
	...
	return fmt.Sprintf(
		"url=%s account=%v(%s) tokenType=%s mimic=%t meta.user_id=%q system.preview=%q headers={%s}",
		req.URL.String(),
		aid,
		aname,
		tokenType,
...
type GatewayCache interface {
	...
	// cache driver.
	GetSessionAccountID(ctx context.Context, groupID string, sessionHash string) (string, error)
	// SetSessionAccountID 设置粘性会话与账号的绑定关系
	// Set the binding between sticky session and account
	SetSessionAccountID(ctx context.Context, groupID string, sessionHash string, accountID string, ttl time.Duration) error
	// RefreshSessionTTL 刷新粘性会话的过期时间
	// Refresh the expiration time of a sticky session
	RefreshSessionTTL(ctx context.Context, groupID string, sessionHash string, ttl time.Duration) error
	// DeleteSessionAccountID 删除粘性会话绑定，用于账号不可用时主动清理
	// Delete sticky session binding, used to proactively clean up when account becomes unavailable
	DeleteSessionAccountID(ctx context.Context, groupID string, sessionHash string) error
...
// derefGroupID safely dereferences a UUID string, returning empty for nil.
func derefGroupID(groupID *string) string {
	if groupID == nil {
		return ""
	}
	return *groupID
...
func modelsListCacheKey(groupID *string, platform string) string {
...
func prefetchedStickyAccountIDFromContext(ctx context.Context, groupID *string) string {
	prefetchedGroupID, ok := prefetchedStickyGroupIDFromContext(ctx)
	if !ok || prefetchedGroupID != derefGroupID(groupID) {
		return ""
	}
	if accountID, ok := PrefetchedStickyAccountIDFromContext(ctx); ok && accountID != "" {
		return accountID
	}
	return ""
}
...
func (e *UpstreamFailoverError) Error() string {
	...
	return fmt.Sprintf("upstream error: %v (failover)", e.StatusCode)
}
...
func (s *GatewayService) TempUnscheduleRetryableError(ctx context.Context, accountID string, failoverErr *UpstreamFailoverError) {
	...
	case http.StatusBadRequest:
		tempUnscheduleRetryableBadRequest(ctx, s.accountRepo, accountID, "[handler]")
...
type GatewayService struct {
	...
	debugGatewayBodyFile  atomic.Pointer[os.File] // non-nil when EASYSUB2API_DEBUG_GATEWAY_BODY is set
...
// pricing. It preserves the constructor used by focused unit and integration
// helpers; production wiring uses NewGatewayServiceWithUserLevel.
func NewGatewayService(
	...
	return NewGatewayServiceWithUserLevel(
		...
		rpmCache, digestStore, settingService, tlsFPProfileService, channelService,
		resolver, compositeResolver, balanceNotifyService, userPlatformQuotaRepo, nil,
	)
}

// NewGatewayServiceWithUserLevel creates a GatewayService with rolling user
// level and dynamic-rate selection enabled.
func NewGatewayServiceWithUserLevel(
	accountRepo AccountRepository,
	groupRepo GroupRepository,
	usageLogRepo UsageLogRepository,
	usageBillingRepo UsageBillingRepository,
	...
	svc.debugModelRouting.Store(parseDebugEnvBool(compatibilityEnv("EASYSUB2API_DEBUG_MODEL_ROUTING", "SUB2API_DEBUG_MODEL_ROUTING")))
	svc.debugClaudeMimic.Store(parseDebugEnvBool(compatibilityEnv("EASYSUB2API_DEBUG_CLAUDE_MIMIC", "SUB2API_DEBUG_CLAUDE_MIMIC")))
	if path := strings.TrimSpace(compatibilityEnv(debugGatewayBodyEnv, legacyDebugGatewayBodyEnv)); path != "" {
		svc.initDebugGatewayBodyFile(path)
	}
	return svc
}

func compatibilityEnv(canonicalName, legacyName string) string {
	if value, exists := os.LookupEnv(canonicalName); exists {
		if _, legacyExists := os.LookupEnv(legacyName); legacyExists {
			slog.Warn("legacy_environment_variable_ignored", "legacy", legacyName, "replacement", canonicalName)
		}
		return value
	}
	if value, exists := os.LookupEnv(legacyName); exists {
		slog.Warn("legacy_environment_variable_used", "legacy", legacyName, "replacement", canonicalName)
		return value
	}
	return ""
}
...
func (s *GatewayService) GenerateSessionHash(parsed *ParsedRequest) string {
	...
	if parsed.SessionContext != nil {
		...
		_, _ = combined.WriteString(":")
		_, _ = combined.WriteString(parsed.SessionContext.APIKeyID)
		_, _ = combined.WriteString("|")
	}
...
func (s *GatewayService) BindStickySession(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
	if sessionHash == "" || accountID == "" || s.cache == nil {
		return nil
	}
...
func (s *GatewayService) bindGatewayStickySessionDuringSelection(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
...
func (s *GatewayService) BindStickySessionAfterProfitAdmission(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
	if sessionHash == "" || accountID == "" || s.cache == nil {
		return nil
	}
	if !gatewayProfitControlGateActive(ctx) {
...
// Returns an empty string if no binding exists or on error.
func (s *GatewayService) GetCachedSessionAccountID(ctx context.Context, groupID *string, sessionHash string) (string, error) {
...
func (s *GatewayService) FindAnthropicSession(_ context.Context, groupID string, prefixHash, digestChain string) (uuid string, accountID string, matchedChain string, found bool) {
...
func (s *GatewayService) SaveAnthropicSession(_ context.Context, groupID string, prefixHash, digestChain, uuid string, accountID string, oldDigestChain string) error {
	if digestChain == "" || s.digestStore == nil {
		return nil
	}
...
func (s *GatewayService) FindGeminiSession(_ context.Context, groupID string, prefixHash, digestChain string) (uuid string, accountID string, matchedChain string, found bool) {
	if digestChain == "" || s.digestStore == nil {
		return "", "", "", false
	}
	return s.digestStore.Find(groupID, prefixHash, digestChain)
...
func (s *GatewayService) SaveGeminiSession(_ context.Context, groupID string, prefixHash, digestChain, uuid string, accountID string, oldDigestChain string) error {
	if digestChain == "" || s.digestStore == nil {
		return nil
	}
	s.digestStore.Save(groupID, prefixHash, digestChain, uuid, accountID, oldDigestChain)
...
func (s *GatewayService) DoGrokNativeResponsesJSON(ctx context.Context, account *Account, body []byte) ([]byte, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, fmt.Errorf("grok upstream %v: %s", resp.StatusCode, msg)
	}
	return respBytes, nil
}

func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *string, platform string) []string {
...
func (s *GatewayService) InvalidateAvailableModelsCache(groupID *string, platform string) {
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { // #nosec G703 -- path is an operator-controlled debug setting.
		path = filepath.Join(path, debugGatewayBodyDefaultFilename)
	}

	// 确保父目录存在
	if dir := filepath.Dir(path); dir != "." {
		if err := os.MkdirAll(dir, 0755); err != nil { // #nosec G703 -- path is an operator-controlled debug setting.
			slog.Error("failed to create gateway debug log directory", "dir", dir, "error", err)
			return
		}
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644) // #nosec G703 -- path is an operator-controlled debug setting.
	if err != nil {
...
// 启用方式（环境变量）：
//
//	EASYSUB2API_DEBUG_GATEWAY_BODY=1                          # 写入 gateway_debug.log
//	EASYSUB2API_DEBUG_GATEWAY_BODY=/tmp/gateway_debug.log     # 写入指定路径
//
