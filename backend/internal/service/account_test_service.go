...
const (
	...
	defaultGrokTTSTestText       = "Hello from EasySub2api account connectivity test."
...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID string, modelID string, prompt string, mode string, opts ...AccountTestOptions) error {
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	resp, err := s.doAccountTestWithProtection(req, proxyURL, account, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
...
func (s *AccountTestService) testBedrockAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string) error {
	...
	resp, err := s.doAccountTestWithProtection(req, proxyURL, account, nil)
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Request failed: %s", err.Error()))
	}
	defer func() { _ = resp.Body.Close() }()
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID string) error {
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return s.sendErrorAndEnd(c, fmt.Sprintf("Grok Responses API returned %v: %s", resp.StatusCode, string(body)))
	}
...
func (s *AccountTestService) testGrokImageGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt, imageDataURL string) error {
	...
	if hasSourceImage {
		...
		s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("source image ready (%v chars data URL)\n", len(normalized))})
	}
...
func (s *AccountTestService) testGrokVideoGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt string, opts AccountTestOptions) error {
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusAccepted && resp.StatusCode != http.StatusCreated {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Grok videos API returned %v: %s", resp.StatusCode, string(body)))
	}
	...
	for time.Now().Before(deadline) {
		...
		if statusResp.StatusCode != http.StatusOK && statusResp.StatusCode != http.StatusAccepted {
			return s.sendErrorAndEnd(c, fmt.Sprintf("Grok video status returned %v: %s", statusResp.StatusCode, string(statusBody)))
		}
...
func (s *AccountTestService) emitGrokVideoResult(c *gin.Context, ctx context.Context, account *Account, authToken, requestID string, statusBody []byte) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return s.sendErrorAndEnd(c, fmt.Sprintf("Grok video content returned %v: %s", resp.StatusCode, truncateString(string(body), 300)))
	}
	...
	s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("video content downloaded: content-type=%s bytes=%v\n", ct, len(body))})
...
func (s *AccountTestService) testGrokWebSearch(c *gin.Context, ctx context.Context, account *Account, authToken, query string) error {
	...
	const maxResults = 5
	prompt := fmt.Sprintf(
		`Search the web for the user query below. Return ONLY valid JSON with this exact shape: {"results":[{"url":"https://...","title":"page title","snippet":"concise factual summary"}]}. Return at most %v unique results. Every URL must be an actual web_search source. Populate a non-empty title and snippet for every result. Do not wrap the JSON in markdown.
...
%s`, maxResults, query)
	...
	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != http.StatusOK {
		return s.sendErrorAndEnd(c, fmt.Sprintf("standalone web_search probe returned %v: %s", resp.StatusCode, string(body)))
	}
	...
	s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("web_search ok: query=%q tool_calls=%v sources=%v\n", query, searchCount, sourceCount)})
...
func (s *AccountTestService) testGrokTTS(c *gin.Context, ctx context.Context, account *Account, authToken, text string) error {
	...
	for _, payload := range payloads {
		...
		if resp.StatusCode == http.StatusOK {
			...
			s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("tts ok: content-type=%s bytes=%v\n", ct, len(body))})
	...
	return s.sendErrorAndEnd(c, fmt.Sprintf("Grok TTS returned %v: %s", lastCode, lastBody))
}
...
func (s *AccountTestService) testGrokSTT(c *gin.Context, ctx context.Context, account *Account, authToken, audioDataURL string) error {
	...
	if audioDataURL = strings.TrimSpace(audioDataURL); audioDataURL != "" {
		...
		s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("uploaded audio: mime=%s bytes=%v\n", mime, len(audioBytes))})
	} else {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return s.sendErrorAndEnd(c, fmt.Sprintf("Grok STT returned %v: %s", resp.StatusCode, string(respBody)))
	}
...
func (s *AccountTestService) testGrokRealtime(c *gin.Context, ctx context.Context, account *Account, authToken, modelID string) error {
	...
	if dialErr != nil {
		...
		if status > 0 {
			return s.sendErrorAndEnd(c, fmt.Sprintf("Grok Realtime WS handshake failed (HTTP %v): %s", status, detail))
		}
...
func normalizeAccountTestImageDataURL(raw string) (string, error) {
	...
	if len(data) > maxDecodedImage {
		return "", fmt.Errorf(
			"source image is too large (%v bytes decoded). Please use a smaller image (under ~4 MB) for admin edit tests",
			len(data),
		)
	}
	...
	if cfg.Width < 8 || cfg.Height < 8 {
		return "", fmt.Errorf(
			"source image is too small (%vx%v). xAI requires both width and height to be at least 8 pixels",
...
func formatGrokImageTransportError(err error, hasSourceImage bool, payloadBytes int) string {
	...
	return base + fmt.Sprintf(
		" (edit payload ~%v bytes). Tips: use a smaller source image (<4 MB / lower resolution), ensure the account proxy is stable, and retry. xAI /images/edits expects image as {\"url\":\"data:image/...;base64,...\",\"type\":\"image_url\"}.",
		payloadBytes,
	)
}
...
func formatGrokImagesAPIError(status int, body []byte, hasSourceImage bool) string {
	...
	prefix := fmt.Sprintf("Grok images API returned %v: %s", status, msg)
...
func decodeAccountTestDataURL(raw string) (data []byte, mime string, err error) {
	...
	if len(decoded) > maxAccountTestMediaBytes {
		return nil, "", fmt.Errorf("media exceeds %v byte limit", maxAccountTestMediaBytes)
	}
...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	if len(results) == 0 {
		if refusal := extractOpenAIImagesModelRefusal(body); refusal != "" {
			return s.sendErrorAndEnd(c, sanitizeUpstreamErrorMessage(refusal))
		}
		if direct {
...
func (s *AccountTestService) RunTestBackground(ctx context.Context, accountID string, modelID string) (*ScheduledTestResult, error) {
	startedAt := time.Now()
