...
func (e *OpenAIWSClientCloseError) Error() string {
	...
	if e.err == nil {
		return fmt.Sprintf("openai ws client close: %v %s", int(e.statusCode), strings.TrimSpace(e.reason))
	}
	return fmt.Sprintf("openai ws client close: %v %s: %v", int(e.statusCode), strings.TrimSpace(e.reason), e.err)
}
