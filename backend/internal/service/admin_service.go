...
type AdminService interface {
	...
	GetUserIncludeDeleted(ctx context.Context, id string) (*User, error)
	CreateUser(ctx context.Context, input *CreateUserInput) (*User, error)
	UpdateUser(ctx context.Context, id string, input *UpdateUserInput) (*User, error)
	DeleteUser(ctx context.Context, id string) error
	UpdateUserBalance(ctx context.Context, userID string, balance float64, operation string, notes string) (*User, error)
	UpdateUserWalletBalance(ctx context.Context, userID string, balance float64, operation, balanceType string, bonusValidityDays int, notes string) (*User, error)
	BatchUpdateConcurrency(ctx context.Context, userIDs []string, value int, mode string) (int, error)
	BatchUpdateLimits(ctx context.Context, userIDs []string, concurrency, rpmLimit *int) (int, error)
	GetUserAPIKeys(ctx context.Context, userID string, page, pageSize int, sortBy, sortOrder string) ([]APIKey, int64, error)
	GetUserUsageStats(ctx context.Context, userID string, period string) (any, error)
	GetUserRPMStatus(ctx context.Context, userID string) (*UserRPMStatus, error)
	// GetUserBalanceHistory returns paginated balance/concurrency change records for a user.
	// codeType is optional - pass empty string to return all types.
	// Also returns totalRecharged (sum of all positive balance top-ups).
	GetUserBalanceHistory(ctx context.Context, userID string, page, pageSize int, codeType string) ([]RedeemCode, int64, float64, error)
	BindUserAuthIdentity(ctx context.Context, userID string, input AdminBindAuthIdentityInput) (*AdminBoundAuthIdentity, error)
	...
	DuplicateGroup(ctx context.Context, id string, actorScope, operationKey string) (*Group, error)
	// RecoverDuplicateGroup returns a previously committed copy for an ambiguous retry.
	// It never creates a group.
	RecoverDuplicateGroup(ctx context.Context, id string, actorScope, operationKey string) (*Group, error)
	UpdateGroup(ctx context.Context, id string, input *UpdateGroupInput) (*Group, error)
	DeleteGroup(ctx context.Context, id string) error
	ListCompositeRoutes(ctx context.Context, groupID string) ([]CompositeModelRoute, error)
	CreateCompositeRoute(ctx context.Context, groupID string, input CompositeRouteInput) (*CompositeModelRoute, error)
	UpdateCompositeRoute(ctx context.Context, groupID, routeID string, input CompositeRouteInput) (*CompositeModelRoute, error)
	DeleteCompositeRoute(ctx context.Context, groupID, routeID string) error
	PreviewCompositeRoute(ctx context.Context, groupID string, input CompositeRoutePreviewRequest) (*CompositeRouteDecision, error)
	GetGroupAPIKeys(ctx context.Context, groupID string, page, pageSize int) ([]APIKey, int64, error)
	GetGroupRateMultipliers(ctx context.Context, groupID string) ([]UserGroupRateEntry, error)
	ClearGroupRateMultipliers(ctx context.Context, groupID string) error
	BatchSetGroupRateMultipliers(ctx context.Context, groupID string, entries []GroupRateMultiplierInput) error
	ClearGroupRPMOverrides(ctx context.Context, groupID string) error
	BatchSetGroupRPMOverrides(ctx context.Context, groupID string, entries []GroupRPMOverrideInput) error
	UpdateGroupSortOrders(ctx context.Context, updates []GroupSortOrderUpdate) error

	// API Key management (admin)
	AdminUpdateAPIKeyGroupID(ctx context.Context, keyID string, groupID *string) (*AdminUpdateAPIKeyGroupIDResult, error)
	AdminResetAPIKeyRateLimitUsage(ctx context.Context, keyID string) (*APIKey, error)

	// ReplaceUserGroup 替换用户的专属分组：授予新分组权限、迁移 Key、移除旧分组权限
	ReplaceUserGroup(ctx context.Context, userID, oldGroupID, newGroupID string) (*ReplaceUserGroupResult, error)

	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus, sortBy, sortOrder string) ([]Account, int64, error)
	// ListAccountsForSchedulerScoreFilter 返回符合过滤条件的全部账号（不分页），
	// 作为账号列表页计算 OpenAI 调度分数的过滤范围池。
	ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) ([]Account, error)
	// ListOpenAISchedulableAccountsForSchedulerScore 返回指定分组（nil 为未分组）内
	// 可调度的 OpenAI 账号，用于按组计算调度分数。
	ListOpenAISchedulableAccountsForSchedulerScore(ctx context.Context, groupID *string) ([]Account, error)
	...
	DuplicateAccount(ctx context.Context, id string, actorScope, operationKey string) (*Account, error)
	// RecoverDuplicateAccount returns a previously committed duplicate for an ambiguous retry.
	// It never creates an account.
	RecoverDuplicateAccount(ctx context.Context, id string, actorScope, operationKey string) (*Account, error)
	UpdateAccount(ctx context.Context, id string, input *UpdateAccountInput) (*Account, error)
	// UpdateAccountExtra 仅对 Extra 做 JSONB 增量合并（key 级覆盖），不会影响其它字段或运行态键。
	// 用于刷新流程持久化 account_uuid / org_uuid 等少量键，避免被全量快照覆盖。
	UpdateAccountExtra(ctx context.Context, id string, updates map[string]any) error
	DeleteAccount(ctx context.Context, id string) error
	RefreshAccountCredentials(ctx context.Context, id string) (*Account, error)
	ClearAccountError(ctx context.Context, id string) (*Account, error)
	SetAccountError(ctx context.Context, id string, errorMsg string) error
	...
	SetAccountSchedulable(ctx context.Context, id string, schedulable bool) (*Account, error)
	...
	RevertAccountProxyFallback(ctx context.Context, id string) error
	// CreateShadow 为指定 OpenAI OAuth 母账号创建 spark 维度影子账号（一母一影）。
	// 影子账号不持凭据（Credentials 恒为空），透传母账号凭据；继承母账号的 ProxyID。
	CreateShadow(ctx context.Context, parentID string, opts ShadowOptions) (*Account, error)
	...
	BatchDeleteProxies(ctx context.Context, ids []string) (*ProxyBatchDeleteResult, error)
	GetProxyAccounts(ctx context.Context, proxyID string) ([]ProxyAccountSummary, error)
	CheckProxyExists(ctx context.Context, host string, port int, username, password string) (bool, error)
	TestProxy(ctx context.Context, id string) (*ProxyTestResult, error)
	CheckProxyQuality(ctx context.Context, id string) (*ProxyQualityCheckResult, error)
	...
	ExpireRedeemCode(ctx context.Context, id string) (*RedeemCode, error)
...
type UpdateUserInput struct {
	...
	Status        string
	AllowedGroups *[]string // 使用指针区分"未提供"和"设置为空数组"
...
type CreateGroupInput struct {
	...
	DailyLimitUSD             *float64 // 日平台积分限额（历史字段名）
	WeeklyLimitUSD            *float64 // 周平台积分限额（历史字段名）
	MonthlyLimitUSD           *float64 // 月平台积分限额（历史字段名）
	...
	FallbackGroupID              *string // 降级分组 ID
	// 无效请求兜底分组 ID（仅 anthropic 平台使用）
	FallbackGroupIDOnInvalidRequest *string
	...
	CopyAccountsFromGroupIDs []string
}
...
type UpdateGroupInput struct {
	...
	WeeklyLimitUSD       *float64 // 周平台积分限额（历史字段名）
	MonthlyLimitUSD      *float64 // 月平台积分限额（历史字段名）
	// HTTP updates track field presence so partial updates do not clear limits.
...
type AdminUpdateAPIKeyGroupIDResult struct {
	...
	GrantedGroupID         *string // the group ID that was auto-granted
...
type GenerateRedeemCodesInput struct {
	...
	Type              string
	Value             float64
	GroupID           *string // 订阅类型专用：关联的分组ID
	ValidityDays      int     // 订阅类型专用：有效天数
	BonusValidityDays int     // 余额类型专用：到账后赠送余额有效天数
	ExpiresAt         *time.Time
}
...
type adminRechargeAffiliateAccruer interface {
	AccrueInviteRebate(ctx context.Context, inviteeUserID string, baseRechargeAmount float64) (float64, error)
}

type userGroupRateBatchReader interface {
	GetByUserIDs(ctx context.Context, userIDs []string) (map[string]map[string]float64, error)
}
