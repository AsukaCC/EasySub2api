...
func (s *CNProviderQuotaService) QueryUsage(ctx context.Context, accountID string) (*CNProviderQuotaProbeResult, error) {
...
func (s *CNProviderQuotaService) queryUsage(ctx context.Context, accountID string) (*CNProviderQuotaProbeResult, error) {
	...
	if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
		...
		result.Error = fmt.Sprintf("Authentication failed (HTTP %v)", resp.StatusCode)
		return result, nil
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		result.Error = fmt.Sprintf("API error (HTTP %v): %s", resp.StatusCode, truncate(strings.TrimSpace(string(bodyBytes)), 240))
		return result, nil
	}
	...
	case PlatformOpenCodeGo:
		...
		if !gjson.ValidBytes(bodyBytes) || gjson.GetBytes(bodyBytes, "error").Exists() || len(tiers) == 0 {
			result.Error = "Invalid OpenCode quota response"
			return result, nil
		}
...
func (s *CNProviderQuotaService) loadCodingPlanAccount(ctx context.Context, accountID string) (*Account, error) {
