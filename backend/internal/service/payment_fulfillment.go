...
func (s *PaymentService) toPaid(ctx context.Context, o *dbent.PaymentOrder, tradeNo string, paid float64, pk string) error {
	...
	).SetStatus(OrderStatusPaid).SetPaymentTradeNo(tradeNo).SetPaidAt(now).ClearFailedAt().ClearFailedReason().ClearDeletedAt().Save(ctx)
	if err != nil {
...
func (s *PaymentService) applyAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	...
	sourceOrderID := o.ID
	rebateRecipient := s.affiliateService.rebateRecipient(txCtx)
