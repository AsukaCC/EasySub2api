...
func (s *IdempotencyCleanupService) Start() {
	...
	s.startOnce.Do(func() {
		logger.LegacyPrintf("service.idempotency_cleanup", "[IdempotencyCleanup] started interval=%s batch=%v", s.interval, s.batch)
		go s.runLoop()
	})
}
...
func (s *IdempotencyCleanupService) cleanupOnce() {
	...
	if deleted > 0 {
		logger.LegacyPrintf("service.idempotency_cleanup", "[IdempotencyCleanup] cleaned expired records count=%v", deleted)
	}
}
