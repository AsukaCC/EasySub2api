...
func (r *detachedIdempotencyRepo) MarkSucceeded(ctx context.Context, id string, status int, body string, expiresAt time.Time) error {
...
func (r *detachedIdempotencyRepo) ExtendProcessingLock(ctx context.Context, id, fingerprint string, lockedUntil, expiresAt time.Time) (bool, error) {
...
func TestIdempotencyCoordinator_DetachedExecutionPersistsAfterCancellation(t *testing.T) {
	for _, deadlineExpires := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.True(t, repo.savedWithBound)
