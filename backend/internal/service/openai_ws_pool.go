...
func (e *openAIWSDialError) Error() string {
	...
	if e.StatusCode > 0 {
		return fmt.Sprintf("openai ws dial failed: status=%v err=%v", e.StatusCode, e.Err)
	}
...
func newOpenAIWSConn(id string, _ string, ws openAIWSClientConn, handshakeHeaders http.Header, onClose ...func()) *openAIWSConn {
	now := time.Now()
	conn := &openAIWSConn{
		id:               id,
		ws:               ws,
...
func (c *openAIWSConn) closeWith(force bool) {
	...
	c.closeOnce.Do(func() {
		close(c.closedCh)
		if c.ws != nil {
			if closer, ok := c.ws.(openAIWSForceCloser); ok && force {
				_ = closer.CloseNow()
			} else {
				_ = c.ws.Close()
			}
...
func (p *openAIWSConnPool) acquire(ctx context.Context, req openAIWSAcquireRequest, retry int, queueWait *openAIWSAcquireQueueWait) (*openAIWSConnLease, error) {
	if p == nil || req.Account == nil || req.Account.ID == "" {
...
func (p *openAIWSConnPool) recordLastSuccessfulAcquire(accountID string, generation uint64, req openAIWSAcquireRequest) {
	if p == nil || accountID == "" {
		return
	}
...
func (p *openAIWSConnPool) getOrCreateAccountPool(accountID string) *openAIWSAccountPool {
	if p == nil || accountID == "" {
		return nil
	}
...
func (p *openAIWSConnPool) ensureAccountPoolLocked(accountID string) *openAIWSAccountPool {
	return p.getOrCreateAccountPool(accountID)
}

func (p *openAIWSConnPool) getAccountPool(accountID string) (*openAIWSAccountPool, bool) {
	if p == nil || accountID == "" {
		return nil, false
	}
...
func (p *openAIWSConnPool) notifyAccountPoolChanged(accountID string) {
	ap, ok := p.getAccountPool(accountID)
	if !ok || ap == nil {
		return
	}
...
func (p *openAIWSConnPool) AccountPoolLoad(accountID string) (inflight int, waiters int, conns int) {
	if p == nil || accountID == "" {
		return 0, 0, 0
	}
	ap, ok := p.getAccountPool(accountID)
...
func (p *openAIWSConnPool) ensureTargetIdleAsync(accountID string) {
	if p == nil || accountID == "" {
		return
	}
...
func (p *openAIWSConnPool) prewarmConns(accountID string, req openAIWSAcquireRequest, total int, generations ...uint64) {
	generation := uint64(0)
...
func (p *openAIWSConnPool) ClearAccount(accountID string) {
	if p == nil || accountID == "" {
		return
	}
	ap, ok := p.getAccountPool(accountID)
...
func (p *openAIWSConnPool) evictConn(accountID string, connID string) {
	if p == nil || accountID == "" || stringsTrim(connID) == "" {
		return
	}
...
func (p *openAIWSConnPool) PinConn(accountID string, connID string) bool {
	if p == nil || accountID == "" {
		return false
	}
...
func (p *openAIWSConnPool) UnpinConn(accountID string, connID string) {
	if p == nil || accountID == "" {
		return
	}
	connID = stringsTrim(connID)
...
func (p *openAIWSConnPool) dialConn(ctx context.Context, req openAIWSAcquireRequest) (*openAIWSConn, error) {
	...
	p.openConns.Add(1)
	pooledConn := newOpenAIWSConn(id, req.Account.ID, conn, handshakeHeaders, func() {
		p.openConns.Add(-1)
		p.totalConnSlots.Add(-1)
	})
	accountID := req.Account.ID
...
func (p *openAIWSConnPool) nextConnID(accountID string) string {
	seq := p.seq.Add(1)
	buf := make([]byte, 0, 32)
