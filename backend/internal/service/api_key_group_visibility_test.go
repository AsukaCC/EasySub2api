...
func (r *visibilityUserRepo) GetByID(context.Context, string) (*User, error) { return r.user, r.err }
...
func (r *visibilitySubRepo) ListActiveByUserID(_ context.Context, userID string) ([]UserSubscription, error) {
	r.calls++
...
func TestGetUserAllowedGroupIDSetIncludesActiveSubscriptions(t *testing.T) {
	...
	svc := &APIKeyService{
		userRepo:    &visibilityUserRepo{user: &User{ID: "1", AllowedGroups: []string{"7"}}},
		userSubRepo: subs,
	}
	visible, err := svc.GetUserAllowedGroupIDSet(context.Background(), "1")
	require.NoError(t, err)
	require.Equal(t, map[string]struct{}{"7": {}, "42": {}}, visible)
}
