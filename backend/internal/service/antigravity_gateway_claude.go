...
func (s *AntigravityGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte, isStickySession bool) (*ForwardResult, error) {
	...
	result, err := s.antigravityRetryLoop(antigravityRetryLoopParams{
		...
		groupID:         "",              // Forward 方法没有 groupID，由上层处理粘性会话清除
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest && isSignatureRelatedError(respBody) && s.settingService.IsSignatureRectifierEnabled(ctx) {
			...
			for _, stage := range retryStages {
				...
				logger.LegacyPrintf("service.antigravity_gateway", "Antigravity account %s: detected signature-related 400, retrying once (%s)", account.ID, stage.name)
				...
				if retryErr != nil {
					...
					logger.LegacyPrintf("service.antigravity_gateway", "Antigravity account %s: signature retry request failed (%s): %v", account.ID, stage.name, retryErr)
					continue
				}
		...
		if resp.StatusCode == http.StatusBadRequest && respBody != nil && !isSignatureRelatedError(respBody) {
			...
			if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
				...
				if claudeReq.Thinking == nil || claudeReq.Thinking.Type != "adaptive" {
					...
					logger.LegacyPrintf("service.antigravity_gateway", "Antigravity account %s: detected budget_tokens constraint error, retrying with rectified budget (budget_tokens=%d, max_tokens=%d)", account.ID, BudgetRectifyBudgetTokens, BudgetRectifyMaxTokens)
					...
					if txErr == nil {
						...
						if retryErr == nil {
							...
							logger.LegacyPrintf("service.antigravity_gateway", "Antigravity account %s: budget rectifier retry failed: %v", account.ID, retryErr)
						}
					}
				}
			}
		...
		if resp.StatusCode >= 400 {
			...
			s.handleUpstreamError(ctx, prefix, account, resp.StatusCode, resp.Header, respBody, originalModel, "", "", isStickySession)
			...
			if resp.StatusCode == http.StatusBadRequest {
				...
				if isGoogleProjectConfigError(msg) {
					...
					upstreamDetail := s.getUpstreamErrorDetail(respBody)
					log.Printf("%s status=400 google_config_error failover=true upstream_message=%q account=%s", prefix, upstreamMsg, account.ID)
					appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
						Platform:           account.Platform,
						AccountID:          account.ID,
						AccountName:        account.Name,
