...
func TestSanitizeOpsUpstreamErrorsForQueueBoundsAndRedacts(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		proxyID := "proxy-id-" + strconv.Itoa(i+1)
	...
	for i, event := range events {
		if event.ProxyID == nil || *event.ProxyID != "proxy-id-"+strconv.Itoa(i+1) {
			t.Fatalf("event %d proxy id = %v, want proxy-id-%d", i, event.ProxyID, i+1)
		}
