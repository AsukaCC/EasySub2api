...
func (a *Account) GeminiOAuthType() string {
	if a == nil || a.Platform != PlatformGemini || a.Type != AccountTypeOAuth {
		return ""
	}
...
func normalizeRequestedModelForLookup(platform, requestedModel string) string {
	...
	if (platform == PlatformGemini || platform == PlatformAntigravity) && trimmed == "gemini-3.1-pro-preview-customtools" {
...
// Responses endpoint. DeepSeek uses /responses while Kimi and MiniMax use
// /v1/responses.
...
func (a *Account) IsMixedSchedulingEnabled() bool {
	...
	enabled, ok := a.Extra["mixed_scheduling"].(bool)
	return ok && enabled
}
...
func (a *Account) IsOveragesEnabled() bool {
	...
	enabled, ok := a.Extra["allow_overages"].(bool)
	return ok && enabled
}
