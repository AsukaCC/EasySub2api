...
func summarizeOpenAIWSPayloadKeySizes(payload map[string]any, topN int) string {
	...
	for idx := 0; idx < topN; idx++ {
		item := sizes[idx]
		parts = append(parts, fmt.Sprintf("%s:%v", item.Key, item.Size))
	}
...
func summarizeOpenAIWSInput(input any) string {
	...
	return fmt.Sprintf(
		"items=%v,text_chars=%v,image_data_urls=%v,image_data_url_chars=%v,image_remote_urls=%v",
		itemCount,
		textChars,
		imageDataURLs,
		imageDataURLChars,
...
func logOpenAIWSBindResponseAccountWarn(groupID, accountID string, responseID string, err error) {
	if err == nil {
		return
	}
	logger.L().Warn(
...
func classifyOpenAIWSDialError(err error) string {
	...
	if status := coderws.CloseStatus(baseErr); status != -1 {
		return normalizeOpenAIWSLogValue(fmt.Sprintf("ws_close_%v", int(status)))
	}
