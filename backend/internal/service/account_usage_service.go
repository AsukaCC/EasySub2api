...
import (
	...
	httppool "github.com/AsukaCC/EasySub2api/internal/pkg/httpclient"
	openaipkg "github.com/AsukaCC/EasySub2api/internal/pkg/openai"
...
type UsageLogRepository interface {
	...
	GetByID(ctx context.Context, id string) (*UsageLog, error)
	Delete(ctx context.Context, id string) error

	ListByUser(ctx context.Context, userID string, params pagination.PaginationParams) ([]UsageLog, *pagination.PaginationResult, error)
	ListByAPIKey(ctx context.Context, apiKeyID string, params pagination.PaginationParams) ([]UsageLog, *pagination.PaginationResult, error)
	ListByAccount(ctx context.Context, accountID string, params pagination.PaginationParams) ([]UsageLog, *pagination.PaginationResult, error)

	ListByUserAndTimeRange(ctx context.Context, userID string, startTime, endTime time.Time) ([]UsageLog, *pagination.PaginationResult, error)
	ListByAPIKeyAndTimeRange(ctx context.Context, apiKeyID string, startTime, endTime time.Time) ([]UsageLog, *pagination.PaginationResult, error)
	ListByAccountAndTimeRange(ctx context.Context, accountID string, startTime, endTime time.Time) ([]UsageLog, *pagination.PaginationResult, error)
	...
	GetAccountWindowStats(ctx context.Context, accountID string, startTime time.Time) (*usagestats.AccountStats, error)
	GetAccountTodayStats(ctx context.Context, accountID string) (*usagestats.AccountStats, error)

	// Admin dashboard stats
	GetDashboardStats(ctx context.Context) (*usagestats.DashboardStats, error)
	GetUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID string, model string, requestType *int16, stream *bool, billingType *int8) ([]usagestats.TrendDataPoint, error)
	GetModelStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, requestType *int16, stream *bool, billingType *int8) ([]usagestats.ModelStat, error)
	GetEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, requestType *int16, stream *bool, billingType *int8) ([]usagestats.EndpointStat, error)
	GetUpstreamEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, requestType *int16, stream *bool, billingType *int8) ([]usagestats.EndpointStat, error)
	GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, requestType *int16, stream *bool, billingType *int8) ([]usagestats.GroupStat, error)
	...
	GetUserDashboardStats(ctx context.Context, userID string) (*usagestats.UserDashboardStats, error)
	GetAPIKeyDashboardStats(ctx context.Context, apiKeyID string) (*usagestats.UserDashboardStats, error)
	GetUserUsageTrendByUserID(ctx context.Context, userID string, startTime, endTime time.Time, granularity string) ([]usagestats.TrendDataPoint, error)
	GetUserModelStats(ctx context.Context, userID string, startTime, endTime time.Time) ([]usagestats.ModelStat, error)
	...
	// Account stats
	GetAccountUsageStats(ctx context.Context, accountID string, startTime, endTime time.Time) (*usagestats.AccountUsageStatsResponse, error)

	// Aggregated stats (optimized)
	GetUserStatsAggregated(ctx context.Context, userID string, startTime, endTime time.Time) (*usagestats.UsageStats, error)
	GetAPIKeyStatsAggregated(ctx context.Context, apiKeyID string, startTime, endTime time.Time) (*usagestats.UsageStats, error)
	GetAccountStatsAggregated(ctx context.Context, accountID string, startTime, endTime time.Time) (*usagestats.UsageStats, error)
	GetModelStatsAggregated(ctx context.Context, modelName string, startTime, endTime time.Time) (*usagestats.UsageStats, error)
	GetDailyStatsAggregated(ctx context.Context, userID string, startTime, endTime time.Time) ([]map[string]any, error)
}
...
const (
	...
	antigravityErrorTTL = 1 * time.Minute
...
type UsageCache struct {
	...
	apiFlight         singleflight.Group
	antigravityFlight singleflight.Group
	openAIProbeCache  sync.Map
	grokProbeCache    sync.Map
}
...
type ClaudeUsageFetchOptions struct {
	...
	AccountID   string                  // 账号 ID（用于连接池隔离）
...
func (s *AccountUsageService) getUsageForAccount(ctx context.Context, account *Account, forceProbe bool) (*UsageInfo, error) {
	...
	if account.CanGetUsage() {
		...
		if apiResp == nil {
			...
			flightKey := fmt.Sprintf("usage:%v", accountID)
...
func (s *AccountUsageService) GetUsage(ctx context.Context, accountID string, force ...bool) (*UsageInfo, error) {
...
func (s *AccountUsageService) GetUsageBatch(ctx context.Context, accountIDs []string, force bool) (map[string]*UsageInfo, map[string]string, error) {
	...
	usageByAccount := make(map[string]*UsageInfo, len(uniqueIDs))
...
func (s *AccountUsageService) GetPassiveUsage(ctx context.Context, accountID string) (*UsageInfo, error) {
	account, err := s.accountRepo.GetByID(ctx, accountID)
	if err != nil {
		return nil, fmt.Errorf("get account failed: %w", err)
	}
...
// 这样下次被动加载时能看到最新值。
func (s *AccountUsageService) syncActiveToPassive(ctx context.Context, accountID string, usage *UsageInfo) {
...
func (s *AccountUsageService) shouldProbeOpenAICodexSnapshot(accountID string, now time.Time, force ...bool) bool {
	if s == nil || s.cache == nil || accountID == "" {
		return true
	}
	forceProbe := len(force) > 0 && force[0]
	if !forceProbe {
...
func (s *AccountUsageService) persistOpenAICodexProbeSnapshot(accountID string, updates map[string]any) {
	if s == nil || s.accountRepo == nil || accountID == "" {
		return
	}
	if len(updates) == 0 {
		return
...
func extractOpenAICodexProbeUpdates(resp *http.Response) (map[string]any, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("openai codex probe returned status %v", resp.StatusCode)
	}
	return nil, nil
}
...
func (s *AccountUsageService) getGeminiUsage(ctx context.Context, account *Account) (*UsageInfo, error) {
	...
	stats, err := s.usageLogRepo.GetModelStatsWithFilters(ctx, dayStart, now, "", "", account.ID, "", nil, nil, nil)
	if err != nil {
	...
	minuteStats, err := s.usageLogRepo.GetModelStatsWithFilters(ctx, minuteStart, now, "", "", account.ID, "", nil, nil, nil)
	if err != nil {
...
func grokLocalUsage24h(ctx context.Context, repo UsageLogRepository, accountID string, now time.Time) *WindowStats {
	if repo == nil || accountID == "" {
		return nil
	}
...
func (s *AccountUsageService) shouldProbeGrokBilling(accountID string, now time.Time, force bool) bool {
	if force || s == nil || s.cache == nil || accountID == "" {
		return true
	}
...
func (s *AccountUsageService) addWindowStats(ctx context.Context, account *Account, usage *UsageInfo) {
	...
	if windowStats == nil {
		...
		if err != nil {
			log.Printf("Failed to get window stats for account %v: %v", account.ID, err)
			return
		}
...
func (s *AccountUsageService) GetTodayStats(ctx context.Context, accountID string) (*WindowStats, error) {
...
func (s *AccountUsageService) GetTodayStatsBatch(ctx context.Context, accountIDs []string) (map[string]*WindowStats, error) {
	uniqueIDs := make([]string, 0, len(accountIDs))
	seen := make(map[string]struct{}, len(accountIDs))
	for _, accountID := range accountIDs {
		if accountID == "" {
	...
	result := make(map[string]*WindowStats, len(uniqueIDs))
	if len(uniqueIDs) == 0 {
		return result, nil
	}
...
func (s *AccountUsageService) GetAccountUsageStats(ctx context.Context, accountID string, startTime, endTime time.Time) (*usagestats.AccountUsageStatsResponse, error) {
...
func (s *AccountUsageService) tryClearRecoverableAccountError(ctx context.Context, account *Account) {
	...
	if err := s.accountRepo.ClearError(ctx, account.ID); err != nil {
		log.Printf("[usage] failed to clear recoverable account error for account %v: %v", account.ID, err)
		return
	}
...
// 用于账号列表页面显示当前窗口费用
func (s *AccountUsageService) GetAccountWindowStats(ctx context.Context, accountID string, startTime time.Time) (*usagestats.AccountStats, error) {
