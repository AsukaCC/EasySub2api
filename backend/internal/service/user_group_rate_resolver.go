...
func (r *userGroupRateResolver) Resolve(ctx context.Context, userID, groupID string, groupDefaultMultiplier float64) float64 {
	if r == nil || userID == "" || groupID == "" {
		return groupDefaultMultiplier
	}

	key := fmt.Sprintf("%v:%v", userID, groupID)
	if r.cache != nil {
	...
	if err != nil {
		...
		logger.LegacyPrintf(r.logComponent, "get user group rate failed, fallback to group default: user=%v group=%v err=%v", userID, groupID, err)
		return groupDefaultMultiplier
	}
