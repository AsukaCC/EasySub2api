...
func applyUpstreamModelMetadataToCodexDescriptor(descriptor *configuredCodexModelDescriptor, metadata codexModelMetadataOverride) {
	if descriptor == nil {
		return
	}
	...
	if metadata.reasoningConflict {
		...
		descriptor.SupportedReasoningLevels = []configuredCodexReasoningLevel{{Effort: "none", Description: configuredCodexReasoningLevelDescription("none")}}
	} else if metadata.Reasoning != nil && *metadata.Reasoning {
