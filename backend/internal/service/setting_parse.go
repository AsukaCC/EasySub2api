...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyAuthSourceDefaultLinuxDoBonusValidityDays:  "0",
		...
		SettingKeyAuthSourceDefaultOIDCBonusValidityDays:     "0",
		...
		SettingKeyAuthSourceDefaultWeChatBonusValidityDays:   "0",
		...
		SettingKeyAuthSourceDefaultGitHubBonusValidityDays:   "0",
		...
		SettingKeyAuthSourceDefaultGoogleBonusValidityDays:   "0",
		...
		SettingKeyAuthSourceDefaultDingTalkBonusValidityDays: "0",
		...
		SettingKeyAvailableChannelsUserVisible: "false",
		...
		SettingKeyAffiliateUserVisible:          "false",
		SettingPaymentUserVisible:               "false",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		SiteBillingMode:                        siteBillingMode(!isFalseSettingValue(settings[SettingKeySubscriptionEnabled]), settings[SettingBalancePayDisabled] == "true"),
	...
	if days, err := strconv.Atoi(settings[SettingKeyBonusBalanceDefaultValidityDays]); err == nil && days > 0 {
		result.BonusBalanceDefaultValidityDays = days
	} else {
		result.BonusBalanceDefaultValidityDays = defaultBonusValidityDays
	}
	...
	result.ChannelMonitorUserVisible = userVisibilitySetting(settings, SettingKeyChannelMonitorUserVisible, result.ChannelMonitorEnabled)
	...
	result.AvailableChannelsUserVisible = userVisibilitySetting(settings, SettingKeyAvailableChannelsUserVisible, result.AvailableChannelsEnabled)
...
func userVisibilitySetting(settings map[string]string, key string, enabled bool) bool {
...
func parseDefaultSubscriptions(raw string) []DefaultSubscriptionSetting {
	...
	for _, item := range items {
		if item.GroupID == "" || item.ValidityDays <= 0 {
			continue
		}
...
func parseProviderDefaultGrantSettings(settings map[string]string, keys authSourceDefaultKeySet) ProviderDefaultGrantSettings {
	...
	if v, err := strconv.Atoi(strings.TrimSpace(settings[keys.bonusValidityDays])); err == nil && v > 0 {
		result.BonusValidityDays = v
	}
...
func writeProviderDefaultGrantUpdates(updates map[string]string, keys authSourceDefaultKeySet, settings ProviderDefaultGrantSettings) {
	...
	updates[keys.bonusValidityDays] = strconv.Itoa(settings.BonusValidityDays)
...
func mergeProviderDefaultGrantSettings(globalDefaults ProviderDefaultGrantSettings, providerDefaults ProviderDefaultGrantSettings) ProviderDefaultGrantSettings {
	result := ProviderDefaultGrantSettings{
		...
		BonusValidityDays: globalDefaults.BonusValidityDays,
	...
	if providerDefaults.BonusValidityDays > 0 {
		result.BonusValidityDays = providerDefaults.BonusValidityDays
	}
