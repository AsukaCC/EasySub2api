...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	ctx, preemptCleanup, _ := s.BeginOpenAIWSIngressSessionPreemptionWithClient(ctx, c, account, firstClientMessage, clientConn)
	defer preemptCleanup()
	defer func() {
		if isOpenAIWSSessionPreempted(ctx) {
			resultErr = errOpenAIWSSessionPreempted
		}
	}()
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		if originalModel == "" {
			...
			// model，后续 turn 复用同一 session-level model。策略匹配必须先
			// 使用该客户端模型，再进行账号模型映射。
		...
		nativeCompactionV2 := HasCompactionTriggerInInput(normalized)
		if next, policyErr := applyOpenAIWSReasoningEffortPolicyForModel(normalized, hooks, originalModel); policyErr != nil {
		...
		if codexBridgeEnabled {
			...
			if ensureOpenAIResponsesImageGenerationTool(payloadMap) {
				bridgeModified = true
				logOpenAIWSModeInfo("ingress_ws_codex_image_tool_injected account_id=%v", account.ID)
			}
			if ensureOpenAIResponsesImageGenerationToolChoiceAuto(payloadMap) {
				bridgeModified = true
				logOpenAIWSModeInfo("ingress_ws_codex_image_tool_choice_auto account_id=%v", account.ID)
			}
			...
			if applyCodexImageGenerationBridgeInstructions(payloadMap) {
				bridgeModified = true
				logOpenAIWSModeInfo("ingress_ws_codex_image_bridge_instructions_added account_id=%v", account.ID)
			}
			if bridgeModified {
		...
		if isCodexCLI && codexImageGenerationExplicitToolPolicy == codexImageGenerationExplicitToolPolicyStrip {
			if stripped, changed, stripErr := stripOpenAIImageGenerationToolsFromRawPayload(normalized); stripErr != nil {
				...
				normalized = stripped
				logOpenAIWSModeInfo("ingress_ws_codex_image_tool_stripped_by_policy account_id=%v", account.ID)
			}
		}
		if stripped, changed, stripErr := stripCodexSparkImageGenerationToolFromRawPayload(normalized, upstreamModel); stripErr != nil {
			...
			normalized = stripped
			logOpenAIWSModeInfo("ingress_ws_codex_spark_image_tool_stripped account_id=%v", account.ID)
		}
	...
	readClientMessage := func() ([]byte, error) {
		...
		if readErr != nil {
			...
			if errors.As(readErr, &closeErr) && closeErr.StatusCode() == coderws.StatusNormalClosure {
				logOpenAIWSModeInfo("ingress_ws_inter_turn_idle_timeout account_id=%v timeout_seconds=%v", account.ID, int(idleTimeout.Seconds()))
			}
			return nil, readErr
		}
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		logOpenAIWSModeInfo(
			"ingress_ws_http_bridge_start account_id=%v account_type=%s payload_bytes=%v threshold_bytes=%v has_session_hash=%v store_disabled=%v",
			account.ID,
			account.Type,
		...
		for turn := 1; ; turn++ {
			...
			if needsBridgeReplay && turnReplayInputExists {
				...
				logOpenAIWSModeInfo(
					"ingress_ws_http_bridge_replay_input account_id=%v turn=%v input_items=%v previous_response_id_present=%v has_tool_output=%v",
					account.ID,
					turn,
					len(turnReplayInput),
			...
			// turnReplayInput 可能共享同一头数组（转移自当前 input），保存历史必须经
			// combine 新建头，禁止就地 append。
			...
			if readErr != nil {
				if isOpenAIWSClientDisconnectError(readErr) {
					...
					logOpenAIWSModeInfo(
						"ingress_ws_http_bridge_client_closed account_id=%v close_status=%s close_reason=%s",
						account.ID,
						closeStatus,
	...
	logOpenAIWSModeInfo(
		"ingress_ws_protocol_confirm account_id=%v account_type=%s transport=%s ws_host=%s ws_path=%s ws_mode=%s store_disabled=%v has_session_hash=%v has_previous_response_id=%v",
		account.ID,
		account.Type,
	...
	if debugEnabled {
		logOpenAIWSModeDebug(
			"ingress_ws_start account_id=%v account_type=%s transport=%s ws_host=%s preferred_conn_id=%s has_session_hash=%v has_previous_response_id=%v store_disabled=%v",
			account.ID,
			account.Type,
			normalizeOpenAIWSLogValue(string(wsDecision.Transport)),
			wsHost,
	...
	if firstPayload.previousResponseID != "" {
		...
		logOpenAIWSModeInfo(
			"ingress_ws_continuation_probe account_id=%v turn=%v previous_response_id=%s previous_response_id_kind=%s preferred_conn_id=%s session_hash=%s header_session_id=%s header_conversation_id=%s has_turn_state=%v turn_state_len=%v has_prompt_cache_key=%v store_disabled=%v",
			account.ID,
			1,
	...
	acquireTurnLease = func(turn int, preferred string, forcePreferredConn bool, forceNewConn bool) (*openAIWSConnLease, error) {
		...
		if acquireErr != nil {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_upstream_acquire_fail account_id=%v turn=%v reason=%s dial_status=%v dial_class=%s dial_close_status=%s dial_close_reason=%s dial_resp_server=%s dial_resp_via=%s dial_resp_cf_ray=%s dial_resp_x_request_id=%s cause=%s preferred_conn_id=%s force_preferred_conn=%v ws_host=%s ws_path=%s proxy_enabled=%v",
				account.ID,
				turn,
		...
		logOpenAIWSModeInfo(
			"ingress_ws_upstream_connected account_id=%v turn=%v conn_id=%s conn_reused=%v conn_pick_ms=%v queue_wait_ms=%v preferred_conn_id=%s",
			account.ID,
			turn,
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, requestedReasoningEffort *string, nativeCompactionV2 bool) (*OpenAIForwardResult, error) {
		...
		if debugEnabled {
			logOpenAIWSModeDebug(
				"ingress_ws_turn_request_sent account_id=%v turn=%v conn_id=%s payload_bytes=%v",
				account.ID,
				turn,
		...
		for {
			...
			// cyber_policy 记录必须先于下方 error 事件的各类早退（恢复重试 / failover），
			// 否则 error 形态的策略拒绝会漏记。
			...
			if eventType == "error" {
				...
				if fallbackReason == openAIWSFallbackReasonInvalidEncryptedContent {
					...
					if digests := collectOpenAIEncryptedContentDigestsRaw(payload); len(digests) > 0 {
						...
						logOpenAIWSModeInfo(
							"ingress_ws_invalid_encrypted_lineage_mark account_id=%v turn=%v digests=%v",
							account.ID,
							turn,
							len(digests),
						)
				...
				if recoverablePrevNotFound {
					...
					logOpenAIWSModeInfo(
						"ingress_ws_prev_response_recoverable account_id=%v turn=%v conn_id=%s idx=%v reason=%s code=%s type=%s message=%s previous_response_id=%s previous_response_id_kind=%s response_id=%s store_disabled=%v has_prompt_cache_key=%v",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(lease.ConnID(), openAIWSIDValueMaxLen),
						eventCount,
					...
					logOpenAIWSModeInfo(
						"ingress_ws_error_event account_id=%v turn=%v conn_id=%s idx=%v fallback_reason=%s err_code=%s err_type=%s err_message=%s previous_response_id=%s previous_response_id_kind=%s response_id=%s store_disabled=%v has_prompt_cache_key=%v",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(lease.ConnID(), openAIWSIDValueMaxLen),
						eventCount,
			...
			if !clientDisconnected {
				...
				// Rewrite only the client-facing copy. Account state and terminal
				// failure handling below must continue to inspect the original code.
				...
				if err := writeClientMessage(clientMessage); err != nil {
					if isOpenAIWSClientDisconnectError(err) {
						...
						logOpenAIWSModeInfo(
							"ingress_ws_client_disconnected_drain account_id=%v turn=%v conn_id=%s close_status=%s close_reason=%s",
							account.ID,
							turn,
							truncateOpenAIWSLogValue(lease.ConnID(), openAIWSIDValueMaxLen),
							closeStatus,
			...
			if isTerminalEvent {
				...
				if debugEnabled {
					logOpenAIWSModeDebug(
						"ingress_ws_turn_completed account_id=%v turn=%v conn_id=%s response_id=%s duration_ms=%v events=%v token_events=%v terminal_events=%v first_event=%s last_event=%s first_token_ms=%v client_disconnected=%v",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(lease.ConnID(), openAIWSIDValueMaxLen),
						truncateOpenAIWSLogValue(responseID, openAIWSIDValueMaxLen),
	...
	releaseSessionLease := func() {
		...
		if debugEnabled {
			logOpenAIWSModeDebug(
				"ingress_ws_upstream_released account_id=%v conn_id=%s",
				account.ID,
	...
	recoverIngressPrevResponseNotFound := func(relayErr error, turn int, connID string) bool {
		...
		if isStrictAffinityTurn(currentPayload) {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_prev_response_recovery_layer2 account_id=%v turn=%v conn_id=%s store_disabled_conn_mode=%s action=drop_previous_response_id_retry",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		if dropErr != nil || !removed {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_prev_response_recovery_skip account_id=%v turn=%v conn_id=%s reason=%s",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		if setInputErr != nil {
			logOpenAIWSModeInfo(
				"ingress_ws_prev_response_recovery_skip account_id=%v turn=%v conn_id=%s reason=set_full_input_error cause=%s",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		logOpenAIWSModeInfo(
			"ingress_ws_prev_response_recovery account_id=%v turn=%v conn_id=%s action=drop_previous_response_id retry=1",
			account.ID,
			turn,
			truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		)
	...
	retryIngressTurn := func(relayErr error, turn int, connID string) bool {
		...
		if isStrictAffinityTurn(currentPayload) {
			logOpenAIWSModeInfo(
				"ingress_ws_turn_retry_skip account_id=%v turn=%v conn_id=%s reason=strict_affinity",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
			)
		...
		turnRetry++
		logOpenAIWSModeInfo(
			"ingress_ws_turn_retry account_id=%v turn=%v retry=%v reason=%s conn_id=%s",
			account.ID,
			turn,
			turnRetry,
	...
	for {
		...
		if shouldInferIngressFunctionCallOutputPreviousResponseID(
			...
			if setPrevErr != nil {
				logOpenAIWSModeInfo(
					"ingress_ws_function_call_output_prev_infer_skip account_id=%v turn=%v conn_id=%s reason=set_previous_response_id_error cause=%s expected_previous_response_id=%s",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
				...
				logOpenAIWSModeInfo(
					"ingress_ws_function_call_output_prev_infer account_id=%v turn=%v conn_id=%s action=set_previous_response_id previous_response_id=%s",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
					truncateOpenAIWSLogValue(expectedPrev, openAIWSIDValueMaxLen),
		...
		if replayInputErr != nil {
			logOpenAIWSModeInfo(
				"ingress_ws_replay_input_skip account_id=%v turn=%v conn_id=%s reason=build_error cause=%s",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
		...
		if storeDisabled && turn > 1 && currentPreviousResponseID != "" {
			...
			if strictErr != nil {
				logOpenAIWSModeInfo(
					"ingress_ws_prev_response_strict_eval account_id=%v turn=%v conn_id=%s action=keep_previous_response_id reason=%s cause=%s previous_response_id=%s expected_previous_response_id=%s has_function_call_output=%v",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
				...
				if dropErr != nil || !removed {
					...
					logOpenAIWSModeInfo(
						"ingress_ws_prev_response_strict_eval account_id=%v turn=%v conn_id=%s action=keep_previous_response_id reason=%s drop_reason=%s previous_response_id=%s expected_previous_response_id=%s has_function_call_output=%v",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
						normalizeOpenAIWSLogValue(strictReason),
					...
					if setInputErr != nil {
						logOpenAIWSModeInfo(
							"ingress_ws_prev_response_strict_eval account_id=%v turn=%v conn_id=%s action=keep_previous_response_id reason=%s drop_reason=set_full_input_error previous_response_id=%s expected_previous_response_id=%s cause=%s has_function_call_output=%v",
							account.ID,
							turn,
							truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
							normalizeOpenAIWSLogValue(strictReason),
						...
						currentPayloadBytes = len(updatedWithInput)
						logOpenAIWSModeInfo(
							"ingress_ws_prev_response_strict_eval account_id=%v turn=%v conn_id=%s action=drop_previous_response_id_full_create reason=%s previous_response_id=%s expected_previous_response_id=%s has_function_call_output=%v",
							account.ID,
							turn,
							truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
							normalizeOpenAIWSLogValue(strictReason),
		...
		if shouldPreflightPing {
			if pingErr := sessionLease.PingWithTimeout(openAIWSProbePingTO); pingErr != nil {
				logOpenAIWSModeInfo(
					"ingress_ws_upstream_preflight_ping_fail account_id=%v turn=%v conn_id=%s cause=%s",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
				...
				if forcePreferredConn {
					...
					if !turnPrevRecoveryTried && currentPreviousResponseID != "" && (!hasFCOutput || hasReplayToolContext) {
						...
						if dropErr != nil || !removed {
							...
							logOpenAIWSModeInfo(
								"ingress_ws_preflight_ping_recovery_skip account_id=%v turn=%v conn_id=%s reason=%s previous_response_id=%s",
								account.ID,
								turn,
								truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
								normalizeOpenAIWSLogValue(reason),
							...
							if setInputErr != nil {
								logOpenAIWSModeInfo(
									"ingress_ws_preflight_ping_recovery_skip account_id=%v turn=%v conn_id=%s reason=set_full_input_error previous_response_id=%s cause=%s",
									account.ID,
									turn,
									truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
									truncateOpenAIWSLogValue(currentPreviousResponseID, openAIWSIDValueMaxLen),
								...
								logOpenAIWSModeInfo(
									"ingress_ws_preflight_ping_recovery account_id=%v turn=%v conn_id=%s action=drop_previous_response_id_retry previous_response_id=%s has_function_call_output=%v has_replay_tool_context=%v",
									account.ID,
									turn,
									truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
									truncateOpenAIWSLogValue(currentPreviousResponseID, openAIWSIDValueMaxLen),
					...
					if hasFCOutput && currentPreviousResponseID != "" {
						...
						logOpenAIWSModeInfo(
							"ingress_ws_preflight_ping_recovery_skip account_id=%v turn=%v conn_id=%s reason=%s action=fail_close previous_response_id=%s has_replay_tool_context=%v",
							account.ID,
							turn,
							truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
							reason,
		...
		if currentPreviousResponseID != "" {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_turn_chain account_id=%v turn=%v conn_id=%s previous_response_id=%s previous_response_id_kind=%s last_turn_response_id=%s chained_from_last=%v preferred_conn_id=%s header_session_id=%s header_conversation_id=%s has_turn_state=%v turn_state_len=%v has_prompt_cache_key=%v store_disabled=%v",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
				truncateOpenAIWSLogValue(currentPreviousResponseID, openAIWSIDValueMaxLen),
		...
		if strictStateErr != nil {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_prev_response_strict_state_skip account_id=%v turn=%v conn_id=%s reason=build_error cause=%s",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		if readErr != nil {
			if isOpenAIWSClientDisconnectError(readErr) {
				closeStatus, closeReason := summarizeOpenAIWSReadCloseError(readErr)
				logOpenAIWSModeInfo(
					"ingress_ws_client_closed account_id=%v conn_id=%s close_status=%s close_reason=%s",
					account.ID,
					truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
					closeStatus,
					truncateOpenAIWSLogValue(closeReason, openAIWSHeaderValueMaxLen),
		...
		if nextPayload.promptCacheKey != "" {
			...
			if updHdrErr != nil {
				logOpenAIWSModeInfo("ingress_ws_update_headers_failed account_id=%v err=%v", account.ID, updHdrErr)
			} else {
				baseAcquireReq.Headers = updatedHeaders
			}
		}
		...
		if nextPayload.previousResponseID != "" {
			...
			logOpenAIWSModeInfo(
				"ingress_ws_next_turn_chain account_id=%v turn=%v next_turn=%v conn_id=%s previous_response_id=%s previous_response_id_kind=%s last_turn_response_id=%s chained_from_last=%v has_prompt_cache_key=%v store_disabled=%v",
				account.ID,
				turn,
				turn+1,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		if stateStore != nil && nextPayload.previousResponseID != "" {
			if stickyConnID, ok := stateStore.GetResponseConn(nextPayload.previousResponseID); ok {
				if sessionConnID != "" && stickyConnID != "" && stickyConnID != sessionConnID {
					logOpenAIWSModeInfo(
						"ingress_ws_keep_session_conn account_id=%v turn=%v conn_id=%s sticky_conn_id=%s previous_response_id=%s",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
