...
type UsageCleanupRepository interface {
	...
	UpdateTaskProgress(ctx context.Context, taskID string, deletedRows int64) error
	// CancelTask 将任务标记为 canceled（仅允许 pending/running）
	CancelTask(ctx context.Context, taskID string, canceledBy string) (bool, error)
	MarkTaskSucceeded(ctx context.Context, taskID string, deletedRows int64) error
	MarkTaskFailed(ctx context.Context, taskID string, deletedRows int64, errorMsg string) error
