...
type apiKeyRateLimitLoader interface {
	GetRateLimitData(ctx context.Context, keyID string) (*APIKeyRateLimitData, error)
}
		...
		case cacheWriteUpdateSubscriptionUsage:
			if s.cache != nil {
				if err := s.cache.UpdateSubscriptionUsage(ctx, task.userID, task.groupID, task.amount); err != nil {
					logger.LegacyPrintf("service.billing_cache", "Warning: update subscription cache failed for user %v group %v: %v", task.userID, task.groupID, err)
				}
			}
		case cacheWriteDeductBalance:
			if s.cache != nil {
				if err := s.cache.DeductUserBalance(ctx, task.userID, task.amount); err != nil {
					logger.LegacyPrintf("service.billing_cache", "Warning: deduct balance cache failed for user %v: %v", task.userID, err)
				}
			}
...
func (s *BillingCacheService) logCacheWriteDrop(task cacheWriteTask, reason string) {
	...
	logger.LegacyPrintf("service.billing_cache", "Warning: cache write queue %s, dropped %v tasks in last %s (latest kind=%s user %v group %v)",
		reason,
		dropped,
...
func (s *BillingCacheService) GetUserBalance(ctx context.Context, userID string) (float64, error) {
	if s.cache == nil {
		// Redis不可用，直接查询数据库
	...
	value, err, _ := s.balanceLoadSF.Do(userID, func() (any, error) {
...
func (s *BillingCacheService) getUserBalanceFromDB(ctx context.Context, userID string) (float64, error) {
...
func (s *BillingCacheService) setBalanceCache(ctx context.Context, userID string, balance float64) {
	if s.cache == nil {
		return
	}
	if err := s.cache.SetUserBalance(ctx, userID, balance); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: set balance cache failed for user %v: %v", userID, err)
	}
}

// DeductBalanceCache 扣减余额缓存（同步调用）
func (s *BillingCacheService) DeductBalanceCache(ctx context.Context, userID string, amount float64) error {
	if s.cache == nil {
		return nil
	}
...
func (s *BillingCacheService) QueueDeductBalance(userID string, amount float64) {
	if s.cache == nil {
		return
	}
	...
	if err := s.DeductBalanceCache(ctx, userID, amount); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: deduct balance cache fallback failed for user %v: %v", userID, err)
	}
}

// InvalidateUserBalance 失效用户余额缓存
func (s *BillingCacheService) InvalidateUserBalance(ctx context.Context, userID string) error {
	if s.cache == nil {
		return nil
	}
	if err := s.cache.InvalidateUserBalance(ctx, userID); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: invalidate balance cache failed for user %v: %v", userID, err)
		return err
	}
	return nil
}
...
func (s *BillingCacheService) GetSubscriptionStatus(ctx context.Context, userID, groupID string) (*subscriptionCacheData, error) {
	if s.cache == nil {
...
func (s *BillingCacheService) getSubscriptionFromDB(ctx context.Context, userID, groupID string) (*subscriptionCacheData, error) {
...
func (s *BillingCacheService) setSubscriptionCache(ctx context.Context, userID, groupID string, data *subscriptionCacheData) {
	if s.cache == nil || data == nil {
		return
	}
	if err := s.cache.SetSubscriptionCache(ctx, userID, groupID, s.convertToPortsData(data)); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: set subscription cache failed for user %v group %v: %v", userID, groupID, err)
	}
}

// UpdateSubscriptionUsage 更新订阅用量缓存（同步调用）
func (s *BillingCacheService) UpdateSubscriptionUsage(ctx context.Context, userID, groupID string, costUSD float64) error {
	if s.cache == nil {
		return nil
	}
...
func (s *BillingCacheService) QueueUpdateSubscriptionUsage(userID, groupID string, costUSD float64) {
	if s.cache == nil {
		return
	}
	// 队列满时同步回退，确保订阅用量及时更新。
	...
	if err := s.UpdateSubscriptionUsage(ctx, userID, groupID, costUSD); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: update subscription cache fallback failed for user %v group %v: %v", userID, groupID, err)
	}
}

// InvalidateSubscription 失效指定订阅缓存
func (s *BillingCacheService) InvalidateSubscription(ctx context.Context, userID, groupID string) error {
	if s.cache == nil {
		return nil
	}
	if err := s.cache.InvalidateSubscriptionCache(ctx, userID, groupID); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: invalidate subscription cache failed for user %v group %v: %v", userID, groupID, err)
		return err
	}
	return nil
}
...
func (s *BillingCacheService) InvalidateAPIKeyRateLimit(ctx context.Context, keyID string) error {
	if s.cache == nil {
		return nil
	}
	if err := s.cache.InvalidateAPIKeyRateLimit(ctx, keyID); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: invalidate api key rate limit cache failed for key %v: %v", keyID, err)
		return err
	}
	return nil
}
		...
		go func() {
			...
			if s.apiKeyRateLimitLoader != nil {
				...
				if loader, ok := s.apiKeyRateLimitLoader.(interface {
					ResetRateLimitWindows(ctx context.Context, id string) error
				}); ok {
					if err := loader.ResetRateLimitWindows(resetCtx, keyID); err != nil {
						logger.LegacyPrintf("service.billing_cache", "Warning: reset rate limit windows failed for api key %v: %v", keyID, err)
					}
				}
			}
			...
			if s.cache != nil {
				if err := s.cache.InvalidateAPIKeyRateLimit(resetCtx, keyID); err != nil {
					logger.LegacyPrintf("service.billing_cache", "Warning: invalidate rate limit cache failed for api key %v: %v", keyID, err)
				}
			}
		}()
	}
...
func (s *BillingCacheService) QueueUpdateAPIKeyRateLimitUsage(apiKeyID string, cost float64) {
	if s.cache == nil {
		return
	}
...
func (s *BillingCacheService) checkRPM(ctx context.Context, user *User, group *Group) error {
		...
		if user.UserGroupRPMOverride != nil {
			...
			if err != nil {
				logger.LegacyPrintf(
					...
					"Warning: rpm override lookup failed for user=%v group=%v: %v",
		...
		if override != nil {
			...
			if *override > 0 {
				...
				if incErr != nil {
					logger.LegacyPrintf(
						"service.billing_cache",
						"Warning: rpm increment (override) failed for user=%v group=%v: %v",
			...
			if err != nil {
				logger.LegacyPrintf(
					"service.billing_cache",
					"Warning: rpm increment (group) failed for user=%v group=%v: %v",
					user.ID, group.ID, err,
				)
				// fail-open
	...
	if user.RPMLimit > 0 {
		...
		if err != nil {
			logger.LegacyPrintf(
				"service.billing_cache",
				"Warning: rpm increment (user) failed for user=%v: %v",
				user.ID, err,
			)
...
func (s *BillingCacheService) checkBalanceEligibility(ctx context.Context, userID string) error {
	balance, err := s.GetUserBalance(ctx, userID)
	if err != nil {
		if s.circuitBreaker != nil {
			s.circuitBreaker.OnFailure(err)
		}
		logger.LegacyPrintf("service.billing_cache", "ALERT: billing balance check failed for user %v: %v", userID, err)
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID string, group *Group, subscription *UserSubscription) error {
	// 获取订阅缓存数据
	...
	if err != nil {
		...
		logger.LegacyPrintf("service.billing_cache", "ALERT: billing subscription check failed for user %v group %v: %v", userID, group.ID, err)
		return ErrBillingServiceUnavailable.WithCause(err)
	}
	if s.circuitBreaker != nil {
		s.circuitBreaker.OnSuccess()
...
func (b *billingCircuitBreaker) OnFailure(err error) {
	...
	default:
		...
		if b.failures >= b.failureThreshold {
			...
			b.openedAt = time.Now()
			b.halfOpenRemaining = 0
			logger.LegacyPrintf("service.billing_cache", "ALERT: billing circuit breaker opened after %v failures: %v", b.failures, err)
		}
	}
}
...
func (b *billingCircuitBreaker) OnSuccess() {
	...
	if previousState != billingCircuitClosed {
		...
		logger.LegacyPrintf("service.billing_cache", "INFO: billing circuit breaker failures reset from %v", previousFailures)
	}
}
...
// checkUserPlatformQuotaEligibility 检查用户在指定平台的积分配额（存储列沿用 *_usd）。
//
...
func (s *BillingCacheService) checkUserPlatformQuotaEligibility(
	...
	if cacheErr == nil && ok && entry != nil && entry.SchemaVersion == UserPlatformQuotaCacheSchemaV1 {
		...
		if windowExpired && s.cache != nil && !isSentinel {
			...
			if setErr := s.cache.SetUserPlatformQuotaCache(setCtx, userID, platform, refreshed, ttl); setErr != nil {
				logger.LegacyPrintf("service.billing_cache",
					"Warning: refresh expired user platform quota cache failed user=%v platform=%s: %v",
	...
	case <-ctx.Done():
		...
		logger.LegacyPrintf("service.billing_cache", "Warning: user platform quota check ctx cancelled user=%v platform=%s: %v (fail-open)", userID, platform, ctx.Err())
		return nil
	}
	if dbErr != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: load user platform quota failed user=%v platform=%s: %v (fail-open)", userID, platform, dbErr)
		return nil
	}
	...
	if rec == nil {
		...
		if s.cache != nil && cacheErr == nil {
			...
			if setErr := s.cache.SetUserPlatformQuotaCache(setCtx, userID, platform, sentinel, sentinelTTL); setErr != nil {
				...
				logger.LegacyPrintf("service.billing_cache", "Warning: set sentinel quota cache failed user=%v platform=%s: %v", userID, platform, setErr)
			}
			setCancel()
		}
		return nil
	...
	if s.cache != nil {
		...
		if setErr := s.cache.SetUserPlatformQuotaCache(setCtx, userID, platform, newEntry, ttl); setErr != nil {
			logger.LegacyPrintf("service.billing_cache", "Warning: set user platform quota cache failed user=%v platform=%s: %v", userID, platform, setErr)
		}
		setCancel()
	}
...
func (s *BillingCacheService) HasUserPlatformQuotaLimit(ctx context.Context, userID string, platform string) bool {
	if s.cfg.RunMode == config.RunModeSimple {
		return false
	}
	if s.cache == nil {
