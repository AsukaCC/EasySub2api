...
func ProvideOAuthRefreshAPI(accountRepo AccountRepository, tokenCache OAuthTokenCache) *OAuthRefreshAPI {
...
func ProvideGeminiTokenProvider(
	...
	provider := NewGeminiTokenProvider(accountRepo, tokenCache, geminiOAuthService)
	provider.SetRefreshAPI(refreshAPI, NewGeminiTokenRefresher(geminiOAuthService))
...
func ProvideAntigravityTokenProvider(
	...
	provider := NewAntigravityTokenProvider(accountRepo, tokenCache, antigravityOAuthService)
	provider.SetRefreshAPI(refreshAPI, NewAntigravityTokenRefresher(antigravityOAuthService))
...
func ProvideSubscriptionExpiryService(userSubRepo UserSubscriptionRepository, subscriptionService *SubscriptionService, settingRepo SettingRepository, notificationEmailService *NotificationEmailService, lockCache LeaderLockCache, db *sql.DB) *SubscriptionExpiryService {
	svc := NewSubscriptionExpiryService(userSubRepo, time.Minute)
	svc.SetSubscriptionService(subscriptionService)
...
func ProvideRateLimitService(
	...
	svc.SetGeminiQuotaDependencies(usageRepo, geminiQuotaService)
...
var ProviderSet = wire.NewSet(
	...
	NewUsageService,
	NewDashboardService,
	ProvideAccountQuotaDashboardService,
...
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, billingCacheService *BillingCacheService, authCacheInvalidator APIKeyAuthCacheInvalidator) *PaymentService {
	svc := NewPaymentService(entClient, registry, loadBalancer, redeemService, subscriptionSvc, configService, userRepo, groupRepo, affiliateService)
	svc.SetNotificationEmailService(notificationEmailService)
	svc.SetWalletCacheInvalidators(billingCacheService, authCacheInvalidator)
	return svc
}
...
// ProvideChannelMonitorRunner keeps the legacy runner in the Wire graph so
// shutdown wiring remains stable, but active probes are retired and the runner
// is intentionally never started or attached as a scheduler.
