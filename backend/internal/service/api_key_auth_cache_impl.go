...
const apiKeyAuthSnapshotVersion = 21 // v21: group level and dynamic rate fields (force refresh of pre-fix snapshots)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		GroupIDs:    append([]string(nil), apiKey.GroupIDs...),
		Name:        apiKey.Name,
	...
	if apiKey.GroupID != nil && *apiKey.GroupID != "" && s.userGroupRateRepo != nil {
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			LevelRateMultipliers:            cloneGroupLevelRateMultipliers(apiKey.Group.LevelRateMultipliers),
			DynamicRateRules:                cloneGroupDynamicRateRules(apiKey.Group.DynamicRateRules),
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		GroupIDs:    append([]string(nil), snapshot.GroupIDs...),
		Key:         key,
		Name:        snapshot.Name,
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			LevelRateMultipliers:            cloneGroupLevelRateMultipliers(snapshot.Group.LevelRateMultipliers),
			DynamicRateRules:                cloneGroupDynamicRateRules(snapshot.Group.DynamicRateRules),
