...
// 默认采用官方 codex_cli_rs 身份，版本段随当前自动同步版本一起更新。
...
// 未填面板 UA 时按当前生效的客户端版本号拼出标准 Codex CLI UA。
//
...
func ValidateCodexWhitelistEntriesJSON(raw string) error {
	...
	for i, e := range entries {
		if !e.IsWhitelistable() {
			return fmt.Errorf("entry %v: whitelist requires a non-empty originator and at least one non-empty ua_contains (double-factor AND; otherwise the rule never matches)", i)
		}
	}
	return nil
}
...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	...
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		if err != nil {
			...
			return gatewayForwardingSettingsResult{openAITTFTMode: OpenAITTFTModeVisible, fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: true}, nil
		}
	...
	return gatewayForwardingSettingsResult{openAITTFTMode: OpenAITTFTModeVisible, fp: true, claudeOAuthSystemPromptInjection: true, clientDatelineNormalization: true}
}

// GetOpenAITTFTMode returns the configured Responses first_token_ms boundary.
