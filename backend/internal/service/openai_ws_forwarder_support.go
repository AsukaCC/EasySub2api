...
func (s *OpenAIGatewayService) performOpenAIWSGeneratePrewarm(
	...
	if decision.Transport != OpenAIUpstreamTransportResponsesWebsocketV2 {
		logOpenAIWSModeInfo(
			"prewarm_skip account_id=%v conn_id=%s reason=transport_not_v2 transport=%s",
			account.ID,
			connID,
	...
	if strings.TrimSpace(previousResponseID) != "" {
		logOpenAIWSModeInfo(
			"prewarm_skip account_id=%v conn_id=%s reason=has_previous_response_id previous_response_id=%s",
			account.ID,
			connID,
	...
	if lease.IsPrewarmed() {
		logOpenAIWSModeInfo("prewarm_skip account_id=%v conn_id=%s reason=already_prewarmed", account.ID, connID)
		return nil
	}
	if NeedsToolContinuation(reqBody) {
		logOpenAIWSModeInfo("prewarm_skip account_id=%v conn_id=%s reason=tool_continuation", account.ID, connID)
		return nil
	}
	prewarmStart := time.Now()
	logOpenAIWSModeInfo("prewarm_start account_id=%v conn_id=%s", account.ID, connID)
	...
	if err := lease.WriteJSONWithContextTimeout(ctx, prewarmPayload, s.openAIWSWriteTimeout()); err != nil {
		lease.MarkBroken()
		logOpenAIWSModeInfo(
			"prewarm_write_fail account_id=%v conn_id=%s cause=%s",
			account.ID,
			connID,
	...
	logOpenAIWSModeInfo("prewarm_write_sent account_id=%v conn_id=%s payload_bytes=%v", account.ID, connID, len(prewarmPayloadJSON))
	...
	for {
		...
		if readErr != nil {
			...
			logOpenAIWSModeInfo(
				"prewarm_read_fail account_id=%v conn_id=%s close_status=%s close_reason=%s cause=%s events=%v",
				account.ID,
				connID,
				closeStatus,
				closeReason,
		...
		if prewarmEventCount <= openAIWSPrewarmEventLogHead || eventType == "error" || isOpenAIWSTerminalEvent(eventType) {
			logOpenAIWSModeInfo(
				"prewarm_event account_id=%v conn_id=%s idx=%v type=%s bytes=%v",
				account.ID,
				connID,
				prewarmEventCount,
		...
		if eventType == "error" {
			...
			logOpenAIWSModeInfo(
				"prewarm_error_event account_id=%v conn_id=%s idx=%v fallback_reason=%s can_fallback=%v err_code=%s err_type=%s err_message=%s",
				account.ID,
				connID,
				prewarmEventCount,
	...
	logOpenAIWSModeInfo(
		"prewarm_done account_id=%v conn_id=%s response_id=%s events=%v terminal_events=%v duration_ms=%v",
		account.ID,
		connID,
...
func getOpenAIGroupIDFromContext(c *gin.Context) string {
	if c == nil {
		return ""
	}
...
func (s *OpenAIGatewayService) selectAccountByPreviousResponseIDForCapability(
	...
	if accountID == "" || account == nil || store == nil {
		return nil, nil
	}
...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	requiredCapability OpenAIEndpointCapability,
	requireCompact bool,
) (string, *Account, string, OpenAIWSStateStore) {
	if s == nil {
		return "", nil, "", nil
	}
...
func (s *OpenAIGatewayService) isOpenAIWSFallbackCooling(accountID string) bool {
	if s == nil || accountID == "" {
		return false
	}
...
func (s *OpenAIGatewayService) markOpenAIWSFallbackCooling(accountID string, _ string) {
	if s == nil || accountID == "" {
		return
	}
	cooldown := s.openAIWSFallbackCooldown()
...
func (s *OpenAIGatewayService) clearOpenAIWSFallbackCooling(accountID string) {
	if s == nil || accountID == "" {
		return
	}
	s.openaiWSFallbackUntil.Delete(accountID)
