...
func ValidateOpenCodeAccount(account *Account) error {
	if !account.IsOpenCodeGo() {
		return nil
	}
	invalid := func(message string) error {
		return infraerrors.New(http.StatusBadRequest, "INVALID_OPENCODE_CREDENTIALS", message)
	}
	if account.Type != AccountTypeAPIKey || strings.TrimSpace(account.GetCredential("api_key")) == "" {
		return invalid("OpenCode requires an API key account with a non-empty api_key")
	}
	if raw, present := account.Credentials["account_mode"]; present && raw != AccountModeZen && raw != AccountModeGo {
		return invalid("account_mode must be zen or go")
	}
	if raw, present := account.Credentials["api_protocol"]; present && raw != APIProtocolAdaptive && raw != APIProtocolChatCompletions && raw != APIProtocolResponses && raw != APIProtocolAnthropic {
		return invalid("api_protocol must be adaptive, chat_completions, responses or anthropic")
	}
	if raw, present := account.Credentials[openCodeGoProtocolRulesKey]; present && raw != nil {
		if _, err := parseOpenCodeGoProtocolRules(raw); err != nil {
			return invalid(err.Error())
		}
	}
	if raw, present := account.Credentials["api_base_urls"]; present && raw != nil {
		urls, ok := raw.(map[string]any)
		if !ok {
			return invalid("api_base_urls must be an object")
		}
		for protocol, value := range urls {
			if !isNativeOpenCodeGoProtocol(protocol) {
				return invalid("unknown protocol in api_base_urls")
			}
			if _, ok := value.(string); !ok {
				return invalid("api_base_urls values must be strings")
			}
		}
	}
	return nil
...
func (a *Account) IsMultiProtocolAPIKey() bool {
	return a != nil && a.Type == AccountTypeAPIKey && IsMultiProtocolAPIKeyProvider(a.Platform)
}

// Each protocol resolves its own endpoint without mutating the shared account.
func (a *Account) openCodeProtocolBaseURL(protocol string) string {
	...
	if base := strings.TrimRight(strings.TrimSpace(a.GetCredential("base_url")), "/"); base != "" {
