...
func convertOutputFormatToInlineSchema(body []byte) []byte {
	...
	basePath := fmt.Sprintf("messages.%v.content", lastUserIdx)

	if content.IsArray() {
		// 追加一个 text block 到 content 数组末尾
		idx := len(content.Array())
		body, _ = sjson.SetBytes(body, fmt.Sprintf("%s.%v.type", basePath, idx), "text")
		body, _ = sjson.SetBytes(body, fmt.Sprintf("%s.%v.text", basePath, idx), string(schemaJSON))
...
func removeCustomFieldFromTools(body []byte) []byte {
	...
	for i := range tools.Array() {
		body, err = sjson.DeleteBytes(body, fmt.Sprintf("tools.%v.custom", i))
		if err != nil {
			// 删除失败不影响整体流程，跳过
			continue
		}
...
func sanitizeBedrockCacheControl(body []byte, modelID string) []byte {
	...
	if systemArr.Exists() && systemArr.IsArray() {
		for i, item := range systemArr.Array() {
			...
			body = deleteCacheControlUnsupportedFields(body, fmt.Sprintf("system.%v.cache_control", i), cc, isClaude45)
		}
	}
	...
	for mi, msg := range messages.Array() {
		...
		for ci, block := range content.Array() {
			...
			body = deleteCacheControlUnsupportedFields(body, fmt.Sprintf("messages.%v.content.%v.cache_control", mi, ci), cc, isClaude45)
		}
	}
...
func sanitizeBedrockToolUseIDs(body []byte) []byte {
	...
	for mi, msg := range messages.Array() {
		...
		for ci, block := range content.Array() {
			...
			case "tool_use":
				body = sanitizeIDField(body, block.Get("id").String(), fmt.Sprintf("messages.%v.content.%v.id", mi, ci))
			case "tool_result":
				body = sanitizeIDField(body, block.Get("tool_use_id").String(), fmt.Sprintf("messages.%v.content.%v.tool_use_id", mi, ci))
			}
		}
	}
	return body
