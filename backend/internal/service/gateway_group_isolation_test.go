...
func (m *groupAwareMockAccountRepo) ListSchedulableByGroupIDAndPlatform(ctx context.Context, groupID string, platform string) ([]Account, error) {
	var result []Account
	for _, acc := range m.allAccounts {
		if acc.Platform == platform && acc.IsSchedulable() && testAccountBelongsToGroup(acc, groupID) {
			result = append(result, acc)
		}
	}
	return result, nil
...
func (m *groupAwareMockAccountRepo) ListSchedulableByGroupIDAndPlatforms(ctx context.Context, groupID string, platforms []string) ([]Account, error) {
	platformSet := make(map[string]bool, len(platforms))
	for _, p := range platforms {
		platformSet[p] = true
	}
	var result []Account
	for _, acc := range m.allAccounts {
		if platformSet[acc.Platform] && acc.IsSchedulable() && testAccountBelongsToGroup(acc, groupID) {
			result = append(result, acc)
		}
	}
	return result, nil
...
func testAccountBelongsToGroup(acc Account, groupID string) bool {
...
func TestGroupIsolation_UngroupedKey_ShouldOnlyScheduleUngroupedAccounts(t *testing.T) {
	...
	require.Equal(t, "2", acc.ID, "应选中未分组的账号 ID=2")
}
...
func TestGroupIsolation_GroupedKey_ShouldOnlyScheduleMatchingGroupAccounts(t *testing.T) {
	...
	require.NotNil(t, acc)
	require.Equal(t, "3", acc.ID, "应选中分组 100 内的账号 ID=3")
}
...
func TestGroupIsolation_SimpleMode_SkipsGroupIsolation(t *testing.T) {
	...
	require.Equal(t, "2", acc.ID, "SimpleMode 应按优先级选择，不考虑分组")
}
...
func TestGroupIsolation_SimpleMode_GroupedAccountAlsoSchedulable(t *testing.T) {
	...
	require.NotNil(t, acc)
	require.Equal(t, "1", acc.ID, "SimpleMode 应能调度已分组账号")
}
