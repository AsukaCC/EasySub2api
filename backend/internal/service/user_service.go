...
type UserListFilters struct {
	...
	Attributes    map[string]string // Custom attribute filters: attributeID -> value
...
type UserRepository interface {
	...
	UpsertUserAvatar(ctx context.Context, userID string, input UpsertUserAvatarInput) (*UserAvatar, error)
	...
	GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []string) (map[string]*time.Time, error)
	GetLatestUsedAtByUserID(ctx context.Context, userID string) (*time.Time, error)
	UpdateUserLastActiveAt(ctx context.Context, userID string, activeAt time.Time) error

	UpdateBalance(ctx context.Context, id string, amount float64) error
	DeductBalance(ctx context.Context, id string, amount float64) error
	// AdjustBalance 原子地把 delta 累加到余额上，并返回变更前后的值。结果为负时
	// 拒绝写入并返回 ErrBalanceNegative。管理员的加/扣款必须走这里而不是
	// "读余额→算新值→整行写回"，否则并发的计费扣款会被旧快照抹掉。
	AdjustBalance(ctx context.Context, id string, delta float64) (BalanceChange, error)
	// SetBalance 原子地把余额置为 value（value 必须 >= 0），返回变更前后的值。
	SetBalance(ctx context.Context, id string, value float64) (BalanceChange, error)
	UpdateConcurrency(ctx context.Context, id string, amount int) error
	BatchSetConcurrency(ctx context.Context, userIDs []string, value int) (int, error)
	BatchAddConcurrency(ctx context.Context, userIDs []string, delta int) (int, error)
	...
	RemoveGroupFromAllowedGroups(ctx context.Context, groupID string) (int64, error)
	// AddGroupToAllowedGroups 将指定分组增量添加到用户的 allowed_groups（幂等，冲突忽略）
	AddGroupToAllowedGroups(ctx context.Context, userID string, groupID string) error
	// RemoveGroupFromUserAllowedGroups 移除单个用户的指定分组权限
	RemoveGroupFromUserAllowedGroups(ctx context.Context, userID string, groupID string) error
	ListUserAuthIdentities(ctx context.Context, userID string) ([]UserAuthIdentityRecord, error)
	UnbindUserAuthProvider(ctx context.Context, userID string, provider string) error
...
// UserDeletionRepository exposes physical deletion and restoration of legacy
// archived users without expanding UserRepository and unrelated test doubles.
type UserDeletionRepository interface {
	PermanentlyDeleteUser(ctx context.Context, userID string) (*UserPurgeResult, error)
...
type RedeemUserAdjustmentRepository interface {
	ApplyRedeemBalanceAdjustment(ctx context.Context, id string, delta float64) error
	ApplyRedeemConcurrencyAdjustment(ctx context.Context, id string, delta int) error
}
...
type UserIdentitySummarySet struct {
	...
	LinuxDo  UserIdentitySummary `json:"-"` // Historical records remain readable but are never exposed or bindable.
...
func (s *UserService) GetProfileIdentitySummaries(ctx context.Context, userID string, user *User) (UserIdentitySummarySet, error) {
	if user == nil {
		var err error
		user, err = s.userRepo.GetByID(ctx, userID)
		if err != nil {
...
func (s *UserService) UnbindUserAuthProvider(ctx context.Context, userID string, provider string) (*User, error) {
	user, _, err := s.UnbindUserAuthProviderWithResult(ctx, userID, provider)
	return user, err
}

func (s *UserService) UnbindUserAuthProviderWithResult(ctx context.Context, userID string, provider string) (*User, bool, error) {
...
func (s *UserService) UpdateProfile(ctx context.Context, userID string, req UpdateProfileRequest) (*User, error) {
...
func (s *UserService) updateProfile(ctx context.Context, userID string, req UpdateProfileRequest) (*User, int, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
...
func (s *UserService) SetAvatar(ctx context.Context, userID string, raw string) (*UserAvatar, error) {
...
func (s *UserService) canUnbindProvider(provider string, user *User, records []UserAuthIdentityRecord) bool {
	...
	for _, candidate := range []string{"oidc", "wechat", "dingtalk"} {
...
func (s *UserService) listUserAuthIdentities(ctx context.Context, userID string) ([]UserAuthIdentityRecord, error) {
...
// 该操作为尽力而为，不应中断正常请求。
func (s *UserService) TouchLastActive(ctx context.Context, userID string) {
...
func (s *UserService) TouchLastActiveForUser(ctx context.Context, user *User) {
	if s == nil || s.userRepo == nil || user == nil || user.ID == "" {
		return
	}
	...
	_, err, _ := s.lastActiveTouchSF.Do(user.ID, func() (any, error) {
		latest := time.Now()
		if v, ok := s.lastActiveTouchL1.Load(user.ID); ok {
...
func (s *UserService) UpdateConcurrency(ctx context.Context, userID string, concurrency int) error {
...
func (s *UserService) SendNotifyEmailCode(ctx context.Context, userID string, email string, emailService *EmailService, cache EmailCache, locale ...string) error {
...
func checkNotifyCodeRateLimit(ctx context.Context, cache EmailCache, userID string, email string) error {
...
func (s *UserService) sendNotifyVerifyEmail(ctx context.Context, emailService *EmailService, userID string, email, code, locale string) error {
...
func (s *UserService) VerifyAndAddNotifyEmail(ctx context.Context, userID string, email, code string, cache EmailCache) error {
...
func (s *UserService) addOrVerifyNotifyEmail(ctx context.Context, userID string, email string) error {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return err
	}
	...
	if len(user.BalanceNotifyExtraEmails) >= maxNotifyEmails {
		return infraerrors.BadRequest("TOO_MANY_NOTIFY_EMAILS", fmt.Sprintf("maximum %v notification emails allowed", maxNotifyEmails))
	}
...
func (s *UserService) RemoveNotifyEmail(ctx context.Context, userID string, email string) error {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return err
	}
...
func (s *UserService) ToggleNotifyEmail(ctx context.Context, userID string, email string, disabled bool) error {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return err
	}
