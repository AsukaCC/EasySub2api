...
type teamLinkedAccountRepoStub struct {
	...
	listCalls    int
	setErrorIDs  []string
	setErrorMsgs map[string]string
	failSetError map[string]error
}
...
func (r *teamLinkedAccountRepoStub) SetError(ctx context.Context, id string, errorMsg string) error {
	if err, ok := r.failSetError[id]; ok {
		return err
	}
	r.setErrorIDs = append(r.setErrorIDs, id)
	if r.setErrorMsgs == nil {
		r.setErrorMsgs = make(map[string]string)
	}
	r.setErrorMsgs[id] = errorMsg
	return nil
}

func newTeamLinkedAccount(id string, teamID string) Account {
	return Account{
		ID:          id,
...
func newTeamLinkedTestService(repo *teamLinkedAccountRepoStub) (*RateLimitService, *runtimeBlockRecorder) {
	rl := NewRateLimitService(repo, &config.Config{}, nil)
...
func TestTeamLinkedError_FanoutMarksSameTeamAccounts(t *testing.T) {
	...
	require.Equal(t, []string{"2", "6", "1"}, repo.setErrorIDs)
	...
	require.Equal(t, "2", blocker.accounts[0].ID)
	require.Equal(t, "6", blocker.accounts[1].ID)
	require.Equal(t, "1", blocker.accounts[2].ID)
}
...
func TestTeamLinkedError_GenericPaymentErrorDoesNotFanout(t *testing.T) {
	...
	require.Equal(t, []string{"1"}, repo.setErrorIDs)
...
func TestTeamLinkedError_DedupWithinTTL(t *testing.T) {
	...
	require.Equal(t, []string{"2", "6", "1", "2"}, repo.setErrorIDs)
...
func TestTeamLinkedError_APIKeyTriggerDoesNotFanout(t *testing.T) {
	...
	require.Equal(t, []string{"4"}, repo.setErrorIDs)
	require.Zero(t, repo.listCalls)
}
...
func TestTeamLinkedError_DirectCallSkipsTriggerAccount(t *testing.T) {
	...
	require.Equal(t, []string{"2", "6"}, repo.setErrorIDs)
...
func TestTeamLinkedError_SetErrorFailureDoesNotAbortRemaining(t *testing.T) {
	repo := &teamLinkedAccountRepoStub{
		...
		failSetError: map[string]error{"2": errors.New("db down")},
	}
	rl, blocker := newTeamLinkedTestService(repo)
	trigger := newTeamLinkedAccount("1", "team-A")
	...
	require.Equal(t, []string{"6"}, repo.setErrorIDs)
