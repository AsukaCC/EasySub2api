...
func QuantizeRateMultiplier(v float64) float64 {
	if v == 0 || math.IsNaN(v) || math.IsInf(v, 0) {
		return v
	}
	quantized, _ := decimal.NewFromFloat(v).Round(4).Float64()
	return quantized
}
...
func buildUsageBillingFingerprint(c *UsageBillingCommand) string {
	...
	raw := fmt.Sprintf(
		"%v|%v|%v|%s|%s|%s|%s|%v|%v|%v|%v|%v|%v|%s|%v|%0.10f|%0.10f|%0.10f|%0.10f|%0.10f",
		c.UserID,
		c.AccountID,
		c.APIKeyID,
		...
		usageBillingIDOrEmpty(c.SubscriptionID),
		c.BalanceCost,
		c.SubscriptionCost,
		c.APIKeyQuotaCost,
	...
	if c.DynamicRatePlan != nil {
		if encoded, err := json.Marshal(c.DynamicRatePlan); err == nil {
			raw += "|dynamic:" + string(encoded)
		}
	}
...
func usageBillingIDOrEmpty(v *string) string {
	if v == nil {
		return ""
	}
	return *v
