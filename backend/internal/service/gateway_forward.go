...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if c != nil {
		s.debugLogGatewaySnapshot("CLIENT_ORIGINAL", c.Request.Header, body, map[string]string{
			"account":      fmt.Sprintf("%v(%s)", account.ID, account.Name),
	...
	if ResolveThinkingProtocol(reqModel) == ThinkingProtocolPassbackRequired {
		if rewritten, applied := NormalizeChineseLLMThinking(body, reqModel); applied {
			...
			logger.LegacyPrintf("service.gateway", "Account %v: rewrote thinking.type for %s (Anthropic-SDK default 'enabled' -> vendor-specific)", account.ID, reqModel)
		}
	}
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					logger.LegacyPrintf("service.gateway", "[warn] Account %v: thinking blocks have invalid signature, retrying with filtered blocks", account.ID)
					...
					if buildErr == nil {
						...
						if retryErr == nil {
							if retryResp.StatusCode < 400 {
								...
								logger.LegacyPrintf("service.gateway", "Account %v: thinking block retry succeeded (blocks downgraded)", account.ID)
								resp = retryResp
								break
							}
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									logger.LegacyPrintf("service.gateway", "Account %v: signature retry still failing and looks tool-related, retrying with tool blocks downgraded", account.ID)
									...
									if buildErr2 == nil {
										...
										logger.LegacyPrintf("service.gateway", "Account %v: tool-downgrade signature retry failed: %v", account.ID, retryErr2)
									} else {
										logger.LegacyPrintf("service.gateway", "Account %v: tool-downgrade signature retry build failed: %v", account.ID, buildErr2)
									}
								}
							}
						...
						logger.LegacyPrintf("service.gateway", "Account %v: signature error retry failed: %v", account.ID, retryErr)
					} else {
						logger.LegacyPrintf("service.gateway", "Account %v: signature error retry build request failed: %v", account.ID, buildErr)
					}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						logger.LegacyPrintf("service.gateway", "Account %v: detected budget_tokens constraint error, retrying with rectified budget (budget_tokens=%v, max_tokens=%v)", account.ID, BudgetRectifyBudgetTokens, BudgetRectifyMaxTokens)
						...
						if buildErr == nil {
							...
							logger.LegacyPrintf("service.gateway", "Account %v: budget rectifier retry failed: %v", account.ID, retryErr)
						} else {
							logger.LegacyPrintf("service.gateway", "Account %v: budget rectifier retry build failed: %v", account.ID, buildErr)
						}
					}
				}
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				logger.LegacyPrintf("service.gateway", "Account %v: upstream error %v, retry %v/%v after %v (elapsed=%v/%v)",
	...
	if resp.StatusCode >= 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
		if s.shouldFailoverUpstreamError(resp.StatusCode) {
			...
			// 调试日志：打印重试耗尽后的错误响应
			logger.LegacyPrintf("service.gateway", "[Forward] Upstream error (retry exhausted, failover): Account=%v(%s) Status=%v RequestID=%s Body=%s",
	...
	if resp.StatusCode >= 400 && s.shouldFailoverUpstreamError(resp.StatusCode) {
		...
		// 调试日志：打印上游错误响应
		logger.LegacyPrintf("service.gateway", "[Forward] Upstream error (failover): Account=%v(%s) Status=%v RequestID=%s Body=%s",
			account.ID, account.Name, resp.StatusCode, resp.Header.Get("x-request-id"), truncateString(string(respBody), 1000))
			...
			if s.shouldFailoverOn400(respBody) {
				...
				if s.cfg.Gateway.LogUpstreamErrorBody {
					logger.LegacyPrintf("service.gateway",
						"Account %v: 400 error, attempting failover: %s",
						account.ID,
						truncateForLog(respBody, s.cfg.Gateway.LogUpstreamErrorBodyMaxBytes),
					)
				} else {
					logger.LegacyPrintf("service.gateway", "Account %v: 400 error, attempting failover", account.ID)
				}
				s.handleFailoverSideEffects(ctx, resp, account, reqModel)
	...
	if reqStream {
		...
		if err != nil {
			...
			if errors.As(err, &sseErr) {
				...
				logger.LegacyPrintf("service.gateway",
					"[Forward] SSE error event in stream: Account=%v(%s) RequestID=%s Body=%s",
...
func (s *GatewayService) ResolveChannelMapping(ctx context.Context, groupID string, model string) ChannelMappingResult {
...
func (s *GatewayService) IsModelRestricted(ctx context.Context, groupID string, model string) bool {
	if s.channelService == nil {
		return false
	}
...
func (s *GatewayService) ResolveChannelMappingAndRestrict(ctx context.Context, groupID *string, model string) (ChannelMappingResult, bool) {
	if s.channelService == nil {
...
func (s *GatewayService) checkChannelPricingRestriction(ctx context.Context, groupID *string, requestedModel string) bool {
...
func (s *GatewayService) isUpstreamModelRestrictedByChannel(ctx context.Context, groupID string, account *Account, requestedModel string) bool {
	if s.channelService == nil {
		return false
	}
...
func (s *GatewayService) needsUpstreamChannelRestrictionCheck(ctx context.Context, groupID *string) bool {
...
func (s *GatewayService) isStickyAccountUpstreamRestricted(ctx context.Context, groupID *string, account *Account, requestedModel string) bool {
	if groupID == nil {
		return false
	}
