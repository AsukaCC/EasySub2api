...
func stripMessageCacheControl(body []byte) []byte {
	...
	messages.ForEach(func(_, msg gjson.Result) bool {
		...
		content.ForEach(func(_, block gjson.Result) bool {
			...
			path := fmt.Sprintf("messages.%v.content.%v.cache_control", msgIdx, blockIdx)
...
func injectCacheControlOnLastContentBlock(body []byte, idx int, msg *gjson.Result) []byte {
	...
	if content.Type == gjson.String {
		...
		if next, err := sjson.SetRawBytes(body, fmt.Sprintf("messages.%v.content", idx), []byte(blockRaw)); err == nil {
			body = next
		}
		return body
	}
	...
	pathPrefix := fmt.Sprintf("messages.%v.content.%v.cache_control", idx, lastBlockIdx)
