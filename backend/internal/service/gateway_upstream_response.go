...
func (s *GatewayService) handleErrorResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, requestedModel ...string) (*ForwardResult, error) {
	...
	if readErr != nil {
		...
		// 避免静默吞掉导致误判。
		logger.LegacyPrintf("service.gateway", "[Forward] Failed to fully read upstream error body: Account=%v(%s) Status=%v err=%v",
			account.ID, account.Name, resp.StatusCode, readErr)
	}

	// 调试日志：打印上游错误响应
	logger.LegacyPrintf("service.gateway", "[Forward] Upstream error (non-retryable): Account=%v(%s) Status=%v RequestID=%s Body=%s",
	...
	if isClaudeCodeCredentialScopeError(upstreamMsg) && c != nil {
		if v, ok := c.Get(claudeMimicDebugInfoKey); ok {
			if line, ok := v.(string); ok && strings.TrimSpace(line) != "" {
				logger.LegacyPrintf("service.gateway", "[ClaudeMimicDebugOnError] status=%v request_id=%s %s",
					resp.StatusCode,
	...
	if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
		logger.LegacyPrintf("service.gateway",
			"Upstream error %v (account=%v platform=%s type=%s): %s",
			resp.StatusCode,
			account.ID,
			account.Platform,
			account.Type,
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		if summary == "" {
			return nil, fmt.Errorf("upstream error: %v (passthrough rule matched)", resp.StatusCode)
		}
		return nil, fmt.Errorf("upstream error: %v (passthrough rule matched) message=%s", resp.StatusCode, summary)
	}
	...
	case 400:
		...
		if summary == "" {
			return nil, fmt.Errorf("upstream error: %v", resp.StatusCode)
		}
		return nil, fmt.Errorf("upstream error: %v message=%s", resp.StatusCode, summary)
	case 401:
	...
	return nil, fmt.Errorf("upstream error: %v message=%s", resp.StatusCode, upstreamMsg)
}
...
func (s *GatewayService) handleRetryExhaustedSideEffects(ctx context.Context, resp *http.Response, account *Account) {
	...
	if account.IsOAuth() && statusCode == 403 {
		...
		logger.LegacyPrintf("service.gateway", "Account %v: marked as error after %v retries for status %v", account.ID, maxRetryAttempts, statusCode)
	} else {
		// API Key 未配置错误码：不标记账号状态
		logger.LegacyPrintf("service.gateway", "Account %v: upstream error %v after %v retries (not marking account)", account.ID, statusCode, maxRetryAttempts)
	}
}
...
func (s *GatewayService) handleRetryExhaustedError(ctx context.Context, resp *http.Response, c *gin.Context, account *Account) (*ForwardResult, error) {
	...
	if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
		logger.LegacyPrintf("service.gateway",
			"Upstream error %v retries_exhausted (account=%v platform=%s type=%s): %s",
			resp.StatusCode,
			account.ID,
			account.Platform,
			account.Type,
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		if summary == "" {
			return nil, fmt.Errorf("upstream error: %v (retries exhausted, passthrough rule matched)", resp.StatusCode)
		}
		return nil, fmt.Errorf("upstream error: %v (retries exhausted, passthrough rule matched) message=%s", resp.StatusCode, summary)
	}
	...
	if upstreamMsg == "" {
		return nil, fmt.Errorf("upstream error: %v (retries exhausted)", resp.StatusCode)
	}
	return nil, fmt.Errorf("upstream error: %v (retries exhausted) message=%s", resp.StatusCode, upstreamMsg)
}
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				if errors.Is(ev.err, bufio.ErrTooLong) {
					logger.LegacyPrintf("service.gateway", "SSE line too long: account=%v max_size=%v error=%v", account.ID, maxLineSize, ev.err)
					sendErrorEvent("response_too_large", fmt.Sprintf("upstream SSE line exceeded %v bytes", maxLineSize))
				...
				if !c.Writer.Written() {
					logger.LegacyPrintf("service.gateway", "Upstream stream read error before any client output (account=%v), failing over: %v", account.ID, ev.err)
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.gateway", "Stream data interval timeout: account=%v model=%s interval=%s", account.ID, originalModel, streamInterval)
