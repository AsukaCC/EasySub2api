...
type APIKeyQuotaUpdater interface {
	UpdateQuotaUsed(ctx context.Context, apiKeyID string, cost float64) error
	UpdateRateLimitUsage(ctx context.Context, apiKeyID string, cost float64) error
}
		...
		if deps.billingCacheService.HasUserPlatformQuotaLimit(billingCtx, p.User.ID, p.Platform) {
			...
			if deps.cfg == nil || !deps.cfg.Database.UserPlatformQuotaFlusherEnabled {
				...
				if err := deps.userPlatformQuotaRepo.IncrementUsageWithReset(billingCtx, p.User.ID, p.Platform, cost.ActualCost, time.Now().UTC()); err != nil {
					...
					logger.LegacyPrintf("service.gateway", "ALERT: legacy incr user platform quota DB failed user=%v platform=%s cost=%f: %v", p.User.ID, p.Platform, cost.ActualCost, err)
				}
			}
...
func simpleModeKeyRateLimitBillingEnabled(cfg *config.Config, key *APIKey) bool {
	return cfg != nil && cfg.RunMode == config.RunModeSimple && cfg.SimpleModeKeyRateLimitEnabled && key != nil && key.HasRateLimits()
}
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	// An empty billing mode is the legacy representation of token billing
...
func applyUsageBilling(ctx context.Context, requestID string, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps, repo UsageBillingRepository) (bool, error) {
	...
	if result.FinalActualCost != nil {
		p.Cost.ActualCost = *result.FinalActualCost
		if usageLog != nil {
			usageLog.ActualCost = *result.FinalActualCost
			if result.FinalRateMultiplier != nil {
				usageLog.RateMultiplier = *result.FinalRateMultiplier
			}
		}
	}
	...
	if deps.userLevelService != nil && p.User != nil && p.Cost.ActualCost > 0 {
		deps.userLevelService.RecordSpend(p.User.ID, p.Cost.ActualCost)
	}
	return true, nil
}
	...
	if p.SimpleModeKeyRateLimitOnly {
		...
		if p.APIKey != nil && deps.billingCacheService != nil {
			if err := deps.billingCacheService.InvalidateAPIKeyRateLimit(ctx, p.APIKey.ID); err != nil {
				logger.LegacyPrintf("service.gateway", "Warning: invalidate simple-mode api key rate-limit cache failed for key %v: %v", p.APIKey.ID, err)
			}
		}
		deps.deferredService.ScheduleLastUsedUpdate(p.Account.ID)
		return
		...
		if deps.billingCacheService.HasUserPlatformQuotaLimit(ctx, p.User.ID, p.Platform) {
			...
			if deps.cfg == nil || !deps.cfg.Database.UserPlatformQuotaFlusherEnabled {
				...
				go func() {
					defer func() {
						if r := recover(); r != nil {
							logger.LegacyPrintf("service.gateway", "ALERT: panic in user platform quota incr goroutine user=%v platform=%s: %v", userID, platform, r)
						}
					}()
					defer dbCancel()
					if err := deps.userPlatformQuotaRepo.IncrementUsageWithReset(dbCtx, userID, platform, cost, time.Now().UTC()); err != nil {
						...
						logger.LegacyPrintf("service.gateway", "ALERT: incr user platform quota DB failed user=%v platform=%s cost=%f: %v", userID, platform, cost, err)
					}
				}()
			}
			// flusher_enabled=true:不直写 DB,flusher 异步批量刷
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	account := input.Account
	subscription := input.Subscription
	var ratePlan *UserRatePlan
	apiKey, subscription, ratePlan = applySelectionBillingSnapshot(apiKey, subscription, input.Selection)
	ApplyForwardImageBillingResolution(result)

	// 强制缓存计费：将 input_tokens 转为 cache_read_input_tokens
	// 用于粘性会话切换时的特殊计费处理
	if input.ForceCacheBilling && result.Usage.InputTokens > 0 {
		logger.LegacyPrintf("service.gateway", "force_cache_billing: %v input_tokens → cache_read_input_tokens (account=%v)",
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple && !simpleModeKeyRateLimitOnly {
		...
		logger.LegacyPrintf("service.gateway", "[SIMPLE MODE] Usage recorded (not billed): user=%v, tokens=%v", usageLog.UserID, usageLog.TotalTokens())
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.SearchCount > 0 {
		...
		if price != nil && *price == 0 {
			logger.LegacyPrintf("service.gateway", "[Billing] search_price_per_1k explicit 0; search free group_model=%s count=%v", billingModel, result.SearchCount)
		}
...
// calculateTokenCost 计算 Token 费用。
...
func optionalSubscriptionID(subscription *UserSubscription) *string {
	if subscription != nil {
