...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	// Apply explicit routing for every account type. Empty routing preserves the
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
		logger.LegacyPrintf("service.gateway", "Account %v: detected thinking block signature error on count_tokens, retrying with filtered thinking blocks", account.ID)
	...
	if resp.StatusCode >= 400 {
		...
		if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
			logger.LegacyPrintf("service.gateway",
				"count_tokens upstream error %v (account=%v platform=%s type=%s): %s",
				resp.StatusCode,
				account.ID,
				account.Platform,
				account.Type,
		...
		if upstreamMsg == "" {
			return fmt.Errorf("upstream error: %v", resp.StatusCode)
		}
		return fmt.Errorf("upstream error: %v message=%s", resp.StatusCode, upstreamMsg)
	}
...
func (s *GatewayService) forwardCountTokensAnthropicAPIKeyPassthrough(ctx context.Context, c *gin.Context, account *Account, body []byte) error {
	...
	if resp.StatusCode >= 400 {
		...
		if isCountTokensUnsupported404(resp.StatusCode, respBody) {
			logger.LegacyPrintf("service.gateway",
				"[count_tokens] Upstream does not support count_tokens (404), returning 404: account=%v name=%s msg=%s",
...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	if account.Type == AccountTypeAPIKey {
		...
		if customURL == "" {
			return nil, nil, fmt.Errorf("custom_base_url is enabled but not configured for account %v", account.ID)
		}
