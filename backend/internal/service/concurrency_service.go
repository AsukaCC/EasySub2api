...
type ConcurrencyCache interface {
	...
	AcquireAccountSlot(ctx context.Context, accountID string, maxConcurrency int, requestID string) (bool, error)
	ReleaseAccountSlot(ctx context.Context, accountID string, requestID string) error
	GetAccountConcurrency(ctx context.Context, accountID string) (int, error)
	GetAccountConcurrencyBatch(ctx context.Context, accountIDs []string) (map[string]int, error)

	// 账号等待队列（账号级）
	IncrementAccountWaitCount(ctx context.Context, accountID string, maxWait int) (bool, error)
	DecrementAccountWaitCount(ctx context.Context, accountID string) error
	GetAccountWaitingCount(ctx context.Context, accountID string) (int, error)

	// 用户槽位管理
	// 键格式: concurrency:user:{userID}（有序集合，成员为 requestID）
	AcquireUserSlot(ctx context.Context, userID string, maxConcurrency int, requestID string) (bool, error)
	ReleaseUserSlot(ctx context.Context, userID string, requestID string) error
	GetUserConcurrency(ctx context.Context, userID string) (int, error)

	// 等待队列计数（每次入队都会刷新 TTL，避免长时间排队时计数提前过期）
	IncrementWaitCount(ctx context.Context, userID string, maxWait int) (bool, error)
	DecrementWaitCount(ctx context.Context, userID string) error

	// 批量负载查询（只读）
	GetAccountsLoadBatch(ctx context.Context, accounts []AccountWithConcurrency) (map[string]*AccountLoadInfo, error)
	GetUsersLoadBatch(ctx context.Context, users []UserWithConcurrency) (map[string]*UserLoadInfo, error)

	// 清理过期槽位（后台任务）
	CleanupExpiredAccountSlots(ctx context.Context, accountID string) error
...
type OpenAIWSIngressLeaseCache interface {
	AcquireOpenAIWSIngressLease(ctx context.Context, apiKeyID string, maxConnections int, leaseID string) (bool, error)
	RefreshOpenAIWSIngressLease(ctx context.Context, apiKeyID string, leaseID string) (bool, error)
	ReleaseOpenAIWSIngressLease(ctx context.Context, apiKeyID string, leaseID string) error
}
...
func (l *OpenAIWSIngressLease) Release() {
	...
	l.stopOnce.Do(func() {
		...
		if l.cache == nil || l.apiKeyID == "" || l.leaseID == "" {
			return
		}
		...
		if err := l.cache.ReleaseOpenAIWSIngressLease(releaseCtx, l.apiKeyID, l.leaseID); err != nil {
			logger.L().Warn("openai_ws_ingress_lease_release_failed",
				zap.String("api_key_id", l.apiKeyID),
				zap.Error(err),
			)
		}
	})
...
func (s *ConcurrencyService) AcquireOpenAIWSIngressLease(ctx context.Context, apiKeyID string, maxConnections int) (*OpenAIWSIngressLease, bool, error) {
	if maxConnections <= 0 {
		return nil, true, nil
	}
	if s == nil || s.cache == nil || apiKeyID == "" {
...
func (s *ConcurrencyService) AcquireAccountSlot(ctx context.Context, accountID string, maxConcurrency int) (*AcquireResult, error) {
		...
		return &AcquireResult{
			...
			ReleaseFunc: func() {
				...
				if err := s.cache.ReleaseAccountSlot(bgCtx, accountID, requestID); err != nil {
					logger.LegacyPrintf("service.concurrency", "Warning: failed to release account slot for %v (req=%s): %v", accountID, requestID, err)
				}
			},
		}, nil
	}
...
// Returns a release function that MUST be called when the request completes.
func (s *ConcurrencyService) AcquireUserSlot(ctx context.Context, userID string, maxConcurrency int) (*AcquireResult, error) {
	// If maxConcurrency is 0 or negative, no limit
	if maxConcurrency <= 0 {
		return &AcquireResult{
			Acquired:    true,
		...
		return &AcquireResult{
			...
			ReleaseFunc: func() {
				...
				if err := s.cache.ReleaseUserSlot(bgCtx, userID, requestID); err != nil {
					logger.LegacyPrintf("service.concurrency", "Warning: failed to release user slot for %v (req=%s): %v", userID, requestID, err)
				}
			},
		}, nil
	}
...
func (s *ConcurrencyService) TrackAPIKeySlot(ctx context.Context, apiKeyID string) func() {
	if s == nil || s.cache == nil || apiKeyID == "" {
		return func() {}
	}
	...
	cancel()
	if err != nil {
		logger.LegacyPrintf("service.concurrency", "Warning: failed to track api key slot for %v (req=%s): %v", apiKeyID, requestID, err)
		return func() {}
	}
	...
	return func() {
		...
		if err := cache.ReleaseAPIKeySlot(bgCtx, apiKeyID, requestID); err != nil {
			logger.LegacyPrintf("service.concurrency", "Warning: failed to release api key slot for %v (req=%s): %v", apiKeyID, requestID, err)
		}
	}
}
...
func (s *ConcurrencyService) GetAPIKeyConcurrencyBatch(ctx context.Context, apiKeyIDs []string) (map[string]int, error) {
...
func zeroAPIKeyConcurrencyMap(apiKeyIDs []string) map[string]int {
...
func (s *ConcurrencyService) IncrementWaitCount(ctx context.Context, userID string, maxWait int) (bool, error) {
	if s.cache == nil {
	...
	if err != nil {
		...
		logger.LegacyPrintf("service.concurrency", "Warning: increment wait count failed for user %v: %v", userID, err)
		return true, nil
	}
	return result, nil
}
...
func (s *ConcurrencyService) DecrementWaitCount(ctx context.Context, userID string) {
	if s.cache == nil {
		return
	}
	...
	if err := s.cache.DecrementWaitCount(bgCtx, userID); err != nil {
		logger.LegacyPrintf("service.concurrency", "Warning: decrement wait count failed for user %v: %v", userID, err)
	}
}

// IncrementAccountWaitCount increments the wait queue counter for an account.
func (s *ConcurrencyService) IncrementAccountWaitCount(ctx context.Context, accountID string, maxWait int) (bool, error) {
	if s.cache == nil {
		return true, nil
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.concurrency", "Warning: increment wait count failed for account %v: %v", accountID, err)
		return true, nil
	}
	return result, nil
}
...
func (s *ConcurrencyService) DecrementAccountWaitCount(ctx context.Context, accountID string) {
	if s.cache == nil {
		return
	}
	...
	if err := s.cache.DecrementAccountWaitCount(bgCtx, accountID); err != nil {
		logger.LegacyPrintf("service.concurrency", "Warning: decrement wait count failed for account %v: %v", accountID, err)
	}
}

// GetAccountWaitingCount gets current wait queue count for an account.
func (s *ConcurrencyService) GetAccountWaitingCount(ctx context.Context, accountID string) (int, error) {
	if s.cache == nil {
		return 0, nil
	}
...
func (s *ConcurrencyService) GetAccountsLoadBatch(ctx context.Context, accounts []AccountWithConcurrency) (map[string]*AccountLoadInfo, error) {
	return s.getAccountsLoadBatch(ctx, accounts, true)
}

// GetAccountsLoadBatchFresh 绕过极短 TTL 缓存，用于抢槽失败后的实时刷新兜底。
func (s *ConcurrencyService) GetAccountsLoadBatchFresh(ctx context.Context, accounts []AccountWithConcurrency) (map[string]*AccountLoadInfo, error) {
	return s.getAccountsLoadBatch(ctx, accounts, false)
}

func (s *ConcurrencyService) getAccountsLoadBatch(ctx context.Context, accounts []AccountWithConcurrency, allowCache bool) (map[string]*AccountLoadInfo, error) {
	if len(accounts) == 0 {
	...
	loadMap, _ := value.(map[string]*AccountLoadInfo)
	if loadMap == nil {
		return map[string]*AccountLoadInfo{}, nil
	}
	return loadMap, nil
...
func (s *ConcurrencyService) fetchAccountsLoadBatch(ctx context.Context, accounts []AccountWithConcurrency) (map[string]*AccountLoadInfo, error) {
	if s.cache == nil {
		return map[string]*AccountLoadInfo{}, nil
	}
	baseCtx := context.Background()
...
func (s *ConcurrencyService) getCachedAccountLoadBatch(key string, now time.Time) (map[string]*AccountLoadInfo, bool) {
...
func (s *ConcurrencyService) storeCachedAccountLoadBatch(key string, loadMap map[string]*AccountLoadInfo, expiresAt time.Time) {
	s.accountLoadCacheMu.Lock()
...
func accountLoadBatchCacheKey(accounts []AccountWithConcurrency) string {
	...
	for _, account := range accounts {
		...
		binary.LittleEndian.PutUint64(buf[:], uint64(int64(account.MaxConcurrency)))
		_, _ = hash.Write(buf[:])
	}
	sum := hash.Sum(nil)
...
func cloneAccountLoadMap(loadMap map[string]*AccountLoadInfo) map[string]*AccountLoadInfo {
	if len(loadMap) == 0 {
		return map[string]*AccountLoadInfo{}
	}
	clone := make(map[string]*AccountLoadInfo, len(loadMap))
...
func (s *ConcurrencyService) GetUsersLoadBatch(ctx context.Context, users []UserWithConcurrency) (map[string]*UserLoadInfo, error) {
	if s.cache == nil {
		return map[string]*UserLoadInfo{}, nil
	}
	return s.cache.GetUsersLoadBatch(ctx, users)
}

// GetUserConcurrency returns the number of active request slots for one user.
// A missing cache is treated as an empty runtime load so read-only dashboard
// pages remain available during cache warm-up or maintenance.
func (s *ConcurrencyService) GetUserConcurrency(ctx context.Context, userID string) (int, error) {
	if s == nil || s.cache == nil {
		return 0, nil
	}
	return s.cache.GetUserConcurrency(ctx, userID)
}

// CleanupExpiredAccountSlots removes expired slots for one account (background task).
func (s *ConcurrencyService) CleanupExpiredAccountSlots(ctx context.Context, accountID string) error {
	if s.cache == nil {
		return nil
	}
...
func (s *ConcurrencyService) GetAccountConcurrencyBatch(ctx context.Context, accountIDs []string) (map[string]int, error) {
	if len(accountIDs) == 0 {
