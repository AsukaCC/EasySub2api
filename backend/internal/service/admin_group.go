...
func (s *adminServiceImpl) GetGroup(ctx context.Context, id string) (*Group, error) {
	return s.groupRepo.GetByID(ctx, id)
}

func (s *adminServiceImpl) GetGroupModelsListCandidates(ctx context.Context, id string, platform string) ([]string, error) {
...
func (s *adminServiceImpl) ListCompositeRoutes(ctx context.Context, groupID string) ([]CompositeModelRoute, error) {
...
func (s *adminServiceImpl) CreateCompositeRoute(ctx context.Context, groupID string, input CompositeRouteInput) (*CompositeModelRoute, error) {
	if err := s.requireCompositeGroup(ctx, groupID); err != nil {
		return nil, err
	}
	if s.compositeRouteRepo == nil {
...
func (s *adminServiceImpl) UpdateCompositeRoute(ctx context.Context, groupID, routeID string, input CompositeRouteInput) (*CompositeModelRoute, error) {
	if err := s.requireCompositeGroup(ctx, groupID); err != nil {
		return nil, err
	}
	if s.compositeRouteRepo == nil {
...
func (s *adminServiceImpl) DeleteCompositeRoute(ctx context.Context, groupID, routeID string) error {
	if err := s.requireCompositeGroup(ctx, groupID); err != nil {
		return err
	}
	if s.compositeRouteRepo == nil {
...
func (s *adminServiceImpl) PreviewCompositeRoute(ctx context.Context, groupID string, input CompositeRoutePreviewRequest) (*CompositeRouteDecision, error) {
	if err := s.requireCompositeGroup(ctx, groupID); err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) requireCompositeGroup(ctx context.Context, groupID string) error {
	group, err := s.groupRepo.GetByIDLite(ctx, groupID)
	if err != nil {
		return err
	}
	if group.Platform != PlatformComposite {
		return fmt.Errorf("group %v is not a composite group", groupID)
	}
	return nil
}

func (s *adminServiceImpl) compositeRouteBelongsToGroup(ctx context.Context, groupID, routeID string) (bool, error) {
...
func compositeRouteFromInput(groupID string, input CompositeRouteInput) (*CompositeModelRoute, error) {
...
func compositeDefaultModelsListCandidateIDs() []string {
	...
	ids := make([]string, 0)
	for _, platform := range []string{PlatformAnthropic, PlatformOpenAI, PlatformGrok, PlatformOpenCodeGo} {
...
// sanitizeGroupOpenAIFast clears OpenAI-only controls when a group is moved to
// a platform whose gateway cannot honor service-tier Fast semantics.
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if input.FallbackGroupID != nil {
		if err := s.validateFallbackGroup(ctx, "", *input.FallbackGroupID); err != nil {
			return nil, err
		}
	}
	fallbackOnInvalidRequest := input.FallbackGroupIDOnInvalidRequest
	if fallbackOnInvalidRequest != nil && *fallbackOnInvalidRequest == "" {
		fallbackOnInvalidRequest = nil
	}
	// 校验无效请求兜底分组
	if fallbackOnInvalidRequest != nil {
		if err := s.validateFallbackGroupOnInvalidRequest(ctx, "", platform, subscriptionType, *fallbackOnInvalidRequest); err != nil {
			return nil, err
		}
	}
	...
	var accountIDsToCopy []string
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		// 去重源分组 IDs
		seen := make(map[string]struct{})
		uniqueSourceGroupIDs := make([]string, 0, len(input.CopyAccountsFromGroupIDs))
		...
		for _, srcGroupID := range uniqueSourceGroupIDs {
			...
			if err != nil {
				return nil, fmt.Errorf("source group %v not found: %w", srcGroupID, err)
			}
			if !canCopyAccountsFromGroupPlatform(platform, srcGroup.Platform) {
				return nil, fmt.Errorf("source group %v platform mismatch: expected %s, got %s", srcGroupID, platform, srcGroup.Platform)
			}
		}
	...
	group := &Group{
		...
		LevelRateMultipliers:            levelRates,
		DynamicRateRules:                dynamicRules,
	...
	if group.RequireOAuthOnly && groupSupportsOAuthOnlyFilter(group.Platform) && len(accountIDsToCopy) > 0 {
		...
		oauthIDs := make(map[string]struct{}, len(accounts))
		for _, acc := range accounts {
...
func (s *adminServiceImpl) validateFallbackGroup(ctx context.Context, currentGroupID, fallbackGroupID string) error {
	// 不能将自己设置为降级分组
	if currentGroupID != "" && currentGroupID == fallbackGroupID {
	...
	for {
		...
		if currentGroupID != "" && nextID == currentGroupID {
			return fmt.Errorf("fallback group cycle detected")
		}
...
func (s *adminServiceImpl) validateFallbackGroupOnInvalidRequest(ctx context.Context, currentGroupID string, platform, subscriptionType string, fallbackGroupID string) error {
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id string, input *UpdateGroupInput) (*Group, error) {
	...
	if input.DynamicRateRules != nil {
		...
		group.DynamicRateRules = normalized
	}
	...
	// 限额字段：nil/负数 表示"无限制"，0 表示"不允许用量"，正数表示具体限额。
	// HTTP 更新请求记录字段是否出现；仅在出现时覆盖，避免只保存倍率规则
	// 的部分更新把既有分组限额误清空。直接调用 service 的旧代码仍可通过
	// 非 nil 指针触发更新，保持向后兼容。
	...
	if input.FallbackGroupIDOnInvalidRequest != nil {
		if *input.FallbackGroupIDOnInvalidRequest != "" {
			fallbackOnInvalidRequest = input.FallbackGroupIDOnInvalidRequest
		} else {
			fallbackOnInvalidRequest = nil
		}
	...
	// 校验仅在本次显式携带配置时进行，避免旧 ID 阻塞无关字段更新。
	...
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		...
		for _, srcGroupID := range uniqueSourceGroupIDs {
			...
			if !canCopyAccountsFromGroupPlatform(group.Platform, srcGroup.Platform) {
				return nil, fmt.Errorf("source group %v platform mismatch: expected %s, got %s", srcGroupID, group.Platform, srcGroup.Platform)
			}
		}
...
func (s *adminServiceImpl) DeleteGroup(ctx context.Context, id string) error {
		...
		go func() {
			...
			for _, userID := range affectedUserIDs {
				if err := s.billingCacheService.InvalidateSubscription(cacheCtx, userID, groupID); err != nil {
					logger.LegacyPrintf("service.admin", "invalidate subscription cache failed: user_id=%v group_id=%v err=%v", userID, groupID, err)
				}
			}
		}()
	}
...
func (s *adminServiceImpl) GetGroupAPIKeys(ctx context.Context, groupID string, page, pageSize int) ([]APIKey, int64, error) {
...
func (s *adminServiceImpl) GetGroupRateMultipliers(ctx context.Context, groupID string) ([]UserGroupRateEntry, error) {
...
func (s *adminServiceImpl) BatchSetGroupRPMOverrides(ctx context.Context, groupID string, entries []GroupRPMOverrideInput) error {
	if s.userGroupRateRepo == nil {
		return nil
	}
	for _, e := range entries {
		if e.RPMOverride != nil && *e.RPMOverride < 0 {
			return infraerrors.BadRequest("INVALID_RPM_OVERRIDE", fmt.Sprintf("rpm_override must be >= 0 (user_id=%v)", e.UserID))
		}
	}
...
func (s *adminServiceImpl) AdminUpdateAPIKeyGroupID(ctx context.Context, keyID string, groupID *string) (*AdminUpdateAPIKeyGroupIDResult, error) {
...
// AdminUpdateAPIKeyGroups replaces all group bindings for an API key.
// An empty slice explicitly unbinds the key from every group.
func (s *adminServiceImpl) AdminUpdateAPIKeyGroups(ctx context.Context, keyID string, groupIDs []string) (*AdminUpdateAPIKeyGroupIDResult, error) {
	apiKey, err := s.apiKeyRepo.GetByID(ctx, keyID)
	if err != nil {
		return nil, err
	}

	groupIDs = NormalizeAPIKeyGroupIDs(groupIDs)
	...
	for _, group := range groups {
		...
		if err := s.userRepo.AddGroupToAllowedGroups(ctx, apiKey.UserID, group.ID); err != nil {
...
func (s *adminServiceImpl) AdminResetAPIKeyRateLimitUsage(ctx context.Context, keyID string) (*APIKey, error) {
	apiKey, err := s.apiKeyRepo.GetByID(ctx, keyID)
	if err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) ReplaceUserGroup(ctx context.Context, userID, oldGroupID, newGroupID string) (*ReplaceUserGroupResult, error) {
