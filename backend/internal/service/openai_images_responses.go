...
func buildOpenAIImagesResponsesRequest(parsed *OpenAIImagesRequest, toolModel string) ([]byte, error) {
	...
	for index, imageURL := range inputImages {
		...
		input, _ = sjson.SetRawBytes(input, fmt.Sprintf("0.content.%v", index+1), part)
	}
...
func openAIImagesUpstreamErrorFromHTTP(statusCode int, header http.Header, body []byte) *OpenAIImagesUpstreamError {
	...
	if message == "" {
		message = fmt.Sprintf("Upstream request failed (status %v)", statusCode)
	}
	if errType == "" {
...
func (s *OpenAIGatewayService) handleOpenAIImagesErrorResponse(
	...
	// Capacity shedding is request-scoped for OpenAI. Keep it out of the
	// capability/account health path even when this handler is reached directly
	// instead of through the normal failover pre-check.
	...
	if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
		logger.LegacyPrintf("service.openai_gateway",
			"OpenAI images upstream error %v (account=%v platform=%s type=%s): %s",
			resp.StatusCode,
			account.ID,
			account.Platform,
			account.Type,
	...
	if account.UsesOpenAICodexProtocol() && isOpenAIImagesMainModelError(resp.StatusCode, body) {
		upErr := openAIImagesUpstreamErrorFromHTTP(resp.StatusCode, resp.Header, body)
		writeOpenAIImagesUpstreamErrorResponse(c, upErr)
		return nil, upErr
	}
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	// 复用 Codex 认证、影子账号及指纹头，只切换已构造请求的端点和响应协议。
	...
	if resp.StatusCode >= 400 {
		...
		if !direct && isOpenAIImagesSelfBuiltRequest(upstreamCtx) && isOpenAIImageGenerationUnavailableCode(respBody) {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			...
			return nil, newOpenAIUpstreamFailoverErrorForPlatform(
				account.Platform,
				resp.StatusCode,
				resp.Header,
				respBody,
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthResponseError(
	...
	if code, message, ok := OpenAIUpstreamStreamReadErrorDetails(err); ok {
		...
		return newOpenAIUpstreamFailoverErrorForPlatform(
			account.Platform,
			statusCode,
			headers,
			responseBody,
