...
type APIKey struct {
	...
	Name    string
	GroupID *string
	// GroupIDs contains all groups allowed to use this key. GroupID remains
	// the primary group for backward-compatible pricing and request settings.
	GroupIDs    []string
	Status      string
	IPWhitelist []string
	IPBlacklist []string
	...
	// Quota fields
	Quota     float64    // Quota limit in platform points (0 = unlimited)
	QuotaUsed float64    // Used quota amount
	ExpiresAt *time.Time // Expiration time (nil = never expires)

	// Rate limit fields
	RateLimit5h   float64    // Rate limit in platform points per 5h (0 = unlimited)
	RateLimit1d   float64    // Rate limit in platform points per 1d (0 = unlimited)
	RateLimit7d   float64    // Rate limit in platform points per 7d (0 = unlimited)
...
type APIKeyListFilters struct {
	Search  string
	Status  string
	GroupID *string // nil=不筛选, 0=无分组, >0=指定分组
}
