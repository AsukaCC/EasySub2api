...
func (s *OpenCodeGoUsageService) GetState(ctx context.Context, accountID string) (*OpenCodeGoUsageState, error) {
...
func (s *OpenCodeGoUsageService) SetAutoRefresh(ctx context.Context, accountID string, enabled bool) (*OpenCodeGoUsageState, error) {
	if s == nil || s.accountRepo == nil {
		return nil, ErrOpenCodeGoUsageUnavailable
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
...
func (s *OpenCodeGoUsageService) Refresh(ctx context.Context, accountID string) (*OpenCodeGoUsageState, error) {
			...
			if _, refreshErr := s.refreshAccount(ctx, accountID, settings, true); refreshErr != nil {
				if errors.Is(refreshErr, ErrOpenCodeGoUsageIdentityChanged) {
					if disableErr := writer.DisableOpenCodeGoUsageAutoRefresh(ctx, &expected); disableErr != nil {
						logger.LegacyPrintf("service.opencode_go_usage", "disable_auto_refresh_failed: account_id=%s err=%v", accountID, disableErr)
					}
					return nil
				}
				logger.LegacyPrintf("service.opencode_go_usage", "refresh_due_failed: account_id=%s err=%v", accountID, refreshErr)
			}
			return nil
		})
	}
...
func (s *OpenCodeGoUsageService) refreshAccount(ctx context.Context, accountID string, settings *OpenCodeGoUsageSettings, requireEnabled bool) (*OpenCodeGoUsageSnapshot, error) {
	if s == nil || s.accountRepo == nil {
		return nil, ErrOpenCodeGoUsageUnavailable
	}
	if settings == nil {
			...
			if writer, ok := s.accountRepo.(openCodeGoUsageRepository); ok {
				...
				if listErr != nil {
					...
					logger.LegacyPrintf("service.opencode_go_usage",
						"group_last_used_lookup_failed: account_id=%s err=%v", account.ID, listErr)
				} else {
