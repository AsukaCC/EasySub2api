...
func (s *PaymentConfigService) ListProviderInstancesWithConfig(ctx context.Context) ([]ProviderInstanceResponse, error) {
	...
	for _, inst := range instances {
		...
		if err != nil {
			return nil, fmt.Errorf("decrypt config for instance %v: %w", inst.ID, err)
		}
...
func (s *PaymentConfigService) countPendingOrders(ctx context.Context, providerInstanceID string) (int, error) {
	return s.entClient.PaymentOrder.Query().
		Where(
			paymentorder.ProviderInstanceIDEQ(providerInstanceID),
			paymentorder.StatusIn(pendingOrderStatuses...),
		).Count(ctx)
}

func (s *PaymentConfigService) countPendingOrdersByPlan(ctx context.Context, planID string) (int, error) {
	return s.entClient.PaymentOrder.Query().
		Where(
...
func (s *PaymentConfigService) CreateProviderInstance(ctx context.Context, req CreateProviderInstanceRequest) (*dbent.PaymentProviderInstance, error) {
	...
	if err := s.validateVisibleMethodEnablementConflicts(ctx, "", req.ProviderKey, typesStr, req.Enabled); err != nil {
		return nil, err
	}
	if req.Enabled {
...
func (s *PaymentConfigService) UpdateProviderInstance(ctx context.Context, id string, req UpdateProviderInstanceRequest) (*dbent.PaymentProviderInstance, error) {
...
// GetRefundEligibleInstanceIDs separates providers that support immediate
// self-service refunds from providers that can only be reached through a
// manually reviewed ticket.
func (s *PaymentConfigService) GetRefundEligibleInstanceIDs(ctx context.Context) (userRefundIDs, refundEnabledIDs []string, err error) {
	instances, err := s.entClient.PaymentProviderInstance.Query().
		Where(
	...
	userRefundIDs = make([]string, 0, len(instances))
	refundEnabledIDs = make([]string, 0, len(instances))
	for _, inst := range instances {
		refundEnabledIDs = append(refundEnabledIDs, inst.ID)
		if inst.AllowUserRefund {
			userRefundIDs = append(userRefundIDs, inst.ID)
		}
	}
	return userRefundIDs, refundEnabledIDs, nil
}

// GetUserRefundEligibleInstanceIDs preserves the original service contract.
func (s *PaymentConfigService) GetUserRefundEligibleInstanceIDs(ctx context.Context) ([]string, error) {
	ids, _, err := s.GetRefundEligibleInstanceIDs(ctx)
	return ids, err
}

func (s *PaymentConfigService) mergeConfig(ctx context.Context, id string, newConfig map[string]string) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decrypt existing config for instance %v: %w", id, err)
	}
	if existing == nil {
...
func (s *PaymentConfigService) DeleteProviderInstance(ctx context.Context, id string) error {
	count, err := s.countPendingOrders(ctx, id)
	if err != nil {
	...
	if count > 0 {
		return infraerrors.Conflict("PENDING_ORDERS",
			fmt.Sprintf("this instance has %v in-progress orders and cannot be deleted — wait for orders to complete or disable the instance first", count))
	}
