...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	logger.LegacyPrintf("service.gateway", "[Anthropic 自动透传] 命中 API Key 透传分支: account=%v name=%s model=%s stream=%v",
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				logger.LegacyPrintf("service.gateway", "Anthropic passthrough account %v: upstream error %v, retry %v/%v after %v (elapsed=%v/%v)",
	...
	if resp.StatusCode >= 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
		if s.shouldFailoverUpstreamError(resp.StatusCode) {
			...
			logger.LegacyPrintf("service.gateway", "[Anthropic Passthrough] Upstream error (retry exhausted, failover): Account=%v(%s) Status=%v RequestID=%s Body=%s",
	...
	if resp.StatusCode >= 400 && s.shouldFailoverUpstreamError(resp.StatusCode) {
		...
		logger.LegacyPrintf("service.gateway", "[Anthropic Passthrough] Upstream error (failover): Account=%v(%s) Status=%v RequestID=%s Body=%s",
			account.ID, account.Name, resp.StatusCode, resp.Header.Get("x-request-id"), truncateString(string(respBody), 1000))
...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	applyProxyTimezoneHeader(ctx, account, req.Header, s.proxyTimezoneResolver)
...
func (s *GatewayService) handleStreamingResponseAnthropicAPIKeyPassthrough(
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				if errors.Is(ev.err, bufio.ErrTooLong) {
					logger.LegacyPrintf("service.gateway", "[Anthropic passthrough] SSE line too long: account=%v max_size=%v error=%v", account.ID, maxLineSize, ev.err)
			...
			if !clientDisconnected {
				...
				if _, err := io.WriteString(w, restored); err != nil {
					...
					logger.LegacyPrintf("service.gateway", "[Anthropic passthrough] Client disconnected during streaming, continue draining upstream for usage: account=%v", account.ID)
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.gateway", "[Anthropic passthrough] Stream data interval timeout: account=%v model=%s interval=%s", account.ID, model, streamInterval)
			if s.rateLimitService != nil {
		...
		case <-keepaliveCh:
			...
			if _, err := fmt.Fprint(w, "event: ping\ndata: {\"type\": \"ping\"}\n\n"); err != nil {
				clientDisconnected = true
				logger.LegacyPrintf("service.gateway", "[Anthropic passthrough] Client disconnected during keepalive ping, continue draining upstream for usage: account=%v", account.ID)
				continue
			}
			flusher.Flush()
			lastDataAt = time.Now()
...
func invalidNonStreamingJSONFailoverError(
	...
	logger.LegacyPrintf(
		"service.gateway",
		"Account %v(%s): upstream returned non-JSON 2xx response, attempting failover: status=%v request_id=%s error=%v",
		accountID,
		accountName,
		resp.StatusCode,
