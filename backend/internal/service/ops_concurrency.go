...
type opsAccountStatsRepository interface {
	ListOpsAccountsForStats(ctx context.Context, platformFilter string, groupIDFilter *string) ([]Account, error)
}

func (s *OpsService) listAllAccountsForOps(ctx context.Context, platformFilter string, groupIDFilter *string) ([]Account, error) {
...
func (s *OpsService) getAccountsLoadMapBestEffort(ctx context.Context, accounts []Account) map[string]*AccountLoadInfo {
	...
	unique := make(map[string]int, len(accounts))
	...
	out := make(map[string]*AccountLoadInfo, len(batch))
...
func (s *OpsService) GetConcurrencyStats(
...
) (map[string]*PlatformConcurrencyInfo, map[string]*GroupConcurrencyInfo, map[string]*AccountConcurrencyInfo, *time.Time, error) {
	...
	group := make(map[string]*GroupConcurrencyInfo)
	account := make(map[string]*AccountConcurrencyInfo)
...
func (s *OpsService) getUsersLoadMapBestEffort(ctx context.Context, users []User) map[string]*UserLoadInfo {
	if s == nil || s.concurrencyService == nil {
		return map[string]*UserLoadInfo{}
	}
	if len(users) == 0 {
		return map[string]*UserLoadInfo{}
	}
	...
	// De-duplicate IDs (and keep the max concurrency to avoid under-reporting).
	unique := make(map[string]int, len(users))
	for _, u := range users {
		if u.ID == "" {
			continue
		}
	...
	out := make(map[string]*UserLoadInfo, len(batch))
	for i := 0; i < len(batch); i += opsConcurrencyBatchChunkSize {
		end := i + opsConcurrencyBatchChunkSize
		if end > len(batch) {
			end = len(batch)
...
func (s *OpsService) GetUserConcurrencyStats(ctx context.Context) (map[string]*UserConcurrencyInfo, *time.Time, error) {
	if err := s.RequireMonitoringEnabled(ctx); err != nil {
		return nil, nil, err
	}
	...
	result := make(map[string]*UserConcurrencyInfo)
