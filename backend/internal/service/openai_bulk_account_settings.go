...
func validateBulkOpenAISettingsTargets(
	...
	targetsByID map[string]*Account,
) (int, error) {
...
func invalidBulkOpenAITarget(accountID string, message string) error {
	return infraerrors.BadRequest(
		"OPENAI_BULK_TARGET_INVALID",
		fmt.Sprintf("account %v: %s", accountID, message),
	).WithMetadata(map[string]string{"account_id": accountID})
}
