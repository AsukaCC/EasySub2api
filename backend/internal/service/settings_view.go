...
type SystemSettings struct {
	...
	AffiliateInviterBindingRewardPoints       float64
	AffiliateInviterBindingRewardValidityDays int
	AffiliateInviteeBindingRewardPoints       float64
	AffiliateInviteeBindingRewardValidityDays int
	DefaultUserRPMLimit                       int
	DefaultSubscriptions                      []DefaultSubscriptionSetting
	DefaultUserLevelRuleIDs                   []string
	...
	OpenAITTFTMode                         string // Responses first_token_ms mode; visible by default
...
// account's image capability is paused after an explicit upstream rejection.
...
type OpenAIFastPolicyRule struct {
	...
	Scope                string   `json:"scope"`                            // "all" | "oauth" | "apikey" | "bedrock"
	UserIDs              []string `json:"user_ids,omitempty"`               // 空=所有 EasySub2api 用户；非空=仅指定 API Key 所属用户
	ErrorMessage         string   `json:"error_message,omitempty"`          // 自定义错误消息 (action=block 时生效)
	ModelWhitelist       []string `json:"model_whitelist,omitempty"`        // 模型匹配模式列表（为空=对所有模型生效）
	FallbackAction       string   `json:"fallback_action,omitempty"`        // 未匹配白名单的模型的处理方式
	FallbackErrorMessage string   `json:"fallback_error_message,omitempty"` // 未匹配白名单时的自定义错误消息 (fallback_action=block 时生效)
