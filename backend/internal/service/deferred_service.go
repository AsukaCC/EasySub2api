...
func (s *DeferredService) ScheduleLastUsedUpdate(accountID string) {
...
func (s *DeferredService) flushLastUsed() {
	...
	if err := s.accountRepo.BatchUpdateLastUsed(ctx, updates); err != nil {
		log.Printf("[DeferredService] BatchUpdateLastUsed failed (%v accounts): %v", len(updates), err)
		for id, ts := range updates {
			s.lastUsedUpdates.Store(id, ts)
		}
	} else {
		log.Printf("[DeferredService] BatchUpdateLastUsed flushed %v accounts", len(updates))
	}
}
