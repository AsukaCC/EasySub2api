...
func TestIsolateOpenAISessionID(t *testing.T) {
	t.Run("empty_raw_returns_empty", func(t *testing.T) {
		assert.Equal(t, "", isolateOpenAISessionID("api-key-1", ""))
		assert.Equal(t, "", isolateOpenAISessionID("api-key-1", "   "))
	})

	t.Run("deterministic", func(t *testing.T) {
		a := isolateOpenAISessionID("api-key-42", "sess_abc123")
		b := isolateOpenAISessionID("api-key-42", "sess_abc123")
		assert.Equal(t, a, b)
	})

	t.Run("different_apiKeyID_different_result", func(t *testing.T) {
		a := isolateOpenAISessionID("api-key-1", "same_session")
		b := isolateOpenAISessionID("api-key-2", "same_session")
		require.NotEqual(t, a, b, "不同 API Key 使用相同 session_id 应产生不同隔离值")
	})

	t.Run("different_raw_different_result", func(t *testing.T) {
		a := isolateOpenAISessionID("api-key-1", "session_a")
		b := isolateOpenAISessionID("api-key-1", "session_b")
		require.NotEqual(t, a, b)
	})

	t.Run("format_is_16_hex_chars", func(t *testing.T) {
		result := isolateOpenAISessionID("api-key-99", "test_session")
	...
	t.Run("zero_apiKeyID_still_works", func(t *testing.T) {
		result := isolateOpenAISessionID("", "session")
		assert.NotEmpty(t, result)
		// apiKeyID=0 与 apiKeyID=1 应产生不同结果
		other := isolateOpenAISessionID("api-key-1", "session")
