...
func (s *UserAttributeService) GetDefinition(ctx context.Context, id string) (*UserAttributeDefinition, error) {
...
func (s *UserAttributeService) UpdateDefinition(ctx context.Context, id string, input UpdateAttributeDefinitionInput) (*UserAttributeDefinition, error) {
...
func (s *UserAttributeService) DeleteDefinition(ctx context.Context, id string) error {
...
func (s *UserAttributeService) ReorderDefinitions(ctx context.Context, orders map[string]int) error {
	return s.defRepo.UpdateDisplayOrders(ctx, orders)
}

// GetUserAttributes retrieves all attribute values for a user
func (s *UserAttributeService) GetUserAttributes(ctx context.Context, userID string) ([]UserAttributeValue, error) {
...
func (s *UserAttributeService) GetBatchUserAttributes(ctx context.Context, userIDs []string) (map[string]map[string]string, error) {
	...
	for _, v := range values {
		if result[v.UserID] == nil {
			result[v.UserID] = make(map[string]string)
		}
...
func (s *UserAttributeService) UpdateUserAttributes(ctx context.Context, userID string, inputs []UpdateUserAttributeInput) error {
	...
	defMap := make(map[string]*UserAttributeDefinition, len(defs))
	for i := range defs {
...
func (s *UserAttributeService) validateValue(def *UserAttributeDefinition, value string) error {
	...
	if v.MinLength != nil && len(value) < *v.MinLength {
		return validationError(fmt.Sprintf("%s must be at least %v characters", def.Name, *v.MinLength))
	}
	if v.MaxLength != nil && len(value) > *v.MaxLength {
		return validationError(fmt.Sprintf("%s must be at most %v characters", def.Name, *v.MaxLength))
	}
	...
	if def.Type == AttributeTypeNumber && value != "" {
		...
		if v.Min != nil && num < *v.Min {
			return validationError(fmt.Sprintf("%s must be at least %v", def.Name, *v.Min))
		}
		if v.Max != nil && num > *v.Max {
			return validationError(fmt.Sprintf("%s must be at most %v", def.Name, *v.Max))
		}
	}
