...
type GrokOAuthRefreshMutationRepository interface {
	SetGrokOAuthRefreshErrorIfCredentialsUnchanged(ctx context.Context, id string, expectedCredentials map[string]any, expectedProxyID *string, errorMsg string) (bool, error)
	SetGrokOAuthRefreshTempUnschedulableIfCredentialsUnchanged(ctx context.Context, id string, expectedCredentials map[string]any, expectedProxyID *string, until time.Time, reason string) (bool, error)
}
...
func NewTokenRefreshService(
	...
	antigravityRefresher := NewAntigravityTokenRefresher(antigravityOAuthService)
	...
	s.registrations = []tokenRefreshRegistration{
		...
		{platform: PlatformAntigravity, refresher: antigravityRefresher, executor: antigravityRefresher},
...
func (s *TokenRefreshService) candidateAfterID() string {
	s.candidateMu.Lock()
	defer s.candidateMu.Unlock()
	return s.afterID
}

func (s *TokenRefreshService) setCandidateAfterID(afterID string) {
	s.candidateMu.Lock()
	s.afterID = afterID
...
func (s *TokenRefreshService) notifyAccountSchedulingBlockCleared(accountID string) {
...
func (s *TokenRefreshService) processRefreshContext(parent context.Context) {
	...
	for {
		...
		if !page.HasMore && page.NextAfterID == "" && len(accounts) == 0 {
			s.setCandidateAfterID("")
			break
		}
...
func isStrictlyIncreasingAccountPage(accounts []Account, afterID string) bool {
	previous := afterID
...
func (s *TokenRefreshService) retryBackoff(accountID string, attempt int) time.Duration {
	...
	jitterPercent := int64(75) + int64((h.Sum32()+uint32(attempt*17))%51)
