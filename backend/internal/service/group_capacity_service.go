...
type groupCapacityAccountLister interface {
	ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []string) ([]GroupAccountCapacityRow, error)
}
...
func (s *GroupCapacityService) listActiveGroupIDs(ctx context.Context) ([]string, error) {
...
func (s *GroupCapacityService) getGroupCapacitiesSequential(ctx context.Context, groupIDs []string) []GroupCapacitySummary {
...
func (s *GroupCapacityService) getGroupCapacitiesBatch(ctx context.Context, groupIDs []string, lister groupCapacityAccountLister) ([]GroupCapacitySummary, error) {
	results := make([]GroupCapacitySummary, len(groupIDs))
	groupIndex := make(map[string]int, len(groupIDs))
	...
	sessionTimeouts := make(map[string]time.Duration)
	...
	var sessionsMap map[string]int
...
func accountIDsForGroupsWithLimit(refs []groupCapacityAccountRef, groupIndex map[string]int, summaries []GroupCapacitySummary, include func(GroupCapacitySummary) bool) []string {
...
func (s *GroupCapacityService) getGroupCapacity(ctx context.Context, groupID string) (GroupCapacitySummary, error) {
