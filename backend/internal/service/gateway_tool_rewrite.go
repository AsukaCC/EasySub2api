...
func applyToolNameRewriteToBody(body []byte, rw *ToolNameRewrite) []byte {
	...
	if tools.IsArray() {
		...
		tools.ForEach(func(_, t gjson.Result) bool {
			...
			if next, err := sjson.SetBytes(body, fmt.Sprintf("tools.%v.name", idx), fake); err == nil {
				body = next
			}
			return true
		})
	...
	if messages.IsArray() {
		messages.ForEach(func(msgKey, msg gjson.Result) bool {
			...
			content.ForEach(func(blkKey, blk gjson.Result) bool {
				...
				if fake, ok := rw.Forward[name]; ok {
					path := fmt.Sprintf("messages.%v.content.%v.name", msgIdx, blkIdx)
...
func applyToolsLastCacheBreakpoint(body []byte) []byte {
	...
	if existingCC.Exists() {
		if next, err := sjson.SetBytes(body, fmt.Sprintf("tools.%v.cache_control.ttl", lastIdx), claude.DefaultCacheControlTTL); err == nil {
			body = next
		}
		return body
	}
	...
	if next, err := sjson.SetRawBytes(body, fmt.Sprintf("tools.%v.cache_control", lastIdx), []byte(raw)); err == nil {
		body = next
	}
	return body
}
...
func stripDeferredToolCacheControl(body []byte) []byte {
	...
	for idx, tool := range tools.Array() {
		...
		if next, err := sjson.DeleteBytes(body, fmt.Sprintf("tools.%v.cache_control", idx)); err == nil {
			body = next
		}
	}
	return body
