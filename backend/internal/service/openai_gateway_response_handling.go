...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	if stageFirstOutput {
		...
		defer func() {
			if err := firstOutputStage.Close(); err != nil {
				logger.LegacyPrintf("service.openai_gateway", "OpenAI first-output staging cleanup failed: account=%v model=%s error=%v", account.ID, originalModel, err)
			}
		}()
	}
	...
	handlePendingWriteError := func(err error) {
		if firstOutputStage != nil && !firstOutputStage.closed {
			...
			logger.LegacyPrintf("service.openai_gateway", "%s: account=%v model=%s error=%v", message, account.ID, originalModel, err)
	...
	handleScanErr := func(scanErr error) (*openaiStreamingResult, error, bool) {
		...
		if errors.Is(scanErr, errOpenAIFirstOutputScannerLimit) && !firstOutputProgressObserved {
			logger.LegacyPrintf("service.openai_gateway", "SSE token exceeded guarded first-output limit: account=%v limit=%v error=%v", account.ID, openAIFirstOutputStageMaxBytes+openAIFirstOutputScannerFramingAllowance, scanErr)
		...
		if errors.Is(scanErr, bufio.ErrTooLong) && stageFirstOutput && !firstOutputProgressObserved {
			logger.LegacyPrintf("service.openai_gateway", "SSE line too long before first output: account=%v max_size=%v error=%v", account.ID, maxLineSize, scanErr)
			failoverErr := s.newOpenAIStreamFailoverError(
				c, account, false, upstreamRequestID, nil,
				"OpenAI SSE line exceeds guarded first-output limit",
			)
		...
		if postOutputCapacityErr != nil {
			return resultWithUsage(), postOutputCapacityErr, true
		}
		if sawTerminalEvent {
			if !sawFailedEvent {
				s.clearOpenAIProxyStreamDisconnect(account)
		...
		if errors.Is(scanErr, bufio.ErrTooLong) {
			logger.LegacyPrintf("service.openai_gateway", "SSE line too long: account=%v max_size=%v error=%v", account.ID, maxLineSize, scanErr)
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "error" && account != nil && account.Platform == PlatformOpenAI &&
				...
				// Capacity-shed signals must enter the bounded account retry path
				// before a broad custom passthrough rule can turn them into a final
				// response. Preserve the existing rule precedence for other errors.
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 && firstOutputTimeout <= 0 {
		...
		for documentScanner.Scan() {
			...
			if sawTerminalEvent && !sawFailedEvent && !eventInProgress {
	...
	for {
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.openai_gateway", "Stream data interval timeout: account=%v model=%s interval=%s", account.ID, originalModel, streamInterval)
...
func (s *OpenAIGatewayService) bindHTTPResponseAccount(ctx context.Context, c *gin.Context, account *Account, responseID string) {
	if s == nil || account == nil || account.ID == "" {
		return
	}
...
func (s *OpenAIGatewayService) writeOpenAINonStreamingProtocolError(resp *http.Response, c *gin.Context, message string, payload []byte, accounts ...*Account) error {
