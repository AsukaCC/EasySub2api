...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID string) (*OpenAIQuotaUsage, error) {
	...
	for recovered := false; ; {
		...
		if !resp.IsSuccessState() {
			...
			return nil, infraerrors.Newf(mapUpstreamStatus(status), "OPENAI_QUOTA_UPSTREAM_ERROR", "upstream returned %v: %s", status, body)
		}
		break
	}
...
func (s *OpenAIQuotaService) CacheResetCreditsSnapshot(ctx context.Context, accountID string, credits *OpenAIRateLimitResetCredits) error {
...
func (s *OpenAIQuotaService) CacheCreditsSnapshot(ctx context.Context, accountID string, usage *OpenAIQuotaUsage) error {
	if usage == nil {
		return infraerrors.New(http.StatusBadGateway, "OPENAI_QUOTA_EMPTY_USAGE", "empty quota response")
	}
...
func (s *OpenAIQuotaService) queryResetCreditDetails(ctx context.Context, client *req.Client, accessToken, chatGPTAccountID string, fedRAMP bool, accountID string) *openAIRateLimitResetCreditDetails {
...
func (s *OpenAIQuotaService) ResetCredit(ctx context.Context, accountID string) (*OpenAIQuotaResetResult, error) {
	...
	for recovered := false; ; {
		...
		if !resp.IsSuccessState() {
			...
			return nil, infraerrors.Newf(mapUpstreamStatus(status), "OPENAI_QUOTA_RESET_UPSTREAM_ERROR", "upstream returned %v: %s", status, body)
		}
		break
	}
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID string) (accessToken, chatGPTAccountID, proxyURL string, fedRAMP bool, err error) {
...
func (s *OpenAIQuotaService) recoverAgentIdentityTask(ctx context.Context, accountID string, expectedTaskID string) error {
...
func (s *OpenAIQuotaService) isAgentIdentityAccount(ctx context.Context, accountID string) bool {
	if s == nil || s.accountRepo == nil {
		return false
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
...
func (s *OpenAIQuotaService) redactQuotaErrorBody(ctx context.Context, accountID string, body string) string {
	if s == nil || s.accountRepo == nil {
		return body
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
