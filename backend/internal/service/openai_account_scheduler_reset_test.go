...
func openAIPlanScores(plan openAIAccountLoadPlan) map[string]float64 {
	scores := make(map[string]float64, len(plan.candidates))
...
func TestBuildOpenAIAccountLoadPlan_ResetWeightPrefersSoonestReset(t *testing.T) {
	...
	filtered := []*Account{
		{ID: "account-1", Priority: 0, SessionWindowEnd: &later},
		{ID: "account-2", Priority: 0, SessionWindowEnd: &soon},
	}
	sched := openAIResetTestScheduler(5.0)

	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{}, filtered, map[string]*AccountLoadInfo{})
	scores := openAIPlanScores(plan)
	require.Greater(t, scores["account-2"], scores["account-1"], "重置时间最早的账号（ID=2）得分更高")
}
...
func TestBuildOpenAIAccountLoadPlan_ResetWeightZeroNoEffect(t *testing.T) {
	...
	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{}, filtered, map[string]*AccountLoadInfo{})
	scores := openAIPlanScores(plan)
	require.Equal(t, scores["account-1"], scores["account-2"], "Reset 权重为 0 时两账号得分相同")
}
...
func TestBuildOpenAIAccountLoadPlan_ResetWeightIgnoresNilWindow(t *testing.T) {
	...
	filtered := []*Account{
		{ID: "account-1", Priority: 0, SessionWindowEnd: nil},
		{ID: "account-2", Priority: 0, SessionWindowEnd: &soon},
	}
	sched := openAIResetTestScheduler(5.0)
	...
	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{}, filtered, map[string]*AccountLoadInfo{})
	scores := openAIPlanScores(plan)
	require.Greater(t, scores["account-2"], scores["account-1"], "拥有活跃窗口的账号得分高于无窗口账号")
}
...
func TestBuildOpenAIAccountLoadPlan_QuotaHeadroomPrefersHigher7dRemaining(t *testing.T) {
	...
	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{}, filtered, map[string]*AccountLoadInfo{})
	scores := openAIPlanScores(plan)
	require.Greater(t, scores["account-2"], scores["account-1"], "7d 剩余额度更高的账号得分应更高")
}
...
func TestBuildOpenAIAccountLoadPlan_QuotaHeadroomZeroNoEffect(t *testing.T) {
	...
	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{}, filtered, map[string]*AccountLoadInfo{})
	scores := openAIPlanScores(plan)
	require.Equal(t, scores["account-1"], scores["account-2"], "quota_headroom 权重为 0 时不应影响打分")
}
