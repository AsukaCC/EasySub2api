...
func (s *GatewayService) handleBedrockStreamingResponse(
	...
	for {
		...
		case ev, ok := <-events:
			...
			if !clientDisconnected {
				...
				if writeErr != nil {
					...
					logger.LegacyPrintf("service.gateway", "[Bedrock] Client disconnected during streaming, continue draining for usage: account=%v", account.ID)
				} else {
					flusher.Flush()
				}
			}
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.gateway", "[Bedrock] Stream data interval timeout: account=%v model=%s interval=%s", account.ID, model, streamInterval)
...
func (d *bedrockEventStreamDecoder) Decode() ([]byte, error) {
	for {
		...
		if totalLength < 16 { // minimum: 12 prelude + 4 message_crc
			return nil, fmt.Errorf("invalid eventstream frame: total_length=%v", totalLength)
		}
