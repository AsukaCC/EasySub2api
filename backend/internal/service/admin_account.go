...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, groupID, privacyMode, expiryStatus)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) ([]Account, error) {
...
func (s *adminServiceImpl) GetAccount(ctx context.Context, id string) (*Account, error) {
	return s.accountRepo.GetByID(ctx, id)
}

func (s *adminServiceImpl) GetAccountsByIDs(ctx context.Context, ids []string) ([]*Account, error) {
	if len(ids) == 0 {
		return []*Account{}, nil
	}
...
func duplicateAccountGroups(source *Account) ([]AccountGroup, []string) {
	if len(source.AccountGroups) > 0 {
		groups := make([]AccountGroup, 0, len(source.AccountGroups))
		groupIDs := make([]string, 0, len(source.AccountGroups))
	...
	groupIDs := append([]string(nil), source.GroupIDs...)
...
func duplicateAccountOperationID(sourceID string, actorScope, operationKey string) string {
	...
	payload := "admin.accounts.duplicate\x00" + actorScope + "\x00" + sourceID + "\x00" + operationKey
...
func (s *adminServiceImpl) RecoverDuplicateAccount(ctx context.Context, id string, actorScope, operationKey string) (*Account, error) {
...
func (s *adminServiceImpl) DuplicateAccount(ctx context.Context, id string, actorScope, operationKey string) (*Account, error) {
			...
			go func() {
				defer func() {
					if recovered := recover(); recovered != nil {
						slog.Error("create_account_antigravity_privacy_panic", "account_id", account.ID, "recover", recovered)
					}
				}()
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id string, input *UpdateAccountInput) (*Account, error) {
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	result := &BulkUpdateAccountsResult{
		SuccessIDs: make([]string, 0, len(input.AccountIDs)),
		FailedIDs:  make([]string, 0, len(input.AccountIDs)),
	...
	targetsByID := make(map[string]*Account, len(cachedTargets))
	...
	if len(input.Credentials) > 0 {
		for _, acc := range cachedTargets {
			if acc != nil && acc.IsCredentialShadow() {
				return nil, infraerrors.Newf(http.StatusBadRequest, "SPARK_SHADOW_NO_CREDENTIALS",
					"spark shadow account %v cannot hold credentials; manage credentials on the parent account", acc.ID)
			}
		}
	}
		...
		for _, acc := range cachedTargets {
			if acc != nil && acc.IsCredentialShadow() {
				return nil, infraerrors.Newf(http.StatusBadRequest, "SPARK_SHADOW_PROXY_INHERITED",
					"spark shadow account %v proxy is inherited from its parent and cannot be set in bulk; manage it on the parent account", acc.ID)
			}
		}
	}
	...
	if repoUpdates.ProxyID != nil {
		var effectiveProxyID *string
		if *repoUpdates.ProxyID != "" {
...
func (s *adminServiceImpl) resolveBulkUpdateTargetIDs(ctx context.Context, filters *BulkUpdateAccountFilters) ([]string, error) {
	if filters == nil {
		return nil, nil
	}
	...
	default:
		parsedGroupID, err := uuid.Parse(strings.TrimSpace(filters.Group))
		if err != nil {
			return nil, fmt.Errorf("invalid group filter: %w", err)
		}
		groupID = parsedGroupID.String()
	}
	expiryStatus := strings.TrimSpace(filters.ExpiryStatus)
	switch expiryStatus {
	case "", AccountExpiryStatusExpiring, AccountExpiryStatusExpired:
	default:
		return nil, infraerrors.BadRequest("INVALID_EXPIRY_STATUS_FILTER", "invalid expiry status filter")
	}
...
func (s *adminServiceImpl) DeleteAccount(ctx context.Context, id string) error {
	...
	for _, shadow := range shadows {
		if err := s.accountRepo.Delete(ctx, shadow.ID); err != nil {
			return fmt.Errorf("cascade delete spark shadow %v: %w", shadow.ID, err)
		}
	}
...
func (s *adminServiceImpl) RefreshAccountCredentials(ctx context.Context, id string) (*Account, error) {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) ClearAccountError(ctx context.Context, id string) (*Account, error) {
...
func (s *adminServiceImpl) SetAccountError(ctx context.Context, id string, errorMsg string) error {
	return s.accountRepo.SetError(ctx, id, errorMsg)
}

func (s *adminServiceImpl) SetAccountSchedulable(ctx context.Context, id string, schedulable bool) (*Account, error) {
...
func (s *adminServiceImpl) RevertAccountProxyFallback(ctx context.Context, id string) error {
...
func (s *adminServiceImpl) CreateShadow(ctx context.Context, parentID string, opts ShadowOptions) (*Account, error) {
	// 1. 加载母账号并校验平台/类型
	...
	if len(groupIDs) > 0 {
		...
		groupIDs = append([]string(nil), parent.GroupIDs...)
...
func (s *adminServiceImpl) propagateProxyToShadows(ctx context.Context, parentID string, proxyID *string) error {
...
func propagateAccountProxyToShadows(ctx context.Context, repo AccountRepository, parentID string, proxyID *string) error {
	...
	for _, shadow := range shadows {
		...
		if err := repo.Update(ctx, shadow); err != nil {
			return fmt.Errorf("update spark shadow %v proxy: %w", shadow.ID, err)
		}
	}
	return nil
}

func (s *adminServiceImpl) validateGroupIDsExist(ctx context.Context, groupIDs []string) error {
	if len(groupIDs) == 0 {
		return nil
	}
	if s.groupRepo == nil {
...
func (s *adminServiceImpl) ResetAccountQuota(ctx context.Context, id string) error {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
		return err
	}
...
func (s *adminServiceImpl) ForceOpenAIPrivacy(ctx context.Context, account *Account) string {
	...
	if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{"privacy_mode": mode}); err != nil {
		logger.LegacyPrintf("service.admin", "force_update_openai_privacy_mode_failed: account_id=%v err=%v", account.ID, err)
		return mode
	}
	if account.Extra == nil {
		account.Extra = make(map[string]any)
...
// EnsureAntigravityPrivacy configures privacy once for an Antigravity OAuth account.
func (s *adminServiceImpl) EnsureAntigravityPrivacy(ctx context.Context, account *Account) string {
	...
	return s.setAndPersistAntigravityPrivacy(ctx, account, "update_antigravity_privacy_mode_failed")
}

// ForceAntigravityPrivacy reapplies privacy regardless of the stored state.
func (s *adminServiceImpl) ForceAntigravityPrivacy(ctx context.Context, account *Account) string {
	if account == nil || account.Platform != PlatformAntigravity || account.Type != AccountTypeOAuth {
		return ""
	}
	return s.setAndPersistAntigravityPrivacy(ctx, account, "force_update_antigravity_privacy_mode_failed")
}

func (s *adminServiceImpl) setAndPersistAntigravityPrivacy(ctx context.Context, account *Account, logKey string) string {
	...
	if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{"privacy_mode": mode}); err != nil {
		logger.LegacyPrintf("service.admin", "%s: account_id=%s err=%v", logKey, account.ID, err)
		return mode
	}
