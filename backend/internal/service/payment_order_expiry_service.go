...
const (
	...
	// paymentOrderExpiryLeaderLockTTL must exceed all bounded reconciliation and
	// expiry stages so the lock never expires mid-run.
...
func (s *PaymentOrderExpiryService) runOnce() {
	...
	attemptedRefunds, resolvedRefunds, refundErr := s.paymentSvc.ReconcilePendingPaymentRefunds(refundCtx, 25)
	refundCancel()
	if refundErr != nil {
		slog.Warn("[PaymentOrderExpiry] pending refund reconciliation completed with errors",
			"attempted", attemptedRefunds, "resolved", resolvedRefunds, "error", refundErr)
	} else if attemptedRefunds > 0 {
		slog.Info("[PaymentOrderExpiry] reconciled pending payment refunds",
			"attempted", attemptedRefunds, "resolved", resolvedRefunds)
	}
