...
func DescribeInvalidJSON(body []byte) error {
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		...
		if errors.As(err, &syntaxErr) {
			return fmt.Errorf("invalid json (len=%v, offset=%v): %s", len(body), syntaxErr.Offset, syntaxErr.Error())
		}
		return fmt.Errorf("invalid json (len=%v): %w", len(body), err)
	}
	...
	return fmt.Errorf("invalid json (len=%v)", len(body))
}
...
func NormalizeGLMOpenAIReasoningEffort(body []byte, mappedModel string) ([]byte, bool) {
	...
	// GLM-5.3 原生支持 low 档，不再把 low 抬到 high。
