...
func describeUsageCleanupFilters(filters UsageCleanupFilters) string {
	...
	if filters.UserID != nil {
		parts = append(parts, fmt.Sprintf("user_id=%v", *filters.UserID))
	}
	if filters.APIKeyID != nil {
		parts = append(parts, fmt.Sprintf("api_key_id=%v", *filters.APIKeyID))
	}
	if filters.AccountID != nil {
		parts = append(parts, fmt.Sprintf("account_id=%v", *filters.AccountID))
	}
	if filters.GroupID != nil {
		parts = append(parts, fmt.Sprintf("group_id=%v", *filters.GroupID))
	}
	...
	if filters.BillingType != nil {
		parts = append(parts, fmt.Sprintf("billing_type=%v", *filters.BillingType))
	}
...
func (s *UsageCleanupService) Start() {
	...
	s.startOnce.Do(func() {
		...
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] started (interval=%s max_range_days=%v batch_size=%v task_timeout=%s)", interval, s.maxRangeDays(), s.batchSize(), s.taskTimeout())
	})
}
...
func (s *UsageCleanupService) CreateTask(ctx context.Context, filters UsageCleanupFilters, createdBy string) (*UsageCleanupTask, error) {
	if s == nil || s.repo == nil {
	...
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] create_task requested: operator=%v %s", createdBy, describeUsageCleanupFilters(filters))
	sanitizeUsageCleanupFilters(&filters)
	if err := s.validateFilters(filters); err != nil {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] create_task rejected: operator=%v err=%v %s", createdBy, err, describeUsageCleanupFilters(filters))
		return nil, err
	}
	...
	if err := s.repo.CreateTask(ctx, task); err != nil {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] create_task persist failed: operator=%v err=%v %s", createdBy, err, describeUsageCleanupFilters(filters))
		return nil, fmt.Errorf("create cleanup task: %w", err)
	}
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] create_task persisted: task=%v operator=%v status=%s deleted_rows=%v %s", task.ID, createdBy, task.Status, task.DeletedRows, describeUsageCleanupFilters(filters))
	go s.runOnce()
	return task, nil
}
...
func (s *UsageCleanupService) runOnce() {
	...
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task claimed: task=%v status=%s created_by=%v deleted_rows=%v %s", task.ID, task.Status, task.CreatedBy, task.DeletedRows, describeUsageCleanupFilters(task.Filters))
...
func (s *UsageCleanupService) executeTask(ctx context.Context, task *UsageCleanupTask) {
	...
	start := time.Now()
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task started: task=%v batch_size=%v deleted_rows=%v %s", task.ID, batchSize, deletedTotal, describeUsageCleanupFilters(task.Filters))
	var batchNum int

	for {
		if ctx != nil && ctx.Err() != nil {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task interrupted: task=%v err=%v", task.ID, ctx.Err())
			return
		}
		...
		if canceled {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task canceled: task=%v deleted_rows=%v duration=%s", task.ID, deletedTotal, time.Since(start))
			return
		}
		...
		if err != nil {
			if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
				...
				logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task interrupted: task=%v err=%v", task.ID, err)
				return
			}
			s.markTaskFailed(task.ID, deletedTotal, err)
			return
		...
		if deleted > 0 {
			...
			if err := s.repo.UpdateTaskProgress(updateCtx, task.ID, deletedTotal); err != nil {
				logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task progress update failed: task=%v deleted_rows=%v err=%v", task.ID, deletedTotal, err)
			}
			cancel()
		}
		if batchNum <= 3 || batchNum%20 == 0 || deleted < int64(batchSize) {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task batch done: task=%v batch=%v deleted=%v deleted_total=%v", task.ID, batchNum, deleted, deletedTotal)
		}
	...
	if err := s.repo.MarkTaskSucceeded(updateCtx, task.ID, deletedTotal); err != nil {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] update task succeeded failed: task=%v err=%v", task.ID, err)
	} else {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task succeeded: task=%v deleted_rows=%v duration=%s", task.ID, deletedTotal, time.Since(start))
	}

	if s.dashboard != nil {
		if err := s.dashboard.TriggerRecomputeRange(task.Filters.StartTime, task.Filters.EndTime); err != nil {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] trigger dashboard recompute failed: task=%v err=%v", task.ID, err)
		} else {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] trigger dashboard recompute: task=%v start=%s end=%s", task.ID, task.Filters.StartTime.UTC().Format(time.RFC3339), task.Filters.EndTime.UTC().Format(time.RFC3339))
		}
	}
}

func (s *UsageCleanupService) markTaskFailed(taskID string, deletedRows int64, err error) {
	msg := strings.TrimSpace(err.Error())
	if len(msg) > 500 {
		msg = msg[:500]
	}
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task failed: task=%v deleted_rows=%v err=%s", taskID, deletedRows, msg)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if updateErr := s.repo.MarkTaskFailed(ctx, taskID, deletedRows, msg); updateErr != nil {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] update task failed failed: task=%v err=%v", taskID, updateErr)
	}
}

func (s *UsageCleanupService) isTaskCanceled(ctx context.Context, taskID string) (bool, error) {
	if s == nil || s.repo == nil {
	...
	if status == UsageCleanupStatusCanceled {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] task cancel detected: task=%v", taskID)
	}
...
func (s *UsageCleanupService) validateFilters(filters UsageCleanupFilters) error {
	...
	if maxDays > 0 {
		...
		if delta > time.Duration(maxDays)*24*time.Hour {
			return infraerrors.BadRequest("USAGE_CLEANUP_RANGE_TOO_LARGE", fmt.Sprintf("date range exceeds %v days", maxDays))
		}
	}
	return nil
}

func (s *UsageCleanupService) CancelTask(ctx context.Context, taskID string, canceledBy string) error {
	if s == nil || s.repo == nil {
	...
	canceledBy = strings.TrimSpace(canceledBy)
	if canceledBy == "" {
	...
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] cancel_task requested: task=%v operator=%v status=%s", taskID, canceledBy, status)
	if status == UsageCleanupStatusCanceled {
		logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] cancel_task idempotent hit: task=%v operator=%v", taskID, canceledBy)
		return nil
	}
	...
	if !ok {
		...
		if getErr == nil && currentStatus == UsageCleanupStatusCanceled {
			logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] cancel_task idempotent race hit: task=%v operator=%v", taskID, canceledBy)
			return nil
		}
		return infraerrors.New(http.StatusConflict, "USAGE_CLEANUP_CANCEL_CONFLICT", "cleanup task cannot be canceled in current status")
	}
	logger.LegacyPrintf("service.usage_cleanup", "[UsageCleanup] cancel_task done: task=%v operator=%v", taskID, canceledBy)
	return nil
}
...
func sanitizeUsageCleanupFilters(filters *UsageCleanupFilters) {
	...
	if filters.APIKeyID != nil && *filters.APIKeyID == "" {
		filters.APIKeyID = nil
	}
	if filters.AccountID != nil && *filters.AccountID == "" {
