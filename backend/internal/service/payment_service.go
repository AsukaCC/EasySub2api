...
func (s *PaymentService) SetWalletCacheInvalidators(billingCacheService *BillingCacheService, authCacheInvalidator APIKeyAuthCacheInvalidator) {
	s.billingCacheService = billingCacheService
	s.authCacheInvalidator = authCacheInvalidator
}

func (s *PaymentService) GetWalletSummary(ctx context.Context, userID string) (WalletSummary, error) {
	return s.userRepo.GetWalletSummary(ctx, userID)
}

func (s *PaymentService) ExpireBonusBalances(ctx context.Context, limit int) (int, error) {
	userIDs, err := s.userRepo.ExpireBonusBalances(ctx, limit)
	if err != nil {
