	...
	for index, rawTool := range tools {
		if customTool, ok := rawTool.(string); ok {
			if strings.TrimSpace(customTool) == "" {
				return false, fmt.Errorf("responses Lite custom tool at index %v must not be empty", index)
			}
		...
		if !ok {
			return false, fmt.Errorf("responses Lite tool at index %v must be an object", index)
		}
		...
		case "":
			return false, fmt.Errorf("responses Lite tool at index %v is missing type", index)
		default:
			return false, fmt.Errorf("responses Lite does not support top-level tool type %q at index %v", toolType, index)
		}
	}
