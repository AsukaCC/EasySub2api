...
func (s *BackupService) runScheduledBackup() {
	...
	schedule, err := s.GetSchedule(ctx)
	if err != nil {
		logger.LegacyPrintf("service.backup", "load backup schedule: %v", err)
		return
	}
	...
	logger.LegacyPrintf("service.backup", "[Backup] 开始执行定时备份, 过期天数: %v", expireDays)
	...
	logger.LegacyPrintf("service.backup", "[Backup] 定时备份完成: id=%s size=%v", record.ID, record.SizeBytes)
...
func (s *BackupService) createCompressedBackupFile(ctx context.Context) (string, int64, error) {
	...
	archive, err := os.CreateTemp("", "easysub2api-backup-*.sql.gz")
	if err != nil {
		_ = dumpReader.Close()
...
func (s *BackupService) uploadBackupArchive(ctx context.Context, record *BackupRecord, objectStore BackupObjectStore, cfg *BackupS3Config, archivePath string) error {
	...
	for i, part := range localParts {
		if _, err := objectStore.UploadFile(ctx, record.Parts[i].S3Key, part.Path, "application/octet-stream"); err != nil {
			...
			cleanupErr := deleteBackupObjectKeys(cleanupCtx, objectStore, record)
			cleanupCancel()
			return errors.Join(fmt.Errorf("upload backup part %v: %w", part.Index, err), cleanupErr)
		}
	}
	return nil
}
...
func (s *BackupService) downloadBackupParts(ctx context.Context, objectStore BackupObjectStore, parts []BackupPart) (path string, err error) {
	...
	for i, part := range ordered {
		if part.Index != i+1 || part.S3Key == "" || part.SizeBytes <= 0 {
			return "", fmt.Errorf("invalid backup part metadata at index %v", i+1)
		}
	}

	archive, err := os.CreateTemp("", "easysub2api-restore-*.sql.gz")
	if err != nil {
	...
	for _, part := range ordered {
		...
		if downloadErr != nil {
			cleanup()
			return "", fmt.Errorf("download backup part %v: %w", part.Index, downloadErr)
		}
		hash := sha256.New()
		...
		if copyErr != nil {
			cleanup()
			return "", fmt.Errorf("read backup part %v: %w", part.Index, copyErr)
		}
		if closeErr != nil {
			cleanup()
			return "", fmt.Errorf("close backup part %v: %w", part.Index, closeErr)
		}
		if written != part.SizeBytes {
			cleanup()
			return "", fmt.Errorf("backup part %v size mismatch: got %v, want %v", part.Index, written, part.SizeBytes)
		}
		if part.SHA256 != "" && !strings.EqualFold(part.SHA256, hex.EncodeToString(hash.Sum(nil))) {
			cleanup()
			return "", fmt.Errorf("backup part %v checksum mismatch", part.Index)
		}
	}
...
func (s *BackupService) GetBackupDownloadURL(ctx context.Context, backupID string) (BackupDownloadResponse, error) {
	...
	if len(record.Parts) > 0 {
		...
		for i, part := range parts {
			if part.Index != i+1 || part.S3Key == "" || part.SizeBytes <= 0 {
				return download, fmt.Errorf("invalid backup part metadata at index %v", i+1)
			}
			url, presignErr := objectStore.PresignURL(ctx, part.S3Key, 1*time.Hour)
			if presignErr != nil {
				return download, fmt.Errorf("presign backup part %v: %w", part.Index, presignErr)
			}
...
func (s *BackupService) cleanupOldBackups(ctx context.Context, schedule *BackupScheduleConfig) error {
	...
	if len(toDelete) > 0 || metadataChanged {
		...
		if deletedCount > 0 {
			logger.LegacyPrintf("service.backup", "[Backup] 自动清理了 %v 个过期备份", deletedCount)
		}
