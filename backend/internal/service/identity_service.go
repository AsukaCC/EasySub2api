...
type IdentityCache interface {
	GetFingerprint(ctx context.Context, accountID string) (*Fingerprint, error)
	SetFingerprint(ctx context.Context, accountID string, fp *Fingerprint) error
	// GetMaskedSessionID 获取固定的会话ID（用于会话ID伪装功能）
	// 返回的 sessionID 是一个 UUID 格式的字符串
	// 如果不存在或已过期（15分钟无请求），返回空字符串
	GetMaskedSessionID(ctx context.Context, accountID string) (string, error)
	// SetMaskedSessionID 设置固定的会话ID，TTL 为 15 分钟
	// 每次调用都会刷新 TTL
	SetMaskedSessionID(ctx context.Context, accountID string, sessionID string) error
}
...
func (s *IdentityService) GetOrCreateFingerprint(ctx context.Context, accountID string, headers http.Header) (*Fingerprint, error) {
	...
	if err == nil && cached != nil {
		...
		if !uaAcceptable && clientUA != "" && isNewerVersion(clientUA, cached.UserAgent) {
			logger.LegacyPrintf("service.identity",
				"Rejected fingerprint user-agent for account %v: %q (malformed or implausible version)",
				accountID, clientUA)
		}
		...
		if !isAcceptableFingerprintUserAgent(cached.UserAgent) {
			...
			if uaAcceptable {
				...
				cached.UserAgent = claude.DefaultUserAgent()
			}
			needWrite = true
			logger.LegacyPrintf("service.identity",
				"Replaced malformed cached fingerprint for account %v: %q -> %q",
		...
		if needWrite {
			...
			if err := s.cache.SetFingerprint(ctx, accountID, cached); err != nil {
				logger.LegacyPrintf("service.identity", "Warning: failed to refresh fingerprint for account %v: %v", accountID, err)
			}
		}
		return cached, nil
	}
	...
	if err := s.cache.SetFingerprint(ctx, accountID, fp); err != nil {
		logger.LegacyPrintf("service.identity", "Warning: failed to cache fingerprint for account %v: %v", accountID, err)
	}

	logger.LegacyPrintf("service.identity", "Created new fingerprint for account %v with client_id: %s", accountID, fp.ClientID)
	return fp, nil
}
...
func (s *IdentityService) RewriteUserID(body []byte, accountID string, accountUUID, cachedClientID, fingerprintUA string) ([]byte, error) {
	...
	seed := fmt.Sprintf("%v::%s", accountID, sessionTail)
...
func (s *IdentityService) RewriteUserIDWithMasking(ctx context.Context, body []byte, account *Account, accountUUID, cachedClientID, fingerprintUA string) ([]byte, error) {
	...
	if err != nil {
		logger.LegacyPrintf("service.identity", "Warning: failed to get masked session ID for account %v: %v", account.ID, err)
		return newBody, nil
	}
	...
	if maskedSessionID == "" {
		...
		logger.LegacyPrintf("service.identity", "Generated new masked session ID for account %v: %s", account.ID, maskedSessionID)
	}

	// 刷新 TTL（每次请求都刷新，保持 15 分钟有效期）
	if err := s.cache.SetMaskedSessionID(ctx, account.ID, maskedSessionID); err != nil {
		logger.LegacyPrintf("service.identity", "Warning: failed to set masked session ID for account %v: %v", account.ID, err)
	}
...
func generateRandomUUID() string {
	...
	if _, err := rand.Read(b); err != nil {
		// fallback: 使用时间戳生成
		h := sha256.Sum256([]byte(fmt.Sprintf("%v", time.Now().UnixNano())))
		b = h[:16]
	}
