...
func (s *AuthService) restoreOAuthRegistrationInvitation(ctx context.Context, invitationCode string, userID string) error {
	if s == nil || s.settingService == nil || !s.settingService.IsInvitationCodeEnabled(ctx) {
		return nil
	}
...
func (s *AuthService) useOAuthRegistrationInvitation(ctx context.Context, invitationID, userID string) error {
	if client := s.oauthEmailFlowClient(ctx); client != nil {
...
func (s *AuthService) updateOAuthSignupSource(ctx context.Context, userID string, signupSource string) {
	client := s.oauthEmailFlowClient(ctx)
	if client == nil || userID == "" || strings.TrimSpace(signupSource) == "" {
		return
	}
...
func (s *AuthService) RecordSuccessfulLogin(ctx context.Context, userID string) {
	if s != nil && s.userRepo != nil && userID != "" {
