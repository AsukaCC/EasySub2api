...
func GrokMediaVideoRequestSessionHash(requestID string, userID, apiKeyID string) string {
	requestID = strings.TrimSpace(requestID)
	if requestID == "" || userID == "" || apiKeyID == "" {
		return ""
	}
	ownerSeed := fmt.Sprintf("%v:%v:%s", userID, apiKeyID, requestID)
...
func (s *OpenAIGatewayService) BindGrokMediaVideoRequestAccount(
	...
	groupID *string,
	requestID string,
	userID, apiKeyID, accountID string,
) error {
...
func grokVideoPendingBillingKey(requestID string, userID, apiKeyID string) string {
	requestID = strings.TrimSpace(requestID)
	if requestID == "" || userID == "" || apiKeyID == "" {
		return ""
	}
	return fmt.Sprintf("%v:%v:%s", userID, apiKeyID, requestID)
}
...
func grokMediaSignedVideoContentURL(body []byte, requestID string) (string, error) {
	...
	// An upstream EasySub2api rewrites protected content URLs to its own proxy
...
func prepareGrokMediaForwardBody(endpoint GrokMediaEndpoint, body []byte, contentType string) ([]byte, string, error) {
	...
	if len(images) > grokMediaMaxEditSourceImages {
		return nil, "", fmt.Errorf("a maximum of %v source images is supported for image edits", grokMediaMaxEditSourceImages)
	}
	if len(images) > 0 {
	...
	for _, field := range fields {
		...
		if value.IsArray() {
			for index := range value.Array() {
				var err error
				out, err = canonicalizeGrokMediaImageURLObject(out, fmt.Sprintf("%s.%v", field, index))
				if err != nil {
					return nil, err
				}
			}
...
func (s *OpenAIGatewayService) handleGrokMediaErrorResponse(
	...
	if upstreamMsg == "" {
		upstreamMsg = fmt.Sprintf("xAI upstream returned status %v", resp.StatusCode)
	}
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		return nil, fmt.Errorf("upstream error: %v (passthrough rule matched) message=%s", resp.StatusCode, upstreamMsg)
	}
	...
	if !account.ShouldHandleErrorCode(resp.StatusCode) {
		...
		return nil, fmt.Errorf("upstream error: %v (not in custom error codes) message=%s", resp.StatusCode, upstreamMsg)
	}
	...
	return nil, fmt.Errorf("upstream error: %v %s", resp.StatusCode, upstreamMsg)
}
