...
func userFeatureAvailable(values map[string]string, enabledKey, visibleKey string, enabledDefault bool) bool {
	enabledValue, enabledExists := values[enabledKey]
	enabled := enabledDefault
	if enabledExists && strings.TrimSpace(enabledValue) != "" {
		enabled = enabledValue == "true"
	}
	if !enabled {
		return false
	}
	visibleValue, visibleExists := values[visibleKey]
	if !visibleExists || strings.TrimSpace(visibleValue) == "" {
		return true
	}
	return visibleValue == "true"
}

func (s *SettingService) isUserFeatureAvailable(ctx context.Context, enabledKey, visibleKey string, enabledDefault bool) bool {
	if s == nil || s.settingRepo == nil {
		return false
	}
	values, err := s.settingRepo.GetMultiple(ctx, []string{enabledKey, visibleKey})
	if err != nil {
		return false
	}
	return userFeatureAvailable(values, enabledKey, visibleKey, enabledDefault)
}

func (s *SettingService) IsChannelMonitorUserAvailable(ctx context.Context) bool {
	return s.isUserFeatureAvailable(ctx, SettingKeyChannelMonitorEnabled, SettingKeyChannelMonitorUserVisible, true)
}

func (s *SettingService) IsAvailableChannelsUserAvailable(ctx context.Context) bool {
	return s.isUserFeatureAvailable(ctx, SettingKeyAvailableChannelsEnabled, SettingKeyAvailableChannelsUserVisible, false)
}

func (s *SettingService) IsModelPlazaUserAvailable(ctx context.Context) bool {
	return s.isUserFeatureAvailable(ctx, SettingKeyModelPlazaEnabled, SettingKeyModelPlazaUserVisible, false)
}

func (s *SettingService) IsPaymentUserAvailable(ctx context.Context) bool {
...
// rebate freeze policy remains permanently fixed at seven days.
...
func (s *SettingService) GetBonusBalanceDefaultValidityDays(ctx context.Context) int {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyBonusBalanceDefaultValidityDays)
	if err == nil {
...
func (s *SettingService) GetAuthSourceDefaultSettings(ctx context.Context) (*AuthSourceDefaultSettings, error) {
	keys := []string{
		...
		SettingKeyAuthSourceDefaultEmailBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultLinuxDoBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultOIDCBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultWeChatBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultGitHubBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultGoogleBonusValidityDays,
		...
		SettingKeyAuthSourceDefaultDingTalkBonusValidityDays,
...
func (s *SettingService) ResolveAuthSourceGrantSettings(ctx context.Context, signupSource string, firstBind bool) (ProviderDefaultGrantSettings, bool, error) {
	result := ProviderDefaultGrantSettings{
		...
		BonusValidityDays: s.GetBonusBalanceDefaultValidityDays(ctx),
...
func (s *SettingService) SetBetaPolicySettings(ctx context.Context, settings *BetaPolicySettings) error {
	...
	for i, rule := range settings.Rules {
		if rule.BetaToken == "" {
			return fmt.Errorf("rule[%v]: beta_token cannot be empty", i)
		}
		if !validActions[rule.Action] {
			return fmt.Errorf("rule[%v]: invalid action %q", i, rule.Action)
		}
		if !validScopes[rule.Scope] {
			return fmt.Errorf("rule[%v]: invalid scope %q", i, rule.Scope)
		}
		// Validate model_whitelist patterns
		for j, pattern := range rule.ModelWhitelist {
			trimmed := strings.TrimSpace(pattern)
			if trimmed == "" {
				return fmt.Errorf("rule[%v]: model_whitelist[%v] cannot be empty", i, j)
			}
			settings.Rules[i].ModelWhitelist[j] = trimmed
		}
		// Validate fallback_action
		if rule.FallbackAction != "" && !validActions[rule.FallbackAction] {
			return fmt.Errorf("rule[%v]: invalid fallback_action %q", i, rule.FallbackAction)
		}
	}
...
func (s *SettingService) SetOpenAIFastPolicySettings(ctx context.Context, settings *OpenAIFastPolicySettings) error {
	...
	for i, rule := range settings.Rules {
		...
		if !validTiers[tier] {
			return fmt.Errorf("rule[%v]: invalid service_tier %q", i, rule.ServiceTier)
		}
		...
		seenUserIDs := make(map[string]struct{}, len(rule.UserIDs))
		for j, userID := range rule.UserIDs {
			if userID == "" {
				return fmt.Errorf("rule[%v]: user_ids[%v] must be positive", i, j)
			}
			if _, exists := seenUserIDs[userID]; exists {
				return fmt.Errorf("rule[%v]: user_ids[%v] duplicates user_id %v", i, j, userID)
			}
