...
type grokCredentialConditionalStateRepository interface {
	SetGrokCredentialErrorIfMatch(context.Context, string, GrokCredentialMutationSnapshot, string) (bool, error)
	SetGrokCredentialTempUnschedulableIfMatch(context.Context, string, GrokCredentialMutationSnapshot, time.Time, string) (bool, error)
}
...
func (s *OpenAIGatewayService) validateCurrentGrokCredentialFailure(
	...
	if s == nil || s.accountRepo == nil || accountID == "" || ctx == nil || ctx.Err() != nil {
		if ctx != nil {
			return "", ctx.Err()
		}
		return "", context.Canceled
...
func (s *OpenAIGatewayService) grokCredentialMutationLock(accountID string) *oauthRefreshLocalLock {
...
func (s *OpenAIGatewayService) grokCredentialMutationCommitted(accountID string, class grokCredentialFailureClass, until time.Time) bool {
...
func (s *OpenAIGatewayService) resolveGrokCredentialCASMiss(ctx context.Context, accountID string, snapshot GrokCredentialMutationSnapshot) (string, error) {
	if ctx.Err() != nil {
		return "", ctx.Err()
	}
	if token, ok := s.grokCredentialConcurrentlyRefreshedToken(ctx, accountID, snapshot); ok {
...
func (s *OpenAIGatewayService) grokCredentialConcurrentlyRefreshedToken(ctx context.Context, accountID string, baseline GrokCredentialMutationSnapshot) (string, bool) {
	if s == nil || s.accountRepo == nil || accountID == "" || ctx == nil || ctx.Err() != nil {
		return "", false
	}
	checkCtx, cancel := context.WithTimeout(ctx, time.Second)
...
func grokCredentialProxyIDsEqual(left, right *string) bool {
