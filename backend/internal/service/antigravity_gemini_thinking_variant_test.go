...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		// 首选档位不存在时，按 high → medium → low → tiered 降级。
		...
		{"explicit mapping missing low falls back to high", catalog, "gemini-3.6-flash", budget("1000"), "gemini-3.6-flash-high", true},
		...
		{"bare passthrough absent from credentials: still resolved", map[string]string{
			"gemini-3.7-flash-medium": "gemini-3.7-flash-medium",
		}, "gemini-3.7-flash", budget("4000"), "gemini-3.7-flash-medium", true},
		{"empty mapping falls back to default catalog", map[string]string{}, "gemini-3.6-flash", budget("1000"), "gemini-3.6-flash-low", true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
