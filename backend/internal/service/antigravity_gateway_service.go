...
func (e *AntigravityAccountSwitchError) Error() string {
	return fmt.Sprintf("account %s model %s rate limited, need switch",
...
func (s *AntigravityGatewayService) handleAntigravityModelRateLimitBeforePolicy(p antigravityRetryLoopParams, statusCode int, headers http.Header, respBody []byte) bool {
	...
	logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d model_rate_limited_before_error_policy model=%s account=%s reset_in=%v",
...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string) (*TestConnectionResult, error) {
	...
	prefix := fmt.Sprintf("[antigravity-Test] account=%s(%s)", account.ID, account.Name)
...
func testConnectionHandleError(
	...
	requestedModel string, _ string, _ string, _ bool,
) *handleModelRateLimitResult {
	logger.LegacyPrintf("service.antigravity_gateway",
		"%s test_handle_error status=%d model=%s account=%s body=%s",
