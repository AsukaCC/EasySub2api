...
func liveGroupID(groupID *string) string {
	if groupID == nil {
		return ""
	}
	return *groupID
...
func logLiveCreateStageFailure(ctx context.Context, accountID string, stage string, err error) {
...
func (s *OpenAIGatewayService) dialLiveSideband(ctx context.Context, record *LiveCallRecord) (liveFrameConn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dial live sideband (status %v): %w", status, err)
	}
...
func (s *OpenAIGatewayService) releaseLiveLease(accountID, userID, apiKeyID string, leaseID string) {
	cache, err := s.liveConcurrencyCache()
	if err != nil {
		return
	}
