...
type OAuthRefreshAPI struct {
	...
	tokenCache  OAuthTokenCache // 可选，nil = 无分布式锁
...
func NewOAuthRefreshAPI(accountRepo AccountRepository, tokenCache OAuthTokenCache, lockTTL ...time.Duration) *OAuthRefreshAPI {
...
func (api *OAuthRefreshAPI) loadGrokDurableAccountAfterPersist(parent context.Context, cacheKey string, accountID string) (*Account, error) {
	cleanupParent := context.Background()
	if parent != nil {
		cleanupParent = context.WithoutCancel(parent)
	}
