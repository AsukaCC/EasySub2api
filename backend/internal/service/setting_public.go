...
func normalizeLoginAgreementDocuments(docs []LoginAgreementDocument) []LoginAgreementDocument {
	...
	for i, doc := range docs {
		...
		if id == "" {
			sum := sha256.Sum256([]byte(fmt.Sprintf("%v:%s:%s", i, title, content)))
			id = hex.EncodeToString(sum[:])[:12]
		}
		baseID := id
		for suffix := 2; seen[id] > 0; suffix++ {
			id = fmt.Sprintf("%s-%v", baseID, suffix)
		}
		seen[id]++
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		InvitationCodeEnabled: settings[SettingKeyInvitationCodeEnabled] == "true" &&
			userFeatureAvailable(settings, SettingKeyAffiliateEnabled, SettingKeyAffiliateUserVisible, false),
		...
		PaymentEnabled:              userFeatureAvailable(settings, SettingPaymentEnabled, SettingPaymentUserVisible, false),
		...
		ChannelMonitorEnabled:                userFeatureAvailable(settings, SettingKeyChannelMonitorEnabled, SettingKeyChannelMonitorUserVisible, true),
...
// normalizeChannelMonitorMode keeps the legacy field API-compatible while
// permanently selecting passive monitoring.
...
// dormant legacy service and always fails closed.
func (r ChannelMonitorRuntime) ActiveProbesAllowed() bool {
	return false
}

// Passive aggregation is the only channel monitoring implementation.
func (r ChannelMonitorRuntime) PassiveAggregationAllowed() bool {
	return r.Enabled
}

// GetChannelMonitorRuntime reads the channel monitor feature flags directly from
// the settings store. Fail-open: on error returns Enabled=true in passive mode.
