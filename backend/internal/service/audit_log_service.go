...
func (s *AuditLogService) ClearAll(ctx context.Context, trace *AuditLog) (int64, error) {
	...
	if trace != nil {
		...
		if err := s.repo.Insert(ctx, trace); err != nil {
			...
			return deleted, fmt.Errorf("audit logs cleared (%v rows) but failed to persist clear-trace record: %w", deleted, err)
		}
	}
	return deleted, nil
}
...
func (s *AuditLogService) runWriter() {
	...
	flush := func() {
		...
		if err != nil {
			...
			_, _ = fmt.Fprintf(os.Stderr, "time=%s level=WARN msg=\"audit log flush failed\" err=%v batch=%v\n",
