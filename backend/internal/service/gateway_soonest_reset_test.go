...
func accWithWindowEnd(id string, end *time.Time) accountWithLoad {
...
func TestFilterBySoonestReset_PicksSoonestFutureWindow(t *testing.T) {
	...
	require.Len(t, got, 1)
	require.Equal(t, "2", got[0].account.ID, "重置时间最早的账号被选中")
}
...
func TestFilterBySoonestReset_IgnoresNilAndExpiredWindows(t *testing.T) {
	...
	got := filterBySoonestReset(accounts)
	require.Len(t, got, 1)
	require.Equal(t, "3", got[0].account.ID, "仅保留拥有未来重置时间的账号")
}
...
func TestFilterBySoonestReset_TiedSoonestKeepsAll(t *testing.T) {
	...
	ids := map[string]bool{got[0].account.ID: true, got[1].account.ID: true}
