...
func (s *stubCredRepo) GetByID(_ context.Context, _ string) (*Account, error) {
	return s.parent, nil
}
...
func TestResolveCredentialAccount(t *testing.T) {
	...
	// 普通账号（非影子）→ 返回自身
	parent := &Account{ID: "account-100", Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive}
	repo := newStubCredRepo(parent)
	got, err := resolveCredentialAccount(ctx, repo, parent)
	require.NoError(t, err)
	require.Equal(t, "account-100", got.ID)

	// 影子账号 + 合法 OpenAI OAuth 母账号 → 返回母账号
	shadow := &Account{ID: "account-200", ParentAccountID: &pid, Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	got, err = resolveCredentialAccount(ctx, repo, shadow)
	require.NoError(t, err)
	require.Equal(t, "account-100", got.ID)

	// 影子账号 + 母账号非 OpenAI OAuth（API Key 类型）→ 返回 error
	badRepo := newStubCredRepo(&Account{ID: "account-100", Platform: PlatformOpenAI, Type: AccountTypeAPIKey})
