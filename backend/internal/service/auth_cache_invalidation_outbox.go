...
type AuthCacheInvalidationOutboxRepository interface {
	...
	DeleteClaimed(ctx context.Context, id string, workerID string) error
	ScheduleSecondPass(ctx context.Context, id string, workerID string, availableAt time.Time) error
	RetryClaimed(ctx context.Context, id string, workerID string, availableAt time.Time, lastError string) error
...
func (w *AuthCacheInvalidationWorker) processEvent(parent context.Context, event AuthCacheInvalidationEvent) {
	...
	if err != nil {
		...
		retryCancel()
		if retryErr != nil {
			w.recordFailure(fmt.Errorf("release failed auth invalidation %v: %w", event.ID, retryErr))
		}
		return
	}
	if event.Stage == 0 {
		...
		nextCancel()
		if err != nil {
			w.recordFailure(fmt.Errorf("schedule second auth invalidation pass %v: %w", event.ID, err))
			return
		}
		w.processed.Add(1)
		w.lastError.Store("")
	...
	ackCancel()
	if err != nil {
		w.recordFailure(fmt.Errorf("ack auth invalidation %v: %w", event.ID, err))
		return
	}
	w.processed.Add(1)
	w.lastError.Store("")
