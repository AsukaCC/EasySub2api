...
func openAIWSExecutionScopeSeed(apiKeyID string, identity, value, lane string) string {
	seed, _ := json.Marshal([4]string{apiKeyID, identity, value, lane})
	return "openai_ws_exec:" + string(seed)
}
...
// 不是可靠身份，不参与抢占。
func resolveOpenAIWSExecutionScope(c *gin.Context, body []byte, apiKeyID string) (scope, threadID string) {
