...
func (s *AntiDegradeService) transition(ctx context.Context, id string, plan func(*AntiDegradeService) error) (*Account, error) {
	...
	planner := &AntiDegradeService{admin: store, cfg: s.cfg}
...
func (s *protectionDraftStore) GetAccount(context.Context, string) (*Account, error) {
	a := *s.account
	a.Extra = maps.Clone(a.Extra)
	return &a, nil
}

func (s *protectionDraftStore) UpdateAccount(_ context.Context, _ string, in *UpdateAccountInput) (*Account, error) {
	a := *s.account
	if in.Extra != nil {
