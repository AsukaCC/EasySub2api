...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	logOpenAIWSModeDebug(
		"dial_target account_id=%v account_type=%s ws_host=%s ws_path=%s",
		account.ID,
		account.Type,
		wsHost,
		wsPath,
	...
	if s.shouldEmitOpenAIWSPayloadSchema(attempt) {
		logOpenAIWSModeInfo(
			"[debug] payload_schema account_id=%v attempt=%v event=%s payload_keys=%s payload_bytes=%v payload_key_sizes=%s input_summary=%s stream=%s payload_strategy=%s removed_keys=%s has_previous_response_id=%v has_prompt_cache_key=%v has_tools=%v",
			account.ID,
			attempt,
			payloadEventType,
	...
	applyCodexAccountIdentityClientMetadataMap(payload, codexAccountIdentitySource(c, account), "")
	logOpenAIWSModeDebug(
		"acquire_start account_id=%v account_type=%s transport=%s preferred_conn_id=%s has_previous_response_id=%v session_hash=%s has_turn_state=%v turn_state_len=%v has_turn_metadata=%v turn_metadata_len=%v store_disabled=%v store_disabled_conn_mode=%s retry_last_reason=%s force_new_conn=%v header_user_agent=%s header_openai_beta=%s header_originator=%s header_accept_language=%s header_session_id=%s header_conversation_id=%s session_id_source=%s conversation_id_source=%s has_prompt_cache_key=%v has_chatgpt_account_id=%v has_authorization=%v has_session_id=%v has_conversation_id=%v proxy_enabled=%v",
		account.ID,
		account.Type,
	...
	if err != nil {
		...
		logOpenAIWSModeInfo(
			"acquire_fail account_id=%v account_type=%s transport=%s reason=%s dial_status=%v dial_class=%s dial_close_status=%s dial_close_reason=%s dial_resp_server=%s dial_resp_via=%s dial_resp_cf_ray=%s dial_resp_x_request_id=%s cause=%s preferred_conn_id=%s force_new_conn=%v ws_host=%s ws_path=%s proxy_enabled=%v",
			account.ID,
			account.Type,
			normalizeOpenAIWSLogValue(string(decision.Transport)),
	...
	logOpenAIWSModeDebug(
		"connected account_id=%v account_type=%s transport=%s conn_id=%s conn_reused=%v conn_pick_ms=%v queue_wait_ms=%v has_previous_response_id=%v",
		account.ID,
		account.Type,
		normalizeOpenAIWSLogValue(string(decision.Transport)),
		connID,
	...
	if previousResponseID != "" {
		logOpenAIWSModeInfo(
			"continuation_probe account_id=%v account_type=%s conn_id=%s previous_response_id=%s previous_response_id_kind=%s preferred_conn_id=%s conn_reused=%v store_disabled=%v session_hash=%s header_session_id=%s header_conversation_id=%s session_id_source=%s conversation_id_source=%s has_turn_state=%v turn_state_len=%v has_prompt_cache_key=%v",
			account.ID,
			account.Type,
	...
	logOpenAIWSModeDebug(
		"handshake account_id=%v conn_id=%s has_turn_state=%v turn_state_len=%v",
		account.ID,
		connID,
		handshakeTurnState != "",
	...
	if err := lease.WriteJSONWithContextTimeout(ctx, payload, s.openAIWSWriteTimeout()); err != nil {
		lease.MarkBroken()
		logOpenAIWSModeInfo(
			"write_request_fail account_id=%v conn_id=%s cause=%s payload_bytes=%v",
			account.ID,
			connID,
			truncateOpenAIWSLogValue(err.Error(), openAIWSLogValueMaxLen),
			resolvePayloadBytes(),
	...
	if debugEnabled {
		logOpenAIWSModeDebug(
			"write_request_sent account_id=%v conn_id=%s stream=%v payload_bytes=%v previous_response_id=%s",
			account.ID,
			connID,
			reqStream,
			resolvePayloadBytes(),
	...
	markClientDisconnected := func(cause string) {
		...
		logOpenAIWSModeInfo(
			"client_disconnected account_id=%v conn_id=%s cause=%s events=%d token_events=%d",
			account.ID,
			connID,
			cause,
			eventCount,
	...
	resultWithUsage := func() *OpenAIForwardResult {
		return &OpenAIForwardResult{
			...
			NativeCompactionV2:            hasCompactionTriggerInRequestMap(reqBody),
			Stream:                        reqStream,
			OpenAIWSMode:                  true,
	...
	emitStreamMessage := func(message []byte, forceFlush bool) {
		...
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI WS Mode] client disconnected, continue draining upstream: account=%v", account.ID)
	}
	flushBufferedStreamEvents := func(reason string) {
		...
		if debugEnabled {
			logOpenAIWSModeDebug(
				"buffer_flush account_id=%v conn_id=%s reason=%s flushed=%v total_flushed=%v client_disconnected=%v",
				account.ID,
				connID,
...
readLoop:
	for {
		...
		if len(pendingJSONDocuments) > 0 {
			...
			if readErr == nil {
				if documents, repaired := splitOpenAIConcatenatedJSONDocuments(message); repaired {
					logOpenAIWSModeInfo(
						"concatenated_json_repaired account_id=%v conn_id=%s documents=%v bytes=%v",
						account.ID,
						truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
						len(documents),
						len(message),
		...
		if readErr == nil && !json.Valid(message) {
			...
			lease.MarkBroken()
			logOpenAIWSModeInfo(
				"invalid_event_json account_id=%v conn_id=%s event_type=%s bytes=%v wrote_downstream=%v",
				account.ID,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
		...
		if readErr != nil {
			...
			logOpenAIWSModeInfo(
				"read_fail account_id=%v conn_id=%s wrote_downstream=%v close_status=%s close_reason=%s cause=%s events=%v token_events=%v terminal_events=%v buffered_pending=%v buffered_flushed=%v first_event=%s last_event=%s",
				account.ID,
				connID,
				wroteDownstream,
				closeStatus,
		...
		if debugEnabled && shouldLogOpenAIWSEvent(eventCount, eventType) {
			logOpenAIWSModeDebug(
				"event_received account_id=%v conn_id=%s idx=%v type=%s bytes=%v token=%v terminal=%v buffered_pending=%v",
				account.ID,
				connID,
				eventCount,
				truncateOpenAIWSLogValue(eventType, openAIWSLogValueMaxLen),
		...
		if eventType == "error" {
			...
			logOpenAIWSModeInfo(
				"error_event account_id=%v conn_id=%s idx=%v fallback_reason=%s can_fallback=%v err_code=%s err_type=%s err_message=%s",
				account.ID,
				connID,
				eventCount,
			...
			if fallbackReason == "previous_response_not_found" {
				logOpenAIWSModeInfo(
					"previous_response_not_found_diag account_id=%v account_type=%s conn_id=%s previous_response_id=%s previous_response_id_kind=%s response_id=%s event_idx=%v req_stream=%v store_disabled=%v conn_reused=%v session_hash=%s header_session_id=%s header_conversation_id=%s session_id_source=%s conversation_id_source=%s has_turn_state=%v turn_state_len=%v has_prompt_cache_key=%v err_code=%s err_type=%s err_message=%s",
					account.ID,
					account.Type,
					connID,
					truncateOpenAIWSLogValue(previousResponseID, openAIWSIDValueMaxLen),
		...
		if reqStream {
			...
			if shouldBuffer {
				...
				if debugEnabled && shouldLogOpenAIWSBufferedEvent(bufferedEventCount) {
					logOpenAIWSModeDebug(
						"buffer_enqueue account_id=%v conn_id=%s idx=%v event_idx=%v event_type=%s buffer_size=%v",
						account.ID,
						connID,
						bufferedEventCount,
						eventCount,
	...
	if !reqStream {
		...
		if len(finalResponse) == 0 {
			logOpenAIWSModeInfo(
				"missing_final_response account_id=%v conn_id=%s events=%v token_events=%v terminal_events=%v wrote_downstream=%v",
				account.ID,
				connID,
				eventCount,
				tokenEventCount,
	...
	logOpenAIWSModeDebug(
		"completed account_id=%v conn_id=%s response_id=%s stream=%v duration_ms=%v events=%v token_events=%v terminal_events=%v buffered_events=%v buffered_flushed=%v first_event=%s last_event=%s first_token_ms=%v wrote_downstream=%v client_disconnected=%v",
		account.ID,
		connID,
