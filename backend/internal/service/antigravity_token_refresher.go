...
func (r *AntigravityTokenRefresher) NeedsRefresh(account *Account, _ time.Duration) bool {
	...
	if needsRefresh {
		fmt.Printf("[AntigravityTokenRefresher] Account %s needs refresh: expires_at=%s, time_until_expiry=%v, window=%v\n",
...
func (r *AntigravityTokenRefresher) Refresh(ctx context.Context, account *Account) (map[string]any, error) {
	...
	if tokenInfo.ProjectIDMissing {
		if tokenInfo.ProjectID != "" {
			...
			log.Printf("[AntigravityTokenRefresher] Account %s: LoadCodeAssist 临时失败，保留旧 project_id", account.ID)
		} else {
			// 从未获取过 project_id，本次也失败，但不返回错误以允许下次重试
			log.Printf("[AntigravityTokenRefresher] Account %s: LoadCodeAssist 失败，project_id 缺失，但 token 已更新，将在下次刷新时重试", account.ID)
		}
	}
