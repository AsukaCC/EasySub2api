...
type ChannelMonitorV2MatrixRow struct {
	...
	// RateMultiplier is the configured base multiplier for the group. It is
	// included for the public group status view; user-specific level/dynamic
	// overrides are intentionally not resolved without an authenticated user.
...
func (s *ChannelMonitorV2Service) UpdateConfig(ctx context.Context, cfg ChannelMonitorV2Config, expectedVersion int, actorID string) (*ChannelMonitorV2Config, error) {
	cfg.Enabled = true
...
func (s *ChannelMonitorV2Service) ParseFilter(rangeValue string, platforms, models []string, groupIDs []string) (ChannelMonitorV2Filter, error) {
	now := s.now().UTC()
	var window, bucket time.Duration
	switch strings.TrimSpace(rangeValue) {
	case "", "90m":
		rangeValue, window, bucket = "90m", 90*time.Minute, time.Minute
	case "24h":
		window, bucket = 24*time.Hour, time.Hour
	case "7d":
		window, bucket = 7*24*time.Hour, 2*time.Hour
	case "14d":
		window, bucket = 14*24*time.Hour, 2*time.Hour
	case "30d":
	...
	return ChannelMonitorV2Filter{
		Range: rangeValue, Platforms: normalizeStringSet(platforms), Models: normalizeStringSet(models), GroupIDs: normalizeEntityIDSet(groupIDs),
...
func (s *ChannelMonitorV2Service) Users(ctx context.Context, filter ChannelMonitorV2Filter, viewerID string, admin bool) (*ChannelMonitorV2List[ChannelMonitorV2UserRow], error) {
	cfg, err := s.getEnabledConfig(ctx)
	if err != nil {
		return nil, err
	}
	...
	for i := range result.Items {
		...
		row.DisplayLabel = fmt.Sprintf("Other user #%v", i+1)
	}
	return result, nil
}
...
func normalizeChannelMonitorV2Config(cfg *ChannelMonitorV2Config) error {
	...
	for i := range cfg.Platforms {
		...
		if _, ok := channelMonitorV2SupportedPlatforms[p.Platform]; !ok {
			return fmt.Errorf("%w: unsupported platform %s", ErrChannelMonitorV2InvalidConfig, p.Platform)
		}
...
func normalizeEntityIDSet(values []string) []string {
	seen := make(map[string]struct{}, len(values))
	out := make([]string, 0, len(values))
	for _, value := range values {
		if value == "" {
...
func normalizeChannelMonitorV2GroupIDs(values []string) ([]string, error) {
	seen := make(map[string]struct{}, len(values))
	out := make([]string, 0, len(values))
	for _, value := range values {
		if value == "" {
			return nil, fmt.Errorf("%w: group_ids must be non-empty", ErrChannelMonitorV2InvalidConfig)
		}
		if _, ok := seen[value]; ok {
			continue
		}
