...
func TestNormalizeCodexModelsManifestConfig(t *testing.T) {
	cfg := GroupCodexModelsManifestConfig{
		Enabled:             true,
		AccountIDs:          []string{" acc-1 ", "", "acc-2", "acc-1"},
		FallbackToScheduler: true,
	}

	out := normalizeCodexModelsManifestConfig(PlatformOpenAI, cfg)
	require.True(t, out.Enabled)
	require.True(t, out.FallbackToScheduler)
	require.Equal(t, []string{"acc-1", "acc-2"}, out.AccountIDs, "去重、去空白并保持首次出现顺序")

	require.Equal(t, GroupCodexModelsManifestConfig{}, normalizeCodexModelsManifestConfig(PlatformAnthropic, cfg), "非 openai 平台归零")

	empty := normalizeCodexModelsManifestConfig(PlatformOpenAI, GroupCodexModelsManifestConfig{Enabled: true, AccountIDs: []string{" ", ""}})
	require.Nil(t, empty.AccountIDs)
}
