...
func ProvideClaudeCodeVersionSyncService(repo SettingRepository, settings *SettingService, client GitHubReleaseClient) *ClaudeCodeVersionSyncService {
	svc := NewClaudeCodeVersionSyncService(repo, settings, client, claudeCodeVersionSyncInterval)
	svc.Start()
	return svc
}
...
// External polling requires an explicit opt-in; missing settings fail closed.
