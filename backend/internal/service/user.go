...
type User struct {
	...
	NextBonusExpiresAt         *time.Time
	NextExpiringBonusAmount    float64
	InitialBalanceKind         string
	InitialBonusValidityDays   int
...
func (u *User) CanBindGroup(groupID string, isExclusive bool) bool {
	// 公开分组（非专属）：所有用户都可以绑定
	if !isExclusive {
		return true
	}
