...
const (
	...
	githubRepo     = "AsukaCC/EasySub2api"
...
func (s *UpdateService) applyReleaseAssets(ctx context.Context, releaseAssets []Asset) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".easysub2api-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "easysub2api")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "easysub2api" || baseName == "easysub2api.exe" || baseName == "sub2api" || baseName == "sub2api.exe" {
				// Additional security: limit file size (max 500MB)
				const maxBinarySize = 500 * 1024 * 1024
				if hdr.Size > maxBinarySize {
					return fmt.Errorf("binary too large: %v bytes (max %v)", hdr.Size, maxBinarySize)
				}
