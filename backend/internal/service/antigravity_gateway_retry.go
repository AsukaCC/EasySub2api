...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldRateLimitModel {
		...
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d oauth_long_delay model=%s account=%s upstream_retry_delay=%v body=%s (model rate limit, switch account)",
		...
		if !s.setAntigravityModelRateLimits(p.ctx, p.accountRepo, p.account, modelName, p.prefix, resp.StatusCode, resetAt, false) {
			...
			logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d rate_limited account=%s (no model mapping)", p.prefix, resp.StatusCode, p.account.ID)
		}
	...
	if shouldSmartRetry {
		...
		if isModelCapacityExhausted {
			...
			if modelName != "" {
				...
				if exists && time.Now().Before(cooldownUntil) {
					log.Printf("%s status=%d model_capacity_exhausted_dedup model=%s account=%s cooldown_until=%v (skip retry)",
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			log.Printf("%s status=%d oauth_smart_retry attempt=%d/%d delay=%v model=%s account=%s",
		...
		if isModelCapacityExhausted {
			...
			log.Printf("%s status=%d smart_retry_exhausted_model_capacity attempts=%d model=%s account=%s body=%s (model capacity exhausted, not switching account)",
		...
		if resp.StatusCode == http.StatusServiceUnavailable && isSingleAccountRetry(p.ctx) {
			logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d smart_retry_exhausted_single_account attempts=%d model=%s account=%s body=%s (return 503 directly)",
		...
		log.Printf("%s status=%d smart_retry_exhausted attempts=%d model=%s account=%s upstream_retry_delay=%v body=%s (switch account)",
...
func (s *AntigravityGatewayService) handleSingleAccountRetryInPlace(
	...
	logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d single_account_503_retry_in_place model=%s account=%s upstream_retry_delay=%v (retrying in-place instead of rate-limiting)",
	...
	for attempt := 1; attempt <= antigravitySingleAccountSmartRetryMaxAttempts; attempt++ {
		...
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d single_account_503_retry attempt=%d/%d delay=%v total_waited=%v model=%s account=%s",
	...
	logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d single_account_503_retry_exhausted attempts=%d total_waited=%v model=%s account=%s body=%s (return 503 directly)",
...
func (s *AntigravityGatewayService) antigravityRetryLoop(p antigravityRetryLoopParams) (*antigravityRetryLoopResult, error) {
	...
	if p.requestedModel != "" && p.account.Platform == PlatformAntigravity &&
		...
		if creditsBody := injectEnabledCreditTypes(p.body); creditsBody != nil {
			...
			logger.LegacyPrintf("service.antigravity_gateway", "%s pre_check: model_rate_limited_credits_inject model=%s account=%s (injecting enabledCreditTypes)",
	...
	if p.requestedModel != "" {
		if remaining := p.account.GetRateLimitRemainingTimeWithContext(p.ctx, p.requestedModel); remaining > 0 {
			...
			if overagesInjected {
				logger.LegacyPrintf("service.antigravity_gateway", "%s pre_check: credits_injected_ignore_rate_limit remaining=%v model=%s account=%s",
				...
				logger.LegacyPrintf("service.antigravity_gateway", "%s pre_check: single_account_retry skipping rate_limit remaining=%v model=%s account=%s (will retry in-place if 503)",
					p.prefix, remaining.Truncate(time.Millisecond), p.requestedModel, p.account.ID)
			} else {
				logger.LegacyPrintf("service.antigravity_gateway", "%s pre_check: rate_limit_switch remaining=%v model=%s account=%s",
					p.prefix, remaining.Truncate(time.Millisecond), p.requestedModel, p.account.ID)
...
func tempUnscheduleGoogleConfigError(ctx context.Context, repo AccountRepository, accountID string, logPrefix string) {
	...
	if err := repo.SetTempUnschedulable(ctx, accountID, until, reason); err != nil {
		log.Printf("%s temp_unschedule_failed account=%s error=%v", logPrefix, accountID, err)
	} else {
		log.Printf("%s temp_unscheduled account=%s until=%v reason=%q", logPrefix, accountID, until.Format("15:04:05"), reason)
	}
}
...
func setModelRateLimitByModelName(ctx context.Context, repo AccountRepository, accountID string, modelName, prefix string, statusCode int, resetAt time.Time, afterSmartRetry bool) bool {
	...
	if afterSmartRetry {
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d model_rate_limited_after_smart_retry model=%s account=%s reset_in=%v", prefix, statusCode, modelName, accountID, time.Until(resetAt).Truncate(time.Second))
	} else {
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d model_rate_limited model=%s account=%s reset_in=%v", prefix, statusCode, modelName, accountID, time.Until(resetAt).Truncate(time.Second))
	}
	return true
}
...
func (s *AntigravityGatewayService) clearStickySession(ctx context.Context, groupID string, sessionHash string) {
	if s == nil || s.cache == nil || strings.TrimSpace(sessionHash) == "" {
		return
	}
	if err := s.cache.DeleteSessionAccountID(ctx, groupID, sessionHash); err != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "[antigravity-Forward] sticky_session_clear_failed group_id=%s session=%s err=%v", groupID, shortSessionHash(sessionHash), err)
	}
}
...
func (s *AntigravityGatewayService) setModelRateLimitAndClearSession(p *handleModelRateLimitParams, info *antigravitySmartRetryInfo) {
	...
	logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d model_rate_limited model=%s account=%s reset_in=%v",
...
func (s *AntigravityGatewayService) updateAccountModelRateLimitInCache(ctx context.Context, account *Account, modelKey string, resetAt time.Time) {
	...
	if err := s.schedulerSnapshot.UpdateAccountInCache(ctx, account); err != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "[antigravity-Forward] cache_update_failed account=%s model=%s err=%v", account.ID, modelKey, err)
	}
}
...
func (s *AntigravityGatewayService) handleUpstreamError(
	...
	groupID string, sessionHash string, isStickySession bool,
	...
	if statusCode == 429 {
		...
		if modelKey != "" {
			...
			if !s.setAntigravityModelRateLimits(ctx, s.accountRepo, account, modelKey, prefix, statusCode, ra, false) {
				...
				logger.LegacyPrintf("service.antigravity_gateway", "%s status=429 model_rate_limited model=%s account=%s reset_at=%v reset_in=%v",
		...
		// 无法解析模型 key，兜底为账号级限流
		ra := s.resolveResetTime(resetAt, defaultDur)
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=429 rate_limited account=%s reset_at=%v reset_in=%v (fallback)",
		...
		if err := s.accountRepo.SetRateLimited(ctx, account.ID, ra); err != nil {
			logger.LegacyPrintf("service.antigravity_gateway", "%s status=429 rate_limit_set_failed account=%s error=%v", prefix, account.ID, err)
		}
		return nil
	}
