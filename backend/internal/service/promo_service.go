...
// 使用事务和行锁确保并发安全
func (s *PromoService) ApplyPromoCode(ctx context.Context, userID string, code string) error {
	code = strings.TrimSpace(code)
	if code == "" {
		return nil
	}
	...
	if _, err := s.userRepo.CreditWallet(txCtx, WalletCreditInput{
		...
		return fmt.Errorf("credit promo bonus balance: %w", err)
	}
...
func (s *PromoService) invalidatePromoCaches(ctx context.Context, userID string, bonusAmount float64) {
...
func (s *PromoService) Create(ctx context.Context, input *CreatePromoCodeInput) (*PromoCode, error) {
	...
	if promoCode.BonusValidityDays <= 0 {
		promoCode.BonusValidityDays = s.bonusValidityDays(ctx)
	}
...
func (s *PromoService) bonusValidityDays(ctx context.Context) int {
	if s.settingService != nil {
		return s.settingService.GetBonusBalanceDefaultValidityDays(ctx)
	}
	return defaultBonusValidityDays
}

// GetByID 根据ID获取优惠码
func (s *PromoService) GetByID(ctx context.Context, id string) (*PromoCode, error) {
...
// Update 更新优惠码
func (s *PromoService) Update(ctx context.Context, id string, input *UpdatePromoCodeInput) (*PromoCode, error) {
	...
	if input.BonusValidityDays != nil && *input.BonusValidityDays > 0 {
		promoCode.BonusValidityDays = *input.BonusValidityDays
	}
...
// ListUsages 获取使用记录
func (s *PromoService) ListUsages(ctx context.Context, promoCodeID string, params pagination.PaginationParams) ([]PromoCodeUsage, *pagination.PaginationResult, error) {
