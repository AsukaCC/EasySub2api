...
func (s *GatewayService) withGatewayProfitControlGate(ctx context.Context, groupID *string) context.Context {
	if _, ok := gatewayTokenRequestPricingAtFromContext(ctx); !ok || groupID == nil || *groupID == "" {
		return ctx
	}
...
func (s *GatewayService) clearForeignProfitControlGate(ctx context.Context, groupID *string) context.Context {
...
func (s *GatewayService) resolveProfitControlGroup(ctx context.Context, groupID string) (*Group, error) {
