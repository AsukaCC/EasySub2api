...
type GroupRepository interface {
	...
	GetByIDLite(ctx context.Context, id string) (*Group, error)
	Update(ctx context.Context, group *Group) error
	Delete(ctx context.Context, id string) error
	DeleteCascade(ctx context.Context, id string) ([]string, error)
	...
	GetAccountCount(ctx context.Context, groupID string) (total int64, active int64, err error)
	DeleteAccountGroupsByGroupID(ctx context.Context, groupID string) (int64, error)
	// GetAccountIDsByGroupIDs 获取多个分组的所有账号 ID（去重）
	GetAccountIDsByGroupIDs(ctx context.Context, groupIDs []string) ([]string, error)
	// BindAccountsToGroup 将多个账号绑定到指定分组
	BindAccountsToGroup(ctx context.Context, groupID string, accountIDs []string) error
...
type GroupDuplicateRepository interface {
	...
	CreateFromSource(ctx context.Context, group *Group, sourceGroupID string) error
}
...
// Update 更新分组
func (s *GroupService) Update(ctx context.Context, id string, req UpdateGroupRequest) (*Group, error) {
	group, err := s.groupRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("get group: %w", err)
	}
...
// GetStats 获取分组统计信息
func (s *GroupService) GetStats(ctx context.Context, id string) (map[string]any, error) {
	group, err := s.groupRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("get group: %w", err)
	}
