...
func (s *SettingService) SetOllamaCloudUsageSettings(ctx context.Context, settings *OllamaCloudUsageSettings) error {
	...
	if settings.IntervalMinutes < ollamaCloudUsageMinIntervalMinutes || settings.IntervalMinutes > ollamaCloudUsageMaxIntervalMinutes {
		return infraerrors.BadRequest(
			...
			fmt.Sprintf("interval_minutes must be between %v and %v", ollamaCloudUsageMinIntervalMinutes, ollamaCloudUsageMaxIntervalMinutes),
		)
	}
	if settings.DebounceMinutes < ollamaCloudUsageMinDebounceMinutes || settings.DebounceMinutes > ollamaCloudUsageMaxDebounceMinutes {
		return infraerrors.BadRequest(
			...
			fmt.Sprintf("debounce_minutes must be between %v and %v", ollamaCloudUsageMinDebounceMinutes, ollamaCloudUsageMaxDebounceMinutes),
		)
	}
	...
	if settings.DebounceMinutes >= settings.IntervalMinutes {
		return infraerrors.BadRequest(
			"INVALID_OLLAMA_CLOUD_USAGE_DEBOUNCE",
			fmt.Sprintf("debounce_minutes (%v) must be less than interval_minutes (%v)", settings.DebounceMinutes, settings.IntervalMinutes),
		)
	}
	normalizeOllamaCloudUsageSettings(settings)
...
func (s *OllamaCloudUsageService) GetState(ctx context.Context, accountID string) (*OllamaCloudUsageState, error) {
...
func (s *OllamaCloudUsageService) SaveSession(ctx context.Context, accountID string, session string) (*OllamaCloudUsageState, error) {
...
func (s *OllamaCloudUsageService) DeleteSession(ctx context.Context, accountID string) (*OllamaCloudUsageState, error) {
	if s == nil || s.accountRepo == nil {
		return nil, ErrOllamaCloudUsageUnavailable
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
...
func (s *OllamaCloudUsageService) SetAutoRefresh(ctx context.Context, accountID string, enabled bool) (*OllamaCloudUsageState, error) {
	if s == nil || s.accountRepo == nil {
		return nil, ErrOllamaCloudUsageUnavailable
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
...
func (s *OllamaCloudUsageService) Refresh(ctx context.Context, accountID string) (*OllamaCloudUsageState, error) {
			...
			if _, refreshErr := s.refreshAccount(ctx, accountID, settings, true); refreshErr != nil {
				if errors.Is(refreshErr, ErrOllamaCloudUsageIdentityChanged) {
					if disableErr := writer.DisableOllamaCloudUsageAutoRefresh(ctx, &expected); disableErr != nil {
						logger.LegacyPrintf("service.ollama_cloud_usage", "disable_auto_refresh_failed: account_id=%v err=%v", accountID, disableErr)
					}
					return nil
				}
				logger.LegacyPrintf("service.ollama_cloud_usage", "refresh_due_failed: account_id=%v err=%v", accountID, refreshErr)
			}
			return nil
		})
	}
...
func (s *OllamaCloudUsageService) refreshAccount(ctx context.Context, accountID string, settings *OllamaCloudUsageSettings, requireEnabled bool) (*OllamaCloudUsageSnapshot, error) {
	if s == nil || s.accountRepo == nil {
		return nil, ErrOllamaCloudUsageUnavailable
	}
	if settings == nil {
			...
			if writer, ok := s.accountRepo.(ollamaCloudUsageRepository); ok {
				...
				if listErr != nil {
					...
					logger.LegacyPrintf("service.ollama_cloud_usage",
						"group_last_used_lookup_failed: account_id=%v err=%v", account.ID, listErr)
				} else {
...
func (s *OllamaCloudUsageService) refreshLoadedAccount(ctx context.Context, account *Account, intervalMinutes int) (*OllamaCloudUsageSnapshot, error) {
	...
	req.Header.Set("User-Agent", "easysub2api-ollama-usage/1")
