...
type monitorRunnerSvc interface {
	...
	RunCheck(ctx context.Context, id string) ([]*CheckResult, error)
}
...
type ChannelMonitorRunner struct {
	...
	mu      sync.Mutex
	tasks   map[string]*scheduledMonitor
	wg      sync.WaitGroup
	started bool
	stopped bool
...
func newChannelMonitorRunner(svc monitorRunnerSvc, settingService *SettingService) *ChannelMonitorRunner {
	...
	return &ChannelMonitorRunner{
		...
		parentCtx:      ctx,
		parentCancel:   cancel,
		tasks:          make(map[string]*scheduledMonitor),
...
func (r *ChannelMonitorRunner) Unschedule(id string) {
	if r == nil {
		return
	}
	r.mu.Lock()
...
func (r *ChannelMonitorRunner) tryAcquireInFlight(id string) bool {
	r.inFlightMu.Lock()
...
func (r *ChannelMonitorRunner) releaseInFlight(id string) {
	r.inFlightMu.Lock()
...
func (r *ChannelMonitorRunner) runOne(id string, name string) {
