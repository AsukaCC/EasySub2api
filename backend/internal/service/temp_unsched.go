...
type TempUnschedCache interface {
	SetTempUnsched(ctx context.Context, accountID string, state *TempUnschedState) error
	GetTempUnsched(ctx context.Context, accountID string) (*TempUnschedState, error)
	DeleteTempUnsched(ctx context.Context, accountID string) error
}
...
type TimeoutCounterCache interface {
	...
	IncrementTimeoutCount(ctx context.Context, accountID string, windowMinutes int) (int64, error)
	// GetTimeoutCount 获取账户当前的超时计数
	GetTimeoutCount(ctx context.Context, accountID string) (int64, error)
	// ResetTimeoutCount 重置账户的超时计数
	ResetTimeoutCount(ctx context.Context, accountID string) error
	// GetTimeoutCountTTL 获取计数器剩余过期时间
	GetTimeoutCountTTL(ctx context.Context, accountID string) (time.Duration, error)
}
