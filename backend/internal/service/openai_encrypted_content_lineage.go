...
// decodeOpenAILineageJSONUseNumber 以 UseNumber 解码，避免大整数在重编码时丢精度。
func decodeOpenAILineageJSONUseNumber(raw []byte, out any) error {
...
func stripOpenAIInvalidEncryptedContentFromReplayItems(items []json.RawMessage, invalid map[string]struct{}) ([]json.RawMessage, int) {
	...
	for _, item := range items {
		...
		if err := decodeOpenAILineageJSONUseNumber(item, &decoded); err != nil {
...
func stripOpenAIInvalidEncryptedContentRaw(payload []byte, invalid map[string]struct{}) ([]byte, int, error) {
	...
	var decoded map[string]any
	if err := decodeOpenAILineageJSONUseNumber(payload, &decoded); err != nil {
...
func (s *OpenAIGatewayService) markOpenAIWSInvalidEncryptedContentLineage(groupID string, sessionHash string, digests []string) {
...
// 时（常态）零成本返回 nil。
func (s *OpenAIGatewayService) sessionInvalidEncryptedContentDigests(groupID string, sessionHash string) map[string]struct{} {
...
func (s *OpenAIGatewayService) markOpenAIWSInvalidEncryptedContentLineageFromPayload(
	...
	logOpenAIWSModeInfo("%s account_id=%s turn=%d digests=%d", logKey, accountID, turn, len(digests))
}
...
func (s *OpenAIGatewayService) stripSessionInvalidEncryptedContentLogged(
	...
	if stripErr != nil {
		logOpenAIWSModeInfo(
			"%s_skip account_id=%s turn=%d reason=strip_error cause=%s",
			logKey,
			accountID,
			turn,
	...
	if strippedCount > 0 {
		logOpenAIWSModeInfo(
			"%s account_id=%s turn=%d stripped_items=%d",
			logKey,
			accountID,
			turn,
			strippedCount,
