...
func (s *OpenAIGatewayService) WithOpenAIRequestPricingContext(ctx context.Context, groupID *string) (context.Context, time.Time) {
...
func (s *OpenAIGatewayService) WithOpenAITurnPricingContext(ctx context.Context, groupID *string) (context.Context, time.Time) {
	pricingAt := timezone.Now()
	ctx = context.WithValue(ctx, openAIPricingAtCtxKey{}, pricingAt)
...
func (s *OpenAIGatewayService) withOpenAIProfitControlGate(ctx context.Context, groupID *string) context.Context {
	if _, suppressed := ctx.Value(openAIProfitControlSuppressCtxKey{}).(struct{}); suppressed {
		return ctx
	}
	if groupID != nil {
...
func (s *OpenAIGatewayService) resolveOpenAIProfitControlGate(ctx context.Context, groupID *string) *openAIProfitControlGate {
	if s == nil || groupID == nil || *groupID == "" {
		return nil
	}
...
func (s *OpenAIGatewayService) bindOpenAIStickySessionDuringSelection(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
...
// recovers.
func (s *OpenAIGatewayService) BindStickySessionAfterProfitAdmission(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
...
func profitControlObserverKey(groupID string, platform string) string {
	return platform + ":" + fmt.Sprintf("%v", groupID)
}

func (o *openAIProfitControlObserver) stats(groupID string, platform string) *openAIProfitControlGroupStats {
...
func (o *openAIProfitControlObserver) recordInstall(groupID string, platform string, threshold float64) {
	s := o.stats(groupID, platform)
	s.installs.Add(1)
	o.maybeLog(groupID, platform, threshold, s)
}

func (o *openAIProfitControlObserver) recordVeto(groupID string, platform string, threshold float64, reason string) {
	s := o.stats(groupID, platform)
	switch reason {
...
func (o *openAIProfitControlObserver) recordRefreshFailure(groupID string, platform string, threshold float64) {
	s := o.stats(groupID, platform)
...
func (o *openAIProfitControlObserver) maybeLog(groupID string, platform string, threshold float64, s *openAIProfitControlGroupStats) {
