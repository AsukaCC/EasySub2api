...
type ProxyRepository interface {
	...
	ListByIDs(ctx context.Context, ids []string) ([]Proxy, error)
	...
	CountAccountsByProxyID(ctx context.Context, proxyID string) (int64, error)
	ListAccountSummariesByProxyID(ctx context.Context, proxyID string) ([]ProxyAccountSummary, error)
...
// GetByID 根据ID获取代理
func (s *ProxyService) GetByID(ctx context.Context, id string) (*Proxy, error) {
...
// Update 更新代理
func (s *ProxyService) Update(ctx context.Context, id string, req UpdateProxyRequest) (*Proxy, error) {
	proxy, err := s.proxyRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("get proxy: %w", err)
	}
...
func (s *ProxyService) TestConnection(ctx context.Context, id string) error {
	proxy, err := s.proxyRepo.GetByID(ctx, id)
	if err != nil {
		return fmt.Errorf("get proxy: %w", err)
	}
...
// GetURL 获取代理URL
func (s *ProxyService) GetURL(ctx context.Context, id string) (string, error) {
	proxy, err := s.proxyRepo.GetByID(ctx, id)
	if err != nil {
		return "", fmt.Errorf("get proxy: %w", err)
	}
