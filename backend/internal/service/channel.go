...
func (c *Channel) Clone() *Channel {
	...
	cp := *c
	if c.GroupIDs != nil {
		cp.GroupIDs = make([]string, len(c.GroupIDs))
	...
	if c.AccountStatsPricingRules != nil {
		...
		for i, rule := range c.AccountStatsPricingRules {
			...
			if rule.GroupIDs != nil {
				cp.AccountStatsPricingRules[i].GroupIDs = make([]string, len(rule.GroupIDs))
				copy(cp.AccountStatsPricingRules[i].GroupIDs, rule.GroupIDs)
			}
			if rule.AccountIDs != nil {
				cp.AccountStatsPricingRules[i].AccountIDs = make([]string, len(rule.AccountIDs))
...
func validateSingleInterval(iv *PricingInterval, idx int) error {
	if iv.MinTokens < 0 {
		return fmt.Errorf("interval #%v: min_tokens (%v) must be >= 0", idx+1, iv.MinTokens)
	}
	if iv.MaxTokens != nil {
		if *iv.MaxTokens <= 0 {
			return fmt.Errorf("interval #%v: max_tokens (%v) must be > 0", idx+1, *iv.MaxTokens)
		}
		if *iv.MaxTokens <= iv.MinTokens {
			return fmt.Errorf("interval #%v: max_tokens (%v) must be > min_tokens (%v)",
...
func validateIntervalPrices(iv *PricingInterval, idx int) error {
	...
	for _, p := range prices {
		if p.val != nil && *p.val < 0 {
			return fmt.Errorf("interval #%v: %s must be >= 0", idx+1, p.name)
		}
	}
	return nil
}
...
func validateIntervalOverlap(sorted []PricingInterval) error {
	for i, iv := range sorted {
		...
		if iv.MaxTokens == nil && i < len(sorted)-1 {
			return fmt.Errorf("interval #%v: unbounded interval (max_tokens=null) must be the last one",
				i+1)
		}
		if i == 0 {
			continue
		...
		if prev.MaxTokens == nil || *prev.MaxTokens > iv.MinTokens {
			return fmt.Errorf("interval #%v and #%v overlap: prev max=%s > cur min=%v",
