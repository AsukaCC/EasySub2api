...
type OpenAIAccountScheduleRequest struct {
	...
	StickyPreviousAccountID string
	StickyWeighted          bool
...
type OpenAIAccountScheduler interface {
	...
	ReportResult(accountID string, success bool, firstTokenMs *int)
	ReportSwitch()
...
func (s *openAIAccountRuntimeStats) loadOrCreate(accountID string) *openAIAccountRuntimeStat {
...
func (s *openAIAccountRuntimeStats) report(accountID string, success bool, firstTokenMs *int) {
	if s == nil || accountID == "" {
		return
	}
	const alpha = 0.2
...
func (s *openAIAccountRuntimeStats) snapshot(accountID string) (errorRate float64, ttft float64, hasTTFT bool) {
	if s == nil || accountID == "" {
		return 0, 0, false
	}
...
func newOpenAISelectionProbeBudget() *openAISelectionProbeBudget {
	return &openAISelectionProbeBudget{attempted: make(map[string]struct{})}
}
...
func (b *openAISelectionProbeBudget) recordAcquire(accountID string) bool {
	if b == nil {
		return false
	}
	if !b.limited {
	...
	if b.attempted == nil {
		b.attempted = make(map[string]struct{})
	}
	b.acquires++
...
func (b *openAISelectionProbeBudget) wasAttempted(accountID string) bool {
	if b == nil {
		return false
	}
...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if err := CheckActiveRequestModel(ctx, req.RequestedModel); err != nil {
	...
	var escapedStickyFallback *OpenAIAccountScheduleRequest
	if !req.StickyWeighted {
		...
		if escapedStickyID != "" {
			...
			fallback := req
			escapedStickyFallback = &fallback
			req.ExcludedIDs = cloneExcludedAccountIDs(req.ExcludedIDs)
			if req.ExcludedIDs == nil {
				req.ExcludedIDs = make(map[string]struct{})
			}
	...
	// Escape changes this attempt's candidate order, not the durable affinity.
	// A degraded but eligible sticky account remains a last resort when no
	// alternative can be acquired or queued. Operational errors must propagate.
	if escapedStickyFallback != nil && selection == nil &&
		(errors.Is(err, ErrNoAvailableAccounts) || errors.Is(err, ErrNoAvailableCompactAccounts)) {
		selection, candidateCount, topK, loadSkew, err = s.selectByLoadBalance(ctx, *escapedStickyFallback)
	}
	...
	if selection != nil && selection.Account != nil {
		...
		decision.SelectedAccountType = selection.Account.Type
		if req.StickyWeighted {
			if req.StickyPreviousAccountID != "" && selection.Account.ID == req.StickyPreviousAccountID {
				decision.StickyPreviousHit = true
			}
			if req.StickyAccountID != "" && selection.Account.ID == req.StickyAccountID {
				decision.StickySessionHit = true
			}
		}
	}
...
func openAIStickyAccountMatchesGroup(account *Account, groupID *string) bool {
	if account == nil {
		return false
	}
	if groupID == nil {
...
func (s *defaultOpenAIAccountScheduler) shouldEscapeStickyAccount(accountID string, cfg openAIStickyEscapeConfig) (reason string, errorRate float64, ttft float64, shouldEscape bool) {
	if !cfg.enabled || s == nil || s.stats == nil || accountID == "" {
		return "", 0, 0, false
	}
...
func deriveOpenAISelectionSeed(req OpenAIAccountScheduleRequest) uint64 {
	...
	if req.GroupID != nil {
		_, _ = hasher.Write([]byte(*req.GroupID))
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	now := time.Now()
	upstreamCostFactors := map[string]float64(nil)
	...
	for i := range candidates {
		...
		if req.StickyWeighted {
			if req.PreviousResponseCanMove && req.StickyPreviousAccountID != "" && item.account.ID == req.StickyPreviousAccountID {
				item.score += weights.Previous
			}
			if req.StickyAccountID != "" && item.account.ID == req.StickyAccountID {
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	buildSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		if req.StickyWeighted {
			for _, stickyID := range []string{req.StickyPreviousAccountID, req.StickyAccountID} {
				if stickyID == "" {
					continue
				}
		...
		selected := make(map[string]struct{}, len(primary))
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []string{req.StickyPreviousAccountID, req.StickyAccountID} {
		if accountID == "" {
			continue
		}
		if req.ExcludedIDs != nil {
...
func (s *defaultOpenAIAccountScheduler) lookupShadowParentAccount(ctx context.Context, id string) *Account {
	if s == nil || s.service == nil {
		return nil
	}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if !parentHealthyForShadow(account, func(id string) *Account {
...
func (s *defaultOpenAIAccountScheduler) ReportResult(accountID string, success bool, firstTokenMs *int) {
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	return s.selectAccountWithSchedulerForGroup(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				...
				if effectiveExcludedIDs == nil {
					effectiveExcludedIDs = make(map[string]struct{})
				}
	...
	if sessionHash != "" && s.cache != nil {
		if accountID, err := s.getStickySessionAccountID(ctx, groupID, sessionHash); err == nil && accountID != "" {
...
func cloneExcludedAccountIDs(excludedIDs map[string]struct{}) map[string]struct{} {
	if len(excludedIDs) == 0 {
		return nil
	}
	cloned := make(map[string]struct{}, len(excludedIDs))
...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(accountID string, model string, success bool, firstTokenMs *int) {
	if success {
...
func (s *RateLimitService) BuildOpenAIAccountSchedulerScoreSnapshot(
	...
	accounts []*Account,
	loadMap map[string]*AccountLoadInfo,
) map[string]OpenAIAccountSchedulerScoreSnapshot {
...
func buildOpenAIAccountSchedulerScoreSnapshot(
	...
	result := make(map[string]OpenAIAccountSchedulerScoreSnapshot, len(candidates))
	for _, candidate := range candidates {
		priorityFactor := 1.0
		if maxPriority > minPriority {
...
func openAIUpstreamCostFactors(accounts []*Account, now time.Time, oauthSchedulingRateMultiplier float64) map[string]float64 {
...
func newOpenAILegacyUpstreamRateOrder(accounts []*Account, now time.Time, oauthSchedulingRateMultiplier float64) openAILegacyUpstreamRateOrder {
	rates := make(map[string]float64, len(accounts))
	var first float64
	distinct := false
	for _, account := range accounts {
		if account == nil {
