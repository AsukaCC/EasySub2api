...
type TotpCache interface {
	...
	GetSetupSession(ctx context.Context, userID string) (*TotpSetupSession, error)
	SetSetupSession(ctx context.Context, userID string, session *TotpSetupSession, ttl time.Duration) error
	DeleteSetupSession(ctx context.Context, userID string) error
	...
	// Rate limiting
	IncrementVerifyAttempts(ctx context.Context, userID string) (int, error)
	GetVerifyAttempts(ctx context.Context, userID string) (int, error)
	ClearVerifyAttempts(ctx context.Context, userID string) error

	// Step-up grant methods (敏感操作 sudo 窗口)
	SetStepUpGrant(ctx context.Context, userID string, sessionKey string, ttl time.Duration) error
	HasStepUpGrant(ctx context.Context, userID string, sessionKey string) (bool, error)
}
...
func (s *TotpService) GetStatus(ctx context.Context, userID string) (*TotpStatus, error) {
...
func (s *TotpService) InitiateSetup(ctx context.Context, userID string, emailCode, password string) (*TotpSetupResponse, error) {
...
func (s *TotpService) CompleteSetup(ctx context.Context, userID string, totpCode, setupToken string) error {
	// Check if TOTP feature is enabled globally
	if !s.settingService.IsTotpEnabled(ctx) {
...
// If email verification is enabled, emailCode is required; otherwise password is required
func (s *TotpService) Disable(ctx context.Context, userID string, emailCode, password string) error {
	// Get user
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
...
func (s *TotpService) VerifyCode(ctx context.Context, userID string, code string) error {
...
// 返回授权有效期，供前端展示/设置提醒。
func (s *TotpService) VerifyStepUp(ctx context.Context, userID string, sessionKey, code string) (time.Duration, error) {
...
func (s *TotpService) HasStepUpGrant(ctx context.Context, userID string, sessionKey string) (bool, error) {
	return s.cache.HasStepUpGrant(ctx, userID, sessionKey)
}

// CreateLoginSession creates a temporary login session for 2FA
func (s *TotpService) CreateLoginSession(ctx context.Context, userID string, email string) (string, error) {
...
func (s *TotpService) IsTotpEnabledForUser(ctx context.Context, userID string) (bool, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
...
func (s *TotpService) GetVerificationMethod(ctx context.Context, userID string) (*VerificationMethod, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get user: %w", err)
	}
...
func (s *TotpService) SendVerifyCode(ctx context.Context, userID string, locale ...string) error {
