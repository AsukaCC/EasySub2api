...
func (s *AntigravityGatewayService) setCreditsExhausted(ctx context.Context, account *Account) {
	...
	if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, creditsExhaustedKey, resetAt); err != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "set credits exhausted failed: account=%s err=%v", account.ID, err)
		return
	}
	s.updateAccountModelRateLimitInCache(ctx, account, creditsExhaustedKey, resetAt)
	logger.LegacyPrintf("service.antigravity_gateway", "credits_exhausted_marked account=%s reset_at=%s",
...
func (s *AntigravityGatewayService) clearCreditsExhausted(ctx context.Context, account *Account) {
	if account == nil || account.ID == "" || account.Extra == nil {
		return
	}
	...
	if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{
		...
		logger.LegacyPrintf("service.antigravity_gateway", "clear credits exhausted failed: account=%s err=%v", account.ID, err)
	}
}
...
func (s *AntigravityGatewayService) attemptCreditsOveragesRetry(
	...
	logger.LegacyPrintf("service.antigravity_gateway", "%s status=429 credit_overages_retry model=%s account=%s (injecting enabledCreditTypes)",
	...
	if err != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "%s credit_overages_failed model=%s account=%s build_request_err=%v",
	...
	if err == nil && creditsResp != nil && creditsResp.StatusCode < 400 {
		...
		logger.LegacyPrintf("service.antigravity_gateway", "%s status=%d credit_overages_success model=%s account=%s",
...
func (s *AntigravityGatewayService) handleCreditsRetryFailure(
	...
	if shouldMarkCreditsExhausted(creditsResp, creditsRespBody, reqErr) && account != nil {
		...
		logger.LegacyPrintf("service.antigravity_gateway", "%s credit_overages_failed model=%s account=%s marked_exhausted=true status=%d body=%s",
			prefix, modelKey, account.ID, creditsStatusCode, truncateForLog(creditsRespBody, 200))
		return
	}
	if account != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "%s credit_overages_failed model=%s account=%s marked_exhausted=false status=%d err=%v body=%s",
