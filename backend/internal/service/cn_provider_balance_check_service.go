...
func (s *CNProviderBalanceCheckService) runOnce() {
	...
	if paused > 0 || cleared > 0 {
		log.Printf("[CNBalance] paused=%v cleared=%v (threshold=%.2f)", paused, cleared, threshold)
	}
}

// probeQuota 探测单个 coding plan 账号的滚动窗口用量并落 extra 快照。
// 不在此处做停调/恢复决策：调度阈值评估读取快照统一判定（含暂停账号的续停）。
func (s *CNProviderBalanceCheckService) probeQuota(ctx context.Context, accountID string, platform string) {
	if s.quotaService == nil {
		return
	}
	result, err := s.quotaService.QueryUsage(ctx, accountID)
	if err != nil {
		log.Printf("[CNBalance] quota probe account %v (%s) failed: %v", accountID, platform, err)
		return
	}
	if result != nil && !result.Success && result.Error != "" {
		log.Printf("[CNBalance] quota probe account %v (%s) error: %s", accountID, platform, result.Error)
	}
}
...
func (s *CNProviderBalanceCheckService) checkOne(ctx context.Context, account *Account, threshold float64) cnBalanceCheckOutcome {
	...
	if low {
		...
		if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, time.Now().Add(s.cooldown()), reason); err != nil {
			log.Printf("[CNBalance] pause account %v failed: %v", account.ID, err)
			return cnBalanceNoChange
		}
		log.Printf("[CNBalance] paused account %v (%s): balance=%.4g %s", account.ID, account.Platform, result.Balance, result.Currency)
	...
	if account.TempUnschedulableUntil != nil && strings.HasPrefix(account.TempUnschedulableReason, cnBalanceLowReasonPrefix) {
		if err := s.accountRepo.ClearTempUnschedulable(ctx, account.ID); err != nil {
			log.Printf("[CNBalance] clear account %v failed: %v", account.ID, err)
			return cnBalanceNoChange
		}
		log.Printf("[CNBalance] reactivated account %v (%s): balance=%.4g %s", account.ID, account.Platform, result.Balance, result.Currency)
