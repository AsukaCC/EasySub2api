...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/platform/liveattestation"
...
type OpenAIForwardResult struct {
	...
	ServiceTier *string
	// ReasoningEffort is extracted after group policy and model-family remapping.
	// Stored for usage records display; nil means not provided / not applicable.
	ReasoningEffort *string
	// RequestedReasoningEffort is the client-requested effort before mapping.
	RequestedReasoningEffort *string
	// NativeCompactionV2 is independent from the transport request type.
...
func (t *accountWriteThrottle) Allow(id string, now time.Time) bool {
	if t == nil || id == "" || t.minInterval <= 0 {
		return true
	}
...
type OpenAIGatewayService struct {
	...
	// openaiOAuth429RetryStartedAt keeps a short request-local retry window for
	// transient OAuth 429s. Exhausted quota 429s bypass this window and use the
	// provider reset time instead.
	openaiOAuth429RetryStartedAt        sync.Map // key: account ID, value: time.Time
...
// NewOpenAIGatewayServiceWithUserLevel creates an OpenAI gateway service with
// rolling user level and dynamic-rate selection enabled.
func NewOpenAIGatewayServiceWithUserLevel(
	accountRepo AccountRepository,
	usageLogRepo UsageLogRepository,
	usageBillingRepo UsageBillingRepository,
	userRepo UserRepository,
...
func (s *OpenAIGatewayService) ResolveChannelMapping(ctx context.Context, groupID string, model string) ChannelMappingResult {
...
func (s *OpenAIGatewayService) IsModelRestricted(ctx context.Context, groupID string, model string) bool {
	if s.channelService == nil {
		return false
	}
...
func (s *OpenAIGatewayService) ResolveChannelMappingAndRestrict(ctx context.Context, groupID *string, model string) (ChannelMappingResult, bool) {
	if s.channelService == nil {
...
func (s *OpenAIGatewayService) checkChannelPricingRestriction(ctx context.Context, groupID *string, requestedModel string) bool {
...
func (s *OpenAIGatewayService) isUpstreamModelRestrictedByChannel(ctx context.Context, groupID string, account *Account, requestedModel string, requireCompact bool) bool {
	if s.channelService == nil {
		return false
	}
...
func (s *OpenAIGatewayService) needsUpstreamChannelRestrictionCheck(ctx context.Context, groupID *string) bool {
...
func (s *OpenAIGatewayService) billingDeps() *billingDeps {
	return &billingDeps{
		...
		userLevelService:      s.userLevelService,
	}
}
...
func (s *OpenAIGatewayService) InvalidateAgentIdentityWSConnections(accountID string) {
...
func (s *OpenAIGatewayService) logOpenAIWSModeBootstrap() {
	...
	logOpenAIWSModeInfo(
		"bootstrap enabled=%v oauth_enabled=%v apikey_enabled=%v force_http=%v responses_websockets_v2=%v responses_websockets=%v payload_log_sample_rate=%.3f event_flush_batch_size=%v event_flush_interval_ms=%v prewarm_cooldown_ms=%v retry_backoff_initial_ms=%v retry_backoff_max_ms=%v retry_jitter_ratio=%.3f retry_total_budget_ms=%v ws_read_limit_bytes=%v",
		wsCfg.Enabled,
		wsCfg.OAuthEnabled,
		wsCfg.APIKeyEnabled,
		wsCfg.ForceHTTP,
...
func isolateOpenAISessionID(apiKeyID string, raw string) string {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return ""
	}
	h := xxhash.New()
	_, _ = fmt.Fprintf(h, "k%v:", apiKeyID)
...
func (s *OpenAIGatewayService) GetAccessToken(ctx context.Context, account *Account) (string, string, error) {
	...
	case AccountTypeSetupToken:
		...
		// Setup tokens are inference-only bearer credentials. They use the Codex
		// forwarding protocol without participating in OAuth refresh.
		accessToken := account.GetOpenAIAccessToken()
		if accessToken == "" {
			return "", "", errors.New("access_token not found in credentials")
		}
