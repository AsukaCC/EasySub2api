...
type PasskeyRepository interface {
	EnsureUserHandle(ctx context.Context, userID string, candidate []byte) ([]byte, error)
	GetUserHandle(ctx context.Context, userID string) ([]byte, error)
	GetByCredentialID(ctx context.Context, credentialID []byte) (*PasskeyCredentialRecord, error)
	ListByUserID(ctx context.Context, userID string) ([]PasskeyCredentialRecord, error)
	Create(ctx context.Context, record *PasskeyCredentialRecord) (*PasskeyCredentialRecord, error)
	UpdateCredential(ctx context.Context, userID string, credential *webauthn.Credential, usedAt time.Time) error
	Rename(ctx context.Context, userID, credentialID string, name string) error
...
func (s *PasskeyService) List(ctx context.Context, userID string) ([]PasskeyCredentialSummary, error) {
	if err := s.requireEnabled(); err != nil {
		return nil, err
	}
...
func (s *PasskeyService) Rename(ctx context.Context, userID, credentialID string, name string) error {
	if err := s.requireEnabled(); err != nil {
		return err
	}
...
func (s *PasskeyService) Delete(ctx context.Context, userID, credentialID string, password string) error {
	if err := s.requireEnabled(); err != nil {
		return err
	}
	user, err := s.userRepo.GetByID(ctx, userID)
