...
func (r *capacityShedAccountRepoStub) SetTempUnschedulable(_ context.Context, _ string, _ time.Time, _ string) error {
	r.tempUnschedCalls++
	return nil
}
...
func TestOpenAIHTTPCapacityShedIsRequestScoped(t *testing.T) {
	...
	require.Equal(t, "Our servers are currently overloaded. Please try again later.", failoverErr.ClientMessage)

	repo := &capacityShedAccountRepoStub{}
	(&GatewayService{accountRepo: repo}).TempUnscheduleRetryableError(context.Background(), "1", failoverErr)
	require.Zero(t, repo.tempUnschedCalls)

	// A valid JSON error may contain the overload phrase in echoed request data;
	// only explicit error fields should classify it as capacity shedding.
	echoed := []byte(`{"error":{"type":"invalid_request_error","message":"invalid request"},"input":"Our servers are currently overloaded"}`)
	require.False(t, isOpenAIRequestScopedCapacityShed("", echoed))
...
// Large metadata and empty structural events must remain staged: they are
// protocol preamble, not semantic output. A message-only overload arriving
// afterwards must still fail over before any bytes reach the client.
...
// 即使上游只发送 post-output error 帧而没有 response.failed/终止事件，也要保留
// 容量降载标记，避免流结束时被当作普通断流并错误计入账号健康失败。
func TestOpenAIStreamCapacityShedErrorOnlyAfterOutputKeepsMarker(t *testing.T) {
	gin.SetMode(gin.TestMode)
	stream := strings.Join([]string{
		"event: response.created",
		`data: {"type":"response.created","response":{"id":"resp-error-only"}}`,
		"",
		"event: response.output_text.delta",
		`data: {"type":"response.output_text.delta","delta":"partial"}`,
		"",
	...
	for _, passthrough := range []bool{false, true} {
		t.Run(map[bool]string{false: "native", true: "passthrough"}[passthrough], func(t *testing.T) {
			...
			resp := &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(stream)),
				Header:     http.Header{"X-Request-Id": []string{"rid-error-only"}},
			}
			account := &Account{ID: "1", Platform: PlatformOpenAI, Type: AccountTypeOAuth, Name: "acc"}
			var err error
			if passthrough {
			...
			require.Error(t, err)
			require.True(t, IsOpenAICapacityShedError(err))
			body := rec.Body.String()
			require.Contains(t, body, "partial")
			require.Contains(t, body, `"code":"server_error"`)
			require.NotContains(t, body, "server_is_overloaded")
...
func TestSanitizeOpenAICapacityShedErrorCodeForClient(t *testing.T) {
	cases := []struct {
		...
		{
			name:        "普通错误码与容量文案并存时不添加server_error",
