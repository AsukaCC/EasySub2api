...
func computeBasicStats(st *DashboardStats, orders []*dbent.PaymentOrder, todayStart time.Time) {
	...
	for _, o := range orders {
		...
		st.TotalAmount[currency] += paymentOrderNetRevenue(o)
		currencyCounts[currency]++
		if o.PaidAt != nil && !o.PaidAt.Before(todayStart) {
			st.TodayAmount[currency] += paymentOrderNetRevenue(o)
			todayCount++
		}
	}
...
func buildDailySeries(orders []*dbent.PaymentOrder, since time.Time, days int) []DailyStats {
	...
	for _, o := range orders {
		...
		ds.Amount[PaymentOrderCurrency(o)] += paymentOrderNetRevenue(o)
		ds.Count++
	}
...
func buildMethodDistribution(orders []*dbent.PaymentOrder) []PaymentMethodStat {
	...
	for _, o := range orders {
		...
		ms.Amount[PaymentOrderCurrency(o)] += paymentOrderNetRevenue(o)
		ms.Count++
	}
...
func buildTopUsers(orders []*dbent.PaymentOrder) TopUsersByCurrency {
	userMap := make(map[string]map[string]*TopUserStat)
	for _, o := range orders {
		currency := PaymentOrderCurrency(o)
		...
		us.Amount += paymentOrderNetRevenue(o)
	}
...
func paymentOrderNetRevenue(order *dbent.PaymentOrder) float64 {
	if order == nil {
		return 0
	}
	net := decimal.NewFromFloat(order.PayAmount).Round(2).
		Sub(decimal.NewFromFloat(order.RefundedGatewayAmount).Round(2))
	if net.IsNegative() {
		return 0
	}
	amount, _ := net.Round(2).Float64()
	return amount
}

// --- Audit Logs ---

func (s *PaymentService) writeAuditLog(ctx context.Context, oid string, action, op string, detail map[string]any) {
	dj, _ := json.Marshal(detail)
	_, err := s.entClient.PaymentAuditLog.Create().SetOrderID(oid).SetAction(action).SetDetail(string(dj)).SetOperator(op).Save(ctx)
	if err != nil {
		slog.Error("audit log failed", "orderID", oid, "action", action, "error", err)
	}
}

func (s *PaymentService) GetOrderAuditLogs(ctx context.Context, oid string) ([]*dbent.PaymentAuditLog, error) {
	return s.entClient.PaymentAuditLog.Query().Where(paymentauditlog.OrderIDEQ(oid)).Order(paymentauditlog.ByCreatedAt()).All(ctx)
}
