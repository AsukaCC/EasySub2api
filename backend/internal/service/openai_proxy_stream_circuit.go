...
type openAIProxyStreamCircuit struct {
	...
	entries  map[string]openAIProxyStreamCircuitEntry
}
...
func newOpenAIProxyStreamCircuit(settings openAIProxyStreamCircuitSettings) *openAIProxyStreamCircuit {
	...
	return &openAIProxyStreamCircuit{
		settings: settings,
		entries:  make(map[string]openAIProxyStreamCircuitEntry),
	}
}
...
func (c *openAIProxyStreamCircuit) recordFailure(proxyID string, now time.Time) (bool, time.Time) {
	if c == nil || c.settings.disabled || proxyID == "" {
...
func (c *openAIProxyStreamCircuit) recordSuccess(proxyID string) bool {
	if c == nil || proxyID == "" {
		return false
	}
	c.mu.Lock()
...
func (c *openAIProxyStreamCircuit) isBlocked(proxyID string, now time.Time) bool {
	if c == nil || c.settings.disabled || proxyID == "" {
		return false
	}
	c.mu.Lock()
...
func (c *openAIProxyStreamCircuit) ensureCapacityLocked(now time.Time) {
	...
	var oldestProxyID string
	var oldest time.Time
	for proxyID, entry := range c.entries {
		if oldestProxyID == "" || entry.lastTouched.Before(oldest) {
...
func openAIProxyStreamCircuitProxyID(account *Account) (string, bool) {
	if account == nil || account.Platform != PlatformOpenAI || account.ProxyID == nil || *account.ProxyID == "" {
		return "", false
	}
