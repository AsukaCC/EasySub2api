...
func (s *AccountTestService) ProbeOpenAIAPIKeyResponsesSupport(ctx context.Context, accountID string) {
	account, err := s.accountRepo.GetByID(ctx, accountID)
	if err != nil {
		logger.LegacyPrintf("service.openai_probe", "probe_load_account_failed: account_id=%v err=%v", accountID, err)
		return
	}
	...
	if account.IsCNProvider() {
		// CN providers default to Chat Completions. DeepSeek and Kimi accounts
		// explicitly configured for their native Responses endpoint are forced
		// onto that route without a redundant network probe; Zhipu remains a
		// Chat fallback.
	...
	if apiKey == "" {
		logger.LegacyPrintf("service.openai_probe", "probe_skip_no_apikey: account_id=%v", accountID)
		return
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.openai_probe", "probe_invalid_baseurl: account_id=%v base_url=%q err=%v", accountID, baseURL, err)
		return
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.openai_probe", "probe_build_request_failed: account_id=%v err=%v", accountID, err)
		return
	}
	...
	if err != nil {
		...
		logger.LegacyPrintf("service.openai_probe", "probe_request_failed: account_id=%v url=%s err=%v", accountID, probeURL, err)
		return
	}
	...
	if readErr != nil {
		...
		logger.LegacyPrintf("service.openai_probe", "probe_read_body_failed: account_id=%v url=%s err=%v", accountID, probeURL, readErr)
		return
	}
	...
	if !responsesProbeVerdictIsConclusive(resp.StatusCode, bodyBytes) {
		logger.LegacyPrintf("service.openai_probe",
			"probe_inconclusive_keep_unknown: account_id=%v base_url=%s probe_model=%s status=%v response_status=%s reason=%s",
	...
	if err := s.accountRepo.UpdateExtra(ctx, accountID, map[string]any{
		...
		logger.LegacyPrintf("service.openai_probe", "probe_persist_failed: account_id=%v supported=%v err=%v", accountID, supported, err)
		return
	}
	...
	logger.LegacyPrintf("service.openai_probe",
		"probe_done: account_id=%v base_url=%s probe_model=%s status=%v supported=%v",
