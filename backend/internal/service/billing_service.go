...
type BillingCache interface {
	...
	SetUserBalance(ctx context.Context, userID string, balance float64) error
	DeductUserBalance(ctx context.Context, userID string, amount float64) error
	InvalidateUserBalance(ctx context.Context, userID string) error

	// Subscription operations
	GetSubscriptionCache(ctx context.Context, userID, groupID string) (*SubscriptionCacheData, error)
	SetSubscriptionCache(ctx context.Context, userID, groupID string, data *SubscriptionCacheData) error
	UpdateSubscriptionUsage(ctx context.Context, userID, groupID string, cost float64) error
	InvalidateSubscriptionCache(ctx context.Context, userID, groupID string) error

	// API Key rate limit operations
	GetAPIKeyRateLimit(ctx context.Context, keyID string) (*APIKeyRateLimitCacheData, error)
	SetAPIKeyRateLimit(ctx context.Context, keyID string, data *APIKeyRateLimitCacheData) error
	UpdateAPIKeyRateLimitUsage(ctx context.Context, keyID string, cost float64) error
...
type ModelPricing struct {
	...
	ReasoningEffortMultipliers         map[string]float64 // nil inherits the model defaults; an empty map explicitly disables multipliers.
...
func reasoningEffortValue(value *string) string {
	if value == nil {
		return ""
	}
...
// DeepSeek official off-peak rates in USD/token. Peak pricing is twice the
// off-peak rate during 01:00-04:00 and 06:00-10:00 UTC on Beijing weekdays.
const (
...
func (s *BillingService) initFallbackPricing() {
	s.fallbackPrices["claude-opus-5-5"] = &ModelPricing{
		...
		CacheCreation5mPrice: 5e-6, CacheCreation1hPrice: 8e-6, SupportsCacheBreakdown: true,
	...
	// Claude Fable 5.x uses the same input/output and cache-write prices.
	...
	// Cards store official off-peak rates; the default pricing path applies the
	// weekday peak multiplier at the request's billing timestamp.
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	// Specific cards take precedence; unknown DeepSeek IDs fall back to Flash so
	// new aliases cannot silently bypass billing.
...
func (s *BillingService) GetModelPricingWithChannel(model string, channelPricing *ChannelModelPricing) (*ModelPricing, error) {
	...
	if channelPricing.CacheWritePrice != nil {
		...
		if channelPricing.CacheWrite1hPrice == nil {
			// Preserve legacy behavior when only cache_write_price is configured.
	...
	pricing.ReasoningEffortMultipliers = maps.Clone(effectiveReasoningEffortMultipliers(model, channelPricing.ReasoningEffortMultipliers))
...
type CostInput struct {
	...
	ReasoningEffort           string                // 最终生效的推理等级
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// Official model policy applies to catalog pricing only. Group/channel
	// overrides retain the operator's explicit price semantics.
	...
	pricing = applyDeepSeekPeakPricing(input.Model, pricing, input.PricingAt,
		resolved.Source == PricingSourceLiteLLM)
...
// normalizeCacheCreationBreakdown caps contradictory 5m/1h details at an
// explicitly positive aggregate while retaining their reported ratio.
...
func (s *BillingService) CalculateCost(model string, tokens UsageTokens, rateMultiplier float64, reasoningEffort ...string) (*CostBreakdown, error) {
	return s.calculateCostInternal(model, tokens, rateMultiplier, "", nil, reasoningEffort...)
}

func (s *BillingService) CalculateCostWithServiceTier(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string, reasoningEffort ...string) (*CostBreakdown, error) {
	return s.calculateCostInternal(model, tokens, rateMultiplier, serviceTier, nil, reasoningEffort...)
}
...
func (s *BillingService) calculateCostWithServiceTierPolicy(
	...
	return s.calculateCostInternalWithPolicy(model, tokens, rateMultiplier, serviceTier, nil, longContextBillingEnabled, reasoningEffort...)
}

func (s *BillingService) calculateCostInternal(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string, channelPricing *ChannelModelPricing, reasoningEffort ...string) (*CostBreakdown, error) {
	return s.calculateCostInternalWithPolicy(model, tokens, rateMultiplier, serviceTier, channelPricing, true, reasoningEffort...)
}
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	pricing = applyDeepSeekPeakPricing(model, pricing, time.Time{}, channelPricing == nil)
	...
	applyCostBreakdownMultiplier(breakdown, reasoningEffortBillingMultiplier(effort, pricing.ReasoningEffortMultipliers))
	return breakdown, nil
}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	if !needsCacheCreationPolicy && !needsReasoningPolicy && fastRatio <= 0 {
		return pricing
	}
	cloned := *pricing
...
func applyDeepSeekPeakPricing(model string, pricing *ModelPricing, at time.Time, enabled bool) *ModelPricing {
	if !enabled || pricing == nil || !isDeepSeekModel(model) {
		return pricing
	}
	if at.IsZero() {
		at = timezone.Now()
