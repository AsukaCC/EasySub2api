...
func (s *APIKeyService) InvalidateAuthCacheByUserID(ctx context.Context, userID string) {
	if userID == "" {
		return
	}
...
func (s *APIKeyService) InvalidateAuthCacheByGroupID(ctx context.Context, groupID string) {
	if groupID == "" {
		return
	}
