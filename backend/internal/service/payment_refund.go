...
func (s *PaymentService) getOrderProviderInstance(ctx context.Context, o *dbent.PaymentOrder) (*dbent.PaymentProviderInstance, error) {
	...
	return s.entClient.PaymentProviderInstance.Get(ctx, instIDStr)
}
...
func (s *PaymentService) getRefundOrderProviderInstance(ctx context.Context, o *dbent.PaymentOrder) (*dbent.PaymentProviderInstance, error) {
	...
	inst, err := s.entClient.PaymentProviderInstance.Get(ctx, instIDStr)
	if err != nil {
		if dbent.IsNotFound(err) {
			return nil, fmt.Errorf("order %v refund provider instance %s is missing", o.ID, instIDStr)
		}
		return nil, err
	}
	return inst, nil
...
func (s *PaymentService) RequestRefund(ctx context.Context, oid, uid string, reason string) error {
	order, err := s.entClient.PaymentOrder.Get(ctx, oid)
	if err != nil {
		return infraerrors.NotFound("NOT_FOUND", "order not found")
	}
	key := fmt.Sprintf("legacy-user-refund:%s:%.2f", oid, order.RefundedPrincipalAmount)
	_, err = s.CreateSelfServiceRefund(ctx, CreatePaymentRefundInput{
		OrderID: oid, UserID: uid, RequestedBy: uid,
		IdempotencyKey: key, Reason: reason,
	})
	return err
}

func (s *PaymentService) validateRefundRequest(ctx context.Context, oid, uid string) (*dbent.PaymentOrder, error) {
...
func (s *PaymentService) PrepareRefund(ctx context.Context, oid string, amt float64, reason string, force, deduct bool) (*RefundPlan, *RefundResult, error) {
	o, err := s.entClient.PaymentOrder.Get(ctx, oid)
	if err != nil {
		return nil, nil, infraerrors.NotFound("NOT_FOUND", "order not found")
	}
	if o.OrderType != payment.OrderTypeBalance {
		return nil, nil, infraerrors.BadRequest("INVALID_ORDER_TYPE", "subscription cancellation does not use payment refunds")
	}
	...
	if rr == "" {
		rr = fmt.Sprintf("refund order:%v", o.ID)
	}
...
func (s *PaymentService) prepDeduct(ctx context.Context, o *dbent.PaymentOrder, p *RefundPlan, force bool) *RefundResult {
	if o.OrderType == payment.OrderTypeSubscription {
		...
		if o.SubscriptionGroupID != nil && o.SubscriptionDays != nil {
			...
			if p.RefundAmount < o.Amount && o.Amount > 0 {
				p.SubDaysToDeduct = int(math.Round(float64(*o.SubscriptionDays) * p.RefundAmount / o.Amount))
				if p.SubDaysToDeduct < 1 {
					p.SubDaysToDeduct = 1
				}
			}
...
type availableBalanceDeductor interface {
	DeductAvailableBalance(ctx context.Context, id string, amount float64) (float64, error)
}

func (s *PaymentService) deductAvailableBalance(ctx context.Context, userID string, amount float64) (float64, error) {
...
func (s *PaymentService) ExecuteRefund(ctx context.Context, p *RefundPlan) (*RefundResult, error) {
	...
	if p.DeductionType == payment.DeductionTypeSubscription && p.SubDaysToDeduct > 0 && p.SubscriptionID != "" {
		if !s.hasAuditLog(ctx, p.OrderID, "REFUND_ROLLBACK_FAILED") {
...
func (s *PaymentService) gwRefund(ctx context.Context, p *RefundPlan) (*payment.RefundResponse, error) {
	...
	resp, err := prov.Refund(ctx, payment.RefundRequest{
		...
		RequestID: fmt.Sprintf("legacy-refund-%s-%.2f", p.Order.ID, p.RefundAmount),
...
func (s *PaymentService) QueryAndFinalizeRefund(ctx context.Context, oid string) (*RefundResult, error) {
	newRefundCount, countErr := s.entClient.PaymentRefund.Query().Where(
		paymentrefund.OrderIDEQ(oid), paymentrefund.StatusEQ(RefundStatusPending),
	).Count(ctx)
	if countErr == nil && newRefundCount > 0 {
		result, err := s.QueryPaymentRefund(ctx, oid)
		if err != nil {
			return nil, err
		}
		return &RefundResult{
			Success: result.Status == RefundStatusSucceeded,
			Warning: func() string {
				if result.Status == RefundStatusPending {
					return "gateway refund is still pending confirmation"
				}
			...
			BalanceDeducted: result.BasePoints + result.BonusPoints - result.BonusExpiredOffset,
		}, nil
	}
	o, err := s.entClient.PaymentOrder.Get(ctx, oid)
	if err != nil {
	...
	if o.OrderType != payment.OrderTypeBalance {
		return nil, infraerrors.BadRequest("INVALID_ORDER_TYPE", "subscription cancellation does not use payment refunds")
	}
...
func (s *PaymentService) refundFinalizePlan(o *dbent.PaymentOrder) *RefundPlan {
	...
	if reason == "" {
		reason = fmt.Sprintf("refund order:%v", o.ID)
	}
	return &RefundPlan{
		OrderID:       o.ID,
		Order:         o,
...
func (s *PaymentService) latestRefundPendingDetail(ctx context.Context, oid string) refundPendingAuditDetail {
	logEntry, err := s.entClient.PaymentAuditLog.Query().
		Where(paymentauditlog.OrderIDEQ(oid), paymentauditlog.ActionEQ("REFUND_PENDING")).
...
func (s *PaymentService) getRefundProvider(ctx context.Context, o *dbent.PaymentOrder) (payment.Provider, error) {
	...
	if inst == nil {
		return nil, fmt.Errorf("refund provider instance is unavailable for order %v", o.ID)
	}
...
func (s *PaymentService) markRefundOk(ctx context.Context, p *RefundPlan) (*RefundResult, error) {
	if err := s.restoreSubscriptionWalletRefund(ctx, p); err != nil {
		return nil, err
	}
...
func (s *PaymentService) restoreSubscriptionWalletRefund(ctx context.Context, p *RefundPlan) error {
	if p == nil || p.Order == nil || p.Order.OrderType != payment.OrderTypeSubscription || p.Order.WalletHoldID == nil || p.Order.WalletAmount <= 0 || p.Order.Amount <= 0 {
		return nil
	}
	amount := p.Order.WalletAmount * p.RefundAmount / p.Order.Amount
	if amount <= 0 {
		return nil
	}
	_, err := s.userRepo.RefundWalletHold(ctx, *p.Order.WalletHoldID, amount, fmt.Sprintf("wallet-subscription-refund:%s:%.8f", p.Order.ID, p.RefundAmount))
	if err != nil {
		return fmt.Errorf("restore subscription wallet refund: %w", err)
	}
	return nil
}
