...
func (s *GatewayService) SelectAccount(ctx context.Context, groupID *string, sessionHash string) (*Account, error) {
	return s.SelectAccountForModel(ctx, groupID, sessionHash, "")
}

// SelectAccountForModel 选择支持指定模型的账号（粘性会话+优先级+模型映射）
func (s *GatewayService) SelectAccountForModel(ctx context.Context, groupID *string, sessionHash string, requestedModel string) (*Account, error) {
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}, metadataUserID string, easysub2apiUserID string) (*AccountSelectionResult, error) {
	...
	if billingUserID == "" {
		billingUserID, _ = ctx.Value(ctxkey.UserID).(string)
	}
	if s.userLevelService != nil && billingUserID != "" && len(groupIDs) > 0 {
	...
	return s.selectAccountWithLoadAwarenessForGroup(ctx, groupID, sessionHash, requestedModel, excludedIDs, metadataUserID, easysub2apiUserID)
}

func (s *GatewayService) selectAccountWithLoadAwarenessForGroup(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}, metadataUserID string, easysub2apiUserID string) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
	excludedIDsList := make([]string, 0, len(excludedIDs))
	...
	if prefetch := prefetchedStickyAccountIDFromContext(ctx, groupID); prefetch != "" {
	...
	if s.debugModelRoutingEnabled() && requestedModel != "" {
		...
		logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] select entry: group_id=%v group_platform=%s model=%s session=%s sticky_account=%v load_batch=%v concurrency=%v",
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		// 复制排除列表，用于会话限制拒绝时的重试
		localExcluded := make(map[string]struct{})
		...
		for {
			...
			if stickyAccountID != "" && stickyAccountID == account.ID && s.concurrencyService != nil {
	...
	isExcluded := func(accountID string) bool {
		if excludedIDs == nil {
			return false
		}
	...
	if group != nil && requestedModel != "" && platform == PlatformAnthropic &&
		...
		if s.debugModelRoutingEnabled() {
			logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] context group routing: group_id=%v model=%s enabled=%v rules=%v matched_ids=%v session=%s sticky_account=%v",
			...
			if len(routingAccountIDs) == 0 && group.ModelRoutingEnabled && len(group.ModelRouting) > 0 {
				...
				logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] context group routing miss: group_id=%v model=%s patterns(sample)=%v", group.ID, requestedModel, keys)
			}
		}
	}
	...
	if len(routingAccountIDs) > 0 && s.concurrencyService != nil {
		...
		var modelScopeSkippedIDs []string // 记录因模型限流被跳过的账号 ID
		...
		if s.debugModelRoutingEnabled() {
			logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] routed candidates: group_id=%v model=%s routed=%v candidates=%v filtered(excluded=%v missing=%v unsched=%v platform=%v model_scope=%v model_mapping=%v channel_restricted=%v window_cost=%v)",
		...
		if len(routingCandidates) > 0 {
			// 1.5. 在路由账号范围内检查粘性会话
			if sessionHash != "" && stickyAccountID != "" {
				slog.Debug("sticky.layer1_5_checking",
					"sticky_account_id", stickyAccountID,
					"in_routing_list", containsEntityID(routingAccountIDs, stickyAccountID),
					"is_excluded", isExcluded(stickyAccountID),
					"in_account_map", func() bool { _, ok := accountByID[stickyAccountID]; return ok }(),
					"session", shortSessionHash(sessionHash),
				)
				if containsEntityID(routingAccountIDs, stickyAccountID) && !isExcluded(stickyAccountID) {
					// 粘性账号在路由列表中，优先使用
					if stickyAccount, ok := accountByID[stickyAccountID]; ok {
						...
						if rpmPass { // 粘性会话窗口费用+RPM 检查
							...
							if err == nil && result.Acquired {
								...
								if !s.checkAndRegisterSession(ctx, stickyAccount, sessionHash) {
									...
									if s.debugModelRoutingEnabled() {
										logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] routed sticky hit: group_id=%v model=%s session=%s account=%v", derefGroupID(groupID), requestedModel, shortSessionHash(sessionHash), stickyAccountID)
									}
						...
						if stickyCacheMissReason != "" {
							...
							logger.LegacyPrintf("service.gateway", "[StickyCacheMiss] reason=%s account_id=%v session=%s current_rpm=%v base_rpm=%v",
								stickyCacheMissReason, stickyAccountID, shortSessionHash(sessionHash), currentRPM, baseRPM)
						}
					} else {
						_ = s.cache.DeleteSessionAccountID(ctx, derefGroupID(groupID), sessionHash)
						logger.LegacyPrintf("service.gateway", "[StickyCacheMiss] reason=account_cleared account_id=%v session=%s current_rpm=0 base_rpm=0",
			...
			if len(routingAvailable) > 0 {
				...
				for _, item := range routingAvailable {
					...
					if err == nil && result.Acquired {
						...
						if s.debugModelRoutingEnabled() {
							logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] routed select: group_id=%v model=%s session=%s account=%v", derefGroupID(groupID), requestedModel, shortSessionHash(sessionHash), item.account.ID)
						}
				...
				for _, item := range routingAvailable {
					...
					if s.debugModelRoutingEnabled() {
						logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] routed wait: group_id=%v model=%s session=%s account=%v", derefGroupID(groupID), requestedModel, shortSessionHash(sessionHash), item.account.ID)
					}
	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID != "" && !isExcluded(stickyAccountID) {
		accountID := stickyAccountID
		if accountID != "" && !isExcluded(accountID) {
		...
		slog.Debug("sticky.layer1_5_no_routing_skip",
			"sticky_account_id", stickyAccountID,
			"is_excluded", func() bool { return stickyAccountID != "" && isExcluded(stickyAccountID) }(),
			"session", shortSessionHash(sessionHash),
			"reason", func() string {
				if stickyAccountID == "" {
...
func (s *GatewayService) tryAcquireByLegacyOrder(ctx context.Context, candidates []*Account, groupID *string, sessionHash string, preferOAuth bool) (*AccountSelectionResult, bool, error) {
...
func (s *GatewayService) groupFromContext(ctx context.Context, groupID string) *Group {
...
func (s *GatewayService) resolveGroupByID(ctx context.Context, groupID string) (*Group, error) {
...
func (s *GatewayService) routingAccountIDsForRequest(ctx context.Context, groupID *string, requestedModel string, platform string) []string {
	...
	if group.Platform != PlatformAnthropic && group.Platform != PlatformComposite {
		if s.debugModelRoutingEnabled() {
			logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] skip: non-anthropic group platform: group_id=%v group_platform=%s model=%s", group.ID, group.Platform, requestedModel)
		}
		return nil
	}
	ids := group.GetRoutingAccountIDs(requestedModel)
	if s.debugModelRoutingEnabled() {
		logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] routing lookup: group_id=%v model=%s enabled=%v rules=%v matched_ids=%v",
			group.ID, requestedModel, group.ModelRoutingEnabled, len(group.ModelRouting), ids)
	}
	return ids
}

func (s *GatewayService) resolveGatewayGroup(ctx context.Context, groupID *string) (*Group, *string, error) {
	if groupID == nil {
		return nil, nil, nil
	}
...
func (s *GatewayService) checkClaudeCodeRestriction(ctx context.Context, groupID *string) (*Group, *string, error) {
	if groupID == nil {
...
func (s *GatewayService) resolvePlatform(ctx context.Context, groupID *string, group *Group, requestedModel string) (string, bool, error) {
	forcePlatform, hasForcePlatform := ctx.Value(ctxkey.ForcePlatform).(string)
	if hasForcePlatform && forcePlatform != "" {
...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, groupID *string, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	if groupIDs := apiKeyGroupIDsFromContext(ctx, groupID); len(groupIDs) > 1 {
		accounts, useMixed, err := listSchedulableAccountsForGroups(ctx, s.accountRepo, groupIDs, platform, hasForcePlatform)
		if err != nil {
...
func apiKeyGroupIDsFromContext(ctx context.Context, primary *string) []string {
	if ctx != nil {
		if ids, ok := ctx.Value(ctxkey.APIKeyGroupIDs).([]string); ok {
			ids = NormalizeAPIKeyGroupIDs(ids)
			if len(ids) > 0 {
				return ids
			}
		}
...
func listSchedulableAccountsForGroups(ctx context.Context, repo AccountRepository, groupIDs []string, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	_ = hasForcePlatform
	useMixed := false
	...
	for _, groupID := range groupIDs {
		var current []Account
		var err error
		current, err = repo.ListSchedulableByGroupIDAndPlatform(ctx, groupID, platform)
		if err != nil {
			return nil, useMixed, err
		}
...
func (s *GatewayService) IsSingleAntigravityAccountGroup(ctx context.Context, groupID *string) bool {
...
func (s *GatewayService) isAccountInGroup(account *Account, groupID *string) bool {
	if account == nil {
		return false
	}
	if groupID == nil {
...
func (s *GatewayService) tryAcquireAccountSlot(ctx context.Context, accountID string, maxConcurrency int) (*AcquireResult, error) {
...
type usageLogWindowStatsBatchProvider interface {
	GetAccountWindowStatsBatch(ctx context.Context, accountIDs []string, startTime time.Time) (map[string]*usagestats.AccountStats, error)
}
...
func windowCostFromPrefetchContext(ctx context.Context, accountID string) (float64, bool) {
	if ctx == nil || accountID == "" {
		return 0, false
	}
	m, ok := ctx.Value(windowCostPrefetchContextKey).(map[string]float64)
	if !ok || len(m) == 0 {
		return 0, false
	}
...
func (s *GatewayService) withWindowCostPrefetch(ctx context.Context, accounts []Account) context.Context {
	...
	costs := make(map[string]float64, len(accountIDs))
	...
	missingByStart := make(map[int64][]string)
...
func rpmFromPrefetchContext(ctx context.Context, accountID string) (int, bool) {
	if v, ok := ctx.Value(rpmPrefetchContextKey).(map[string]int); ok {
...
func (s *GatewayService) IncrementAccountRPM(ctx context.Context, accountID string) error {
	if s.rpmCache == nil {
		return nil
	}
...
func (s *GatewayService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	if hydrated == nil {
		return nil, fmt.Errorf("selected gateway account %v not found during hydration", account.ID)
	}
	return hydrated, nil
}
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}, platform string) (*Account, error) {
	preferOAuth := false
	...
	if len(routingAccountIDs) > 0 {
		...
		if sessionHash != "" && s.cache != nil {
			...
			if err == nil && accountID != "" && containsEntityID(routingAccountIDs, accountID) {
				if _, excluded := excludedIDs[accountID]; !excluded {
					...
					if err == nil {
						...
						if !clearSticky && s.isGatewayAccountProfitEligible(ctx, account) && s.isAccountInGroup(account, groupID) && account.Platform == platform && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) && s.isAccountSchedulableForQuota(account) && s.isAccountSchedulableForWindowCost(ctx, account, true) && s.isAccountSchedulableForRPM(ctx, account, true) && !s.isStickyAccountUpstreamRestricted(ctx, groupID, account, requestedModel) {
							if s.debugModelRoutingEnabled() {
								logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] legacy routed sticky hit: group_id=%v model=%s session=%s account=%v", derefGroupID(groupID), requestedModel, shortSessionHash(sessionHash), accountID)
							}
							return account, nil
						}
					}
		...
		routingSet := make(map[string]struct{}, len(routingAccountIDs))
		...
		if selected != nil {
			if sessionHash != "" && s.cache != nil {
				if err := s.bindGatewayStickySessionDuringSelection(ctx, groupID, sessionHash, selected.ID); err != nil {
					logger.LegacyPrintf("service.gateway", "set session account failed: session=%s account_id=%v err=%v", sessionHash, selected.ID, err)
				}
			}
			if s.debugModelRoutingEnabled() {
				logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] legacy routed select: group_id=%v model=%s session=%s account=%v", derefGroupID(groupID), requestedModel, shortSessionHash(sessionHash), selected.ID)
			}
			return selected, nil
		}
...
type selectionFailureStats struct {
	...
	SamplePlatformIDs  []string
	SampleMappingIDs   []string
...
func (s *GatewayService) logDetailedSelectionFailure(
	...
	logger.LegacyPrintf(
		"service.gateway",
		"[SelectAccountDetailed] group_id=%v model=%s platform=%s session=%s total=%v eligible=%v excluded=%v unschedulable=%v platform_filtered=%v model_unsupported=%v model_rate_limited=%v profit_threshold=%v profit_invalid_account_rate=%v sample_platform_filtered=%v sample_model_unsupported=%v sample_model_rate_limited=%v",
...
func appendSelectionFailureSampleID(samples []string, id string) []string {
	const limit = 5
...
func appendSelectionFailureRateSample(samples []string, accountID string, remaining time.Duration) []string {
	const limit = 5
	if len(samples) >= limit {
		return samples
	}
	return append(samples, fmt.Sprintf("%v(%s)", accountID, remaining))
}

func summarizeSelectionFailureStats(stats selectionFailureStats) string {
	return fmt.Sprintf(
		"total=%v eligible=%v excluded=%v unschedulable=%v platform_filtered=%v model_unsupported=%v model_rate_limited=%v profit_threshold=%v profit_invalid_account_rate=%v",
		stats.Total,
		stats.Eligible,
		stats.Excluded,
		stats.Unschedulable,
