...
func (s *OpenAIGatewayService) forwardAnthropicViaNativeAnthropicEndpoint(
	...
	logger.LegacyPrintf("service.gateway", "[CN Anthropic 直通] account=%v(%s) platform=%s model=%s upstream=%s stream=%v",
		account.ID, account.Name, account.Platform, originalModel, upstreamModel, clientStream)

	apiKey := strings.TrimSpace(account.GetOpenAIProtocolAPIKey())
	if apiKey == "" {
		return nil, fmt.Errorf("account %v missing api_key", account.ID)
	}
...
func (s *OpenAIGatewayService) nativeAnthropicTargetURL(account *Account) (string, error) {
	...
	if baseURL == "" {
		return "", fmt.Errorf("account %v has no anthropic protocol base url", account.ID)
	}
...
func (s *OpenAIGatewayService) buildNativeAnthropicUpstreamRequest(
	...
	// 的 base 取值同源（GetAnthropicProtocolBaseURL），详见 helper 注释。
	...
	// Ollama Cloud 上游按实际 base_url 强制 Bearer，与 nativeAnthropicTargetURL 同源。
...
func (s *OpenAIGatewayService) handleNativeAnthropicStreamingResponse(
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				if errors.Is(ev.err, bufio.ErrTooLong) {
					logger.LegacyPrintf("service.gateway", "[CN Anthropic 直通] SSE line too long: account=%v max_size=%v error=%v", account.ID, maxLineSize, ev.err)
			...
			if !clientDisconnected {
				...
				if _, err := io.WriteString(w, restored); err != nil {
					...
					logger.LegacyPrintf("service.gateway", "[CN Anthropic 直通] Client disconnected during streaming, continue draining upstream for usage: account=%v", account.ID)
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.gateway", "[CN Anthropic 直通] Stream data interval timeout: account=%v model=%s interval=%s", account.ID, upstreamModel, streamInterval)
			if s.rateLimitService != nil {
		...
		case <-keepaliveCh:
			...
			if _, err := fmt.Fprint(w, "event: ping\ndata: {\"type\": \"ping\"}\n\n"); err != nil {
				clientDisconnected = true
				logger.LegacyPrintf("service.gateway", "[CN Anthropic 直通] Client disconnected during keepalive ping, continue draining upstream for usage: account=%v", account.ID)
				continue
			}
			flusher.Flush()
			lastDataAt = time.Now()
