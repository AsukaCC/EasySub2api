...
func (s *OpenAIGatewayService) BindStickySession(ctx context.Context, groupID *string, sessionHash string, accountID string) error {
...
func (s *OpenAIGatewayService) SelectAccount(ctx context.Context, groupID *string, sessionHash string) (*Account, error) {
	return s.SelectAccountForModel(ctx, groupID, sessionHash, "")
}

// SelectAccountForModel selects an account supporting the requested model
func (s *OpenAIGatewayService) SelectAccountForModel(ctx context.Context, groupID *string, sessionHash string, requestedModel string) (*Account, error) {
	return s.SelectAccountForModelWithExclusions(ctx, groupID, sessionHash, requestedModel, nil)
}

// SelectAccountForModelWithExclusions selects an account supporting the requested model while excluding specified accounts.
// SelectAccountForModelWithExclusions 选择支持指定模型的账号，同时排除指定的账号。
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}) (*Account, error) {
	return s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(ctx), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, "", "", false)
}
...
func isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	if account != nil && CheckActiveModel(requestedModel, resolveOpenAIAccountUpstreamModelForRequest(account, requestedModel, requireCompact)) != nil {
		return false
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
	if account == nil || account.Platform != platform || !account.IsOpenAICompatible() ||
		!compositeAccountOwnsRequestedAlias(ctx, account) ||
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *string, platform string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}, requireCompact bool, stickyAccountID string, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *string, platform string, sessionHash, requestedModel string, excludedIDs map[string]struct{}, requireCompact bool, stickyAccountID string, requiredCapability OpenAIEndpointCapability) *Account {
	if sessionHash == "" {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *string, platform string, accounts []Account, requestedModel string, excludedIDs map[string]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, openAISelectionFilterStats) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	compactBlocked := false
	filterStats := openAISelectionFilterStats{pool: len(accounts)}
	needsUpstreamCheck := s.needsUpstreamChannelRestrictionCheck(ctx, groupID)
	eligible := make([]*Account, 0, len(accounts))
	compactTiers := make(map[string]int, len(accounts))
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}) (*AccountSelectionResult, error) {
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *string, platform string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	if s.checkChannelPricingRestriction(ctx, groupID, requestedModel) {
		slog.Warn("channel pricing restriction blocked request",
			"group_id", derefGroupID(groupID),
	...
	// a burst must not migrate the durable conversation binding.
	...
	parentCacheL2 := make(map[string]*Account)
	parentLookupL2 := func(id string) *Account {
	...
	tryAcquireFromLoadMap := func(loadMap map[string]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *string, platform string) ([]Account, error) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	if groupIDs := apiKeyGroupIDsFromContext(ctx, groupID); len(groupIDs) > 1 {
		accounts, _, err := listSchedulableAccountsForGroups(ctx, s.accountRepo, groupIDs, platform, true)
		if err != nil {
			return nil, err
		}
...
func (s *OpenAIGatewayService) tryAcquireAccountSlot(ctx context.Context, accountID string, maxConcurrency int) (*AcquireResult, error) {
...
func (s *OpenAIGatewayService) parentAccountLookup(ctx context.Context) func(string) *Account {
	return func(id string) *Account {
		if s.accountRepo == nil {
			return nil
		}
		a, _ := s.accountRepo.GetByID(ctx, id)
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, groupID *string, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *string, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	if account == nil {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) openAIAccountMatchesSchedulingGroup(account *Account, groupID *string) bool {
...
func (s *OpenAIGatewayService) getSchedulableAccount(ctx context.Context, accountID string) (*Account, error) {
	var (
		account *Account
		err     error
	)
...
func (s *OpenAIGatewayService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	if hydrated == nil {
		return nil, fmt.Errorf("selected openai account %v not found during hydration", account.ID)
	}
	return hydrated, nil
}
