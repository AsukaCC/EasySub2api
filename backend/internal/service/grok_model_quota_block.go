...
func grokModelQuotaBlockKey(accountID string, model string) string {
	return strings.TrimSpace(strings.ToLower(model)) + "|" + accountID
}

// markGrokModelQuotaBlock soft-blocks accountID for model until the given time.
func markGrokModelQuotaBlock(accountID string, model string, until time.Time) {
	model = strings.TrimSpace(model)
	if accountID == "" || model == "" || until.IsZero() {
		return
	}
	now := time.Now()
...
func markGrokModelTransientBlock(accountID string, model string, until time.Time) {
	model = strings.TrimSpace(model)
	if accountID == "" || model == "" || until.IsZero() {
		return
	}
...
func storeGrokModelQuotaBlock(accountID string, model string, until, now time.Time) {
...
func isGrokModelQuotaBlocked(accountID string, model string, now time.Time) bool {
	model = strings.TrimSpace(model)
	if accountID == "" || model == "" {
		return false
	}
