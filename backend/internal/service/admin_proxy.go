...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/util/httputil"
)
...
func (s *adminServiceImpl) GetProxy(ctx context.Context, id string) (*Proxy, error) {
	return s.proxyRepo.GetByID(ctx, id)
}

func (s *adminServiceImpl) GetProxiesByIDs(ctx context.Context, ids []string) ([]Proxy, error) {
...
func (s *adminServiceImpl) UpdateProxy(ctx context.Context, id string, input *UpdateProxyInput) (*Proxy, error) {
...
func (s *adminServiceImpl) DeleteProxy(ctx context.Context, id string) error {
...
func (s *adminServiceImpl) BatchDeleteProxies(ctx context.Context, ids []string) (*ProxyBatchDeleteResult, error) {
...
func (s *adminServiceImpl) GetProxyAccounts(ctx context.Context, proxyID string) ([]ProxyAccountSummary, error) {
...
func (s *adminServiceImpl) TestProxy(ctx context.Context, id string) (*ProxyTestResult, error) {
	proxy, err := s.proxyRepo.GetByID(ctx, id)
	if err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) CheckProxyQuality(ctx context.Context, id string) (*ProxyQualityCheckResult, error) {
	proxy, err := s.proxyRepo.GetByID(ctx, id)
	if err != nil {
		return nil, err
	}
...
func runProxyQualityTarget(ctx context.Context, client *http.Client, target proxyQualityTarget) ProxyQualityCheckItem {
	...
	if _, ok := target.AllowedStatuses[resp.StatusCode]; ok {
		...
		if resp.StatusCode >= http.StatusOK && resp.StatusCode < http.StatusMultipleChoices {
			item.Message = fmt.Sprintf("HTTP %v", resp.StatusCode)
		} else {
			item.Message = fmt.Sprintf("HTTP %v（目标可达）", resp.StatusCode)
		}
		return item
	}
	...
	item.Status = "fail"
	item.Message = fmt.Sprintf("非预期状态码: %v", resp.StatusCode)
	return item
}
...
func (s *adminServiceImpl) saveProxyQualitySnapshot(ctx context.Context, proxyID string, result *ProxyQualityCheckResult, exitInfo *ProxyExitInfo) {
	if result == nil {
		return
	}
	score := result.Score
...
func (s *adminServiceImpl) saveProxyLatency(ctx context.Context, proxyID string, info *ProxyLatencyInfo) {
