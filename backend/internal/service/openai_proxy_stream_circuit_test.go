...
func TestOpenAIProxyStreamCircuitThresholdTTLAndSuccessReset(t *testing.T) {
	...
	tripped, _ := circuit.recordFailure("proxy-1", base)
	require.False(t, tripped)
	require.False(t, circuit.isBlocked("proxy-1", base))
	require.True(t, circuit.recordSuccess("proxy-1"))

	tripped, _ = circuit.recordFailure("proxy-1", base.Add(10*time.Second))
	require.False(t, tripped, "success must clear the previous failure observation")
	tripped, until := circuit.recordFailure("proxy-1", base.Add(20*time.Second))
	require.True(t, tripped)
	require.Equal(t, base.Add(20*time.Second+10*time.Minute), until)
	require.True(t, circuit.isBlocked("proxy-1", until.Add(-time.Nanosecond)))
	require.False(t, circuit.isBlocked("proxy-1", until), "TTL expiry must re-admit the proxy")

	tripped, _ = circuit.recordFailure("proxy-2", base)
	require.False(t, tripped)
	tripped, _ = circuit.recordFailure("proxy-2", base.Add(2*time.Minute))
...
func TestOpenAIProxyStreamCircuitCollapsesBurstFailures(t *testing.T) {
	...
	tripped, _ := circuit.recordFailure("proxy-1", base)
	require.False(t, tripped)
	tripped, _ = circuit.recordFailure("proxy-1", base.Add(time.Second))
	require.False(t, tripped, "burst failures inside the collapse interval must merge")
	tripped, _ = circuit.recordFailure("proxy-1", base.Add(2*time.Second))
	require.False(t, tripped, "burst failures inside the collapse interval must merge")
	require.False(t, circuit.isBlocked("proxy-1", base.Add(2*time.Second)))

	// A second, distinct incident past the collapse interval still trips.
	tripped, _ = circuit.recordFailure("proxy-1", base.Add(5*time.Second))
	require.True(t, tripped)
	require.True(t, circuit.isBlocked("proxy-1", base.Add(5*time.Second)))
}
...
func TestOpenAIProxyStreamCircuitActiveBlockCount(t *testing.T) {
	...
	require.Equal(t, 0, circuit.activeBlockCount(base))
	tripped, until := circuit.recordFailure("proxy-1", base)
	require.True(t, tripped)
	circuit.recordFailure("proxy-2", base) // second proxy also tripped (threshold 1)
...
func TestOpenAIProxyStreamQuarantineBypassContext(t *testing.T) {
	proxyID := "proxy-7"
	account := &Account{ID: "account-1", Platform: PlatformOpenAI, ProxyID: &proxyID}
...
func TestOpenAIProxyStreamCircuitBoundsEntries(t *testing.T) {
	...
	circuit.recordFailure("proxy-1", base)
	circuit.recordFailure("proxy-2", base.Add(time.Second))
	circuit.recordFailure("proxy-3", base.Add(2*time.Second))

	circuit.mu.Lock()
	defer circuit.mu.Unlock()
	require.Len(t, circuit.entries, 2)
	_, oldestRetained := circuit.entries["proxy-1"]
