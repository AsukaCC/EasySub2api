...
func (r *CompositeRouteResolver) Resolve(ctx context.Context, groupID string, model, endpoint string) (CompositeRouteDecision, error) {
	...
	if r != nil && r.modelOwnershipResolver != nil && groupID != "" {
		ownership, err := r.modelOwnershipResolver(ctx, groupID, model)
		if err != nil {
			// Known provider model IDs can still use the deterministic detector when
			// the account catalog is temporarily unavailable. Unknown aliases cannot.
