...
type UserMsgQueueCache interface {
	...
	AcquireLock(ctx context.Context, accountID string, requestID string, lockTtlMs int) (acquired bool, err error)
	// ReleaseLock 释放锁并记录完成时间
	ReleaseLock(ctx context.Context, accountID string, requestID string) (released bool, err error)
	// GetLastCompletedMs 获取上次完成时间（毫秒时间戳，Redis TIME 源）
	GetLastCompletedMs(ctx context.Context, accountID string) (int64, error)
...
func (s *UserMessageQueueService) TryAcquire(ctx context.Context, accountID string) (*QueueLockResult, error) {
	if s.cache == nil {
	...
	if err != nil {
		logger.LegacyPrintf("service.umq", "AcquireLock failed for account %v: %v", accountID, err)
		return &QueueLockResult{Acquired: true}, nil // fail-open
	}
...
// Release 释放串行锁
func (s *UserMessageQueueService) Release(ctx context.Context, accountID string, requestID string) error {
	if s.cache == nil || requestID == "" {
		return nil
	}
	released, err := s.cache.ReleaseLock(ctx, accountID, requestID)
	if err != nil {
		logger.LegacyPrintf("service.umq", "ReleaseLock failed for account %v: %v", accountID, err)
		return err
	}
	if !released {
		logger.LegacyPrintf("service.umq", "ReleaseLock no-op for account %v (requestID mismatch or expired)", accountID)
	}
	return nil
}
...
func (s *UserMessageQueueService) EnforceDelay(ctx context.Context, accountID string, baseRPM int) error {
	if s.cache == nil {
		return nil
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.umq", "GetLastCompletedMs failed for account %v: %v", accountID, err)
...
func (s *UserMessageQueueService) CalculateRPMAwareDelay(ctx context.Context, accountID string, baseRPM int) time.Duration {
	...
	if baseRPM <= 0 || s.rpmCache == nil {
		...
		if err != nil {
			logger.LegacyPrintf("service.umq", "GetRPM failed for account %v: %v", accountID, err)
...
func (s *UserMessageQueueService) StartCleanupWorker(interval time.Duration) {
	...
	runCleanup := func() {
		...
		if cleaned > 0 {
			logger.LegacyPrintf("service.umq", "Cleanup completed: released %v orphaned locks", cleaned)
		}
	}
