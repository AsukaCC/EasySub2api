...
func (s *NotificationEmailService) RememberRecipientLocale(ctx context.Context, userID string, email, acceptLanguage string) {
	locale := normalizeNotificationLocale(acceptLanguage)
	if strings.TrimSpace(acceptLanguage) == "" || s == nil || s.settingRepo == nil {
		return
	}
	if userID != "" {
		_ = s.settingRepo.Set(ctx, notificationEmailLocaleUserKeyPrefix+userID, locale)
	}
...
func (s *NotificationEmailService) ResolveRecipientLocale(ctx context.Context, userID string, email string) string {
	if s == nil || s.settingRepo == nil {
		return notificationEmailDefaultLocale
	}
	if userID != "" {
		if locale, err := s.settingRepo.GetValue(ctx, notificationEmailLocaleUserKeyPrefix+userID); err == nil && strings.TrimSpace(locale) != "" {
...
func validateNotificationEmailTemplate(event, subject, htmlBody string) error {
	...
	if len([]rune(subject)) > notificationEmailMaxSubjectLength {
		return fmt.Errorf("email subject cannot exceed %v characters", notificationEmailMaxSubjectLength)
	}
	if strings.TrimSpace(htmlBody) == "" {
		return errors.New("email html cannot be empty")
	}
	if len([]byte(htmlBody)) > notificationEmailMaxHTMLLength {
		return fmt.Errorf("email html cannot exceed %v bytes", notificationEmailMaxHTMLLength)
	}
...
func notificationEmailSampleVariables(locale string) map[string]string {
	if normalizeNotificationLocale(locale) == notificationEmailLocaleChinese {
		variables := map[string]string{
			...
			"current_points":       "12.34",
			"threshold":            "20.00",
			"threshold_points":     "20.00",
			"recharge_url":         "https://example.com/recharge",
			"recharge_amount":      "50.00",
			"principal_amount_cny": "50.00",
			"base_points":          "50.00",
			"bonus_points":         "5.00",
			"credited_points":      "55.00",
			"order_id":             "1024",
...
var notificationEmailEventDefinitions = map[string]NotificationEmailEventInfo{
	...
	NotificationEmailEventBalanceLow: {
		...
		Description:  "Optional alert sent when platform points cross the configured threshold.",
		Category:     "billing",
		Optional:     true,
		Placeholders: append(append([]string{}, notificationEmailCommonPlaceholders...), "current_balance", "current_points", "threshold", "threshold_points", "recharge_url", "unsubscribe_url"),
	},
	NotificationEmailEventBalanceRechargeSuccess: {
		...
		Category:     "billing",
		Optional:     false,
		Placeholders: append(append([]string{}, notificationEmailCommonPlaceholders...), "recharge_amount", "current_balance", "principal_amount_cny", "base_points", "bonus_points", "credited_points", "current_points", "order_id"),
	},
	NotificationEmailEventAccountQuotaAlert: {
...
<p class="muted"><a href="{{unsubscribe_url}}">退订此类订阅提醒</a></p>`),
	...
	NotificationEmailEventBalanceLow: {
		notificationEmailDefaultLocale: {
			Subject: "[{{site_name}}] Low platform points alert",
			HTML: notificationEmailCard("#d97706", "Low platform points alert", `
<p>Hello {{recipient_name}},</p>
<p>Your current platform points are <strong>{{current_points}} points</strong>, below the configured threshold of <strong>{{threshold_points}} points</strong>.</p>
<p>Please recharge in time to avoid service interruption.</p>
<p><a class="button" href="{{recharge_url}}">Recharge now</a></p>
<p class="muted"><a href="{{unsubscribe_url}}">Unsubscribe from optional points alerts</a></p>`),
		},
		notificationEmailLocaleChinese: {
			Subject: "[{{site_name}}] 平台积分不足提醒",
			HTML: notificationEmailCard("#d97706", "平台积分不足提醒", `
<p>{{recipient_name}}，您好：</p>
<p>您当前平台积分为 <strong>{{current_points}} 积分</strong>，已低于提醒阈值 <strong>{{threshold_points}} 积分</strong>。</p>
<p>请及时充值以免服务中断。</p>
<p><a class="button" href="{{recharge_url}}">立即充值</a></p>
<p class="muted"><a href="{{unsubscribe_url}}">退订此类积分提醒</a></p>`),
		},
	},
	NotificationEmailEventBalanceRechargeSuccess: {
		notificationEmailDefaultLocale: {
			Subject: "[{{site_name}}] Points recharge successful",
			HTML: notificationEmailCard("#16a34a", "Recharge successful", `
<p>Hello {{recipient_name}},</p>
<p>Your CNY recharge of <strong>¥{{principal_amount_cny}}</strong> has been completed.</p>
<p>Credited: <strong>{{credited_points}} points</strong> (base {{base_points}}, bonus {{bonus_points}}).</p>
<p>Current platform points: <strong>{{current_points}} points</strong></p>
<p>Order ID: {{order_id}}</p>`),
		},
		notificationEmailLocaleChinese: {
			Subject: "[{{site_name}}] 平台积分充值成功",
			HTML: notificationEmailCard("#16a34a", "平台积分充值成功", `
<p>{{recipient_name}}，您好：</p>
			<p>您的人民币充值 <strong>¥{{principal_amount_cny}}</strong> 已完成。</p>
			<p>到账 <strong>{{credited_points}} 积分</strong>（基础 {{base_points}}，赠送 {{bonus_points}}）。</p>
			<p>当前平台积分：<strong>{{current_points}} 积分</strong></p>
			<p>订单号：{{order_id}}</p>`),
		},
	},
	NotificationEmailEventAccountQuotaAlert: {
