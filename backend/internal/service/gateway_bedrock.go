...
func (s *GatewayService) ApplyBedrockCCCompat(c *gin.Context, body []byte, model string, account *Account, groupID *string) []byte {
...
func (s *GatewayService) isBedrockCCCompatEnabled(ctx context.Context, account *Account, groupID *string) bool {
...
func (s *GatewayService) forwardBedrock(
	...
	logger.LegacyPrintf("service.gateway", "[Bedrock] 命中 Bedrock 分支: account=%v name=%s model=%s->%s stream=%v",
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				logger.LegacyPrintf("service.gateway", "[Bedrock] account %v: upstream error %v, retry %v/%v after %v",
...
func (s *GatewayService) handleBedrockUpstreamErrors(
	...
	if s.shouldRetryUpstreamError(account, resp.StatusCode) {
		if s.shouldFailoverUpstreamError(resp.StatusCode) {
			...
			logger.LegacyPrintf("service.gateway", "[Bedrock] Upstream error (retry exhausted, failover): Account=%v(%s) Status=%v Body=%s",
