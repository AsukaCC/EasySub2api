...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	// 防止用户注册历史 OAuth 合成邮箱，避免第三方登录与本地账号发生碰撞。
	if isReservedEmail(email) {
		return "", nil, ErrEmailReserved
	}
	// Legacy registration access codes are retired. aff_code is canonical, while
	// invitation_code remains an OAuth compatibility alias for the same code.
	var invitationRedeemCode *RedeemCode
	registrationAffiliateCode := normalizeRegistrationAffiliateCode(invitationCode, affiliateCode)
	requiredAffiliateCode := ""
	if s.settingService.IsInvitationCodeEnabled(ctx) {
		if registrationAffiliateCode == "" {
			return "", nil, ErrInvitationCodeRequired
		}
		if s.affiliateService == nil {
			return "", nil, ErrServiceUnavailable
		}
		requiredAffiliateCode = registrationAffiliateCode
	}
	...
	user := &User{
		...
		InitialBonusValidityDays: grantPlan.BonusValidityDays,
		Concurrency:              grantPlan.Concurrency,
		RPMLimit:                 defaultRPMLimit,
		Status:                   StatusActive,
	}

	if err := s.createUserAndClaimInvitation(ctx, user, invitationRedeemCode, requiredAffiliateCode); err != nil {
	...
	if s.affiliateService != nil {
		if _, err := s.affiliateService.EnsureUserAffiliate(ctx, user.ID); err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to initialize affiliate profile for user %v: %v", user.ID, err)
		}
		if code := registrationAffiliateCode; code != "" && requiredAffiliateCode == "" {
			if err := s.affiliateService.BindInviterByCode(ctx, user.ID, code); err != nil {
				// 邀请码绑定失败不影响注册，只记录日志
				logger.LegacyPrintf("service.auth", "[Auth] Failed to bind affiliate inviter for user %v: %v", user.ID, err)
			}
		}
	}
	...
	if promoCode != "" && s.promoService != nil && s.settingService != nil && s.settingService.IsPromoCodeEnabled(ctx) {
		if err := s.promoService.ApplyPromoCode(ctx, user.ID, promoCode); err != nil {
			// 优惠码应用失败不影响注册，只记录日志
			logger.LegacyPrintf("service.auth", "[Auth] Failed to apply promo code for user %v: %v", user.ID, err)
		} else {
			// 重新获取用户信息以获取更新后的余额
...
// - 如果邮箱不存在：创建新用户并登录
//
// 注意：该函数用于面板第三方 OAuth 登录，与上游账号 OAuth 无关。
...
func (s *AuthService) loginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, invitationCode, affiliateCode, promoCode, signupSource string) (*TokenPair, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			if err := s.createOAuthUserWithRequiredAffiliate(ctx, newUser, requiredAffiliateCode); err != nil {
				if errors.Is(err, ErrEmailExists) {
					user, err = s.userRepo.GetByEmail(ctx, email)
					if err != nil {
						logger.LegacyPrintf("service.auth", "[Auth] Database error getting user after conflict: %v", err)
...
func (s *AuthService) ApplyOAuthSignupPromoCode(ctx context.Context, userID string, promoCode string) {
	if userID == "" {
		return
	}
...
func (s *AuthService) applyOAuthSignupPromoCode(ctx context.Context, user *User, promoCode string) *User {
	...
	if user == nil || user.ID == "" || promoCode == "" || s.promoService == nil || s.settingService == nil || !s.settingService.IsPromoCodeEnabled(ctx) {
		return user
	}
	if err := s.promoService.ApplyPromoCode(ctx, user.ID, promoCode); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to apply promo code for oauth user %v: %v", user.ID, err)
		return user
	}
	if updatedUser, err := s.userRepo.GetByID(ctx, user.ID); err == nil {
		return updatedUser
...
func normalizeRegistrationAffiliateCode(invitationCode, affiliateCode string) string {
...
func (s *AuthService) requiredRegistrationAffiliateCode(ctx context.Context, invitationCode, affiliateCode string) (string, error) {
	code := normalizeRegistrationAffiliateCode(invitationCode, affiliateCode)
...
func (s *AuthService) createOAuthUserWithRequiredAffiliate(ctx context.Context, user *User, requiredAffiliateCode string) error {
	commitUser := func(execCtx context.Context) error {
		...
		if _, err := s.affiliateService.BindInviterByCodeWithResult(execCtx, user.ID, requiredAffiliateCode); err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Rejected OAuth registration: affiliate invitation unavailable (user_id=%v err=%v)", user.ID, err)
	...
	if err := commitUser(txCtx); err != nil {
		return err
	}
...
func (s *AuthService) assignSubscriptions(ctx context.Context, userID string, items []DefaultSubscriptionSetting, notes string) {
...
func (s *AuthService) resolveSignupGrantPlan(ctx context.Context, signupSource string) signupGrantPlan {
	...
	plan.BonusValidityDays = s.settingService.GetBonusBalanceDefaultValidityDays(ctx)
	...
	plan.BonusValidityDays = resolved.BonusValidityDays
...
func (s *AuthService) bindOAuthAffiliate(ctx context.Context, userID string, affiliateCode string) {
	if s.affiliateService == nil || userID == "" {
		return
	}
	if _, err := s.affiliateService.EnsureUserAffiliate(ctx, userID); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to initialize affiliate profile for user %v: %v", userID, err)
	}
	if code := strings.TrimSpace(affiliateCode); code != "" {
		if err := s.affiliateService.BindInviterByCode(ctx, userID, code); err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to bind affiliate inviter for user %v: %v", userID, err)
		}
	}
}
...
func (s *AuthService) updateUserSignupSource(ctx context.Context, userID string, signupSource string) {
	...
	if err := s.entClient.User.UpdateOneID(userID).
		...
		Exec(ctx); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to update signup source: user_id=%v source=%s err=%v", userID, signupSource, err)
	}
}

func (s *AuthService) touchUserLogin(ctx context.Context, userID string) {
	if s == nil || s.entClient == nil || userID == "" {
		return
	}
	now := time.Now().UTC()
	if err := s.entClient.User.UpdateOneID(userID).
		...
		SetLastActiveAt(now).
		Exec(ctx); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to touch login timestamps: user_id=%v err=%v", userID, err)
	}
}
...
func (s *AuthService) backfillEmailIdentityOnSuccessfulLogin(ctx context.Context, user *User) {
	...
	if s.shouldApplyEmailFirstBindDefaults(ctx, user.ID, identity, created) {
		if err := s.ApplyProviderDefaultSettingsOnFirstBind(ctx, user.ID, "email"); err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to apply email first bind defaults: user_id=%v err=%v", user.ID, err)
		}
	}
}
...
func (s *AuthService) shouldApplyEmailFirstBindDefaults(
	...
	if s == nil || s.entClient == nil || userID == "" || identity == nil || identity.UserID != userID {
		return false
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to inspect email first bind grant state: user_id=%v err=%v", userID, err)
		return false
	}
	return !hasGrant
}
...
func (s *AuthService) ensureEmailAuthIdentity(ctx context.Context, user *User, source string) (*dbent.AuthIdentity, bool) {
	if s == nil || s.entClient == nil || user == nil || user.ID == "" {
		return nil, false
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to inspect email auth identity: user_id=%v email=%s err=%v", user.ID, email, err)
		return nil, false
	}
	...
	if !existed {
		...
		if err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to ensure email auth identity: user_id=%v email=%s err=%v", user.ID, email, err)
			return nil, false
		}
	}
	...
	if err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to reload email auth identity: user_id=%v email=%s err=%v", user.ID, email, err)
		return nil, false
	}
	if identity.UserID != user.ID {
		logger.LegacyPrintf("service.auth", "[Auth] Email auth identity ownership mismatch: user_id=%v email=%s owner_id=%v", user.ID, email, identity.UserID)
		return nil, false
	}
...
// 最终可用性仍由 Use 的条件更新兜底。
func (s *AuthService) createUserAndClaimInvitation(ctx context.Context, user *User, invitation *RedeemCode, requiredAffiliateCode string) error {
	commitUser := func(execCtx context.Context) error {
		...
		if requiredAffiliateCode != "" {
			...
			if _, err := s.affiliateService.BindInviterByCodeWithResult(execCtx, user.ID, requiredAffiliateCode); err != nil {
				logger.LegacyPrintf("service.auth", "[Auth] Rejected registration: affiliate invitation unavailable (user_id=%v err=%v)", user.ID, err)
				return ErrInvitationCodeInvalid
			}
		}
		if invitation == nil {
	...
	if invitation == nil && requiredAffiliateCode == "" {
		return commitUser(ctx)
	}
	if s.entClient == nil {
		return commitUser(ctx)
...
func (s *AuthService) ResetPassword(ctx context.Context, email, token, newPassword string) error {
	...
	if err := s.userRepo.Update(ctx, user, UserUpdateFields{PasswordHash: true}); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Database error updating password for user %v: %v", user.ID, err)
		return ErrServiceUnavailable
	}

	// Also revoke all refresh tokens for this user
	if err := s.RevokeAllUserSessions(ctx, user.ID); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to revoke refresh tokens for user %v: %v", user.ID, err)
...
func (s *AuthService) RefreshTokenPair(ctx context.Context, refreshToken string) (*TokenPairWithUser, error) {
	...
	if s.settingService != nil && s.settingService.IsSessionBindingEnabled(ctx) && data.BindingHash != "" {
		if current := sessionBindingHashFromContext(ctx); current != "" && current != data.BindingHash {
			_ = s.refreshTokenCache.DeleteTokenFamily(ctx, data.FamilyID)
			logger.LegacyPrintf("service.auth", "[Auth] Session binding mismatch on refresh for user %v, family revoked", data.UserID)
...
func (s *AuthService) RevokeAllUserTokens(ctx context.Context, userID string) error {
	...
	if err := s.RevokeAllUserSessions(ctx, userID); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to revoke refresh sessions after token invalidation for user %v: %v", userID, err)
	}
	return nil
}
...
func (s *AuthService) snapshotPlatformQuotaDefaults(ctx context.Context, userID string, plan *signupGrantPlan) error {
	...
	if err := s.userPlatformQuotaRepo.BulkInsertInitial(ctx, records); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Warning: snapshot platform quota failed user=%v: %v (fail-open)", userID, err)
