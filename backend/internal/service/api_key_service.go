...
type APIKeyRepository interface {
	...
	GetKeyAndOwnerID(ctx context.Context, id string) (string, string, error)
	...
	DeleteWithAudit(ctx context.Context, id string) error

	ListByUserID(ctx context.Context, userID string, params pagination.PaginationParams, filters APIKeyListFilters) ([]APIKey, *pagination.PaginationResult, error)
	VerifyOwnership(ctx context.Context, userID string, apiKeyIDs []string) ([]string, error)
	CountByUserID(ctx context.Context, userID string) (int64, error)
	ExistsByKey(ctx context.Context, key string) (bool, error)
	ListByGroupID(ctx context.Context, groupID string, params pagination.PaginationParams) ([]APIKey, *pagination.PaginationResult, error)
	SearchAPIKeys(ctx context.Context, userID string, keyword string, limit int) ([]APIKey, error)
	ClearGroupIDByGroupID(ctx context.Context, groupID string) (int64, error)
	// UpdateGroupIDByUserAndGroup 将用户下绑定 oldGroupID 的所有 Key 迁移到 newGroupID
	UpdateGroupIDByUserAndGroup(ctx context.Context, userID, oldGroupID, newGroupID string) (int64, error)
	CountByGroupID(ctx context.Context, groupID string) (int64, error)
	ListKeysByUserID(ctx context.Context, userID string) ([]string, error)
	ListKeysByGroupID(ctx context.Context, groupID string) ([]string, error)

	// Quota methods
	IncrementQuotaUsed(ctx context.Context, id string, amount float64) (float64, error)
	UpdateLastUsed(ctx context.Context, id string, usedAt time.Time) error

	// Rate limit methods
	IncrementRateLimitUsage(ctx context.Context, id string, cost float64) error
	ResetRateLimitWindows(ctx context.Context, id string) error
	GetRateLimitData(ctx context.Context, id string) (*APIKeyRateLimitData, error)
}

type apiKeyAllByUserIDLister interface {
	ListAllByUserID(ctx context.Context, userID string, filters APIKeyListFilters) ([]APIKey, error)
}
...
type APIKeyCache interface {
	GetCreateAttemptCount(ctx context.Context, userID string) (int, error)
	IncrementCreateAttemptCount(ctx context.Context, userID string) error
	DeleteCreateAttemptCount(ctx context.Context, userID string) error
...
type APIKeyAuthCacheInvalidator interface {
	...
	InvalidateAuthCacheByUserID(ctx context.Context, userID string)
	InvalidateAuthCacheByGroupID(ctx context.Context, groupID string)
}
...
type CreateAPIKeyRequest struct {
	...
	// Quota fields
	Quota         float64 `json:"quota"`           // Quota limit in platform points (0 = unlimited)
...
type UpdateAPIKeyRequest struct {
	...
	// Quota fields
	Quota           *float64   `json:"quota"`       // Platform point quota (nil = no change, 0 = unlimited)
...
// NormalizeAPIKeyGroupIDs de-duplicates group IDs while preserving the
// administrator's order. The first ID is used as the primary group.
func NormalizeAPIKeyGroupIDs(ids []string) []string {
...
type RateLimitCacheInvalidator interface {
	InvalidateAPIKeyRateLimit(ctx context.Context, keyID string) error
}
...
func (s *APIKeyService) checkAPIKeyRateLimit(ctx context.Context, userID string) error {
	if s.cache == nil {
		return nil
	}
...
func (s *APIKeyService) incrementAPIKeyErrorCount(ctx context.Context, userID string) {
	if s.cache == nil {
		return
	}
...
// Create 创建API Key
func (s *APIKeyService) Create(ctx context.Context, userID string, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	groupIDs := NormalizeAPIKeyGroupIDs(req.GroupIDs)
	if len(groupIDs) == 0 && req.GroupID != nil {
		groupIDs = NormalizeAPIKeyGroupIDs([]string{*req.GroupID})
	}
	// 验证所有分组权限
...
// List 获取用户的API Key列表
func (s *APIKeyService) List(ctx context.Context, userID string, params pagination.PaginationParams, filters APIKeyListFilters) ([]APIKey, *pagination.PaginationResult, error) {
...
func (s *APIKeyService) listByCurrentConcurrency(ctx context.Context, userID string, params pagination.PaginationParams, filters APIKeyListFilters) ([]APIKey, *pagination.PaginationResult, error) {
...
func (s *APIKeyService) currentConcurrencyForAPIKey(ctx context.Context, apiKeyID string) int {
	if s == nil || s.concurrencyService == nil || apiKeyID == "" {
		return 0
	}
	counts, err := s.concurrencyService.GetAPIKeyConcurrencyBatch(ctx, []string{apiKeyID})
	if err != nil {
		return 0
	}
...
func (s *APIKeyService) VerifyOwnership(ctx context.Context, userID string, apiKeyIDs []string) ([]string, error) {
	if len(apiKeyIDs) == 0 {
		return []string{}, nil
	}
...
// Update 更新API Key
func (s *APIKeyService) Update(ctx context.Context, id string, userID string, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.GroupIDs != nil {
		...
		apiKey.GroupIDs = NormalizeAPIKeyGroupIDs([]string{*req.GroupID})
		fields.GroupID = true
		fields.GroupIDs = true
	}
...
// Delete 删除API Key
func (s *APIKeyService) Delete(ctx context.Context, id string, userID string) error {
...
// 该操作为尽力而为，不应阻塞主请求链路。
func (s *APIKeyService) TouchLastUsed(ctx context.Context, keyID string) error {
	if keyID == "" {
		return nil
	}
	...
	_, err, _ := s.lastUsedTouchSF.Do(keyID, func() (any, error) {
		latest := time.Now()
		if v, ok := s.lastUsedTouchL1.Load(keyID); ok {
...
func (s *APIKeyService) IncrementUsage(ctx context.Context, keyID string) error {
	// 使用Redis计数器
	if s.cache != nil {
		cacheKey := fmt.Sprintf("apikey:usage:%v:%s", keyID, timezone.Now().Format("2006-01-02"))
...
// - 订阅类型分组：用户有有效订阅的
func (s *APIKeyService) GetAvailableGroups(ctx context.Context, userID string) ([]Group, error) {
	// 获取用户信息
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get user: %w", err)
	...
	// 构建订阅分组 ID 集合
	subscribedGroupIDs := make(map[string]bool)
...
func (s *APIKeyService) canUserBindGroupInternal(user *User, group *Group, subscribedGroupIDs map[string]bool) bool {
	// 订阅类型分组：需要有效订阅
	if group.IsSubscriptionType() {
...
func (s *APIKeyService) SearchAPIKeys(ctx context.Context, userID string, keyword string, limit int) ([]APIKey, error) {
...
// GetUserAllowedGroupIDSet 返回 user_allowed_groups 授权及有效订阅对应的专属分组 ID 集合。
//
// 与 GetAvailableGroups 的区别：这里是「橱窗」语义（模型广场用），保留普通授权分组，
// 不关心分组是否活跃；有效订阅也授予对应分组的可见性。返回值恒非 nil。
func (s *APIKeyService) GetUserAllowedGroupIDSet(ctx context.Context, userID string) (map[string]struct{}, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get user: %w", err)
	}
	allowed := make(map[string]struct{}, len(user.AllowedGroups))
...
func (s *APIKeyService) UpdateQuotaUsed(ctx context.Context, apiKeyID string, cost float64) error {
	if cost <= 0 {
		return nil
	}

	type quotaStateReader interface {
		IncrementQuotaUsedAndGetState(ctx context.Context, id string, amount float64) (*APIKeyQuotaUsageState, error)
	}
...
func (s *APIKeyService) GetRateLimitData(ctx context.Context, id string) (*APIKeyRateLimitData, error) {
...
func (s *APIKeyService) UpdateRateLimitUsage(ctx context.Context, apiKeyID string, cost float64) error {
	if cost <= 0 {
		return nil
	}
