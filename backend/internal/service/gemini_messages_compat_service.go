...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/googleapi"
...
func (s *GeminiMessagesCompatService) SelectAccountForModel(ctx context.Context, groupID *string, sessionHash string, requestedModel string) (*Account, error) {
	return s.SelectAccountForModelWithExclusions(ctx, groupID, sessionHash, requestedModel, nil)
}

func (s *GeminiMessagesCompatService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *string, sessionHash string, requestedModel string, excludedIDs map[string]struct{}) (*Account, error) {
	// 1. 确定目标平台和调度模式
...
func (s *GeminiMessagesCompatService) resolvePlatformAndSchedulingMode(ctx context.Context, groupID *string) (platform string, useMixedScheduling bool, hasForcePlatform bool, err error) {
...
func (s *GeminiMessagesCompatService) isAccountUsableForRequestWithPrecheck(
	...
	requestedModel, platform string,
	useMixedScheduling bool,
	precheckResult map[string]bool,
) bool {
	// 检查模型调度能力
...
func (s *GeminiMessagesCompatService) passesRateLimitPreCheckWithCache(ctx context.Context, account *Account, requestedModel string, precheckResult map[string]bool) bool {
	...
	if err != nil {
		logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini PreCheck] Account %s precheck error: %v", account.ID, err)
	}
	return ok
}
...
func (s *GeminiMessagesCompatService) buildPreCheckUsageResultMap(ctx context.Context, accounts []Account, requestedModel string) map[string]bool {
...
func (s *GeminiMessagesCompatService) getSchedulableAccount(ctx context.Context, accountID string) (*Account, error) {
...
func (s *GeminiMessagesCompatService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	if hydrated == nil {
		return nil, fmt.Errorf("selected gemini account %s not found during hydration", account.ID)
	}
	return hydrated, nil
}

func (s *GeminiMessagesCompatService) listSchedulableAccountsOnce(ctx context.Context, groupID *string, platform string, hasForcePlatform bool) ([]Account, error) {
	if s.schedulerSnapshot != nil {
...
func (s *GeminiMessagesCompatService) HasAntigravityAccounts(ctx context.Context, groupID *string) (bool, error) {
...
func (s *GeminiMessagesCompatService) SelectAccountForAIStudioEndpoints(ctx context.Context, groupID *string) (*Account, error) {
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			if attempt < geminiMaxRetries {
				logger.LegacyPrintf("service.gemini_messages_compat", "Gemini account %s: upstream request failed, retry %d/%d: %v", account.ID, attempt, geminiMaxRetries, err)
		...
		if resp.StatusCode == http.StatusBadRequest && signatureRetryStage < 2 {
			...
			if isGeminiSignatureRelatedError(respBody) {
				...
				if txErr == nil {
					logger.LegacyPrintf("service.gemini_messages_compat", "Gemini account %s: detected signature-related 400, retrying with downgraded Claude blocks (%s)", account.ID, stageName)
		...
		if resp.StatusCode >= 400 && s.shouldRetryGeminiUpstreamError(account, resp.StatusCode) {
			...
			if attempt < geminiMaxRetries {
				...
				logger.LegacyPrintf("service.gemini_messages_compat", "Gemini account %s: upstream status %d, retry %d/%d", account.ID, resp.StatusCode, attempt, geminiMaxRetries)
				sleepGeminiBackoff(attempt)
				continue
			}
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest {
			...
			if isGoogleProjectConfigError(msg400) {
				...
				log.Printf("[Gemini] status=400 google_config_error failover=true upstream_message=%q account=%s", upstreamMsg, account.ID)
				appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
					Platform:           account.Platform,
					AccountID:          account.ID,
					ProxyID:            opsUpstreamProxyID(account),
...
func (s *GeminiMessagesCompatService) handleGeminiUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, body []byte) {
	...
	if resetAt == nil {
		...
		if isCodeAssist || oauthType == "google_one" {
			...
			if isCodeAssist {
				logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %s (Code Assist, tier=%s, project=%s) rate limited, cooldown=%v", account.ID, tierID, projectID, time.Until(ra).Truncate(time.Second))
			} else {
				logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %s (Google One OAuth, tier=%s, project=%s) rate limited, cooldown=%v", account.ID, tierID, projectID, time.Until(ra).Truncate(time.Second))
			}
		} else if account.IsVertexServiceAccount() {
			// Vertex 按量付费：短窗限速，无每日配额，短冷却后即可恢复
			ra = time.Now().Add(geminiVertexFallbackCooldown)
			logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %v (Vertex service account, project=%s) rate limited, cooldown=%v", account.ID, projectID, time.Until(ra).Truncate(time.Second))
		} else {
			// API Key / AI Studio OAuth: PST 午夜
			if ts := nextGeminiDailyResetUnix(); ts != nil {
				ra = time.Unix(*ts, 0)
				logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %s (API Key/AI Studio, type=%s) rate limited, reset at PST midnight (%v)", account.ID, account.Type, ra)
			} else {
				// 兜底：5 分钟
				ra = time.Now().Add(5 * time.Minute)
				logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %s rate limited, fallback to 5min", account.ID)
			}
		}
	...
	logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] Account %s rate limited until %v (oauth_type=%s, tier=%s)",
