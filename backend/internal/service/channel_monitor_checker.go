...
func runCheckForModel(ctx context.Context, provider, endpoint, apiKey, model string, opts *CheckOptions) *CheckResult {
	...
	if statusCode < 200 || statusCode >= 300 {
		...
		res.Message = truncateMessage(sanitizeErrorMessage(fmt.Sprintf("upstream HTTP %v: %s", statusCode, bodySnippet)))
		return res
	}
...
func finalizeOperationalOrDegraded(res *CheckResult, latency time.Duration, latencyMs int) *CheckResult {
	if latency >= monitorDegradedThreshold {
		...
		res.Message = truncateMessage(fmt.Sprintf("slow response: %vms", latencyMs))
		return res
	}
...
//  2. 上游 HTTP body 文本里直接出现的 sk-* / xai-* / JWT 等密钥碎片
//
// 监控模块独立维护该规则，避免与网关错误处理耦合。
