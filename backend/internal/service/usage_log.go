...
const (
	...
	RequestTypeTest         RequestType = 6
)

func (t RequestType) IsValid() bool {
	switch t {
	case RequestTypeUnknown, RequestTypeSync, RequestTypeStream, RequestTypeWSV2, RequestTypeCyberBlocked, RequestTypeLive, RequestTypeTest:
		return true
	default:
		return false
	}
...
func ParseUsageRequestType(value string) (RequestType, error) {
	...
	case "test":
		return RequestTypeTest, nil
	default:
		return RequestTypeUnknown, fmt.Errorf("invalid request_type, allowed values: unknown, sync, stream, ws_v2, cyber, live, test")
	}
}
...
type UsageLog struct {
	...
	ServiceTier *string
	// ReasoningEffort is the effective effort after group policy and model-family remapping.
