...
import (
	...
	openaiwsv2 "github.com/AsukaCC/EasySub2api/internal/service/openai_ws_v2"
...
type openAIWSPassthroughUsageMeta struct {
	...
	nativeCompactionV2       atomic.Bool
...
func (m *openAIWSPassthroughUsageMeta) captureInboundReasoningAndCompaction(originalBody []byte, modelCandidates ...string) {
	if m == nil {
		return
	}
	...
	m.nativeCompactionV2.Store(HasCompactionTriggerInInput(originalBody))
}
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	logOpenAIWSV2Passthrough(
		"relay_start account_id=%v model=%s previous_response_id=%s first_message_type=%s first_message_bytes=%v",
		account.ID,
	...
	capturedReasoningModel := initialRequestModel
	...
	usageMeta.captureInboundReasoningAndCompaction(originalFirstClientMessage, capturedReasoningModel, capturedSessionModel)
	...
	logOpenAIWSV2Passthrough(
		"relay_dial_start account_id=%v ws_host=%s ws_path=%s proxy_enabled=%v",
		account.ID,
		wsHost,
		wsPath,
	...
	for {
		...
		logOpenAIWSV2Passthrough(
			"relay_dial_failed account_id=%v status_code=%v err=%s",
			account.ID,
			statusCode,
	...
	logOpenAIWSV2Passthrough(
		"relay_dial_ok account_id=%v status_code=%v upstream_request_id=%s",
		account.ID,
		statusCode,
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			var originalResponseCreate []byte
			if isResponseCreate {
				...
				policyModel := strings.TrimSpace(gjson.GetBytes(payload, "model").String())
				if policyModel == "" {
					policyModel = capturedReasoningModel
				}
				if next, policyErr := applyOpenAIWSReasoningEffortPolicyForModel(payload, hooks, policyModel); policyErr != nil {
			...
			if strings.TrimSpace(gjson.GetBytes(payload, "type").String()) == "session.update" {
				if updated := strings.TrimSpace(gjson.GetBytes(payload, "session.model").String()); updated != "" {
					capturedReasoningModel = updated
				}
			}
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			FirstTokenPredicate: func(payload []byte, eventType string) bool {
				return openAIWSMessageStartsTTFT(payload, eventType, ttftMode)
			},
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				turnResult := &OpenAIForwardResult{
					...
					NativeCompactionV2:            usageMeta.nativeCompactionV2.Load(),
					Stream:                        true,
					OpenAIWSMode:                  true,
				...
				logOpenAIWSV2Passthrough(
					"relay_turn_completed account_id=%v turn=%v request_id=%s terminal_event=%s turn_requested_model=%s turn_upstream_model=%s duration_ms=%v first_token_ms=%v input_tokens=%v output_tokens=%v cache_read_tokens=%v",
					account.ID,
					turnNo,
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				logOpenAIWSV2Passthrough(
					"relay_rate_limit_failover account_id=%v err_code=%s err_type=%s err_message=%s",
					account.ID,
			...
			OnTrace: func(event openaiwsv2.RelayTraceEvent) {
				logOpenAIWSV2Passthrough(
					"relay_trace account_id=%v stage=%s direction=%s msg_type=%s bytes=%v graceful=%v wrote_downstream=%v err=%s",
					account.ID,
	...
	if relayExit == nil {
		logOpenAIWSV2Passthrough(
			"relay_completed account_id=%v request_id=%s terminal_event=%s duration_ms=%v c2u_frames=%v u2c_frames=%v dropped_frames=%v turns=%v",
			account.ID,
	...
	logOpenAIWSV2Passthrough(
		"relay_failed account_id=%v stage=%s wrote_downstream=%v err=%s duration_ms=%v c2u_frames=%v u2c_frames=%v dropped_frames=%v turns=%v",
		account.ID,
...
func openaiwsv2RelayMessageTypeName(msgType coderws.MessageType) string {
	...
	case coderws.MessageBinary:
		return "binary"
	default:
		return fmt.Sprintf("unknown(%v)", msgType)
	}
}
