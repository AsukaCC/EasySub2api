...
func duplicateGroupOperationID(sourceID string, actorScope, operationKey string) string {
	...
	payload := "admin.groups.duplicate\x00" + actorScope + "\x00" + sourceID + "\x00" + operationKey
...
func duplicateGroupName(sourceName string, copyNumber int) string {
	...
	suffix := " (Copy)"
	if copyNumber > 1 {
		suffix = fmt.Sprintf(" (Copy %v)", copyNumber)
	}
...
func cloneGroupModelRouting(value map[string][]string) map[string][]string {
	if value == nil {
		return nil
	}
	cloned := make(map[string][]string, len(value))
	for model, accountIDs := range value {
		cloned[model] = append([]string(nil), accountIDs...)
	}
	return cloned
}
...
func cloneGroupLevelRateMultipliers(value map[string]float64) map[string]float64 {
	if value == nil {
		return nil
	}
	cloned := make(map[string]float64, len(value))
	for level, multiplier := range value {
		cloned[level] = multiplier
	}
	return cloned
}

func cloneGroupDynamicRateRules(value []GroupDynamicRateRule) []GroupDynamicRateRule {
...
func cloneGroupForDuplicate(source *Group, operationID string) *Group {
	return &Group{
		...
		LevelRateMultipliers:            cloneGroupLevelRateMultipliers(source.LevelRateMultipliers),
		DynamicRateRules:                cloneGroupDynamicRateRules(source.DynamicRateRules),
		...
		CcsCodexWSEnabled:               source.CcsCodexWSEnabled,
...
func (s *adminServiceImpl) RecoverDuplicateGroup(ctx context.Context, id string, actorScope, operationKey string) (*Group, error) {
...
func (s *adminServiceImpl) DuplicateGroup(ctx context.Context, id string, actorScope, operationKey string) (*Group, error) {
