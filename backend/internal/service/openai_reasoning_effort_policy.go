...
// effort values accepted as a mapping target or as a model capability.
...
// ReasoningEffortOverLimitError is returned when a group policy is configured
// to deny an explicit reasoning effort above its ceiling.
...
// NormalizeMaxReasoningEffortOverLimit canonicalizes the action taken when an
// explicit effort exceeds the group ceiling. Empty retains legacy downgrade
// behavior.
...
func normalizeReasoningEffortMatchType(matchType, model string) (string, error) {
	...
	// Legacy {from,to} rules have no model scope. Treat a match type without a
	// model as the same all-model rule so it cannot become an unusable rule.
	if model == "" {
		return "", nil
	}
	switch matchType {
...
// selectReasoningEffortMapping applies deterministic precedence: exact model,
// longest prefix/suffix, then legacy all-model rule, with original order as the
// final tie breaker.
func selectReasoningEffortMapping(mappings []ReasoningEffortMapping, from, requestModel string) (ReasoningEffortMapping, bool) {
	...
	for i, mapping := range mappings {
		...
		if err != nil || !requestModelMatchesReasoningEffortMapping(requestModel, matchType, model) {
			continue
		}
		strength := 1
		patternLen := 0
		...
		candidates = append(candidates, candidate{mapping: mapping, matchStrength: strength, patternLen: patternLen, index: i})
	}
	if len(candidates) == 0 {
...
func NormalizeReasoningEffortMappings(platform string, raw []ReasoningEffortMapping) ([]ReasoningEffortMapping, error) {
	if len(raw) > maxReasoningEffortMappings {
		return nil, fmt.Errorf("reasoning effort mappings cannot exceed %v entries", maxReasoningEffortMappings)
	}
	...
	for i, mapping := range raw {
		...
		if from == "" || to == "" {
			return nil, fmt.Errorf("reasoning effort mapping %v contains an empty or unknown value", i+1)
		}
		if len(from) > maxReasoningEffortValueLen || len(to) > maxReasoningEffortValueLen {
			return nil, fmt.Errorf("reasoning effort mapping %v values cannot exceed %v characters", i+1, maxReasoningEffortValueLen)
		}
		if from != "none" {
			if _, err := normalizeMaxReasoningEffortForPlatform(platform, from); err != nil {
				return nil, fmt.Errorf("reasoning effort mapping %v source: %w", i+1, err)
			}
		} else if len(reasoningEffortValuesForPlatform(platform)) == 0 {
			return nil, fmt.Errorf(
				"reasoning effort mapping %v source: reasoning effort policy is only supported for platforms %q and %q",
				i+1, PlatformOpenAI, PlatformComposite,
			)
		}
		if _, err := normalizeMaxReasoningEffortForPlatform(platform, to); err != nil {
			return nil, fmt.Errorf("reasoning effort mapping %v target: %w", i+1, err)
		}
		model := strings.TrimSpace(mapping.Model)
...
// WithRequestedReasoningEffort stores the client-requested effort captured
// before group policy or model-family remapping.
...
// RequestedReasoningEffortFromContext returns the captured inbound effort.
...
// WithOpenAIReasoningEffortPolicyForModel additionally freezes the canonical
// client model. Bridges may rewrite body.model before applying the policy; the
// frozen value keeps model-scoped rules aligned with the inbound request.
func WithOpenAIReasoningEffortPolicyForModel(ctx context.Context, maxEffort string, mappings []ReasoningEffortMapping, overLimit, model string) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
...
func ApplyOpenAIReasoningEffortPolicyFromContext(ctx context.Context, body []byte) ([]byte, bool, error) {
	...
	return ApplyOpenAIReasoningEffortPolicyForModel(body, policy.maxEffort, policy.mappings, policy.overLimit, policy.model)
}
...
// ApplyOpenAIReasoningEffortPolicy applies one model-scoped mapping and then
// caps known effort levels or rejects values above the ceiling when configured
// with the deny action. Omitted values remain untouched so upstream defaults
// stay in control.
...
// ApplyOpenAIReasoningEffortPolicyForModel applies the policy using the
// canonical client model when provided. An empty model falls back to body.model
// for backwards compatibility with existing callers.
func ApplyOpenAIReasoningEffortPolicyForModel(body []byte, maxEffort string, mappings []ReasoningEffortMapping, overLimit, requestModel string) ([]byte, bool, error) {
