...
const AccountListGroupUngrouped = "__ungrouped__"
...
type AccountRepository interface {
	...
	ListCRSAccountIDs(ctx context.Context) (map[string]string, error)
	Update(ctx context.Context, account *Account) error
	Delete(ctx context.Context, id string) error

	List(ctx context.Context, params pagination.PaginationParams) ([]Account, *pagination.PaginationResult, error)
	ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) ([]Account, *pagination.PaginationResult, error)
	// ListAllWithFilters 返回符合过滤条件的全部账号（不分页），用于账号列表页
	// 计算 OpenAI 调度分数的过滤范围池。
	ListAllWithFilters(ctx context.Context, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) ([]Account, error)
	ListByGroup(ctx context.Context, groupID string) ([]Account, error)
	ListActive(ctx context.Context) ([]Account, error)
	ListByPlatform(ctx context.Context, platform string) ([]Account, error)

	UpdateLastUsed(ctx context.Context, id string) error
	BatchUpdateLastUsed(ctx context.Context, updates map[string]time.Time) error
	SetError(ctx context.Context, id string, errorMsg string) error
	ClearError(ctx context.Context, id string) error
	SetSchedulable(ctx context.Context, id string, schedulable bool) error
	AutoPauseExpiredAccounts(ctx context.Context, now time.Time) (int64, error)
	BindGroups(ctx context.Context, accountID string, groupIDs []string) error

	ListSchedulable(ctx context.Context) ([]Account, error)
	ListSchedulableByGroupID(ctx context.Context, groupID string) ([]Account, error)
	ListSchedulableByPlatform(ctx context.Context, platform string) ([]Account, error)
	ListSchedulableByGroupIDAndPlatform(ctx context.Context, groupID string, platform string) ([]Account, error)
	ListSchedulableByPlatforms(ctx context.Context, platforms []string) ([]Account, error)
	ListSchedulableByGroupIDAndPlatforms(ctx context.Context, groupID string, platforms []string) ([]Account, error)
	...
	ListModelAvailabilityCandidates(ctx context.Context, groupID *string, platforms []string, includeGrouped bool) ([]Account, error)

	SetRateLimited(ctx context.Context, id string, resetAt time.Time) error
	SetModelRateLimit(ctx context.Context, id string, scope string, resetAt time.Time, reason ...string) error
	SetOverloaded(ctx context.Context, id string, until time.Time) error
	SetTempUnschedulable(ctx context.Context, id string, until time.Time, reason string) error
	ClearTempUnschedulable(ctx context.Context, id string) error
	ClearRateLimit(ctx context.Context, id string) error
	ClearModelRateLimits(ctx context.Context, id string) error
	UpdateSessionWindow(ctx context.Context, id string, start, end *time.Time, status string) error
	// UpdateSessionWindowEnd 仅更新 5h 窗口的结束时间，不动 start / status。
	// 用于 active poll 拿到新 ResetsAt 后回写，避免覆盖请求路径上记录的 status。
	UpdateSessionWindowEnd(ctx context.Context, id string, end time.Time) error
	UpdateExtra(ctx context.Context, id string, updates map[string]any) error
	BulkUpdate(ctx context.Context, ids []string, updates AccountBulkUpdate) (int64, error)
	// IncrementQuotaUsed 原子递增 API Key 账号的配额用量（总/日/周）
	IncrementQuotaUsed(ctx context.Context, id string, amount float64) error
	// ResetQuotaUsed 重置 API Key 账号所有维度的配额用量为 0
	ResetQuotaUsed(ctx context.Context, id string) error
	...
	RevertProxyFallback(ctx context.Context, accountID string) error
	...
	ListShadowsByParent(ctx context.Context, parentID string) ([]*Account, error)
}
...
// GetByID 根据ID获取账号
func (s *AccountService) GetByID(ctx context.Context, id string) (*Account, error) {
...
func (s *AccountService) ListByGroup(ctx context.Context, groupID string) ([]Account, error) {
...
// Update 更新账号
func (s *AccountService) Update(ctx context.Context, id string, req UpdateAccountRequest) (*Account, error) {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("get account: %w", err)
	}
...
func (s *AccountService) validateGroupIDsExist(ctx context.Context, groupIDs []string) error {
	if len(groupIDs) == 0 {
		return nil
	}
	if s.groupRepo == nil {
...
// UpdateStatus 更新账号状态
func (s *AccountService) UpdateStatus(ctx context.Context, id string, status string, errorMessage string) error {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
...
func (s *AccountService) UpdateLastUsed(ctx context.Context, id string) error {
...
func (s *AccountService) GetCredential(ctx context.Context, id string, key string) (string, error) {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
		return "", fmt.Errorf("get account: %w", err)
	}
...
func (s *AccountService) TestCredentials(ctx context.Context, id string) error {
	account, err := s.accountRepo.GetByID(ctx, id)
	if err != nil {
		return fmt.Errorf("get account: %w", err)
	}
	...
	case PlatformGemini, PlatformAntigravity:
		// Connectivity is verified by AccountTestService using the native provider protocol.
		return nil
	case PlatformGrok:
