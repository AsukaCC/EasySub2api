...
func (s *OpsSystemLogSink) run() {
	...
	flush := func(baseCtx context.Context) {
		...
		if err != nil {
			...
			_, _ = fmt.Fprintf(os.Stderr, "time=%s level=WARN msg=\"ops system log sink flush failed\" err=%v batch=%v failures=%v backoff=%s\n",
...
func (s *OpsSystemLogSink) flushBatch(baseCtx context.Context, batch []*logger.LogEvent) (int, error) {
	...
	for _, event := range batch {
		...
		userID := asIDPtr(fields["user_id"])
		apiKeyID := asIDPtr(fields["api_key_id"])
		accountID := asIDPtr(fields["account_id"])
