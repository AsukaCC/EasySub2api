	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			if attempt < geminiMaxRetries {
				logger.LegacyPrintf("service.gemini_chat_completions", "Gemini account %s: upstream request failed, retry %d/%d: %v", account.ID, attempt, geminiMaxRetries, err)
		...
		if resp.StatusCode >= 400 && s.shouldRetryGeminiUpstreamError(account, resp.StatusCode) {
			...
			if attempt < geminiMaxRetries {
				...
				logger.LegacyPrintf("service.gemini_chat_completions", "Gemini account %s: upstream status %d, retry %d/%d", account.ID, resp.StatusCode, attempt, geminiMaxRetries)
				sleepGeminiBackoff(attempt)
				continue
			}
			resp = &http.Response{
