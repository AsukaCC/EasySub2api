...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if wsDecision.Transport == OpenAIUpstreamTransportResponsesWebsocketV2 {
		logOpenAIWSModeDebug(
			"selected account_id=%v account_type=%s transport=%s reason=%s model=%s stream=%v",
			account.ID,
			account.Type,
	...
	if wsDecision.Transport == OpenAIUpstreamTransportResponsesWebsocketV2 {
		...
		logOpenAIWSModeDebug(
			"forward_start account_id=%v account_type=%s model=%s stream=%v has_previous_response_id=%v",
			account.ID,
			account.Type,
			upstreamModel,
			reqStream,
		...
		recoverPrevResponseNotFound := func(attempt int) bool {
			...
			if previousResponseID == "" {
				logOpenAIWSModeInfo(
					"reconnect_prev_response_recovery_skip account_id=%v attempt=%v reason=missing_previous_response_id previous_response_id_present=false",
					account.ID,
					attempt,
				)
				return false
			...
			if HasFunctionCallOutput(wsReqBody) {
				logOpenAIWSModeInfo(
					"reconnect_prev_response_recovery_skip account_id=%v attempt=%v reason=has_function_call_output previous_response_id_present=true",
					account.ID,
					attempt,
				)
				return false
			...
			logOpenAIWSModeInfo(
				"reconnect_prev_response_recovery account_id=%v attempt=%v action=drop_previous_response_id retry=1 previous_response_id=%s previous_response_id_kind=%s",
				account.ID,
				attempt,
		...
		recoverInvalidEncryptedContent := func(attempt int) bool {
			...
			if !removedReasoningItems {
				logOpenAIWSModeInfo(
					"reconnect_invalid_encrypted_content_recovery_skip account_id=%v attempt=%v reason=missing_encrypted_reasoning_items",
					account.ID,
					attempt,
				)
				return false
			...
			logOpenAIWSModeInfo(
				"reconnect_invalid_encrypted_content_recovery account_id=%v attempt=%v action=drop_encrypted_reasoning_items retry=1 previous_response_id_present=%v previous_response_id=%s previous_response_id_kind=%s has_function_call_output=%v dropped_previous_response_id=%v",
				account.ID,
				attempt,
				previousResponseID != "",
				truncateOpenAIWSLogValue(previousResponseID, openAIWSIDValueMaxLen),
	...
	wsRetryLoop:
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			if retryable && attempt < maxAttempts {
				...
				if retryBudget > 0 && time.Since(retryStartedAt)+backoff > retryBudget {
					...
					logOpenAIWSModeInfo(
						"reconnect_budget_exhausted account_id=%v attempts=%v max_retries=%v reason=%s elapsed_ms=%v budget_ms=%v",
						account.ID,
						attempt,
				...
				logOpenAIWSModeInfo(
					"reconnect_retry account_id=%v retry=%v max_retries=%v reason=%s backoff_ms=%v",
					account.ID,
					attempt,
					openAIWSReconnectRetryLimit,
					normalizeOpenAIWSLogValue(reason),
			...
			if retryable {
				s.recordOpenAIWSRetryExhausted()
				logOpenAIWSModeInfo(
					"reconnect_exhausted account_id=%v attempts=%v max_retries=%v reason=%s",
					account.ID,
					attempt,
					openAIWSReconnectRetryLimit,
					normalizeOpenAIWSLogValue(reason),
				...
				logOpenAIWSModeInfo(
					"reconnect_stop account_id=%v attempt=%v reason=%s",
					account.ID,
					attempt,
					normalizeOpenAIWSLogValue(reason),
				)
		...
		if wsErr == nil {
			...
			logOpenAIWSModeDebug(
				"forward_succeeded account_id=%v request_id=%s stream=%v has_first_token_ms=%v first_token_ms=%v ws_attempts=%v",
				account.ID,
				requestID,
				reqStream,
				hasFirstTokenMs,
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		compatMessagesBridge := isBridgeRequest
		...
		if compatMessagesBridge {
			// Compatibility bridges intentionally omit Codex originator negotiation.
	...
	if account.UsesOpenAICodexProtocol() {
		if isBridgeRequest {
			candidateUA := s.codexIdentityOverrideUA(account)
			if !codexIdentityEnforcement.Load() {
				candidateUA = req.Header.Get("user-agent")
			}
			// Bridges still need a valid UA/version without originator negotiation.
