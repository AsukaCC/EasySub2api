...
//   - Fixed points: threshold=400, limit=1000 -> fires when usage reaches 600 (remaining drops to 400)
...
func (s *BalanceNotifyService) asyncSendQuotaAlert(adminEmails []string, accountID string, accountName, platform string, dim quotaDim, newUsed, effectiveThreshold float64, siteName string) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
...
func (s *BalanceNotifyService) sendBalanceLowEmails(recipients []string, userID string, userName, userEmail string, balance, threshold float64, siteName, rechargeURL string) {
	...
	if s.notificationEmailService != nil {
		...
		for _, to := range recipients {
			...
			err := s.notificationEmailService.Send(ctx, NotificationEmailSendInput{
				...
				SourceID:       firstNonEmpty(userID, userEmail),
				ReminderKey:    time.Now().UTC().Format("2006-01-02"),
				Variables: map[string]string{
					"current_balance":  formatPlatformPoints(balance),
					"current_points":   formatPlatformPoints(balance),
					"threshold":        formatPlatformPoints(threshold),
					"threshold_points": formatPlatformPoints(threshold),
	...
	subject := fmt.Sprintf("[%s] 积分不足提醒 / Points Low Alert", sanitizeEmailHeader(siteName))
...
func (s *BalanceNotifyService) sendQuotaAlertEmails(adminEmails []string, accountID string, accountName, platform string, dim quotaDim, used float64, siteName string) {
	...
	if s.notificationEmailService != nil {
		...
		for _, to := range adminEmails {
			...
			err := s.notificationEmailService.Send(ctx, NotificationEmailSendInput{
				...
				SourceID:       fmt.Sprintf("%v-%s", accountID, dim.name),
				ReminderKey:    time.Now().UTC().Format("2006-01-02"),
				Variables: map[string]string{
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p style="font-size: 18px; color: #333;">%s，您的平台积分不足</p>
            <p style="color: #666;">Dear %s, your platform points are running low</p>
            <div class="balance">%s 积分 / points</div>
            <div class="info">
                <p>您的平台积分已低于提醒阈值 <strong>%s 积分</strong>。</p>
                <p>Your platform points have fallen below the alert threshold of <strong>%s points</strong>.</p>
                <p>请及时充值以免服务中断。</p>
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="metric"><span class="metric-label">账号 ID / Account ID</span><span class="metric-value">#%v</span></div>
...
func (s *BalanceNotifyService) buildBalanceLowEmailBody(userName string, balance, threshold float64, siteName, rechargeURL string) string {
	...
	balancePoints := formatPlatformPoints(balance)
	thresholdPoints := formatPlatformPoints(threshold)
	return fmt.Sprintf(balanceLowEmailTemplate, siteName, userName, userName, balancePoints, thresholdPoints, thresholdPoints, rechargeBlock)
}

// buildQuotaAlertEmailBody builds HTML email for account quota alert.
func (s *BalanceNotifyService) buildQuotaAlertEmailBody(accountID string, accountName, platform, dimLabel string, used, limit, remaining float64, thresholdDisplay, siteName string) string {
