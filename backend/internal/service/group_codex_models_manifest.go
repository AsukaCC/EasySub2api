...
//   - Account IDs are trimmed and de-duplicated preserving first-seen order;
//     empty IDs are dropped. The list is kept even when disabled so toggling
//     enabled back on does not lose the previous selection.
func normalizeCodexModelsManifestConfig(platform string, cfg GroupCodexModelsManifestConfig) GroupCodexModelsManifestConfig {
	...
	seen := make(map[string]struct{}, len(cfg.AccountIDs))
	out.AccountIDs = make([]string, 0, len(cfg.AccountIDs))
	for _, raw := range cfg.AccountIDs {
...
func (s *adminServiceImpl) validateCodexModelsManifestConfig(ctx context.Context, groupID string, cfg GroupCodexModelsManifestConfig) error {
	if !cfg.Enabled {
		return nil
	}
	if len(cfg.AccountIDs) == 0 {
	...
	members := make(map[string]struct{}, len(accounts))
