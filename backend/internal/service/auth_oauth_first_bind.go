...
func (s *AuthService) ApplyProviderDefaultSettingsOnFirstBind(
	...
	if s == nil || s.entClient == nil || s.settingService == nil || userID == "" {
		return nil
	}
...
ON CONFLICT (user_id, provider_type, grant_reason) DO NOTHING`,
	...
	if providerDefaults.Balance != 0 {
		validityDays := providerDefaults.BonusValidityDays
		if validityDays <= 0 {
			validityDays = s.settingService.GetBonusBalanceDefaultValidityDays(ctx)
		}
		expiresAt := time.Now().UTC().Add(time.Duration(validityDays) * 24 * time.Hour)
		if _, err := s.userRepo.CreditWallet(ctx, WalletCreditInput{
			UserID: userID, Amount: providerDefaults.Balance, Kind: WalletKindBonus, ExpiresAt: &expiresAt,
			SourceType: "auth_source_first_bind", SourceID: strings.TrimSpace(providerType),
			IdempotencyKey: "wallet-auth-first-bind:" + userID + ":" + strings.TrimSpace(providerType),
		}); err != nil {
