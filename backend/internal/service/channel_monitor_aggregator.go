...
func (s *ChannelMonitorService) BatchMonitorStatusSummary(
	...
	extrasByID map[string][]string,
) map[string]MonitorStatusSummary {
	out := make(map[string]MonitorStatusSummary, len(ids))
	if len(ids) == 0 {
		return out
	}
	...
	if err != nil {
		...
		latestMap = map[string][]*ChannelMonitorLatest{}
	}
	...
	if err != nil {
		...
		availMap = map[string][]*ChannelMonitorAvailability{}
	}
...
func collectMonitorIndexes(monitors []*ChannelMonitor) ([]string, map[string]string, map[string][]string) {
	ids := make([]string, 0, len(monitors))
	primaryByID := make(map[string]string, len(monitors))
	extrasByID := make(map[string][]string, len(monitors))
	for _, m := range monitors {
		ids = append(ids, m.ID)
...
func (s *ChannelMonitorService) batchLatest(ctx context.Context, ids []string) map[string][]*ChannelMonitorLatest {
	latestMap, err := s.repo.ListLatestForMonitorIDs(ctx, ids)
	if err != nil {
...
func (s *ChannelMonitorService) batchTimeline(
	...
	if err != nil {
		...
		return map[string][]*ChannelMonitorHistoryEntry{}
	}
	return timelineMap
}
...
// 不暴露 api_key。
func (s *ChannelMonitorService) GetUserDetail(ctx context.Context, id string) (*UserMonitorDetail, error) {
...
func (s *ChannelMonitorService) collectAvailabilityWindows(ctx context.Context, monitorID string) (map[int]map[string]*ChannelMonitorAvailability, error) {
	...
	for _, w := range windows {
		...
		if err != nil {
			return nil, fmt.Errorf("compute availability %vd: %w", w, err)
		}
