...
func openAIImagesResponsesMainModelValue() string {
	if model := strings.TrimSpace(os.Getenv("OPENAI_IMAGES_MAIN_MODEL")); model != "" {
		return model
	}
...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, parsed *OpenAIImagesRequest) (OpenAIUsage, int, []string, error) {
...
func mergeOpenAIUsage(dst *OpenAIUsage, body []byte) {
	...
	if parsed, ok := extractOpenAIUsageFromJSONBytes(body); ok {
		mergeOpenAIUsageNonZero(dst, parsed)
	}
}

func mergeOpenAIUsageNonZero(dst *OpenAIUsage, parsed OpenAIUsage) {
	if dst == nil {
		return
	}
...
func (e *openAIImageStatusError) Error() string {
	...
	if e.StatusCode > 0 {
		return fmt.Sprintf("openai image backend request failed: status %v", e.StatusCode)
	}
	return "openai image backend request failed"
}
...
func newOpenAIImageStatusError(resp *req.Response, fallback string, errorBodyReadLimit int64) error {
	...
	if message == "" {
		...
		message = fmt.Sprintf("%s: status %v", prefix, statusCode)
	}
