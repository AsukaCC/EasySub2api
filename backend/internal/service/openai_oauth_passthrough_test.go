...
func TestOpenAIGatewayService_CodexFingerprintCompactDoesNotRewriteBodyCacheKeyOrMetadata(t *testing.T) {
	...
	require.Equal(t, scopeCodexAccountIdentityValue(account, "", "session", "body-session"), gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
	require.Equal(t, scopeCodexAccountIdentityValue(account, "", "session", "body-session"), gjson.GetBytes(upstream.lastBody, "client_metadata.session_id").String())
