...
func ResolveProtectionRuntime(a *Account, cfg *config.Config) *ProtectionRuntimeState {
	if a == nil {
		return nil
	}
...
func (s *AntiDegradeService) SetRuntimeConfig(cfg *config.Config) { s.cfg = cfg }
func (s *AccountTestService) ProtectionConfig() *config.Config    { return s.cfg }
