...
type CompositeTokenCacheInvalidator struct {
	cache OAuthTokenCache // 统一使用一个缓存接口，通过缓存键前缀区分平台
}

func NewCompositeTokenCacheInvalidator(cache OAuthTokenCache) *CompositeTokenCacheInvalidator {
...
func (c *CompositeTokenCacheInvalidator) InvalidateToken(ctx context.Context, account *Account) error {
	...
	accountIDKey := "account:" + account.ID
