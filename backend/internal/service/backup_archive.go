...
func splitBackupFile(srcPath string, partSize int64) (parts []localBackupPart, err error) {
	...
	for index := 1; remaining > 0; index++ {
		partFile, createErr := os.CreateTemp("", "easysub2api-backup-part-*")
		if createErr != nil {
		...
		if copyErr != nil {
			...
			return nil, fmt.Errorf("write backup part %v: %w", index, copyErr)
		}
		if closeErr != nil {
			_ = os.Remove(partPath)
			return nil, fmt.Errorf("close backup part %v: %w", index, closeErr)
		}
