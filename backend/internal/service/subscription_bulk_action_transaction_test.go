...
func (r *transactionalBulkSubscriptionRepo) GetByID(ctx context.Context, _ string) (*UserSubscription, error) {
...
func (r *transactionalBulkSubscriptionRepo) ExtendExpiry(ctx context.Context, _ string, expiry time.Time) error {
	r.pending[dbent.TxFromContext(ctx)].ExpiresAt = expiry
	return nil
}

func (r *transactionalBulkSubscriptionRepo) UpdateStatus(ctx context.Context, _ string, status string) error {
	if r.statusFailure {
...
func (r *transactionalBulkSubscriptionRepo) ResetUsageWindows(ctx context.Context, _ string, daily, weekly, monthly bool, dailyStart, periodicStart time.Time) error {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			repo := &transactionalBulkSubscriptionRepo{
				committed:       UserSubscription{ID: "sub-1", UserID: "user-1", GroupID: "group-1", Status: status, ExpiresAt: expiresAt, DailyUsageUSD: 7},
			...
			t.Cleanup(svc.Stop)
			input := &BulkSubscriptionActionInput{SubscriptionIDs: []string{"sub-1"}, Action: tc.action, Days: 7, Daily: true}
			...
			require.Equal(t, expiresAt, repo.committed.ExpiresAt)
			...
			if tc.action == "extend" {
				require.Equal(t, expiresAt.AddDate(0, 0, 7), repo.committed.ExpiresAt)
