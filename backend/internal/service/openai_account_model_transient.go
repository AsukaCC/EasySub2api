...
func openAIAccountModelTransientKey(accountID string, model string) (openAIAccountModelKey, bool) {
...
func (s *openAIAccountModelTransientState) recordFailure(accountID string, model string, now time.Time) openAIAccountModelTransientDecision {
...
func (s *openAIAccountModelTransientState) recordSuccess(accountID string, model string) {
	key, ok := openAIAccountModelTransientKey(accountID, model)
	if s == nil || !ok {
		return
	}
...
func (s *openAIAccountModelTransientState) isBlocked(accountID string, model string, now time.Time) bool {
	key, ok := openAIAccountModelTransientKey(accountID, model)
	if s == nil || !ok {
		return false
	}
