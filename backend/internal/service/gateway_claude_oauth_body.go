...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/anthropicfp"
		...
		sys.ForEach(func(_, item gjson.Result) bool {
			if item.Get("type").String() == "text" {
				...
				if textResult.Exists() && textResult.Type == gjson.String {
					...
					if sanitized != text {
						if next, ok := setJSONValueBytes(out, fmt.Sprintf("system.%v.text", index), sanitized); ok {
							out = next
							modified = true
						}
					}
			...
			if opts.stripSystemCacheControl && item.Get("cache_control").Exists() {
				if next, ok := deleteJSONPathBytes(out, fmt.Sprintf("system.%v.cache_control", index)); ok {
					out = next
					modified = true
				}
			}
...
func buildStableSessionSeed(accountID string, clientDiscriminator, firstUserText string) string {
	var b strings.Builder
	_, _ = b.WriteString(accountID)
	_, _ = b.WriteString("::")
...
func sessionContextDiscriminator(sc *SessionContext) string {
	...
	return sc.ClientIP + ":" + NormalizeSessionUserAgent(sc.UserAgent) + ":" + sc.APIKeyID
}
...
func buildClaudeOAuthSystemPromptBlocksJSON(body []byte, expansionPrompt string, blocksConfig string) ([][]byte, error) {
	...
	for i, block := range blocks {
		...
		if blockType != "text" {
			return nil, fmt.Errorf("system block %v type %q is not supported", i, block.Type)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("system block %v cache_control: %w", i, err)
		}
...
func ValidateClaudeOAuthSystemPromptBlocksConfig(raw string) error {
	...
	for i, block := range blocks {
		...
		if blockType != "text" {
			return infraerrors.BadRequest("INVALID_CLAUDE_OAUTH_SYSTEM_PROMPT_BLOCKS", fmt.Sprintf("system block %v type must be text", i))
		}
		if _, err := decodeClaudeOAuthSystemPromptCacheControl(block.CacheControl); err != nil {
			return infraerrors.BadRequest("INVALID_CLAUDE_OAUTH_SYSTEM_PROMPT_BLOCKS", fmt.Sprintf("system block %v cache_control is invalid", i))
		}
	}
	return nil
}
	...
	if system.IsArray() {
		...
		system.ForEach(func(_, item gjson.Result) bool {
			if item.Get("cache_control").Exists() {
				path := fmt.Sprintf("system.%v.cache_control", sysIndex)
		...
		messages.ForEach(func(_, msg gjson.Result) bool {
			...
			if content.IsArray() {
				...
				content.ForEach(func(_, item gjson.Result) bool {
					if item.Get("cache_control").Exists() {
						path := fmt.Sprintf("messages.%v.content.%v.cache_control", msgIndex, contentIndex)
						if item.Get("type").String() == "thinking" {
							invalidThinking = append(invalidThinking, cacheControlPath{
								path: path,
								log:  fmt.Sprintf("[Warning] Removed illegal cache_control from thinking block in messages[%v].content[%v]", msgIndex, contentIndex),
							})
						} else {
	...
	if tools.IsArray() {
		...
		tools.ForEach(func(_, tool gjson.Result) bool {
			if tool.Get("cache_control").Exists() {
				toolPaths = append(toolPaths, fmt.Sprintf("tools.%v.cache_control", toolIndex))
			}
			toolIndex++
			return true
		})
...
func forceEphemeralCacheControlTTL(body []byte, ttl string) []byte {
	...
	if system.IsArray() {
		...
		system.ForEach(func(_, block gjson.Result) bool {
			idx++
			addPath(fmt.Sprintf("system.%v", idx), block)
			return true
		})
	}
	...
	if messages.IsArray() {
		...
		messages.ForEach(func(_, msg gjson.Result) bool {
			...
			content.ForEach(func(_, block gjson.Result) bool {
				contentIdx++
				addPath(fmt.Sprintf("messages.%v.content.%v", msgIdx, contentIdx), block)
				return true
			})
			return true
		})
	...
	if tools.IsArray() {
		...
		tools.ForEach(func(_, tool gjson.Result) bool {
			idx++
			addPath(fmt.Sprintf("tools.%v", idx), tool)
			return true
		})
	}
