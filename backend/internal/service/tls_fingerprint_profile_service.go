...
type TLSFingerprintProfileRepository interface {
	...
	GetByID(ctx context.Context, id string) (*model.TLSFingerprintProfile, error)
...
type TLSFingerprintProfileService struct {
	...
	localCache map[string]*model.TLSFingerprintProfile
...
func NewTLSFingerprintProfileService(
	...
	svc := &TLSFingerprintProfileService{
		repo:       repo,
		cache:      cache,
		localCache: make(map[string]*model.TLSFingerprintProfile),
	}
...
// GetByID 根据 ID 获取模板
func (s *TLSFingerprintProfileService) GetByID(ctx context.Context, id string) (*model.TLSFingerprintProfile, error) {
...
// Delete 删除模板
func (s *TLSFingerprintProfileService) Delete(ctx context.Context, id string) error {
...
func (s *TLSFingerprintProfileService) GetProfileByID(id string) *tlsfingerprint.Profile {
	s.localMu.RLock()
...
func (s *TLSFingerprintProfileService) ResolveTLSProfile(account *Account) *tlsfingerprint.Profile {
	if profile, err := resolveMode1TLSProfile(account); err == nil && profile != nil {
		return profile
	}
...
func (s *TLSFingerprintProfileService) setLocalCache(profiles []*model.TLSFingerprintProfile) {
	m := make(map[string]*model.TLSFingerprintProfile, len(profiles))
...
func (s *TLSFingerprintProfileService) invalidateAndNotify(ctx context.Context) {
	...
	if err := s.reloadFromDB(ctx); err != nil {
		...
		s.localMu.Lock()
		s.localCache = make(map[string]*model.TLSFingerprintProfile)
		s.localMu.Unlock()
	}
