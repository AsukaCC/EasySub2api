...
type schedulerAccountQueryCache struct {
	...
	snapshotAccountIDs map[schedulerAccountQueryKey][]string
}
...
type schedulerSnapshotAccountIDWriter interface {
	SetSnapshotAndReturnAccountIDs(ctx context.Context, bucket SchedulerBucket, token SchedulerBucketWriteToken, accounts []Account) ([]string, error)
	SetSnapshotByAccountIDs(ctx context.Context, bucket SchedulerBucket, token SchedulerBucketWriteToken, accountIDs []string) error
}
...
func newSchedulerAccountQueryCache(taskSets ...[]schedulerBucketWriteTask) *schedulerAccountQueryCache {
	queries := &schedulerAccountQueryCache{
		...
		snapshotAccountIDs: make(map[schedulerAccountQueryKey][]string),
	}
...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *string, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	useMixed := false
...
func (s *SchedulerSnapshotService) GetAccount(ctx context.Context, accountID string) (*Account, error) {
	if accountID == "" {
		return nil, nil
	}
	if err := ctx.Err(); err != nil {
	...
	if s.cache != nil {
		...
		if err != nil {
			logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] account cache read failed: id=%v err=%v", accountID, err)
		} else if account != nil {
			return account, nil
		}
	}
...
func (s *SchedulerSnapshotService) GetGroupByID(ctx context.Context, groupID string) (*Group, error) {
	if s.groupRepo == nil {
		return nil, nil
	}
...
func (s *SchedulerSnapshotService) GetGroupByIDLite(ctx context.Context, groupID string) (*Group, error) {
	if s.groupRepo == nil {
		return nil, nil
	}
...
func (s *SchedulerSnapshotService) pollOutbox() {
	...
	for _, event := range events {
		...
		cancel()
		if err != nil {
			logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox handle failed: id=%v type=%s err=%v", event.ID, event.EventType, err)
			return
		}
	}
...
func (s *SchedulerSnapshotService) cleanupConsumedOutbox(watermark string) {
	if s == nil || s.outboxRepo == nil || watermark == "" {
		return
	}
	...
	for {
		...
		if err != nil {
			logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox cleanup failed: watermark=%v err=%v", watermark, err)
			return
		}
...
func (s *SchedulerSnapshotService) handleLastUsedEvent(ctx context.Context, payload map[string]any) error {
	...
	updates := make(map[string]time.Time, len(raw))
...
func (s *SchedulerSnapshotService) handleBulkAccountEvent(ctx context.Context, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	rawIDs := parseEntityIDSlice(payload["account_ids"])
	if len(rawIDs) == 0 {
		return nil
	}

	ids := make([]string, 0, len(rawIDs))
	seenIDs := make(map[string]struct{}, len(rawIDs))
	...
	preloadGroupIDs := parseEntityIDSlice(payload["group_ids"])
	...
	rebuildGroupSet := make(map[string]struct{}, len(preloadGroupIDs))
	...
	rebuildGroupIDs := make([]string, 0, len(rebuildGroupSet))
	...
	platformGroupSets := make(map[string]map[string]struct{}, len(accounts))
	addPlatformGroups := func(platform string, groupIDs []string) {
	...
	for _, platform := range schedulerSnapshotPlatforms() {
		...
		platformGroupIDs := make([]string, 0, len(groupSet))
...
func (s *SchedulerSnapshotService) handleAccountEvent(ctx context.Context, accountID *string, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	var groupIDs []string
	if payload != nil {
		groupIDs = parseEntityIDSlice(payload["group_ids"])
	}
...
func (s *SchedulerSnapshotService) handleGroupEvent(ctx context.Context, groupID *string, seen map[batchSeenKey]struct{}) error {
	if groupID == nil || *groupID == "" || s.isRunModeSimple() {
		return nil
	}
	if seen != nil {
...
func (s *SchedulerSnapshotService) reconcileGroupLifecycle(ctx context.Context, groupID string, seen map[batchSeenKey]struct{}) error {
...
func (s *SchedulerSnapshotService) prepareGroupLifecycle(ctx context.Context, groupID string, knownHistorical []SchedulerBucket) (plan schedulerGroupLifecyclePlan, retErr error) {
	if groupID == "" || s.isRunModeSimple() {
	...
	if !acquired {
		return schedulerGroupLifecyclePlan{}, fmt.Errorf("%w: group=%v", ErrSchedulerGroupLifecycleLeaseBusy, groupID)
	}
	leaseHeld := true
	defer func() {
		if leaseHeld {
	...
	if err == nil && (group == nil || group.ID != groupID || !group.Hydrated) {
		return schedulerGroupLifecyclePlan{}, fmt.Errorf("untrusted scheduler group lifecycle state: group=%v", groupID)
	}
...
func markGroupLifecycleSeen(seen map[batchSeenKey]struct{}, groupID string) {
	if seen == nil {
		return
	}
...
func (s *SchedulerSnapshotService) rebuildByAccount(ctx context.Context, account *Account, groupIDs []string, reason string, seen map[batchSeenKey]struct{}) error {
	if account == nil {
		return nil
	}
...
func schedulerBucketsForGroup(groupID string) []SchedulerBucket {
	if groupID == "" {
		return nil
	}
...
func schedulerCanonicalBuckets(groupID string) []SchedulerBucket {
...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []string, reason string, seen map[batchSeenKey]struct{}) error {
	groupIDs = s.normalizeGroupIDs(groupIDs)
	if len(groupIDs) == 0 {
		return nil
	}
...
func (s *SchedulerSnapshotService) bucketsForPlatform(platform string, groupIDs []string, seen map[batchSeenKey]struct{}) []SchedulerBucket {
	if platform == "" {
		return nil
	}
...
func (s *SchedulerSnapshotService) rebuildFullSnapshot(ctx context.Context, reason string) error {
	...
	activeGroups := make(map[string]struct{}, len(activeGroupIDs))
	for _, groupID := range activeGroupIDs {
		activeGroups[groupID] = struct{}{}
	}

	registeredByGroup := make(map[string][]SchedulerBucket)
	for _, bucket := range registered {
		registeredByGroup[bucket.GroupID] = append(registeredByGroup[bucket.GroupID], bucket)
	}

	groupZeroCanonical := schedulerCanonicalBuckets("")
	capturedTasks, err := s.captureFullRebuildCanonicalTasks(ctx, groupZeroCanonical)
	if err != nil {
		return err
	}
	ordinaryBuckets := appendBucketsExcept(nil, registeredByGroup[""], groupZeroCanonical)
...
func (s *SchedulerSnapshotService) listActiveSchedulerGroupIDs(ctx context.Context) ([]string, error) {
	if s.groupRepo == nil {
		return nil, ErrSchedulerCacheNotReady
	}
...
func (s *SchedulerSnapshotService) checkOutboxLag(ctx context.Context, watermark string) {
	...
	if backlogThreshold > 0 {
		...
		} else if maxID != "" && maxID != watermark {
			if counter, ok := s.outboxRepo.(interface {
				CountAfter(context.Context, string, int) (int64, error)
			}); ok {
				backlog, maxErr = counter.CountAfter(ctx, watermark, backlogThreshold)
				if maxErr != nil {
					backlogKnown = false
				}
			} else {
	...
	if logLagWarning {
		logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox lag warning: %vs", lagSeconds)
	}
	...
	switch reason {
	case "outbox_lag":
		logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox lag rebuild triggered: lag=%s failures=%v", lag, failures)
		rebuildErr = s.triggerFullRebuild(reason)
	case "outbox_backlog":
		logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox backlog rebuild triggered: backlog=%v", backlog)
		rebuildErr = s.triggerFullRebuild(reason)
	}
...
func (s *SchedulerSnapshotService) bucketFor(groupID *string, platform string, mode string) SchedulerBucket {
...
func (s *SchedulerSnapshotService) normalizeGroupID(groupID *string) string {
	if s.isRunModeSimple() {
		return ""
	}
...
func (s *SchedulerSnapshotService) normalizeGroupIDs(groupIDs []string) []string {
	if s.isRunModeSimple() {
		return []string{""}
	}
	if len(groupIDs) == 0 {
...
func parseEntityIDSlice(value any) []string {
	var raw []string
