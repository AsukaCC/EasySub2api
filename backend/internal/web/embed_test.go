...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>EasySub2api - AI API Gateway</title></head><body></body></html>`)
		...
		assert.NotContains(t, string(result), "EasySub2api")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">EasySub2api</title></head><body></body></html>`)
	...
	t.Run("escapes_ampersand_in_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>EasySub2api</title></head><body></body></html>`)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>EasySub2api</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	...
	t.Run("matching_etag_still_returns_fresh_nonce", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		// A stale client validator must not produce a 304 because the response
		// body needs the nonce generated for this request.
		w2 := httptest.NewRecorder()
