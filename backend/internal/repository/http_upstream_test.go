...
func mustGetOrCreateClient(t *testing.T, svc *httpUpstreamService, proxyURL string, accountID string, concurrency int) *upstreamClientEntry {
	t.Helper()
	entry, err := svc.getOrCreateClient(proxyURL, fmt.Sprintf("account-%s", accountID), concurrency)
	require.NoError(t, err, "getOrCreateClient(%q, %s, %d)", proxyURL, accountID, concurrency)
	return entry
}
