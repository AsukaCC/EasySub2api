...
SELECT ua.user_id,
       ...
       GREATEST(
           ua.aff_quota + COALESCE(matured.matured_frozen_quota, 0) - COALESCE(reserved.reserved_reversal_quota, 0),
           0
       )::double precision,
...
LEFT JOIN (
    SELECT user_id,
           COALESCE(SUM(GREATEST(amount - reversed_amount, 0)), 0)::double precision AS matured_frozen_quota
    FROM user_affiliate_ledger
...
LEFT JOIN (
    SELECT user_id,
           COALESCE(SUM(reserved_reversal_amount), 0)::double precision AS reserved_reversal_quota
    FROM user_affiliate_ledger
    WHERE action = 'accrue' AND reserved_reversal_amount > 0
    GROUP BY user_id
) reserved ON reserved.user_id = ua.user_id
WHERE ua.user_id = $1
LIMIT 1`

const affiliateTransferClaimSQL = `
WITH locked_rebates AS MATERIALIZED (
	SELECT reserved_reversal_amount
	FROM user_affiliate_ledger
	WHERE user_id = $1 AND action = 'accrue'
	FOR UPDATE
),
reserved AS (
	SELECT COALESCE(SUM(reserved_reversal_amount), 0)::double precision AS amount
	FROM locked_rebates
),
claimed AS (
	SELECT GREATEST(ua.aff_quota - r.amount, 0)::double precision AS amount
	FROM user_affiliates ua
	CROSS JOIN reserved r
	WHERE ua.user_id = $1
	  AND ua.aff_quota > r.amount
	FOR UPDATE OF ua
),
cleared AS (
	UPDATE user_affiliates ua
	SET aff_quota = ua.aff_quota - c.amount,
	    updated_at = NOW()
	FROM claimed c
	WHERE ua.user_id = $1
	RETURNING c.amount
...
func (r *affiliateRepository) EnsureUserAffiliate(ctx context.Context, userID string) (*service.AffiliateSummary, error) {
	if userID == "" {
...
func (r *affiliateRepository) BindInviter(ctx context.Context, userID, inviterID string) (bool, error) {
	var bound bool
...
func (r *affiliateRepository) AccrueQuota(ctx context.Context, inviterID, inviteeUserID string, amount float64, freezeHours int, sourceOrderID *string) (bool, error) {
	if amount <= 0 {
		return false, nil
	}
...
VALUES ($1, 'accrue', $2, $3, $4, NOW() + make_interval(hours => $5), NOW(), NOW())`,
				inviterID, amount, inviteeUserID, nullableStringArg(sourceOrderID), freezeHours); err != nil {
				return fmt.Errorf("insert affiliate accrue ledger: %w", err)
			}
		} else {
			if _, err = txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, source_user_id, source_order_id, created_at, updated_at)
VALUES ($1, 'accrue', $2, $3, $4, NOW(), NOW())`, inviterID, amount, inviteeUserID, nullableStringArg(sourceOrderID)); err != nil {
				return fmt.Errorf("insert affiliate accrue ledger: %w", err)
			}
		}
...
func (r *affiliateRepository) GetAccruedRebateFromInvitee(ctx context.Context, inviterID, inviteeUserID string) (float64, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx,
		`SELECT COALESCE(SUM(GREATEST(amount - reversed_amount, 0)), 0)::double precision FROM user_affiliate_ledger WHERE user_id = $1 AND source_user_id = $2 AND action = 'accrue'`,
...
func (r *affiliateRepository) ThawFrozenQuota(ctx context.Context, userID string) (float64, error) {
	var thawed float64
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		var err error
...
func thawFrozenQuotaTx(txCtx context.Context, txClient *dbent.Client, userID string) (float64, error) {
...
WITH matured AS (
    ...
    RETURNING GREATEST(amount - reversed_amount, 0) AS thaw_amount
)
SELECT COALESCE(SUM(thaw_amount), 0) FROM matured`, userID)
	if err != nil {
...
func (r *affiliateRepository) TransferQuotaToBalance(ctx context.Context, userID string) (float64, float64, error) {
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		rows, err := txClient.QueryContext(txCtx, affiliateTransferClaimSQL, userID)
		if err != nil {
		...
		walletBefore, err := lockWalletUser(txCtx, txClient, userID)
		if err != nil {
			return err
		}
...
WHERE ual.action = 'accrue'
  AND ual.source_order_id IS NOT NULL`
	...
	orderBy := buildAffiliateRecordOrderBy(filter, map[string]string{
		...
		"reversed_points":   "ual.reversed_amount",
		"net_rebate_points": "GREATEST(ual.amount - ual.reversed_amount, 0)",
...
SELECT po.id,
	   ...
	   ual.reserved_reversal_amount::double precision,
	   ual.reversed_amount::double precision,
	   GREATEST(ual.amount - ual.reversed_amount, 0)::double precision,
       po.payment_type,
       po.status,
       ual.created_at
`+baseJoin+where+`
...
LIMIT $`+fmt.Sprint(len(args)-1)+` OFFSET $`+fmt.Sprint(len(args)), args...)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.PayAmount,
			&item.RebateAmount,
			&item.ReservedReversalPoints,
			&item.ReversedPoints,
			&item.NetRebatePoints,
			&item.PaymentType,
			&item.OrderStatus,
			&item.CreatedAt,
		); err != nil {
...
func (r *affiliateRepository) GetAffiliateUserOverview(ctx context.Context, userID string) (*service.AffiliateUserOverview, error) {
	if userID == "" {
		return nil, service.ErrUserNotFound
	}
	client := clientFromContext(ctx, r.client)
...
func ensureUserAffiliateWithClient(ctx context.Context, client affiliateQueryExecer, userID string) (*service.AffiliateSummary, error) {
...
func queryAffiliateByUserID(ctx context.Context, client affiliateQueryExecer, userID string) (*service.AffiliateSummary, error) {
	rows, err := client.QueryContext(ctx, `
SELECT user_id,
       aff_code,
       aff_code_custom,
       ...
       GREATEST(
           aff_quota - COALESCE((
               SELECT SUM(ual.reserved_reversal_amount)
               FROM user_affiliate_ledger ual
               WHERE ual.user_id = user_affiliates.user_id AND ual.action = 'accrue'
           ), 0),
           0
       )::double precision,
...
WHERE user_id = $1`, userID)
	...
	if inviterID.Valid {
		out.InviterID = &inviterID.String
	}
	if rebateRate.Valid {
...
func (r *affiliateRepository) UpdateUserAffCode(ctx context.Context, userID string, newCode string) error {
	if userID == "" {
...
func (r *affiliateRepository) ResetUserAffCode(ctx context.Context, userID string) (string, error) {
	if userID == "" {
		return "", service.ErrUserNotFound
	}
	var newCode string
...
func (r *affiliateRepository) SetUserRebateRate(ctx context.Context, userID string, ratePercent *float64) error {
	if userID == "" {
		return service.ErrUserNotFound
	}
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
...
func (r *affiliateRepository) BatchSetUserRebateRate(ctx context.Context, userIDs []string, ratePercent *float64) error {
	if len(userIDs) == 0 {
		return nil
	}
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
