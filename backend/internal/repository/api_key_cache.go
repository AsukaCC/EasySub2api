...
func (c *apiKeyCache) GetCreateAttemptCount(ctx context.Context, userID string) (int, error) {
...
func (c *apiKeyCache) IncrementCreateAttemptCount(ctx context.Context, userID string) error {
	key := apiKeyRateLimitKey(userID)
	pipe := c.rdb.Pipeline()
	pipe.Incr(ctx, key)
