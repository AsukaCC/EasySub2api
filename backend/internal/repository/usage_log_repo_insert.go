...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	groupID := nullID(log.GroupID)
	subscriptionID := nullID(log.SubscriptionID)
	...
	channelID := nullID(log.ChannelID)
...
func usageLogBatchKey(requestID string, apiKeyID string) string {
	return requestID + "\x1f" + apiKeyID
}
...
func (r *usageLogRepository) bestEffortRecentKey(requestID string, apiKeyID string) (string, bool) {
