...
func (r *opsRepository) DeleteAlertRule(ctx context.Context, id string) error {
	if r == nil || r.db == nil {
...
func (r *opsRepository) GetAlertEventByID(ctx context.Context, eventID string) (*service.OpsAlertEvent, error) {
	if r == nil || r.db == nil {
		return nil, fmt.Errorf("nil ops repository")
	}
...
func (r *opsRepository) GetActiveAlertEvent(ctx context.Context, ruleID string) (*service.OpsAlertEvent, error) {
	if r == nil || r.db == nil {
		return nil, fmt.Errorf("nil ops repository")
	}
...
func (r *opsRepository) GetLatestAlertEvent(ctx context.Context, ruleID string) (*service.OpsAlertEvent, error) {
	if r == nil || r.db == nil {
		return nil, fmt.Errorf("nil ops repository")
	}
	if strings.TrimSpace(ruleID) == "" {
...
func (r *opsRepository) UpdateAlertEventStatus(ctx context.Context, eventID string, status string, resolvedAt *time.Time) error {
	if r == nil || r.db == nil {
		return fmt.Errorf("nil ops repository")
	}
	if strings.TrimSpace(eventID) == "" {
...
func (r *opsRepository) UpdateAlertEventEmailSent(ctx context.Context, eventID string, emailSent bool) error {
	if r == nil || r.db == nil {
		return fmt.Errorf("nil ops repository")
	}
	if strings.TrimSpace(eventID) == "" {
...
RETURNING id, rule_id, platform, group_id, region, until, COALESCE(reason,''), created_by, created_at`
	...
	row := r.db.QueryRowContext(
		...
		input.RuleID,
		platform,
		opsNullString(input.GroupID),
		opsNullString(input.Region),
		input.Until,
		opsNullString(input.Reason),
		opsNullString(input.CreatedBy),
	)
...
func (r *opsRepository) IsAlertSilenced(ctx context.Context, ruleID string, platform string, groupID *string, region *string, now time.Time) (bool, error) {
	if r == nil || r.db == nil {
...
LIMIT 1`
	...
	var dummy int
	err := r.db.QueryRowContext(ctx, q, ruleID, platform, opsNullString(groupID), opsNullString(region), now).Scan(&dummy)
	if err != nil {
		if err == sql.ErrNoRows {
			return false, nil
		}
...
func buildOpsAlertEventsWhere(filter *service.OpsAlertEventFilter) (string, []any) {
	...
	if filter.BeforeFiredAt != nil && !filter.BeforeFiredAt.IsZero() && filter.BeforeID != nil && strings.TrimSpace(*filter.BeforeID) != "" {
	...
	if filter.GroupID != nil && strings.TrimSpace(*filter.GroupID) != "" {
