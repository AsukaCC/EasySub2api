...
func (r *serverTimingRows) Next(dest []driver.Value) error {
	...
	servertiming.RecordInterval(r.ctx, servertiming.MetricDatabase, startedAt, time.Now())
	if err == nil {
		normalizeUUIDDriverValues(r.Rows, dest)
	}
	return err
}

func normalizeUUIDDriverValues(rows driver.Rows, dest []driver.Value) {
	typeNames, ok := rows.(driver.RowsColumnTypeDatabaseTypeName)
	if !ok {
		return
	}
	for i, value := range dest {
		raw, ok := value.([]byte)
		if !ok || !strings.EqualFold(typeNames.ColumnTypeDatabaseTypeName(i), "UUID") {
			continue
		}
		dest[i] = string(raw)
	}
