...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID string) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_cache_read_price, image_output_price, per_request_price, reasoning_effort_multipliers, time_pricing, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_write_1h_price = $6, cache_read_price = $7, image_input_price = $8, image_cache_read_price = $9, image_output_price = $10, per_request_price = $11, reasoning_effort_multipliers = $12, time_pricing = $13, platform = $14, updated_at = NOW()
		 WHERE id = $15`,
		...
		pricing.ImageInputPrice, pricing.ImageCacheReadPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, reasoningJSON, timePricingJSON, pricing.Platform, pricing.ID,
	)
	if err != nil {
	...
	if rows == 0 {
		return fmt.Errorf("pricing entry not found: %s", pricing.ID)
	}
	return nil
}

func (r *channelRepository) DeleteModelPricing(ctx context.Context, id string) error {
...
func (r *channelRepository) ReplaceModelPricing(ctx context.Context, channelID string, pricingList []service.ChannelModelPricing) error {
...
func (r *channelRepository) batchLoadModelPricing(ctx context.Context, channelIDs []string) (map[string][]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_cache_read_price, image_output_price, per_request_price, reasoning_effort_multipliers, time_pricing, created_at, updated_at
	...
	// 按 channelID 分组
	pricingMap := make(map[string][]service.ChannelModelPricing, len(channelIDs))
...
func (r *channelRepository) batchLoadIntervals(ctx context.Context, pricingIDs []string) (map[string][]service.PricingInterval, error) {
	rows, err := r.db.QueryContext(ctx,
	...
	intervalMap := make(map[string][]service.PricingInterval, len(pricingIDs))
	for rows.Next() {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []string, error) {
	var result []service.ChannelModelPricing
	var pricingIDs []string
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.ImageInputPrice, &p.ImageCacheReadPrice, &p.ImageOutputPrice, &p.PerRequestPrice, &reasoningJSON, &timePricingJSON, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
		...
		p.ReasoningEffortMultipliers, err = unmarshalReasoningEffortMultipliers(reasoningJSON)
		if err != nil {
			return nil, nil, err
		}
...
func setGroupIDsTx(ctx context.Context, exec dbExec, channelID string, groupIDs []string) error {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_cache_read_price, image_output_price, per_request_price, reasoning_effort_multipliers, time_pricing)
		...
		pricing.ImageInputPrice, pricing.ImageCacheReadPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, reasoningJSON, timePricingJSON,
...
func replaceModelPricingTx(ctx context.Context, exec dbExec, channelID string, pricingList []service.ChannelModelPricing) error {
