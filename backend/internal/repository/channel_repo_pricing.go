...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID string) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_cache_read_price, image_output_price, per_request_price, max_reasoning_effort_multiplier, time_pricing, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_write_1h_price = $6, cache_read_price = $7, image_input_price = $8, image_cache_read_price = $9, image_output_price = $10, per_request_price = $11, max_reasoning_effort_multiplier = $12, time_pricing = $13, platform = $14, updated_at = NOW()
		 WHERE id = $15`,
		...
		pricing.ImageInputPrice, pricing.ImageCacheReadPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.MaxReasoningEffortMultiplier, timePricingJSON, pricing.Platform, pricing.ID,
	)
	if err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []string, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.ImageInputPrice, &p.ImageCacheReadPrice, &p.ImageOutputPrice, &p.PerRequestPrice, &p.MaxReasoningEffortMultiplier, &timePricingJSON, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_cache_read_price, image_output_price, per_request_price, max_reasoning_effort_multiplier, time_pricing)
		...
		pricing.ImageInputPrice, pricing.ImageCacheReadPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.MaxReasoningEffortMultiplier, timePricingJSON,
