...
func (r *groupRepository) Create(ctx context.Context, groupIn *service.Group) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &groupIn.ID, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group create failed: group=%s err=%v", groupIn.ID, err)
	}
	return nil
}
...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	levelRates := groupIn.LevelRateMultipliers
	if levelRates == nil {
		levelRates = map[string]float64{}
	}
	dynamicRules := groupIn.DynamicRateRules
	if dynamicRules == nil {
		dynamicRules = []service.GroupDynamicRateRule{}
	}
	builder := client.Group.Create().
		...
		SetLevelRateMultipliers(levelRates).
		SetDynamicRateRules(dynamicRules).
...
func (r *groupRepository) CreateFromSource(ctx context.Context, groupIn *service.Group, sourceGroupID string) error {
	if groupIn == nil {
		return errors.New("group is nil")
	}
...
func (r *groupRepository) GetByID(ctx context.Context, id string) (*service.Group, error) {
	out, err := r.GetByIDLite(ctx, id)
	if err != nil {
		return nil, err
	}
	counts, err := r.loadAccountCounts(ctx, []string{out.ID})
	if err == nil {
		c := counts[out.ID]
...
func (r *groupRepository) GetByIDLite(ctx context.Context, id string) (*service.Group, error) {
...
func (r *groupRepository) Update(ctx context.Context, groupIn *service.Group) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &groupIn.ID, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group update failed: group=%s err=%v", groupIn.ID, err)
	}
	return nil
}
...
func (r *groupRepository) Delete(ctx context.Context, id string) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &id, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group delete failed: group=%s err=%v", id, err)
	}
	return nil
}
...
func (r *groupRepository) listWithAccountCountSort(ctx context.Context, q *dbent.GroupQuery, params pagination.PaginationParams, total int) ([]service.Group, *pagination.PaginationResult, error) {
	...
	pageIdx := make(map[string]int, len(page))
	for i, e := range page {
		pageIDs[i] = e.id
		pageIdx[e.id] = i
	}
...
func (r *groupRepository) ListActiveIDs(ctx context.Context) ([]string, error) {
	if r.sql != nil {
...
func (r *groupRepository) ExistsByIDs(ctx context.Context, ids []string) (map[string]bool, error) {
...
func (r *groupRepository) GetAccountCount(ctx context.Context, groupID string) (total int64, active int64, err error) {
...
func (r *groupRepository) DeleteAccountGroupsByGroupID(ctx context.Context, groupID string) (int64, error) {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &groupID, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group account clear failed: group=%s err=%v", groupID, err)
	}
	return affected, nil
}

func (r *groupRepository) DeleteCascade(ctx context.Context, id string) ([]string, error) {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &id, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group cascade delete failed: group=%s err=%v", id, err)
	}
...
func (r *groupRepository) loadAccountCounts(ctx context.Context, groupIDs []string) (counts map[string]groupAccountCounts, err error) {
	counts = make(map[string]groupAccountCounts, len(groupIDs))
	if len(groupIDs) == 0 {
		return counts, nil
	}
...
func (r *groupRepository) GetAccountIDsByGroupIDs(ctx context.Context, groupIDs []string) ([]string, error) {
	if len(groupIDs) == 0 {
		return nil, nil
	}
...
func (r *groupRepository) BindAccountsToGroup(ctx context.Context, groupID string, accountIDs []string) error {
	if len(accountIDs) == 0 {
		return nil
	}
	...
	// 发送调度器事件
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &groupID, nil); err != nil {
		logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue bind accounts to group failed: group=%s err=%v", groupID, err)
	}
...
func (r *groupRepository) UpdateSortOrders(ctx context.Context, updates []service.GroupSortOrderUpdate) error {
	...
	sortOrderByID := make(map[string]int, len(updates))
	...
	for _, id := range groupIDs {
		if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &id, nil); err != nil {
			logger.LegacyPrintf("repository.group", "[SchedulerOutbox] enqueue group sort update failed: group=%s err=%v", id, err)
		}
	}
	return nil
}
