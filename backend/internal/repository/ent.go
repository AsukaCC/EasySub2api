...
import (
	...
	"github.com/AsukaCC/EasySub2api/migrations"
...
func InitEnt(cfg *config.Config) (*ent.Client, *sql.DB, error) {
	...
	// Keep the connector wrapper active even when Server-Timing headers are
	// disabled. Timing collection is request-scoped and becomes a no-op without
	// a collector, while the wrapper also normalizes native PostgreSQL UUID
	// values for Ent's string-backed UUID fields.
