...
func (c *openAI403CounterCache) IncrementOpenAI403Count(ctx context.Context, accountID string, windowMinutes int) (int64, error) {
	key := fmt.Sprintf("%s%s", openAI403CounterPrefix, accountID)
...
func (c *openAI403CounterCache) ResetOpenAI403Count(ctx context.Context, accountID string) error {
	key := fmt.Sprintf("%s%s", openAI403CounterPrefix, accountID)
