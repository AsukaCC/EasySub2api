...
func (r *announcementRepository) GetByID(ctx context.Context, id string) (*service.Announcement, error) {
...
func (r *announcementRepository) ListActive(ctx context.Context, now time.Time) ([]service.Announcement, error) {
	q := r.client.Announcement.Query().
		Where(
			...
			announcement.Or(
				announcement.NotifyModeEQ(service.AnnouncementNotifyModeSilent),
				announcement.And(
