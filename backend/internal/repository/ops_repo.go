...
func (r *opsRepository) InsertErrorLog(ctx context.Context, input *service.OpsInsertErrorLogInput) (string, error) {
	if r == nil || r.db == nil {
...
func opsInsertErrorLogArgs(input *service.OpsInsertErrorLogInput) []any {
	return []any{
		...
		opsNullString(input.UserID),
		opsNullString(input.APIKeyID),
		opsNullString(input.AccountID),
...
func (r *opsRepository) GetErrorLogByID(ctx context.Context, id string) (*service.OpsErrorLogDetail, error) {
	if r == nil || r.db == nil {
		return nil, fmt.Errorf("nil ops repository")
	}
...
func (r *opsRepository) UpdateErrorResolution(ctx context.Context, errorID string, resolved bool, resolvedByUserID *string, resolvedAt *time.Time) error {
	if r == nil || r.db == nil {
		return fmt.Errorf("nil ops repository")
	}
	if strings.TrimSpace(errorID) == "" {
...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	if filter.APIKeyID != nil && strings.TrimSpace(*filter.APIKeyID) != "" {
