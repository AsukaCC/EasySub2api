...
func TestUsageLogRepositoryEndpointStatsKeepsPointsAndAccountCostSeparate(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	mock.ExpectQuery("SUM\\(actual_cost\\).*as actual_cost.*account_stats_cost.*as account_cost").
		...
		WillReturnRows(sqlmock.NewRows([]string{"endpoint", "requests", "total_tokens", "cost", "actual_cost", "account_cost"}).
			AddRow("/v1/responses", int64(2), int64(300), 1.2, 2.4, 0.7))
	...
	require.Equal(t, 2.4, rows[0].ActualCost)
	require.Equal(t, 0.7, rows[0].AccountCost)
	require.NoError(t, mock.ExpectationsWereMet())
}
