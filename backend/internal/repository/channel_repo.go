...
func (r *channelRepository) GetByID(ctx context.Context, id string) (*service.Channel, error) {
	ch := &service.Channel{}
...
func (r *channelRepository) batchLoadGroupIDs(ctx context.Context, channelIDs []string) (map[string][]string, error) {
	rows, err := r.db.QueryContext(ctx,
	...
	groupMap := make(map[string][]string, len(channelIDs))
	for rows.Next() {
...
func (r *channelRepository) ExistsByNameExcluding(ctx context.Context, name string, excludeID string) (bool, error) {
	var exists bool
	err := r.db.QueryRowContext(ctx,
...
func (r *channelRepository) GetGroupIDs(ctx context.Context, channelID string) ([]string, error) {
	rows, err := r.db.QueryContext(ctx,
...
func (r *channelRepository) SetGroupIDs(ctx context.Context, channelID string, groupIDs []string) error {
	return setGroupIDsTx(ctx, r.db, channelID, groupIDs)
}

func (r *channelRepository) GetChannelIDByGroupID(ctx context.Context, groupID string) (string, error) {
...
func (r *channelRepository) GetGroupsInOtherChannels(ctx context.Context, channelID string, groupIDs []string) ([]string, error) {
	if len(groupIDs) == 0 {
		return nil, nil
	}
	rows, err := r.db.QueryContext(ctx,
...
func (r *channelRepository) GetGroupPlatforms(ctx context.Context, groupIDs []string) (map[string]string, error) {
	if len(groupIDs) == 0 {
