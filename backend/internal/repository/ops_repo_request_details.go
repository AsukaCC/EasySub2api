...
`, cte, where, sort, len(args)+1, len(args)+2)
	...
	for rows.Next() {
		...
		item := &service.OpsRequestDetail{
			...
			ErrorID:    toStringPtr(errorID),
			Phase:      phase.String,
			Severity:   severity.String,
			Message:    message.String,

			UserID:    toStringPtr(userID),
			APIKeyID:  toStringPtr(apiKeyID),
			AccountID: toStringPtr(accountID),
			GroupID:   toStringPtr(groupID),
