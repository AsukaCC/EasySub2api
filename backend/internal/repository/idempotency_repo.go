...
func (r *idempotencyRepository) MarkSucceeded(ctx context.Context, id string, responseStatus int, responseBody string, expiresAt time.Time) error {
	query := `
		UPDATE idempotency_records
		SET status = $2,
			response_status = $3,
...
func (r *idempotencyRepository) MarkFailedRetryable(ctx context.Context, id string, errorReason string, lockedUntil, expiresAt time.Time) error {
	query := `
		UPDATE idempotency_records
		SET status = $2,
			error_reason = $3,
