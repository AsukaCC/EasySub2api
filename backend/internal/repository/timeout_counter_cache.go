...
func (c *timeoutCounterCache) IncrementTimeoutCount(ctx context.Context, accountID string, windowMinutes int) (int64, error) {
	key := fmt.Sprintf("%s%s", timeoutCounterPrefix, accountID)
...
func (c *timeoutCounterCache) GetTimeoutCount(ctx context.Context, accountID string) (int64, error) {
	key := fmt.Sprintf("%s%s", timeoutCounterPrefix, accountID)
...
func (c *timeoutCounterCache) ResetTimeoutCount(ctx context.Context, accountID string) error {
	key := fmt.Sprintf("%s%s", timeoutCounterPrefix, accountID)
	return c.rdb.Del(ctx, key).Err()
}

// GetTimeoutCountTTL 获取计数器剩余过期时间
func (c *timeoutCounterCache) GetTimeoutCountTTL(ctx context.Context, accountID string) (time.Duration, error) {
	key := fmt.Sprintf("%s%s", timeoutCounterPrefix, accountID)
