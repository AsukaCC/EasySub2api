...
func (r *authCacheInvalidationOutboxRepository) ScheduleSecondPass(ctx context.Context, id string, workerID string, availableAt time.Time) error {
	...
	if affected != 1 {
		return fmt.Errorf("auth cache invalidation claim %s cannot schedule second pass", id)
	}
	return nil
}

func (r *authCacheInvalidationOutboxRepository) DeleteClaimed(ctx context.Context, id string, workerID string) error {
	result, err := r.db.ExecContext(ctx, `
	...
	if affected != 1 {
		return fmt.Errorf("auth cache invalidation claim %s is no longer owned by %s", id, workerID)
	}
	return nil
}

func (r *authCacheInvalidationOutboxRepository) RetryClaimed(ctx context.Context, id string, workerID string, availableAt time.Time, lastError string) error {
	result, err := r.db.ExecContext(ctx, `
		UPDATE auth_cache_invalidation_outbox
