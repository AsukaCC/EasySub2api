...
func redeemRateLimitKey(userID string) string {
	return fmt.Sprintf("%s%s", redeemRateLimitKeyPrefix, userID)
}
...
func (c *redeemCache) GetRedeemAttemptCount(ctx context.Context, userID string) (int, error) {
...
func (c *redeemCache) IncrementRedeemAttemptCount(ctx context.Context, userID string) error {
	key := redeemRateLimitKey(userID)
	pipe := c.rdb.Pipeline()
	pipe.Incr(ctx, key)
