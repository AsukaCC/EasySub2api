...
func (r *usageLogRepository) GetUserStatsAggregated(ctx context.Context, userID string, startTime, endTime time.Time) (*usagestats.UsageStats, error) {
	query := `
		SELECT
...
func (r *usageLogRepository) GetAPIKeyStatsAggregated(ctx context.Context, apiKeyID string, startTime, endTime time.Time) (*usagestats.UsageStats, error) {
	query := `
		SELECT
			COUNT(*) as total_requests,
			COALESCE(SUM(input_tokens), 0) as total_input_tokens,
...
// 3. 利用数据库索引优化聚合查询性能
func (r *usageLogRepository) GetAccountStatsAggregated(ctx context.Context, accountID string, startTime, endTime time.Time) (*usagestats.UsageStats, error) {
	query := `
		SELECT
			COUNT(*) as total_requests,
			COALESCE(SUM(input_tokens), 0) as total_input_tokens,
...
func (r *usageLogRepository) GetDailyStatsAggregated(ctx context.Context, userID string, startTime, endTime time.Time) (result []map[string]any, err error) {
...
func (r *usageLogRepository) GetAccountTodayStats(ctx context.Context, accountID string) (*usagestats.AccountStats, error) {
...
func (r *usageLogRepository) GetAccountWindowStats(ctx context.Context, accountID string, startTime time.Time) (*usagestats.AccountStats, error) {
	query := `
		SELECT
			COUNT(*) as requests,
			COALESCE(SUM(input_tokens + output_tokens + cache_creation_tokens + cache_read_tokens), 0) as tokens,
...
func (r *usageLogRepository) GetAccountWindowStatsBatch(ctx context.Context, accountIDs []string, startTime time.Time) (map[string]*usagestats.AccountStats, error) {
	result := make(map[string]*usagestats.AccountStats, len(accountIDs))
	if len(accountIDs) == 0 {
		return result, nil
	}
...
func (r *usageLogRepository) GetBatchUserUsageStats(ctx context.Context, userIDs []string, startTime, endTime time.Time) (map[string]*BatchUserUsageStats, error) {
	result := make(map[string]*BatchUserUsageStats)
	normalizedUserIDs := normalizeIDs(userIDs)
...
// If startTime is zero, defaults to 30 days ago.
func (r *usageLogRepository) GetBatchAPIKeyUsageStats(ctx context.Context, apiKeyIDs []string, startTime, endTime time.Time) (map[string]*BatchAPIKeyUsageStats, error) {
	result := make(map[string]*BatchAPIKeyUsageStats)
	normalizedAPIKeyIDs := normalizeIDs(apiKeyIDs)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&apiKeyID, &total, &todayTotal, &totalRequests, &totalTokens); err != nil {
			_ = rows.Close()
			return nil, err
		}
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	conditions, args = appendUsageLogUserRoleScopeCondition(conditions, args, filters.UserRoleScope, "")
	...
	runEndpoints := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.NativeCompactionV2, filters.BillingType, filters.BillingMode, filters.UserRoleScope)
		if err != nil {
	...
	runPaths := func(c context.Context) {
		res, err := r.getEndpointPathStatsWithFilters(c, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.NativeCompactionV2, filters.BillingType, filters.BillingMode, filters.UserRoleScope)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, modelSource string, requestType *int16, stream *bool, nativeCompactionV2 *bool, billingType *int8, billingMode string, scopes ...string) (results []EndpointStat, err error) {
	userRoleScope := ""
	if len(scopes) > 0 {
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, modelSource string, requestType *int16, stream *bool, nativeCompactionV2 *bool, billingType *int8, billingMode string, scopes ...string) (results []EndpointStat, err error) {
	userRoleScope := ""
	if len(scopes) > 0 {
		userRoleScope = scopes[0]
	}
...
func (r *usageLogRepository) GetEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, requestType *int16, stream *bool, billingType *int8) ([]EndpointStat, error) {
...
func (r *usageLogRepository) GetUpstreamEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID string, model string, requestType *int16, stream *bool, billingType *int8) ([]EndpointStat, error) {
...
func (r *usageLogRepository) GetAccountUsageStats(ctx context.Context, accountID string, startTime, endTime time.Time) (resp *AccountUsageStatsResponse, err error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&date, &requests, &tokens, &cost, &accountCost, &userCost); err != nil {
			return nil, err
		}
	...
	for i := range history {
		h := &history[i]
		totalAccountCost += h.AccountCost
		totalUserCost += h.UserCost
		totalStandardCost += h.Cost
		totalRequests += h.Requests
		totalTokens += h.Tokens

		if highestCostDay == nil || h.AccountCost > highestCostDay.AccountCost {
			highestCostDay = h
		}
	...
	if highestCostDay != nil {
		summary.HighestCostDay = &struct {
			...
			Cost:     highestCostDay.AccountCost,
	...
	if highestRequestDay != nil {
		summary.HighestRequestDay = &struct {
			...
			Cost:     highestRequestDay.AccountCost,
			UserCost: highestRequestDay.UserCost,
		}
	}

	models, err := r.GetModelStatsWithFilters(ctx, startTime, endTime, "", "", accountID, "", nil, nil, nil)
	if err != nil {
		models = []ModelStat{}
	}
	endpoints, endpointErr := r.GetEndpointStatsWithFilters(ctx, startTime, endTime, "", "", accountID, "", "", nil, nil, nil)
	if endpointErr != nil {
		logger.LegacyPrintf("repository.usage_log", "GetEndpointStatsWithFilters failed in GetAccountUsageStats: %v", endpointErr)
		endpoints = []EndpointStat{}
	}
	upstreamEndpoints, upstreamEndpointErr := r.GetUpstreamEndpointStatsWithFilters(ctx, startTime, endTime, "", "", accountID, "", "", nil, nil, nil)
