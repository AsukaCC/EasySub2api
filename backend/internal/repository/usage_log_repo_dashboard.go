...
func (r *usageLogRepository) GetUserStats(ctx context.Context, userID string, startTime, endTime time.Time) (*UserStats, error) {
	query := `
		SELECT
...
func (r *usageLogRepository) GetUserDashboardStats(ctx context.Context, userID string) (*UserDashboardStats, error) {
...
func (r *usageLogRepository) getPerformanceStatsByAPIKey(ctx context.Context, apiKeyID string) (rpm, tpm int64, err error) {
	fiveMinutesAgo := time.Now().Add(-5 * time.Minute)
	query := `
		SELECT
			COUNT(*) as request_count,
...
func (r *usageLogRepository) GetAPIKeyDashboardStats(ctx context.Context, apiKeyID string) (*UserDashboardStats, error) {
	stats := &UserDashboardStats{}
	today := timezone.Today()
