...
func (c *RPMCacheImpl) currentMinuteKey(ctx context.Context, accountID string) (string, error) {
	serverTime, err := c.rdb.Time(ctx).Result()
	if err != nil {
		return "", fmt.Errorf("redis TIME: %w", err)
	}
	minuteTS := serverTime.Unix() / 60
	return fmt.Sprintf("%s%s:%d", rpmKeyPrefix, accountID, minuteTS), nil
}
...
// 使用 TxPipeline (MULTI/EXEC) 执行 INCR + EXPIRE，保证原子性且兼容 Redis Cluster
func (c *RPMCacheImpl) IncrementRPM(ctx context.Context, accountID string) (int, error) {
...
func (c *RPMCacheImpl) GetRPM(ctx context.Context, accountID string) (int, error) {
	key, err := c.currentMinuteKey(ctx, accountID)
	if err != nil {
...
func (c *RPMCacheImpl) GetRPMBatch(ctx context.Context, accountIDs []string) (map[string]int, error) {
	if len(accountIDs) == 0 {
	...
	for _, id := range accountIDs {
		key := fmt.Sprintf("%s%s:%s", rpmKeyPrefix, id, minuteSuffix)
