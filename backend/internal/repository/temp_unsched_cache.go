...
func (c *tempUnschedCache) SetTempUnsched(ctx context.Context, accountID string, state *service.TempUnschedState) error {
	key := fmt.Sprintf("%s%s", tempUnschedPrefix, accountID)
...
func (c *tempUnschedCache) GetTempUnsched(ctx context.Context, accountID string) (*service.TempUnschedState, error) {
	key := fmt.Sprintf("%s%s", tempUnschedPrefix, accountID)
...
func (c *tempUnschedCache) DeleteTempUnsched(ctx context.Context, accountID string) error {
	key := fmt.Sprintf("%s%s", tempUnschedPrefix, accountID)
