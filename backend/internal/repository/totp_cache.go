...
func (c *TotpCache) GetSetupSession(ctx context.Context, userID string) (*service.TotpSetupSession, error) {
	key := fmt.Sprintf("%s%s", totpSetupKeyPrefix, userID)
...
func (c *TotpCache) SetSetupSession(ctx context.Context, userID string, session *service.TotpSetupSession, ttl time.Duration) error {
	key := fmt.Sprintf("%s%s", totpSetupKeyPrefix, userID)
...
func (c *TotpCache) DeleteSetupSession(ctx context.Context, userID string) error {
	key := fmt.Sprintf("%s%s", totpSetupKeyPrefix, userID)
...
func (c *TotpCache) IncrementVerifyAttempts(ctx context.Context, userID string) (int, error) {
	key := fmt.Sprintf("%s%s", totpAttemptsKeyPrefix, userID)
...
func (c *TotpCache) GetVerifyAttempts(ctx context.Context, userID string) (int, error) {
	key := fmt.Sprintf("%s%s", totpAttemptsKeyPrefix, userID)
...
func (c *TotpCache) ClearVerifyAttempts(ctx context.Context, userID string) error {
	key := fmt.Sprintf("%s%s", totpAttemptsKeyPrefix, userID)
	return c.rdb.Del(ctx, key).Err()
}

func totpStepUpKey(userID string, sessionKey string) string {
	return fmt.Sprintf("%s%s:%s", totpStepUpKeyPrefix, userID, sessionKey)
}

// SetStepUpGrant 记录一次 step-up 验证通过（sudo 窗口），绑定用户+会话。
func (c *TotpCache) SetStepUpGrant(ctx context.Context, userID string, sessionKey string, ttl time.Duration) error {
	return c.rdb.Set(ctx, totpStepUpKey(userID, sessionKey), "1", ttl).Err()
}

// HasStepUpGrant 检查 step-up 授权是否仍在有效期内。
func (c *TotpCache) HasStepUpGrant(ctx context.Context, userID string, sessionKey string) (bool, error) {
