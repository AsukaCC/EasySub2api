...
func (r *proxyRepository) GetByID(ctx context.Context, id string) (*service.Proxy, error) {
...
func (r *proxyRepository) ListByIDs(ctx context.Context, ids []string) ([]service.Proxy, error) {
	if len(ids) == 0 {
...
func lockProxyProbeIdentity(ctx context.Context, client *dbent.Client, proxyID string) (proxyProbeIdentity, error) {
...
func invalidateProxyProbeSnapshots(ctx context.Context, exec sqlExecutor, proxyID string) ([]string, error) {
...
func enqueueProxyProbeAccountChanges(ctx context.Context, exec sqlExecutor, accountIDs []string) error {
...
func (r *proxyRepository) CountAccountsByProxyID(ctx context.Context, proxyID string) (int64, error) {
	var count int64
...
func (r *proxyRepository) ListAccountSummariesByProxyID(ctx context.Context, proxyID string) ([]service.ProxyAccountSummary, error) {
...
func (r *proxyRepository) GetAccountCountsForProxies(ctx context.Context) (counts map[string]int64, err error) {
...
func (r *proxyRepository) SweepExpiredProxies(ctx context.Context, now time.Time) (int64, error) {
	...
	byID := make(map[string]service.Proxy, len(all))
	for _, p := range all {
		byID[p.ID] = p
	}

	var totalChanged int64
	allChangedAccountIDs := make([]string, 0)
	...
	for _, p := range all {
		...
		if !change && p.FallbackMode == service.FallbackModeProxy {
			...
			logger.LegacyPrintf("repository.proxy", "[ProxyExpiry] proxy %s expired but fallback chain unresolved (cycle/all-expired); accounts kept", p.ID)
		}
...
func sortedUniqueAccountIDs(accountIDs []string) []string {
	if len(accountIDs) < 2 {
		return accountIDs
	}
...
func (r *proxyRepository) sweepOneExpiredProxy(ctx context.Context, snapshot service.Proxy, now time.Time, target *string, change bool) ([]string, error) {
...
func (r *proxyRepository) sweepOneExpiredProxyOnExec(ctx context.Context, exec sqlExecutor, snapshot service.Proxy, now time.Time, target *string, change bool) ([]string, error) {
	proxyID := snapshot.ID
