...
import (
	...
	"github.com/AsukaCC/EasySub2api/ent/channelmonitorrequesttemplate"
...
func (r *channelMonitorRequestTemplateRepository) GetByID(ctx context.Context, id string) (*service.ChannelMonitorRequestTemplate, error) {
...
func (r *channelMonitorRequestTemplateRepository) Delete(ctx context.Context, id string) error {
	client := clientFromContext(ctx, r.client)
...
func (r *channelMonitorRequestTemplateRepository) ApplyToMonitors(ctx context.Context, id string, monitorIDs []string) (int64, error) {
	if len(monitorIDs) == 0 {
		return 0, nil
	}
...
func (r *channelMonitorRequestTemplateRepository) CountAssociatedMonitors(ctx context.Context, id string) (int64, error) {
	count, err := r.client.ChannelMonitor.Query().
		Where(channelmonitor.TemplateIDEQ(id)).
		Count(ctx)
	if err != nil {
		return 0, fmt.Errorf("count monitors for template %s: %w", id, err)
	}
...
func (r *channelMonitorRequestTemplateRepository) ListAssociatedMonitors(ctx context.Context, id string) ([]*service.AssociatedMonitorBrief, error) {
	rows, err := r.client.ChannelMonitor.Query().
		Where(channelmonitor.TemplateIDEQ(id)).
		Order(dbent.Asc(channelmonitor.FieldName)).
		All(ctx)
	if err != nil {
		return nil, fmt.Errorf("list associated monitors for template %s: %w", id, err)
	}
