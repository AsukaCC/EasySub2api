...
LIMIT $`+fmt.Sprint(len(queryArgs)-1)+` OFFSET $`+fmt.Sprint(len(queryArgs)),
	...
	for rows.Next() {
		...
		var latency, queueDelay sql.NullInt64
...
func (r *contentModerationRepository) CountFlaggedByUserSince(ctx context.Context, userID string, since time.Time, excludeCyberPolicy bool) (int, error) {
...
func (r *contentModerationRepository) UpdateLogEmailSent(ctx context.Context, id string, sent bool) error {
