...
func TestAffiliateRepository_ThawFrozenQuota_DoesNotRestoreReversedPoints(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	txCtx := dbent.NewTxContext(ctx, tx)
	client := tx.Client()
	...
	user := mustCreateUser(t, client, &service.User{
		Email:        fmt.Sprintf("affiliate-thaw-reversed-%d@example.com", time.Now().UnixNano()),
		PasswordHash: "hash", Role: service.RoleUser, Status: service.StatusActive,
	})
	affCode := fmt.Sprintf("REV%09d", time.Now().UnixNano()%1_000_000_000)
	_, err := client.ExecContext(txCtx, `
INSERT INTO user_affiliates
    (user_id, aff_code, aff_quota, aff_frozen_quota, aff_history_quota, created_at, updated_at)
VALUES ($1, $2, 0, 6, 10, NOW(), NOW())`, user.ID, affCode)
	require.NoError(t, err)
	_, err = client.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger
    (user_id, action, amount, reversed_amount, frozen_until, created_at, updated_at)
VALUES ($1, 'accrue', 10, 4, NOW() - INTERVAL '1 minute', NOW(), NOW())`, user.ID)
	require.NoError(t, err)

	repo := NewAffiliateRepository(client, integrationDB)
	thawed, err := repo.ThawFrozenQuota(txCtx, user.ID)
	require.NoError(t, err)
	require.InDelta(t, 6.0, thawed, 1e-9)
	...
	require.InDelta(t, 0.0, querySingleFloat(t, txCtx, client,
		"SELECT aff_frozen_quota::double precision FROM user_affiliates WHERE user_id = $1", user.ID), 1e-9)
}
