...
func (r *channelMonitorV2Repository) GetDimensions(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) (*service.ChannelMonitorV2Dimensions, error) {
	...
	query := `SELECT m.platform, COALESCE(g.name, ''), lower(COALESCE(NULLIF(TRIM(g.platform), ''), 'unknown')), m.group_id, m.model,
	          ...
	          LEFT JOIN groups g ON g.id = NULLIF(m.group_id, '` + postgresNilUUID + `'::uuid) ` + where + `
	...
	groupCounts := map[string]groupValue{}
	...
	for rows.Next() {
		...
		if groupID != "" && groupID != postgresNilUUID {
...
func (r *channelMonitorV2Repository) GetSnapshot(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, admin bool) (*service.ChannelMonitorV2Snapshot, error) {
	...
	histograms, err := r.loadHistograms(ctx, effectiveFilter, cfg, "", true)
	if err != nil {
		return nil, err
	}
...
func (r *channelMonitorV2Repository) GetModels(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, admin bool) (*service.ChannelMonitorV2List[service.ChannelMonitorV2ModelRow], error) {
	...
	hist, err := r.loadHistograms(ctx, effectiveFilter, cfg, "", false)
	if err != nil {
		return nil, err
	}
...
func (r *channelMonitorV2Repository) GetMatrix(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, groupBy service.ChannelMonitorV2GroupBy, admin bool) (*service.ChannelMonitorV2Matrix, error) {
	...
	for _, fact := range facts {
		...
		if (groupBy == service.ChannelMonitorV2GroupByPlatformGroup || groupBy == service.ChannelMonitorV2GroupByPlatformGroupModel) &&
			(fact.GroupID == "" || fact.GroupID == postgresNilUUID) {
			continue
		}
		...
		if key.groupID != "" && key.groupID != postgresNilUUID {
	...
	for _, histogram := range histograms {
		...
		if (groupBy == service.ChannelMonitorV2GroupByPlatformGroup || groupBy == service.ChannelMonitorV2GroupByPlatformGroupModel) &&
			(histogram.GroupID == "" || histogram.GroupID == postgresNilUUID) {
			continue
		}
	...
	for key, acc := range accs {
		...
		if (groupBy == service.ChannelMonitorV2GroupByPlatformGroup || groupBy == service.ChannelMonitorV2GroupByPlatformGroupModel) &&
			(key.groupID == "" || key.groupID == postgresNilUUID) {
			continue
		}
		metrics := acc.total.metric(minutes, admin)
		applyIgnoredErrors(&metrics, ignoredByDim[key])
		row := service.ChannelMonitorV2MatrixRow{Platform: key.platform, GroupName: acc.groupName, RateMultiplier: acc.rateMultiplier, Model: key.model, Metrics: metrics, Health: service.ChannelMonitorV2HealthForWithThresholds(metrics, cfg.HealthThresholds), Buckets: []service.ChannelMonitorV2TrendPoint{}}
		if key.groupID != "" && key.groupID != postgresNilUUID {
			groupID := key.groupID
			row.GroupID = &groupID
		}
...
func channelMonitorV2MatrixDimensionKey(groupBy service.ChannelMonitorV2GroupBy, cfg service.ChannelMonitorV2Config, platform string, groupID string, model string) channelMonitorV2MatrixKey {
...
func seedChannelMonitorV2MatrixAccumulators(filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, groupBy service.ChannelMonitorV2GroupBy, groupInfo map[string]channelMonitorV2GroupInfo) map[channelMonitorV2MatrixKey]*channelMonitorV2MatrixAccumulator {
	...
	groupIDs := []string{postgresNilUUID}
	if needsGroup {
	...
	for _, platform := range platforms {
		...
		for _, groupID := range groupIDs {
			...
			for _, model := range models {
				...
				if accs[key] == nil {
					accs[key] = &channelMonitorV2MatrixAccumulator{groupName: info.name, rateMultiplier: info.rateMultiplier, total: newMetricAccumulator(), buckets: make(map[string]*metricAccumulator)}
				}
			}
		}
	}
...
func configuredChannelMonitorV2GroupIDs(filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) []string {
	groups := append([]string(nil), cfg.GroupIDs...)
	if len(filter.GroupIDs) > 0 {
		if len(groups) > 0 {
			groups = intersectStrings(groups, filter.GroupIDs)
		} else {
			groups = append([]string(nil), filter.GroupIDs...)
		}
	}
	return groups
}
...
func (r *channelMonitorV2Repository) listActiveGroupIDs(ctx context.Context) ([]string, error) {
...
func (r *channelMonitorV2Repository) loadChannelMonitorV2GroupInfo(ctx context.Context, groupIDs []string) (map[string]channelMonitorV2GroupInfo, error) {
	out := map[string]channelMonitorV2GroupInfo{}
	if len(groupIDs) == 0 {
		return out, nil
	}
	rows, err := r.db.QueryContext(ctx, `SELECT id, COALESCE(name, ''), lower(COALESCE(NULLIF(TRIM(platform), ''), 'unknown')), COALESCE(rate_multiplier, 1) FROM groups WHERE id = ANY($1) AND deleted_at IS NULL AND status = 'active'`, pq.Array(groupIDs))
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	for rows.Next() {
		...
		if err := rows.Scan(&id, &name, &platform, &rateMultiplier); err != nil {
			return nil, err
		}
		out[id] = channelMonitorV2GroupInfo{name: name, platform: platform, rateMultiplier: rateMultiplier}
	}
	return out, rows.Err()
}
...
func (r *channelMonitorV2Repository) loadErrorDetails(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) (map[string][]service.ChannelMonitorV2ErrorDetail, error) {
	...
	if groupScopeEmpty {
		...
		conditions = append(conditions, fmt.Sprintf("COALESCE(current_error.group_id, '%s'::uuid) = ANY($%d)", postgresNilUUID, len(args)))
	}
	query := `SELECT
			lower(COALESCE(NULLIF(TRIM(current_error.platform), ''), 'unknown')) AS platform,
			COALESCE(current_error.group_id, '` + postgresNilUUID + `'::uuid) AS group_id,
...
func (r *channelMonitorV2Repository) GetUsers(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, admin bool) (*service.ChannelMonitorV2List[service.ChannelMonitorV2UserRow], error) {
	...
	histograms, err := r.loadHistograms(ctx, filter, cfg, "*", false)
	if err != nil {
		return nil, err
	}
	for _, histogram := range histograms {
...
func (r *channelMonitorV2Repository) loadHistograms(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, userID string, byBucket bool) ([]channelMonitorV2Histogram, error) {
	where, args, bucketSeconds := channelMonitorV2WhereWithRollup(filter, cfg, "h")
	if userID == "*" {
		where += " AND h.user_id <> '" + postgresNilUUID + "'::uuid"
	} else {
		if userID == "" {
			userID = postgresNilUUID
		}
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCountsByMatrixKey(
	...
	for rows.Next() {
		var bucket time.Time
		var platform, model, groupID string
		var count int64
