...
func sessionLimitKey(accountID string) string {
	return sessionLimitKeyPrefix + accountID
}

// windowCostKey 生成窗口费用缓存的 Redis 键
func windowCostKey(accountID string) string {
	return windowCostKeyPrefix + accountID
}

// RegisterSession 注册会话活动
func (c *sessionLimitCache) RegisterSession(ctx context.Context, accountID string, sessionUUID string, maxSessions int, idleTimeout time.Duration) (bool, error) {
...
func (c *sessionLimitCache) RefreshSession(ctx context.Context, accountID string, sessionUUID string, idleTimeout time.Duration) error {
	if sessionUUID == "" {
		return nil
	}
...
func (c *sessionLimitCache) GetActiveSessionCountBatch(ctx context.Context, accountIDs []string, idleTimeouts map[string]time.Duration) (map[string]int, error) {
	if len(accountIDs) == 0 {
		return make(map[string]int), nil
	}

	results := make(map[string]int, len(accountIDs))

	// 使用 pipeline 批量执行
	pipe := c.rdb.Pipeline()

	cmds := make(map[string]*redis.Cmd, len(accountIDs))
...
func (c *sessionLimitCache) IsSessionActive(ctx context.Context, accountID string, sessionUUID string) (bool, error) {
	if sessionUUID == "" {
		return false, nil
	}
...
func (c *sessionLimitCache) GetWindowCost(ctx context.Context, accountID string) (float64, bool, error) {
...
func (c *sessionLimitCache) SetWindowCost(ctx context.Context, accountID string, cost float64) error {
	key := windowCostKey(accountID)
	return c.rdb.Set(ctx, key, cost, windowCostCacheTTL).Err()
}

// GetWindowCostBatch 批量获取窗口费用缓存
func (c *sessionLimitCache) GetWindowCostBatch(ctx context.Context, accountIDs []string) (map[string]float64, error) {
	if len(accountIDs) == 0 {
	...
	results := make(map[string]float64, len(accountIDs))
