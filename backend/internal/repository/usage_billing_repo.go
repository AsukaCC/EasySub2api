...
type usageBillingRepository struct {
	db     *sql.DB
	wallet service.WalletRepository
}

func NewUsageBillingRepository(_ *dbent.Client, sqlDB *sql.DB, wallet service.UserRepository) service.UsageBillingRepository {
	return &usageBillingRepository{db: sqlDB, wallet: wallet}
}
...
func (r *usageBillingRepository) claimUsageBillingRequest(ctx context.Context, tx *sql.Tx, requestID string, apiKeyID string, requestFingerprint string) (bool, error) {
	var id string
...
func (r *usageBillingRepository) applyUsageBillingEffects(ctx context.Context, tx *sql.Tx, cmd *service.UsageBillingCommand, result *service.UsageBillingApplyResult) error {
	if err := applyDynamicRateBilling(ctx, tx, cmd, result); err != nil {
		return err
	}
...
func applyDynamicRateBilling(ctx context.Context, tx *sql.Tx, cmd *service.UsageBillingCommand, result *service.UsageBillingApplyResult) error {
	if cmd == nil || cmd.DynamicRatePlan == nil || cmd.DynamicRatePlan.StandardCost <= 0 {
		return nil
	}
	...
	for _, rule := range rules {
		...
		var personalUsedAmount float64
		if err := tx.QueryRowContext(ctx, `
			SELECT used_amount
			FROM user_dynamic_rate_usage
			WHERE user_id = $1 AND group_id = $2 AND rule_id = $3 AND quota_key = $4
			FOR UPDATE
		`, cmd.UserID, plan.GroupID, rule.RuleID, rule.QuotaKey).Scan(&personalUsedAmount); err != nil {
			return err
		}
		if personalLimited {
	...
	if remainingStandard > 0 {
		finalCost += remainingStandard * plan.FallbackMultiplier
	}
	finalCost = service.QuantizeUsageBillingAmount(finalCost)
	finalMultiplier := service.QuantizeRateMultiplier(finalCost / plan.StandardCost)
...
func userExistsForBilling(ctx context.Context, tx *sql.Tx, userID string) (bool, error) {
	var exists int
	err := tx.QueryRowContext(ctx, `
		SELECT 1
		FROM users
...
func incrementUsageBillingAPIKeyQuota(ctx context.Context, tx *sql.Tx, apiKeyID string, amount float64) (bool, error) {
	var exhausted bool
	err := tx.QueryRowContext(ctx, `
		UPDATE api_keys
...
func incrementUsageBillingAPIKeyRateLimit(ctx context.Context, tx *sql.Tx, apiKeyID string, cost float64) error {
...
func incrementUsageBillingAccountQuota(ctx context.Context, tx *sql.Tx, accountID string, amount float64) (*service.AccountQuotaState, error) {
	...
	if crossedTotal || crossedDaily || crossedWeekly {
		if err := enqueueSchedulerOutbox(ctx, tx, service.SchedulerOutboxEventAccountChanged, &accountID, nil, nil); err != nil {
			logger.LegacyPrintf("repository.usage_billing", "[SchedulerOutbox] enqueue quota exceeded failed: account=%s err=%v", accountID, err)
			return nil, err
		}
	}
	return &state, nil
