...
import (
	...
	"github.com/AsukaCC/EasySub2api/ent/channelmonitor"
	"github.com/AsukaCC/EasySub2api/ent/channelmonitorhistory"
...
func (r *channelMonitorRepository) GetByID(ctx context.Context, id string) (*service.ChannelMonitor, error) {
...
func (r *channelMonitorRepository) Delete(ctx context.Context, id string) error {
	client := clientFromContext(ctx, r.client)
...
func (r *channelMonitorRepository) MarkChecked(ctx context.Context, id string, checkedAt time.Time) error {
	client := clientFromContext(ctx, r.client)
...
func (r *channelMonitorRepository) ListHistory(ctx context.Context, monitorID string, model string, limit int) ([]*service.ChannelMonitorHistoryEntry, error) {
...
func (r *channelMonitorRepository) ListLatestPerModel(ctx context.Context, monitorID string) ([]*service.ChannelMonitorLatest, error) {
	const q = `
...
func (r *channelMonitorRepository) ComputeAvailability(ctx context.Context, monitorID string, windowDays int) ([]*service.ChannelMonitorAvailability, error) {
	if windowDays <= 0 {
		windowDays = 7
	}
	const q = `
...
func (r *channelMonitorRepository) ListLatestForMonitorIDs(ctx context.Context, ids []string) (map[string][]*service.ChannelMonitorLatest, error) {
	out := make(map[string][]*service.ChannelMonitorLatest, len(ids))
	if len(ids) == 0 {
		return out, nil
	}
	const q = `
...
func (r *channelMonitorRepository) ListRecentHistoryForMonitors(
	ctx context.Context,
	ids []string,
	primaryModels map[string]string,
	perMonitorLimit int,
) (map[string][]*service.ChannelMonitorHistoryEntry, error) {
	out := make(map[string][]*service.ChannelMonitorHistoryEntry, len(ids))
...
func buildMonitorModelPairs(ids []string, primaryModels map[string]string) ([]string, []string) {
	if len(ids) == 0 || len(primaryModels) == 0 {
		return nil, nil
	}
	pairIDs := make([]string, 0, len(ids))
...
func (r *channelMonitorRepository) ComputeAvailabilityForMonitors(ctx context.Context, ids []string, windowDays int) (map[string][]*service.ChannelMonitorAvailability, error) {
	out := make(map[string][]*service.ChannelMonitorAvailability, len(ids))
	if len(ids) == 0 {
		return out, nil
	}
	if windowDays <= 0 {
