...
func billingBalanceKey(userID string) string {
	return fmt.Sprintf("%s%s", billingBalanceKeyPrefix, userID)
}

// billingSubKey generates the Redis key for subscription cache.
func billingSubKey(userID, groupID string) string {
	return fmt.Sprintf("%s%s:%s", billingSubKeyPrefix, userID, groupID)
}
...
func billingRateLimitKey(keyID string) string {
	return fmt.Sprintf("%s%s", billingRateLimitKeyPrefix, keyID)
}
...
func (c *billingCache) GetUserBalance(ctx context.Context, userID string) (float64, error) {
...
func (c *billingCache) SetUserBalance(ctx context.Context, userID string, balance float64) error {
	key := billingBalanceKey(userID)
	return c.rdb.Set(ctx, key, balance, jitteredTTL()).Err()
}

func (c *billingCache) DeductUserBalance(ctx context.Context, userID string, amount float64) error {
	key := billingBalanceKey(userID)
	_, err := deductBalanceScript.Run(ctx, c.rdb, []string{key}, amount, int(jitteredTTL().Seconds())).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		log.Printf("Warning: deduct balance cache failed for user %s: %v", userID, err)
		return err
	}
	return nil
}

func (c *billingCache) InvalidateUserBalance(ctx context.Context, userID string) error {
	key := billingBalanceKey(userID)
	return c.rdb.Del(ctx, key).Err()
}

func (c *billingCache) GetSubscriptionCache(ctx context.Context, userID, groupID string) (*service.SubscriptionCacheData, error) {
...
func (c *billingCache) SetSubscriptionCache(ctx context.Context, userID, groupID string, data *service.SubscriptionCacheData) error {
	if data == nil {
		return nil
	}
...
func (c *billingCache) UpdateSubscriptionUsage(ctx context.Context, userID, groupID string, cost float64) error {
	key := billingSubKey(userID, groupID)
	_, err := updateSubUsageScript.Run(ctx, c.rdb, []string{key}, cost, int(jitteredTTL().Seconds())).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		log.Printf("Warning: update subscription usage cache failed for user %s group %s: %v", userID, groupID, err)
		return err
	}
	return nil
}

func (c *billingCache) InvalidateSubscriptionCache(ctx context.Context, userID, groupID string) error {
	key := billingSubKey(userID, groupID)
	return c.rdb.Del(ctx, key).Err()
}
...
func (c *billingCache) GetAPIKeyRateLimit(ctx context.Context, keyID string) (*service.APIKeyRateLimitCacheData, error) {
...
func (c *billingCache) SetAPIKeyRateLimit(ctx context.Context, keyID string, data *service.APIKeyRateLimitCacheData) error {
	if data == nil {
		return nil
	}
	key := billingRateLimitKey(keyID)
...
func (c *billingCache) UpdateAPIKeyRateLimitUsage(ctx context.Context, keyID string, cost float64) error {
	key := billingRateLimitKey(keyID)
	now := time.Now().Unix()
	...
	if err != nil && !errors.Is(err, redis.Nil) {
		log.Printf("Warning: update rate limit usage cache failed for api key %s: %v", keyID, err)
		return err
	}
	return nil
}

func (c *billingCache) InvalidateAPIKeyRateLimit(ctx context.Context, keyID string) error {
	key := billingRateLimitKey(keyID)
	return c.rdb.Del(ctx, key).Err()
}
...
func userPlatformQuotaCacheKey(userID string, platform string) string {
	return fmt.Sprintf("billing:user_platform_quota:%s:%s", userID, platform)
}
...
func (c *billingCache) GetUserPlatformQuotaCache(ctx context.Context, userID string, platform string) (*service.UserPlatformQuotaCacheEntry, bool, error) {
...
func (c *billingCache) SetUserPlatformQuotaCache(ctx context.Context, userID string, platform string, entry *service.UserPlatformQuotaCacheEntry, ttl time.Duration) error {
	if entry == nil {
		return nil
	}
	key := userPlatformQuotaCacheKey(userID, platform)
...
func (c *billingCache) DeleteUserPlatformQuotaCache(ctx context.Context, userID string, platform string) error {
...
func userPlatformQuotaDirtyMember(userID string, platform string) string {
	return userID + ":" + platform
}

func (c *billingCache) IncrUserPlatformQuotaUsageCache(ctx context.Context, userID string, platform string, cost float64, ttl time.Duration, markDirty bool) error {
	member := ""
	if markDirty {
