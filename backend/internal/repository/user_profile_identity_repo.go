...
func (r *userRepository) ListUserAuthIdentities(ctx context.Context, userID string) ([]service.UserAuthIdentityRecord, error) {
...
func (r *userRepository) UnbindUserAuthProvider(ctx context.Context, userID string, provider string) error {
...
func selectOwnedCompatibleIdentity(records []*dbent.AuthIdentity, userID string) *dbent.AuthIdentity {
...
func hasCompatibleIdentityConflict(records []*dbent.AuthIdentity, userID string) bool {
	for _, record := range records {
		if record.UserID != userID {
			return true
		}
...
func selectOwnedCompatibleChannel(records []*dbent.AuthIdentityChannel, userID string) *dbent.AuthIdentityChannel {
...
func hasCompatibleChannelConflict(records []*dbent.AuthIdentityChannel, userID string) bool {
	for _, record := range records {
		if record.Edges.Identity != nil && record.Edges.Identity.UserID != userID {
			return true
		}
...
func (r *userRepository) UpsertIdentityAdoptionDecision(ctx context.Context, input IdentityAdoptionDecisionInput) (*dbent.IdentityAdoptionDecision, error) {
	...
	err := r.WithUserProfileIdentityTx(ctx, func(txCtx context.Context) error {
		...
		if input.IdentityID != nil && strings.TrimSpace(*input.IdentityID) != "" {
			if _, err := client.IdentityAdoptionDecision.Update().
				Where(
...
func identityAdoptionDecisionLockKeys(pendingAuthSessionID string, identityID *string) []string {
	keys := []string{fmt.Sprintf("identity-adoption:pending:%s", pendingAuthSessionID)}
	if identityID != nil && strings.TrimSpace(*identityID) != "" {
		keys = append(keys, fmt.Sprintf("identity-adoption:identity:%s", *identityID))
	}
	return keys
}

func (r *userRepository) GetIdentityAdoptionDecisionByPendingAuthSessionID(ctx context.Context, pendingAuthSessionID string) (*dbent.IdentityAdoptionDecision, error) {
...
func (r *userRepository) UpdateUserLastLoginAt(ctx context.Context, userID string, loginAt time.Time) error {
...
func (r *userRepository) UpdateUserLastActiveAt(ctx context.Context, userID string, activeAt time.Time) error {
	_, err := clientFromContext(ctx, r.client).User.UpdateOneID(userID).
...
func (r *userRepository) GetUserAvatar(ctx context.Context, userID string) (*service.UserAvatar, error) {
...
func (r *userRepository) UpsertUserAvatar(ctx context.Context, userID string, input service.UpsertUserAvatarInput) (*service.UserAvatar, error) {
	exec, err := r.userProfileIdentitySQL(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *userRepository) DeleteUserAvatar(ctx context.Context, userID string) error {
	exec, err := r.userProfileIdentitySQL(ctx)
	if err != nil {
		return err
	}
