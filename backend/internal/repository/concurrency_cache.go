...
func accountSlotKey(accountID string) string {
	return accountSlotKeyPrefix + accountID
}

func userSlotKey(userID string) string {
	return userSlotKeyPrefix + userID
}

func apiKeySlotKey(apiKeyID string) string {
	return apiKeySlotKeyPrefix + apiKeyID
}

func liveAccountSlotKey(accountID string) string {
	return liveAccountSlotKeyPrefix + accountID
}

func liveUserSlotKey(userID string) string {
	return liveUserSlotKeyPrefix + userID
}

func liveAPIKeySlotKey(apiKeyID string) string {
	return liveAPIKeySlotKeyPrefix + apiKeyID
}

func openAIWSIngressLeaseKey(apiKeyID string) string {
	return openAIWSIngressLeaseKeyPrefix + apiKeyID
}

func waitQueueKey(userID string) string {
	return waitQueueKeyPrefix + userID
}

func accountWaitKey(accountID string) string {
	return accountWaitKeyPrefix + accountID
}
...
type slotIndexSpec struct {
	indexKey string
	slotKey  func(string) string
	waitKey  func(string) string
}
...
func (c *concurrencyCache) touchActiveIndexAt(ctx context.Context, indexKey string, id string, expireAt int64) {
	if c == nil || c.rdb == nil || id == "" || expireAt <= 0 {
		return
	}
	if err := c.rdb.ZAdd(ctx, indexKey, redis.Z{
		...
		logger.LegacyPrintf("repository.concurrency", "Warning: touch active index %s for %s failed: %v", indexKey, id, err)
	}
}

func (c *concurrencyCache) refreshAccountActiveIndex(ctx context.Context, accountID string) {
	c.refreshActiveIndex(ctx, accountActiveIndexKey, accountID, accountSlotKey(accountID), accountWaitKey(accountID))
}

func (c *concurrencyCache) refreshUserActiveIndex(ctx context.Context, userID string) {
...
func (c *concurrencyCache) refreshActiveIndex(ctx context.Context, indexKey string, id string, slotKey, waitKey string) {
	if c == nil || c.rdb == nil || id == "" {
		return
	}
	now, err := c.redisUnixSeconds(ctx)
	if err != nil {
		logger.LegacyPrintf("repository.concurrency", "Warning: refresh active index %s for %s failed: %v", indexKey, id, err)
		return
	}
...
func (c *concurrencyCache) readActiveLoadForKey(ctx context.Context, id string, slotKey, waitKey string, now int64) (activeIndexLoad, error) {
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID string, maxConcurrency int, requestID string) (bool, error) {
...
func (c *concurrencyCache) ReleaseAccountSlot(ctx context.Context, accountID string, requestID string) error {
	key := accountSlotKey(accountID)
...
func (c *concurrencyCache) GetAccountConcurrency(ctx context.Context, accountID string) (int, error) {
	key := accountSlotKey(accountID)
...
func (c *concurrencyCache) GetAccountConcurrencyBatch(ctx context.Context, accountIDs []string) (map[string]int, error) {
	if len(accountIDs) == 0 {
	...
	for _, accountID := range accountIDs {
		slotKey := accountSlotKeyPrefix + accountID
		liveKey := liveAccountSlotKeyPrefix + accountID
		pipe.ZRemRangeByScore(ctx, slotKey, "-inf", strconv.FormatInt(cutoffTime, 10))
...
func (c *concurrencyCache) AcquireUserSlot(ctx context.Context, userID string, maxConcurrency int, requestID string) (bool, error) {
...
func (c *concurrencyCache) ReleaseUserSlot(ctx context.Context, userID string, requestID string) error {
	key := userSlotKey(userID)
	if err := c.rdb.ZRem(ctx, key, requestID).Err(); err != nil {
		return err
	}
...
func (c *concurrencyCache) GetUserConcurrency(ctx context.Context, userID string) (int, error) {
	key := userSlotKey(userID)
	// 时间戳在 Lua 脚本内使用 Redis TIME 命令获取
...
func (c *concurrencyCache) TrackAPIKeySlot(ctx context.Context, apiKeyID string, requestID string) error {
	key := apiKeySlotKey(apiKeyID)
	_, err := trackSlotScript.Run(ctx, c.rdb, []string{key}, c.slotTTLSeconds, requestID).Result()
	return err
}

func (c *concurrencyCache) ReleaseAPIKeySlot(ctx context.Context, apiKeyID string, requestID string) error {
	key := apiKeySlotKey(apiKeyID)
	return c.rdb.ZRem(ctx, key, requestID).Err()
}

func (c *concurrencyCache) AcquireOpenAIWSIngressLease(ctx context.Context, apiKeyID string, maxConnections int, leaseID string) (bool, error) {
	if c == nil || c.rdb == nil || apiKeyID == "" || maxConnections <= 0 || leaseID == "" {
		return false, nil
	}
...
func (c *concurrencyCache) RefreshOpenAIWSIngressLease(ctx context.Context, apiKeyID string, leaseID string) (bool, error) {
	if c == nil || c.rdb == nil || apiKeyID == "" || leaseID == "" {
		return false, nil
	}
...
func (c *concurrencyCache) ReleaseOpenAIWSIngressLease(ctx context.Context, apiKeyID string, leaseID string) error {
	if c == nil || c.rdb == nil || apiKeyID == "" || leaseID == "" {
		return nil
	}
...
func (c *concurrencyCache) AcquireLiveLease(
	...
	if c == nil || c.rdb == nil || accountID == "" || userID == "" || apiKeyID == "" || leaseID == "" {
		return false, nil
	}
	replacing := 0
...
func (c *concurrencyCache) RefreshLiveLease(ctx context.Context, accountID, userID, apiKeyID string, leaseID string) (bool, error) {
...
func (c *concurrencyCache) ReleaseLiveLease(ctx context.Context, accountID, userID, apiKeyID string, leaseID string) error {
	if c == nil || c.rdb == nil || leaseID == "" {
		return nil
	}
...
func (c *concurrencyCache) GetAPIKeyConcurrencyBatch(ctx context.Context, apiKeyIDs []string) (map[string]int, error) {
	if len(apiKeyIDs) == 0 {
		return map[string]int{}, nil
	}
	...
	for _, apiKeyID := range apiKeyIDs {
		slotKey := apiKeySlotKeyPrefix + apiKeyID
		liveKey := liveAPIKeySlotKeyPrefix + apiKeyID
		pipe.ZRemRangeByScore(ctx, slotKey, "-inf", strconv.FormatInt(cutoffTime, 10))
		pipe.ZRemRangeByScore(ctx, liveKey, "-inf", strconv.FormatInt(now.Unix()-liveLeaseTTLSeconds, 10))
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID string, maxWait int) (bool, error) {
...
func (c *concurrencyCache) DecrementWaitCount(ctx context.Context, userID string) error {
	key := waitQueueKey(userID)
...
func (c *concurrencyCache) IncrementAccountWaitCount(ctx context.Context, accountID string, maxWait int) (bool, error) {
...
func (c *concurrencyCache) DecrementAccountWaitCount(ctx context.Context, accountID string) error {
	key := accountWaitKey(accountID)
	_, err := decrementWaitScript.Run(ctx, c.rdb, []string{key}).Result()
	if err == nil {
...
func (c *concurrencyCache) GetAccountWaitingCount(ctx context.Context, accountID string) (int, error) {
	key := accountWaitKey(accountID)
...
func (c *concurrencyCache) GetAccountsLoadBatch(ctx context.Context, accounts []service.AccountWithConcurrency) (map[string]*service.AccountLoadInfo, error) {
	if len(accounts) == 0 {
	...
	for _, acc := range accounts {
		slotKey := accountSlotKeyPrefix + acc.ID
		liveKey := liveAccountSlotKeyPrefix + acc.ID
		waitKey := accountWaitKeyPrefix + acc.ID
		pipe.ZRemRangeByScore(ctx, slotKey, "-inf", strconv.FormatInt(cutoffTime, 10))
		pipe.ZRemRangeByScore(ctx, liveKey, "-inf", strconv.FormatInt(now.Unix()-liveLeaseTTLSeconds, 10))
		ac := accountCmds{
			id:             acc.ID,
	...
	loadMap := make(map[string]*service.AccountLoadInfo, len(accounts))
	for _, ac := range cmds {
...
func (c *concurrencyCache) GetUsersLoadBatch(ctx context.Context, users []service.UserWithConcurrency) (map[string]*service.UserLoadInfo, error) {
	if len(users) == 0 {
	...
	for _, u := range users {
		slotKey := userSlotKeyPrefix + u.ID
		liveKey := liveUserSlotKeyPrefix + u.ID
		waitKey := waitQueueKeyPrefix + u.ID
		pipe.ZRemRangeByScore(ctx, slotKey, "-inf", strconv.FormatInt(cutoffTime, 10))
		pipe.ZRemRangeByScore(ctx, liveKey, "-inf", strconv.FormatInt(now.Unix()-liveLeaseTTLSeconds, 10))
		uc := userCmds{
			id:             u.ID,
	...
	loadMap := make(map[string]*service.UserLoadInfo, len(users))
	for _, uc := range cmds {
...
func (c *concurrencyCache) CleanupExpiredAccountSlots(ctx context.Context, accountID string) error {
	key := accountSlotKey(accountID)
