...
VALUES `)
		...
		for i, item := range valid {
			...
			userID, apiKeyID := postgresNilUUID, postgresNilUUID
			if item.UserID != nil {
			...
			if item.APIKeyID != nil {
				if value := strings.TrimSpace(*item.APIKeyID); value != "" {
					apiKeyID = value
				}
			}
...
func (r *opsRepository) ListIngressRejects(ctx context.Context, filter *service.OpsIngressRejectFilter) (*service.OpsIngressRejectList, error) {
	...
	if filter.UserID != nil && strings.TrimSpace(*filter.UserID) != "" && *filter.UserID != postgresNilUUID {
		add("user_id = $%d", *filter.UserID)
	}
	if filter.APIKeyID != nil && strings.TrimSpace(*filter.APIKeyID) != "" && *filter.APIKeyID != postgresNilUUID {
...
FROM ops_ingress_reject_aggregates %s ORDER BY bucket_start DESC,id DESC LIMIT $%d OFFSET $%d`, where, len(args)-1, len(args))
	...
	for rows.Next() {
		...
		if userID.Valid && userID.String != postgresNilUUID {
			item.UserID = &userID.String
		}
		if apiKeyID.Valid && apiKeyID.String != postgresNilUUID {
