...
SELECT date_trunc('minute', ul.created_at), %s, COALESCE(ul.group_id, '00000000-0000-0000-0000-000000000000'::uuid), %s,
...
SELECT date_trunc('minute', ul.created_at), %s, COALESCE(ul.group_id, '00000000-0000-0000-0000-000000000000'::uuid), %s, ul.user_id,
       COUNT(DISTINCT COALESCE(NULLIF(ul.request_id, ''), 'usage:' || ul.id::text))
         FILTER (WHERE COALESCE(ul.request_type, 0) NOT IN (4, 6) AND ` + usageLogSuccessFilterUL + `),
       COALESCE(SUM(ul.input_tokens) FILTER (WHERE ` + usageLogSuccessFilterUL + `), 0),
       COALESCE(SUM(ul.output_tokens) FILTER (WHERE ` + usageLogSuccessFilterUL + `), 0),
...
LEFT JOIN groups g ON g.id = ul.group_id
LEFT JOIN accounts a ON a.id = ul.account_id
CROSS JOIN LATERAL (VALUES ('00000000-0000-0000-0000-000000000000'::uuid), (ul.user_id)) audience(user_id)
...
WITH dedup AS (
  ...
  SELECT DISTINCT ON (COALESCE(NULLIF(request_id, ''), 'error:' || id::text))
    ...
    COALESCE(group_id, '00000000-0000-0000-0000-000000000000'::uuid) AS group_id,
