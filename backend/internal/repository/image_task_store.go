...
func (s *imageTaskStore) CompareAndSetStatus(ctx context.Context, id, expected string, task *service.ImageTaskRecord, ttl time.Duration) error {
	...
	payload, err := json.Marshal(task); if err != nil { return err }
	secs := int64(ttl / time.Second); if secs < 1 { secs = 1 }
	...
	if err != nil { return err }; if result == 0 { return service.ErrImageTaskCanceled }; return nil
}
...
func (q *ImageTaskQueue) Ensure(ctx context.Context) error {
	...
	if err != nil && !strings.Contains(err.Error(), "BUSYGROUP") { return err }; return nil
}

func (q *ImageTaskQueue) Enqueue(ctx context.Context, taskID string) error {
	_, err := q.rdb.XAdd(ctx, &redis.XAddArgs{Stream: imageTaskQueueStream, Values: map[string]any{"task_id": taskID}}).Result(); return err
}
...
func (q *ImageTaskQueue) Read(ctx context.Context, consumer string, block time.Duration) ([]redis.XMessage, error) {
	...
	if err != nil { return nil, err }; if len(entries) == 0 { return nil, nil }; return entries[0].Messages, nil
}
...
func (q *ImageTaskQueue) Ack(ctx context.Context, id string) error { _, err := q.rdb.XAck(ctx, imageTaskQueueStream, imageTaskQueueGroup, id).Result(); return err }
