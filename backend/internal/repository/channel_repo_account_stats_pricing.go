...
func (r *channelRepository) batchLoadAccountStatsPricingRules(ctx context.Context, channelIDs []string) (map[string][]service.AccountStatsPricingRule, error) {
	// 1. 查询规则
	...
	result := make(map[string][]service.AccountStatsPricingRule, len(channelIDs))
...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []string) (map[string][]service.ChannelModelPricing, error) {
	if len(ruleIDs) == 0 {
		return make(map[string][]service.ChannelModelPricing), nil
	}
	...
	pricingMap := make(map[string][]service.ChannelModelPricing, len(ruleIDs))
	for rows.Next() {
...
func (r *channelRepository) loadAccountStatsPricingRules(ctx context.Context, channelID string) ([]service.AccountStatsPricingRule, error) {
	result, err := r.batchLoadAccountStatsPricingRules(ctx, []string{channelID})
	if err != nil {
		return nil, err
	}
...
func replaceAccountStatsPricingRulesTx(ctx context.Context, tx *sql.Tx, channelID string, rules []service.AccountStatsPricingRule) error {
...
func (r *channelRepository) batchLoadAccountStatsIntervals(ctx context.Context, pricingIDs []string) (map[string][]service.PricingInterval, error) {
	if len(pricingIDs) == 0 {
		return nil, nil
	}
	rows, err := r.db.QueryContext(ctx,
	...
	result := make(map[string][]service.PricingInterval)
	for rows.Next() {
