...
import (
	...
	"github.com/AsukaCC/EasySub2api/internal/pkg/proxyutil"
...
func (s *httpUpstreamService) Do(req *http.Request, proxyURL string, accountID string, accountConcurrency int) (*http.Response, error) {
...
func (s *httpUpstreamService) acquireClientWithTLS(proxyURL string, accountID string, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile) (*upstreamClientEntry, error) {
	return s.getClientEntryWithTLS(proxyURL, accountID, accountConcurrency, profile, upstreamProfile, true, true)
}

// getClientEntryWithTLS 获取或创建带 TLS 指纹的客户端条目
// TLS 指纹客户端使用独立的缓存键，与普通客户端隔离
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID string, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
...
// 用于请求路径，避免在获取后被淘汰
func (s *httpUpstreamService) acquireClient(proxyURL string, accountID string, accountConcurrency int) (*upstreamClientEntry, error) {
...
func (s *httpUpstreamService) acquireClientWithProfile(proxyURL string, accountID string, accountConcurrency int, profile service.HTTPUpstreamProfile) (*upstreamClientEntry, error) {
...
func (s *httpUpstreamService) getOrCreateClient(proxyURL string, accountID string, accountConcurrency int) (*upstreamClientEntry, error) {
...
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID string, accountConcurrency int, profile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	// 获取隔离模式
	isolation := s.getIsolationMode()
	// 标准化代理 URL 并解析
	proxyKey, parsedProxy, err := normalizeProxyURL(proxyURL)
...
func buildCacheKey(isolation, proxyKey string, accountID string, protocolMode string) string {
	var base string
	switch isolation {
	case config.ConnectionPoolIsolationAccount:
		base = fmt.Sprintf("account:%s", accountID)
	case config.ConnectionPoolIsolationAccountProxy:
		base = fmt.Sprintf("account:%s|proxy:%s", accountID, proxyKey)
	default:
