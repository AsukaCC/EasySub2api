...
import (
	...
	"github.com/AsukaCC/EasySub2api/ent/authidentity"
...
var _ service.UserDeletionRepository = (*userRepository)(nil)
...
func (r *userRepository) create(ctx context.Context, userIn *service.User, guardEmailAlias bool, domainLimit string) error {
	...
	if err := ensureNormalizedEmailAvailableWithClient(txCtx, txClient, "", userIn.Email); err != nil {
		return err
	}
	...
	initialBalance := userIn.Balance
	if initialBalance > 0 {
		kind := userIn.InitialBalanceKind
		if kind == "" {
	...
	if initialBalance > 0 {
		userIn.Balance = initialBalance
		if userIn.InitialBalanceKind == "" || userIn.InitialBalanceKind == service.WalletKindBonus {
			userIn.BonusBalance = initialBalance
		}
	}
	return nil
}
...
func (r *userRepository) GetByID(ctx context.Context, id string) (*service.User, error) {
	...
	groups, err := r.loadAllowedGroups(ctx, []string{id})
	if err != nil {
		return nil, err
	}
...
func (r *userRepository) GetByIDIncludeDeleted(ctx context.Context, id string) (*service.User, error) {
...
func (r *userRepository) GetByEmail(ctx context.Context, email string) (*service.User, error) {
	...
	out := userEntityToService(m)
	groups, err := r.loadAllowedGroups(ctx, []string{m.ID})
	if err != nil {
		return nil, err
	}
...
func ensureEmailAuthIdentityWithClient(ctx context.Context, client *dbent.Client, userID string, email string, source string) error {
...
func replaceEmailAuthIdentityWithClient(ctx context.Context, client *dbent.Client, userID string, oldEmail, newEmail string, source string) error {
...
func (r *userRepository) deleteUser(ctx context.Context, exec *dbent.Client, id string) error {
...
func (r *userRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	...
	if filters.OnlyDeleted {
		q = q.Where(dbuser.DeletedAtNotNil())
	}
...
// values returned by GetBatchUserUsageStats: successful actual_cost over either
// today or the rolling 30-day window, optionally restricted to one effective
// platform. The aggregate is evaluated before OFFSET/LIMIT, so the result is a
// true global sort rather than a reorder of the current page.
func userUsageOrder(sortBy, sortOrder string) []func(*entsql.Selector) {
	metric := ""
	platform := ""
	switch sortBy {
	case "usage_today":
	...
	case "usage_total":
		metric = "total"
	case "usage_anthropic_today":
		metric, platform = "today", "anthropic"
	case "usage_anthropic_total":
		metric, platform = "total", "anthropic"
	case "usage_openai_today":
		metric, platform = "today", "openai"
	case "usage_openai_total":
		metric, platform = "total", "openai"
	default:
		return nil
	}
	...
	// start is generated by the server, not user input. RFC3339Nano contains no
	// SQL metacharacters and preserves the configured timezone offset.
	startLiteral := start.Format(time.RFC3339Nano)
	direction := "DESC"
	...
	return []func(*entsql.Selector){func(s *entsql.Selector) {
		platformClause := ""
		if platform != "" {
			platformClause = fmt.Sprintf(" AND LOWER(%s) = '%s'", usageLogEffectivePlatformExpr, platform)
		}
		expression := fmt.Sprintf(`(
			...
			WHERE ul.user_id = %s
			  AND ul.created_at >= '%s'::timestamptz
			  AND %s%s
		)`, s.C(dbuser.FieldID), startLiteral, usageLogSuccessFilterUL, platformClause)
		s.OrderExpr(entsql.Expr(expression + " " + direction))
		s.OrderBy(tieOrder(s.C(dbuser.FieldID)))
	}}
}

func (r *userRepository) GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []string) (map[string]*time.Time, error) {
...
func (r *userRepository) GetLatestUsedAtByUserID(ctx context.Context, userID string) (*time.Time, error) {
	latestByUserID, err := r.GetLatestUsedAtByUserIDs(ctx, []string{userID})
	if err != nil {
		return nil, err
	}
...
func (r *userRepository) PermanentlyDeleteUser(ctx context.Context, userID string) (*service.InactiveUserPurgeResult, error) {
	...
	defer func() { _ = tx.Rollback() }()
	opCtx := dbent.NewTxContext(ctx, tx)
	result, err := r.permanentlyDeleteUser(opCtx, tx.Client(), userID)
	if err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
...
func (r *userRepository) RestoreArchivedUser(ctx context.Context, userID string) error {
	if existingTx := dbent.TxFromContext(ctx); existingTx != nil {
		return r.restoreArchivedUser(ctx, existingTx.Client(), userID)
	}
	tx, err := r.client.Tx(ctx)
	if err != nil {
		return err
	...
	defer func() { _ = tx.Rollback() }()
	opCtx := dbent.NewTxContext(ctx, tx)
	if err := r.restoreArchivedUser(opCtx, tx.Client(), userID); err != nil {
		return err
	}
	return tx.Commit()
}

func (r *userRepository) restoreArchivedUser(ctx context.Context, exec *dbent.Client, userID string) error {
	archiveCtx := mixins.SkipSoftDelete(ctx)
	user, err := exec.User.Query().Where(dbuser.IDEQ(userID)).Only(archiveCtx)
	if err != nil {
		return translatePersistenceError(err, service.ErrUserNotFound, nil)
	}
	if user.DeletedAt == nil {
		return service.ErrUserNotArchived
	}
	if _, err := exec.User.UpdateOneID(userID).ClearDeletedAt().Save(archiveCtx); err != nil {
		return translatePersistenceError(err, service.ErrUserNotFound, service.ErrEmailExists)
	}
	if err := ensureEmailAuthIdentityWithClient(ctx, exec, userID, user.Email, "admin_restore"); err != nil {
...
func purgeInactiveUserOwnedRows(ctx context.Context, exec *dbent.Client, userIDs []string) error {
	...
	queries := []string{
		...
		`DELETE FROM payment_refunds WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM support_tickets WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM payment_orders WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM usage_cleanup_tasks WHERE created_by = ANY($1::uuid[])`,
		`DELETE FROM batch_image_jobs WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM channel_monitor_v2_latency_histograms_1m WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM channel_monitor_v2_latency_histograms_rollup WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM channel_monitor_v2_user_metrics_1m WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM channel_monitor_v2_user_metrics_rollup WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM deleted_api_key_audits WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM ops_error_logs WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM ops_ingress_reject_aggregates WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM ops_system_logs WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM orphan_allowed_groups_audit WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM usage_dashboard_daily_users WHERE user_id = ANY($1::uuid[])`,
		`DELETE FROM usage_dashboard_hourly_users WHERE user_id = ANY($1::uuid[])`,
	}
	for _, query := range queries {
		if _, err := exec.ExecContext(ctx, query, arrayArg); err != nil {
			return err
		}
	}
	return nil
...
func (r *userRepository) filterUsersByAttributes(ctx context.Context, attrs map[string]string) ([]string, error) {
	if len(attrs) == 0 {
		return nil, nil
	}
...
func (r *userRepository) DeductAvailableBalance(ctx context.Context, id string, amount float64) (deducted float64, err error) {
	if amount < 0 {
...
func (r *userRepository) UpdateConcurrency(ctx context.Context, id string, amount int) error {
	client := clientFromContext(ctx, r.client)
...
func (r *userRepository) ApplyRedeemConcurrencyAdjustment(ctx context.Context, id string, delta int) error {
	const updateSQL = `
		UPDATE users
...
func (r *userRepository) BatchSetConcurrency(ctx context.Context, userIDs []string, value int) (int, error) {
	if len(userIDs) == 0 {
		return 0, nil
	}
	if value < 0 {
...
func (r *userRepository) BatchAddConcurrency(ctx context.Context, userIDs []string, delta int) (int, error) {
	if len(userIDs) == 0 {
		return 0, nil
	}
	res, err := r.sql.ExecContext(ctx,
...
func (r *userRepository) BatchUpdateLimits(ctx context.Context, userIDs []string, concurrency, rpmLimit *int) (int, error) {
...
func existsByEmailAliasWithClient(ctx context.Context, client *dbent.Client, email string) (bool, error) {
	_, exists, err := emailAliasOwnerIDWithClient(ctx, client, email, "")
	return exists, err
}

// emailAliasOwnerIDWithClient returns the user owning the normalized inbox
// identity. The current user is returned as an existing match only when no
// other account owns the same inbox, which lets callers safely replace their
// own email alias without hiding a historical duplicate.
func emailAliasOwnerIDWithClient(ctx context.Context, client *dbent.Client, email string, currentUserID string) (string, bool, error) {
	if client == nil {
		return "", false, nil
	}
	...
	for _, candidate := range candidates {
		...
		if candidate.ID != "" && candidate.ID != currentUserID {
...
// UpdateEmailWithAliasGuard updates the primary email and password inside the
// caller's transaction. Advisory locks and the final alias recheck close the
// race between the service-layer availability check and the database write.
...
func ensureNormalizedEmailAvailableWithClient(ctx context.Context, client *dbent.Client, userID string, email string) error {
	client = clientFromContext(ctx, client)
	if client == nil {
		return nil
	}
...
func (r *userRepository) AddGroupToAllowedGroups(ctx context.Context, userID string, groupID string) error {
	client := clientFromContext(ctx, r.client)
...
func (r *userRepository) RemoveGroupFromAllowedGroups(ctx context.Context, groupID string) (int64, error) {
...
func (r *userRepository) RemoveGroupFromUserAllowedGroups(ctx context.Context, userID string, groupID string) error {
	client := clientFromContext(ctx, r.client)
...
func (r *userRepository) loadAllowedGroups(ctx context.Context, userIDs []string) (map[string][]string, error) {
...
// 仅操作 user_allowed_groups 联接表，legacy users.allowed_groups 列已弃用。
func (r *userRepository) syncUserAllowedGroupsWithClient(ctx context.Context, client *dbent.Client, userID string, groupIDs []string) error {
	if client == nil {
		return nil
	}
	...
	desired := make(map[string]struct{}, len(groupIDs))
...
func (r *userRepository) UpdateTotpSecret(ctx context.Context, userID string, encryptedSecret *string) error {
	client := clientFromContext(ctx, r.client)
