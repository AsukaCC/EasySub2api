...
import (
	...
	"github.com/AsukaCC/EasySub2api/ent/userplatformquota"
...
type UserPlatformQuotaRepository interface {
	...
	GetByUserPlatform(ctx context.Context, userID string, platform string) (*UserPlatformQuotaRecord, error)
	// ListByUser 查询用户的所有平台配额记录（排除软删除）。
	ListByUser(ctx context.Context, userID string) ([]UserPlatformQuotaRecord, error)
	// IncrementUsageWithReset 原子地累加用量，若窗口已过期则先重置再累加。
	IncrementUsageWithReset(ctx context.Context, userID string, platform string, cost float64, now time.Time) error
	// ResetExpiredWindow 重置指定窗口（daily/weekly/monthly）的用量与起始时间。
	ResetExpiredWindow(ctx context.Context, userID string, platform string, window string, newStart time.Time) error
	// UpsertForUser 全量替换该用户所有平台限额配置（详见 service.UserPlatformQuotaRepository.UpsertForUser）。
	UpsertForUser(ctx context.Context, userID string, records []UserPlatformQuotaRecord) error
...
func (r *userPlatformQuotaRepository) GetByUserPlatform(ctx context.Context, userID string, platform string) (*UserPlatformQuotaRecord, error) {
	client := clientFromContext(ctx, r.client)
...
// ListByUser 查询用户的所有平台配额记录（排除软删除）。
func (r *userPlatformQuotaRepository) ListByUser(ctx context.Context, userID string) ([]UserPlatformQuotaRecord, error) {
	client := clientFromContext(ctx, r.client)
...
func (r *userPlatformQuotaRepository) IncrementUsageWithReset(ctx context.Context, userID string, platform string, cost float64, now time.Time) error {
...
func (r *userPlatformQuotaRepository) ResetExpiredWindow(ctx context.Context, userID string, platform string, window string, newStart time.Time) error {
	client := clientFromContext(ctx, r.client)
...
func (r *userPlatformQuotaRepository) UpsertForUser(ctx context.Context, userID string, records []UserPlatformQuotaRecord) error {
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
...
func softDeleteMissingPlatforms(ctx context.Context, client *dbent.Client, userID string, keepPlatforms []string, now time.Time) error {
	var (
		query string
		args  []any
	)
...
func updateLimitsRow(ctx context.Context, client *dbent.Client, userID string, rec UserPlatformQuotaRecord, now time.Time) (int64, error) {
...
func insertLimitsRow(ctx context.Context, client *dbent.Client, userID string, rec UserPlatformQuotaRecord, now time.Time) error {
