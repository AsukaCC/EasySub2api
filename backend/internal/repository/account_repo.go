...
func (r *accountRepository) Create(ctx context.Context, account *service.Account) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &account.ID, nil, buildSchedulerGroupPayload(account.GroupIDs)); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue account create failed: account=%s err=%v", account.ID, err)
	}
	return nil
}
...
func (r *accountRepository) GetByID(ctx context.Context, id string) (*service.Account, error) {
...
func (r *accountRepository) GetByIDs(ctx context.Context, ids []string) ([]*service.Account, error) {
	if len(ids) == 0 {
	...
	accountIDs := make([]string, 0, len(entAccounts))
	entByID := make(map[string]*dbent.Account, len(entAccounts))
	...
	outByID := make(map[string]*service.Account, len(entAccounts))
...
func (r *accountRepository) ExistsByID(ctx context.Context, id string) (bool, error) {
...
func (r *accountRepository) ListCRSAccountIDs(ctx context.Context) (map[string]string, error) {
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id string, credentials map[string]any) error {
...
func (r *accountRepository) Delete(ctx context.Context, id string) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, buildSchedulerGroupPayload(groupIDs)); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue account delete failed: account=%s err=%v", id, err)
	}
	return nil
}
...
func (r *accountRepository) accountListFilteredQuery(platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) *dbent.AccountQuery {
	...
	if expiryStatus != "" {
		...
		case service.AccountExpiryStatusExpiring:
			q = q.Where(
				...
				dbaccount.ExpiresAtLTE(now.Add(7*24*time.Hour)),
			)
...
func (r *accountRepository) ListAllWithFilters(ctx context.Context, platform, accountType, status, search string, groupID string, privacyMode, expiryStatus string) ([]service.Account, error) {
	accounts, err := r.accountListFilteredQuery(platform, accountType, status, search, groupID, privacyMode, expiryStatus).All(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *accountRepository) ListOpsAccountsForStats(ctx context.Context, platformFilter string, groupIDFilter *string) ([]service.Account, error) {
	...
	if groupIDFilter != nil && *groupIDFilter != "" {
...
func (r *accountRepository) ListByGroup(ctx context.Context, groupID string) ([]service.Account, error) {
...
func (r *accountRepository) ListOAuthRefreshCandidatePage(ctx context.Context, options service.OAuthRefreshPageOptions) (*service.OAuthRefreshCandidatePage, error) {
	...
	afterID := strings.TrimSpace(options.AfterID)
	if afterID == "" {
		afterID = postgresNilUUID
	}
	rows, err := r.sql.QueryContext(ctx, query, pq.Array(options.Platforms), afterID, options.Limit)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	...
	accountsByID := make(map[string]*service.Account, len(accounts))
	for _, account := range accounts {
		if account != nil {
...
func (r *accountRepository) UpdateLastUsed(ctx context.Context, id string) error {
	now := time.Now()
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountLastUsed, &id, nil, payload); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue last used failed: account=%s err=%v", id, err)
	}
	return nil
}

func (r *accountRepository) BatchUpdateLastUsed(ctx context.Context, updates map[string]time.Time) error {
	if len(updates) == 0 {
		return nil
	}
...
func (r *accountRepository) SetError(ctx context.Context, id string, errorMsg string) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue set error failed: account=%s err=%v", id, err)
	}
...
func (r *accountRepository) syncSchedulerAccountSnapshot(ctx context.Context, accountID string) {
	if r == nil || r.schedulerCache == nil || accountID == "" {
		return
	}
	account, err := r.GetByID(ctx, accountID)
	if err != nil {
		logger.LegacyPrintf("repository.account", "[Scheduler] sync account snapshot read failed: id=%s err=%v", accountID, err)
		return
	}
	if err := r.schedulerCache.SetAccount(ctx, account); err != nil {
		logger.LegacyPrintf("repository.account", "[Scheduler] sync account snapshot write failed: id=%s err=%v", accountID, err)
	}
}

func (r *accountRepository) syncSchedulerAccountSnapshotDetached(ctx context.Context, accountID string) {
...
func (r *accountRepository) deleteSchedulerAccountSnapshot(ctx context.Context, accountID string) {
	if r == nil || r.schedulerCache == nil || accountID == "" {
		return
	}
	if err := r.schedulerCache.DeleteAccount(ctx, accountID); err != nil {
		logger.LegacyPrintf("repository.account", "[Scheduler] delete account snapshot failed: id=%s err=%v", accountID, err)
	}
}

func (r *accountRepository) syncSchedulerAccountSnapshots(ctx context.Context, accountIDs []string) {
	...
	for _, account := range accounts {
		...
		if err := r.schedulerCache.SetAccount(ctx, account); err != nil {
			logger.LegacyPrintf("repository.account", "[Scheduler] batch sync account snapshot write failed: id=%s err=%v", account.ID, err)
		}
	}
}

func (r *accountRepository) ClearError(ctx context.Context, id string) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue clear error failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) AddToGroup(ctx context.Context, accountID, groupID string, priority int) error {
	...
	payload := buildSchedulerGroupPayload([]string{groupID})
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountGroupsChanged, &accountID, nil, payload); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue add to group failed: account=%s group=%s err=%v", accountID, groupID, err)
	}
	return nil
}

func (r *accountRepository) RemoveFromGroup(ctx context.Context, accountID, groupID string) error {
	...
	payload := buildSchedulerGroupPayload([]string{groupID})
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountGroupsChanged, &accountID, nil, payload); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue remove from group failed: account=%s group=%s err=%v", accountID, groupID, err)
	}
	return nil
}

func (r *accountRepository) GetGroups(ctx context.Context, accountID string) ([]service.Group, error) {
...
func (r *accountRepository) BindGroups(ctx context.Context, accountID string, groupIDs []string) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountGroupsChanged, &accountID, nil, payload); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue bind groups failed: account=%s err=%v", accountID, err)
	}
	return nil
}
...
func (r *accountRepository) ListSchedulableByGroupID(ctx context.Context, groupID string) ([]service.Account, error) {
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []string) ([]service.GroupAccountCapacityRow, error) {
...
func (r *accountRepository) ListSchedulableByGroupIDAndPlatform(ctx context.Context, groupID string, platform string) ([]service.Account, error) {
...
func (r *accountRepository) ListSchedulableByGroupIDAndPlatforms(ctx context.Context, groupID string, platforms []string) ([]service.Account, error) {
	if len(platforms) == 0 {
		return nil, nil
	}
...
func (r *accountRepository) SetRateLimited(ctx context.Context, id string, resetAt time.Time) error {
	now := time.Now()
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue rate limit failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}
...
func (r *accountRepository) SetRateLimitedIfLater(ctx context.Context, id string, resetAt time.Time) error {
	now := time.Now()
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue extended rate limit failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}
...
func (r *accountRepository) ClearRateLimitIfObserved(ctx context.Context, id string, observedLimitedAt, observedResetAt time.Time) (bool, error) {
	updated, err := r.client.Account.Update().
		Where(
			dbaccount.IDEQ(id),
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue observed rate-limit clear failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return true, nil
}

func (r *accountRepository) SetModelRateLimit(ctx context.Context, id string, scope string, resetAt time.Time, reason ...string) error {
	if scope == "" {
		return nil
	}
	now := time.Now().UTC()
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue model rate limit failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) SetOverloaded(ctx context.Context, id string, until time.Time) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue overload failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) SetTempUnschedulable(ctx context.Context, id string, until time.Time, reason string) error {
	result, err := r.sql.ExecContext(ctx, `
		UPDATE accounts
		SET temp_unschedulable_until = $1,
			temp_unschedulable_reason = $2,
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue temp unschedulable failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}
...
func (r *accountRepository) ClearTempUnschedulable(ctx context.Context, id string) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue clear temp unschedulable failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) ClearRateLimit(ctx context.Context, id string) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
		ClearRateLimitedAt().
		ClearRateLimitResetAt().
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue clear rate limit failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) ClearModelRateLimits(ctx context.Context, id string) error {
	client := clientFromContext(ctx, r.client)
	result, err := client.ExecContext(
		ctx,
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue clear model rate limit failed: account=%s err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}

func (r *accountRepository) UpdateSessionWindow(ctx context.Context, id string, start, end *time.Time, status string) error {
	...
	if start != nil || end != nil {
		if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
			logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue session window update failed: account=%s err=%v", id, err)
		}
	}
	return nil
}

func (r *accountRepository) UpdateSessionWindowEnd(ctx context.Context, id string, end time.Time) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue session window end update failed: account=%s err=%v", id, err)
	}
	return nil
}

func (r *accountRepository) SetSchedulable(ctx context.Context, id string, schedulable bool) error {
	_, err := r.client.Account.Update().
		Where(dbaccount.IDEQ(id)).
		SetSchedulable(schedulable).
		Save(ctx)
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue schedulable change failed: account=%s err=%v", id, err)
	}
	if !schedulable {
		r.syncSchedulerAccountSnapshot(ctx, id)
	}
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []string, updates service.AccountBulkUpdate) (int64, error) {
	if len(ids) == 0 {
		return 0, nil
	}
	...
	if updates.ProxyID != nil {
		// 空 UUID 表示清除代理。
		if strings.TrimSpace(*updates.ProxyID) == "" {
...
func (r *accountRepository) queryAccountsByGroup(ctx context.Context, groupID string, opts accountGroupQueryOptions) ([]service.Account, error) {
	...
	accountMap := make(map[string]*dbent.Account, len(groups))
...
func (r *accountRepository) loadProxies(ctx context.Context, proxyIDs []string) (map[string]*service.Proxy, error) {
	proxyMap := make(map[string]*service.Proxy)
	proxyIDs = uniqueIDs(proxyIDs)
	if len(proxyIDs) == 0 {
		return proxyMap, nil
	}
...
func (r *accountRepository) loadAccountGroups(ctx context.Context, accountIDs []string) (map[string][]*service.Group, map[string][]string, map[string][]service.AccountGroup, error) {
	groupsByAccount := make(map[string][]*service.Group)
	groupIDsByAccount := make(map[string][]string)
	accountGroupsByAccount := make(map[string][]service.AccountGroup)
	...
	for start := 0; start < len(accountIDs); start += postgresParameterBatchSize {
		...
		groupIDs := make([]string, 0, len(entries))
...
func (r *accountRepository) loadGroups(ctx context.Context, groupIDs []string) (map[string]*service.Group, error) {
	groupMap := make(map[string]*service.Group)
	groupIDs = uniqueIDs(groupIDs)
	if len(groupIDs) == 0 {
		return groupMap, nil
	}
...
func (r *accountRepository) loadAccountGroupIDs(ctx context.Context, accountID string) ([]string, error) {
...
func mergeGroupIDs(a []string, b []string) []string {
...
func buildSchedulerGroupPayload(groupIDs []string) any {
	if len(groupIDs) == 0 {
		return nil
	}
...
func (r *accountRepository) IncrementQuotaUsed(ctx context.Context, id string, amount float64) error {
	rows, err := r.sql.QueryContext(ctx,
	...
	if limit > 0 && newUsed >= limit && (newUsed-amount) < limit {
		if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
			logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue quota exceeded failed: account=%s err=%v", id, err)
		}
	}
	return nil
}
...
func (r *accountRepository) ResetQuotaUsed(ctx context.Context, id string) error {
	_, err := r.sql.ExecContext(ctx,
		`UPDATE accounts SET extra = (
			COALESCE(extra, '{}'::jsonb)
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue quota reset failed: account=%s err=%v", id, err)
	}
	return nil
}
...
func (r *accountRepository) RevertProxyFallback(ctx context.Context, accountID string) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &accountID, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] revert fallback enqueue failed: account=%s err=%v", accountID, err)
	}
	return nil
}
...
func (r *accountRepository) ListShadowsByParent(ctx context.Context, parentID string) ([]*service.Account, error) {
