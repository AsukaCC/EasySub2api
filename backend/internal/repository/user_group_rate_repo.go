...
func (r *userGroupRateRepository) GetByUserID(ctx context.Context, userID string) (map[string]float64, error) {
...
func (r *userGroupRateRepository) GetByUserIDs(ctx context.Context, userIDs []string) (map[string]map[string]float64, error) {
	result := make(map[string]map[string]float64, len(userIDs))
	if len(userIDs) == 0 {
		return result, nil
	}
...
func (r *userGroupRateRepository) GetByGroupID(ctx context.Context, groupID string) ([]service.UserGroupRateEntry, error) {
	query := `
...
func (r *userGroupRateRepository) GetByUserAndGroup(ctx context.Context, userID, groupID string) (*float64, error) {
...
func (r *userGroupRateRepository) GetRPMOverrideByUserAndGroup(ctx context.Context, userID, groupID string) (*int, error) {
...
func (r *userGroupRateRepository) SyncUserGroupRates(ctx context.Context, userID string, rates map[string]*float64) error {
	if len(rates) == 0 {
	...
	var clearGroupIDs []string
	upsertGroupIDs := make([]string, 0, len(rates))
	...
	if len(upsertGroupIDs) > 0 {
		...
		_, err := r.sql.ExecContext(ctx, `
			...
			SELECT
				...
				$2::timestamptz,
				$2::timestamptz
			FROM unnest($3::uuid[], $4::double precision[]) AS data(group_id, rate_multiplier)
...
func (r *userGroupRateRepository) SyncGroupRateMultipliers(ctx context.Context, groupID string, entries []service.GroupRateMultiplierInput) error {
	keepUserIDs := make([]string, 0, len(entries))
	...
	_, err := r.sql.ExecContext(ctx, `
		INSERT INTO user_group_rate_multipliers (user_id, group_id, rate_multiplier, created_at, updated_at)
		SELECT data.user_id, $1::uuid, data.rate_multiplier, $2::timestamptz, $2::timestamptz
		FROM unnest($3::uuid[], $4::double precision[]) AS data(user_id, rate_multiplier)
		ON CONFLICT (user_id, group_id)
...
func (r *userGroupRateRepository) SyncGroupRPMOverrides(ctx context.Context, groupID string, entries []service.GroupRPMOverrideInput) error {
	keepUserIDs := make([]string, 0, len(entries))
	var clearUserIDs []string
	upsertUserIDs := make([]string, 0, len(entries))
	...
	if len(upsertUserIDs) > 0 {
		...
		_, err := r.sql.ExecContext(ctx, `
			...
			SELECT data.user_id, $1::uuid, data.rpm_override, $2::timestamptz, $2::timestamptz
			FROM unnest($3::uuid[], $4::integer[]) AS data(user_id, rpm_override)
			ON CONFLICT (user_id, group_id)
...
func (r *userGroupRateRepository) ClearGroupRPMOverrides(ctx context.Context, groupID string) error {
	if _, err := r.sql.ExecContext(ctx, `
		UPDATE user_group_rate_multipliers
		SET rpm_override = NULL, updated_at = NOW()
		WHERE group_id = $1
...
func (r *userGroupRateRepository) DeleteByGroupID(ctx context.Context, groupID string) error {
	_, err := r.sql.ExecContext(ctx, `DELETE FROM user_group_rate_multipliers WHERE group_id = $1`, groupID)
	return err
}

// DeleteByUserID 删除指定用户的所有专属条目
func (r *userGroupRateRepository) DeleteByUserID(ctx context.Context, userID string) error {
