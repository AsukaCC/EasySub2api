...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	...
	if err == nil {
		...
		if err := r.writeGroupIDs(ctx, key.ID, key.GroupIDs); err != nil {
			return err
		}
	}
...
func (r *apiKeyRepository) GetByID(ctx context.Context, id string) (*service.APIKey, error) {
	m, err := r.activeQuery().
...
func (r *apiKeyRepository) GetKeyAndOwnerID(ctx context.Context, id string) (string, string, error) {
	m, err := r.activeQuery().
		Where(apikey.IDEQ(id)).
	...
	m, err := r.activeQuery().
		...
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				...
				user.FieldStatus,
				user.FieldRole,
				user.FieldRechargeBalance,
				user.FieldBonusBalance,
				user.FieldFrozenRechargeBalance,
				user.FieldFrozenBonusBalance,
...
func (r *apiKeyRepository) Delete(ctx context.Context, id string) error {
	...
	tombstoneKey := fmt.Sprintf("__deleted__%s__%d", id, time.Now().UnixNano())
...
func (r *apiKeyRepository) DeleteWithAudit(ctx context.Context, id string) error {
	tombstoneKey := fmt.Sprintf("__deleted__%s__%d", id, time.Now().UnixNano())
...
func (r *apiKeyRepository) deleteWithTombstone(ctx context.Context, exec *dbent.Client, id string, tombstoneKey string) error {
...
func (r *apiKeyRepository) apiKeyListByUserIDQuery(userID string, filters service.APIKeyListFilters) *dbent.APIKeyQuery {
...
func (r *apiKeyRepository) ListByUserID(ctx context.Context, userID string, params pagination.PaginationParams, filters service.APIKeyListFilters) ([]service.APIKey, *pagination.PaginationResult, error) {
...
func (r *apiKeyRepository) ListAllByUserID(ctx context.Context, userID string, filters service.APIKeyListFilters) ([]service.APIKey, error) {
...
func (r *apiKeyRepository) latestUsageLogIPs(ctx context.Context, apiKeyIDs []string) (result map[string]string, err error) {
	...
	out := make(map[string]string, len(apiKeyIDs))
	for rows.Next() {
		var apiKeyID string
...
func latestUsageLogIPsQuery(apiKeyIDs []string, dialectName string) (string, []any) {
	if dialectName == dialect.Postgres {
		...
		FROM unnest($1::uuid[]) AS requested(api_key_id)
		CROSS JOIN LATERAL (
...
func (r *apiKeyRepository) VerifyOwnership(ctx context.Context, userID string, apiKeyIDs []string) ([]string, error) {
	if len(apiKeyIDs) == 0 {
		return []string{}, nil
	}
...
func (r *apiKeyRepository) ListByGroupID(ctx context.Context, groupID string, params pagination.PaginationParams) ([]service.APIKey, *pagination.PaginationResult, error) {
...
func (r *apiKeyRepository) SearchAPIKeys(ctx context.Context, userID string, keyword string, limit int) ([]service.APIKey, error) {
	q := r.activeQuery()
	if userID != "" {
...
func (r *apiKeyRepository) ClearGroupIDByGroupID(ctx context.Context, groupID string) (int64, error) {
	client := clientFromContext(ctx, r.client)
	result, err := client.ExecContext(ctx, `
		...
		SET
			group_ids = COALESCE((
				...
				FROM jsonb_array_elements_text(COALESCE(group_ids, '[]'::jsonb)) WITH ORDINALITY AS items(value, ord)
				WHERE value <> $1
			), '[]'::jsonb),
			group_id = CASE
		...
		WHERE deleted_at IS NULL
			AND (group_id = $1 OR COALESCE(group_ids, '[]'::jsonb) @> jsonb_build_array($1::text))`, groupID)
	if err != nil {
		return 0, err
	}
...
func (r *apiKeyRepository) UpdateGroupIDByUserAndGroup(ctx context.Context, userID, oldGroupID, newGroupID string) (int64, error) {
	client := clientFromContext(ctx, r.client)
	result, err := client.ExecContext(ctx, `
		UPDATE api_keys
		SET
			group_ids = COALESCE((
				SELECT jsonb_agg(to_jsonb(CASE WHEN value = $2 THEN $3 ELSE value END) ORDER BY ord)
				FROM jsonb_array_elements_text(COALESCE(group_ids, '[]'::jsonb)) WITH ORDINALITY AS items(value, ord)
			), '[]'::jsonb),
		...
		WHERE user_id = $1
			...
			AND (group_id = $2 OR COALESCE(group_ids, '[]'::jsonb) @> jsonb_build_array($2::text))`, userID, oldGroupID, newGroupID)
	if err != nil {
		return 0, err
	}
	n, err := result.RowsAffected()
...
func (r *apiKeyRepository) CountByGroupID(ctx context.Context, groupID string) (int64, error) {
	count, err := r.activeQuery().Where(apiKeyGroupMembershipPredicate(groupID)).Count(ctx)
	return int64(count), err
}

func (r *apiKeyRepository) ListKeysByUserID(ctx context.Context, userID string) ([]string, error) {
...
func (r *apiKeyRepository) ListKeysByGroupID(ctx context.Context, groupID string) ([]string, error) {
	keys, err := r.activeQuery().
		Where(apiKeyGroupMembershipPredicate(groupID)).
		Select(apikey.FieldKey).
		Strings(ctx)
	if err != nil {
		return nil, err
...
// apiKeyGroupMembershipPredicate keeps legacy group_id and the multi-group
// JSON column compatible during rolling upgrades and for old records.
func apiKeyGroupMembershipPredicate(groupID string) predicate.APIKey {
	return predicate.APIKey(func(s *entsql.Selector) {
		s.Where(entsql.P(func(b *entsql.Builder) {
			b.Ident(s.C("group_id")).WriteString(" = ").Arg(groupID).
				WriteString(" OR ").Ident(s.C("group_ids")).WriteString(" @> jsonb_build_array(").Arg(groupID).
				WriteString("::text)")
		}))
	})
}
...
func (r *apiKeyRepository) IncrementQuotaUsed(ctx context.Context, id string, amount float64) (float64, error) {
...
func (r *apiKeyRepository) IncrementQuotaUsedAndGetState(ctx context.Context, id string, amount float64) (*service.APIKeyQuotaUsageState, error) {
	query := `
		UPDATE api_keys
		SET
...
func (r *apiKeyRepository) IncrementRateLimitUsage(ctx context.Context, id string, cost float64) error {
...
func (r *apiKeyRepository) ResetRateLimitWindows(ctx context.Context, id string) error {
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys SET
...
func (r *apiKeyRepository) GetRateLimitData(ctx context.Context, id string) (result *service.APIKeyRateLimitData, err error) {
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	if m.GroupID != nil {
		out.GroupIDs = []string{*m.GroupID}
	}
	if m.Edges.User != nil {
		out.User = userEntityToService(m.Edges.User)
		if allowed := m.Edges.User.Edges.AllowedGroups; len(allowed) > 0 {
			out.User.AllowedGroups = make([]string, 0, len(allowed))
...
// writeGroupIDs is kept outside the generated Ent schema so rolling upgrades
// can deploy the application before Ent code generation is run everywhere.
func (r *apiKeyRepository) writeGroupIDs(ctx context.Context, keyID string, groupIDs []string) error {
	if r.sql == nil {
		return nil
	}
	ids := service.NormalizeAPIKeyGroupIDs(groupIDs)
	raw, err := json.Marshal(ids)
	if err != nil {
		return err
	}
	_, err = r.sql.ExecContext(ctx, `UPDATE api_keys SET group_ids = $1::jsonb WHERE id = $2`, raw, keyID)
	return err
}

func (r *apiKeyRepository) loadGroupIDs(ctx context.Context, keyID string, fallback *string) ([]string, error) {
	if r.sql == nil {
		if fallback == nil {
			return []string{}, nil
		}
	...
	rows, err := r.sql.QueryContext(ctx, `SELECT COALESCE(group_ids, '[]'::jsonb) FROM api_keys WHERE id = $1`, keyID)
	if err != nil {
		return nil, err
	}
	...
	ids = service.NormalizeAPIKeyGroupIDs(ids)
	if len(ids) == 0 && fallback != nil {
		ids = []string{*fallback}
	}
	return ids, nil
}
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		Balance:                    service.NewWalletSummary(u.RechargeBalance, u.BonusBalance, u.FrozenRechargeBalance, u.FrozenBonusBalance).AvailableBalance,
		BonusBalance:               u.BonusBalance,
		FrozenRechargeBalance:      u.FrozenRechargeBalance,
		FrozenBonusBalance:         u.FrozenBonusBalance,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		LevelRateMultipliers:            g.LevelRateMultipliers,
		DynamicRateRules:                g.DynamicRateRules,
		...
		CcsCodexWSEnabled:               g.CcsCodexWsEnabled,
