...
func (a *userPlatformQuotaServiceAdapter) GetByUserPlatform(ctx context.Context, userID string, platform string) (*service.UserPlatformQuotaRecord, error) {
...
func (a *userPlatformQuotaServiceAdapter) IncrementUsageWithReset(ctx context.Context, userID string, platform string, cost float64, now time.Time) error {
	return a.inner.IncrementUsageWithReset(ctx, userID, platform, cost, now)
}

// ListByUser 查询用户的所有平台配额记录。
func (a *userPlatformQuotaServiceAdapter) ListByUser(ctx context.Context, userID string) ([]service.UserPlatformQuotaRecord, error) {
...
func (a *userPlatformQuotaServiceAdapter) UpsertForUser(ctx context.Context, userID string, records []service.UserPlatformQuotaRecord) error {
...
func (a *userPlatformQuotaServiceAdapter) ResetExpiredWindow(ctx context.Context, userID string, platform string, window string, newStart time.Time) error {
...
func (a *genericUserPlatformQuotaAdapter) GetByUserPlatform(ctx context.Context, userID string, platform string) (*service.UserPlatformQuotaRecord, error) {
	rec, err := a.inner.GetByUserPlatform(ctx, userID, platform)
	if err != nil || rec == nil {
		return nil, err
	}
...
func (a *genericUserPlatformQuotaAdapter) IncrementUsageWithReset(ctx context.Context, userID string, platform string, cost float64, now time.Time) error {
	return a.inner.IncrementUsageWithReset(ctx, userID, platform, cost, now)
}

// ListByUser 查询用户的所有平台配额记录（通用 adapter 实现）。
func (a *genericUserPlatformQuotaAdapter) ListByUser(ctx context.Context, userID string) ([]service.UserPlatformQuotaRecord, error) {
	rows, err := a.inner.ListByUser(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (a *genericUserPlatformQuotaAdapter) UpsertForUser(ctx context.Context, userID string, records []service.UserPlatformQuotaRecord) error {
	repoRecords := toRepoRecords(records)
	return a.inner.UpsertForUser(ctx, userID, repoRecords)
}

// ResetExpiredWindow 转发至 repository.ResetExpiredWindow（通用 adapter），并包装 sentinel。
func (a *genericUserPlatformQuotaAdapter) ResetExpiredWindow(ctx context.Context, userID string, platform string, window string, newStart time.Time) error {
	err := a.inner.ResetExpiredWindow(ctx, userID, platform, window, newStart)
	if errors.Is(err, ErrUserPlatformQuotaNotFound) {
		return fmt.Errorf("%w: %w", service.ErrUserPlatformQuotaNotFound, err)
	}
