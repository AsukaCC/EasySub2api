...
func umqLockKey(accountID string) string {
	// 格式: umq:{123}:lock — 花括号确保 Redis Cluster hash tag 生效
	return umqKeyPrefix + "{" + accountID + "}" + umqLockSuffix
}

func umqLastKey(accountID string) string {
	// 格式: umq:{123}:last — 与 lockKey 同一 hash slot
	return umqKeyPrefix + "{" + accountID + "}" + umqLastSuffix
}

// AcquireLock 尝试获取账号级串行锁
// 无论成功与否都尽力写入锁索引：成功时登记自己的锁，失败时回填观测到的持有者锁，
// 保证任何被争用的锁都能被后台 reconcile 发现，无需扫描所有锁 key。
func (c *userMsgQueueCache) AcquireLock(ctx context.Context, accountID string, requestID string, lockTtlMs int) (bool, error) {
	...
	if expireAtMs > 0 {
		if err := c.rdb.ZAdd(ctx, umqLockIndexKey, redis.Z{
			...
			logger.LegacyPrintf("repository.umq", "Warning: update lock index for account %s failed: %v", accountID, err)
		}
	}
...
func (c *userMsgQueueCache) ReleaseLock(ctx context.Context, accountID string, requestID string) (bool, error) {
	...
	if result == 1 {
		...
		if err := c.rdb.ZRem(ctx, umqLockIndexKey, accountID).Err(); err != nil {
			logger.LegacyPrintf("repository.umq", "Warning: remove lock index for account %s failed: %v", accountID, err)
		}
	}
	return result == 1, nil
}
...
func (c *userMsgQueueCache) GetLastCompletedMs(ctx context.Context, accountID string) (int64, error) {
...
func (c *userMsgQueueCache) ReconcileExpiredLockCandidates(ctx context.Context, maxCount int) (int, error) {
	...
	for _, member := range members {
		accountID := strings.TrimSpace(member)
		if accountID == "" {
