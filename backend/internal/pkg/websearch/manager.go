...
type ProviderConfig struct {
	...
	ProxyID      string `json:"-"`                       // resolved proxy ID for unavailability tracking
...
const (
	...
	proxyUnavailableKey = "websearch:proxy_unavailable:%s"
...
func (m *Manager) filterAvailableProviders(ctx context.Context, accountProxyURL string) []ProviderConfig {
	...
	for _, cfg := range m.configs {
		...
		if proxyID != "" && !m.isProxyAvailable(ctx, proxyID) {
...
func (m *Manager) isProxyAvailable(ctx context.Context, proxyID string) bool {
	if m.redis == nil || proxyID == "" {
		return true
	}
	key := fmt.Sprintf(proxyUnavailableKey, proxyID)
...
func resolveProxyID(cfg ProviderConfig, accountProxyURL string) string {
	if accountProxyURL != "" {
		return "" // account proxy has no ID in provider config
	}
	return cfg.ProxyID
}
