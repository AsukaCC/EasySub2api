...
func Do(client *http.Client, req *http.Request) (*http.Response, error) {
	...
	if req == nil || !Active(req.Context()) {
		return client.Do(req) // #nosec G704 -- callers construct and validate the upstream request.
	}
	startedAt := time.Now()
	response, err := client.Do(req) // #nosec G704 -- callers construct and validate the upstream request.
