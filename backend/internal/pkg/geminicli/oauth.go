...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// SECURITY: This repo does not embed built-in OAuth credentials.
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		if clientID == "" || secret == "" {
			return OAuthConfig{}, infraerrors.Newf(http.StatusBadRequest, "GEMINI_CLI_OAUTH_CLIENT_MISSING", "built-in Gemini CLI OAuth client is not configured; set %s and %s or provide a custom OAuth client", GeminiCLIOAuthClientIDEnv, GeminiCLIOAuthClientSecretEnv)
		}
