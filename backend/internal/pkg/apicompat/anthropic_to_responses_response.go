...
func anthToResHandleContentBlockStart(evt *AnthropicStreamEvent, state *AnthropicEventToResponsesState) []ResponsesStreamEvent {
	...
	case "thinking":
		// A text block deliberately leaves its message item open for later text
		// parts. Close it before switching item types so its accumulated output is
		// not replaced by the reasoning item.
		events = append(events, closeCurrentResponsesItem(state)...)
