...
func (p *darwinProvider) Generate(ctx context.Context) (string, error) {
	...
	command.Env = []string{
		"PATH=/usr/bin:/bin",
		"EASYSUB2API_DEVICECHECK_MODULE=" + modulePath,
		"EASYSUB2API_ATTESTATION_BUNDLE_ID=" + bundleID,
		"EASYSUB2API_ATTESTATION_SIGNALS=" + string(signalsJSON),
	}
...
const addon = require(process.env.EASYSUB2API_DEVICECHECK_MODULE);
const signals = JSON.parse(process.env.EASYSUB2API_ATTESTATION_SIGNALS);
const bundleID = process.env.EASYSUB2API_ATTESTATION_BUNDLE_ID;
