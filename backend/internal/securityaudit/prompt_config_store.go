...
func (m *ConfigManager) Save(ctx context.Context, req UpdateConfigRequest, actorID string) (PublicConfig, error) {
	...
	if m.redis != nil {
		...
		// Compatibility release: older nodes still subscribe to the legacy channel.
		_ = m.redis.Publish(ctx, LegacyConfigInvalidationChannel, strconv.FormatInt(next.ConfigVersion, 10)).Err()
	}
	return PublicFromStorage(next, active.RiskControlEnabled, active.InvalidTokenEndpointIDs()), nil
}

func (m *ConfigManager) buildNextStorage(current storageConfig, req UpdateConfigRequest, actorID string) (storageConfig, error) {
	...
	next := storageConfig{
		...
		AllGroups: req.AllGroups, GroupIDs: append([]string(nil), req.GroupIDs...),
...
func (m *ConfigManager) subscribeLoop(ctx context.Context) {
	defer m.wg.Done()
	pubsub := m.redis.Subscribe(ctx, ConfigInvalidationChannel, LegacyConfigInvalidationChannel)
...
func cloneStorageConfig(cfg storageConfig) storageConfig {
	...
	cfg.GroupIDs = append([]string(nil), cfg.GroupIDs...)
