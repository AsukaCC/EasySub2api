...
type PromptAdminService interface {
	...
	SaveConfig(context.Context, UpdateConfigRequest, string) (PublicConfig, error)
	...
	DeleteEvent(context.Context, string) (*DeleteResult, error)
	DeleteEventsByIDs(context.Context, []string) (*DeleteResult, error)
	PreviewDelete(context.Context, EventFilter, string) (*DeletePreview, error)
	DeleteByFilter(context.Context, DeleteByFilterRequest, string) (*DeleteResult, error)
}
...
func eventFilterFromQuery(c *gin.Context) (EventFilter, error) {
	groupID, err := optionalUUIDQuery(c, "group_id")
	if err != nil {
		return EventFilter{}, err
	}
	userID, err := optionalUUIDQuery(c, "user_id")
	if err != nil {
		return EventFilter{}, err
	}
	apiKeyID, err := optionalUUIDQuery(c, "api_key_id")
	if err != nil {
		return EventFilter{}, err
	}
	filter := EventFilter{
...
func optionalUUIDQuery(c *gin.Context, key string) (*string, error) {
