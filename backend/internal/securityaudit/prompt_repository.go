...
type JobRepository interface {
	...
	PublishQueued(ctx context.Context, jobID string) error
	MarkStagingFailed(ctx context.Context, jobID string, code, message string) error
	ClaimNextJob(ctx context.Context, now time.Time) (*Job, bool, error)
	RefreshLease(ctx context.Context, jobID string, claimVersion int64, now time.Time) error
	Complete(ctx context.Context, job *Job, result *NormalizedResult, storePassEvents bool) (*Event, error)
	Retry(ctx context.Context, jobID string, claimVersion int64, next time.Time, code, message string) error
	Fail(ctx context.Context, jobID string, claimVersion int64, code, message string) error
...
func (r *PostgreSQLRepository) PublishQueued(ctx context.Context, jobID string) error {
...
func (r *PostgreSQLRepository) MarkStagingFailed(ctx context.Context, jobID string, code, _ string) error {
...
func (r *PostgreSQLRepository) RefreshLease(ctx context.Context, jobID string, claimVersion int64, now time.Time) error {
	result, err := r.db.ExecContext(ctx, `
...
func (r *PostgreSQLRepository) Retry(ctx context.Context, jobID string, claimVersion int64, next time.Time, code, _ string) error {
	code, message := sanitizeStoredError(code)
	result, err := r.db.ExecContext(ctx, `
...
func (r *PostgreSQLRepository) Fail(ctx context.Context, jobID string, claimVersion int64, code, _ string) error {
	code, message := sanitizeStoredError(code)
	result, err := r.db.ExecContext(ctx, `
...
func insertEvent(ctx context.Context, queryer sqlQueryer, jobID string, snapshot PromptSnapshot, configVersion int64, result *NormalizedResult) (*Event, error) {
...
func scanJob(row rowScanner) (*Job, error) {
	...
	job.Snapshot.UserID = nullableIDValue(userID)
	job.Snapshot.APIKeyID = nullableIDValue(apiKeyID)
	job.Snapshot.GroupID = nullableIDPtr(groupID)
...
func nullableIDValue(value sql.NullString) string {
	if !value.Valid {
		return ""
	}
	return value.String
...
func nullableIDPtr(value sql.NullString) *string {
	if !value.Valid {
		return nil
	}
	result := value.String
