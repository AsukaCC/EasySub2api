...
type ProxyFallbackConfig struct {
	...
	//   - GitHub Release 更新检查
	//   - 定价数据拉取
	// 不影响 AI 账号网关连接（Anthropic/OpenAI/Grok），
...
type GatewayOpenAIWSConfig struct {
	...
	// 连接池参数
	// MaxTotalConns: 当前实例 OpenAI WS 连接池的全局连接上限
...
func configureConfigSource(setConfigFile, addConfigPath func(string)) {
	...
	addConfigPath("/etc/easysub2api")
	addConfigPath("/etc/sub2api") // Deprecated compatibility path for one release.
}
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "easysub2api")
	...
	viper.SetDefault("webauthn.rp_display_name", "EasySub2api")
	...
	viper.SetDefault("database.dbname", "easysub2api")
	...
	viper.SetDefault("dashboard_cache.key_prefix", "easysub2api:")
	...
	viper.SetDefault("gateway.openai_ws.max_total_conns", 512)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIWS.MaxTotalConns <= 0 {
		return fmt.Errorf("gateway.openai_ws.max_total_conns must be positive")
	}
