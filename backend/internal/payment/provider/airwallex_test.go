...
func TestAirwallexRefundUsesStableRequestID(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/authentication/login":
			_, _ = w.Write([]byte(`{"token":"token-1","expires_at":"2099-01-01T00:00:00Z"}`))
		case "/api/v1/pa/refunds/create":
			require.NoError(t, json.NewDecoder(r.Body).Decode(&refundRequest))
			_, _ = w.Write([]byte(`{"id":"ref_123","payment_intent_id":"int_123","amount":12.34,"currency":"CNY","status":"SETTLED"}`))
		default:
			http.NotFound(w, r)
		}
	...
	prov := mustTestAirwallexProvider(t, server)
	_, err := prov.Refund(context.Background(), payment.RefundRequest{
		TradeNo: "int_123", RequestID: "refund-ticket-1", Amount: "12.34",
	})
	require.NoError(t, err)
	require.Equal(t, airwallexDeterministicRequestID("refund", "int_123", "refund-ticket-1"), refundRequest.RequestID)
}

func TestAirwallexQueryRefundWithoutResponseIDReplaysStableRequest(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/pa/refunds/create":
			require.NoError(t, json.NewDecoder(r.Body).Decode(&refundRequest))
			_, _ = w.Write([]byte(`{"id":"ref_recovered","payment_intent_id":"int_123","amount":12.34,"currency":"CNY","status":"SETTLED"}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	provider := mustTestAirwallexProvider(t, server)
	response, err := provider.QueryRefund(context.Background(), payment.RefundQueryRequest{
		TradeNo: "int_123", OrderID: "sub2_order", RequestID: "refund-ticket-1", Amount: "12.34",
	})
	require.NoError(t, err)
	require.Equal(t, "ref_recovered", response.RefundID)
	require.Equal(t, airwallexDeterministicRequestID("refund", "int_123", "refund-ticket-1"), refundRequest.RequestID)
}
