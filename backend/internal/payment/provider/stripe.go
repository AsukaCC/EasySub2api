...
func (s *Stripe) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	params.SetIdempotencyKey(stripeRefundIdempotencyKey(req, amountInMinorUnit))
	params.Context = ctx
...
func stripeRefundIdempotencyKey(req payment.RefundRequest, amountInMinorUnit int64) string {
	if requestID := strings.TrimSpace(req.RequestID); requestID != "" {
		return "re-" + requestID
	}
	return fmt.Sprintf("re-%s-%d", req.OrderID, amountInMinorUnit)
}

// QueryRefund retrieves a Stripe refund by refund ID when available. If the
// create response was lost before its refund ID was persisted, replay the
// original create with the same idempotency key. Listing the latest refund for
// a PaymentIntent is ambiguous once an order has multiple partial refunds.
func (s *Stripe) QueryRefund(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
	...
	if strings.TrimSpace(req.TradeNo) == "" || strings.TrimSpace(req.RequestID) == "" {
		return nil, fmt.Errorf("stripe query refund: missing payment intent or provider request id")
	}
	return s.Refund(ctx, payment.RefundRequest{
		TradeNo: req.TradeNo, OrderID: req.OrderID, RequestID: req.RequestID,
