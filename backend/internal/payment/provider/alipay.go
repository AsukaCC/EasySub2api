...
func (a *Alipay) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	refundID := alipayRefundRequestID(req)
...
// QueryRefund queries the exact out_request_no used for TradeRefund. If the
// create response was lost and Alipay has no record yet, replaying TradeRefund
// with the same out_request_no is idempotent.
func (a *Alipay) QueryRefund(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
	...
	if requestID == "" {
		requestID = strings.TrimSpace(req.RefundID)
	}
	if requestID == "" {
		return nil, fmt.Errorf("alipay query refund: missing provider request id")
	}
	result, err := alipayTradeFastPayRefundQuery(ctx, client, alipay.TradeFastPayRefundQuery{
		OutTradeNo: req.OrderID, OutRequestNo: requestID,
	})
	if err == nil && result != nil && strings.EqualFold(strings.TrimSpace(result.RefundStatus), "REFUND_SUCCESS") {
		return &payment.RefundResponse{RefundID: requestID, Status: payment.ProviderStatusSuccess}, nil
	}
	if err != nil && !isTradeNotExist(err) {
		return nil, fmt.Errorf("alipay TradeFastPayRefundQuery: %w", err)
	}
	response, replayErr := a.Refund(ctx, payment.RefundRequest{
		TradeNo: req.TradeNo, OrderID: req.OrderID, RequestID: requestID,
		Amount: req.Amount, Reason: "refund status recovery",
	})
	if response != nil {
		return response, nil
	}
	if replayErr != nil {
		return nil, fmt.Errorf("alipay recover refund by out_request_no: %w", replayErr)
	}
	return &payment.RefundResponse{RefundID: requestID, Status: payment.ProviderStatusPending}, nil
}
