...
func TestWxpayRefundIDUsesStableRequestID(t *testing.T) {
	t.Parallel()
	require.Equal(t, "refund-ticket-1", wxpayRefundID("sub2_order", "12.34", "refund-ticket-1"))
	require.Equal(t, "sub2_order-refund-1234", wxpayRefundID("sub2_order", "12.34", ""))
}

func TestWxpayRefundQueryIDFallsBackToStableRequestID(t *testing.T) {
	t.Parallel()
	req := payment.RefundQueryRequest{
		OrderID: "sub2_order", Amount: "12.34", RequestID: "refund-ticket-1",
	}
	require.Equal(t, "refund-ticket-1", wxpayRefundQueryID(req))
	req.RefundID = "provider-refund-id"
	require.Equal(t, "provider-refund-id", wxpayRefundQueryID(req))
}

func TestWxpayRefundProviderStatus(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, payment.ProviderStatusFailed, wxpayRefundProviderStatus(refunddomestic.STATUS_ABNORMAL))
...
func TestCreatePaymentMobileH5IncludesConfiguredSceneInfo(t *testing.T) {
	...
	wxpayH5Prepay = func(ctx context.Context, svc h5.H5ApiService, req h5.PrepayRequest) (*h5.PrepayResponse, *core.APIResult, error) {
		...
		if got := wxSV(req.SceneInfo.H5Info.AppName); got != "EasySub2api" {
			t.Fatalf("scene_info.h5_info.app_name = %q, want %q", got, "EasySub2api")
		}
