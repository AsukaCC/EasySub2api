...
func TestAlipayRefundRequestIDIsStableAcrossRetries(t *testing.T) {
	t.Parallel()
	req := payment.RefundRequest{OrderID: "sub2_order", RequestID: "refund-ticket-1", Amount: "12.34"}
	require.Equal(t, "refund-ticket-1", alipayRefundRequestID(req))
	require.Equal(t, alipayRefundRequestID(req), alipayRefundRequestID(req))
}

func TestAlipayQueryRefundUsesStableProviderRequestID(t *testing.T) {
	...
	response, err := provider.QueryRefund(context.Background(), payment.RefundQueryRequest{
		OrderID: "sub2_order", RequestID: "refund-ticket-1", Amount: "12.34",
	})
	require.NoError(t, err)
	require.Equal(t, "sub2_order", captured.OutTradeNo)
	require.Equal(t, "refund-ticket-1", captured.OutRequestNo)
	require.Equal(t, "refund-ticket-1", response.RefundID)
