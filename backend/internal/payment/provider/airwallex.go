...
func (a *Airwallex) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	payload := airwallexCreateRefundRequest{
		RequestID:       airwallexRefundRequestID(intentID, req),
...
func airwallexRefundRequestID(intentID string, req payment.RefundRequest) string {
	requestIdentity := strings.TrimSpace(req.RequestID)
	if requestIdentity == "" {
		requestIdentity = req.Amount
	}
	return airwallexDeterministicRequestID("refund", intentID, requestIdentity)
}
...
func (a *Airwallex) QueryRefund(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
	...
	if refundID == "" {
		// Airwallex create is idempotent by request_id. Replaying is the only
		// reliable recovery when the create response (and refund ID) was lost.
		response, err := a.Refund(ctx, payment.RefundRequest{
		...
		if err != nil {
			return nil, fmt.Errorf("airwallex recover refund by request id: %w", err)
		}
