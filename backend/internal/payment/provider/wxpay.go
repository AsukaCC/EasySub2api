...
func (w *Wxpay) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	cur := wxpayCurrency
	outRefundNo := wxpayRefundID(req.OrderID, req.Amount, req.RequestID)
	...
	if res.Status != nil {
		st = wxpayRefundProviderStatus(*res.Status)
	}
...
func (w *Wxpay) QueryRefund(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
	...
	outRefundNo := wxpayRefundQueryID(req)
	if outRefundNo == "" {
	...
	if err != nil {
		// Replaying the same out_refund_no is idempotent in WeChat Pay and
		// recovers a create response that was lost before persistence.
		replayed, replayErr := w.Refund(ctx, payment.RefundRequest{
		...
		if replayErr != nil {
			return nil, fmt.Errorf("wxpay query refund: %w; stable replay: %v", err, replayErr)
		}
...
func wxpayRefundQueryID(req payment.RefundQueryRequest) string {
	if refundID := strings.TrimSpace(req.RefundID); refundID != "" {
		return refundID
	}
	return wxpayRefundID(req.OrderID, req.Amount, req.RequestID)
}

func wxpayRefundID(orderID, amount, requestID string) string {
