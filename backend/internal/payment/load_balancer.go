...
type LoadBalancer interface {
	GetInstanceConfig(ctx context.Context, instanceID string) (map[string]string, error)
...
type instanceCandidate struct {
	...
	dailyUsed float64 // captured gateway amount plus pending recharge reservations
}
...
func (lb *DefaultLoadBalancer) SelectInstance(
	...
	candidates := lb.attachDailyUsage(ctx, instances, paymentType)
...
// attachDailyUsage queries captured CNY gateway usage and pending reservations in one pass.
func (lb *DefaultLoadBalancer) attachDailyUsage(
	...
	visibleMethod := normalizeVisibleMethodSupportType(paymentType)
	storedMethods := []string{string(visibleMethod)}
	if alias := legacyVisibleMethodAlias(visibleMethod); alias != "" {
		storedMethods = append(storedMethods, string(alias))
	}
	...
	err := lb.db.PaymentOrder.Query().
		Where(
			...
			paymentorder.OrderTypeEQ(OrderTypeBalance),
			paymentorder.PaymentTypeIn(storedMethods...),
			paymentorder.Or(
				paymentorder.And(
...
func filterByLimits(candidates []instanceCandidate, paymentType PaymentType, orderAmount float64) []instanceCandidate {
	...
	orderAmountDecimal := decimal.NewFromFloat(orderAmount).Round(2)
	for _, c := range candidates {
		cl := getInstanceChannelLimits(c.inst, paymentType)

		if cl.SingleMin > 0 && orderAmountDecimal.LessThan(decimal.NewFromFloat(cl.SingleMin).Round(2)) {
			slog.Info("order below instance single min, skipping",
				"instance_id", c.inst.ID, "order", orderAmount, "min", cl.SingleMin)
			continue
		}
		if cl.SingleMax > 0 && orderAmountDecimal.GreaterThan(decimal.NewFromFloat(cl.SingleMax).Round(2)) {
			slog.Info("order above instance single max, skipping",
				"instance_id", c.inst.ID, "order", orderAmount, "max", cl.SingleMax)
			continue
		}
		dailyTotal := decimal.NewFromFloat(c.dailyUsed).Round(2).Add(orderAmountDecimal)
		if cl.DailyLimit > 0 && dailyTotal.GreaterThan(decimal.NewFromFloat(cl.DailyLimit).Round(2)) {
...
func (lb *DefaultLoadBalancer) buildSelection(selected *dbent.PaymentProviderInstance) (*InstanceSelection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decrypt instance %s config: %w", selected.ID, err)
	}
	if config == nil {
...
// GetInstanceDailyAmount returns today's captured recharge gateway amount for an instance.
...
func (lb *DefaultLoadBalancer) GetInstanceConfig(ctx context.Context, instanceID string) (map[string]string, error) {
