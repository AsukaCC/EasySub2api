...
func main() {
	...
	for _, report := range reports {
		...
		for _, v := range report.Verdicts {
			...
			fmt.Printf("  账号 %-36s %-24s 平台=%-12s U=%-8s 来源=%-19s %s%s\n",
...
func parsePreviewInputs(raw []byte, assumeEnabled bool) ([]service.ProfitPreviewGroupInput, error) {
	...
	for i, entry := range doc.Groups {
		if strings.TrimSpace(entry.Group.ID) == "" || strings.TrimSpace(entry.Group.Platform) == "" {
		...
		overrides := make(map[string]float64, len(entry.UserOverrides))
