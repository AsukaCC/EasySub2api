...
--
-- Follows 273_dynamic_discount_coefficient.sql in this customized repository.
-- The platform checks remain supersets of migration 272.
...
-- Preserve configured platforms and leave the new provider explicitly disabled.
UPDATE channel_monitor_v2_config
SET platforms = platforms || '[{"platform":"opencode_go","enabled":false,"models":[]}]'::jsonb,
    version = version + 1,
    updated_at = NOW()
WHERE id = 1
  AND NOT EXISTS (
      SELECT 1 FROM jsonb_array_elements(platforms) AS entry
      WHERE entry ->> 'platform' = 'opencode_go'
  );
