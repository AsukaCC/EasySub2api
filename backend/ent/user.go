...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldID, user.FieldEmail, user.FieldPasswordHash, user.FieldRole, user.FieldStatus, user.FieldUsername, user.FieldNotes, user.FieldTotpSecretEncrypted, user.FieldSignupSource, user.FieldBalanceNotifyThresholdType, user.FieldBalanceNotifyExtraEmails:
...
func (_m *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldFrozenBonusBalance:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field frozen_bonus_balance", values[i])
			} else if value.Valid {
				_m.FrozenBonusBalance = value.Float64
			}
