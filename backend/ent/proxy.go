...
func (*Proxy) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case proxy.FieldPort, proxy.FieldExpiryWarnDays:
			values[i] = new(sql.NullInt64)
		case proxy.FieldID, proxy.FieldName, proxy.FieldProtocol, proxy.FieldHost, proxy.FieldUsername, proxy.FieldPassword, proxy.FieldStatus, proxy.FieldFallbackMode, proxy.FieldBackupProxyID:
	...
	for i := range columns {
		...
		case proxy.FieldBackupProxyID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.BackupProxyID = new(string)
				*_m.BackupProxyID = value.String
			}
