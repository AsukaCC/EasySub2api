...
func (Announcement) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("notify_mode").
			...
			Comment("通知模式: silent(仪表盘), popup(弹窗提醒), banner(顶部滚动)"),
		...
		field.String("created_by").SchemaType(postgresUUIDSchema).
			Optional().
			Nillable().
			Comment("创建人用户ID（管理员）"),
		field.String("updated_by").SchemaType(postgresUUIDSchema).
			Optional().
			Nillable().
			Comment("更新人用户ID（管理员）"),
