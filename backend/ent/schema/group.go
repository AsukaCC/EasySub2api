...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("daily_limit_usd").
			...
			Comment("Daily platform point limit; column name is retained for compatibility"),
		field.Float("weekly_limit_usd").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,8)"}).
			Comment("Weekly platform point limit; column name is retained for compatibility"),
		field.Float("monthly_limit_usd").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,8)"}).
			Comment("Monthly platform point limit; column name is retained for compatibility"),
		...
		field.String("fallback_group_id").SchemaType(postgresUUIDSchema).
			Optional().
			Nillable().
			Comment("非 Claude Code 请求降级使用的分组 ID"),
		field.String("fallback_group_id_on_invalid_request").SchemaType(postgresUUIDSchema).
			Optional().
			Nillable().
			Comment("无效请求兜底使用的分组 ID"),

		// 模型路由配置 (added by migration 040)
		field.JSON("model_routing", map[string][]string{}).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"}).
		...
		field.Bool("free_openai_fast").
			Default(false).
			Comment("是否让此 OpenAI/Composite 分组的 Fast 请求按 Standard 价格向用户计费"),
