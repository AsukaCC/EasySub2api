...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("proxy_id").SchemaType(postgresUUIDSchema).
			Optional().
			Nillable(),
		field.String("proxy_fallback_origin_id").SchemaType(postgresUUIDSchema).
			Optional().Nillable().
		...
		field.String("subscription_tier").MaxLen(100).Optional().Nillable().
			Comment("Normalized subscription tier used by admin filtering and billing."),
