...
type Account struct {
	...
	ProxyFallbackOriginID *string `json:"proxy_fallback_origin_id,omitempty"`
	...
	for i := range columns {
		...
		case account.FieldProxyFallbackOriginID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.ProxyFallbackOriginID = new(string)
				*_m.ProxyFallbackOriginID = value.String
			}
